...
取得 Hadoop、其生態系統以及 Azure HDInsight 中的巨量資料的簡介：HDInsight 中的 Hadoop 是什麼、Hadoop 元件有哪些、常見術語及巨量資料分析的案例。同時深入了解使用 HDInsight 中的 Hadoop 的文件、教學課程及資源。
...
* Azure HDInsight 會使用 **Linux** 或 **Windows** 做為基礎作業系統，在雲端上部署或佈建 Hadoop 叢集。
	...
	* **Windows 上的 HDInsight** - Windows Server 上的 Hadoop 叢集。如果您熟悉 Windows、想要從現有以 Windows 為基礎的 Hadoop 方案移轉，或想要與 .NET 或其他 Windows 功能整合，則使用此選項。
	...
	類別 | Linux 上的 HDInsight | Windows 上的 HDInsight 
	---------| -------------------| --------------------
	**叢集作業系統** | Ubuntu 12.04 長期支援 (LTS) | Windows Server 2012 R2
	**叢集類型** | Hadoop | Hadoop、HBase、Storm
	**部署** | Azure 管理入口網站、跨平台命令列、Azure PowerShell | Azure 管理入口網站、跨平台命令列、Azure PowerShell、HDInsight .NET SDK
	**叢集 UI** | Ambari | 叢集儀表板
	**遠端存取** | 安全殼層 (SSH) | 遠端桌面通訊協定 (RDP)
	

* HDInsight 使用 **HortonWorks Data Platform (HDP)** Hadoop 散發。

* Apache Hadoop 是巨量資料管理和分析的軟體架構。HDInsight 針對特定工作負載提供數個組態，或者您可以<a href="http://azure.microsoft.com/documentation/articles/hdinsight-hadoop-customize-cluster/" target="_blank">使用指令碼動作自訂叢集</a>。
...
* 最先進的 Hadoop 元件。如需詳細資訊，請參閱 
* HDInsight 所提供 Hadoop 叢集版本的新功能 [component-versioning]。

* 叢集的高可用性和可靠性。如需詳細資訊，請參閱＜[HDInsight 上 Hadoop 叢集的可用性和可靠性](hdinsight-high-availability.md)＞。

* 有效率又符合經濟效益的資料存放區，採用 Azure Blob 儲存體，是一個 Hadoop 相容選項。如需詳細資訊，請參閱＜[在 HDInsight 上將 Azure Blob 儲存體與 Hadoop 搭配使用](../hdinsight-use-blob-storage.md)＞。
...
* [開始使用 Linux 上的 HDInsight](../hdinsight-hadoop-linux-get-started.md) - 在 Linux 上佈建 HDInsight Hadoop 叢集以及執行範例 Hive 查詢的快速入門教學課程。

* [使用自訂選項在 Linux 上佈建 HDInsight](hdinsight-hadoop-provision-linux-clusters.md) - 深入了解如何使用自訂選項，透過 Azure 管理入口網站、Azure 跨平台命令列或 Azure PowerShell 佈建 HDInsight Hadoop 叢集。
...
* <a target="_blank" https://msdn.microsoft.com/library/hh231701.aspx">深入了解 SQL Server Analysis Services</a>。
