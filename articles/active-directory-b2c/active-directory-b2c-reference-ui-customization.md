...
本節中您會發現 Azure AD B2C 合併至 `<div id="api"></div>` 元素 (位於您的內容中) 的 HTML5 片段 (適用於每種頁面)。您可以使用樣式表來自訂這些 UI 元素。這些樣式表可以覆寫我們在 `<head>` 片段中加入至這些頁面類型的預設樣式表。

> [AZURE.IMPORTANT]在預覽期間，確切的 UI 元素應會隨著我們了解您的意見反應並據以調整而改變。檢查預設頁面的原始程式碼，以取得下列各項的詳細資訊。
