...
6. 在 [允許的 JavaScript 原始來源] 欄位中輸入 [https://login.microsoftonline.com](https://login.microsoftonline.com)，並在 [允許的傳回 URL] 欄位中輸入 [https://login.microsoftonline.com/te/{directory}/oauth2/authresp](https://login.microsoftonline.com/te/{directory}/oauth2/authresp)，其中 **{directory}** 會取代為您目錄的名稱 (例如 contoso.onmicrosoft.com)。按一下 [儲存]。

    > [AZURE.NOTE]**{directory}** 值區分大小寫。
