...
對我們在 GitHub 上的範例進行貢獻：[Microsoft Azure Active Directory 範例與文件](https://github.com/AzureADSamples)。
...
| C#/.NET | [WebApp-WSFederation-DotNet](https://github.com/AzureADSamples/WebApp-WSFederation-DotNet) | 使用 WS-同盟 (ASP.Net WS-同盟 OWIN 中介軟體) 驗證來自 Azure AD 租用戶的使用者。
...
| C#/.NET | [WebAPI-ManuallyValidateJwt-DotNet](https://github.com/AzureADSamples/WebAPI-ManuallyValidateJwt-DotNet) | 原生用戶端應用程式，其包含邏輯以處理 Web API 中的 JWT Token，而不使用 OWIN 中介軟體。
...
| C#/Xamarin | [NativeClient-Xamarin-iOS](http://github.com/AzureADSamples/NativeClient-Xamarin-iOS) | iOS 適用之原生 Azure AD 驗證程式庫 (ADAL) 的 Xamarin 繫結。
...
| JavaScript | [WebAPI-Nodejs](http://github.com/AzureADSamples/WebAPI-Nodejs) | 設定已與 Azure AD 整合獲得 API 保護的 REST API 服務。包含 Node.js 伺服器與 Web API。
...
這些程式碼範例示範如何建置精靈或伺服器應用程式，使用 [Azure AD 驗證程式庫 (ADAL)](http://go.microsoft.com/fwlink/?LinkID=258232) 和 [Azure AD 中的 OAuth 2.0](https://msdn.microsoft.com/library/azure/dn645545.aspx) 從 Web API 取得資源。
...
| 語言/平台 | 範例 | 描述
| ----------------- | ------ | -----------
| Java | [WebApp-GraphAPI-Java](http://github.com/AzureADSamples/WebApp-GraphAPI-Java) | Web 應用程式，其使用 Graph API 來存取 Azure AD 目錄資料。
| PHP | [WebApp-GraphAPI-PHP](http://github.com/AzureADSamples/WebApp-GraphAPI-PHP) | Web 應用程式，其使用 Graph API 來存取 Azure AD 目錄資料。
| C#/.NET | [WebApp-GraphAPI-DotNet](http://github.com/AzureADSamples/WebApp-GraphAPI-DotNet) | Web 應用程式，其使用 Graph API 來存取 Azure AD 目錄資料。
...
| PHP | [WebApp-GraphAPI-DirectoryExtensions-PHP](https://github.com/AzureADSamples/WebApp-GraphAPI-DirectoryExtensions-PHP) | PHP 應用程式，其呼叫 Graph API 來登錄延伸模組，然後讀取、更新和刪除延伸模組屬性中的值。
...
| 語言/平台 | 範例 | 描述
| ----------------- | ------ | -----------
| C#/.NET | [WebApp-GroupClaims-DotNet](https://github.com/AzureADSamples/WebApp-GroupClaims-DotNet) | 在與 Azure AD 整合的應用程式中使用 Azure Active Directory 群組宣告執行角色型存取控制 (RBAC)。
| C#/.NET | [WebApp-RoleClaims-DotNet](https://github.com/AzureADSamples/WebApp-RoleClaims-DotNet) | 在與 Azure AD 整合的應用程式中使用 Azure Active Directory 應用程式角色執行角色型存取控制 (RBAC)。
...
| 語言/平台 | 範例 | 描述
| ----------------- | ------ | -----------
| C#/.NET | [Windows Azure AD 應用程式中的角色式和 ACL 式授權](http://go.microsoft.com/fwlink/?LinkId=331694) | 在與 Azure AD 整合的應用程式中執行角色式授權 (RBAC) 和 ACL 式授權。
| C#/.NET |  [AAL - Windows 市集應用程式到 REST 服務 - 驗證](http://go.microsoft.com/fwlink/?LinkId=330605) |使用 Windows 市集 Beta 適用的 [Azure AD 驗證程式庫 (ADAL)](http://go.microsoft.com/fwlink/?LinkID=258232) (先前稱為 AAL)，將使用者驗證功能新增至 Windows 市集應用程式。
| C#/.NET | [ADAL - 原生應用程式到 REST 服務 - 透過瀏覽器對話方塊對 AAD 驗證](http://go.microsoft.com/fwlink/?LinkId=259814) | 使用 [Azure AD 驗證程式庫 (ADAL)](http://go.microsoft.com/fwlink/?LinkID=258232)，將使用者驗證功能新增至 WPF 用戶端。
| C#/.NET | [ADAL - 原生應用程式到 REST 服務 - 透過瀏覽器對話方塊對 ACS 驗證](http://code.msdn.microsoft.com/AAL-Native-App-to-REST-de57f2cc) | 使用 [Azure AD 驗證程式庫 (ADAL)](http://go.microsoft.com/fwlink/?LinkID=258232) 和[存取控制服務 2.0 (ACS)](http://msdn.microsoft.com/library/azure/hh147631.aspx)，將使用者驗證功能新增至 WPF 用戶端。
...
| C#/.NET | [使用 Azure AD 新增登入至 Web 應用程式](https://msdn.microsoft.com/library/azure/dn151790.aspx) |設定 .NET 應用程式對您的 Azure AD 企業目錄執行 Web 單一登入。
| C#/.NET | [開發多租用戶 Web 應用程式與 Azure AD](https://msdn.microsoft.com/library/azure/dn151789.aspx) | 使用 Azure AD 新增某個 .NET 應用程式的單一登入和目錄存取功能，以跨多個組織運作。
...
| C#/.NET | [Azure AD Graph 差異查詢的範例應用程式](http://go.microsoft.com/fwlink/?LinkId=275398) | 使用 Graph API 中的差異查詢來取得 Azure AD 租用戶中使用者物件的定期變更。
| C#/.NET | [對 Azure AD 整合多租用戶雲端應用程式的範例應用程式](http://go.microsoft.com/fwlink/?LinkId=275397) | 將多租用戶應用程式整合到 Azure AD。
| C#/.NET | [使用 Azure AD (預覽版) 保護 Windows 市集應用程式和 REST Web 服務](https://msdn.microsoft.com/library/azure/dn169448.aspx) | 使用 Azure AD 和 [Azure AD 驗證程式庫 (ADAL)](http://go.microsoft.com/fwlink/?LinkID=258232) 建立簡單 Web API 資源和 Windows 市集用戶端應用程式。
