...
## 概觀
Azure 基礎結構服務可讓您以敏捷的方式部署各種不同的運算解決方案。利用 Azure 虛擬機器，您幾乎可以進行即時部署，而且只需要以分鐘計價。利用對 Windows、Linux、SQL Server、Oracle、IBM、SAP 和 BizTalk 的支援，您幾乎可以在所有作業系統上使用任何語言部署所有工作負載。這些優點可讓您將內部部署的舊版應用程式移轉至 Azure，以便獲得營運費用的節約。

將內部部署應用程式移轉至 Azure 的一個重要層面是處理這些應用程式的身分識別需求。目錄感知應用程式可能會依賴 LDAP 對公司目錄讀取或寫入存取，或仰賴 Windows 整合式驗證 (Kerberos 或 NTLM 驗證) 來驗證使用者。在 Windows Server 上執行的企業營運應用程式通常會部署在加入網域的機器上，以安全地使用群組原則來管理。為了要將內部部署應用程式「隨即轉移」(lift-and-shift) 至雲端，必須解決企業身分識別基礎結構上的這些相依性。

為了滿足他們部署在 Azure 中的應用程式的身分識別需求，系統管理員經常轉向下列其中一個解決方案：

- 在 Azure 基礎結構服務和企業目錄內部部署之間執行的工作負載之間部署站對站 VPN 連線。
- 使用 Azure 虛擬機器設定複本網域控制站，來延伸公司的 AD 網域/樹系基礎結構。
- 使用部署為 Azure 虛擬機器的網域控制站在 Azure 中部署獨立網域。

所有這些方法都面臨高成本和管理負擔。系統管理員需要在 Azure 中使用虛擬機器才能部署網域控制站。他們需要後續管理、保護、修補、監視、備份和疑難排解這些虛擬機器。對使用 VPN 連線到內部部署目錄的依賴度，會導致 Azure 中部署的工作負載易於造成暫時性網路問題或中斷。這會導致較低的執行時間，並降低這些應用程式的可靠性。
...
## 案例和使用案例
在本節中，我們看看一些案例和受益於 Azure AD 網域服務的使用案例。
...
-	**簡單** - 您可以滿足部署到 Azure 基礎結構服務的虛擬機器的身分識別 - 只需求簡單按幾下滑鼠，而不需在 Azure 中部署和管理身分識別基礎結構，或將連線設定回您的內部部署身分識別基礎結構。

-	**整合式** - Azure AD 網域服務與 Azure AD 租用戶密切整合。您現在可以依賴 Azure AD，讓它成為能夠同時迎合現代化應用程式及傳統目錄感知應用程式需求的整合雲端式企業目錄。

-	**相容** - 因為 Azure AD 網域服務已內建在 Windows Server Active Directory 經實證的企業級基礎結構上，您的應用程式可以依賴與 Windows Server Active Directory 功能更高程度的相容性。請注意，並非 Windows Server AD 中的所有功能目前都可在 Azure AD 網域服務中使用。不過，可用的功能與您在內部部署基礎結構中依賴的相對應 Windows Server AD 功能相容。LDAP、Kerberos、NTLM、群組原則和 Azure AD 網域服務所提供的網域加入功能構成了成熟的產品服務，已對各種 Windows Server 版本經過測試並進一步修改。

-	**符合成本效益** - 有了 Azure AD 網域服務，您可以避免與管理身分識別基礎結構來支援傳統目錄感知應用程式相關聯的基礎結構和管理負擔。您可以將這些應用程式移至 Azure 基礎結構服務，並受益於營運費用的大幅節約。
