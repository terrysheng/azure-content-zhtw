...
<h2><span  class="short-header">目錄</span>目錄</h2>
...
* [檢視 ACS 所傳送的宣告](#bkmk_viewClaims)
* [在 ACS 管理入口網站中檢視應用程式](#bkmk_VP)
* [新增身分識別提供者](#add-IP)
* [接下來](#whats-next)

<h2><span  class="short-header">什麼是 ACS？</span>什麼是 ACS？</h2>
...
如需 ACS 的詳細資訊，請參閱[存取控制服務 2.0][1] (英文)。
...
<h2><span  class="short-header">先決條件</span>先決條件</h2>
...
* Visual Studio 2012 的 Identity and Access Tool (若要下載，請參閱 [Identity and Access Tool][2] (英文))

<h2><span  class="short-header">建立存取控制命名空間</span>建立存取控制命名空間</h2>


若要使用 Azure 中的 Active Directory 存取控制，請建立存取控制命名空間。此命名空間可提供在您的應用程式內定址 ACS 資源的唯一範圍。

1.  登入 [Azure 管理入口網站][3] (https://manage.WindowsAzure.com)。
...
2.  按一下 **檔案**，然後按一下 **新增專案**。 3.  選取 Visual C#/Web 範本，然後選取 **ASP.NET MVC 4 Web 應用程式**。
    
    在本指南中，我們將使用 MVC 應用程式，但是您可以使用任何類型的 Web 應用程式進行此工作。
    
    ![](./media/active-directory-dotnet-how-to-use-access-control/rzMvc.png)

4.  在 **名稱** 中，輸入 **MvcACS**，然後按一下 **確定**。
5.  在下一個對話方塊中，選取 **網際網路應用程式**，然後按一下 **確定**。
6.  編輯 *Views\\Shared\_LoginPartial.cshtml* 檔案，並將其內容取代為下列程式碼：
...
<h2><span  class="short-header">整合您的 Web 應用程式與 ACS</span>整合您的 Web 應用程式與 ACS</h2>
...
1.  在 **方案總管] 中以滑鼠右鍵按一下 MvcACS 專案，然後選取 [Identity and Access**。
    ...
    如果 **Identity and Access** 選項未出現在內容功能表上，請安裝 Identity and Access Tool。如需詳細資訊，請參閱[身分識別與存取工具][2] (英文)。
...
<h2><span  class="short-header">測試與 ACS 的整合</span>測試與 ACS 的整合</h2>
...
<h2><a name="bkmk_viewClaims"></a>檢視 ACS 所傳送的宣告</h2>
...
2. 在 HomeController 類別中，新增 *Claims* 方法：
...
2.  在 *Claims* 方法上按一下滑鼠右鍵並選取 **加入檢視**。

	![](./media/active-directory-dotnet-how-to-use-access-control/rzAv.png)

1.  按一下 **新增**。

2.  以下列程式碼取代 *Views\\Home\\Claims.cshtml* 檔案的內容：
...
3.  執行應用程式並瀏覽到 *Claims* 方法：

![](./media/active-directory-dotnet-how-to-use-access-control/rzCl.png)

如需關於在應用程式中使用宣告的詳細資訊，請參閱 [Windows Identity Foundation 文件庫 (機器翻譯)][4]。

<h2><a name="bkmk_VP"></a>在 ACS 管理入口網站中檢視應用程式</h2>
...
<h2><span  class="short-header">新增身分識別提供者</span>新增身分識別提供者</h2>


讓我們使用 ACS 管理入口網站來變更 MvcACS 應用程式的驗證。在此範例中，我們將新增 Google 作為 MvcACS
的身分識別提供者。
...
<h2><span  class="short-header">接下來</span>接下來</h2>
...
若要進一步探索 ACS 功能及試試其他案例，請參閱[存取控制服務 2.0][1] (英文)。
