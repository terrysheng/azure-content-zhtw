...
本教學課程將對 PHP 開發人員顯示如何運用 Azure Active Directory，對 Office 365 客戶的使用者啟用單一登入。您將了解作法：
...
1. 開啟新的 Eclipse 執行個體。
2. 從 [**檔案**] 功能表中，按一下 [**新增**]，然後按一下 [**新增 PHP 專案**]。 
3. 在 [**新增 PHP 專案**] 專案上，將專案命名為 *phpSample*，然後按一下 [**完成**]。
4. 從左邊的 [**PHP 總管**] 功能表中，在  *phpProject* 上按一下滑鼠右鍵、按一下 [**新增**]，然後按一下 [**PHP File**]。
5. 在 [**新增 PHP 檔案**] 對話方塊上，將檔案命名為 **index.php**，然後按一下 [**完成**]。
...
11. 在 [**執行 PHP Web 應用程式**] 功能表上，按一下 [**確定**]。
...
1. 下載並安裝 Office 365 PowerShell Cmdlet (如果尚未這樣做)。
2. 從 [**開始**] 功能表中，執行 **Windows PowerShell 適用的 Azure Active Directory 模組**主控台。此主控台提供命令行環境，用於設定 Office 365 租用戶的相關屬性，例如建立和修改服務主體。
...
4. 在 Eclipse 中，於 **phpSample** 專案上按一下滑鼠右鍵，按一下 [**新增**]，然後按一下 [**PHP 檔案**]。 

5. 在 [**新增 PHP 檔案**] 對話方塊上，將檔案命名為 **secureResource.php**，然後按一下 [**完成**]。
