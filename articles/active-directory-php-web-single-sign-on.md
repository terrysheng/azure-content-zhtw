...
8.  在 IIS 管理員中，展開左功能表中 **Sites** 資料夾、在 **[預設網站首頁]** 上按一下滑鼠右鍵，然後按一下 **[新增應用程式...]**。
...
12. **index.php** 頁面將在 Eclipse 的新索引標籤中開啟。此頁面應該只顯示下列文字：*索引頁面*。
...
5.  現在您將為應用程式建立新的服務主體。輸入下列命令並按 Enter 鍵：
    ...
    > [WACOM.NOTE] 您應該儲存此輸出，尤其是產生的對稱金鑰。此金鑰只會在建立服務主體期間顯示給您，而且日後您將無法擷取它。需要其他值，才能使用圖形 API 來讀取和寫入目錄中的資訊。
