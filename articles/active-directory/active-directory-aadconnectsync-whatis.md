<properties
	pageTitle="Azure AD Connect 同步處理案例概觀"
...
Azure Active Directory Connect 同步處理服務 (Azure AD Connect 同步處理) 是 Azure AD Connect 的主要元件，負責與在您內部部署環境和雲端中 Azure AD 間同步處理身分識別資料相關的所有操作。從同步處理觀點來看，Azure AD Connect 同步處理是 DirSync、Azure AD 同步，以及已設定 Active Directory 連接器之 Forefront Identity Manager 的後續版本。
...
- 如需支援 Auzre AD Connect 同步處理的多樹系案例概觀，請參閱[多樹系案例概觀](active-directory-aadconnectsync-scenario-overview.md)。
...
- 如果想要變更預設設定，請參閱 [active-directory-aadconnectsync-best-practices-changing-default-configuration](active-directory-aadconnectsync-best-practices-changing-default-configuration.md)
- 如果想要探索同步處理至 Azure AD 的屬性，請參閱 [active-directory-aadconnectsync-attributes-synchronized](active-directory-aadconnectsync-attributes-synchronized.md)
