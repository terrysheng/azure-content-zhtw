...
1. 在管理入口網站中，按一下 [Active Directory]，然後按一下您組織目錄的名稱。
2. 按一下 [群組] 索引標籤。
3. 在 [群組] 頁面上，按一下您想要在其中新增成員之群組的名稱。根據預設，這會顯示所選群組的 [成員] 索引標籤。
4. 在該群組的頁面上，按一下 [加入成員]。
5. 在 [加入成員] 頁面上，按一下您想要新增為此群組成員之使用者或群組的名稱，並確定這個名稱新增至 [已選取] 窗格中。
...
2. 您現在可以為將控制此群組的動態成員資格如何運作的群組，設定一個簡單的單一規則。請確定已選取 [加入使用者，其中] 選項按鈕，然後從下拉式功能表中選取一個使用者屬性 (例如，department、jobTitle 等)。 
