...
內部 URL | 這是應用程式 Proxy 連接器在內部存取應用程式所使用的內部 URL。這應該是從您私人網路內部存取應用程式所使用的已發佈應用程式的 URL。這是有效的 URL，不含空格或符號。您可以指定後端伺服器上要發佈的特定路徑，而伺服器的其餘部分則不發佈。例如，這可讓您發佈位於相同的 SharePoint 伺服器，但具有不同的名稱和存取規則的不同網站。
路徑指定於內部 URL 欄位中，而且將可在外部 URL 中看到。內部路徑和外部路徑必須相同。轉譯標頭中的 URL | 針對不需要轉譯 HTTP 主機標頭的應用程式 (例如某些 SharePoint 組態)，將此設定為 [否]。這樣會同時為要求和回應標頭停用標頭轉譯。內部驗證方法 | 如果您使用應用程式 Proxy 進行預先驗證，您可以設定內部驗證方法，讓使用者可以從單一登入 (SSO) 到此應用程式受益。<p>若您的應用程式使用 IWA，則選取 [整合式 Windows 驗證 (IWA)]，且您可設定「Kerberos 限制委派」(KCD) 以啟用此應用程式的 SSO。<p>若您的應用程式不使用 IWA，請選取 [無]。<p>使用 IWA 的應用程式必須設為使用 KCD，否則「應用程式 Proxy」將無法發佈這些應用程式。<p>如需詳細資訊，請參閱 SSO for On Prem IWA Apps 的使用 KCD 與應用程式 Proxy 內部應用程式 SPN 搭配 |這是內部部署應用程式 Proxy 中設定之內部應用程式的服務主體名稱 (SPN)。應用程式 Proxy 連接器會使用 SPN 擷取使用 Kerberos 限制委派 (KCD) 的應用程式的 Kerberos 權杖。<p>如需詳細資訊，請參閱[啟用單一登入](active-directory-application-proxy-sso-using-kcd.md)
