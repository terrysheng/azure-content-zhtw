...
預先驗證方法 | <p>設定您希望應用程式使用的預先驗證方法的類型：</p><p>a.Microsoft Azure Active Directory (Microsoft Azure AD) – 當使用者嘗試存取應用程式時，應用程式 Proxy 會將使用者重新導向，以便使用將會驗證使用者的 Azure AD 登入，以確保使用者具有目錄和應用程式所需的權限。</p><p>b.通過 – 不執行預先驗證。</p>
...
內部驗證方法 | <p>如果您使用應用程式 Proxy 進行預先驗證，您可以設定內部驗證方法，讓使用者可以從單一登入 (SSO) 到此應用程式受益。</p> <p> 若您的應用程式使用 IWA，則選取 [整合式 Windows 驗證 (IWA)]，而且您可以設定「Kerberos 限制委派」(KCD) 以針對此應用程式啟用 SSO。使用 IWA 的應用程式必須設為使用 KCD，否則「應用程式 Proxy」將無法發佈這些應用程式。</p> <p>若您的應用程式不使用 IWA，請選取 [無]。</p> <p>如需詳細資訊，請參閱[透過應用程式 Proxy 單一登入](active-directory-application-proxy-sso-using-kcd.md)。</p>
