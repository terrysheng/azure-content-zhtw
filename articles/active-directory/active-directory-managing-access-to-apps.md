...
- **以群組為基礎的指派 (僅限付費 Azure AD)**：具備全域、使用者或 AU 權限的 IT 系統管理員可以將群組指派給應用程式。特定使用者的存取權決於在使用者嘗試存取應用程式時，使用者是否為群組的成員。在此模式中，系統管理員可以有效地建立指派規則，說明「獲指派群組的任何現有成員可存取應用程式」。使用這個指派選項，系統管理員可以受益於 Azure AD 群組管理選項，包括屬性式動態群組、外部系統群組 (例如：內部部署的 AD 或工作日)、系統管理員或自助管理的群組。單一群組可以輕鬆地指派給多個應用程式，確保與指派同質性的應用程式可以共用指派規則，降低整體管理複雜度。請注意目前以群組為基礎的應用程式指派不支援巢狀群組成員資格。
