...
7.  在結果窗格中，選取 [Aha!]，然後按一下 [完成] 以加入應用程式。
...
1.  在 Azure AD 入口網站的 [Aha!] 應用程式整合頁面上，按一下 [設定單一登入] 來開啟 [設定單一登入] 對話方塊。
...
4.  於 [在 Aha! 設定單一登入] 頁面上，按一下 [下載中繼資料] 以下載您的中繼資料檔，然後將中繼資料檔儲存在您的本機電腦中。
