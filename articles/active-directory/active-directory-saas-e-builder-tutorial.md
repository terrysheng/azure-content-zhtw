...
完成本教學課程之後，您指派給 E-Builder 的 Azure AD 使用者就能夠單一登入您 e-Builder 公司網站 (服務提供者起始登入) 的應用程式，或是使用[存取面板簡介](active-directory-saas-access-panel-introduction.md)進行單一登入。
...
7.  在結果窗格中，選取 [e-Builder]，然後按一下 [完成] 以加入應用程式。
...
3.  在 [設定應用程式 URL] 頁面的 [e-Builder 登入 URL] 文字方塊中，使用下列模式輸入您的 URL："*https://\<租用戶名稱>.e-Builder.com*"，然後按 [下一步]。

    ![設定應用程式 URL](./media/active-directory-saas-e-builder-tutorial/IC777383.png "設定應用程式 URL")

4.  於 [在 E-Builder 設定單一登入] 頁面上，按一下 [下載中繼資料] 以下載您的中繼資料，然後在本機將資料檔另存為 **c:\\E-BuilderMetaData.xml**。
