...
完成本教學課程或使用[存取面板簡介](active-directory-saas-access-panel-introduction.md)之後，您指派給 Egnyte 的 Azure AD 使用者就能夠單一登入您 Egnyte 公司網站 (服務提供者起始登入) 的應用程式。
...
7.  在結果窗格中，選取 [Egnyte]，然後按一下 [完成] 來新增應用程式。
...
1.  在 Azure AD 入口網站的 **Egnyte** 應用程式整合頁面中，按一下 [設定單一登入] 來開啟 [設定單一登入] 對話方塊。

    ![設定單一登入](./media/active-directory-saas-egnyte-tutorial/IC787815.png "設定單一登入")

2.  在 [要如何讓使用者登入 Egnyte] 頁面上，選取 [Microsoft Azure AD 單一登入]，然後按 [下一步]。
...
9.  在 [單一登入驗證] 區段中，執行下列步驟：
    ...
    2.  針對 [身分識別提供者]，選取 [AzureAD]。
    3.  在 Azure 入口網站的 [設定在 Egnyte 單一登入] 對話方塊頁面中，複製**遠端登入 URL** 值，然後將它貼至 [身分識別提供者登入 URL] 文字方塊中。
4.  在 Azure 入口網站的 [設定在 Egnyte 單一登入] 對話方塊頁面中，複製**實體身分識別碼**值，然後將其貼至 [身分識別提供者實體身分識別碼] 文字方塊中。
    5.  從您下載的憑證建立「Base-64 編碼」檔案。  

        >[AZURE.TIP]如需詳細資訊，請參閱[如何將二進位憑證轉換成文字檔](http://youtu.be/PlgrzUZ-Y1o)

    6.  在記事本中開啟您的 Base-64 編碼的憑證，將其內容複製到剪貼簿，然後貼到 [識別提供者憑證] 文字方塊。
