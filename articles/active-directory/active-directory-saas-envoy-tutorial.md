...
完成本教學課程之後，您指派給 Envoy 的 Azure AD 使用者就能夠單一登入您 Envoy 公司網站 (服務提供者起始登入) 的應用程式，或是使用[存取面板簡介](active-directory-saas-access-panel-introduction.md)進行單一登入。
...
2.  在 [要如何讓使用者登入 Envoy] 頁面上，選取 [Microsoft Azure AD 單一登入]，然後按 [下一步]。
...
4.  於 [在 Envoy 設定單一登入] 頁面上，按一下 [下載憑證] 以下載您的憑證，然後在本機將憑證檔案另存為 **c:\\Envoy.cer**。
...
9.  在 [SAML 驗證] 組態區段中，執行下列步驟：
    ...
    1.  從匯出的憑證複製**指紋**值，然後將它貼至 [指紋] 文字方塊中。  

        >[AZURE.TIP]如需詳細資訊，請參閱[如何擷取憑證的指紋值](http://youtu.be/YKQF266SAxI)。

    2.  於 Azure 入口網站的 [在 Envoy 設定單一登入] 對話頁面上，複製 **SAML SSO URL** 值，然後將它貼至 [識別提供者 HTTP SAML URL] 文字方塊中。
    3.  按一下 [儲存變更]。
