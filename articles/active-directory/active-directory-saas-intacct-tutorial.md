...
完成本教學課程或使用[存取面板簡介](active-directory-saas-access-panel-introduction.md)之後，您指派給 Intacct 的 Azure AD 使用者就能夠單一登入您 Intacct 公司網站 (服務提供者起始登入) 的應用程式。
...
7.  在結果窗格中，選取 [Intacct]，然後按一下 [完成] 以新增應用程式。
...
1.  在 Azure AD 入口網站的 **Intacct** 應用程式整合頁面中，按一下 [設定單一登入] 來開啟 [設定單一登入] 對話方塊。

    ![設定單一登入](./media/active-directory-saas-intacct-tutorial/IC790033.png "設定單一登入")

2.  在 [要如何讓使用者登入 Intacct] 頁面上，選取 [Microsoft Azure AD 單一登入]，然後按 [下一步]。

    ![設定單一登入](./media/active-directory-saas-intacct-tutorial/IC790034.png "設定單一登入")

3.  在 [設定應用程式 URL] 頁面上，於 [Intacct 登入 URL] 文字方塊中，使用下列模式輸入您的 URL："**https://Intacct.com/company*"，然後按 [下一步]。
...
8.  在 [單一登入 (SSO)] 區段中，執行下列步驟：
    ...
    1.  選取 [啟用單一登入]。
    2.  針對 [身分識別提供者類型]，選取 [SAML 2.0]。
    3.  在 Azure 入口網站的 [設定在 Intacct 單一登入] 對話方塊頁面中，複製**簽發者 URL** 值，然後將它貼到 [簽發者] 文字方塊中。
    4.  在 Azure 入口網站的 [設定在 Intacct 單一登入] 對話方塊頁面中，複製**遠端登入 URL** 值，然後將它貼至 [登入 URL] 文字方塊中。
