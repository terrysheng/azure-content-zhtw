...
完成本教學課程或使用[存取面板簡介](active-directory-saas-access-panel-introduction.md)之後，您指派給 Mimecast Personal Portal 的 Azure AD 使用者就能夠單一登入您 Mimecast Personal Portal 公司網站 (服務提供者起始登入) 的應用程式。
...
1.  在 Azure AD 入口網站的 **Mimecast Personal Portal** 應用程式整合頁面中，按一下 [設定單一登入] 來開啟 [設定單一登入] 對話方塊。

    ![設定單一登入](./media/active-directory-saas-mimecast-personal-portal-tutorial/IC794994.png "設定單一登入")

2.  在 [要如何讓使用者登入 Mimecast Personal Portal] 頁面上，選取 [Microsoft Azure AD 單一登入]，然後按 [下一步]。

    ![設定單一登入](./media/active-directory-saas-mimecast-personal-portal-tutorial/IC794995.png "設定單一登入")

3.  在 [設定應用程式 URL] 頁面上，於 [Mimecast Personal Portal 登入 URL] 文字方塊中，輸入使用者登入您的 Mimecast Personal Portal 應用程式時所使用的 URL (例如：“https://webmail-uk.mimecast.com” 或 “https://webmail-us.mimecast.com”)，然後按 [下一步]。
...
9.  在 [驗證設定檔] 區段中，執行下列步驟：
    ...
    4.  在 Azure 入口網站的 [設定在 Mimecast Personal Portal 單一登入] 對話方塊頁面中，複製**簽發者 URL** 值，然後將它貼至 [簽發者 URL] 文字方塊中。
    5.  在 Azure 入口網站的 [設定在 Mimecast Personal Portal 單一登入] 對話方塊頁面中，複製**遠端登入 URL** 值，然後將它貼至 [登入 URL] 文字方塊中。
    6.  在 Azure 入口網站的 [設定在 Mimecast Personal Portal 單一登入] 對話方塊頁面中，複製**遠端登入 URL** 值，然後將它貼至 [登出 URL] 文字方塊中。  
    ...
    8.  在記事本中開啟您的 base-64 編碼的憑證，移除第一行 (“*--*“) 與最後一行 (“*--*“)，將它的其餘內容複製到您的剪貼簿，然後貼到 [身分識別提供者憑證 (中繼資料)] 文字方塊中。
    9.  選取 [允許單一登入]。
    10. 按一下 [儲存]。
...
1.  以系統管理員身分登入您的 **Mimecast Personal Portal**。
