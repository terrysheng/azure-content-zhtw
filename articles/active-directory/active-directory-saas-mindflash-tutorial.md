...
完成本教學課程之後，您指派給 Mindflash 的 Azure AD 使用者就能夠單一登入您 Mindflash 公司網站 (服務提供者起始登入) 的應用程式，或是使用[存取面板簡介](active-directory-saas-access-panel-introduction.md)進行單一登入。
...
3.  在 [設定應用程式 URL] 頁面的 [登入 URL] 文字方塊中，使用下列模式輸入您的 URL："**http://company.mindflash.com*"，然後按 [下一步]。

    ![設定應用程式 URL](./media/active-directory-saas-mindflash-tutorial/IC787137.png "設定應用程式 URL")

4.  於 [在 Mindflash 設定單一登入] 頁面上，按一下 [下載中繼資料]，然後將中繼資料檔儲存在您的電腦中。
...
4.  在 [新增使用者] 區段中，執行下列步驟：
    ...
    1.  在相關的文字方塊中，輸入您要佈建之有效 AAD 帳戶的 [名字]、[姓氏] 和 [電子郵件]。
    2.  按一下 [新增]。
