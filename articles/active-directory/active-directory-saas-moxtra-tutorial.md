...
1. 在 Azure AD 入口網站的 [Moxtra] 應用程式整合頁面上，按一下 [設定單一登入] 來開啟 [設定單一登入] 對話方塊。
<br><br> ![設定單一登入][6] <br>

2. 在 [您希望使用者如何登入 Moxtra] 頁面上，選取 [Azure AD 單一登入]，然後按 [下一步]。
...
1. 在左邊工具列中，按一下 [管理控制台] > [SAML 單一登入]，然後按一下 [新增]。
...
<br><br>![設定單一登入](./media/active-directory-saas-moxtra-tutorial/tutorial_moxtra_08.png) <br>
    ...
    b.在 Azure 入口網站的 [設定在 Moxtra 單一登入] 對話方塊頁面上，複製 [實體 ID] 值，然後將它貼至 [IdP 實體 ID] 文字方塊中。

    c.在 Azure 入口網站的 [設定在 Moxtra 單一登入] 對話方塊頁面上，複製 [遠端登入 URL] 值，然後將它貼至 [登入 URL] 文字方塊中。

    d.在 [AuthnContextClassRef] 文字方塊中，輸入 **urn:oasis:names:tc:SAML:2.0:ac:classes:Password**。

    e.在 Azure 入口網站的 [設定在 Moxtra 單一登入] 對話方塊頁面上，複製 [名稱識別碼格式] 值，然後將它貼至 [名稱識別碼格式] 文字方塊中。
...
6. 在 Azure AD 入口網站上，選取單一登入設定確認，然後按一下 [下一步]。
...
1. 針對下表中的每個資料列，執行下列步驟：
    ...
    | firstname | givenname |
    | lastname | surname |
    | idpid | *<來自 Azure 入口網站中 [設定在 Moxtra 單一登入] 對話方塊的 [實體 ID] 值>* |
...
### 建立 Azure AD 測試使用者
本節目標是在 Azure 入口網站中建立名為 Britta Simon 的測試使用者。<br>
...
1. 在左邊工具列中，按一下 [管理控制台] > [使用者管理]，然後按一下 [新增使用者]。
...
1. 在 Azure 入口網站上，若要開啟應用程式檢視，請在目錄檢視中，按一下頂端功能表中的 [應用程式]。
<br><br>![指派使用者][201] <br>
...
本節目標是使用存取面板來測試您的 Azure AD 單一登入設定。<br>
