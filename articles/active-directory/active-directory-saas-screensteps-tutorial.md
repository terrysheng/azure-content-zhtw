<properties 
    ...
    description="了解如何使用 ScreenSteps 搭配 Azure Active Directory 來啟用單一登入、自動化佈建和更多功能！" 
...
完成本教學課程或使用[存取面板簡介](active-directory-saas-access-panel-introduction.md)之後，您指派給 ScreenSteps 的 Azure AD 使用者就能夠單一登入您 ScreenSteps 公司網站 (服務提供者起始登入) 的應用程式。
...
7.  在結果窗格中，選取 [ScreenSteps]，然後按一下 [完成] 以新增應用程式。
...
1.  在 Azure AD 入口網站的 **ScreenSteps** 應用程式整合頁面中，按一下 [設定單一登入] 來開啟 [設定單一登入] 對話方塊。
...
3.  在 [設定應用程式 URL] 頁面上，於 [ScreenSteps 登入 URL] 文字方塊中，使用下列模式輸入您的 URL："*https://\<tenant-name>.ScreenSteps.com*"，然後按 [下一步]。
...
10. 在 [遠端驗證端點] 區段中，執行下列步驟：
    ...
    1.  在 Azure AD 入口網站的 [設定在 ScreenSteps 單一登入] 頁面中，複製 [遠端登入 URL] 值，然後將其貼至 [遠端登入 URL] 文字方塊中。
    2.  在 Azure AD 入口網站的 [設定在 ScreenSteps 單一登入] 頁面中，複製 [遠端登出 URL] 值，然後將其貼至 [遠端登出 URL] 文字方塊中。
    3.  按一下 [選擇檔案]，然後上傳下載的憑證。
    4.  按一下 [更新]。

11. 在 Azure AD 入口網站上，選取單一登入設定確認，然後按一下 [完成] 來關閉 [設定單一登入] 對話方塊。

    ![設定單一登入](./media/active-directory-saas-screensteps-tutorial/IC778542.png "設定單一登入")
##設定使用者佈建
  
若要讓 Azure AD 使用者可以登入 ScreenSteps，則必須將他們佈建到 **ScreenSteps**。**ScreenSteps** 需以手動的方式佈建。
