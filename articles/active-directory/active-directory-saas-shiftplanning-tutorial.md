<properties 
    ...
    description="了解如何使用 ShiftPlanning 搭配 Azure Active Directory 來啟用單一登入、自動化佈建和更多功能！" 
...
完成本教學課程或使用[存取面板簡介](active-directory-saas-access-panel-introduction.md)之後，您指派給 ShiftPlanning 的 Azure AD 使用者就能夠單一登入您 ShiftPlanning 公司網站 (服務提供者起始登入) 的應用程式。
...
7.  在結果窗格中，選取 [ShiftPlanning]，然後按一下 [完成] 以新增應用程式。
...
1.  在 Azure AD 入口網站的 **ShiftPlanning** 應用程式整合頁面中，按一下 [設定單一登入] 來開啟 [設定單一登入] 對話方塊。
...
3.  在 [設定應用程式 URL] 頁面上，於 [ShiftPlanning 登入 URL] 文字方塊中，使用下列模式輸入您的 URL："**https://company.shiftplanning.com/includes/saml/*"，然後按 [下一步]。
...
8.  在 [單一登入] 區段中，執行下列步驟：
    ...
    1.  選取 [已啟用 SAML]。
    2.  選取 [允許密碼登入]
    3.  在 Azure 入口網站的 [設定在 ShiftPlanning 單一登入] 對話方塊頁面中，複製**遠端登入 URL** 值，然後將其貼至 [SAML 簽發者 URL] 文字方塊中。
    4.  在 Azure 入口網站的 [設定在 ShiftPlanning 單一登入] 對話方塊頁面中，複製**遠端登出 URL** 值，然後將其貼至 [遠端登出 URL] 文字方塊中。
