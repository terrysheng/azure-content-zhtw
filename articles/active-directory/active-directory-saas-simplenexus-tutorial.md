<properties 
    ...
    description="了解如何使用 SimpleNexus 搭配 Azure Active Directory 來啟用單一登入、自動化佈建和更多功能！" 
...
完成本教學課程之後，您指派給 SimpleNexus 的 Azure AD 使用者就能夠單一登入您 SimpleNexus 公司網站 (服務提供者起始登入) 的應用程式，或是使用[存取面板簡介](active-directory-saas-access-panel-introduction.md)進行單一登入。
...
7.  在結果窗格中，選取 [SimpleNexus]，然後按一下 [完成] 以加入應用程式。
...
1.  在 Azure AD 入口網站的 [SimpleNexus] 應用程式整合頁面上，按一下 [設定單一登入] 來開啟 [設定單一登入] 對話方塊。
...
4.  於 [在 SimpleNexus 設定單一登入] 頁面上，按一下 [下載中繼資料]，然後將中繼資料檔轉寄給 SimpleNexus 支援小組。
