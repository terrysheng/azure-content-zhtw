<properties 
    ...
    description="了解如何使用 SugarCRM 搭配 Azure Active Directory 來啟用單一登入、自動化佈建和更多功能！" 
...
完成本教學課程或使用[存取面板簡介](active-directory-saas-access-panel-introduction.md)之後，您指派給 Sugar CRM 的 Azure AD 使用者就能夠單一登入您 Sugar CRM 公司網站 (服務提供者起始登入) 的應用程式。
...
7.  在結果窗格中，選取 [Sugar CRM]，然後按一下 [完成] 新增應用程式。
...
1.  在 Azure AD 入口網站的 **Sugar CRM** 應用程式整合頁面中，按一下 [設定單一登入] 開啟 [設定單一登入] 對話方塊。
...
3.  在 [設定應用程式 URL] 頁面上，於 [Sugar CRM 登入 URL] 文字方塊中，輸入使用者用來登入 Sugar CRM 應用程式的 URL (例如："**http://company.sugarondemand.com*")，然後按 [下一步]。
...
9.  在 [SAML 驗證] 區段中，執行下列步驟：
    ...
    1.  在 Azure 入口網站的 [設定在 Sugar CRM 單一登入] 對話方塊頁面中，複製**遠端登入 URL** 值，然後將它貼至 [登入 URL] 文字方塊中。
    2.  在 Azure 入口網站的 [設定在 Sugar CRM 單一登入] 對話方塊頁面中，複製**遠端登入 URL** 值，然後將它貼至 [SLO URL] 文字方塊中。
    3.  從您下載的憑證建立「Base-64 編碼」檔案。

        >[AZURE.TIP]如需詳細資訊，請參閱[如何將二進位憑證轉換成文字檔](http://youtu.be/PlgrzUZ-Y1o)

    4.  在記事本中開啟您的 base-64 編碼的憑證，將它的內容複製到您的剪貼簿，然後將整個憑證貼至 [X.509 憑證] 文字方塊。
    5.  按一下 [儲存]。

10. 在 Azure 入口網站的 [設定在 Sugar CRM 單一登入] 對話方塊上，選取 [單一登入組態確認]，然後按一下 [完成]。
