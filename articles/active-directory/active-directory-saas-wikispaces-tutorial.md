<properties 
    ...
    description="了解如何使用 Wikispaces 搭配 Azure Active Directory 來啟用單一登入、自動化佈建和更多功能！" 
...
完成本教學課程之後，您指派給 Wikispaces 的 Azure AD 使用者就能夠單一登入您 Wikispaces 公司網站 (服務提供者起始登入) 的應用程式，或是使用[存取面板簡介](active-directory-saas-access-panel-introduction.md)進行單一登入。
...
2.  在 [要如何讓使用者登入 Wikispaces] 頁面上，選取 [Microsoft Azure AD 單一登入]，然後按 [下一步]。
...
4.  於 [在 Wikispaces 設定單一登入] 頁面上，按一下 [下載中繼資料]，然後將中繼資料檔儲存在您的電腦中。
...
4.  在 [邀請人員] 區段中，執行下列步驟：
    ...
    1.  在相關的文字方塊中，輸入您要佈建之有效 AAD 帳戶的 [使用者名稱] 或 [電子郵件地址]。
    2.  按一下 [**傳送**]。  
