...
1.  在 Azure AD 入口網站的 **Zscaler** 應用程式整合頁面中，按一下 [設定單一登入] 來開啟 [設定單一登入] 對話方塊。

    ![啟用單一登入](./media/active-directory-saas-zscaler-tutorial/IC769229.png "啟用單一登入")

2.  在 [要如何讓使用者登入 Zscaler] 頁面上，選取 [Microsoft Azure AD 單一登入]，然後按 [下一步]。

    ![設定單一登入](./media/active-directory-saas-zscaler-tutorial/IC769230.png "設定單一登入")

3.  在 [設定應用程式 URL] 頁面上，於 [Zscaler 登入 URL] 文字方塊中，輸入您向 Zscaler 取得的登入 URL，然後按 [下一步]：
...
5.  在 [Proxy 伺服器] 區段中，執行下列步驟：
    ...
    2.  在 [位址] 文字方塊中，輸入 **gateway.zscalertwo.net**。
    3.  在 [連接埠] 文字方塊中，輸入 **80**。
