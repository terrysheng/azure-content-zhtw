...
完成本教學課程或使用[存取面板簡介](active-directory-saas-access-panel-introduction.md)之後，您指派給 ZScaler Two 的 Azure AD 使用者就能夠單一登入您 ZScaler Two 公司網站 (服務提供者起始登入) 的應用程式
...
1.  在 Azure AD 入口網站的 **ZScaler Two** 應用程式整合頁面中，按一下 [設定單一登入] 來開啟 [設定單一登入] 對話方塊。

    ![設定單一登入](./media/active-directory-saas-zscaler-two-tutorial/IC800202.png "設定單一登入")

2.  在 [要如何讓使用者登入 ZScaler Two] 頁面上，選取 [Microsoft Azure AD 單一登入]，然後按 [下一步]。

    ![設定單一登入](./media/active-directory-saas-zscaler-two-tutorial/IC800203.png "設定單一登入")

3.  在 [設定應用程式 URL] 頁面上，於 [ZScaler Two 登入 URL] 文字方塊中，輸入使用者登入 ZScaler Two 應用程式所使用的 URL，然後按 [下一步]。
...
9.  在 [設定 SAML 單一登入參數] 對話方塊頁面上，執行下列步驟，然後按一下 [完成]：
    ...
    1.  在 Azure 入口網站的 [設定在 ZScaler Two 單一登入] 對話方塊頁面中，複製**驗證要求 URL** 值，接著將它貼入 [傳送使用者以供驗證的 SAML 入口網站 URL] 文字方塊中。
...
11. 在 Azure 入口網站的 [設定在 ZScaler Two 單一登入] 對話方塊上，選取單一登入組態確認，然後按一下 [完成]。
