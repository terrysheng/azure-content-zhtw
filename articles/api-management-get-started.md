...
> **Add Representation** 可用來設定多個表示式中的回應。如需詳細資訊，請參閱[回應][]。
...
> 本教學課程步驟會使用「入門」產品，這是經過預先設定而立即可用的產品。如需建立和發行新產品的逐步指南，請參閱[如何建立和發行產品][]。
...
> 在此範例中，我們要為名為 Clayton Gragg 的開發人員進行訂閱。如果您尚未建立任何開發人員帳戶，則可為管理員帳戶進行訂閱。如需建立開發人員帳戶的詳細資訊，請參閱[如何在 Azure API 管理中管理開發人員帳戶][]。
...
-   在[開始使用進階 API 組態][]教學課程中查看其他主題。
  ...
  [檢視分析]: #view-analytics
  [後續步驟]: #next-steps
  [Azure 免費試用]: http://azure.microsoft.com/en-us/pricing/free-trial/?WT.mc_id=api_management_hero_a
