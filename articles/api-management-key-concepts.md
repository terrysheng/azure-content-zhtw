...
「API 管理」可協助組織將 API 發行給外部、合作夥伴及內部開發人員，以發揮其資料與服務的潛力。各地的公司都想要將其作業延伸為數位平台、建立新的管道、尋找新客戶並對現有客戶促進更深入的參與。「API 管理」提供的核心專長認證，透過開發人員參與、商務洞察力、分析、安全性和保護，可確保 API 程式獲致成功。
...
如需詳細資訊，請參閱[如何建立 API][] 和[如何將操作加入至 API][]。
...
-	**管理員** - 管理員可以管理「API 管理」服務執行個體、建立開發人員所使用的 API、作業及產品。
-	**開發人員** - 開發人員是使用您的 API 建置應用程式的客戶。開發人員會獲授與[開發人員入口網站][]的存取權，然後建置呼叫 API 作業的應用程式。
-	**來賓** - 造訪「API 管理」執行個體之開發人員入口網站的未經驗證使用者 (例如潛在客戶) 即歸類到這個群組。他們可獲得特定唯讀存取權限，例如他們可檢視 API 但無法進行呼叫。
...
如需詳細資訊，請參閱如[如何建立或邀請開發人員][]和[何將群組與開發人員建立關聯][]。
...
如需「API 管理」原則的完整清單，請參閱[原則參考文件][]。如需有關使用和設定原則的詳細資訊，請參閱 [API 管理原則][]。如需有關如何建立具有費率限制和配額原則之產品的教學課程，請參閱[如何建立和設定進階產品設定][]。
...
[建立 API 管理執行個體]: ../api-management-get-started/#create-service-instance
