...
除了這些系統群組以外，管理員還可以建立自訂群組，或使用 [使用 Azure Active Directory 相關租用戶中的外部群組](api-management-howto-aad.md/#how-to-add-an-external-azure-active-directory-group)。讓開發人員看見及存取 API 產品時，可將自訂群組和外部群組與系統群組一起使用。例如，您可以針對隸屬於特定合作夥伴組織的開發人員，建立一個自訂群組，並允許他們只能在含有相關 API 的產品中存取 API。使用者可以是多個群組的成員。
