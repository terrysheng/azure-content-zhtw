...
2. 確認您收到登入/註冊的要求：**operation** 查詢參數會設為 "**SignIn**"。
...
5. 將使用者重新導向回到收到要求時所提供的 **returnUrl**。

## <a name="delegate-example-code"> </a> 範例程式碼 ##

這些程式碼範例顯示如何取得*委派驗證金鑰* (在 API 管理入口網站的 [委派] 畫面中設定)，以建立隨後用來驗證簽章的 HMAC，藉此證明所傳遞的 returnUrl 有效。相同的程式碼稍微修改一下後，也適用於 productId 和 userId。

* *用來產生 returnUrl 雜湊的 C# 程式碼* *
