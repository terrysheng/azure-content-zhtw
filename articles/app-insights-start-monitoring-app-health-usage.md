...
您需要 [Microsoft Azure][Microsoft Azure] 的帳戶 (有免費的試用期)。

開始使用 Application Insights 的方法有兩種：

-   (建議) [將 Application Insights 新增至 Visual Studio 專案][將 Application Insights 新增至 Visual Studio 專案]，以便監視應用程式效能和流量。
-   [在不重新部署的情況下將代理程式安裝在伺服器上][在不重新部署的情況下將代理程式安裝在伺服器上] - 不需要重新部署即時網站或修改原始程式碼即可監視即時網站。這種方法能讓您監視效能和例外狀況。您可以在之後新增流量監視功能。

> [WACOM.NOTE] Visual Studio Online 備有[舊版的 Application Insights][舊版的 Application Insights]，其支援的應用程式類型更多。我們正著手將其重新建置並納入 Microsoft Azure 中，而您正在閱讀的內容即是與該新版本有關。

## <a name="add"></a>將 Application Insights 新增至專案中

您需要 [Visual Studio 2013 Update 3][Visual Studio 2013 Update 3] (或更新版本)。
...
按一下任一個磚以查看詳細資料。您可以變更報告顯示的內容。[深入了解如何設定應用程式健全狀況報告。(英文)][深入了解如何設定應用程式健全狀況報告。(英文)]
...
![In your project in Application Insights, click Quick start, Instrument your website, and copy the code][In your project in Application Insights, click Quick start, Instrument your website, and copy the code]
...
![Click through to more data][1]

[逐一點選圖表以查看詳細資料。(英文)][深入了解如何設定應用程式健全狀況報告。(英文)]

### <a name="deploy"></a>5. 部署應用程式

部署應用程式並觀看資料累積情形。

一旦您擁有即時應用程式後，請[設定 Web 測試][設定 Web 測試] (英文) 以確認應用程式處於即時狀態。
...
[追蹤 Web 應用程式的流量 (英文)][追蹤 Web 應用程式的流量 (英文)]

[監視 Web 應用程式的效能 (英文)][深入了解如何設定應用程式健全狀況報告。(英文)]

[擷取及搜尋診斷記錄 (英文)][擷取及搜尋診斷記錄 (英文)]

[疑難排解][疑難排解]

## 詳細資訊

-   [Application Insights - 開始使用 (英文)][Application Insights - 開始使用 (英文)]
-   [立即監視即時網頁伺服器 (英文)][在不重新部署的情況下將代理程式安裝在伺服器上]
-   [監視 Web 應用程式的效能 (英文)][深入了解如何設定應用程式健全狀況報告。(英文)]
-   [搜尋診斷記錄 (英文)][擷取及搜尋診斷記錄 (英文)]
-   [利用 Web 測試追蹤可用性 (英文)][設定 Web 測試]
-   [追蹤流量][追蹤 Web 應用程式的流量 (英文)]
-   [問答集和疑難排解 (英文)][疑難排解]

<!--Link references-->

  [Microsoft Azure]: http://azure.com
  [將 Application Insights 新增至 Visual Studio 專案]: #add
  [在不重新部署的情況下將代理程式安裝在伺服器上]: ../app-insights-monitor-performance-live-website-now/
  ...
  [深入了解如何設定應用程式健全狀況報告。(英文)]: ../app-insights-web-monitor-performance/
  ...
  [設定 Web 測試]: ../app-insights-monitor-web-app-availability/
  [Example application monitor in Application Insights]: ./media/appinsights/appinsights-00-appblade.png
  [追蹤 Web 應用程式的流量 (英文)]: ../app-insights-web-track-usage/
  [擷取及搜尋診斷記錄 (英文)]: ../app-insights-search-diagnostic-logs/
  [疑難排解]: ../app-insights-troubleshoot-faq/
  [Application Insights - 開始使用 (英文)]: ../app-insights-start-monitoring-app-health-usage/
