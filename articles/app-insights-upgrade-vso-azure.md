...
6. 網頁：如果您的專案包含網頁，請取代 <head> 區段中的指令碼。一般來說，主版頁面中只有一份，例如 Views\\Shared\_Layout.cshtml。[從 Azure 的 Application Insights 資源中的 [快速入門] 分頁取得新指令碼][usage]。如果您的網頁主體中包括遙測呼叫 (例如 logEvent 或 logPage)，請[將它們更新為使用新 API][track]。
...
10. 效能計數器：如果您使用效能計數器，您可以編寫自己的程式碼來定期抽樣計數器，並使用 [TrackMetric()][track] 傳送。

## 如果您具有 Java Web 服務...

1. 在您的伺服器機器中，透過從 Web 服務啟動檔中移除 APM 代理程式的參考來停用舊的代理程式。在 TomCat 伺服器上，編輯 Catalina.bat。在 JBoss 伺服器上，編輯 Run.bat。 
2. 重新啟動 Web 服務。
3. 在 Microsoft Azure 入口網站中，[加入新的 Application Insights 資源][java]。在您的開發機器中，將 [Java SDK][java] 加入至您的 Web 專案。您現在可以透過伺服器程式碼[傳送自訂遙測][track]。
