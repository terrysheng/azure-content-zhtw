...
# 保護 API 應用程式：加入 Azure Active Directory 或社交提供者驗證

## 概觀

在[部署 API 應用程式](app-service-dotnet-deploy-api-app.md)教學課程中，您利用**可供任何人使用**存取層級來部署 API 應用程式。本教學課程示範如何保護 API 應用程式，如此一來，只有經過驗證的使用者才可加以存取。
...
本教學課程適用於您在[建立 API 應用程式](app-service-dotnet-create-api-app.md)中建立以及在[部署 API 應用程式](app-service-dotnet-deploy-api-app.md)中部署的 API 應用程式 。
...
3. 在 [**API 應用程式**] 刀鋒視窗中，按一下 [**URL**] 以開啟可呼叫 API 應用程式的瀏覽器視窗。
...
2. 將 `/api/contacts/get/` 新增至瀏覽器網址列中的 URL。
	...
	不同的瀏覽器會以不同方式處理 API 呼叫。下圖顯示從 Chrome 瀏覽器的成功呼叫。
...
當您移至 [Azure 入口網站]中資源群組的刀鋒視窗時，您可以在圖表中看到您的 API 應用程式和閘道。

![Resource group diagram](./media/app-service-api-dotnet-add-authentication/rgdiagram.png)

若要將 API 應用程式設定為只接受經過驗證的要求，您會將其可存取性設定為 [**公用 (已驗證)**]，而且您會將閘道設定為需要提供者 (例如 Azure Active Directory、Google 或 Facebook) 的驗證。

1. 返回您要保護之 API 應用程式的 [**API 應用程式**] 刀鋒視窗。
...
3. 在 [**應用程式設定**] 刀鋒視窗中，將 [**存取層級**] 變更為 [**公用 (已驗證)**]，然後按一下 [**儲存**]。
...
7. 在 [**閘道**] 刀鋒視窗中，按一下 [**設定**]，然後按一下 [**身分識別**]。
	...
	您可以從 [**身分識別**] 刀鋒視窗瀏覽到不同的刀鋒視窗，以使用 Azure Active Directory 和數個其他提供者設定驗證。

	![Identity blade](./media/app-service-api-dotnet-add-authentication/identityblade.png)
  
3. 選擇您要使用的身分識別提供者，並遵循對應文章中的步驟，透過該提供者來設定您的 API 應用程式。這些文章是針對行動應用程式所撰寫的，但與 API 應用程式所用的程序相同。其中有些程序需要您使用[舊的入口網站]。 

 - [Microsoft 帳戶](app-service-mobile-how-to-configure-microsoft-authentication-preview.md)
 - [Facebook 登入](app-service-mobile-how-to-configure-facebook-authentication-preview.md)
 - [Twitter 登入](app-service-mobile-how-to-configure-twitter-authentication-preview.md)
 - [Google 登入](app-service-mobile-how-to-configure-google-authentication-preview.md)
 - [Azure Active Directory](app-service-mobile-how-to-configure-active-directory-authentication-preview.md)

例如，下列螢幕擷取畫面顯示在設定 Azure Active Directory 驗證之後，您應在[舊的入口網站]頁面和 [Azure 入口網站]刀鋒視窗中看到的內容。

在 Azure 入口網站中， [**Azure Active Directory**] 刀鋒視窗有來自您在舊入口網站的 [Azure Active Directory] 索引標籤中建立之應用程式的 [**用戶端識別碼**]，而 [**允許的租用戶**] 有您的 Azure Active Directory 租用戶 (例如，"contoso.onmicrosoft.com")。

![Azure Active Directory blade](./media/app-service-api-dotnet-add-authentication/tdinaadblade.png)

在舊的入口網站中，您在 [**Azure Active Directory**] 索引標籤中建立之應用程式的 [**設定**] 索引標籤會有來自 Azure 入口網站中 [**Azure Active Directory**] 刀鋒視窗的 [**登入 URL**]、[**應用程式識別碼 URI**] 和 [**回覆 URL**]。
...
(影像中的 [回覆 URL] 會顯示的相同 URL 兩次，一次使用 `http:`，而一次使用`https:`。)
...
2. 在瀏覽器中，移至登入 URL： 
	...
	例如，如果將您的資源群組命名為 myfirstrg 並設定閘道以供 Azure Active Directory 驗證，URL 會如下：

    	http://myfirstrggateway.azurewebsites.net/login/aad

	請注意，不同於先前的 URL，此 URL 不包含您的 API 應用程式名稱：閘道 (而非 API 應用程式) 會驗證您的身分。閘道會處理資源群組中所有 API 應用程式的驗證。

	(如果您有登入問題，請嘗試開啟私人或 incognito 視窗。)

3. 在瀏覽器顯示登入頁面時輸入您的認證。 

	如果您設定了 Azure Active Directory 登入，請使用您在[舊入口網站](https://manage.windowsazure.com/)的 [Azure Active Directory] 索引標籤中建立之應用程式的 [**使用者**] 索引標籤中列出的其中一個使用者，例如 admin@contoso.onmicrosoft.com。
...
4. 出現「登入完成」訊息時，再次輸入 API 應用程式之 Get 方法的 URL。
	...
	此時因為您已通過驗證，所以呼叫會成功。閘道會將您視為已經驗證的使用者，並將您的要求傳遞到您的 API 應用程式。
...
您可以看到背景中發生的事情，本節中您會使用瀏覽器工具來建立和提交 Post 要求，然後從 cookie 取得授權權杖並將它包含在 HTTP 標頭中。
...
3. 在 Chrome 開發人員工具的 [**資源**] 索引標籤中，尋找您的閘道的 cookie，然後按三下 **x-zumo-auth** 的值即可全選。
	...
	**注意：**確定您取得 cookie 的所有值。如果您按兩下，就只會取得 cookie 的第一個部分。
...
[舊的入口網站]: https://manage.windowsazure.com/
