...
本教學課程示範如何撰寫 ASP.NET [API 應用程式](app-service-api-apps-why-best-platform.md)的程式碼，以呼叫針對 [**內部**] 存取層級設定的另一個 API 應用程式。這兩個 API 應用程式必須在相同的資源群組中。相同的程式碼可用來從[行動裝置應用程式](app-service-mobile-value-prop-preview.md)呼叫內部 API 應用程式。
