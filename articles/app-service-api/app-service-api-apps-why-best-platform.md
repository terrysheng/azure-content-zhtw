...
API Apps 提供豐富的平台和生態系統，可用來在雲端和內部部署中建置、裝載、使用和分送 API。將您的 API 部署為 API 應用程式，即可受益於企業級安全性、簡單的存取控制、混合式與 SaaS 連線、自動 SDK 產生，並與 [Logic Apps](../app-service-logic/app-service-logic-what-are-logic-apps.md) 完美整合。

API Apps 是 [Azure App Service](../app-service/app-service-value-prop-what-is.md) 套件的其中一部分，此套件的其他部分包括 Web Apps、Mobile Apps 與 Logic Apps。

![](./media/app-service-api-apps-why-best-platform/appservicesuite.png)

## 為何採用 API Apps？

API Apps 可用來開發、部署、發行、使用和管理 RESTful Web API。App Service 提供公開預覽的下列立即可用功能：

- **輕鬆使用** - 使用整合的 [Swagger](http://swagger.io/) 支援，讓各種用戶端都能輕鬆使用您的 API。API Apps SDK 可以各種語言 (包括 C#、Java 和 Javascript) 為您的 API 產生用戶端程式碼。

- **簡易存取控制** - 內建的驗證服務可支援 Azure Active Directory 或協力廠商服務 (例如 Facebook 和 Twitter)。您可以保護 API 應用程式，使其免於未經驗證的存取，且不必變更程式碼。如果您熟悉 [Azure Mobile Services](../mobile-services-windows-dotnet-how-to-use-client-library.md#authentication) 提供的驗證服務，API Apps 建置於該架構上，並將其延伸至 API Apps 裝載的 API。App Service SDK 也可讓您使用簡化的授權碼語法。如需詳細資訊，請參閱 [Azure App Service 中之 API 應用程式與行動裝置應用程式的驗證](../app-service/app-service-authentication-overview.md)。
...
- **與 Logic Apps 整合** - 您建立的 API 應用程式可供 [App Service Logic Apps](../app-service-logic/app-service-logic-what-are-logic-apps.md) 使用。
...
您可以繼續使用現有的 API：您不必變更現有 API 的任何程式碼，就能利用 API Apps 功能，只要將程式碼部署至 API 應用程式即可。您可以在 API 中使用 ASP.NET、Java、PHP、Node.js 或 Python。

API Apps 另外包含 [App Service Web Apps 的功能](../app-service-web/app-service-web-overview.md)。

>[AZURE.NOTE] [Azure API Management](/services/api-management/) 屬於個別服務，可提供端點整合及節流等功能。您可以搭配 API Apps 使用 API 管理。
>
>API Apps 目前處於公開預覽狀態。[App Service Web Apps](../app-service-web/app-service-web-overview.md) 是正式推出 (GA) 的服務，設計旨在建置及裝載全球規模的安全任務關鍵性應用程式。如果您目前正在尋找 GA 服務來建置 API，則 Web Apps 正是最佳選項。當 API Apps 進入 GA 時，我們將提供一個採用現有 Web 應用程式並運用 API Apps 其他功能的路徑。

### API Apps 功能即將推出

在不久的將來，API Apps 平台也將建立 API 的豐富生態系統，方法為使得共用您的程式碼變得輕而易舉：
