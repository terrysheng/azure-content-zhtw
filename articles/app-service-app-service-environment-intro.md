...
## 概觀 ##
App Service 環境是 Azure App Service 的 [Premium][PremiumTier] 服務方案選項，可提供完全隔離和專用的環境，以便安全地執行所有應用程式。這包括 [Web 應用程式][WebApps]、[行動應用程式][MobileApps]、[API 應用程式][APIApps] 和[邏輯應用程式][LogicApps] (具有擴充調整選項)。
