...
**AS2 連接器**公開了封存郵件的功能。封存功能會將訊息儲存在 **Azure Blob 容器** (屬於封裝設定一部分) 中。 
...
瀏覽至 AS2 連接器 API 應用程式執行個體，然後按一下  'Tracking'。使用篩選參數縮小追蹤資訊。您的訊息一旦出現在檢視中，按一下即可查看其詳細檢視。訊息的已封存 URL 會顯示在此詳細檢視中。
