<properties 
	pageTitle="Microsoft Azure 應用程式服務中的企業對企業連接器和 API 應用程式 | Azure" 
...
# Microsoft Azure 應用程式服務中的企業對企業連接器和 API 應用程式
Microsoft Azure 應用程式服務 (簡稱應用程式服務) 包含許多對整合環境而言很重要的 BizTalk API 應用程式。這些 API 應用程式是以 BizTalk Server 內使用的概念和工具為基礎，但現在可做為 Azure 應用程式服務的一部分。 
...
您可以建立所需個數的 API 應用程式並輕鬆地建立它們。您也可以在多個案例或工作流程中重複使用單一 API 應用程式。

不需撰寫任何程式碼，即可執行這項操作。我們就此開始。 
...
需求 | 說明
--- | ---
Azure SQL Database | 儲存 B2B 項目，包括夥伴、結構描述、憑證及協議。每個 B2B API 應用程式都需要自己的 Azure SQL Database。 <br/><br/>**注意** 將連接字串複製到此資料庫。<br/><br/>[建立 Azure SQL Database](sql-database-create-configure.md)
Azure Blob 儲存體容器 | 啟用 AS2 封存功能時，可儲存訊息屬性。如果您不需要 AS2 訊息封存功能，就不需要儲存體容器。 <br/><br/>**注意** 如果您要啟用封存功能，請將連接字串複製到此 Blob 儲存體。<br/><br/>[關於 Azure 儲存體帳戶](storage-create-storage-account.md)
服務匯流排命名空間與其索引鍵值 | 會儲存 X12 和 EDIFACT 批次資料。如果您不需要批次處理，就不需要服務匯流排命名空間。<br/><br/>**注意** 如果您要啟用批次處理，請複製這些值。<br/><br/>[建立服務匯流排命名空間](http://msdn.microsoft.com/library/azure/hh690931.aspx)
TPM 執行個體 | 建立 AS2 連接器和 X12 或 EDIFACT API 應用程式時，需要 BizTalk 交易夥伴管理 (TPM) 執行個體。當您建立 TPM API 應用程式時，您正在建立 TPM 執行個體。 <br/><br/>**注意** 知道您的 TPM API 應用程式的名稱。 


## 建立 API 應用程式
使用 Azure 入口網站或 REST API 可以建立 B2B API 應用程式。 


###  使用 API 建立 API 應用程式
[請參閱有關如何使用 REST API 的文件。](http://go.microsoft.com/fwlink/p/?LinkId=529766)


### 在 Azure 入口網站中建立 B2B API 應用程式
在 Azure 入口網站中，您可以在建立邏輯應用程式、Web 應用程式或行動應用程式時建立 B2B  API 應用程式。或者，您可以使用應用程式自己的刀鋒視窗加以建立。這兩種方法都很簡單，所以視您的需求或喜好而定。有些使用者偏好先使用其特定屬性建立所有 B2B API 應用程式。然後，建立邏輯應用程式/Web 應用程式/行動應用程式，並加入您建立的  B2B API 應用程式。  

下列步驟會使用 API 應用程式刀鋒視窗建立 B2B API 應用程式。


#### 建立 BizTalk 交易夥伴管理 (TPM) API 應用程式

> [AZURE.NOTE] 建立 AS2 連接器和 X12 或 EDIFACT API 應用程式時，需要 BizTalk 交易夥伴管理 (TPM) 執行個體。當您建立 TPM API 應用程式時，您正在建立 TPM 執行個體。

下列步驟可建立 TPM 執行個體：

1. 在 Azure 入口網站開始面板 (首頁) 中，選取 **Marketplace**。**API 應用程式**會列出所有現有的 API 應用程式和連接器。您也可以**搜尋**特定 B2B API 應用程式。
2. 選取 **BizTalk 交易夥伴管理**。在新的刀鋒視窗中，選取 [**建立**]。 
3. 輸入內容： 

	屬性 | 說明
--- | ---
名稱 | 輸入 TPM 執行個體的任何名稱。例如，您可以命名為  *AccountsPayableTPM*。
Package Settings | 將 ADO.NET **資料庫連接字串**輸入到您所建立的 Azure SQL Database。 <br/><br/>當您複製連接字串時，密碼不會加入至連接字串。請務必在連接字串中輸入密碼。
應用程式服務方案 | 列出您的付款計劃。如果您需要更多或更少資源，可以變更該計劃。
定價層 | 可列出您的 Azure 訂用內定價類別的唯讀屬性。 
資源群組 | 建立一個新群組或使用現有的群組。邏輯應用程式、Web 應用程式和行動應用程式的所有 API 應用程式和連接器都必須在相同的資源群組中。 <br/><br/>[使用資源群組](azure-preview-portal-using-resource-groups.md) 說明這個屬性。 
訂閱 | 可列出您目前訂用的唯讀屬性。
位置 | 裝載您的 Azure 服務的地理位置。 
新增到開始面板 | 選取此選項可將 B2B API 應用程式新增至您的開始面板 (首頁)。
...
1. 在 Azure 入口網站開始面板 (首頁) 中，選取 **Marketplace**。**API 應用程式**會列出所有現有的 API 應用程式和連接器。您也可以**搜尋**特定 B2B API 應用程式。
2. 選取 [**AS2 連接器**]。在新的刀鋒視窗中，選取 [**建立**]。 
3. 輸入內容： 

	屬性 | 說明
--- | ---
名稱 | 輸入 AS2 連接器的任何名稱。例如，您可以命名為  *AS2Connector*。
Package Settings | 輸入該 API 應用程式特定的設定，像是 TPM 執行個體名稱。 <br/><br/>請參閱本主題中的[新增 AS2 封裝設定](#AddAS2Conn)以取得特定屬性。 
應用程式服務方案 | 列出您的付款計劃。如果您需要更多或更少資源，可以變更該計劃。
定價層 | 可列出您的 Azure 訂用內定價類別的唯讀屬性。 
資源群組 | 建立一個新群組或使用現有的群組。[使用資源群組](azure-preview-portal-using-resource-groups.md)說明這個屬性。 
訂閱 | 可列出您目前訂用的唯讀屬性。
位置 | 裝載您的 Azure 服務的地理位置。 
新增到開始面板 | 選取此選項可將 B2B API 應用程式新增至您的開始面板 (首頁)。
...
1. 在 Azure 入口網站開始面板 (首頁) 中，選取 **Marketplace**。**API 應用程式**會列出所有現有的 API 應用程式和連接器。您也可以**搜尋**特定 B2B API 應用程式。
2. 選取 **BizTalk X12** 或 **BizTalk EDIFACT**。在新的刀鋒視窗中，選取 [**建立**]。 
3. 輸入內容： 

	屬性 | 說明
--- | ---
名稱 | 輸入 B2B API 應用程式的任何名稱。例如，您可以命名為  *EDI850APIApp*。
封裝設定 | 輸入該 API 應用程式特定的設定，像是 TPM 執行個體名稱。 <br/><br/>請參閱本主題中的 [X12 或 EDIFACT 封裝設定](#AddX12)以取得特定屬性。 
應用程式服務方案 | 列出您的付款計劃。如果您需要更多或更少資源，可以變更該計劃。
定價層 | 可列出您的 Azure 訂用內定價類別的唯讀屬性。 
資源群組 | 建立一個新群組或使用現有的群組。[使用資源群組](azure-preview-portal-using-resource-groups.md)說明這個屬性。 
訂用 | 可列出您目前訂用的唯讀屬性。
...
- 檢視資訊和錯誤事件
- 檢視背景工作處理序 (w3wp) 的記憶體使用量和執行緒計數
- 檢視應用程式和 Web 伺服器記錄檔

如需詳細資訊，請參閱[監控應用程式邏輯](app-service-logic-monitor-your-logic-apps.md)。


## 將 API 應用程式加入至您的應用程式 
Microsoft Azure 應用程式服務會公開可使用這些 B2B API 應用程式的不同應用程式類型。您可以建立新的應用程式或將現有的 B2B API 應用程式加入至邏輯應用程式、行動應用程式或 Web 應用程式。 
...
1. 在 Azure 入口網站開始面板 (首頁) 中，移至 **Marketplace**，然後搜尋您的  邏輯、行動或 Web 應用程式。 

	如果您要建立新的應用程式，請搜尋邏輯應用程式、行動應用程式或 Web 應用程式。選取應用程式，並且在新的刀鋒視窗中選取 [**建立**]。[建立邏輯應用程式](app-service-logic-create-a-logic-app.md)會列出相關步驟。 
...
[應用程式和資料服務連接器](app-service-logic-data-connectors.md)<br/>
