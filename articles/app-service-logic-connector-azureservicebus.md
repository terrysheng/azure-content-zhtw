...
3.	設定 Azure 服務匯流排連接器，如下所示：
	...
	- **定價層** - 選擇連接器的定價層
	- **名稱** - 提供 Azure 服務匯流排連接器的名稱
	- **套件設定**
		- **連接字串** - Azure 服務匯流排連接器的連接字串。範例：Endpoint=sb://[namespace].servicebus.windows.net;SharedAccessKeyName=[name];SharedAccessKey=[key]
		- **實體名稱** - 佇列或主題的名稱
		- **訂用帳戶名稱** - 要自其接收訊息的定用戶之名稱 
...
2.	開啟 [觸發程序和動作] 以開啟邏輯應用程式設計工具，並設定您的流程。 
	...
	![][3]
3.	Azure 服務匯流排連接器就會出現在右側程式庫中的 [此資源群組中的 API 應用程式] 區段。
