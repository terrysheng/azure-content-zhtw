...
FTP 連接器具有一個觸發程序 - TriggerOnFileAvailable。每當新檔案上載到 FTP 伺服器中的特定資料夾時就會觸發該程序。

按一下  'TriggerOnFileAvailable' 觸發程序。
...
請注意，設定的 FTP 觸發程序會顯示已設定的輸入參數，以及 FTP 觸發程序的輸出。 
...
- **存在時附加** - 啟用或停用 'Append If Exist'。啟用時，資料會附加至檔案 (如果存在的話)。停用時，則會覆寫檔案 (如果存在的話)。
- **暫存資料夾** - 選擇性。若已提供，配接器會將檔案上傳至  'Temporary Folder Path'，上傳完成後，檔案將會移至  'Folder Path'。 'Temporary Folder Path' 應位於與  'Folder Path' 相同的實體磁碟上，以確保移動作業不可部分完成。只有在停用  'Append If Exist' 屬性時，才可以使用暫存資料夾。
