...
邏輯應用程式可以根據各種資料來源觸發，並提供連接器以取得及處理屬於流程一部分的資料。Office 365 連接器可讓您傳送和接收電子郵件，及管理 Office 365 帳戶的行事曆和連絡人。您可以執行各種動作，例如傳送、接收和取得電子郵件、建立和刪除行事曆中的事件，以及建立、更新、取得和刪除您的連絡人。
...
## 建立邏輯應用程式##
若要在邏輯應用程式中使用 Office 365 連接器，您應該已經建立好 API 應用程式連接器，或者您可以在邏輯應用程式中即時建立一個連接器。讓我們建立一個會在收到電子郵件 (地址是您的銷售查詢電子郵件識別碼 - 例如 sales@contoso.com) 時觸發的簡單邏輯應用程式。接著，它會建立事件、使用寄件者的詳細資料新增連絡人、傳送電子郵件到您個人帳戶，最後會傳送包含通知的回覆。
...
4.	在組件庫中展開 [新建]，以查看所有可用的 API 應用程式。從組件庫中選取 [Office 365]，[Office 365 觸發程序] 便會被新增至流程。
	![][4]

5.	這便會在與邏輯應用程式相同的資源群組中建立新的 [Office 365 連接器] API 應用程式。建立 API 應用程式大約需要 30 秒的時間。

6.	建立應用程式之後，按一下 [授權] 以提供 Office 365 登入認證。
...
9.	選取 [新的電子郵件] 觸發程序，而輸入參數便會隨即顯示。


10.	將觸發程序頻率變更為 'Minutes'，然後按一下 [✓]。 

	![][9]

11. Office 365 'New Email' 觸發程序便已設定，而且您會看到輸出參數隨即顯示。
	
	![][10]

12.	從組件庫的 [最近使用的] 區段中選取 [Office 365 連接器]，即會新增新的 Office 365 動作。
...
15.	從組件庫的 [最近使用的] 區段中選取 [Office 365 連接器]，即會新增新的 Office 365 動作。

16.	從動作清單中選取 [新增連絡人]，[新增連絡人] 動作的輸入參數便會隨即顯示。

	![][13]

17.	按一下 'Email Address' 欄位旁邊的 [+]，並從觸發程序選取 'From' 輸出欄位值。
...
19.	從組件庫的 [最近使用的] 區段中選取 [Office 365 連接器]，即會新增新的 Office 365 動作。
...
21.	提供傳送電子郵件所需的詳細資料。您可以透過輸入如下所示的內容來建構訊息。設定 'Send Email' 動作之後，請按一下 [✓]。

		Body - @concat('You got a new sales enquiry from',triggers().output.body.From)

	![][20]
22.	從組件庫的 [最近使用的] 區段中選取 [Office 365 連接器]，即會新增新的 Office 365 動作。
...
24.	按一下 'From' 欄位旁邊的 [+]，並從觸發程序的輸出訊息識別碼中選取值，然後按一下 [✓]。

	![][17]

25. 在邏輯應用程式編輯器畫面上按一下 [確定]，然後按一下 [ 'Create']。完成建立大約需要 30 秒的時間。
