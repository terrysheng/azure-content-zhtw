...
若要使用 OneDrive 連接器，您必須先建立 OneDrive 連接器 API 應用程式的執行個體。以下步驟可以達到此目的：

1.	使用 Azure 入口網站左下方的 [+新增] 選項開啟 Azure Marketplace。
2.	瀏覽至 [Web 與行動] > [API 應用程式]，並搜尋「OneDrive 連接器」。
...
1.	建立新的邏輯應用程式，並選擇具有 OneDrive 連接器的相同資源群組。依照下列指示[建立新的邏輯應用程式]。  	
...
9. API 的詳細資訊可在 [OneDrive 連接器 API 定義]中找到。
...
[OneDrive 連接器 API 定義]: https://msdn.microsoft.com/zh-tw/library/dn974227.aspx
