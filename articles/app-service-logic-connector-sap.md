...
在邏輯應用程式中，連接器可以在執行流程時用來擷取、處理或發送資料。 
在某些案例中，您可能需要用到在內部部署安裝並位於防火牆後面的 SAP。透過在流程中運用 SAP 連接器，您可以達到各種案例的目的。幾個範例：  
...
若要使用 SAP 連接器，您必須建立 'SAP Connector' API 應用程式的執行個體。以下步驟可以達到此目的：
...
1. 複製主要連接字串
2. 按一下 'Download and Configure'連結
