...
3.	設定 SFTP 連接器，如下所示：
	...
	- **套件設定** 
		...
		- **SSH 伺服器 HostKey** - 指定 SSH 伺服器的公開主機金鑰指紋。
		- **根資料夾** - 指定根資料夾路徑
		- **加密編碼器** - 指定加密編碼器。
		- **伺服器連接埠** - 指定 SFTP 伺服器連接埠號碼
4. 按一下 [建立]。將建立新的 SFTP 連接器。

5. 透過 [瀏覽] -> [API 應用程式] -> [<剛建立的 API 應用程式名稱>]，然後您可以看到 [安全性] 元件尚未設定。 

	![][2]
6. 按一下 [安全性] 元件來設定 SFTP 連接器的安全性 (使用者名稱、密碼、私密金鑰、PPK 檔案密碼)。 
選取安全性之下的 [密碼]、[私密金鑰]、[多因素] 授權索引標籤，並提供必要的屬性。
...
2.	開啟 [觸發程序和動作] 以開啟邏輯應用程式設計工具，並設定您的流程。 
	...
	![][7]
3.	SFTP 接器就會出現在右側程式庫中的 [此資源群組中的 API 應用程式] 區段。
...
6.	您現在便可以在流程中使用 SFTP 連接器。您可以在流程的其他動作中使用從 SFTP 觸發程序 ("TriggerOnFileAvailable") 所擷取的「檔案」。 
	...
	**注意：** SFTP 觸發程序 "TriggerOnFileAvailable" 處理擷取的檔案之後會將它刪除。

8.	設定 SFTP 觸發程序的輸入屬性，如下所示：

	- **資料夾路徑** - 指定要擷取檔案的資料夾之路徑。
	- **檔案類型：文字或二進位** - 選取檔案類型。
	- **檔案遮罩** - 指定擷取檔案要套用的檔案遮罩。'*' 擷取指定資料夾中的所有檔案。
	- **排除檔案遮罩** - 指定排除檔案要套用的遮罩。如果也有設定「檔案遮罩」屬性，會先套用「排除檔案遮罩」。
...
7.	您可以採用類似方式在流程中使用 SFTP 動作。您可以使用「上傳檔案」動作將檔案上傳至 SFTP 伺服器。設定 [上傳檔案] 的輸入屬性，如下所示：
	...
	- **若存在則附加** - 指定 [true] 或 [false]。設定為 [true] 時，若檔案存在則資料會附加至檔案。設定為 [false] 時，若檔案存在則會覆寫檔案。
	- **暫存資料夾** - 如果有提供，配接器會將檔案上傳到 '暫存資料夾路徑' ，上傳完成之後檔案會移至 '資料夾路徑'。'暫存資料夾路徑' 應和 '資料夾路徑' 在相同的實體磁碟上，以確保移動作業不可部分完成。暫存資料夾只有啟用 '若存在則附加' 屬性時可以使用。
