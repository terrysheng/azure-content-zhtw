...
- **資料持續性**：在透過 AS2 接收到訊息之後，Northwind 想要將它保存以便進一步處理。它可以使用連接器來將訊息保存在其雲端儲存體中。在本教學課程中，Northwind 會採用 Azure Blob 做為雲端儲存體 。
...
1. 建立 **Azure 儲存體 Blob 連接器**的執行個體。這需要 Azure 儲存體帳戶的認證。請確定您在開始建立之前已將它準備好。
2. 建立 **BizTalk 交易夥伴管理**的執行個體。這需要一個空白的 SQL Database 才能運作。請確定您在開始建立之前已將它準備好。
3. 建立 **AS2 連接器**的執行個體。這也需要一個空白的 SQL Database 才能運作。請確定您在開始建立之前已將它準備好。此外，如果您想要在 AS2 處理的過程中封存訊息，您可能要在其建立期間提供 Azure Blob 的認證。
4. 設定已建立的 TPM (交易夥伴管理) 服務：
	1. 瀏覽至上述步驟中所建立之 TPM 服務的執行個體。
	2. 使用 [**夥伴**] 選項 (位於 [ *Components*] 下方) 來**新增**一個名為 **Contoso** 的新合作夥伴，並在其設定檔中加入必要的 AS2 身分識別。
	3. 使用 [**夥伴**] 選項 (位於 [ *Components*] 下方) 來**新增**一個名為 **Northwind** 的新合作夥伴，並在其設定檔中加入必要的 AS2 身分識別。
	4. 使用 [**協議**] 選項 (位於 [ *Components*] 下方) 來**新增** Northwind 和 Contoso 之間的 AS2 協議。在此案例中，Northwind 會是託管夥伴，而是 Contoso 會是來賓夥伴。在此協議建立期間，視情況設定簽章、加密、壓縮和通知。瀏覽所建立的 TPM 服務時，如果需要使用憑證，您可以透過 [**憑證**] 選項將他們上傳。
