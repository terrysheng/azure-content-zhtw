...
這些屬於不同的流程，但它們會使用相同的 QuickBooks 連接器。您可以在無需撰寫任何程式碼的情況下建立和使用這些連接器。讓我們開始吧。 
...
## 建立連接器
您可以使用 Azure 入口網站或使用 REST API 建立連接器。 
...
3. 輸入屬性： 
	...
	屬性 | 說明
--- | ---
名稱 | 為您的連接器輸入任何名稱。例如，您可以將它命名為 *SAPConnector*、*SalesForceGetAccounts* 或 *QuickBooksGetItems*。
Package Settings | 輸入企業系統設定，例如 *SAP User Name* 或 *SugarCRM Server URL*。請參閱本主題中的 [企業系統特有屬性](#AddProperties)。 
...
MongoDB| <ul><li>連接字串</li><li>主機</li><li>連接埠</li><li>使用者名稱</li><li>密碼</li><li>資料庫</li><li>使用 SSL 加密</li><li>內部部署：如果是雲端架構，請輸入 False。如果 MongoDb 是內部部署系統，請輸入 True，並輸入下列屬性：<ul><li>共用存取金鑰名稱</li><li>服務匯流排命名空間</li><li>轉送路徑</li><li>傳送金鑰</li></ul></li></ul>
...
## 其他企業連接器資源
[與內部部署 SAP 伺服器整合] (app-service-logic-integrate-with-an-on-premise-SAP-server.md)<br/>
