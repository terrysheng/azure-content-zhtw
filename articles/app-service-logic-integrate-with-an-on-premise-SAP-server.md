...
7. 在開啟的新分頁中，輸入下列內容：
	...
	7. **封裝設定**
		...
		- **內部部署** - 輸入 SAP 伺服器是否內部部署在防火牆後面。如果設定為 TRUE，則您必須在能夠存取 SAP 伺服器的伺服器上安裝接聽程式代理程式。您可以移至您的 API 應用程式摘要頁面，然後選取 [ 'Hybrid Connection'] 來安裝代理程式。
...
## 安裝混合式接聽程式
瀏覽至您透過 [**瀏覽**] > [**API 應用程式**] > [ *name of your connector*] 所建立的 SAP 連接器
...
在 [連接器] 分頁中，請注意混合式連線的狀態為 *Connected*。
