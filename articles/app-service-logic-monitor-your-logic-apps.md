...
在依照   [建立邏輯應用程式](app-service-logic-create-a-logic-app.md) 中所述步驟建立邏輯應用程式之後，您可以在 Azure 入口網站中查看其執行的完整記錄。若要檢視記錄，請按一下入口網站畫面左側的 [**瀏覽**]，然後選取 [**邏輯應用程式**]。您訂閱中的邏輯應用程式清單便會隨即顯示。邏輯應用程式可以是 [**已啟用**] 或 [**已停用**]。**已啟用**邏輯應用程式代表觸發程序會執行邏輯應用程式以回應觸發程序事件，**已停用**邏輯應用程式代表將不會執行邏輯應用程式以回應事件。
