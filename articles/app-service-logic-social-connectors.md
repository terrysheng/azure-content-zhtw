...
## 什麼是社交 API 應用程式連接器
社交連接器是可以連接到社交應用程式並代表已驗證的使用者執行數個 *Actions*的「API 應用程式」。大部分的連接器也可以使用 *Trigger*來設定。觸發程序是可設定於某些連接器上的事件 (類似於 .NET Framework 中的事件)。觸發程序接著可起始已為該連接器 (或任何其他連接器) 設定的動作。例如，Twitter 連接器的執行個體可使用  *new message* 觸發程序來設定，而該觸發程序定義為任何提及 @VisualStudio 的新推文。此觸發程序可接著設定為會起始轉推推文的 *Action*。 
...
- 使用 Microsoft Azure 入口網站 (不需要撰寫程式碼！)
- 使用 REST API。 
...
3. 輸入連接器的下列屬性： 
	<table>
	    ...
	    <tr><td>應用程式服務計劃</td> <td>列出您的付款計劃。如果您需要增減資源，您可以變更。</th> </td>
        ...
        <tr><td>新增至儀表板</td> <td>選取此選項，可將 API 應用程式新增至儀表板 (首頁)</td></tr>
	</table> 
...
## 將連接器新增至應用程式 
Microsoft Azure 應用程式服務會公開可使用這些連接器的不同應用程式類型。例如，您可以將一或多個連接器 *logically*結合為單一應用程式，以建立 *Logic*應用程式。

若要使用 *Logic*應用程式中的連接器，您可以從清單中選取預先設定的連接器、將其新增至您的設計工作流程中，並確定它已完成所需的設定變更而可供使用。 

若要執行這些步驟，您需要 Web 應用程式、行動應用程式或邏輯應用程式。請參閱 <> 以了解特定步驟。應用程式可供使用後，請新增您的連接器。方式如下：

使用下列步驟，將連接器新增至邏輯應用程式： 

1. 在 Azure 入口網站儀表板 (首頁) 中，移至 [**Marketplace**]，然後搜尋邏輯、行動、或 Web 應用程式。 

	如果您要建立新的應用程式，請搜尋邏輯應用程式、行動應用程式或 Web 應用程式。在新分頁中選取 [應用程式]，然後選取 [**建立**]。[建立邏輯應用程式](app-service-logic-create-a-logic-app.md)中列出了相關步驟。 

2. 開啟您的 [應用程式]，然後選取 [**觸發程序和動作**]。 
...
[連接器和 API 的應用程式清單](app-service-logic-connectors-list.md)
