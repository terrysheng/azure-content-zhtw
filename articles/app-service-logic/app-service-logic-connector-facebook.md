<properties
   pageTitle="在 Azure App Service 的邏輯應用程式中使用 Facebook 連接器"
   description="如何在邏輯應用程式中使用 Facebook 連接器"
...
若要使用 Facebook 連接器，您必須先建立 Facebook 連接器 API 應用程式的執行個體。以下步驟可以達到此目的：
