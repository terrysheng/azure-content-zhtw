...
3.	設定 SFTP 連接器，如下所示：
	...
	![][1] - **位置** - 選擇您要部署連接器的地理位置 - **訂閱** - 選擇您要在其中建立連接器的訂閱 - **資源群組** - 選取或建立應用於存放連接器的資源群組 - **虛擬主機方案** - 選取或建立虛擬主機方案 - **定價層** - 選擇連接器的定價層 - **名稱** - 賦予 SFTP 連接器的名稱 - **封裝設定** - **伺服器位址** - 指定 SFTP 伺服器名稱或 IP 位址 - **接受任何 SSH 伺服器主機金鑰** - 決定是否應該接受任何來自伺服器的 SSH 公開主機金鑰指紋。如果設定為 false，主機金鑰就會跟「SSH 伺服器主機金鑰指紋」屬性中指定的金鑰進行比對 - **SSH 伺服器主機金鑰** - 指定 SSH 伺服器的公開主機金鑰指紋 - **根資料夾** - 指定根資料夾路徑 - **加密密碼** - 指定加密密碼。- **伺服器連接埠** - 指定 SFTP 伺服器連接埠號碼
