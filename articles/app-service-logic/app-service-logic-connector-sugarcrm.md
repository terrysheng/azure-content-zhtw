...
*注意*：如需查詢中支援的引數之詳細資訊，請參閱 [SugarCRM REST API](https://msdn.microsoft.com/library/dn705870) 文件。
...
4.	展開資源庫中的 [此資源群組中的 API 應用程式] 來查看所有可用的 API 應用程式。從資源庫選取 SugarCRM，然後 SugarCRM 連接器就會加入流程：![][3]

5.	選取 [建立帳戶] 動作，而輸入參數便會隨即顯示：![][4]

6.	提供名稱為「Microsoft 帳戶」，然後按一下 ✓：![][5]

7.	從資源庫的 [最近使用的] 區段中選取[ SugarCRM 連接器]，即會新增新的 SugarCRM 動作。

8.	從動作清單中選取 [更新帳戶] (在進階動作 [...] 之下)，就會顯示 [更新帳戶] 動作的輸入參數：![][6]

9.	按一下 [記錄識別碼] 旁邊的 [...]，並從 [建立帳戶] 動作的輸出中挑選 [識別碼] 值：![][7]

10.	提供帳單寄送地址的值，然後按一下 ✓：![][8]
