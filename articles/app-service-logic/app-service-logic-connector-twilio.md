...
2.	瀏覽至 [Web 與行動] > [API 應用程式]，並搜尋「Twilio 連接器」。
3.	設定 Twilio 連接器，如下所示：
	...
	- **名稱** - 提供 Twilio 連接器的名稱
	- **套件設定**
		- **帳戶 SID** - 帳戶的唯一識別碼。您可以從 https://www.twilio.com/user/account/settings 擷取您帳戶的帳戶 SID 
		- **驗證權杖** - 與帳戶關聯的授權權杖。您可以從 https://www.twilio.com/user/account/settings 擷取您帳戶的驗證權杖
...
5.	您現在便可以在流程中使用 Twilio 連接器。您可以在流程中使用 [傳送訊息] 動作來傳送訊息。設定 [傳送訊息] 動作，如下所示：
	- **傳自電話號碼** - 輸入已啟用您想要傳送之訊息類型的 Twilio 電話號碼。只有從 Twilio 購買的電話號碼或簡短程式碼能用於此連接器。
	- **傳到電話號碼** - 目的地電話號碼。接受的格式為：加號 (+) 後面跟隨國碼 (地區碼)，然後是電話號碼。例如， +16175551212。如果省略加號 (+)，Twilio 會使用您在 '傳自'號碼所輸入的國碼 (地區碼)。
	- **文字** - 您要傳送之訊息的文字。
