...
首先，您必須建立兩個您將用到的連接器：**Dropbox 連接器**和 **Twitter 連接器**。若要建立這些項目：

1. 按一下主畫面上的 [**Marketplace**]，並搜尋 **Twitter**。 
...
7. 選擇要執行邏輯應用程式的 [**位置**]。
	...
	![建立 API 應用程式分頁](./media/app-service-logic-create-a-logic-app/gallery.png)
...
1. 按一下畫面左下角的 [**+ 新增**] 按鈕，展開 [**Web + 行動**]，然後按一下 [**邏輯應用程式**]。 
	...
	![建立邏輯應用程式分頁](./media/app-service-logic-create-a-logic-app/createlogicapp.png)
...
觸發程序是可讓您的邏輯應用程式執行的項目。接著，您將新增週期性觸發程序，它會根據預先定義的排程啟動您的工作流程。
...
不到 5 分鐘的時間，您便能夠設定在雲端中執行的簡單邏輯應用程式。若要深入了解如何使用邏輯應用程式功能，請參閱[使用邏輯應用程式功能]。
