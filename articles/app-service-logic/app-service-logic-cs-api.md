...
##建立 C# API 應用程式
若要使用 C# API 應用程式，您必須先建立 C# API 應用程式的執行個體。這可以在建立邏輯應用程式時以內嵌方式完成，或是透過從 Azure Marketplace 選取 C# API 應用程式完成。

##在 Logic Apps 設計工具介面中使用 C# API 應用程式
###觸發程序
您可以建立 Logic App Service (以您定義的間隔) 輪詢的觸發程序，而且如果它傳回除了 `false` 以外的任何內容，邏輯應用程式將會執行，否則會等到下一個輪詢間隔再檢查一次。
