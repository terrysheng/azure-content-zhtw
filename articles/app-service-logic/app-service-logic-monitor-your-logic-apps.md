...
最後，在某個特定動作上，您可以取得所有傳遞至該動作的資料，以及從 [**輸入**] 和 [**輸出**] 區段中接收的資料。按一下連結以查看完整內容 (您也可以複製連結以下載內容)。
...
請注意，對於 [推入] 觸發程序，您將「不」會查看執行在這裡開始的時間，而會看到「回呼註冊」呼叫 (這些是邏輯應用程式註冊以重新呼叫時的呼叫)。如果推入觸發程序未運作，則註冊可能會發生問題 (您可以在 [輸出] 上看到)，但您可能需要特別調查該 API。
