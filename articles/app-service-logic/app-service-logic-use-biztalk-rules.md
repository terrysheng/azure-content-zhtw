...
如果選擇 XML 做為 [詞彙類型]，則必須指定下列的輸入  
  a.結構描述 – 按一下此選項會開啟新刀鋒視窗，讓使用者從已上傳的結構描述清單中選擇，或讓使用者上傳新的結構描述。
...
### 大量新增
上述步驟已擷取建立詞彙定義的經驗。在建立之後，建立的詞彙定義會以清單的形式出現。您可以從相同結構描述產生多個定義，而無需每次都重複上述步驟，但這有特定需求。此時，「大量新增」功能正可彰顯其功效。
選取 [大量新增]，會將您帶往新刀鋒視窗。第一個步驟是選取要建立多個定義的結構描述。選取此項後會開啟新刀鋒視窗，讓您從已上傳的結構描述清單中選擇，或允許您上傳新的結構描述。
此時，XPATHS 屬性會解除鎖定。選取此項後會開啟結構描述檢視器，您可以在當中選取多個節點。建立的多個定義會將其名稱預設為所選節點的名稱。這些名稱一律可在建立後修改。
...
在開發人員建立必要的詞彙後，預期應由商務分析人員透過 Azure 入口網站建立商務原則。
	...
	2.	此頁面會顯示此特定規則應用程式具有的原則清單。分析人員只要輸入原則名稱並點擊索引標籤兩次，即可新增原則。多個原則可以位於單一規則 API 應用程式中。  
	3.	選取已建立的原則，會讓使用者進入原則詳細資料頁面，而得以檢視位於原則中的規則。
![替代文字][8] 
	4.	選取 [新增] 加入新規則。這會使您進入新刀鋒視窗。

##建立規則
規則是條件和動作陳述式的集合。如果條件評估為 true，則會執行動作。在 [建立規則] 刀鋒視窗中，提供唯一的規則名稱 (適用於該原則) 和說明 (選擇性)。
...
##明確向前鏈結
BizTalk 規則支援明確向前鏈結，因此，如果使用者想要重新評估規則以回應特定動作，可以使用特定的關鍵字加以觸發。下列是支援的關鍵字：
