<properties
	...
	keywords="azure app service,app service,行動應用程式,行動後端,saas,azure saas,enterprise mobile"
	documentationCenter=""
...
在本教學課程中，您會將行動應用程式連接到企業的軟體即服務 (SaaS) 解決方案。每當新增新的 TodoItem 時，您都將從 [Azure Active Directory 驗證程式庫單一登入]更新應用程式，藉此在 SharePoint Online 中建立 Microsoft Word 文件。

本教學課程需要下列各項：

* 執行於 Windows 8.1 的 Visual Studio 2013
* 作用中 [SharePoint Online] 訂閱
* 完成 [Azure Active Directory 驗證程式庫單一登入]教學課程。您應使用 SharePoint 訂閱所提供的租用戶。
...
您現在已設定 Azure AD 發出 SharePoint 存取權杖給 App Service。
...
您可以使用 ApiServices.Settings 在程式碼中再次取得這些值。

## <a name="obtain-token"></a>取得存取權杖及呼叫 SharePoint API

若要存取 SharePoint，您必須要以 SharePoint 的特殊存取權杖作為目標對象。若要取得此權杖，需要以 App Service 身分識別回撥 Azure AD，也需要針對使用者所發出的權杖。
...
若要建立 Word 文件，您將使用 OpenXML NuGet 封裝。請開啟 NuGet Manager 並搜尋 DocumentFormat.OpenXml，然後安裝此封裝。

1. 將下列程式碼新增至 TodoItemController。這會基於 TodoItem 建立 Word 文件。文件的文字將會是項目的名稱。
