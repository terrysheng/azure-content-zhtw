...
- **現有的行動應用程式後端** <br/>請按照[快速入門教學課程](app-service-mobile-windows-store-dotnet-get-started.md)的指示，從 [Azure 入口網站]建立和下載新的 .NET 後端行動應用程式。
...
1. 在 [Azure 入口網站]中，瀏覽至您的行動應用程式，然後按一下設定中的 [網路]。
...
1. 在 Visual Studio 2013 中，開啟定義行動應用程式後端的專案。
	...
	若想了解如何下載 .NET 後端專案，請參閱[快速入門教學課程](app-service-mobile-windows-store-dotnet-get-started.md)。
