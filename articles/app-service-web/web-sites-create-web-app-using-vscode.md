<properties
   ...
   description="本教學課程說明如何使用 Visual Studio 程式碼建立 ASP.NET 5 Web 應用程式。"
...
本教學課程示範如何使用 [Visual Studio 程式碼 (VS 程式碼)](http://code.visualstudio.com//Docs/whyvscode) 建立 ASP.NET Web 5 應用程式，並將其部署到 [Azure App Service](../app-service/app-service-value-prop-what-is.md)。ASP.NET 5 是大幅重新設計的 ASP.NET。ASP.NET 5 是新的開放原始碼和跨平台架構，用於使用 .NET 建置新代雲端式 Web 應用程式。如需詳細資訊，請參閱＜[ASP.NET 5 簡介](http://docs.asp.net/en/latest/conceptual-overview/aspnet.html)＞。如需有關 Azure App Service Web Apps 的詳細資訊，請參閱＜[Web Apps 概觀](app-service-web-overview.md)＞。

[AZURE.INCLUDE [app-service-web-try-app-service.md](../../includes/app-service-web-try-app-service.md)]

## 必要條件  

* 安裝 [VS 程式碼](http://code.visualstudio.com/Docs/setup)。
* 安裝 [Node.js](http://nodejs.org/download/) - [Node.js](http://nodejs.org/) 是一種平台，可使用 JavaScript 建置快速和可擴充的伺服器應用程式。節點是執行階段 (節點)，而 [npm](http://www.npmjs.com/) 是節點模組的封裝管理員。您將使用 npm，在本教學課程中建立 ASP.NET 5 web 應用程式的結構。
* 安裝 Git - 您可以從下列位置安裝它：[Chocolatey](https://chocolatey.org/packages/git) 或 [git scm.com](http://git-scm.com/downloads)。如果您不熟悉 Git，請選擇 [git scm.com](http://git-scm.com/downloads)，並從 Windows 命令提示字元選取要使用 Git 與 GitBash 搭配的選項。一旦您安裝 Git，也需要設定 Git 使用者名稱和電子郵件，因為稍後教學課程將需要用到 (從 VS 程式碼執行認可時)。  

## 安裝 ASP.NET 5 和 DNX
ASP.NET 5/DNX 是精簡的 .NET 堆疊，可建置 OS X、Linux 和 Windows 上執行的現代化雲端和 Web 應用程式。已從頭建置，以將最佳化的開發架構提供給已部署至雲端或執行內部部署的應用程式。其由額外負荷最低的模組化元件組成，以便您可以在建構解決方案時保留彈性。
...
本節顯示如何建立新的應用程式 ASP.NET Web 應用程式的結構。您將使用 node 封裝管理員 (npm) 來安裝 [Yeoman](http://yeoman.io/) (應用程式建立結構工具 - 相當於 Visual Studio [**檔案 > 新專案**] 作業的 VS 程式碼)、[Grunt](http://gruntjs.com/) (JavaScript 工作執行程式) 和 [Bower](http://bower.io/) (用戶端封裝管理員)。
...
6. 在命令提示字元輸入下列命令來開啟 VS 程式碼。

		code .

7. 在 VS 程式碼中，選取 [**檔案 > 開啟資料夾**]，然後選取包含 ASP.NET Web 應用程式的資料夾。

	![[選取資料夾] 對話方塊](./media/web-sites-create-web-app-using-vscode/02-open-folder.png)

	VS 程式碼將載入您的專案，並將其檔案顯示在 [**瀏覽**] 視窗中。
...
5. 開啟瀏覽器並瀏覽至下列 URL。
	...
	**http://localhost:5001**
	...
	![瀏覽器中的本機 Web 應用程式](./media/web-sites-create-web-app-using-vscode/08-web-app.png)

## 在 Azure 預覽入口網站中建立 Web 應用程式
...
4. 輸入 [**名稱**] 的值，例如 **SampleWebAppDemo**。請注意，此名稱必須是唯一的，而且當您嘗試輸入名稱時，入口網站將會強制執行。因此，如果選取或輸入不同值，您將需要以該值取代每次出現的 **SampleWebAppDemo**，您會在本教學課程中看到。
...
8. 如果您先前尚未設定部署認證以供發佈 Web 應用程式或其他 App Service 應用程式，請立即設定：
	...
	* 按一下 [儲存]。
	...
	![Reset 部署認證](./media/web-sites-create-web-app-using-vscode/16-azure-credentials.png)
...
1. 在 VS 程式碼中，選取導覽列左側的 [**Git**] 選項。

	![VS 程式碼中的 Git 圖示](./media/web-sites-create-web-app-using-vscode/git-icon.png)
...
6. 使用您稍早複製的 Git URL，建立遠端參考，以便將更新發送至 Web 應用程式。
...
> [AZURE.NOTE]如果您變更應用程式時，則可以重新發行，方法是在 VS 程式碼中選取 **全部認可** 選項，然後在命令提示字元輸入 **git push azure master** 命令。
...
## 摘要
在本教學課程中，您學到如何在 VS 程式碼建立 Web 應用程式，並將其部署至 Azure。如需程式碼的詳細資訊，請參閱文章＜[為什選擇 Visual Studio 程式碼？](https://code.visualstudio.com/Docs/)＞。如需 App Service Web Apps 的相關資訊，請參閱＜[Web 應用程式概觀](app-service-web-overview.md)＞。
