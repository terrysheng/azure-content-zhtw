<properties 
	pageTitle="在 Azure 應用程式服務 Web 應用程式上建立數位行銷活動" 
...
# 在 Azure 應用程式服務 Web 應用程式上建立數位行銷活動
[Azure 應用程式服務](http://go.microsoft.com/fwlink/?LinkId=529714) Web 應用程式是數位行銷活動的絕佳選擇。數位行銷活動通常只有短暫週期，目的是推廣短期的行銷目標。有兩個需要考量的主要案例。在第一個案例中，協力廠商行銷公司在促銷期間為其客戶建立及管理行銷活動。第二個案例則涉及行銷公司建立數位行銷活動資源擁有權，然後將該擁有權轉移給其客戶。客戶再接著自行執行和管理數位行銷活動。這是這兩種案例的絕佳解決方案。 
...
> [AZURE.NOTE]
> 本指南說明在 Azure 應用程式服務 Web 應用程式中執行數位行銷活動最常見的區域和工作。不過，還有其他可在應用程式服務 Web 應用程式中實作的常見解決方案。若要檢閱這些解決方案，請參閱關於[全域網站空間](web-sites-global-web-presence-solution-overview.md) 與[商務應用程式](web-sites-business-application-solution-overview.md)上的其他指南。
...
您現有的 Web 資產 (無論是 .NET、PHP、Java、Node.js 或 Python) 都可以在 Web 應用程式上執行。您可以使用熟悉的 [FTP] 工具將它們移至 Web 應用程式。如果您經常建立數位行銷活動，您可能在原始檔控制管理系統已經有 Web 資產。您可以直接從受歡迎的原始檔控制選項，例如[Visual Studio]、[Visual Studio Online] 和 [Git] (本機、GitHub、BitBucket、DropBox 及 Mercurial 等) 部署至 Web 應用程式。

## 維持敏捷性

透過直接從您現有的原始檔控制持續發佈，以及在應用程式服務 Web 應用程式中執行 A/B 測試，以維持敏捷性。 

在 Web 應用程式的計劃、原型設計及初期開發期間，您和您的客戶可以透過[部署到預備位置] (在 Web 應用程式中)，以在行銷活動應用程式上線之前，先查看 Web 應用程式的實際運作版本。透過整合原始控制檔與應用程式服務 Web 應用程式，您可以[持續發佈]到預備位置，而且不需停機，在準備好時即可切換到實際執行環境。 
...
透過向受歡迎的提供者 (例如 Facebook 與 Twitter) 驗證，應用程式服務 Web 應用程式中的數位行銷活動可以和社交媒體整合。如需在 ASP.NET 應用程式使用這個方法的範例，請參閱[使用驗證和 SQL DB 建立 ASP.NET MVC 應用程式並部署至 Azure 應用程式服務]。 
...
應用程式服務 Web 應用程式[向上延展和向外延展]的能力非常適合無法預期的工作負載，這正是數位行銷活動的情況。透過 [Azure 管理入口網站](http://go.microsoft.com/fwlink/?LinkId=529715)以手動方式、透過[服務管理 API] 或 [PowerShell 指令碼處理]以程式設計方式，或是使用「自動調整」功能自動地將您的 Web 應用程式向外調整。在「標準」層中，「自動調整規模」可讓您依據 CPU 使用量自動向外調整 Web 應用程式。這項功能只會在必要時依據使用者活動向外調整 Web 應用程式，因此可協助您達到最大敏捷性，同時將成本降至最低。如需最佳做法，請參閱 [Troy Hunt] 的[我所學到關於使用 Azure 快速調整 Web 應用程式的 10 件事]。
...
[使用驗證和 SQL DB 建立 ASP.NET MVC 應用程式並部署至 Azure 應用程式服務]:web-sites-dotnet-deploy-aspnet-mvc-app-membership-oauth-sql-database.md
[向上延展和向外延展]:/manage/services/web-sites/how-to-scale-websites/
...
[向上與向外調整]:/manage/services/web-sites/how-to-scale-websites/
