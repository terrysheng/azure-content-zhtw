...
1. 在 [方案總管] 中以滑鼠右鍵按一下專案，再選擇 [發行]。
	...
	發佈設定檔包含用於部署的使用者名稱和密碼。這些認證已事先為您產生，但您可以將它們變更為您選擇的值。您將會在介紹 Azure 入口網站的教學課程章節中看到如何進行這項作業。密碼會加密並存放在 `Properties\PublishProfiles` 資料夾的使用者專屬隱藏檔案中。
...
3. [設定] 刀鋒視窗會顯示更多可用來設定您 Web 應用程式的選項。
	...
	請注意 [發佈] 區段中的 [部署認證] 連結。您會在此建立用於部署的新使用者名稱和密碼。您可以在此輸入自選的使用者名稱和密碼，然後在 Web 專案之 [發佈 Web] 精靈的 [連線] 索引標籤中輸入相同的值。
...
若要刪除 Web 應用程式，簡單的方法是按一下 Azure 入口網站的 [Web 應用程式] 刀鋒視窗頂端的 [刪除] 按鈕。更好的方法是刪除您建立來包含 Web 應用程式的資源群組。在本教學課程中，資源群組只包含 Web 應用程式，但資源群組通常包含相關資源的集合。比方說，當您不再需要用到 Web 應用程式時，該 Web 應用程式所使用的資料庫或 Azure 儲存體帳戶可能也不再需要用到。如果您刪除資源群組，就能刪除它所包含的所有項目。若要使用 Azure 入口網站刪除資源群組，請執行下列步驟。
