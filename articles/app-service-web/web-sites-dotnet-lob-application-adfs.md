...
2.	在 [WebApp-WSFederation-DotNet](https://github.com/AzureADSamples/WebApp-WSFederation-DotNet) 上複製或下載範例解決方案至您的本機目錄。
	...
	> [AZURE.NOTE] [README.md](https://github.com/AzureADSamples/WebApp-WSFederation-DotNet/blob/master/README.md) 的指示說明如何使用 Azure Active Directory 設定應用程式，但在本教學課程，您將使用 AD FS 設定它，因此請改為執行這裡的步驟。
...
6.	返回 [AD FS 管理]，在 [**新增信賴憑證者信任精靈**] 的 [**設定 URL**] 頁面中，選取 [**啟用 WS-同盟被動式通訊協定的支援**] 並輸入您在上一個步驟記下的 Visual Studio 專案的 SSL URL。然後按 [**下一步**]。
