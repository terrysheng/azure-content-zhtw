...
如果您的 Web 應用程式不在標準層 App Service 方案中，UI 會讓您知道，並在您想升級時授與您存取價格層的權限。
...
接著，系統會建立憑證以向您的虛擬網路進行驗證，確認其為訂用帳戶中第一個與該網路建立連線的 Web 應用程式。若要查看憑證，請前往 [Azure 入口網站](http://go.microsoft.com/fwlink/?LinkId=529715)、巡覽至 [虛擬網路]、選取網路，然後選取 [憑證] 索引標籤。
...
一旦網路加入 Web 應用程式之後，該 Web 應用程式將能透過 TCP 或 UDP 存取該虛擬網路中的資源。對於透過站對站 VPN 提供給虛擬網路使用的內部部署系統，如果您想要存取該系統內的資源，便需要在自己的企業網路中新增路由，以允許流量從您的網路流向虛擬網路中所設定的點對站位址。
...
##管理虛擬網路連線##
您可以造訪 App Service 方案的刀鋒視窗，查看目前與 App Service 方案內之 Web 應用程式相關聯的所有虛擬網路清單。您最多可以擁有 5 個與標準層 App Service 方案相關聯的網路。
