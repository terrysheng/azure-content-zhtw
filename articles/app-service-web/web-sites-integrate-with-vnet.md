...
>[AZURE.NOTE]最多可能需要 30 分鐘，提供新的 VNET 才能透過作業中的閘道器完成。完成時，UI 即會更新。
...
>[AZURE.NOTE]儘管 VNET 整合功能將使用站對站 VPN 存取內部部署資源，但是它目前不會使用 ExpressRoute VPN 執行相同的動作。
...
現在，如果 VNET 裝載的 VM 可以觸達內部部署系統，但應用程式不能，則原因可能是下列其中一個：- 未設定路由與內部部署閘道器中的點對站 IP 範圍搭配。- 網路安全性群組不准存取點對站 IP 範圍。- 內部部署防火牆封鎖點對站 IP 範圍中的流量。- 您在 VNET 中有使用者定義的路由 (UDR)，防止點對站型流量觸達內部部署網路
