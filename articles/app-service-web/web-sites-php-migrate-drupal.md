...
如果您正在移動大型而複雜的 Drupal 應用程式，另一個選項就是考慮使用 Azure 雲端服務。如需有關應用程式服務與雲端服務之間差異的詳細資訊，請參閱 <a href="http://go.microsoft.com/fwlink/?LinkId=310123">Azure 網站、雲端服務和 VM：使用時機</a>。如需將 Drupal 移至雲端服務的說明，請參閱<a href="http://blogs.msdn.com/b/brian_swan/archive/2012/03/19/azure-real-world-migrating-drupal-from-lamp-to-windows-azure.aspx">將 Drupal 網站從 LAMP 移轉至 Azure</a>。
 
## 建立 Web 應用程式和 MySQL 資料庫

首先，完成逐步教學課程，了解如何利用 MySQL 建立新的 Web 應用程式：[在 Azure 應用程式服務中建立 PHP-MySQL Web 應用程式並使用 Git 部署][]如果您打算使用 Git 來發行 Drupal 網站，請遵循教學課程中說明如何設定 Git 儲存機制的步驟。請務必遵循＜取得遠端 MySQL 連線資訊＞一節中的指示，因為您稍後會需要該資訊。您可以為了部署 Drupal 網站而忽略其餘的教學課程，但如果這是您第一次使用 Azure 應用程式服務 (和 Git)，您可能會發現發現其他有用文章。
...
此時，您再度需要新的資料庫連線資訊。在文字編輯器中開啟 **/drupal/sites/default/setting.php** 檔案，並以新資料庫的正確值取代 **$databases** 陣列中  'database'、 'username'、 'password' 和  'host' 的值。完成後，您會取得類似下列的程式碼：
...
如果您使用 FTP，請從 [Azure 入口網站](https://portal.azure.com)的 Web 應用程式刀鋒視窗中取得 FTP 王機名稱和使用者名稱。然後，使用任何 FTP 用戶端將 Drupal 檔案上傳至遠端站台的 **/site/wwwroot**資料夾。
...
- [Azure 應用程式服務 Web 應用程式，PHP 觀點][]
- [Azure 應用程式服務、雲端服務和 VM：使用時機][]
- [使用 .user.ini 檔案在 Azure 應用程式服務 Web 應用程式中設定 PHP][]
...
* 如需將舊的入口網站變更為新的入口網站的指引，請參閱：[瀏覽預覽入口網站的參考](http://go.microsoft.com/fwlink/?LinkId=529715)
  ...
  [在 Azure 應用程式服務中建立 PHP-MySQL Web 應用程式並使用 Git 部署]: /develop/php/tutorials/website-w-mysql-and-git/
  
  [Azure 應用程式服務 Web 應用程式，PHP 觀點]: http://blogs.msdn.com/b/silverlining/archive/2012/06/12/windows-azure-websites-a-php-perspective.aspx
  [Azure 應用程式服務、雲端服務和 VM：使用時機]: http://go.microsoft.com/fwlink/?LinkId=310123
