<properties 
	pageTitle="採用 Bottle 的 Python 網站 - Azure 教學課程" 
...
您將建立使用 Bottle Web 架構的應用程式 (請參閱本教學課程適用於 [Django](web-sites-python-create-deploy-django-app.md) 和 [Flask](web-sites-python-create-deploy-flask-app.md))。您會從 Azure 組件庫建立網站、設定 Git 部署，並於本機複製儲存機制。然後您會在本機執行應用程式、進行變更、認可和推送至 Azure。本教學課程示範如何從 Windows 或 Mac/Linux 執行這項操作。
...
- [Azure 上採用 Python Tools 2.1 for Visual Studio的 Bottle 和 MongoDB][]
- [Azure 上採用 Python Tools 2.1 for Visual Studio 的 Bottle 和 Azure 資料表儲存體][]


<!--Link references-->
[Azure 上採用 Python Tools 2.1 for Visual Studio的 Bottle 和 MongoDB]: web-sites-python-ptvs-bottle-table-storage.md
