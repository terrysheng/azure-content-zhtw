...
下列程式碼範例會將出現在*輸入*容器中的文字 Blob 複製到*輸出*容器：
...
屬性建構函式會接受字串參數，以指定容器名稱和 Blob 名稱的預留位置。在此範例中，如果已在*輸入*容器中建立名為 *Blob1.txt*的 Blob，此函數便會在*輸出*容器中建立名為 *Blob1.txt*的 Blob。
...
此程式碼只會複製以 "original-" 做為名稱開頭的 Blob。例如，*輸入* 容器中的*原始 Blob1.txt* 會複製到*輸出* 容器中的 *copy-Blob1.txt*。

如果您需要針對名稱中包含大括號的 Blob 名稱指定名稱模式，請按兩下大括號。例如，如果您想要在*影像*容器中尋找具備如下名稱的 Blob：
...
下列程式碼範例會在將出現於*輸入*容器中的 Blob 複製到*輸出*容器時變更副檔名。此程式碼會記錄*輸入* Blob 的副檔名，並將*輸出* Blob 的副檔名設為 *.txt*。
...
下列程式碼範例會使用一個類別，實作 `ICloudBlobStreamBinder` 來啟用 `BlobTrigger` 屬性，以便將 Blob 繫結至 `WebImage` 類型。
