<properties 
	pageTitle="Azure App Service API Apps 和行動裝置應用程式中的驗證" 
	description="了解如何在 Azure 應用程式服務中針對 API 應用程式和行動裝置應用程式設定和使用驗證" 
...
# Azure App Service 中之 API 應用程式與行動裝置應用程式的驗證

## 概觀

本文說明 [API 應用程式](../app-service-api/app-service-api-apps-why-best-platform.md)和[行動裝置應用程式](../app-service-mobile/app-service-mobile-value-prop-preview.md)的內建驗證功能。

本文結尾處的[後續步驟](#next-steps)一節提供相關作法文件的連結。

## Azure App Service 閘道

Azure App Service 提供內建的驗證服務，可實作 [OAuth 2.0](#oauth) 和 [OpenID Connect](#oauth) 並使用多個*身分識別提供者*。身分識別提供者是受到 Azure App Service 信任的外部服務，可驗證您的應用程式使用者。App Service 支援最熱門的身分識別提供者：
...
含有 API 應用程式或行動裝置應用程式的 Azure 資源群組都會包含*閘道*。閘道是在 Web 應用程式中執行的 Azure 資源，可處理各種系統管理工作，包括在資源群組中驗證 API 應用程式和行動裝置應用程式。
...
* 因為 App Service 會處理更多驗證工作，因此所需的開發和測試時間最少，並可避免實作 OAuth 提供者時所進行之變更的大部分或所有影響。
...
App Service 閘道提供兩種方式來驗證用戶端：*用戶端流程*和*伺服器流程*。在這兩種流程中，用戶端應用程式會將使用者認證 (一般來說是使用者名稱和密碼) 直接傳送給身分識別提供者。不管是哪一種流程，閘道和應用程式都不會收到使用者認證。

### 用戶端流程

用戶端流程表示用戶端應用程式會直接與身分識別提供者通訊來取得提供者的存取權杖。用戶端應用程式將提供者的存取權杖傳送至閘道。閘道建立使用者內容權杖並傳送給用戶端。這個使用者內容權杖亦稱為 Zumo 權杖，此名稱的由來是 [Azure 行動服務](/documentation/services/mobile-services/)的原始程式碼名稱 (API Apps 與 Mobile Apps 的驗證服務是建置在最初針對行動服務所開發的同一個架構上)。

下圖說明此流程。

![](./media/app-service-authentication-overview/clientflow.png)

用戶端接著會在呼叫受保護的 API 應用程式或行動裝置應用程式時在 HTTP 要求中提供 Zumo 權杖。閘道會儲存提供者權杖，並記錄 Zumo 權杖所關聯的提供者權杖。
...
用戶端應用程式和受保護的 API 應用程式或行動裝置應用程式之間的後續互動，其處理方式與用戶端流程相同：用戶端在 HTTP 要求中提供 Zumo 權杖。
...
當 API 應用程式或行動裝置應用程式中執行的程式碼需要代表已登入的使用者呼叫受保護的資源時，即可從閘道的權杖存放區擷取並使用身分識別提供者的權杖，如下圖所示。圖表假設用戶端已經向閘道驗證，且具有 Zumo 權杖。
...
App Service 執行階段支援和 SDK 讓撰寫程式碼來存取這些提供者之一所保護的資源變得相對容易。圖中有一個初步步驟並未顯示出來：您必須對提供者設定帳戶，並在 Azure App Service 中設定提供者的用戶端識別碼和用戶端密碼設定。
...
此外，Visual Studio 可以自動產生程式碼來使用適用於 .NET 的 SDK，進一步簡化您撰寫來呼叫 API 應用程式的程式碼。如需詳細資訊，請參閱[從 .NET 用戶端取用 Azure App Service 中的 API 應用程式](../app-service-api/app-service-api-dotnet-consume.md)。

針對 Mobile Apps，SDK 適用於下列平台：
...
目前 App Service 並不支援在閘道驗證使用自行驗證解決方案，但在[行動服務](mobile-services-dotnet-backend-get-started-custom-authentication.md)則可以支援。

### <a id="apim"></a>Azure API 管理

如果您有想要使用驗證來提供保護的現有 API，您可以在 Azure API 管理服務中這麼做。如需對 API 應用程式使用 API 管理的相關資訊，請參閱 Panos Kefalidis 的這篇部落格文章：[利用 API Apps 的 API 管理](http://www.kefalidis.me/2015/06/taking-advantage-of-api-management-for-api-apps/)

## 後續步驟

本文說明 Azure App Service 針對 API Apps 與 Mobile Apps 所提供的驗證服務。以下是用來了解基礎驗證通訊協定之資源以及如何使用 App Service 驗證功能之相關文件的一些連結。

* [OAuth 2.0、OpenID Connect 和 JSON Web 權杖 (JWT)](#oauth)
* API Apps 資源
	* [API Apps 用戶端流程](#apiaclient)
	* [API Apps 伺服器流程](#apiaserver)
	* [API Apps 代表呼叫](#apiaobo)
* Mobile Apps 資源
	* [Mobile Apps 用戶端流程](#maclient)
	* [Mobile Apps 伺服器流程](#maserver)
	* [Mobile Apps 代表呼叫](#maobo)
...
* [從 .NET 用戶端取用 Azure App Service 中的 API 應用程式](../app-service-api/app-service-api-dotnet-consume.md) - 已驗證呼叫的範例程式碼說明伺服器流程。 

### <a id="apiaobo"></a>API Apps 代表呼叫

* [在 Azure App Service 中部署和設定 SaaS 連接器 API 應用程式](../app-service-api/app-service-api-connnect-your-app-to-saas-connector.md) - 說明如何佈建預先封裝的連接器 API 應用程式、加以設定，並使用瀏覽器工具進行呼叫。
...
### <a id="maobo"></a>Mobile Apps 代表呼叫受保護的資源

* [在行動裝置應用程式中取得存取權杖和呼叫 SharePoint API](../app-service-mobile/app-service-mobile-dotnet-backend-get-started-connect-to-enterprise.md#obtain-token)
