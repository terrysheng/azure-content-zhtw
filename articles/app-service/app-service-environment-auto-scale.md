...
![][scale-rule]
 ...
 任一個**背景工作集區**或**前端**度量可以用來定義自動調整規則。這是您可以在資源刀鋒視窗圖形中監視或設定警示的相同度量資訊。
...
**App Service 環境**設為以如下方式手動調整：* 前端：3 * 背景工作集區 1：10 * 背景工作集區 2：5 * 背景工作集區 3：5
...
如果在**背景工作集區**中裝載了多個 **App Service 方案**，則需要計算**總擴大率**，並且這可以表示為正要裝載在該**背景工作集區**中的所有 **App Service 方案**擴大率的*總和*。
