...
2. 請確認您有運作中的虛擬網路，且其子網路有效。請確定沒有虛擬機器或是雲端部署正在使用子網路。應用程式閘道必須單獨在虛擬網路子網路中。
3. 您將要設定為使用應用程式閘道的伺服器必須存在，或是在虛擬網路中建立其端點，或是指派公用 IP/VIP。

## 建立應用程式閘道需要什麼？
 

當您使用 New-AzureApplicationGateway 命令建立應用程式閘道時不需要設定任何組態，且新建立的資源將必須使用 XML 或設定物件進行設定。
...
您可以使用下列方法設定應用程式閘道：XML 或設定物件。

## 使用 XML 設定應用程式閘道 

在下列範例中，您將使用 XML 檔案來設定所有的應用程式閘道設定並認可至應用程式閘道資源。
...
編輯設定項目括號之間的值。以 .xml 副檔名儲存檔案

>[AZURE.IMPORTANT]通訊協定項目 Http 或 Https 會區分大小寫。

下列範例示範如何使用組態檔設定應用程式閘道負載平衡公用連接埠 80 上的 Http 流量，並將網路流量傳送到 2 個 IP 位址之間的後端連接埠 80：
...
使用 `Set-AzureApplicationGatewayConfig` 將設定物件認可至應用程式閘道資源：
