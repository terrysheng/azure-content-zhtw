...
>[AZURE.NOTE]效能數字也取決於回傳到應用程式閘道的 Web 應用程式 HTTP 回應。


## 監視
 
應用程式閘道會使用探查連接埠監視後端執行個體的健全狀態，定期測試來自閘道的 HttpSettings 區段的 HTTP 回應。探查預期一個在 200-390 的回應程式碼範圍的成功 HTTP 回應，並且會每隔 30 秒測試一次後端 IP 範圍，以查看是否有 HTTP 回應。

收到成功的 HTTP 回應時，IP 位址會標示為狀況良好。如果探查失敗，則會從狀況良好的後端集區移除 IP 位址，流量則會停止流至此伺服器。健全狀況探查會持續每隔 30 秒流向失敗的 Web 執行個體，直到其重新上線。當 Web 執行個體成功回應健全狀況探查時，它會重新加回狀況良好的後端集區，流量會開始再次流向此執行個體。
