...
針對 SSL 憑證組態，**HttpListener** 中的通訊協定應該變更為 Https (區分大小寫)。需要將 **SslCert** 元素加入至 **HttpListener**，並將值設為上述的上傳 SSL 憑證一節中所使用的相同名稱。前端連接埠應該更新為 443。
...
**注意：****Start-AzureApplicationGateway** Cmdlet 最多可能需要15-20 分鐘才能完成。
...
使用 **Get-AzureApplicationGateway** Cmdlet 來檢查閘道狀態。如果上一個步驟中的 **Start-AzureApplicationGateway** 成功，則 *State* 應該是 Running，而且 *VirtualIPs* 和 *DnsName* 應該具有有效的項目。
