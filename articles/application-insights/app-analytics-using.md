...
[這裡有更廣泛的教學課程](app-analytics-tour.md)。

## 撰寫查詢

撰寫以任何列在左側的資料表名稱開頭的查詢。使用 `|` 建立[運算子](app-analytics-queries.md)的管線。
...
> [AZURE.NOTE] 按一下資料行的標頭，即可快速重新排列網頁瀏覽器中可用的結果。但請注意，對大型結果集而言，下載至瀏覽器的資料列數目有限。所以，以這種方式排序並不一定會顯示實際的最高或最低項目。為此，您應該使用[top](app-analytics-queries.md#top-operator) 或 [sort](app-analytics-queries.md#sort-operator) 運算子。

但使用 [take](app-analytics-queries.md#take-operator)、[top](app-analytics-queries.md#top-operator) 或 [summarize](app-analytics-queries.md#summarize-operator) 運算子，避免從伺服器下載大型資料表是相當好的做法。無論如何都自動限制每個查詢 10000 個資料列。
...
根據預設，結果一開始會顯示為資料表，而您會手動選取圖表。但您可以在查詢結尾使用 [Render 指示詞](app-analytics-queries.md#render-directive)以選取圖表。
