...
![結果](./media/app-insights-analytics-aggregations/03.png)
...
![結果](./media/app-insights-analytics-aggregations/04.png)
...
> [AZURE.NOTE] 請使用 itemCount 屬性，而不只是計算要求資料點。這會補償任何進行中的[取樣](app-insights-sampling.md)。例如，如果取樣設定於 33%，則每個資料點的 itemCount 會是 3。

#### Min 和 Max

尋找要求串流中所有記錄的最小和最大時間戳記。由於沒有 group-by 子句，因此輸出中只有一個資料列︰
...
另請參閱相反函式的 [`mvexpand` 運算子](app-insights-analytics-queries.md#mvexpand-operator)。
