...
每個以縱線字元 `|` 做為前置詞的篩選條件即為含有一些參數的運算子執行個體。前面管線所得到的結果會以資料表的形式做為運算子的輸入。大部分情況下，任何參數皆是輸入資料行的[純量運算式](app-insights-analytics-scalars.md)。但在某些情況下，參數是輸入資料行的名稱或是第二個資料表。查詢的結果永遠是資料表，即使它只有一個資料行和一個資料列。
...
`parse` 運算子透過在相同的 `string` 運算式上使用多個 `extract` 應用程式來簡化資料表的 `extend`。當資料表的 `string` 資料行有好幾個值，而您想要將這些值分割到個別資料行時，最適合使用此運算子，開發人員追蹤 ("`printf`"/"`Console.WriteLine`") 陳述式所產生的資料行便是一例。
...
引數必須是數字、日期或時間範圍值。引數不能參考任何資料表的資料行 (如果您想要根據輸入資料表計算範圍，請使用 [range 函式](app-insights-analytics-scalars.md#range)，或許再搭配 [mvexpand 運算子](#mvexpand-operator))。
...
[更多範例](app-insights-analytics-aggregations.md)。
...
* Aggregation︰`count()` 或 `avg()` 等[彙總函式](app-insights-analytics-aggregations.md)的呼叫，以資料行名稱做為引數。請參閱[彙總函式清單](app-insights-analytics-aggregations.md)。
...
* T︰要篩選記錄的表格式輸入。
* Predicate︰T 之資料行的 `boolean` [運算式](app-insights-analytics-scalars.md#boolean)。它會針對 T 中的每個資料列進行評估。
