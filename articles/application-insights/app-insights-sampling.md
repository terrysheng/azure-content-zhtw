...
* **調適性取樣**會自動調整取樣百分比，以達到特定的要求量。預設會在 Application Insights SDK 中針對 ASP.NET 2.0.0 版 beta3 或更新版本加以啟用。 
* **固定取樣率**也可在 SDK 版本 2 更新版本中使用。由您指定取樣百分比。這適用於 JavaScript 用戶端和 ASP.NET 伺服器。用戶端和伺服器會同步處理它們的取樣，讓您可以在 [搜尋] 終於相關的頁面檢視和要求之間瀏覽。
* **擷取取樣**。這會發生在 Application Insights 服務中。您的 SDK 會傳送來自您瀏覽器和伺服器的所有遙測，但我們只會保留某個百分比。這具有調適性，只會在您的 app 傳送的遙測數量超過最低數量時運作，以及只有在 SDK 未執行其他取樣類型之一時才會運作。它也會受到您使用的定價層所影響。雖然它不會減少流量，但可協助您維持在每月配額範圍內。
...
如果您使用 ASP.NET SDK 版本 2.0.0-beta3 或更新版本，調適性取樣會自動啟用。無論您使用的是哪一種 SDK 版本，如果 SDK 中沒有任何取樣，擷取取樣 (在我們的伺服器上) 可能就會運作 。
...
所有形式的取樣都只會在超過特定流量閾值時運作。例如，它將不會影響大部分的偵錯工作階段或低度使用的網站。
...
否則，建議使用調適性取樣。預設會在 ASP.NET 伺服器 SDK 中啟用此選項。

如果您未在 SDK 中使用取樣 (例如，如果您使用 Java，或者在 ApplicationInsights.config 中停用取樣)，則擷取取樣可能會在資料到達 Application Insights 服務時運作。它是一種調適性取樣。
