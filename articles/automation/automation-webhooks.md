...
| 已啟用 | 建立 Runbook 時會預設為啟用。如果您將其設定為 [停用]，則任何用戶端皆無法使用。當您建立 Webhook 時或在建立後的任何時候，您可以設定 [**啟用**] 屬性。 |
...
當用戶端使用 Webhook 啟動 Runbook 時，其無法覆寫 Webhook 中定義的參數值。若要從用戶端接收資料，Runbook 可以接受單一參數，稱之為 [[物件]] 類型的 **$WebhookData**，其中包含用戶端在 POST 要求中所包含的資料。
...
| RequestHeader | 傳入 POST 要求的標頭。 |
| RequestBody | 傳入 POST 要求的本文。 |
...
>[AZURE.NOTE]所有輸入參數的值會使用 Runbook 工作進行記錄。這表示，任何由用戶端提供的輸入會使用 $WebhookData 進行記錄，並可提供任何具有自動化工作存取權的人使用。基於這個原因，您在 Webhook 呼叫中包含機密資訊時應該特別謹慎。
...
3. 按一下 [**參數**] 來提供 Runbook 的參數值。<br>
1. 當您完成 Webhook 設定時按一下 [**確定**]。


## 使用 Webhook

若要使用 Webhook，您的用戶端應用程式必須使用 Webhook 的 URL 來發出 HTTP POST。Webhook 的語法將遵循以下列格式。

	http://<Webhook Server>/token?=<Token Value>


用戶端會收到下列其中一個反映 POST 要求的代碼。

| 代碼 | 文字 | 說明 |
|:---|:----|:---|
| 202 | 已接受 | 已接受要求，並已成功啟動 Runbook。 |
| 400 | 不正確的要求 | 基於下列原因之一不接受此要求。<ul> <li>Webhook 已過期。</li> <li>Webhook 已停用。</li> <li>在 URL 中的權杖無效。</li> </ul>|
| 500 | 內部伺服器錯誤 | URL 有效，但發生錯誤。請重新提交要求。 |

由於 Webhook 並未傳回 Runbook 工作的識別碼，因此用戶端無法判斷 Runbook 工作何時完成或其來自於 Webhook 的完成狀態。您僅能驗證要求已成功提交。

### 範例

下列範例會透過 Windows PowerShell 使用 Webhook 來啟動 Runbook。此範例包含標頭和本文中的資料，這些資料可供 Runbook 使用。請注意，任何可進行 HTTP 要求的語言皆可使用 Webhook。
...
下列範例 Runbook 會接受上一個要求，並從 Webhook 擷取資料。
