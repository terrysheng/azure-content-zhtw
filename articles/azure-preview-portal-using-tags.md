...
若要使用 PowerShell 取得訂閱內的所有標記的清單，請使用 `Get-AzureTag` Cmdlet。
...
使用 `New-AzureTag` Cmdlet 將新的標記加入至分類法。這些標記會加入到自動完成中，即使它們尚未套用至任何資源或資源群組也一樣。若要移除標記名稱/值，請先從任何可能用到此標記的資源中移除標記，再使用 `Remove-AzureTag` Cmdlet 從分類法中移除它。
