...
## 探索 Azure 虛擬機器
探索程序查詢 Azure 訂閱，以及其他資訊中的虛擬機器清單，例如雲端服務名稱和區域。
...
3. 按一下 **探索** 在頁面底部的按鈕。 ![探索按鈕](./media/backup-azure-vms/discover-button.png)
...
## 註冊 Azure 虛擬機器
可保護虛擬機器之前它需要向 Azure 備份服務註冊。註冊程序有兩個主要目標：

1. 將備份擴充功能外掛到虛擬機器中的 VM 代理程式

2. 若要將虛擬機器與 Azure Backup 服務產生關聯

註冊通常是一次性活動。Azure 備份服務會自動處理備份擴充功能的升級和修補，完全不會干擾使用者。如此可減輕備份產品經常對使用者造成的「代理程式管理負擔」。

### 若要註冊的虛擬機器

1. 瀏覽至備份保存庫，您可以找到 **復原服務** 在 Azure 入口網站，然後按一下 **登錄項目** ] 索引標籤

2. 在做為下拉式功能表中選擇的工作負載類型 **Azure 虛擬機器** ，然後按一下 [選取] 按鈕。 ![選取工作負載](./media/backup-azure-vms/discovery-select-workload.png)

3. 按一下 **註冊** 在頁面底部的按鈕。 ![註冊按鈕](./media/backup-azure-vms/register-button.png)

4. 在 [**註冊的項目**] 快顯功能表中，選擇您想要註冊的虛擬機器。如果有兩個或多個具有相同名稱的虛擬機器會使用雲端服務來區別虛擬機器。

    **註冊**作業可以大規模進行，也就是可以一次選取多個要註冊的虛擬機器。如此可大幅減少為了準備虛擬機器來備份而投入的一次性工作量。

    >[AZURE.NOTE]只有虛擬機器未註冊，而且在備份保存庫中，相同的區域中會顯示。
...
4. 這樣會開啟 [**保護項目**] 精靈，供您選取要保護的虛擬機器。如果有兩個或多個具有相同名稱的虛擬機器會使用雲端服務來區別虛擬機器。
...
## 錯誤疑難排解
您可以疑難排解資訊搭配使用 Azure Backup 在下表中列出時發生錯誤。
...
| 註冊 | 資料磁碟連接至虛擬機器數目超過支援的限制-請卸離此虛擬機器上的某些資料磁碟，然後重試作業。Azure backup 支援高達 5 個資料磁碟附加至 Azure 虛擬機器進行備份 | None |
| 註冊 | Microsoft Azure Backup 遇到內部錯誤-等候幾分鐘然後再試一次作業。如果此問題持續發生，請連絡 Microsoft 支援。 | 您可以取得這個錯誤，因為其中一個不支援下列組態： <ol><li>Premium LRS <li>多 NIC <li>負載平衡器 </ol> |
| 註冊 | 找不到 VM 客體代理程式憑證 | 請遵循下列指示以解決此錯誤： <ol><li>下載最新版的 VM 代理程式從 [這裡](http://go.microsoft.com/fwlink/?LinkID=394789&clcid=0x409)。請確定已下載的代理程式版本是 2.6.1198.718 或更高版本。<li>在虛擬機器中安裝 VM 代理程式。</ol> [學習](#validating-vm-agent-installation) 如何檢查 VM 代理程式的版本。 |
| 註冊 | 註冊失敗，安裝代理程式作業逾時 | 請檢查是否支援的虛擬機器的作業系統版本。 |
| 註冊 | 命令執行失敗-另一項作業正在進行此項目上。請在前一個作業完成之前，稍候 | None |
| 備份 | 複製 Vhd 從備份保存庫已逾時-請重試作業在幾分鐘的時間。如果此問題持續發生，請連絡 Microsoft 支援。 | 這是要複製的太多資料時。請檢查您是否擁有小於 6 的資料磁碟。 |
| 備份 | 快照集 VM 子工作已逾時-請重試操作在幾分鐘的時間。如果問題持續發生，請連絡 Microsoft 支援 | 如果 VM 代理程式發生問題，或以某種方式已封鎖到 Azure 基礎結構的網路存取，則會擲回這個錯誤。<ul><li>深入了解 [VM 代理程式問題偵錯](#Troubleshooting-vm-agent-related-issues) <li>深入了解 [偵錯網路問題](#troubleshooting-networking-issues) </ul> |
| 備份 | 備份失敗，發生內部錯誤-請重試操作在幾分鐘的時間。如果問題持續發生，請連絡 Microsoft 支援 | 您可以取得這項錯誤的兩個原因： <ol><li> 沒有太多要複製的資料。請檢查您是否擁有小於 6 的磁碟。<li>已刪除原始的 VM，因此無法進行備份。若要維持備份資料的已刪除的 VM 但停止備份錯誤，請取消保護 VM 並選擇要保留的資料。這樣會停止備份排程以及週期性的錯誤訊息。 |
| 備份 | 無法安裝 Azure 復原服務上所選取的項目-VM 代理程式延伸模組是 Azure 復原服務延伸模組的必要條件。請安裝 Azure VM 代理程式並重新啟動註冊作業 | <ol> <li>請檢查 VM 代理程式是否已正確安裝。<li>請確定已正確設定 VM 組態中的旗標。</ol> [閱讀更多](#validating-vm-agent-installation) 有關安裝 VM 代理程式，以及如何驗證安裝 VM 代理程式。 |
| 備份 | 命令執行失敗-另一項作業目前正在進行此項目上。前一個作業完成之前，請稍候，然後重試 | 現有的備份或還原作業 vm 正在執行，而且現有的作業執行時，就無法啟動新的工作。<br><br>如果您想要取消進行中的作業的選項，新增您投票給 [Azure 意見反應論壇](http://feedback.azure.com/forums/258995-azure-backup-and-scdpm/suggestions/7941501-add-feature-to-allow-cancellation-of-backup-restor)。 |

### 疑難排解 VM 代理程式相關問題

#### VM 代理程式設定
一般而言，VM 代理程式已經在從 Azure 的組件庫建立的 Vm。不過，從內部部署資料中心移轉的虛擬機器將不會包含安裝 VM 代理程式。對於這類的 Vm，必須明確安裝 VM 代理程式。進一步了解 [在現有的 VM 上安裝 VM 代理程式](http://blogs.msdn.com/b/mast/archive/2014/04/08/install-the-vm-agent-on-an-existing-azure-vm.aspx)。

適用於 Windows Vm：

- 下載並安裝 [代理程式 MSI](http://go.microsoft.com/fwlink/?LinkID=394789&clcid=0x409)。您將需要系統管理員權限，才能完成安裝。
- [更新 VM 屬性](http://blogs.msdn.com/b/mast/archive/2014/04/08/install-the-vm-agent-on-an-existing-azure-vm.aspx) 表示已安裝代理程式。


#### 更新 VM 代理程式
更新 VM 代理程式很簡單，重新安裝 [VM 代理程式的二進位檔](http://go.microsoft.com/fwlink/?LinkID=394789&clcid=0x409)。不過，您需要確定 VM 代理程式在更新時，正在執行任何備份作業。


#### 驗證 VM 代理程式安裝
如何在 Windows Vm 上的 VM 代理程式版本檢查：

- 登入到 Azure 虛擬機器並巡覽至資料夾 *C:\WindowsAzure\Packages*。
- 您應該會發現 WaAppAgent.exe 檔案存在。
- 以滑鼠右鍵按一下檔案，請移至 **屬性**, ，然後選取 **詳細資料** ] 索引標籤。
- [產品版本] 欄位應該是 2.6.1198.718 或更高版本

### 疑難排解網路問題
所有的延伸模組，例如備份延伸模組會需要存取公用網際網路運作。不需要公用網際網路存取，可能會出現在有許多種：

- 延伸安裝可能會失敗
- 備份作業 (例如磁碟快照集) 可能會失敗
- 顯示備份作業的狀態可能會失敗

需要解析網際網路的公用位址有電動 [這裡](http://blogs.msdn.com/b/mast/archive/2014/06/18/azure-vm-provisioning-stuck-on-quot-installing-extensions-on-virtual-machine-quot.aspx)。您必須檢查 VNET 的 DNS 設定，並確定 Azure Uri 可以解析。

完成的名稱解析為正確，存取 Azure Ip 也必須提供。若要解除封鎖 Azure 基礎結構的存取，請遵循下列步驟：

1. 取得清單 [Azure 資料中心 Ip](https://msdn.microsoft.com/
2. / library/azure/dn175718.aspx) 是設為允許清單。
2. 解除封鎖 Ip 使用 [新增 NetRoute](https://technet.microsoft.com/library/hh826148.aspx) cmdlet。執行這個 cmdlet 在 Azure VM 中，在提升權限的 PowerShell 視窗 (系統管理員身分執行)。


## 復原點的一致性
處理備份資料時，客戶會擔心 VM 的行為之後已還原。客戶常見的問題包括：
