...
1. 在 [**選取還原執行個體**] 畫面中，指定要將虛擬機器還原至何處的詳細資料。
  ...
  - 選取 VM 的雲端服務：這是建立 VM 的必要步驟。您可以選擇使用現有的雲端服務，或建立新的雲端服務。
        ...
        如果您選擇建立新的雲端服務，會以虛擬機器相同的名稱加以命名；如此一來，該 VM 的名稱就必須夠獨特，才能套用到相關聯的雲端服務。我們只會顯示不與同質群組的還原執行個體詳細資料相關的雲端服務和虛擬網路。[深入了解](https://msdn.microsoft.com/zh-tw/library/azure/jj156085.aspx)。
