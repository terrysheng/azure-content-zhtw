...
6. 在 [保護項目] 精靈的第三個畫面中，選擇和採用備份相關聯的保留範圍。此畫面支援以業界標準 GFS (Grandfather-Father-Son) 為根據的保留配置。進一步閱讀 [長期保留] (backup-azure-vms-introduction.md#Long 詞彙保留)。
...
### 初始備份
完成之後，虛擬機器就受到原則保護，必須等到排程的備份時間讓初始備份完成為止。現在，虛擬機器出現在 [**受保護的項目**] 索引標籤下，且 [保護狀態] 為 *受保護* (擱置中的初始備份)。
...
3. **每月的保留原則**：每月最後一個星期六所進行的備份會保留 120 個月
4. **每年的保留原則**：每年一月第一個星期六所進行的備份會保留 99 個年。
