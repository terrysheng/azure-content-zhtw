...
5. 按一下 [建立保存庫]。
    ...
    要等備份保存庫建立好，可能需要一些時間。若要檢查狀態，可以監控位於入口網站底部的通知。在建立備份保存庫之後，訊息會告訴您已成功建立保存庫，它會列出的資源做為復原服務 **Active**。
...
3. 在 **快速入門] 頁面**, ，按一下 **下載保存庫認證** 提示入口網站產生及下載保存庫認證將用於在備份保存庫註冊您的伺服器。
...
3. 在 [快速啟動] 頁面上，選取您要下載的代理程式類型。您可以選擇下載 **Azure Backup Agent**、**Windows Server 和 System Center Data Protection Manager** 或 **Windows Server Essentials**。如需詳細資訊，請參閱：
	...
	* [安裝 Azure Backup Agent for Windows Server 2012 和 System Center 2012 SP1-Data Protection Manager](http://technet.microsoft.com/library/hh831761.aspx#BKMK_installagent)
