<properties
	pageTitle="教學課程 - 開始使用 Azure 批次 .NET 程式庫"
...
本教學課程會為您示範如何建立可設定程式的主控台應用程式，以及如何支援在 Azure 批次集區的數個運算節點上執行的檔案。本教學課程中建立的工作會從 Azure 儲存體中檔案評估文字，並傳回最常使用的單字。這些範例均以 C# 程式碼撰寫，並使用 Azure 批次 .NET 程式庫。
...
- 帳戶：
	...
	- **批次帳戶**：請參閱 [Azure 批次技術概觀](batch-technical-overview.md)的**批次帳戶**一節。

	- **儲存體帳戶**：請參閱[關於 Azure 儲存體帳戶](../storage-create-storage-account.md)的**建立儲存體帳戶**一節。在本教學課程中，您會在此帳戶中建立名為 **testcon1** 的容器。
...
- NuGet 組件：
	...
	1. 在 Visual Studio 中建立專案之後，以滑鼠右鍵按一下 [**方案總管**] 中的專案，然後選擇 [**管理 NuGet 封裝**]。在線上搜尋 **Azure.Batch**，然後按一下 [安裝] 以安裝 Microsoft Azure 批次封裝與相依性。
...
1. 開啟 GettingStarted 專案的 App.config 檔案，然後將 &lt;appSettings&gt; 元素加入至 &lt;configuration&gt;。
	...
	- **[account-key]**：儲存體帳戶的主索引鍵。您可以從管理入口網站的 [儲存體] 頁面中找到主索引鍵

2. 儲存 App.config 檔案。

若要深入了解，請參閱[設定連接字串](http://msdn.microsoft.com/library/windowsazure/ee758697.aspx)。

### 建立儲存體容器

1. 將這些命名空間宣告加入至 GettingStarted 專案中的 Program.cs 頂端：

		using System.Configuration;
		using Microsoft.WindowsAzure.Storage;
		using Microsoft.WindowsAzure.Storage.Blob;

2. 將此方法加入至 Program 類別，此類別會取得儲存體連接字串、建立容器，以及設定權限：
...
若要深入了解，請參閱[如何使用 .NET 的 Blob 儲存體](../storage-dotnet-how-to-use-blobs.md)

### 建立處理程式

1. 在 [方案總管] 中，建立一個名為 **ProcessTaskData** 的新主控台應用程式專案。

2. 將此程式碼加入至處理檔案中文字的 Main 中：
...
3. 儲存並建置 ProcessTaskData 專案。

### 建立資料檔案

1. 在 GettingStarted 專案中，建立一個名為 **taskdata1** 的新文字檔案、將下列文字複製到其中，然後儲存該檔案。
...
2. 建立一個名為 **taskdata2** 的新文字檔案、將下列文字複製到其中，然後儲存該檔案。
...
3. 建立一個名為 **taskdata3** 的新文字檔案、將下列文字複製到其中，然後儲存該檔案。
...
1. 開啟 GettingStarted 專案的 Program.cs 檔案，然後加入可上傳檔案的這個方法：
...
2. 將此程式碼加入至設定認證以呼叫 Azure 批次服務的 Main 中：
...
1. 將此方法加入至 Program 類別，此類別會將三個工作加入至作業中：
	...
	**[account-name]** 必須取代成您先前建立的儲存體帳戶的名稱。請確定您取得全部四個地方。
