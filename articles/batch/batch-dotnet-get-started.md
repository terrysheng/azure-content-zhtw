<properties
	...
	description="了解 Azure 批次，以及如何針對一個簡單的案例的批次服務進行開發的基本概念"
	services="batch"
...
-	[.NET 適用的教學課程 1： Azure 的批次程式庫](#tutorial1)
-	[.NET 適用的教學課程 2： Azure 的批次應用程式程式庫](#tutorial2)  


背景資訊及 Azure 批次的情況下，請參閱 [Azure 批次技術概觀](batch-technical-overview.md)。

##<a name="tutorial1"></a>.NET 適用的教學課程 1： Azure 的批次程式庫
...
>
>您需要使用 NuGet 來取得 **Microsoft.Azure.Batch.dll** 組件。在 Visual Studio 建立專案之後，以滑鼠右鍵按一下專案中的 **[方案總管]** 選擇 **管理 NuGet 封裝**。線上搜尋 **Azure.Batch** ，然後按一下 [安裝] 來安裝 Azure 批次封裝與相依性。
>
...
>
>此外，您可以參考 [Azure 批次 Hello World 範例](https://code.msdn.microsoft.com/Azure-Batch-Sample-Hello-6573967c) msdn 類似於此處所討論的程式碼範例。
...
-	**帳戶** -唯一識別的服務內的實體。所有處理都是都透過批次帳戶執行。
-	**集區** – 工作虛擬機器上執行哪些工作應用程式的集合。
-	**工作虛擬機器** -指派給集區和工作指派給集區中執行的作業所用的機器。
-	**工作虛擬機器使用者** – 工作虛擬機器上的使用者帳戶。
-	**工作項目** -指定的集區中的工作虛擬機器上執行運算的方式。
-	**作業** -執行個體的工作項目和工作的集合所組成。
-	**工作** -應用程式，與工作相關聯的工作虛擬機器上執行的。
-	**檔案** – 包含工作所處理的資訊。  
...
1.	開啟 Microsoft Visual Studio 2013， **檔案** ] 功能表上，按一下 **新增**, ，然後按一下 **專案**。

2.	從 **Windows**, 下 **Visual C#**, ，按一下 **主控台應用程式**, ，將專案命名為 **GettingStarted**, ，將方案命名 **AzureBatch**, ，然後按一下 **[確定]**。
...
4.	將下列變數加入至 Program 類別：
	...
	取代下列值：- **[集區名稱]** -您想要用於名稱- **[名稱的批次帳戶]** -批次帳戶。 名稱- **[索引鍵的批次帳戶]** -批次帳戶提供給您的索引鍵。 集。
...
2.	建立工作項目時，也會建立作業。您可以指派名稱給工作項目，但作業一定會被指派的名稱 **作業 0000000001**。將下列程式碼加入至 Main (在清單工作項目程式碼之前)，以加入工作項目：
		...
		Console.ReadLine();
3.	儲存並執行程式。狀態是 **Active** 已成功新增的工作項目。您應該會看見下列輸出。
...
1.	在 [方案總管] 中建立新的主控台應用程式專案，名為 **Azuretasks** 中 **AzureBatch** 方案。
...
>[AZURE.NOTE]Azure 儲存體團隊有 [部落格文章](http://blogs.msdn.com/b/windowsazurestorage/archive/2014/03/11/windows-azure-storage-explorers-2014.aspx) 列出可協助您將檔案上傳的 Azure 儲存體總管。
...
1.	將下列變數加入至 Program 類別：
	...
	取代下列值：- **[儲存路徑]** -blob 儲存體中的路徑。例如： http://yiding.blob.core.windows.net/gettingstarted/
...
2.	在導覽窗格的底部，按一下 **新增**。 ![][1]
3.	按一下 **DATA SERVICES**, ，然後 **儲存體**, ，然後按一下 **快速建立**。 ![][2]

4.	在 **URL**, ，輸入要用於儲存體帳戶 URI 中的子網域名稱。此項目可以包含 3 至 24 個小寫字母與數字。此值會成為 URI 內用來將訂閱的 Blob、「佇列」或「表格」資源定址的主機名稱。
5.	選擇 **位置/同質群組** 要在其中尋找儲存體。
6.	(選用) 您可以啟用地理區域複寫。
7.	按一下 [建立儲存體帳戶]。  

如需有關 Azure 儲存體的詳細資訊，請參閱 [如何在.NET 中使用 Azure Blob 儲存體服務](http://azure.microsoft.com/develop/net/how-to-guides/blob-storage/)。


##<a name="tutorial2"></a>.NET 適用的教學課程 2： Azure 的批次的應用程式程式庫
...
>[AZURE.NOTE]若要完成此教學課程，您需要 Azure 帳戶。只需要幾分鐘的時間，您就可以建立免費試用帳戶。如需詳細資訊，請參閱 [Azure 免費試用](http://azure.microsoft.com/pricing/free-trial/)。您可以使用 NuGet 來取得兩者 <a href="http://www.nuget.org/packages/Microsoft.Azure.Batch.Apps.Cloud/">批次的應用程式定域機組</a> 組件和 <a href="http://www.nuget.org/packages/Microsoft.Azure.Batch.Apps/">批次的應用程式用戶端</a> 組件。在 Visual Studio 建立專案之後，以滑鼠右鍵按一下專案中的 **[方案總管]** 選擇 **管理 NuGet 封裝**。您也可以下載 Visual Studio 擴充功能的批次應用程式，其中包括專案範本即可定域機組啟用應用程式和部署應用程式的能力 <a href="https://visualstudiogallery.msdn.microsoft.com/8b294850-a0a5-43b0-acde-57a07f17826a">這裡</a> 或透過搜尋 **批次的應用程式** 在 Visual Studio 中透過擴充功能和更新功能表項目。您也可以找到 <a href="https://go.microsoft.com/fwLink/?LinkID=512183&clcid=0x409">MSDN 上的端對端範例。</a>
>
...
3.	提供方法將作業提交至 Azure 中啟用的應用程式。這可能是您的應用程式 UI 或 Web 入口網站中的外掛程式，或甚至是執行管線中的自動服務。請參閱 <a href="https://go.microsoft.com/fwLink/?LinkID=512183&clcid=0x409">範例</a> 範例的 MSDN 上。
...
####作業
A **作業** 是由使用者提交驚奇的成果。當提交作業時，使用者將指定作業的類型、這項作業的任何設定，以及作業所需的資料。啟用的實作可以代替使用者決定這些詳細資料，或者，在某些情況下，使用者可以透過用戶端明確地提供這項資訊。作業會傳回結果。每個作業都有主要輸出，可能還有預覽輸出。如有需要，作業也可以傳回額外的輸出。

####工作
A **工作** 是一段工作需要完成工作的一部分。當使用者提交作業時，它會細分為較小的工作。然後，服務會處理這些個別的工作，再將工作結果組合成完整的作業輸出。工作的本質視作業類型而定。作業分割器會根據應用程式主要處理的工作部分，定義如何將作業細分為工作。也可以下載個別工作輸出，在某些情況下可能很有用，例如使用者想要從動畫作業中下載個別的工作。

####合併工作
A **合併工作** 是一種特殊的工作來組合成最終作業結果的個別工作的結果。在影片呈現作業中，合併工作可能將呈現的畫面組合成影片，或將所有呈現的畫面壓縮成單一檔案。每個作業即使實際上不需要「合併」，也都會有合併工作。

####檔案
A **檔案** 是一種做為輸入至作業的資料。作業可能沒有相關聯的輸入檔，也可能有一或多個輸入檔。相同的檔案可用於多個工作，例如電影轉譯作業，檔案可能是材質、 模型等。資料分析工作，檔案可能是一組觀察或測量。
