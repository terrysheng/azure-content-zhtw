...
批次支援不同的開發人員案例，可協助您設定及執行大規模的平行工作負載與批次服務。這些案例會利用 API 來建立和管理 VM 集區 (運算節點)，以及排程的作業及其中執行的工作。如需有關批次概念的詳細資訊，請參閱 [Azure 批次的 API 基本概念](batch-api-basics.md)。
...
您可以使用預覽批次應用程式 API 來包裝現有的應用程式，以便在批次於背景中管理的運算節點集區上做為服務執行。應用程式可能是現今在用戶端工作站或計算叢集上執行的應用程式。您可以開發服務，讓使用者將尖峰工作卸載到雲端，或完全在雲端中執行其工作。批次應用程式架構會處理輸入和輸出檔案的移動、將作業分割成工作、作業和工作處理，以及資料保存。

>[AZURE.IMPORTANT]Azure 只會將提供預覽格式的批次應用程式 API。您只應該針對測試或概念證明專案開發它。在未來的服務版本中，會將關鍵的批次應用程式功能整合到批次 API。
...
2.	建立**雲端組件**的 zip 檔，該組件會叫用並分派工作負載至批次服務。這包含兩個元件：
