...
如需深入了解如何處理資料一致性，請參閱 Microsoft 網站上的[資料一致性入門](http://msdn.microsoft.com/library/dn589800.aspx)頁面。
...
>
> 此外，Azure 支援角色中快取。這項功能可讓您建立雲端服務專用的快取。由 Web 或背景工作角色的執行個體所託管的快取，僅能由以相同雲端服務部署單位 (部署單位是以雲端服務部署至特定區域的角色執行個體集合) 一部分來操作的角色進行存取。快取已叢集化，且裝載快取的同一個部署單位中所有角色的執行個體，都會變成相同快取叢集的一部分。不過，此選項不再建議使用，且僅對於已建置使用此選項的現有應用程式提供支援。針對所有新的開發工作，請改用 Azure Redis 快取。
>
> Azure 受管理的快取服務和 Azure In-Role Cache 目前都預定在 2016 年 11 月 16 日淘汰。我們建議您移轉到 Azure Redis Cache，來為這次淘汰做準備。如需詳細資訊，請瀏覽 Microsoft 網站上的「[應該使用哪個 Redis 快取供應項目和大小？](redis-cache/cache-faq.md#what-redis-cache-offering-and-size-should-i-use)」網頁。
...
Microsoft 網站上的[Azure Redis Cache 文件](https://azure.microsoft.com/documentation/services/cache/)頁面，提供如何編寫能使用 Azure Redis Cache 的用戶端應用程式的詳細資訊。其他資訊可從 StackExchange.Redis 網站上的[基本使用方式頁面 (英文)](https://github.com/StackExchange/StackExchange.Redis/blob/master/Docs/Basics.md) 和相同網站上的[流水線和多工器 (英文)](https://github.com/StackExchange/StackExchange.Redis/blob/master/Docs/PipelinesMultiplexers.md) 頁面取得，可提供更多關於非同步作業和流水線搭配 Redis 與 StackExchange 程式庫的資訊。本指引中稍後＜Redis 快取的使用案例＞一節，提供一些更進階的技術範例，以便您套用至保留在 Redis 快取中的資料。
...
- Microsoft 網站上的 [Azure Redis 快取常見問題](redis-cache/cache-faq.md)頁面。
...
- Redis 網站上的[將 Redis 當做 LRU 快取來使用 (英文)](http://redis.io/topics/lru-cache) 頁面。
- Redis 網站上的[交易 (英文)](http://redis.io/topics/transactions) 頁面。
- Redis 網站上的 [Redis 安全性 (英文)](http://redis.io/topics/security)。
- Azure 部落格上的[瀏覽 Azure Redis 快取 (英文)](https://azure.microsoft.com/blog/2014/06/04/lap-around-azure-redis-cache-preview/) 頁面。
...
- Microsoft 網站上的 [Azure Redis 快取的 ASP.NET 工作階段狀態提供者](redis-cache/cache-asp.net-session-state-provider.md)頁面。
