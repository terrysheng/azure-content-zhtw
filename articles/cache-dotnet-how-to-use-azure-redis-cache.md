...
使用 Azure Redis 快取建立的快取，可透過任何 Azure 應用程式來存取。在 Visual Studio 中開發的 .NET 應用程式可以使用 **StackExchange.Redis** 快取用戶端，此用戶端可使用能簡化快取用戶端應用程式組態的 NuGet 封裝來加以設定。 
...
與「Azure Redis 快取」的連線是由 `ConnectionMultiplexer` 類別管理。此類別的設計是要在用戶端應用程式中共用和重複使用，而不需要依據每個作業加以建立。 
...
建立連線之後，就會透過呼叫 `ConnectionMultiplexer.GetDatabase` 方法，傳回對 Redis 快取資料庫的參考。

	// connection referes to a previously configured ConnectionMultiplexer
	IDatabase cache = connection.GetDatabase();

>[AZURE.NOTE] 透過 `GetDatabase` 方法傳回的物件是輕量型傳遞物件，不需要儲存。
...
藉由使用 `StringSet` 和 `StringGet` 方法，即可在快取中儲存或擷取項目。
...
呼叫 `StringGet` 時，如果物件已經存在，便會傳回該物件，如果物件不存在，則會傳回 Null。在此情況下，您可以從需要的資料來源中擷取值，並將它儲存在快取中供後續使用。這稱為另行快取模式。
