...
- TCP\_HIT - 直接從 POP 提供要求給用戶端時會回報此狀態。當資產在最接近用戶端的 POP 上快取且具有有效的 TTL 時，它會立即由 POP 提供。TTL 由下列回應標頭決定：
...
- TCP\_EXPIRED \_HIT - 當要求的目標是有效時間 (TTL) 已過期的資產 (例如當資產的 max-age 已過期) 且目標直接從 POP 提供給用戶端時，就會回報此狀態。
...
此報告指出直接從快取處理的快取要求百分比。此報告提供下列詳細資料：
