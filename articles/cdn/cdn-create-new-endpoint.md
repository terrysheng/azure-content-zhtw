...
CDN 設定檔就是 CDN 端點的集合。每個設定檔包含一或多個 CDN 端點。您可能會想要使用多個設定檔，依網際網路網域、Web 應用程式或其他一些準則來組織您的 CDN 端點。
...
3. 輸入這個 CDN 端點的**名稱**。這個名稱會用於存取位於網域 `<EndpointName>.azureedge.net` 的快取資源。
