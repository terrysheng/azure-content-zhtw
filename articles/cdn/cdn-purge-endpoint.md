...
4. 選取您希望從邊緣節點清除的資產。如果您希望清除所有資產，請按一下 [清除] 核取方塊。或者，在 [路徑] 文字方塊中輸入每個您想要清除之資產的完整路徑 (例如，`/pictures/kitten.png`)。
	...
	>
	> 路徑必須是符合下列[規則運算式](https://msdn.microsoft.com/library/az24scfc.aspx)：`^\/(?:[a-zA-Z0-9-_.\u0020]+\/)**$";` 的相對 URL。星號 (*) 可用為萬用字元 (例如 `/music/*`)。

5. 按一下 [清除] 按鈕。

	![清除按鈕](./media/cdn-purge-endpoint/cdn-purge-button.png)

> [AZURE.NOTE] 清除要求大約需要 2-3 分鐘來處理。每個 CDN 設定檔都有每分鐘 10 個清除要求的限制。
