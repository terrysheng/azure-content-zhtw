...
<p>這項工作中的程序適用於 Azure 雲端服務；對於「網站」，請參閱<a href="../web-sites-configure-ssl-certificate/">設定 Azure 網站的 SSL 憑證</a>。</p>
</div>

此工作包含下列步驟：

-   [步驟 1：取得 SSL 憑證][步驟 1：取得 SSL 憑證]
-   [步驟 2：修改服務定義檔與組態檔][步驟 2：修改服務定義檔與組態檔]
-   [步驟 3：上傳部署套件和憑證][步驟 3：上傳部署套件和憑證]
-   [步驟 4：使用 HTTPS 連線至角色執行個體][步驟 4：使用 HTTPS 連線至角色執行個體]

此工作將使用生產部署；本主題最後將提供關於如何使用預備部署的資訊。

## <a name="step1"> </a><span class="short-header">取得 SSL 憑證</span>步驟 1：取得 SSL 憑證
...
基於測試目的，您可以建立並使用自我簽署憑證。自我簽署憑證不是由 CA 驗證，因此可以使用 cloudapp.net 網域做為網站 URL。例如，以下工作即使用自我簽署憑證，該憑證中使用的一般名稱 (CN) 為 **sslexample.cloudapp.net**。如需關於如何使用 IIS 管理員建立自我簽署憑證的詳細資訊，請參閱[如何建立角色的憑證][如何建立角色的憑證] (英文)。

接下來，您必須在服務定義檔與服務組態檔中加入憑證的相關資訊。

## <a name="step2"> </a><span class="short-header">修改服務 / 組態檔案</span>步驟 2：修改服務定義檔與組態檔

您的應用程式必須已設定為使用憑證，而且您必須新增 HTTPS 端點。因此，您需要更新服務定義檔與服務組態檔。

1.  在開發環境中，開啟服務定義檔
    (CSDEF)，在 **WebRole** 區段內加入 **Certificates**
    區段，並包括下列憑證相關
    資訊：
    ...
    **Certificates** 區段定義憑證的名稱、位置，以及其所在的存放區名稱。我們已選擇將憑證儲存在 CA (憑證授權單位) 存放區中，但是您也可以選擇其他選項。如需詳細資訊，請參閱[如何使憑證與服務產生關聯][如何使憑證與服務產生關聯] (英文)。

2.  在服務定義檔中，於 **Endpoints** 區段內
    加入 **InputEndpoint** 元素，以啟用 HTTPS：
...
3.  在服務定義檔中，於 **Sites** 區段內
    加入 **Binding** 元素。如此會加入 HTTPS 繫結，以將端點
    對應至您的站台：
    ...
    如此即已對服務定義檔完成所有必要變更，
    但是您仍然需要將憑證資訊新增至服務組態檔。

4.  在服務組態檔 (CSCFG) (ServiceConfiguration.Cloud.cscfg) 中，於 **Role** 區段內
    加入 **Certificates** 區段，以將下面顯示的範例
    指紋值取代為憑證的指紋值：
...
您已更新服務定義檔與服務組態檔，
現在請封裝您的部署，以上傳至 Azure。如果
您是使用 **cspack**，請確定未使用
**/generateConfigurationFile** 旗標，因為如此會覆寫
您剛插入的憑證資訊。

## <a name="step3"> </a><span class="short-header">上傳至 Azure</span>步驟 3：上傳部署套件和憑證

您的部署套件已更新為使用該憑證，而且
您已加入 HTTPS 端點。現在您可以利用管理入口網站
將套件和憑證上傳至 Azure。
...
4.  在 [Publish your cloud service] 對話方塊中，輸入雲端服務的必要資訊、為環境選取 [生產]，並確定核取 [Add certificates now]。(如果您的任一個角色包含單一執行個體，請確定核取 [Deploy even if one or more roles contain a single instance]。)

    ![發佈您的雲端服務][發佈您的雲端服務]

5.  按 [下一步] 按鈕。
6.  在 [加入憑證] 對話方塊中，輸入 SSL
    憑證檔 .pfx 的位置、憑證的密碼，然後按一下
    [附加憑證]。

    ![新增憑證][新增憑證]

7.  確定您的憑證列在 [Attached Certificates] 區段內。

    ![附加的憑證][附加的憑證]

8.  按一下 [完成] 按鈕，以建立雲端服務。當部署達到了 [就緒] 狀態時，您可以繼續進行接下來的步驟。

## <a name="step4"> </a><span class="short-header">使用 HTTPS 連線</span>步驟 4：使用 HTTPS 連線至角色執行個體

您的部署已在 Azure 啟動並在執行中，現在您可以使用 HTTPS 來與其連線。
...
2.  在網頁瀏覽器中，將連結修改為使用 **https** 而非 **http**，然後造訪網頁。
    ...
    **注意：**如果使用自我簽署憑證，則當您
    在瀏覽器中瀏覽至與該自我簽署憑證相關聯的 HTTPS 端點時，
    您將會看見憑證錯誤。使用
    信任的憑證授權單位所簽署的憑證，則不會有這個問題；因此，您可以忽略該錯誤。(另一個選項為新增自我簽署憑證至使用者的受信任憑證授權單位憑證存放區。)
...
## <a name="additional_resources"> </a><span class="short-header">其他資源</span>其他資源

-   [如何使憑證與服務產生關聯 (英文)][如何使憑證與服務產生關聯]

-   [如何在 HTTPS 端點上設定 SSL 憑證 (英文)][如何在 HTTPS 端點上設定 SSL 憑證 (英文)]

  [步驟 1：取得 SSL 憑證]: #step1
