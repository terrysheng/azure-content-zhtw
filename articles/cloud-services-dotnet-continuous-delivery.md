...
3.	遵循[這些指示](http://msdn.microsoft.com/library/dd647551.aspx) 為建置流程範本新增活動專案、下載預設範本，將它新增至專案並簽入。為建置流程範本提供新名稱，例如 AzureBuildProcessTemplate。

3.  回到 [**處理序**] 索引標籤，並使用 [**顯示詳細資料**] 以顯示可用建置流程範本的清單。選擇 [**新增...**] 按鈕，並導覽至您剛剛新增並簽入的專案。找到您剛建立的範本並選擇 [**確定**]。
