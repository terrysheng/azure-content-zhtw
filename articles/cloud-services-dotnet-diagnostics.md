...
<tbody>
	...
	<tr>
		...
		<td>您的程式碼所產生的事件，方法是使用 .NET <a href="http://msdn.microsoft.com/library/system.diagnostics.tracing.eventsource(v=vs.110).aspx">EventSource 類別</a>。</td>            
	</tr>
	<tr>
		<td>以資訊清單為基礎的 ETW</td>
...
3.	將 WadConfig.xsd 與組態檔產生關聯。確定 WadExample.xml 編輯器視窗是使用中視窗。按下 **F4** 開啟 [**屬性**] 視窗。在 [**屬性**] 視窗中，按一下 [**結構描述**] 屬性。在 [**結構描述**] 屬性中，按一下 [**...**]。按一下 [**加入...**] 按鈕並瀏覽至您儲存 XSD 檔的位置，然後選取檔案 WadConfig.xsd。按一下 **[確定]**。
...
<h3>步驟 6：查看您的遙測資料</h3>
在 Visual Studio 的 [**伺服器總管**] 中，瀏覽至 wadexample 儲存體帳戶。執行雲端服務約 5 分鐘之後，您應該會看到資料表**WADEnumsTable**、**WADHighFreqTable**、**WADMessageTable**、**WADPerformanceCountersTable** 和 **WADSetOtherTable**。按兩下其中一個資料表以檢視收集的遙測資料。
...
<h3>步驟 6：查看您的遙測資料</h3>
在 Visual Studio 的 [**伺服器總管**] 中，瀏覽至 wadexample 儲存體帳戶。執行 VM 約 5 分鐘之後，您應該會看到資料表**WADEnumsTable**、**WADHighFreqTable**、**WADMessageTable**、**WADPerformanceCountersTable** 和 **WADSetOtherTable**。按兩下其中一個資料表以檢視收集的遙測資料。
...
診斷組態檔定義當診斷監視器啟動時，用來初始化診斷組態設定的值。您可以在下列位置找到範例組態檔及有關其結構描述的詳細文件：[Azure 診斷 1.2 組態結構描述][]。
...
**問：**如何將我的 Visual Studio 解決方案從 Azure 診斷 1.0 升級至 Azure 診斷 1.1？

**答：**將 Visual Studio 解決方案從診斷 1.0 升級至診斷 1.1 (或更新版本) 是手動程序：
...
**答：**如果您在安裝診斷 1.1 時指定 "-Version "1.*""，於下一次重新啟動您的角色或 VM 重新開機時，將會自動更新為符合規則運算式 "1.*" 的最新版本。如果在您安裝 Diagnostics 1.1 時 "-Version "1.1""，您可以更新較新的版本，方法是重新執行 Set- Cmdlet，並指定要安裝的版本。
...
<h2><a name="comparing"></a>比較 Azure 診斷版本</h2>
...
<tbody>
	<tr>
			...
			<td><strong>診斷 1.1/1.2/1.3</strong></td>
	</tr>
