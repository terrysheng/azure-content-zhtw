...
本教學課程示範如何建立具有 ASP.NET MVC 前端的多層式 .NET 應用程式，並將它部署到 [Azure 雲端服務][Azure 雲端服務]。應用程式會使用 [Azure SQL Database][Azure SQL Database]、[Azure Blob 服務][Azure Blob 服務] 和 [Azure 佇列服務][Azure 佇列服務]。您可以從 MSDN Code Gallery [下載 Visual Studio 專案][下載 Visual Studio 專案]。
...
此應用程式會使用[以佇列為中心的工作模式][Azure 佇列服務]，將建立縮圖的 CPU 密集工作轉變為後端程序。

### 替代架構：網站和 WebJob

本教學課程示範如何在 Azure 雲端服務中執行前端與後端。替代方式是在 [Azure 網站][Azure 網站]中執行前端，並且將 [WebJob][WebJob] 功能 (目前處於預覽) 用於後端。如需使用 WebJob 的教學課程，請參閱[開始使用 Azure WebJob SDK][開始使用 Azure WebJob SDK]。如需如何選擇最符合您的案例之服務的詳細資訊，請參閱 [Azure 網站、雲端服務和虛擬機器比較][Azure 網站、雲端服務和虛擬機器比較] (英文)。
...
本教學課程假設您了解 [Azure 雲端服務的基本概念][Azure 雲端服務的基本概念]，例如「Web 角色」和「背景工作角色」術語。同時也假設您知道如何在 Visual Studio 中使用 [ASP.NET MVC][ASP.NET MVC] 或 [Web Form][Web Form] 專案。範例應用程式使用 MVC，但大多數的教學課程內容亦適用於 Web Form。

您不需 Azure 訂閱即可在本機執行應用程式，但需要訂閱才能將應用程式部署至雲端。如果您沒有這類帳戶，可以[啟用自己的 MSDN 訂戶權益][啟用自己的 MSDN 訂戶權益]或是[申請免費試用][申請免費試用]。
...
使用者上傳影像時，執行 Web 角色的前端會將影像儲存在 [Azure Blob][Azure Blob 服務]，並將廣告資訊儲存在資料庫中，加上指向 Blob 的 URL。同時會將訊息寫入 Azure 佇列。執行背景工作角色的後端程序會定期輪詢佇列，看看是否有新訊息。出現新訊息時，背景工作角色會建立該影像的縮圖，並更新該廣告的縮圖 URL 資料庫欄位。以下圖表顯示應用程式的這些部分的互動情況：
...
6.  按 CTRL+F5 執行應用程式。
    ...
    在本機執行雲端服務專案時，Visual Studio 會自動叫用 Azure「計算模擬器」和 Azure「儲存體模擬器」。計算模擬器會使用您電腦的資源來模擬 Web 角色和背景工作角色環境。儲存體模擬器則使用 [SQL Server Express LocalDB][SQL Server Express LocalDB] 資料庫來模擬 Azure 雲端儲存體。
...
8.  輸入部分測試資料，選取要上傳的 *.jpg* 影像，然後按一下 [建立]。

    ![Create page][Create page]

    應用程式會進入索引頁面，但不會顯示新廣告的縮圖，因為處理尚未發生。

9.  稍候片刻，然後重新整理索引頁面以查看縮圖。

    ![索引頁面][Ad list]

10. 按一下廣告的 [詳細資料] 以查看完整大小的影像。
...
您將對 Web 角色使用 [Web.config 轉換][Web.config 轉換]，以及對背景工作角色使用雲端服務環境設定。

> [WACOM.NOTE] 在本節與下一節中，您會將憑證儲存在專案檔中。[請勿將敏感性資料儲存在公用原始程式碼存放庫][請勿將敏感性資料儲存在公用原始程式碼存放庫]。
...
2.  在 Azure 管理入口網站中，依序按一下左側窗格的 [SQL Databases]、您在此教學課程中建立的資料庫、[儀表板] 索引標籤和 [顯示連接字串]。
...
3.  在 *Web.Release.config* 轉換檔中，刪除 `{connectionstring}` 並將來自管理入口網站的 ADO.NET 連接字串貼上至其位置。

4.  在您貼上至 *Web.Release.config* 轉換檔的連接字串中，將 `{your_password_here}` 以您為新 SQL 資料庫建立的密碼取代。
...
10. 選取 `ContosoAdsDbConnectionString` 設定中的文字，然後將您在前一節的教學課程中複製的連接字串貼上。
...
4.  在 [建立儲存體帳戶連接字串] 對話方塊中，按一下 [使用您的訂閱連線]、選取您稍早建立的儲存體帳戶，然後按 [確定]。如果您尚未登入，將提示您輸入 Azure 帳戶憑證。
...
`<Instances>` 設定指定 Azure 將執行背景工作角色所在的虛擬機器數量。[後續步驟][後續步驟]一節包含向外擴大雲端服務之詳細資訊的連結。

### 將專案部署至 Azure

1.  在 [方案總管] 的 **ContosoAdsCloudService** 雲端專案上按一下滑鼠右鍵，然後選取 [發行]。

    ![Publish menu][Publish menu]

2.  在 [發行 Windows Azure Application Web] 精靈的 [登入] 步驟中，按 [下一步]。
...
3.  在精靈的 [設定] 步驟中，按 [下一步]。
    ...
    [進階] 索引標籤中的預設設定適用於本教學課程。如需進階索引標籤的詳細資訊，請參閱[發行 Azure 應用程式精靈][發行 Azure 應用程式精靈]。
...
3.  您可以透過建立、檢視和編輯一些廣告來測試應用程式，正如同您在本機執行應用程式一般。

> [WACOM.NOTE] 完成測試後，請刪除或停止雲端服務。即使您未使用雲端服務，因為已為其保留虛擬機器資源，因此仍會累積費用。如果您讓它保持執行，找到您 URL 的任何人都可以建立和檢視廣告。在 Azure 管理入口網站中，移至您雲端服務的 [儀表板] 索引標籤，然後按一下頁面底端的 [刪除] 按鈕。如果您只想暫時避免他們存取網站，請改為按一下 [停止]。在那個情況下，將持續累積費用。當您不再需要 SQL 資料庫和儲存體帳戶時，可以遵循類似程序來加以刪除。

## 從頭開始建立應用程式

如果您尚未下載
[完整應用程式][下載 Visual Studio 專案]，請現在下載。您會從所下載的專案複製檔案到新專案。
...
2.  在 [新增專案] 對話方塊的左窗格中，展開 [Visual C#]，然後依序選取 [雲端] 範本和選取 [Windows Azure 雲端服務] 範本。
...
6.  在 [變更驗證] 對話方塊中，選取 [不需要驗證]，然後按一下 [確定]。
...
9.  在 [加入新的專案] 對話方塊中，在左窗格的 [Visual C#] 下選擇 [Windows 桌面]，然後按一下 [類別庫] 範本。
...
3.  尋找 *Azure Storage* 封裝，而如果它不在清單中，請按一下 [更新] 來取得最新版本的儲存體用戶端程式庫。
...
3.  在 ContosoAdsWorker 專案中，設定 `System.Drawing` 的參考。
...
1.  在 ContosoAdsWeb 專案中，開啟應用程式 Web.config 檔案，並在 `configSections` 元素之後插入下列 `connectionStrings` 元素。
...
-   [開始使用 MVC 5][ASP.NET MVC]
-   [開始使用 EF 6 和 MVC 5][開始使用 EF 6 和 MVC 5]
-   [.NET 4.5 中非同步程式設計的簡介][.NET 4.5 中非同步程式設計的簡介] (英文)。
...
Azure 背景工作角色環境會在背景工作角色開始時呼叫 `WorkerRole` 類別中的 `OnStart` 類別，並且會在 `OnStart` 方法結束時呼叫 `Run` 方法。
...
`ProcessQueueMessage` 會在找到佇列訊息時呼叫。
...
> [WACOM.NOTE] `ConvertImageToThumbnailJPG` 方法中的程式碼會使用 System.Drawing namespace 中的類別。不過，此命名空間中類別的設計原意是要與 Windows Form 搭配使用。不支援將它們用於 Windows 或 ASP.NET 服務。
...
Contoso Ads 應用程式特意保持簡潔，以做為入門的教學課程。例如，它不會實作[相依性插入][相依性插入]或[存放庫和工作單位模式][存放庫和工作單位模式]、不會[使用介面來記錄][使用介面來記錄]、不會使用 [EF Code First 移轉][EF Code First 移轉]來管理資料模型變更或 [EF 連線復原][EF 連線復原]來管理暫時性網路錯誤等等。

以下是可示範更多真實世界程式碼編寫作法的一些雲端服務範例應用程式，依較不複雜到較複雜的順序列出：

-   [PhluffyFotos][PhluffyFotos]。概念類似 Contoso Ads，但實作更多功能和更多真實世界程式碼編寫實務。
-   [具有表格、佇列和 Blob 的 Azure 雲端服務多層式應用程式][具有表格、佇列和 Blob 的 Azure 雲端服務多層式應用程式]。介紹 Azure 儲存體表格以及 Blob 和佇列，並隨附[逐步教學課程系列][逐步教學課程系列]。
-   [Windows Azure 中的雲端服務基礎][Windows Azure 中的雲端服務基礎]。完整範例示範由 Microsoft 模式和作法小組提供的許多最佳作法。

如需針對雲端進行開發的一般相關資訊，請參閱[使用 Windows Azure 建置真實世界的雲端應用程式][使用 Windows Azure 建置真實世界的雲端應用程式]。

如需 Azure 儲存體最佳作法和模式的簡介影片，請參閱 [Microsoft Azure 儲存體 - 新功能、最佳作法和模式][Microsoft Azure 儲存體 - 新功能、最佳作法和模式]。

如需詳細資訊，請參閱下列資源：

-   [Azure 雲端服務第 1 部分：簡介][Azure 雲端服務第 1 部分：簡介]
-   [Azure 雲端服務][3]
-   [Azure 儲存體][Azure 儲存體]
...
-   [如何使用 .NET 的佇列儲存體][如何使用 .NET 的佇列儲存體]
  ...
  [Azure 網站]: /zh-tw/services/web-sites/
  ...
  [啟用自己的 MSDN 訂戶權益]: /zh-tw/pricing/member-offers/msdn-benefits-details/?WT.mc_id=A55E3C668
  [申請免費試用]: /zh-tw/pricing/free-trial/?WT.mc_id=A55E3C668
  ...
  [發行 Azure 應用程式精靈]: http://msdn.microsoft.com/library/windowsazure/hh535756.aspx
  ...
  [.NET 4.5 中非同步程式設計的簡介]: http://www.asp.net/aspnet/overview/developing-apps-with-windows-azure/building-real-world-cloud-apps-with-windows-azure/web-development-best-practices#async
