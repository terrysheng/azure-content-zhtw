<properties pageTitle="開始使用 Azure 雲端服務和 ASP.NET" metaKeywords="Azure tutorial, Azure storage tutorial, Azure multi-tier tutorial, MVC Web Role tutorial, Azure worker role tutorial, Azure blobs tutorial, Azure queues tutorial" description="了解如何使用 ASP.NET MVC 和 Azure 建立多層式應用程式。應用程式在雲端服務中執行，具備 Web 角色和背景工作角色。它使用 Entity Framework、SQL Database 及 Azure 儲存體佇列和 Blob。" metaCanonical="" services="cloud-services,storage" documentationCenter=".NET" title="Get Started with Azure Cloud Services and ASP.NET" authors="tdykstra,riande" solutions="" manager="wpickett" editor="mollybos" />
...
此應用程式會使用[以佇列為中心的工作模式](http://www.asp.net/aspnet/overview/developing-apps-with-windows-azure/building-real-world-cloud-apps-with-windows-azure/queue-centric-work-pattern)，以將需要大量 CPU 的建立縮圖工作移交給後端程序。 

### 替代架構：網站和 WebJob

本教學課程示範如何在 Azure 雲端服務中執行前端與後端。替代方法是在 [Azure 網站]中執行前端，(/zh-tw/services/web-sites/) 然後使用 [WebJobs](http://go.microsoft.com/fwlink/?LinkId=390226) 功能 (目前為預覽版) 做為後端。如需有關使用 WebJobs 的教學課程，請參閱[開始使用 Azure WebJobs SDK](/zh-tw/documentation/articles/websites-dotnet-webjobs-sdk-get-started/)。如需有關如何最適合您案例之服務的資訊，請參閱 [Azure 網站、雲端服務與虛擬機器之比較](http://azure.microsoft.com/zh-tw/documentation/articles/choose-web-site-cloud-service-vm/)。
...
本教學課程假設您了解 [Azure 雲端服務的基本概念](http://azure.microsoft.com/zh-tw/documentation/articles/fundamentals-application-models/#CloudServices)，例如「Web 角色」和「工作角色」術語。同時也假設您知道如何在 Visual Studio 中使用 [ASP.NET MVC](http://www.asp.net/mvc/tutorials/mvc-5/introduction/getting-started) 或 [Web Forms](http://www.asp.net/web-forms/tutorials/aspnet-45/getting-started-with-aspnet-45-web-forms/introduction-and-overview) 專案。範例應用程式使用 MVC，但大多數的教學課程內容亦適用於 Web Form。 

您不需 Azure 訂閱即可在本機執行應用程式，但需要訂閱才能將應用程式部署至雲端。如果您沒有帳戶，您可以[啟用您的 MSDN 訂閱者權益](/zh-tw/pricing/member-offers/msdn-benefits-details/?WT.mc_id=A55E3C668) 或[註冊免費試用](/zh-tw/pricing/free-trial/?WT.mc_id=A55E3C668)。
...
- [設定開發環境](#setupdevenv)
- [下載並執行已完成的方案](#download-and-run-the-completed-solution)
- [將應用程式部署至 Azure](#deploy-the-application-to-azure)
- [從頭開始建立應用程式](#create-the-application-from-scratch)
- [疑難排解](#troubleshooting) (如果您在執行範例時遇到問題，請到此處。)
- [後續步驟](#next-steps)
...
當使用者上傳影像時，以 Web 角色執行的前端會將影像儲存在 [Azure Blob](http://www.asp.net/aspnet/overview/developing-apps-with-windows-azure/building-real-world-cloud-apps-with-windows-azure/unstructured-blob-storage) 中，並將廣告資訊連同指向該 Blob 的 URL 儲存在資料庫中。同時會將訊息寫入 Azure 佇列。執行背景工作角色的後端程序會定期輪詢佇列，看看是否有新訊息。出現新訊息時，背景工作角色會建立該影像的縮圖，並更新該廣告的縮圖 URL 資料庫欄位。以下圖表顯示應用程式的這些部分的互動情況：
...
1. 下載並解壓縮[已完成的方案](http://code.msdn.microsoft.com/Simple-Azure-Cloud-Service-e01df2e4)。

2. 啟動 Visual Studio。

3. 從 [檔案]**** 功能表中選擇 [開啟專案]****，瀏覽至方案的下載位置，然後開啟該方案檔。
...
3. 在 [方案總管]**** 中，確定已選取 [ContosoAdsCloudService]**** 做為啟始專案。

1. 按 CTRL+F5 執行應用程式。

	在本機執行雲端服務專案時，Visual Studio 會自動叫用 Azure「計算模擬器」和 Azure「儲存體模擬器」。計算模擬器會使用您電腦的資源來模擬 Web 角色和背景工作角色環境。儲存體模擬器則使用 [SQL Server Express LocalDB](http://msdn.microsoft.com/zh-tw/library/hh510202.aspx) 資料庫來模擬 Azure 雲端儲存體。 
...
2. 輸入一些測試資料、選取要上傳的 *.jpg* 影像，然後按一下 [建立]****。
...
在下一節中，您會將方案設定為在雲端中執行時，將 Azure 雲端資源用於佇列、Blob 和應用程式資料庫。如果您想要繼續在本機執行，但仍使用雲端儲存體和資料庫資源，您可以那麼做；只要設定連接字串即可，您將會看到做法。 
...
4. 在 URL 輸入方塊中，輸入 URL 首碼。 

	URL 必須是唯一的。如果他人已使用您選擇的首碼，您將會收到錯誤訊息。

5. 選擇您要部署應用程式的區域。

	此欄位指出雲端服務將託管所在的資料中心。針對生產應用程式，您會選擇離客戶最近的區域。針對此教學課程，請選擇離您最近的區域。
...
1. 在「Azure 管理入口網站」中，按一下 [新增]**** - [資料服務]**** - [SQL Database]**** - **快速建立**。
...
1. 輸入系統管理員的 [登入名稱]**** 和 [密碼]****。 
	...
	若選取 [新的 SQL 資料庫伺服器]****，則不要在此處輸入現有的名稱和密碼，而是輸入新的名稱和密碼；您現在定義的名稱和密碼將供未來存取資料庫時使用。如果選取先前建立的伺服器，系統會提示您提供先前建立之管理使用者帳戶的密碼。

1. 按一下 [建立 SQL Database]****。

	![New SQL Database](./media/cloud-services-dotnet-get-started/newdb.png)	

1. 在 Azure 完成建立資料庫之後，按一下入口網站左窗格中的 [SQL Database]**** 索引標籤，然後按一下新資料庫的名稱。

2. 按一下 [儀表板]**** 索引標籤。

3. 按一下 [管理允許的 IP 位址]****。

4. 在 [允許的服務]**** 下，將 [Azure 服務]**** 變更為 [是]****。
...
4. 在 [URL]**** 方塊中，輸入 URL 首碼。 

	此首碼加上您在方塊下面看到的文字，將是您儲存體帳戶的唯一 URL。如果您輸入的首碼已有人使用，就必須選擇不同的首碼。

5. 將 [地區]**** 下拉式清單設定成您為雲端服務選擇的相同地區。

	當雲端服務與儲存體帳戶位於不同的資料中心 (不同的區域) 時，延遲時間會增加，而且您將由於使用資料中心外的頻寬而付費。資料中心內的頻寬則是免費的。

	Azure 同質群組提供一種機制，可將資料中心內的資源之間的距離縮至最短，因而可以減少延遲時間。本教學課程不會使用同質群組。如需詳細資訊，請參閱[如何在 Azure 中建立同質群組](http://msdn.microsoft.com/zh-tw/library/jj156209.aspx)。
...
5. 按一下 [建立儲存體帳戶]****。 
	...
	在此影像中，已建立一個 URL 為 `contosoads.core.windows.net` 的儲存體帳戶。

### 將方案設定為在 Azure 中執行時使用 Azure SQL 資料庫

每個 Web 專案和背景工作角色專案都有自己的資料庫連接字串，在 Azure 中執行應用程式時，每個都必須指向 Azure SQL 資料庫。

您將針對 Web 角色使用 [Web.config 轉換](http://www.asp.net/mvc/tutorials/deployment/visual-studio-web-deployment/web-config-transformations)，針對背景工作角色使用雲端服務環境設定。

>[WACOM.NOTE] 在本節與下一節中，您將把認證儲存在專案檔中。[請勿將機密資料儲存在公用原始程式碼存放庫中](http://www.asp.net/aspnet/overview/developing-apps-with-windows-azure/building-real-world-cloud-apps-with-windows-azure/source-control#secrets)。

1. 在 ContosoAdsWeb 專案中，開啟應用程式 *Web.config* 檔案的 *Web.Release.config* 轉換檔、刪除包含 `<connectionStrings>` 元素的註解區塊，並在其位置上貼上下列程式碼。 
...
2. 在「Azure 管理入口網站」中，依序按一下左窗格中的 [SQL Databases]****、您為本教學課程建立的資料庫、[儀表板]**** 索引標籤，然後按一下 [顯示連接字串]****。
...
4. 在 *Web.Release.config* 轉換檔中，刪除 `{connectionstring}`，然後在其位置上貼上來自管理入口網站的 ADO.NET 連接字串。

5. 在您貼到 *Web.Release.config* 轉換檔的連接字串中，以您為新 SQL Database 建立的密碼取代 `{your_password_here}`。

7. 儲存檔案。  

6. 選取並複製連接字串 (不含含括的引號)，在以下設定背景工作角色專案的步驟中將會使用該資訊。

5. 在 [方案總管]**** 中，於雲端服務中 [角色]**** 下的 [ContosoAdsWorker]**** 上按一下滑鼠右鍵，然後按一下 [屬性]****。
...
7. 選取 `ContosoAdsDbConnectionString` 設定中的文字，然後將您在本教學課程中前一節複製的連接字串貼上。
...
4. 在 [方案總管]**** 中，於 [ContosoAdsCloudService]**** 專案中 [角色]**** 下的 [ContosoAdsWeb]**** 上按一下滑鼠右鍵，然後按一下 [屬性]****。

	![Role properties](./media/cloud-services-dotnet-get-started/roleproperties.png)	

5. 按一下 [設定]**** 索引標籤。在 [服務組態]**** 下拉式方塊中，選擇 [雲端]****。

	![Cloud configuration](./media/cloud-services-dotnet-get-started/sccloud.png)	

6. 選取 [StorageConnectionString]**** 項目，該行右端就會出現省略符號 (**...**) 按鈕。按一下省略符號按鈕以開啟 [建立儲存體帳戶連接字串]**** 對話方塊。

	![Open Connection String Create box](./media/cloud-services-dotnet-get-started/opencscreate.png)	

1. 在 [建立儲存體連接字串]**** 對話方塊中，按一下 [使用您的訂用帳戶連線]****，選擇您之前建立的儲存體帳戶，然後按一下 [確定]****。如果您尚未登入，將提示您輸入 Azure 帳戶憑證。

	![Create Storage Connection String](./media/cloud-services-dotnet-get-started/createstoragecs.png)	

1. 儲存您的變更。

2. 依照用於 `StorageConnectionString` 連接字串的相同程序，設定 `Microsoft.WindowsAzure.Plugins.Diagnostics.ConnectionString` 連接字串。

	此連接字串用於記錄。

2. 依照用於 **ContosoAdsWeb** 角色的相同程序，為 **ContosoAdsWorker** 角色設定那兩個連接字串。請記得將 [服務組態]**** 設定為 [雲端]****。
...
而 *ServiceConfiguration.Cloud.cscfg* 檔案則包括您在 Visual Studio 中為這些設定輸入的值：
...
`<Instances>` 設定指定 Azure 將在其上執行背景工作角色程式碼的虛擬機器數目。[後續步驟](#next-steps) 一節包含向外延展雲端服務的相關詳細資訊連結。
...
2. 在 [發行 Windows Azure 應用程式]**** 精靈的 [登入]**** 步驟中，按 [下一步]****。
...
3. 在精靈的 [設定]**** 步驟中，按 [下一步]****。
	...
	[進階]**** 索引標籤中的預設設定適用於本教學課程。如需進階索引標籤的相關資訊，請參閱[發行 Azure 應用程式精靈](http://msdn.microsoft.com/library/windowsazure/hh535756.aspx)。

2. 在 [摘要]**** 步驟中，按一下 [發行]****。

	![Summary step](./media/cloud-services-dotnet-get-started/pubsummary.png)	
   
   [Azure 活動記錄檔]**** 即會在 Visual Studio 中開啟。 
...
>[WACOM.NOTE] 完成測試後，請刪除或停止雲端服務。即使您未使用雲端服務，因為已為其保留虛擬機器資源，因此仍會累積費用。如果您讓它保持執行，找到您 URL 的任何人都可以建立和檢視廣告。在 Azure 管理入口網站中，移至您雲端服務的 [儀表板]**** 索引標籤，然後按一下頁面底端的 [刪除]**** 按鈕。如果您只想要暫時避免其他人存取網站，請改為按一下 [停止]****。在那個情況下，將持續累積費用。當您不再需要 SQL 資料庫和儲存體帳戶時，可以遵循類似程序來加以刪除。

## 從頭開始建立應用程式 

如果您尚未下載 
[已完成的應用程式](http://code.msdn.microsoft.com/Simple-Azure-Cloud-Service-e01df2e4)，請立即下載。您會從所下載的專案複製檔案到新專案。
...
1. 在 Visual Studio 中，從 [檔案]**** 選擇 [新增專案]****。

2. 在 [新增專案]**** 對話方塊的左窗格中，展開 [Visual C#]****，然後選擇 [雲端]**** 範本，再選擇 [Windows Azure 雲端服務]**** 範本。
...
5. 看見 Web 角色的 [新增 ASP.NET 專案]**** 對話方塊時，選擇 MVC 範本，然後按一下 [變更驗證]****。
...
9. 在 [方案總管]**** 中，於方案 (不是其中一個專案) 上按一下滑鼠右鍵，然後選擇 [加入 - 新專案]****。
...
13. 尋找「Azure 儲存體」封裝，如果它不在清單中，請按一下 [更新]**** 以取得最新版的儲存體用戶端程式庫。
...
10. 在 ContosoAdsWeb 專案中，設定 ContosoAdsCommon 專案的參考。在 [ContosoAdsWeb] 專案上按一下滑鼠右鍵，然後按一下 [參考]**** - [加入參考]****。在 [參考管理員]**** 對話方塊中，於左窗格中選取 [方案 - 專案]****，選取 [ContosoAdsCommon]，然後按一下 [確定]****。

11. 在 ContosoAdsWorker 專案中，設定 ContosAdsCommon 專案的參考。

	ContosoAdsCommon 將包含將由前端與後端使用的 Entity Framework 資料模型和內容類別。

11. 在 ContosoAdsWorker 專案中，設定對 `System.Drawing` 的參考。
...
3. 在 ContosoAdsWeb 專案中，開啟應用程式 Web.config 檔案，然後在 `configSections` 元素之後插入下列 `connectionStrings` 元素：
...
3. 在 [ContosAdsWeb **[Role]**] 屬性視窗中，按一下 [設定]**** 索引標籤，然後按一下 [加入設定]****。

	將 [服務組態]**** 保持設定為 [所有組態]****。

4. 加入名為 *StorageConnectionString* 的新設定。將 [類型]**** 設定為 *ConnectionString*，並將 [值]**** 設定為 *UseDevelopmentStorage=true*。
...
若要將檔案加入到專案或資料夾中，請在專案或資料夾上按一下滑鼠右鍵，然後按一下 [加入]**** - [現有項目]****。選取您需要的檔案，然後按一下 [加入]****。如果詢問您是否要取代現有的檔案，請按一下 [是]****。
...
3. 在 ContosoAdsWeb 專案中，從所下載的專案加入下列檔案。
	- *Global.asax.cs*  
	- 在 *Views\Shared* 資料夾中： <em>_Layout.cshtml</em>。
...
* [.NET 4.5 中的非同步程式設計簡介](http://www.asp.net/aspnet/overview/developing-apps-with-windows-azure/building-real-world-cloud-apps-with-windows-azure/web-development-best-practices#async)
...
如果尚無 *images* Blob 容器和 *images* 佇列，從 `Application_Start` 方法呼叫的程式碼便會建立它們。這可確保每當您開始使用新儲存體帳戶或開始在新電腦上使用儲存體模擬器時，將自動建立所需的 Blob 容器和佇列。

此程式碼使用來自 *.cscfg* 檔案的儲存體連接字串，來存取儲存體帳戶。
...
類似的程式碼可取得對 *images* 佇列的參考，並建立新佇列。在此情況下，即不需要變更權限。
...
在 HttpPost `Create` 方法上傳 Blob 並更新資料庫之後，它會建立佇列訊息，通知後端程序已有一個影像備妥可供轉換成縮圖。

		string queueMessageString = ad.AdId.ToString();
		var queueMessage = new CloudQueueMessage(queueMessageString);
		await queue.AddMessageAsync(queueMessage);

HttpPost `Edit` 方法的程式碼與此類似，不同點在於如果使用者選取一個新影像檔，就必須刪除任何現有的 Blob。
...
Azure 背景工作角色環境會在背景工作角色開始時，呼叫 `WorkerRole` 類別中的 `OnStart` 方法，然後在 `OnStart` 方法執行完成時，呼叫 `Run` 方法。

`OnStart` 會從 *.cscfg* 檔案取得資料庫連接字串，然後將其傳遞給 Entity Framework DbContext 類別。預設會使用 SQLClient 提供者，使得您不需指定提供者。

		var dbConnString = CloudConfigurationManager.GetSetting("ContosoAdsDbConnectionString");
		db = new ContosoAdsContext(dbConnString);

之後，方法會取得儲存體帳戶的參考，並建立 Blob 容器和佇列 (如果尚不存在)。該動作的程式碼類似於您已在 Web 角色 `Application_Start` 方法中看到的程式碼。

### ContosoAdsWorker - WorkerRole.cs - Run 方法

當 `OnStart` 方法完成其初始化工作時，便會呼叫 `Run` 方法。此方法會執行無限迴圈，監看是否有新的佇列訊息，並在訊息抵達時加以處理。
...
在每次迴圈的反覆運算之後，如果沒有找到佇列訊息，程式會休息一秒。如此可避免背景工作角色產生過度的 CPU 時間和儲存體交易成本。「Microsoft 客戶諮詢小組」曾經遇過一個案例，就是有開發人員忘記將這一點納入考量就部署到生產環境，然後去度假。當他回到工作崗位時，因為他疏忽所造成的代價比度假費用還高。

有時佇列訊息的內容會造成處理錯誤。這稱為「有害訊息」，如果您只是記錄錯誤並重新啟動迴圈，可能就會不斷嘗試處理該訊息。因此，Catch 區塊包含 If 陳述式，檢查以查看應用程式已嘗試處理目前訊息的次數，而如果已超過 5 次，即會從佇列中刪除訊息。

當發現佇列訊息時，便會呼叫 `ProcessQueueMessage`。
...
>[WACOM.NOTE] 為了簡單起見，`ConvertImageToThumbnailJPG` 方法中的程式碼會使用 System.Drawing namespace 中的類別。不過，此命名空間中類別的設計原意是要與 Windows Form 搭配使用。不支援將它們用於 Windows 或 ASP.NET 服務。
...
當您在 Azure 中執行應用程式，或使用 Azure 計算模擬器在本機執行時，Azure 會提供 `RoleEnvironment` 物件。如果您在本機執行時遇到此錯誤，請確定您已將 ContosoAdsCloudService 專案設為啟動專案。如此可將專案設定為使用 Azure 計算模擬器執行。

應用程式使用 Azure RoleEnvironment 的其中一個用途是，取得儲存在 *.cscfg* 檔案中的連接字串值，因此，此例外狀況的另一個原因就是遺漏連接字串。請確定您在 ContosoAdsWeb 專案中同時為雲端和本機組態建立 StorageConnectionString 設定，也在 ContosoAdsWorker 專案中為這兩個組態建立連接字串。如果您在整個方案中執行 [全部尋找]**** 搜尋來尋找 StorageConnectionString，您應該會看見它在 6 個檔案中出現 9 次。

### 無法覆寫連接埠 xxx。低於最小值的新連接埠，對通訊協定 http 允許的值為 8080

請嘗試變更 Web 專案使用的連接埠號碼。在 [ContosoAdsWeb] 專案上按一下滑鼠右鍵，然後按一下 [屬性]****。按一下 [Web]**** 索引標籤，然後變更 [專案 URL]**** 設定中的連接埠號碼。
...
* [具有表格、佇列和 Blob 的 Azure 雲端服務多層式應用程式](http://code.msdn.microsoft.com/windowsazure/Windows-Azure-Multi-Tier-eadceb36)。介紹「Azure 儲存體資料表」以及 Blob 和佇列，並隨附[逐步教學課程系列](http://azure.microsoft.com/zh-tw/documentation/articles/cloud-services-dotnet-multi-tier-app-storage-1-overview/)。
...
如需「Azure 儲存體」最佳做法和模式的簡介影片，請參閱 [Microsoft Azure 儲存體 - 新功能、最佳做法和模式](http://channel9.msdn.com/Events/Build/2014/3-628)。
