<properties urlDisplayName="How to create and deploy" pageTitle="如何建立和部署雲端服務 - Azure" metaKeywords="Azure creating cloud service, deleting cloud service" description="了解如何在 Azure 中使用「快速建立」方法來建立和部署雲端服務。" metaCanonical="" services="cloud-services" documentationCenter="" title="如何建立和部署雲端服務" authors="ryanwi" solutions="" manager="timlt" editor="" />
...
**注意** 如果您計劃從 Windows Team Foundation Services (TFS) 發佈您的雲端服務，請使用「快速建立」，然後從**快速啟動**或儀表板設定 TFS 發佈。如需詳細資訊，請參閱[使用 Visual Studio Online 連續傳遞至 Azure][使用 Visual Studio Online 連續傳遞至 Azure] 或**快速啟動**頁面的說明。
...
-   [準備您的應用程式][準備您的應用程式]
-   [開始之前][開始之前]
-   [作法：使用「快速建立」建立雲端服務][作法：使用「快速建立」建立雲端服務]
-   [作法：上傳雲端服務的憑證][作法：上傳雲端服務的憑證]
-   [作法：部署雲端服務][作法：部署雲端服務]
...
Azure SDK 提供準備這些必要部署檔案的工具。您可以從 [Azure 下載][Azure 下載] (英文) 頁面安裝 SDK，使用您偏好的語言開發應用程式程式碼。

三個雲端服務功能需要特別組態，您才能匯出服務封裝：

-   如果您要部署使用安全通訊端層 (SSL) 進行資料加密的雲端服務，請針對 SSL 設定應用程式。如需詳細資訊，請參閱[設定 HTTPS 端點的 SSL 憑證][設定 HTTPS 端點的 SSL 憑證]。

-   如果您要設定角色執行個體的遠端桌面連線，請設定遠端桌面的角色。如需有關準備遠端存取服務定義檔的詳細資訊，請參閱＜[設定 Azure 中角色的遠端桌面連接][設定 Azure 中角色的遠端桌面連接]＞。

-   如果您要設定雲端服務的詳細資訊監視，請啟用雲端服務的 Azure 診斷。*最小監視* (預設監視層級) 使用從角色執行個體 (虛擬機器) 的主機作業系統收集的效能計數器。「詳細資訊監視」會按照角色執行個體內的效能資料收集其他度量，以便進一步分析應用程式處理期間發生的問題。若要了解如何啟用 Azure 診斷，請參閱[啟用 Azure 診斷][啟用 Azure 診斷] (英文)。

-   若要使用 Web 角色或背景工作角色的部署來建立雲端服務，您必須建立服務封裝。如需封裝的相關檔案的詳細資訊，請參閱＜[設定 Azure 的雲端服務][設定 Azure 的雲端服務]＞。若要建立封裝檔，請參閱＜[封裝 Azure 應用程式][封裝 Azure 應用程式]＞。如果您使用 Visual Studio 來開發應用程式，請參閱＜[使用 Azure Tools 發行雲端服務][使用 Azure Tools 發行雲端服務]＞。

## <span id="begin"></span></a>開始之前

-   如果您尚未安裝 Azure SDK，請按一下 [Install Azure SDK] 開啟 [Azure 下載頁面][Azure 下載] (英文)，然後對於開發程式碼所偏好使用的語言下載 SDK。(您稍後將有機會這麼做。)

-   如果任何角色執行個體需要憑證，請建立憑證。雲端服務需要含有私密金鑰的 .pfx 檔。您建立並部署雲端服務時，可以將憑證上傳至 Azure。如需憑證的相關資訊，請參閱＜[管理憑證][管理憑證]＞。

-   如果您計劃將雲端服務部署至同質群組，請建立同質群組。您可以使用同質群組，將雲端服務和其他 Azure 服務部署到區域中的同一個位置。您可以在管理入口網站的 [網路] 區域中出現的 [同質群組] 頁面上建立同質群組。如需詳細資訊，請參閱＜[在管理入口網站中建立同質群組][在管理入口網站中建立同質群組]＞。

## <span id="quick"></span></a>作法：使用「快速建立」建立雲端服務

1.  在[管理入口網站][管理入口網站]中，按一下 [新增]\>[計算]\>[雲端服務]\>[快速建立]。
...
1.  在[管理入口網站][管理入口網站]中，依序按一下 [雲端服務]、雲端服務的名稱及 [憑證]。
...
5.  按一下 [確定] (勾選記號)。
    ...
    您可以在訊息區域查看上傳的進度，如下所示。上傳完成時，憑證將新增至表格中。在訊息區域中，按一下 [確定] 關閉訊息。

    ![CloudServices\_CertificateProgress][CloudServices\_CertificateProgress]

## <span id="deploy"></span></a>作法：部署雲端服務

1.  在[管理入口網站][管理入口網站]中，依序按一下 [雲端服務]、雲端服務的名稱及 [儀表板]。

    儀表板會在生產環境中開啟，此時您可以選取 [預備]，將應用程式部署在預備環境中。如需詳細資訊，請參閱＜[在 Azure 中管理部署][在 Azure 中管理部署]＞。
...
<!-- -->
  ...
  [設定 Azure 中角色的遠端桌面連接]: http://msdn.microsoft.com/zh-tw/library/hh124107.aspx
  [啟用 Azure 診斷]: http://www.windowsazure.com/zh-tw/develop/net/common-tasks/diagnostics/
  [設定 Azure 的雲端服務]: http://msdn.microsoft.com/zh-tw/library/hh124108.aspx
  [封裝 Azure 應用程式]: http://msdn.microsoft.com/zh-tw/library/hh403979.aspx
  [使用 Azure Tools 發行雲端服務]: http://msdn.microsoft.com/zh-tw/library/ff683672.aspx
  [管理憑證]: http://msdn.microsoft.com/zh-tw/library/gg981929.aspx
  [在管理入口網站中建立同質群組]: http://msdn.microsoft.com/zh-tw/library/jj156209.aspx
  ...
  [在 Azure 中管理部署]: http://msdn.microsoft.com/zh-tw/library/gg433027.aspx
