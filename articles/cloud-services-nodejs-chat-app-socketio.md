...
Socket.IO 提供 node.js 伺服器和用戶端之間的即時通訊。本教學課程會逐步引導您將 socket.IO 型交談應用程式裝載於 Azure 上。
如需 Socket.IO 的詳細資訊，請參閱 <http://socket.io/>。
...
在 Azure 模擬器中測試應用程式之前，必須稍做一些修改。請對 server.js 檔案執行下列步驟：
...
3.  為了確保應用程式會在正確的連接埠上接聽，請在 [記事本] 或您喜歡的編輯器中開啟 server.js，然後變更下一行程式碼，使用 **process.env.port** 來取代 **3000**，如下所示：
...
儲存 server.js 的變更之後，使用下列步驟來安裝必要的模組，然後在 Azure 模擬器中測試應用程式：
...
2.  當瀏覽器視窗開啟時，請輸入暱稱，然後按 Enter 鍵。這樣可讓您以特定暱稱來張貼訊息。若要測試多使用者功能，可使用相同 URL 來開啟其他瀏覽器視窗，並輸入不同的暱稱。
