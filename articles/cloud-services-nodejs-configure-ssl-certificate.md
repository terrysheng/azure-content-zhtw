...
本文中的步驟僅適用於 Web 角色中裝載成 Azure 雲端服務的節點應用程式。關於網站，請參閱＜[設定 Azure 網站的 SSL 憑證](../web-sites-configure-ssl-certificate/)＞。
...
**Publish-AzureServiceProject** Cmdlet 傳回的 **Created Web Site URL** 值包含您的託管應用程式的完整網域名稱。您需要取得此特定完整網域名稱的 SSL 憑證並部署至 Azure。
...
1.  在 **[存放區位置]** 區段中，選取 **[目前使用者]**，然後按 **[下一步]**。這樣會將憑證安裝到您使用者帳戶的憑證存放區。

2.  繼續執行精靈，接受預設值，一直到出現 **[私密金鑰保護]** 畫面為止。在這裡，您必須輸入憑證的密碼 (若有的話)。您也必須選取 **[將這個金鑰設成可匯出]**。最後，按 **[下一步]**。

<!-- -->

    ![private key protection][key-protection]

1.  繼續執行精靈，接受預設值，一直到順利安裝憑證為止。
