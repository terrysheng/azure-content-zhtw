...
[Cloud9 IDE][] 提供跨平台瀏覽器型開發環境。Cloud9 針對 Node.js 專案所支援的其中一個功能，是您可以從 IDE 直接部署至 Azure。Cloud9 也會與 GitHub 和 BitBucket 儲存機制服務整合，因此可以輕鬆地與其他人共用您的專案。

使用 Cloud9，您可以從許多現代瀏覽器和作業系統開發應用程式並將它部署至 Azure，而不需要在本機安裝其他開發工具或 SDK。下面的步驟示範在 Mac 上使用 Google Chrome。
...
5.  將會顯示標題為 **Untitled1** 的新索引標籤。請在 [Untitled1]**** 索引標籤上輸入下列程式碼，以建立 Node 應用程式：
...
6.  若要儲存程式碼，請選取 [檔案]****，然後選取 [另存新檔]****。在 [另存新檔]**** 對話方塊中，輸入 **server.js** 做為檔案名稱，然後按一下 [儲存]****。
...
若要將應用程式部署至 Azure，您需要有帳戶。如果您還沒有 Azure 帳戶，則可以遵循下列步驟來註冊免費試用版：
...
3.  如果這是您第一次建立使用 Azure 的 Cloud9 部署，則必須設定 Azure 發佈設定。請執行下列步驟來下載這些設定，並將其安裝至 Cloud9：
    ...
    3.  在 [Add a deploy target]**** 對話方塊中，選取 [選擇檔案]****，然後選取上一個步驟中下載的檔案。
...
5.  系統將會提示您輸入新託管服務的名稱，以及執行個體數目、主機 OS 和資料中心這類組態選項。指定的部署名稱將會用作 Azure 中的託管服務名稱。在 Azure 系統內，此名稱必須是唯一的。
	...
	> [AZURE.NOTE]在 [Add a deploy target]**** 對話方塊的 [Choose existing deployment]**** 區段下，會列出任何現有 Azure 託管服務；如果選取現有託管服務，則會將此專案部署至該服務。

	> [AZURE.NOTE]如果選取 [啟用 RDP]**** 並提供使用者名稱和密碼，則會啟用您部署的遠端桌面。


## 部署至 Azure 生產環境

1.  選取您在先前步驟中建立的部署。將會顯示對話方塊，其提供此部署的資訊，以及將在部署至 Azure 之後使用的生產 URL。
...
將服務部署至生產或預備環境時，會將虛擬 IP 位址 (VIP) 指派給該環境中的服務。當您想要將服務從預備環境移動至生產環境時，只要執行 VIP 交換就可達成，而不需要重新部署，而 VIP 交換會交換預備和生產部署。VIP 交換會將測試過的預備應用程式放到生產環境中，而生產環境中沒有停機時間。如需詳細資料，請參閱[在 Azure 中管理部署][]。
...
3.  在網頁左側，選取 [託管服務、儲存體帳戶和 CDN]****，然後按一下 [託管服務]****。
	...
	![Azure 管理入口網站][Azure Management Portal]
...
如果您已在建立部署時啟用 RDP 並指定使用者名稱和密碼，則可以選取特定執行個體，然後選取功能區上的 [連線]，以使用 [遠端桌面] 連線到託管服務。

![Connect to an instance](./media/cloud-services-nodejs-develop-deploy-cloud9/connect.png)

當您按一下 [連線] 時，系統會提示您開啟或下載 .RDP 檔案。此檔案包含連線到遠端桌面工作階段所需的資訊。如果在 Windows 系統上執行此檔案，則系統會提示您輸入在建立部署時所輸入的使用者名稱和密碼，接著會將您連線到所選取執行個體的桌面。

> [AZURE.NOTE]用來連線到應用程式託管執行個體的 .RDP 檔案只適用於 WIndows 上的 [遠端桌面] 應用程式。
...
[設定雲端服務的大小]: http://msdn.microsoft.com/library/windowsazure/ee814754.aspx
[在 Azure 中管理部署]: http://msdn.microsoft.com/library/windowsazure/gg433027.aspx
