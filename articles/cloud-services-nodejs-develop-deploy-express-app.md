<properties urlDisplayName="Web App with Express" pageTitle="使用 Express 來建置 Web 應用程式 (Node.js) - Azure 教學課程" metaKeywords="Azure Node.js hello world 教學課程, Azure Node.js hello world, Azure Node.js 開始使用教學課程, Azure Node.js 教學課程, Azure Node.js Express 教學課程" description="以雲端服務教學課程為基礎的教學課程，示範如何使用 Express 模組。" metaCanonical="" services="cloud-services" documentationCenter="nodejs" title="Build a Node.js web application using Express on an Azure Cloud Service" authors="larryfr" solutions="" manager="wpickett" editor="" />
...
2. 切換至 **c:\\node** 目錄，然後輸入下列命令來建立名為 **expressapp** 的新方案和名為 **WebRole1** 的 Web 角色：
	...
	> [WACOM.NOTE] **Add-AzureNodeWebRole** 預設會使用較舊版的 Node.js。上方的 **Set-AzureServiceProjectRole** 陳述式會指示 Azure 使用 0.10.21 版本的節點。 
...
7.  修改 **server.js**，從下列程式碼行中移除其中一個 '.' 字元。
...
1.  輸入下列命令來開啟 index.jade 檔案：
    ...
    Jade 是 Express 應用程式使用的預設檢視引擎。如需有關 Jade 檢視引擎的詳細資訊，請參閱 [http://jade-lang.com][]。
...
測試應用程式之後，請使用 **Stop-AzureEmulator** 來停止模擬器。
