...
若要使用遠端桌面，您必須使用在雲端中進行角色執行個體驗證的使用者名稱、密碼和憑證，設定您的服務定義和服務組態。[Azure PowerShell](http://go.microsoft.com/?linkid=9790229&clcid=0x409) 包含 **Enable-AzureServiceProjectRemoteDesktop** Cmdlet，可為您執行這項設定。
...
1.  在 [Azure 管理入口網站](http://manage.windowsazure.com)中選取 **[雲端服務]**，然後選取先前在步驟 1 中部署的服務
...
5.  按一下 **[連接]** 時會出現安全性提示，要求您輸入存取執行個體所需的認證。請輸入您在[步驟 1](#step1) 中建立的密碼，然後按一下 **[確定]**。
...
當您不需要再透過遠端桌面連接到雲端中的角色執行個體時，請使用 [Azure PowerShell](http://go.microsoft.com/?linkid=9790229&clcid=0x409) 停用遠端桌面存取
