...
在下面的程式碼範例中，以字串形式將位置傳遞給方法。不過，您也可以使用 <code>WindowsAzure\\ServiceManagement\\Models\\Locations</code> 類別，以列舉形式傳遞位置。例如，您可以傳遞 <code>Locations::WEST\_US</code>，而不是將「美國西部」傳遞給接受位置的方法。
