...
本指南將示範如何從 PHP 透過程式設計方式執行一般服務管理工作。[Azure SDK for PHP](../php-download-sdk/) 中的 [ServiceManagementRestProxy](https://github.com/WindowsAzure/azure-sdk-for-php/blob/master/WindowsAzure/ServiceManagement/ServiceManagementRestProxy.php) 類別支援透過程式設計方式存取[管理入口網站](https://manage.windowsazure.com/)中可用的最多服務管理相關功能 (例如**建立、更新以及刪除雲端服務、部署、儲存體服務和同質群組**)。建置需要透過程式設計方式存取服務管理的應用程式時，此功能十分實用。
...
-   [作法：在預備與生產之間移動部署](#MoveDeployments)
-   [作法：刪除部署](#DeleteDeployment)
-   [作法：建立儲存體服務](#CreateStorageService)
-   [作法：刪除儲存體服務](#DeleteStorageService)
-   [作法：建立同質群組](#CreateAffinityGroup)
-   [作法：刪除同質群組](#DeleteAffinityGroup)

什麼是服務管理
--------------

服務管理 API 透過程式設計方式存取透過[管理入口網站](https://manage.windowsazure.com/)取得的最多服務管理功能。Azure SDK for PHP 可讓您管理雲端服務、儲存體帳戶和同質群組。

若要使用服務管理 API，您將需要[建立 Azure 帳戶](/en-us/pricing/free-trial/)。
...
若要連線到服務管理端點，您需要有 Azure 訂閱 ID 和有效管理憑證的路徑。您可以透過[管理入口網站](https://manage.windowsazure.com/)取得訂閱 ID，也可以使用數種方式來建立管理憑證。在本指南中，使用 [OpenSSL](http://www.openssl.org/)，其可從 [OpenSSL for Windows](http://www.openssl.org/related/binaries.html) 下載並在主控台中執行。

您實際需要建立兩個憑證，一個適用於伺服器 (`.cer` 檔案)，另一個則適用於用戶端 (`.pem` 檔案)。若要建立 `.pem` 檔案，請執行這項作業：
...
建立這些檔案之後，需要透過[管理入口網站](https://manage.windowsazure.com/)將 `.cer` 檔案上傳至 Azure，而且需要記下 `.pem` 檔案的儲存位置。

取得訂閱 ID、建立憑證並將 `.cer` 檔案上傳至 Azure 之後，就可以建立連接字串並將它傳遞至 **ServicesBuilder** 類別上的 **createServiceManagementService** 方法，以連線到 Azure 管理端點：
...
在上面的範例中，`$serviceManagementRestProxy` 是 [ServiceManagementRestProxy](https://github.com/WindowsAzure/azure-sdk-for-php/blob/master/WindowsAzure/ServiceManagement/ServiceManagementRestProxy.php) 物件。**ServiceManagementRestProxy** 類別是用來管理 Azure 服務的主要類別。
...
當您建立雲端服務、儲存體服務或同質群組時，需要提供有效的位置。**listLocations** 方法一律會傳回最新的目前可用位置清單。撰寫這份文件時，可用位置如下：
...
> [WACOM.NOTE]
> 在下面的程式碼範例中，以字串形式將位置傳遞給方法。不過，您也可以使用 `WindowsAzure\ServiceManagement\Models\Locations` 類別，以列舉形式傳遞位置。例如，您可以傳遞 `Locations::WEST_US`，而不是將「美國西部」傳遞給接受位置的方法。

作法：建立雲端服務
------------------

當您建立應用程式並在 Azure 中執行它時，其程式碼和組態併稱為 Azure [雲端服務](../cloud-services-what-is/) (在舊版 Azure 中，稱為「託管服務」)。**createHostedServices** 方法可讓您透過提供託管服務名稱 (在 Azure 中必須是唯一的)、標籤 (Base 64 編碼託管服務名稱) 和 **CreateServiceOptions** 物件，來建立新的託管服務。[CreateServiceOptions](https://github.com/WindowsAzure/azure-sdk-for-php/blob/master/WindowsAzure/ServiceManagement/Models/CreateServiceOptions.php) 物件可讓您設定服務的位置*或* 同質群組。
...
您可以使用 **listHostedServices** 方法列出訂閱的所有託管服務，而此方法傳回 [ListHostedServicesResult](https://github.com/WindowsAzure/azure-sdk-for-php/blob/master/WindowsAzure/ServiceManagement/Models/GetDeploymentOptions.php) 物件。呼叫 **getHostedServices** 方法則可讓您重複執行 [HostedServices] 物件的陣列，並擷取服務屬性：
...
您可以使用 **getDeployment** 方法來存取部署屬性。下列範例透過在 [GetDeploymentOptions](https://github.com/WindowsAzure/azure-sdk-for-php/blob/master/WindowsAzure/ServiceManagement/Models/GetDeploymentOptions.php) 物件中指定部署位置來擷取部署，但是您可以改為指定部署名稱。此範例也會逐一查看部署的所有執行個體：
...
若要刪除部署，請使用 **deleteDeployment** 方法。下列範例顯示如何在 [GetDeploymentOptions](https://github.com/WindowsAzure/azure-sdk-for-php/blob/master/WindowsAzure/ServiceManagement/Models/GetDeploymentOptions.php) 物件上使用 **setSlot** 方法，然後將它傳遞給 **deleteDeployment**，以刪除預備環境中的部署。您可以在 [GetDepolymentOptions] 類別上使用 **setName** 方法，以依部署名稱來指定部署，而不是依位置來指定部署。
...
[儲存體服務](../storage-whatis-account/)可讓您存取 Azure [Blob](../storage-php-how-to-use-blobs/)、[資料表](../storage-php-how-to-use-table-storage/)和[佇列](../storage-php-how-to-use-queues/)。若要建立儲存體服務，您需要服務的名稱 (3 到 24 個小寫字元，而且在 Azure 內是唯一的)、標籤 (服務的 Base 64 編碼名稱，最多 100 個字元)，以及位置或同質群組。提供服務的說明是選用作業。位置、同質群組和說明設定於 [CreateServiceOptions](https://github.com/WindowsAzure/azure-sdk-for-php/blob/master/WindowsAzure/ServiceManagement/Models/CreateServiceOptions.php) 物件中，這會傳遞給 **createStorageService** 方法。下列範例顯示如何指定位置來建立儲存體服務。如果您想要使用同質群組，則需要先建立同質群組 (請參閱[作法：建立同質群組](#CreateAffinityGroup))，以及使用 **CreateServiceOptions-\>setAffinityGroup** 方法來設定它。
...
您可以將儲存體服務名稱傳遞給 **deleteStorageService** 方法，以刪除儲存體服務。如果刪除儲存體服務，則會刪除服務中所儲存的所有資料 (Blob、資料表和佇列)。
...
同質群組是 Azure 服務的邏輯分組，而 Azure 服務告訴 Azure 尋找可最佳化效能的服務。例如，您可能在「美國西部」位置中建立同質群組，然後在該同質群組中建立[雲端服務](#CreateCloudService)。如果您之後在相同的同質群組中建立儲存體服務，Azure 會知道將它放到「美國西部」位置，並在相同的同質群組中使用雲端服務，以在資料中心內最佳化來獲得最佳效能。

若要建立同質群組，您需要名稱、標籤 (Base 64 編碼名稱) 和位置。您可以選擇性地提供說明：
...
建立同質群組之後，可以在[建立儲存體服務](#CreateStorageService)時指定群組 (而非位置)。

呼叫 **listAffinityGroups** 方法，然後在 [AffinityGroup](https://github.com/WindowsAzure/azure-sdk-for-php/blob/master/WindowsAzure/ServiceManagement/Models/AffinityGroup.php) 類別上呼叫適當的方法，就可以列出同質群組並檢查其屬性：
