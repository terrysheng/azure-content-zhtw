<properties urlDisplayName="Enable remote debugging with continuous delivery" pageTitle="對連續傳遞啟用遠端偵錯" metaKeywords="" description="了解使用連續傳遞來部署至 Azure 時如何啟用遠端偵錯。" metaCanonical="" services="cloud-services,virtual machines" documentationCenter=".NET" title="使用連續傳遞來發行至 Azure 時啟用遠端偵錯" authors="kempb" solutions="" manager="douge" editor="" />
...
當您使用[連續傳遞][連續傳遞]來發行至 Azure 時，您可以依照下列步驟在 Azure 中啟用遠端偵錯。

本主題內容：

[對雲端服務啟用遠端偵錯][對雲端服務啟用遠端偵錯]

[對虛擬機器啟用遠端偵錯][對虛擬機器啟用遠端偵錯]

## <a name="cloudservice"></a>對雲端服務啟用遠端偵錯

1.  在組建代理程式上，設定 Azure 的初始環境，如[Azure 的命令列建置][Azure 的命令列建置]所述。
2.  因為封裝需要有遠端偵錯執行階段 (msvsmon.exe)，請安裝 [Remote Tools for Visual Studio 2013][Remote Tools for Visual Studio 2013] (如果是使用 Visual Studio 2012，則安裝 [Remote Tools for Visual Studio 2012 Update 4][Remote Tools for Visual Studio 2012 Update 4])。或者，您也可以從已安裝 Visual Studio 的系統中複製遠端偵錯二進位檔案。
3.  建立憑證，如[建立 Azure 的服務憑證][建立 Azure 的服務憑證]所述。保留 .pfx 和 RDP 憑證指紋，並將憑證上傳至目標雲端服務。
...
1.  建立 Azure 虛擬機器。請參閱[建立執行 Windows Server 的虛擬機器][建立執行 Windows Server 的虛擬機器]或[在 Visual Studio 中建立 Azure 虛擬機器][在 Visual Studio 中建立 Azure 虛擬機器]。
2.  在 [Azure 入口網站頁面][Azure 入口網站頁面]，檢視虛擬機器儀表板來查看虛擬機器的「RDP 憑證指紋」。這用在延伸組態中的 ServerThumbprint 值。
3.  建立用戶端憑證，如[建立 Azure 的服務憑證][建立 Azure 的服務憑證]所述 (保留 .pfx 和 RDP 憑證指紋)。
4.  從 Microsoft 下載中心下載 [Azure Powershell][Azure Powershell] (0.7.4 版或更新版本)。
...
6.  將憑證 (.pfx) 匯入至已安裝 Visual Studio with Azure SDK for .NET 2.4 的電腦。
  ...
  [Azure 的命令列建置]: http://msdn.microsoft.com/zh-tw/library/hh535755.aspx
  [Remote Tools for Visual Studio 2013]: http://www.microsoft.com/zh-tw/download/details.aspx?id=40781
  [Remote Tools for Visual Studio 2012 Update 4]: http://www.microsoft.com/zh-tw/download/details.aspx?id=38184
  ...
  [在 Visual Studio 中建立 Azure 虛擬機器]: http://msdn.microsoft.com/zh-tw/library/azure/dn569263.aspx
