...
此定義具有兩個面向：度量與資源。效能度量是指要滿足商業需求所必須達到的數值。這些包含回應時間、輸送量和可用性等項目。效能也包含要達到一定效能度量水準所需的資源使用量水準。成本向來都是商業需求，要獲得資源就需要付出成本，所以效能意味著以最有效率的方式使用資源。
...
在 Azure 應用程式中，所能達成的最大效能改善來自兩個方面：擴充資源和分割資源。要在 Azure 組建可擴充的應用程式，便需要利用依據資源的實體分割來相應放大資源的功能：SQL Database、儲存體和計算節點等。Azure 具有可供整個資料中心使用的資源，能為您處理實體分割作業，所以此一分割可平行執行應用程式工作，進而成就高效能。若要達到此整體效能水準，您需要使用適當的相應放大設計模式。
...
- 資源相應放大和分割：這是實現平行處理原則，進而獲得高效能的基本機制。 
...
* [資料庫設計和模型化基礎觀念 (英文)](http://go.microsoft.com/fwlink/?LinkId=252675) 
...
在決定是否使用 Azure 資料表儲存體時，您必須考量成本和效能。相較於 SQL Database，同樣的資料若儲存在資料表儲存體中，其成本會低上許多。不過您必須仔細考慮資料使用 SQL 關聯式功能的範圍，例如聯結、篩選、查詢等等。如果資料鮮少使用此類功能，則對於 Azure 資料表中儲存體是一種很好的候選方式。
...
* [資料表儲存體和 Azure SQL Database - 比較和對照 (英文)](http://msdn.microsoft.com/library/jj553018.aspx)
...
若要查看相關設計分析範例，此範例會檢查 AdventureWorks 資料庫以便加以同盟，並逐步顯示設計中所含的考量事項，請參閱＜[使用同盟進行資料庫設計的擴充優先方法：第 1 部分 - 挑選同盟及同盟索引鍵 (英文)](http://go.microsoft.com/fwlink/?LinkId=252671)＞。

一旦您決定了要同盟的資料表，就必須新增彙總根資料表的主要索引鍵作為每一個相關資料表的資料欄。

在決定要同盟的資料表之後，另一個問題是參考資料表的位置，以及其他資料庫物件。關於此主旨的完整討論，請參閱＜[使用同盟進行資料庫設計的擴充優先方法：第 2 部分 - 標註和部署同盟的結構描述 (英文)](http://go.microsoft.com/fwlink/?LinkId=252672)＞。如需執行其他進階查詢的說明，請參閱[第 2 部分 (英文)](http://go.microsoft.com/fwlink/?LinkId=252673)。
...
* 在發行同盟之前撰寫的完整說明：[如何利用 SQL Database 進行分區 (英文)](http://go.microsoft.com/fwlink/?LinkId=252678)。 
...
請擬定計劃來提供足夠的快取容量，以便能快取經常存取的物件。在 SQL Database 中，常會使用參考資料表來將數字碼轉換為較長的描述性字元字串。這些資料表通常會包含資料，例如國家 (地區) 和縣 (市) 名稱、有效郵遞區號值，以及您公司內部門名稱等。針對較小的資料表，適合將整個資料表儲存在快取中，至於其他情況則可能僅需儲存最常使用的值。隨著涉及這項資料的多重聯結查詢會帶來效能增益：針對在快取中找到的每個值，會儲存數個硬碟存取途徑。[簡介 Azure 快取服務 (英文)](http://go.microsoft.com/fwlink/?LinkId=252680) 有 Azure 效能和快取的詳盡介紹和討論。關於此主題的最新部落格文章，請參閱 [Windows #Azure 快取效能考量 (英文)](http://go.microsoft.com/fwlink/?LinkId=252681)。
...
如需服務匯流排效能的討論，請參閱[使用服務匯流排代理傳訊改善效能的最佳作法 (英文)](http://go.microsoft.com/fwlink/?LinkID=252683)。
...
* [巨量資料](../big-data.md) 
* [Azure HDInsight 服務](/documentation/services/hdinsight/)
...
* 在 *http://mysql.com* (英文) 搜尋 performance[](http://go.microsoft.com/fwlink/?LinkId=252775) 會出現許多資源。 
...
您也可以使用 SET STATISTICS Transact-SQL 命令，以及使用 SQL Server Management Studio，來檢視查詢產生的執行計劃，因為編寫有效率的查詢程式碼是效能的關鍵因素之一。如需詳細討論，以及作法的逐步說明，請參閱[深入了解 SQL Database 的效能 (英文)](http://go.microsoft.com/fwlink/?LinkId=252877)。另一個有趣方法為分析[內部部署 SQL Database 與 SQL Server](http://go.microsoft.com/fwlink/?LinkId=252878) (英文) 之間的效能。
...
- [Cerebrata (英文)](http://go.microsoft.com/fwlink/?LinkId=252880) 
- [SQL Server 和 SQL Database 效能測試：Enzo SQL Baseline (英文)](http://enzosqlbaseline.codeplex.com/) 
...
* [服務匯流排 (英文)](http://go.microsoft.com/fwlink/?LinkId=246934) 
* [Azure 計劃 - 在環境中整合 Azure 的決策後指南 (英文)](http://go.microsoft.com/fwlink/?LinkId=252884) 
