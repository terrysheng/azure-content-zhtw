...
1. 將下列內容加入所有角色中 **WebRole** 或 **WorkerRole** 節點底下的 *ServiceDefinition.csdef* 檔案：
	...
	上述組態會使用系統管理員權限來執行主控台命令 *install.cmd*，以便安裝 .NET Framework。此設定也會建立名稱為 *NETFXInstall* 的 LocalStorage。啟動指令碼會將暫存資料夾設定為使用此本機儲存資源，如此即可從此資源下載並及安裝 .NET Framework 安裝程式。請務必將此資源的大小設定為至少 1024 MB，以確保架構的安裝正確。如需詳細資訊，請參閱：[使用本機儲存體儲存啟動期間的檔案](https://msdn.microsoft.com/library/azure/hh974419.aspx)
