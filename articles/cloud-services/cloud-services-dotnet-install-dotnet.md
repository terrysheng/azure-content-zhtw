...
本文說明如何在雲端服務 Web 和背景工作角色上安裝 .NET framework。您可以依照下列步驟在 Azure 客體 OS 系列 4 上安裝.NET framework 4.5.2 或 .NET 4.6 。如需最新的客體 OS 版本相關資訊，請參閱 [Azure 客體 OS 版本與 SDK 相容性比較表](cloud-services-guestos-update-matrix.md)。
...
2. 建立 **install.cmd** (在角色上按右鍵，並依序選取 **[新增] > [現有項目]**) 並將它新增至所有角色。因此，所有角色現在應該都有 .NET 安裝程式檔案，以及 install.cmd 檔案。
...
3. 將下列指令碼加入至 **install.cmd** 檔案：
	...
	```
	> [AZURE.IMPORTANT]更新指令碼中 *netfx* 變數的值，以符合您想要安裝的架構版本。若要安裝 .NET 4.5.2，*netfx* 變數應設定為 *"NDP452"*，而若要安裝 .NET 4.6，*netfx* 變數應設定為 *"NDP46"*
...
若要設定診斷，請開啟 *diagnostics.wadcfgx*，並在**目錄**節點底下加入下列內容：
