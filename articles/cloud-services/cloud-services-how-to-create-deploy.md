...
- 如果您計劃將雲端服務部署至同質群組，請建立同質群組。您可以使用同質群組，將雲端服務和其他 Azure 服務部署到區域中的同一個位置。您可以在 Azure 傳統入口網站之 [網路] 區域中出現的 [同質群組] 頁面上建立同質群組。如需詳細資訊，請參閱[在 Azure 傳統入口網站中建立同質群組](http://msdn.microsoft.com/library/jj156209.aspx)。
