...
## 準備 Azure 訂用帳戶
使用 [Azure 入口網站](https://manage.windowsazure.com)對您的 Azure 訂用帳戶執行下列步驟。您必須執行這些步驟，稍後才能從內部部署前端節點部署 Azure 節點。
...
10. 在 [Create Node Template Wizard] 中，執行下列動作：
	...
	c.在 [**提供訂閱資訊**] 頁面上，輸入您的 Azure 訂用帳戶識別碼 (可從您的 Azure <a href="[https://account.windowsazure.com/Subscriptions">帳戶資訊</a>取得)。然後，在 [管理憑證] 中，按一下 [瀏覽] 並選取 [Default HPC Azure Management]。 然後按 [下一步]。
