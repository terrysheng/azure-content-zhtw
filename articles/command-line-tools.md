...
**-c, --connect** 於主機服務中已建立的部署內建立虛擬機器。如果使用這個選項時並未加上 -vmname，則會自動產生新虛擬機器的名稱。
**-n, --vm-name** 指定虛擬機器的名稱。此參數預設使用主機服務名稱。如果未指定 -vmname，則會以 \<service-name\>\<id\> 格式產生新虛擬機器的名稱，其中 \<id\> 是服務中現有虛擬機器的編號加 1。例如，如果您使用這個命令將新的虛擬機器加入已有一個虛擬機器的主機服務 MyService，則新虛擬機器的名稱會是 MyService2。
...
此命令會取得虛擬機器映像的清單。映像可分為三種：Microsoft 建立的映像 (名稱開頭為 "MSFT")、協力廠商建立的映像 (名稱開頭通常為廠商的名稱) 和您自己建立的映像。若要建立映像，您可以擷取現有的虛擬機器，也可以從上傳至 Blob 儲存體的自訂 .vhd 來建立映像。如需使用自訂 .vhd 的詳細資訊，請參閱 vm image create。
...
-   **--job-type** \<job-type\>：選用。Web 工作的類型。有效值為 "triggered" 或 "continuous"。預設會傳回
    所有類型的 Web 工作。
-   **--slot** \<slot\>：要重新啟動之位置的名稱。
...
這個命令會傳回行動服務記錄檔，其中已篩選掉 `error` 以外的所有記錄檔類型。
...
<div class="dev-callout"><b>注意</b>
<p><b>--query</b> 參數的優先順序高於 <b>--type</b>、<b>--skip</b> 和 <b>--top</b>。</p>
</div>
...
這個命令會從 TodoItem 資料表下載 insert 指令碼到 `table` 子資料夾中名為 `todoitem.insert.js` 的檔案。
...
這個命令會建立名為 `getUpdates` 且排定為每小時執行一次的新工作。
...
從網路組態中移除 DNS 伺服器項目。
  ...
  [Azure SDK 安裝程式]: http://go.microsoft.com/fwlink/?LinkId=252249
