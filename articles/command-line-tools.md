<properties urlDisplayName="Command-Line Tools" pageTitle="適用於 Mac 和 Linux 的 Azure 命令列工具" metaKeywords="Azure command-line, Azure tools Mac, Azure tools Linux" description="Learn about using the command-line tool for Mac and Linux in Azure." metaCanonical="" services="web-sites,virtual-machines,mobile-services,cloud-services" documentationCenter="" title="" authors="carolz" solutions="" manager="need to identify contact" editor="" />
...
**-n, --vm-name** 指定虛擬機器的名稱。此參數預設使用主機服務名稱。如果未指定 -vmname，則會以 <service-name><id> 格式產生新虛擬機器的名稱，其中 <id> 是服務中現有虛擬機器的名稱加 1。例如，如果您使用此命令將新的虛擬機器新增至已有一個虛擬機器的主機服務 MyService，則新虛擬機器的名稱會是 MyService2。<br /> 
**-u, --blob-url** 指定要從中建立虛擬機器系統磁碟的 Blob 儲存體 URL。 <br />
...
**-w, --virtual-network-name** 指定要將虛擬機器新增到的虛擬網路。若要設定和管理虛擬網路，可以從 Azure 入口網站進行。<br />
...
此命令會卸離連接至 Azure 虛擬機器的資料磁碟。<lun > 會識別要卸離的磁碟。若要在卸離磁碟前取得磁碟之相關聯磁碟的清單，請使用 vm disk-list <vm-name>。
...
+ **--job-type** <job-type>：選用。Web 工作的類型。有效值為 "triggered" 或 "continuous"。預設回傳
所有類型的 Web 工作。
+ **--slot** <slot>：要重新啟動之位置的名稱。
...
+ **--job-name** <job-name>：必要。Web 工作的名稱。
+ **--run-id** <run-id>：選用。執行歷程記錄的 ID。如果未指定，則會顯示最近一次執行。
+ **--slot** <slot>：要重新啟動之位置的名稱。
...
+ **-h** 或 **--help**：顯示輸出用法資訊。
+ **-s `<id>`** 或 **--subscription `<id>`**：使用特定訂閱 (以 `<id>` 形式指定)。
...
+ **-t `<type>`** 或 **--type `<type>`**：  根據項目 `<type>` 篩選傳回的記錄，它可以是 `information`、`warning` 或 `error`。
+ **-k `<skip>`** 或 **--skip `<skip>`**：略過 `<skip>` 指定的資料列數目。
+ **-p `<top>`** 或 **--top `<top>`**：傳回 `<top>` 指定的特定資料列數目。

<div class="dev-callout"><b>注意</b>
   <p>The <b>--query</b> 參數的優先順序高於 <b>--type</b>、<b>--skip</b> 和 <b>--top</b>。</p>
</div>
...
<div class="dev-callout"><b>注意</b>
   <p>當您重新產生金鑰時，使用舊金鑰的用戶端可能無法存取您的行動服務。重新產生應用程式金鑰時，您應該以新的金鑰值更新您的應用程式。 </p>
</div> 
...
<div class="dev-callout"><b>注意</b>
   <p>建立的新工作會是停用狀態，因為仍然必須上傳指令碼。請使用 <strong>mobile script upload</strong> 命令上傳指令碼，並使用 <strong>mobile job update</strong> 命令啟用工作。</p>
</div> 
...
<div class="dev-callout"><b>注意</b>
   <p>當您將運算模式設定為 `Reserved` 時，您在相同區域中的所有行動服務都會以進階模式執行。</p>
</div>  
