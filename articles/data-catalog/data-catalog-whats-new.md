...
- 支援變更「Azure 資料目錄」所使用的 Azure 訂用帳戶。「Azure 資料目錄」系統管理員可以使用「Azure 資料目錄」入口網站中的 [設定] 頁面，來選取不同的 Azure 訂用帳戶以供計費。

## 2016 年 1 月 29 日當週版本的新功能

自 2016 年 1 月 29 日當週起，「Azure 資料目錄」已新增下列功能：

- 支援手動註冊其他資料來源。使用者現在可以使用「Azure 資料目錄」入口網站中的 [建立手動輸入] 或使用「Azure 資料目錄」REST API 來註冊下列資料來源：
   + OData - 函式、實體集和實體容器
   + HTTP - 檔案、端點、報告和網站
   + 檔案系統 - 檔案
   + SharePoint - 清單
...
- 支援註冊與探索 SAP HANA 檢視和封裝。使用者可以使用「Azure 資料目錄」資料來源註冊工具來註冊 SAP HANA 資料來源，並且可以使用「Azure 資料目錄」入口網站對已註冊的 SAP HANA 資料來源加上註解及進行探索。

## 2016 年 1 月 8 日當週版本的新功能

自 2016 年 1 月 8 日當週起，「Azure 資料目錄」已新增下列功能：

- 在「Azure 資料目錄」入口網站中對資料資產進行釘選及取消釘選的能力。使用者可以選擇釘選資料資產，方便更輕鬆地重新探索及重複使用。
- 「Azure 資料目錄」入口網站中新重新設計的首頁。新的首頁包含目前使用者活動的深入了解 - 包含最近發佈的資產、釘選的資產和已儲存的搜尋 - 以及資料目錄中的活動。
- 「Azure 資料目錄」入口網站中支援持續性使用者設定。使用者體驗設定 - 包括方格檢視或並排顯示檢視、每個頁面上的結果數，以及開啟或關閉搜尋結果醒目提示 - 會在使用者工作階段之間保存。
- 「Azure 資料目錄」現在於兩個新的 Azure 區域中提供。除了美國東部、美國西部、西歐和澳洲東部以外，客戶還可在北歐及東南亞區域佈建「Azure 資料目錄」。如需詳細資訊，請參閱 [Azure 區域](https://azure.microsoft.com/regions/)。
