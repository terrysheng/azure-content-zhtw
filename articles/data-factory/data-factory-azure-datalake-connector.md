...
| dataLakeUri | 指定有關 Azure 資料湖存放區帳戶的資訊。格式如下：https://<Azure Data Lake account name>.azuredatalake.net/webhdfs/v1 | 是 |
...
### 運用 partitionedBy 屬性
如上方所述，您可以透過 **partitionedBy** 區段、Data Factory 巨集和系統變數 (SliceStart 和 SliceEnd，表示指定資料配量的開始和結束時間)，指定時間序列資料的動態 folderPath 和 filename。
...
## Azure 資料湖複製活動類型屬性  
如需定義活動可用的區段和屬性的完整清單，請參閱[建立管線](data-factory-create-pipelines.md)文章。名稱、描述、輸入和輸出資料表、各種原則等屬性都適用於所有活動類型。
...
| 屬性 | 說明 | 允許的值 | 必要 |
| -------- | ----------- | -------------- | -------- |
| copyBehavior | 指定複製行為。 | <p>**PreserveHierarchy：**在目標資料夾中保留檔案的階層，亦即來源檔案之於來源資料夾的相對路徑，與目標檔案之於目標資料夾的相對路徑完全相同。</p><p>**FlattenHierarchy：**來源資料夾中的所有檔案都會在目標資料夾的第一層中。目標檔案都會有自動產生的名稱。</p><p>**MergeFiles：**會將來源資料夾中的所有檔案合併成一個檔案。若已指定檔案/Blob 名稱，合併的檔案名稱將會是指定的名稱；否則，就會是自動產生的檔案名稱。</p> | 否 |
