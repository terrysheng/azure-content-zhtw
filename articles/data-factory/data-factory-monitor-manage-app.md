...
## 啟動監視及管理應用程式 
如要啟動監視及管理應用程式，請按一下您資料管理站的 [DATA FACTORY] 刀鋒視窗上的 [監視應用程式] 圖格。
...
1. 指定警示的 [名稱] 及 [說明]，然後按一下 [下一步]。此時應該會出現 [篩選器] 頁面。
...
2. 選取您想要資料處理站服務提醒您的 [事件]、[狀態] 及 [子狀態] (選擇性)，然後按一下 [下一步]。您應該會看到 [收件者] 頁面。
