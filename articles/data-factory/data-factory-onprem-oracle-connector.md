<properties 
	pageTitle="Oracle 連接器 - 從 Oracle 來回移動資料" 
	description="了解 Data Factory 服務的 Oracle 連接器，其可讓您從做為內部部署的 Oracle 資料庫來回移動資料。" 
...
1.	類型 OnPremisesOracle 的連結服務。
2.	AzureStorage 類型的連結服務。
3.	類型 OracleTable 的輸入資料集。 
4.	AzureBlob 類型的輸出資料集。
5.	具有使用 OracleSource 做為來源和 BlobSink 做為接收之複製活動的管線。
...
例如：從 MyTable 選取 *。<p>若未指定，執行的 SQL 陳述式：從 MyTable 選取 *</p> | 否
