...
* [事件中心服務資訊](/services/event-hubs/)
* [事件中心概觀][overview]
* [Azure 資料流分析簡介][stream]

## 其他資料傳輸選項

混合式連線提供簡單又方便的作法，讓您將 Azure 網站和 Azure 行動服務連接到內部部署資源。開發人員可以建立網站，以將資料從內部部署移至 Azure。如需詳細資料，請參閱[混合式連線概觀][hybrid]。

透過[虛擬網路](/services/virtual-network/)，您可以使用在 Azure 虛擬機器中運作的資料整合工具，安全地連接現場資料中心內的內部部署 SQL Server y 資料庫。唯有相同虛擬網路中的虛擬機器和服務能彼此識別或連接。如果您喜歡的話，甚至還可以透過網路服務提供者或 Exchange 提供者建立連往 Azure 的 [ExpressRoute](/services/expressroute/) 直接連線，並完全略過公用網際網路。

[Azure Marketplace](?../source=datamarket.md) 提供將資料移動到 Azure 的合作夥伴解決方案，如 Storm Managed File Transfer。
