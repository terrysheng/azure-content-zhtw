<properties 
   pageTitle="使用 Azure Preview 入口網站管理 Azure 資料湖分析 | Azure" 
   description="了解如何管理資料湖分析帳戶、資料來源、使用者和工作。" 
...
# 使用 Azure Preview 入口網站管理 Azure 資料湖分析

[AZURE.INCLUDE [manage-selector](../../includes/data-lake-analytics-selector-manage.md)]

了解如何使用 Azure Preview 入口網站來管理 Azure 資料湖分析帳戶、資料來源、使用者和工作。若要使用其他工具查看管理主題，請按一下上方的索引標籤選取器。
...
6. 輸入或選取下列項目：
	...
	- **名稱**：為分析帳戶命名。
	- **資料湖儲存區**：每個資料湖分析帳戶均具有相依的 Azure 資料湖儲存區帳戶。資料湖分析帳戶和相依的資料湖儲存區帳戶必須位於相同的 Azure 資料中心。請依照指示建立新的資料湖儲存區帳戶，或選取現有的帳戶。
...
8. 按一下 [建立]。它會帶領您前往入口網站主畫面。新的磚會新增至「開始面板」，且具有顯示 [部署 Azure 資料湖分析] 的標籤。建立資料湖分析帳戶需要幾分鐘的時間。當帳戶建立之後，入口網站就會在新的刀鋒視窗上開啟帳戶。
...
- [Azure 資料湖儲存區](data-lake-store-overview.md)
- [Azure Blob 儲存體](storage-introduction.md)

當您建立資料湖分析帳戶時，必須指定 Azure 資料湖儲存區帳戶作為預設的儲存體帳戶。預設的資料湖儲存區帳戶是用來儲存工作中繼資料與工作稽核記錄。建立資料湖分析帳戶後，便可新增其他資料湖儲存區帳戶和/或 Azure 儲存體帳戶。

<a name="default-adl-account"></a>**尋找預設的資料湖儲存體帳戶**

- 開啟您要管理的資料湖分析帳戶。如需詳細指示，請參閱[存取資料湖分析帳戶](#access-adla-account)。預設的資料湖儲存區會顯示在 [程式集]。
...
1. 從 Preview 入口網站，按一下左側功能表上的 [瀏覽]，然後按一下 [資料湖儲存區]。
...
[U-SQL 目錄](data-lake-analytics-use-u-sql-catalog.md)是用來建構資料和程式碼，以讓 U-SQL 指令碼可以共用它們。此目錄可以讓 Azure 資料湖中的資料具有最高效能。您可以從 Azure Preview 入口網站檢視 U SQL 目錄。
...
- [運用 Azure Preview 入口網站開始使用資料湖分析](data-lake-analytics-get-started-portal.md)
