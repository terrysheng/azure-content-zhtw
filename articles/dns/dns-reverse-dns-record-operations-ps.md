...
## 建立具有反向 DNS 記錄的新公用 IP 位址
您可以使用 "New-AzureRmPublicIpAddress" Cmdlet，新增指定了反向 DNS 屬性的新公用 IP 位址。
...
- "ReverseFqdn" 正向解析為其所指定之公用 IP 位址的名稱或 IP，或是相同訂用帳戶內任何公用 IP 位址的 "Fqdn" 或 IP，例如，"ReverseFqdn" 為 "app1.contoso.com."，這是 "contosoapp1.northus.cloudapp.azure.com." 的 CName 別名。 驗證檢查只會在設定或修改公用 IP 位址的反向 DNS 屬性時才會執行。不會執行定期的重新驗證。
