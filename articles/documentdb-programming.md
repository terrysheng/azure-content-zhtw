...
DocumentDB 整合了 JavaScript 語言並能夠以交易方式執行的特點，讓開發人員能直接以 JavaScript 撰寫「預存程序」、「觸發程序」和「使用者定義函數 (UDF)」。這一特點讓開發人員得以撰寫可以直接在資料庫儲存體資料分割上傳送和執行的應用程式邏輯。
...
REST API 以及許多平台 (包括 .NET、Node.js 和 JavaScript) 中的用戶端 SDK 都支援建立和執行觸發程序、預存程序和自訂查詢運算子。**本教學課程使用 Node.js SDK** 說明預存程序、觸發程序和 UDF 的語法和用法。
...
內容物件提供可對 DocumentDB 儲存體執行之所有作業的存取權，以及要求和回應物件的存取權。在此例中，我們使用回應物件來設定傳回給用戶端的回應本文。如需詳細資料，請參閱 DocumentDB JavaScript 伺服器 SDK 文件。

讓我們擴大這個範例，並對預存程序新增更多資料庫相關功能。預存程序可以建立、更新、讀取、查詢與刪除集合內的文件和附件。

下一個程式碼片段說明如何使用內容物件來與 DocumentDB 實體互動。
...
簡言之，「不可部分完成的作業」保證會將交易內完成的所有工作視為單一單位，所有工作不是全部認可就是一個都不認可。「一致性」確保資料在交易中一律處於良好內部狀態。「隔離」保證兩個交易不會彼此干擾；一般而言，大部分的商業系統都會提供多個可以根據應用程式的需要來使用的隔離等級。「持久性」確保資料庫中所認可的任何變更一律會存在。
...
此預存程序使用遊戲應用程式內的交易，透過單一作業讓兩位玩家交易項目。預存程序嘗試讀取兩份文件，這兩份文件各自對應到以引數形式傳入的玩家 ID。如果有找到這兩份玩家文件，則預存程序會透過交換他們的項目來更新文件。如果過程中發生任何錯誤，則會擲回 JavaScript 例外狀況，隱含地中止交易。
...
有一點務必要注意，那就是在 DocumentDB 中是以「交易方式」執行觸發程序。此後續觸發程序會在與建立原始文件時的相同交易過程中執行。因此，如果從後續觸發程序擲出例外狀況 (例如，如果我們無法更新中繼資料文件的話)，則整個交易會失敗並予以回復。此時不會建立任何文件，並且會傳回例外狀況。
...
此預存程序是透過對 URI dbs/sehcAA==/colls/sehcAIE2Qy4=/sprocs (其本文包含要建立的預存程序) 執行 POST 要求，來進行註冊。觸發程序和 UDF 的註冊方式類似，分別是對 /triggers 和 /udfs 發出 POST 要求。
接著，對此預存程序的資源連結發出 POST 要求，就可以執行此預存程序：
...
除了 Node.js 用戶端之外，DocumentDB 還支援 .NET、JavaScript 和 Python SDK。使用上述任何 SDK，也可以建立和執行預存程序、觸發程序和 UDF。下列範例說明如何使用 .NET 用戶端建立和執行預存程序。請注意 .NET 類型是如何在預存程序中以 JSON 形式傳入及讀回。
...
此範例說明如何使用 .NET SDK 建立預先觸發程序以及建立已啟用觸發程序的文件。
...
下列範例則說明如何建立使用者定義函數 (UDF) 並將它用於 DocumentDB SQL 查詢中。
...
1.  JSON [][]<http://www.json.org/></a> (英文)

2.  JavaScript ECMA-262 [][1]<http://www.ecma-international.org/publications/standards/Ecma-262.htm></a> (英文)

3.  JavaScript - JSON 類型系統 [][2]<http://www.json.org/js.html></a> (英文)

4.  安全和可攜式資料庫擴充性 - [][3]<http://dl.acm.org/citation.cfm?id=276339></a> (英文)

5.  服務導向資料庫架構 - [][4]<http://dl.acm.org/citation.cfm?id=1066267&coll=Portal&dl=GUIDE></a> (英文)

6.  在 Microsoft SQL Server 中託管 .NET 執行階段 - [][5]<http://dl.acm.org/citation.cfm?id=1007669></a> (英文)
