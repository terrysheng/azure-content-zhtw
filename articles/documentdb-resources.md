...
DocumentDB nosql 文件資料庫服務提供透過 HTTP 的簡單且開放 RESTful 程式設計模型。DocumentDB 所管理的實體稱為「**資源**」，而資源是透過其邏輯 URI 進行唯一識別。開發人員可以使用標準 HTTP 動詞命令、要求/回應標頭和狀態碼來與資源互動。如下圖所示，DocumentDB 的「**資源模型**」 是由資料庫帳戶下轄的一組資源所組成，而每個資源都可以透過邏輯和穩定 URI 來定址。  

>[AZURE.NOTE] DocumentDB 也提供高效率的 TCP 通訊協定，此 TCP 通訊協定在通訊模型中也符合 REST 限制，並且可以透過 .NET 用戶端 SDK 取得。

![][1]  
**資料庫帳戶的階層式資源模型**   

首先使用您的 Azure 訂用帳戶來佈建 DocumentDB **資料庫帳戶**。資料庫帳戶可以是由一組「**資料庫**」所組成，每個資料庫都包含多個「**集合**」，而每個集合又依序包括**預存程序、觸發程序、UDF、文件**及相關**附件**。資料庫也有相關聯的「**使用者**」，每位使用者都有一組可存取集合、預存程序、觸發程序、UDF、文件或附件的「**權限**」。資料庫、使用者、權限和集合是具有已知結構描述的系統定義資源，而文件和附件則包含使用者定義的任意 JSON 內容。  
...
<table width = "400">
    <tbody>
        ...
        <tr>
            ...
            <td width="540" valign="top">
                <p>
                    資料庫帳戶是與一或多個代表所佈建文件儲存體和輸送量的容量單位、一組
                    資料庫和 Blob 儲存體相關聯。您可以使用 Azure 訂用帳戶建立一或多個資料庫帳戶。每個資料庫帳戶都具有唯一的
                    DNS 名稱。
                </p>
            </td>
        </tr>
        <tr>
            ...
            <td width="540" valign="top">
                <p>
                    資料庫是分割給多個集合之文件儲存體的邏輯容器。同時也是使用者的容器。
                </p>
            </td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td width="540" valign="top">
                <p>
                    集合是 JSON 文件和相關聯 JavaScript 應用程式邏輯的容器。查詢和交易的範圍設為
                    集合。
                </p>
            </td>
        </tr>
...
您可以透過 Azure 入口網站 (網址為 [http://portal.azure.com/](http://portal.azure.com/)) 來建立和管理 DocumentDB 資料庫帳戶。建立和管理資料庫帳戶都需要管理存取權，而且只有在 Azure 訂用帳戶下才能執行。在佈建和管理資料庫帳戶時，您可以設定和讀取下列屬性：  
...
<p>請注意，此屬性只會套用至<i>使用者定義資源</i>。所有系統定義資源都是設定成支援具有強式一致性的讀取/查詢。</p></td>
</tr>

<tr>
<td valign="top" ><p>主要伺服器金鑰和次要伺服器金鑰</p></td>
<td valign="top" ><p>這些是主要和次要伺服器金鑰，提供資料庫帳戶下所有資源的管理存取權。</p></td>
</tr>
...
<td valign="top" ><p>資料庫帳戶下佈建的容量單位總數</p></td>
</tr>

<tr>
<td valign="top" ><p>CapacityUnitsConsumed (READ)</p></td>
<td valign="top" ><p>資料庫帳戶目前使用的容量單位總數</p></td>
</tr>

<tr>
<td valign="top" ><p>ProvisionedDocumentStorageInMB (READ)</p></td>
<td valign="top" ><p>資料庫帳戶下跨所有容量單位佈建的文件儲存體總數量 (MB)</p></td>
</tr>

<tr>
<td valign="top" ><p>ReservedDocumentStorageInMB (READ)</p></td>
<td valign="top" ><p>資料庫帳戶下跨所有容量單位保留的文件儲存體總數量 (MB)。集合包含文件時，即會保留文件儲存體。 </p></td>
</tr>

<tr>
<td valign="top" ><p>ConsumedDocumentStorageInMB (READ)</p></td>
<td valign="top" ><p>資料庫帳戶下跨所有容量單位實際使用的文件儲存體總數量 (MB)。這是實際文件以及資料庫帳戶下跨各種集合合併之索引的大小。</p></td>
</tr>
...
請注意，除了從 Azure 入口網站佈建、設定和管理資料庫帳戶之外，您還可以使用程式設計方式，透過 [Azure DocumentDB REST API](http://go.microsoft.com/fwlink/p/?LinkID=402413) 以及用戶端 SDK 來建立和管理 DocumentDB 資料庫帳戶。  

 
#資料庫
DocumentDB 中的資料庫是一或多個集合和使用者的邏輯容器。您可以在 DocumentDB 資料庫帳戶下，可根據供應項目限制建立任意數目的資料庫。  
![][3]  
**資料庫是使用者和集合的邏輯容器**
...
與 DocumentDB 資源模型中的其他資源相同，使用 [Azure DocumentDB REST API](http://go.microsoft.com/fwlink/p/?LinkID=402413) 或任何用戶端 SDK，即可輕鬆地建立、取代、刪除、讀取或列舉資料庫。DocumentDB 保證讀取或查詢資料庫資源之中繼資料的強式一致性。刪除資料庫會自動確定您無法存取其內所含的任何集合或使用者。即使 DocumentDB 在背景回收佈建為所刪除資料庫一部分的儲存體和輸送量，但是針對所刪除資料庫佈建的儲存體和輸送量還是立即可用。   
...
針對預覽版本的 DocumentDB，您只能在建立集合期間設定索引原則。建立集合之後，就無法再更新原則。
...
DocumentDB 查詢模型嘗試打破功能、效率和簡化之間的平衡。DocumentDB 的資料庫引擎本質上會編譯和執行 SQL 查詢陳述式。您可以使用 [Azure DocumentDB REST API](http://go.microsoft.com/fwlink/p/?LinkID=402413) 或任何用戶端 SDK 來查詢集合。.NET SDK 隨附於 LINQ 提供者。在未來的版本中，將會提供原生底線對應，而從用戶端 JavaScript SDK 以及使用伺服器端預存程序和觸發程序都可以使用它。   
...
使用 [Azure DocumentDB REST API](http://go.microsoft.com/fwlink/p/?LinkID=402413) 或任何用戶端 SDK，即可輕鬆地建立、刪除、讀取或列舉 DocumentDB 中的文件。DocumentDB 一律提供讀取或查詢集合之中繼資料的強式一致性。刪除集合會自動確定您無法存取其內所含的任何文件、附件、預存程序、觸發程序和 UDF。即使 DocumentDB 在背景回收佈建為所刪除集合一部分的儲存體和輸送量，但是針對所刪除集合佈建的儲存體和輸送量還是立即可用。   
...
請注意，範例使用易記 ID 來傳達資源階層。資源是透過 REST API 以根據唯一資源識別碼進行存取。 
...
 
#使用者
DocumentDB 中的使用者代表用於分組權限的邏輯命名空間。DocumentDB 中的使用者可能對應至身份識別管理系統或預先定義應用程式角色中的使用者。對於 DocumentDB，使用者只代表一個抽象概念，可用來分組資料庫下的一組權限。   

對於實作您應用程式的多重租用，您可以在 DocumentDB 中建立對應至實際使用者或應用程式租用戶的使用者。您接著可以建立給定使用者的權限，而權限透過各種集合、文件、附件等對應至存取控制。   

應用程式需要隨著使用者成長而調整時，您可以採用各種方式來共用資料。您可以建立每位使用者的模型，以執行下列作業   
