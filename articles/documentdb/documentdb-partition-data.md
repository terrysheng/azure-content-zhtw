<properties      
    ...
    description="檢閱如何運用分區化技術調整資料級別的相關資訊。瞭解有關分區、如何在 DocumentDB 中分割資料，以及使用「雜湊」、「定界」和「查閱」分割時機的資訊。"         
...
- **定界分割**通常用於日期的內容中，因為它提供簡易而自然的機制，所以能讓您依時間戳記進行過時分割。當查詢一般受限於時間範圍時，它也很有用，因為與分割界限對齊。其可讓您透過自然方式，將未排序且不相關的資料集分成群組並加以組織 (例如依組織組成的群組租用戶，或依地理區域組成的州/省群組)。查閱也為在集合之間移轉資料提供更細微的控制。 
...
以直覺方式路由文件建立要求，適用於雜湊與定界分割。文件是從雜湊、查閱或與分割索引鍵對應的範圍值在分割上建立。
