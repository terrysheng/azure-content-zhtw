...
-   **可維護性**：您必須能在多名租用戶使用應用程式的期間升級應用程式及執行其他維護工作。
-   **監控**：您必須能夠不間斷地監視應用程式，以找出問題並進行疑難排解。監視每個租用戶使用應用程式的情形，也是其中一環。

適當實作的多租用戶應用程式，可為使用者提供下列好處。

-   **隔離**：個別租用戶的活動不會影響其他租用戶對應用程式的使用。租用戶之間無法相互存取資料。對租用戶而言，應用程式就如同是他們專用的一般。
-   **可用性**：個別租用戶希望應用程式能持續保有可用性，甚或在 SLA 中明訂相關保證。再次重申，其他租用戶的活動不會影響應用程式的可用性。
-   **延展性**：應用程式可根據個別租用戶的需求因應調整。其他租用戶的存在與動作不會影響應用程式的效能。
-   **成本**：成本比執行專用的單一租用戶應用程式低廉，因為在多租用戶環境下可共用資源。
...
資料管理 (例如 SQL Azure Database) 或 Azure 儲存服務 (例如，為大量非結構化資料提供儲存服務的資料表服務)，以及為大量非結構化文字或二進位資料 (例如視訊、音訊和影像) 提供儲存服務的 Blob 服務。
