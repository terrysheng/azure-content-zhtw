...
Azure SDK for .NET 是一組 Visual Studio 工具、命令列工具、執行階段二進位檔及用戶端程式庫，可協助您開發、測試和部署在 Azure 上執行的應用程式。本文將詳細說明安裝 SDK 後您會有什麼收穫。您可以從 [Azure 下載頁面][]下載 SDK。

Azure SDK for .NET 也包含用來取用 Azure 服務的用戶端程式庫。這些程式庫可使用 [NuGet][] (英文) 個別進行安裝。

## 目錄

-   [安裝 Azure SDK for .NET 時包含的內容][]
-   [安裝 Azure SDK for .NET 時不包含的內容][]
-   [常見問題集][]
-   [資源][]
...
-   [Microsoft ASP.NET 及 Web Tools for Visual Studio][]
-   [Microsoft Azure Tools for Microsoft Visual Studio][]
-   [Microsoft Azure 製作工具][]
-   [Microsoft Azure 模擬器][]
-   [Microsoft Azure 儲存體模擬器][]
-   [Microsoft Azure 儲存體工具][]
...
-   [發行 Web 專案至 Azure 網站][] (英文)。
-   [發行主控台應用程式專案至 Azure WebJobs][] (英文)。
-   [在建立新的 Web 專案或發行 Web 專案時，建立 Azure 網站和 SQL Database 資源][] (英文)。
-   [使用 Windows PowerShell 指令碼來發行至開發和測試環境][]。
-   [在伺服器總管中管理和疑難排解 Azure 網站][] (英文)。
-   [針對網站和 WebJobs 以偵錯模式從遠端執行][] (英文)。
...
-   [在建立新的 Web 專案時，建立 Azure 虛擬機器][] (英文)。
-   [使用 Windows PowerShell 指令碼來發行至開發和測試環境][]。
-   [設定 Azure 專案][]。
-   在 [伺服器總管]中，檢視和管理[雲端服務][]、[虛擬機器][]與[服務匯流排][]。
...
-   [CSPack 命令列工具][]，用於建立部署封裝。
-   [CSEncrypt 命令列工具][]，用於加密密碼，若要透過遠端桌面連線存取雲端服務角色執行個體，便需要用到此密碼。
-   執行階段二進位檔，要有此二進位檔，雲端服務專案才能與專案執行階段環境進行通訊，也才能進行診斷。NuGet 封裝不提供這些二進位檔。

### <span id="emulator"></span></a>Microsoft Azure 模擬器

[Azure 模擬器][]可模擬雲端服務環境，因此您可以在將雲端服務專案部署至 Azure 之前，先在您的電腦本機上進行測試。

### <span id="stgemulator"></span></a>Microsoft Azure 儲存體模擬器

[Azure 儲存體模擬器][]使用 SQL Server 執行個體和本機檔案系統來模擬 Azure 儲存體 (佇列、資料表、Blob)，因此您可以在本機進行測試。

### <span id="stgtools"></span></a>Microsoft Azure 儲存體工具

這會安裝命令列工具 [AzCopy][] (英文)，供您用來將資料傳入和傳出 Azure 儲存體帳戶。
...
-   Visual Studio 外掛程式，可讓[角色中快取][]專案在 Visual Studio 中以本機方式執行。

### <span id="ls"></span></a>LightSwitch Azure Publishing add-on for Visual Studio

這可讓您[將 LightSwitch 專案發行至 Azure 網站][]。LightSwitch 附加元件包含在 Visual Studio 更新以及 Azure SDK for .NET 中。安裝此 SDK 可確保您擁有最新版本的附加元件。
...
-   [用戶端程式庫][NuGet] (英文)。
    ...
    **包含用戶端程式庫的專案範本。**只有 [Azure 雲端服務][建立、開啟及發行雲端服務專案] (英文) 和 [Azure 行動服務][] (英文) 專案範本會自動包含某些用戶端程式庫。如需其他程式庫或其他範本，請安裝您需要的[用戶端程式庫 NuGet 封裝][NuGet]。

-   [Azure PowerShell][] (英文)。

    Azure PowerShell 可讓您[將 Azure 環境的建立及部署自動化][] (英文)。
...
-   [Visual Studio 中已經有許多 Azure 功能。我是否還需要安裝 Azure SDK for .NET？][]
-   [我想要用戶端程式庫。我是否必須要安裝 Azure SDK for .NET 才能取得？][]
-   [我可以在哪找到舊版的 Azure SDK for .NET？][]
-   [Azure SDK for .NET 的版本生命週期原則為何？][]
-   [哪個版本的客體 OS 與 Azure SDK for .NET 相容？][]
...
此 SDK 只會安裝用戶端程式庫，因此您可以在未連線至網際網路的狀態下建立雲端服務專案。[NuGet.org][NuGet] (英文) 上的 NuGet 封裝有提供目前版本的用戶端程式庫。如需詳細資訊，請參閱本文前面的[安裝 Azure SDK for .NET 時不包含的內容][]。

### <span id="olderversions"></span></a>我可以在哪找到舊版的 Azure SDK for .NET？

如需較舊的版本，請參閱 [Azure SDK for .NET][] 下載頁面。

### <span id="lifecycle"></span></a>Azure SDK for .NET 的版本生命週期原則為何？

請參閱 [Microsoft Azure 雲端服務支援週期原則][]。

### <span id="guestos"></span></a>哪個版本的客體 OS 與 Azure SDK for .NET 相容？

請參閱 [Azure 客體 OS 版本與 SDK 相容性比較表][]。

## <span id="resources"></span></a>資源

若要下載 Azure SDK for .NET 或用戶端程式庫，請參閱 [Azure 下載頁面][]。

如需 Azure SDK for .NET 原始程式碼 (包含用戶端程式庫)，請參閱 [GitHub.com/Azure][] (英文)。

如需 Azure 用戶端程式庫的參考文件，請參閱 [Azure .NET 參考][] (英文)。

  [Azure 下載頁面]: /en-us/downloads/
  ...
  [使用 Windows PowerShell 指令碼來發行至開發和測試環境]: http://msdn.microsoft.com/en-us/library/dn642480.aspx
  ...
  [設定 Azure 專案]: http://msdn.microsoft.com/en-us/library/ee405486.aspx
  [雲端服務]: http://msdn.microsoft.com/en-us/library/ff683675.aspx
  [虛擬機器]: http://msdn.microsoft.com/en-us/library/jj131259.aspx
  [服務匯流排]: http://msdn.microsoft.com/en-us/library/jj149828.aspx
  [在 Visual Studio 中對雲端服務或虛擬機器進行偵錯]: http://msdn.microsoft.com/en-us/library/ff683670.aspx
  [CSPack 命令列工具]: http://msdn.microsoft.com/en-us/library/gg432988.aspx
  [CSEncrypt 命令列工具]: http://msdn.microsoft.com/en-us/library/hh404001.aspx
  [Azure 模擬器]: http://msdn.microsoft.com/en-us/library/dn339018.aspx
  [Azure 儲存體模擬器]: http://msdn.microsoft.com/en-us/library/hh403989.aspx
  [AzCopy]: http://aka.ms/AzCopy
  [角色中快取]: http://msdn.microsoft.com/en-us/library/dn386103.aspx
  [將 LightSwitch 專案發行至 Azure 網站]: http://msdn.microsoft.com/en-us/library/jj131261.aspx
  ...
  [Visual Studio 更新]: http://www.visualstudio.com/en-us/downloads/download-visual-studio-vs#DownloadFamilies_5
  ...
  [Azure 客體 OS 版本與 SDK 相容性比較表]: http://msdn.microsoft.com/en-us/library/ee924680.aspx
  [GitHub.com/Azure]: https://github.com/azure/
  [Azure .NET 參考]: /en-us/develop/net/reference/
