...
> The WASB path is virtual path.如需詳細資訊，請參閱[使用 Azure Blob 儲存體搭配 HDInsight][hdinsight-storage]。 
...
Twitter 會使用 OAuth 提供對其 API 的授權存取。OAuth 是一項驗證通訊協定，可讓使用者在無須共用其密碼的情況下，允許應用程式代表他們執行動作。更多資料可在 [oauth.net](http://oauth.net/) 或 Hueniverse 絕佳的 [Beginner's Guide to OAuth](http://hueniverse.com/oauth/) 中找到。
...
2. 按一下 [**建立新的應用程式**]。
3. 輸入 [**名稱**]、[**描述**]、[**網站**]。您可以在 [網站] 欄位中自行設定 URL。下表列出部分要使用的範例值：
...
9. 寫入**消費者金鑰**、**消費者密碼**、**存取權杖**及**存取權杖密碼**。稍後在教學課程中會用到這些值。

在本教學課程中，您將使用 PowerShell 發出 Web 服務呼叫。另一項常用來發出 Web 服務呼叫的工具，是 [*Curl*][curl]。Curl 可從[這裡][curl-download]下載取得。
...
4. 設定指令碼中的前九個變數：	
	...
	<tr><td>$storageAccountName</td><td>刪除用於預設 HDInsight 叢集檔案系統的 Azure 儲存體帳戶。這是佈建時所指定的儲存體帳戶。請參閱 <a href="#prerequisites">必要條件</a>。</td></tr>
	<tr><td>$containerName</td><td>刪除用於預設 HDInsight 叢集檔案系統的 Blob 容器。這是佈建時所指定的容器。請參閱 <a href="#prerequisites">必要條件</a>。</td></tr>
	...
	<tr><td>$oauth_consumer_key</td><td>這是 Twitter 應用程式 <strong>消費者金鑰</strong> 您稍早建立 Twitter 應用程式時所寫下。</td></tr>
...
4. 設定指令碼中的前兩個變數：	
	...
	<tr><td>$hqlScriptFile</td><td>HiveQL 指令碼檔案的位置和檔案名稱。</td></tr>
	</table>
