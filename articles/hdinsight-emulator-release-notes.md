<properties 
	pageTitle="版本資訊：Microsoft HDInsight Emulator for Azure | Azure" 
...
* 已新增指令碼，以簡化將所有 Apache Hadoop 服務設為自動或手動啟動的設定。預設值仍將是自動啟動，但所有服務現在都已可使用 C:\Hadoop 中安裝的 set-onebox-manualstart.cmd 或 set-onebox-autostart.cmd 指令碼進行變更。 

* 所需的安裝相依性數目已大幅降低，使安裝更為快速。 

* 在安裝於 GettingStarted 資料夾中的 RunSamples.ps1 指令碼中執行 Pig 範例時所使用的命令，已修正錯誤。 
...
* 具有下列功能的新 Microsoft HDInsight 儀表板： 
...
* 對於相同的服務，可透過不同的連接埠號碼來存取本機 HDInsight 安裝和 Azure HDInsight 服務上的 REST API 端點： 
	...
	ODBC：使用在 DSN 組態或連接字串中的連接埠 563。 


* 設定本機叢集上的 ASV： 

	在儀表板中，您會看見名為 "local (hdfs)" 的預設本機叢集。如果您要以 ASV 作為本機安裝的儲存體，請執行下列動作： 
	...
	3. 使用完整的 URI 存取該帳戶上的任何檔案：asv://{container}@{account}/{path} (如果您想要使用 HTTPS 來存取資料，則使用 asvs://)。範例：
...
* JavaScript 主控台無法載入 
	...
	* 進行某些安裝時，JavaScript 主控台將會失效，且頁面上會出現 HTTP 404 錯誤。若要解決此問題，請直接瀏覽至 http://localhost:8080 使用主控台。 
...
* Temp 目錄 
	...
	hadoop.tmp.dir 會指向錯誤的位置：它不會指向 C:\hadoop\hdfs，而是指向 c:\hdfs。此錯誤將在下次更新 HDP 位元時修正。 

* 作業系統限制 

	HDInsight Server 必須安裝在 64 位元作業系統上 

* 在 WebPI 搜尋結果中找不到 HDInsight。 

	這通常是因為作業系統限制所致。HDInsight 必須使用 64 位元作業系統，且最低版本必須是 Windows 7 Service Pack 1、Windows Server 2008 R2 Service Pack1、Windows 8 或 Windows Server 2012。
