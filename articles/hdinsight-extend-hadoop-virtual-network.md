...
[Azure 虛擬網路](/documentation/services/virtual-network/) 可讓您建立安全、持續的網路，內含解決方案所需的資源。虛擬網路可讓您：
...
* 在私人網路中將雲端資源結合在一起 (僅限雲端)
	...
	* 在 HDInsight 與 SQL Database 或 SQL Server 或其他執行於虛擬機器的資料儲存體解決方案之間**直接傳輸資料**。
...
* 使用虛擬私人網路 (VPN) 將雲端資源連接到本機資料中心網路 (站對站，或點對站)
	...
	* 在 HDInsight 與您的資料中心之間**直接傳輸資料**。例如，使用 Sqoop 在 SQL Server 往返傳輸資料，或讀取企業營運系統 (LOB) 應用程式所產生的資料。
