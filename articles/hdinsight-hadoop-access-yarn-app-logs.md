<properties 
	pageTitle="以程式設計方式存取 HDInsight 應用程式記錄檔| Azure" 
...
# 以程式設計方式存取 HDInsight 應用程式記錄檔

本主題說明如何以程式設計方式列舉在 HDInsight 中的 Hadoop 叢集上已完成的 YARN 應用程式，以及如何以程式設計方式存取應用程式記錄檔，而不需透過 RDP 連線到您的叢集。具體來說，已加入一個新元件和一個新 API：

  1. 已啟用 HDInsight 叢集上的泛型應用程式歷程記錄伺服器。它是 YARN Timeline Server 內的一個元件，負責儲存和抓取已完成之應用程式的相關泛型資訊。
  2. 已將 Azure HDInsight .Net SDK 中的新 API 加入，可讓您以程式設計方式列舉已在您叢集上執行的應用程式，並下載相關應用程式或容器特定的記錄檔 (純文字)，以協助對發生的任何應用程式問題進行偵錯。


## 必要條件

若要在 .NET 應用程式中使用本主題提供的程式碼，必須有 Azure HDInsight SDK。您可以從 [NuGet](http://nuget.codeplex.com/wikipage?title=Getting%20Started) 取得最新發佈的 SDK 組建。 
...
- [YARN 應用程式和記錄檔](#YARNAppsAndLogs)
- [以程式設計方式列舉應用程式及下載記錄檔](#enumerate-and-download)


## <a name="YARNTimelineServer"></a>YARN Timeline Server

此 <a href="http://hadoop.apache.org/docs/r2.4.0/hadoop-yarn/hadoop-yarn-site/TimelineServer.html" target="_blank">YARN Timeline Server</a> 透過兩個不同的介面提供已完成之應用程式的相關泛型資訊，以及架構特定應用程式資訊。具體而言：

* 儲存及抓取 HDInsight 叢集上泛型應用程式資訊的功能已在版本 3.1.1.374 或更新版本上啟用。 
* Timeline Server 的架構特定應用程式資訊元件目前在 HDInsight 叢集上並未提供。


應用程式的相關泛型資訊包含下列類型的資料： 

* **ApplicationId** (應用程式的唯一識別碼)、 
* 啟動應用程式的「使用者」、 
* 為完成應用程式而進行之「嘗試」的相關資訊，以及 
* 任何指定之應用程式嘗試所使用的「容器」。 

在您的 HDInsight 叢集上，會由「資源管理員」將這項資訊儲存至您預設儲存體帳戶之預設容器中的歷程記錄存放區。透過 REST API 即可抓取這項已完成之應用程式的相關泛型資料：

    GET on https://<cluster-dns-name>.azurehdinsight.net/ws/v1/applicationhistory/apps

我們已將新 API 加入至 HDInsight .Net SDK，方便以程式設計方式抓取這項資料。請注意，您也可以透過直接在叢集節點上執行 YARN CLI 命令 (在使用 RDP 連線到叢集之後) 來抓取泛型資料。

## <a name="YARNAppsAndLogs"></a>YARN 應用程式和記錄檔

YARN (另一種資源協調程式，Yet Another Resource Negotiator) 藉由將資源管理從應用程式排程/監視分離，支援多種程式設計模型 (MapReduce 為其中之一)。這是透過全域 *Resource Manager* (RM)、每一背景工作節點 *Node Managers* (NM) 和每一應用程式 *Application Masters* (AM) 完成。每一應用程式 AM 會與 RM 交涉用來執行您應用程式的資源 (CPU、記憶體、磁碟、網路)。RM 會與 NM 合作來授與這些資源 (以 *Containers*的形式授與)。AM 則是負責追蹤 RM 指派給它之容器的進度。視應用程式的本質而定，一個應用程式可能會需要許多容器。 

此外，每個應用程式可能包含多個 *Application Attempts*，為了在應用程式發生當機時，或因 AM 與 RM 之間通訊中斷時，完成應用程式。因此，容器是被授與應用程式的特定嘗試。就某方面來說，容器會提供 YARN 應用程式所執行之基本工作單位的內容，而在容器的內容中完成的所有工作，都是在配置給該容器的單一背景工作節點上執行。請參閱 [YARN 概念][YARN-concepts]，以取得進一步的參考資料。
...
其中 "*user*" 是啟動應用程式之使用者的名稱，"*applicationId*" 是「YARN 資源管理員」所指派的應用程式唯一識別碼。
...
下一節說明如何以程式設計方式存取應用程式或容器特定記錄檔，而不需透過 RDP 連線到您的 HDInsight 叢集。
...
或者，也可以使用以下程式碼片段，針對已知其 "ApplicationId" (應用程式唯一識別碼) 的應用程式來下載記錄檔。ApplicationId 是「資源管理員」指派給應用程式的全域唯一識別碼。其建構方式是使用「資源管理員」的開始時間，連同提交給它之應用程式的依序遞增計數器，來進行建構。ApplicationId 格式為 "application\_&lt;RM-start-time&gt;\_&lt;Counter&gt;"。請注意，ApplicationId 與 JobId 截然不同。JobId 是 MapReduce 架構特定的概念，而 ApplicationId 則是跨架構的 YARN 概念。在 YARN 中，JobId 可識別特定的 MapReduce 工作，此工作是由提交給「資源管理員」之 MapReduce 應用程式的「應用程式主機」處理。
...
如有需要，您也可以下載應用程式所使用的每個容器 (或任何特定的容器) 的記錄，如下所示。
