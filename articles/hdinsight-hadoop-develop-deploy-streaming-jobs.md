...
Hadoop 為 MapReduce 提供一個串流 API，可讓您以 Java 以外的語言撰寫 map 和 reduce 函數。本教學課程逐步引導您完成一個字數統計案例，從您提供的輸入資料中計算給定單字的出現次數。下圖顯示 MapReduce 架構如何執行字數統計。

![HDI.WordCountDiagram][image-hdi-wordcountdiagram]

本主題說明如何：

- 在 HDInsight Emulator 上使用 C# 開發和測試 Hadoop 串流 MapReduce 程式
...
- 安裝 Azure HDInsight Emulator。如需指示，請參閱[開始使用 HDInsight Emulator][hdinsight-get-started-emulator]。
...
- [在 C# 中開發字數統計 Hadoop 串流程式](#develop)
- [在模擬器上測試程式](#test)
...
##<a name="develop"></a>在 C&#35; 中開發字數統計 Hadoop 串流程式

字數統計方案包含兩個主控台應用程式專案：mapper 和 reducer。對應器應用程式可將每一個字串流傳送至主控台，而歸納器應用程式可計算從文件中串流傳送而來的字數。mapper 和 reducer 都會從標準輸入資料流 (stdin) 循行讀取字元並寫入至標準輸出資料流 (stdout)。
...
1. 將資料上傳至模擬器 HDFS
2. 將對應器和歸納器應用程式上傳至模擬器 HDFS
3. 提交字數統計 MapReduce 工作
4. 檢查工作狀態
5. 擷取工作結果

依預設，HDInsight Emulator 會使用 HDFS 做為預設檔案系統。  您也可以選擇設定 HDInsight Emulator 來使用 Azure Blob 儲存體。如需詳細資料，請參閱[開始使用 HDInsight Emulator][hdinsight-emulator-wasb]。本節中，您將使用 HDFS *copyFromLocal* 命令來上傳檔案。下一節說明如何使用 Azure PowerShell 來上傳檔案。關於其他方案，請參閱[將資料上傳至 HDInsight][hdinsight-upload-data]。
...
3. 執行下列命令來設定變數：
	...
	HDInsight Emulator 叢集名稱是 "http://localhost:50111"。  
...
Azure HDInsight 使用 Azure Blob 儲存體做為預設檔案系統。您可以設定 HDInsight 叢集使用其他 Blob 儲存體來儲存資料檔。本節中，您將建立儲存體帳戶，並將資料檔上傳至 Blob 儲存體。資料檔是位於 %hadoop_home%\share\doc\hadoop\common 目錄中的 .txt 檔案。
...
本節提供的 PowerShell 指令碼會執行與 MapReduce 工作執行有關的所有工作。工作清單包括：
...
3. 設定指令碼中的前四個變數。**$stringPrefix** 用來對 HDInsight 叢集名稱、儲存體帳戶名稱和 Blob 儲存體容器名稱的指定字串加上首碼。因為這些項目的名稱必須為 3 到 24 個字元，請確定您指定的字串和此指令碼使用的名稱，合計不超過名稱的字元限制。$stringPrefix 必須全部為小寫。

	 **$storageAccountName_Data** 和 **$containerName_Data** 是您已在先前步驟中建立的儲存體帳戶和容器。因此，您必須提供這些項目的名稱。這些是用來儲存資料檔案和應用程式。$location 必須符合資料儲存體帳戶位置。
