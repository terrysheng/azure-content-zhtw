<properties 
	pageTitle="在 HDInsight 中使用 R 來自訂叢集| Azure" 
	description="了解如何安裝及使用 R 以自訂 Hadoop 叢集。" 
	services="hdinsight" 
	documentationCenter="" 
	authors="bradsev" 
	manager="paulettm" 
...
您可以使用**指令碼動作**叢集自訂在 HDInsight 上的 Hadoop 中任何類型的叢集上安裝 R。這可讓資料科學家和分析師使用 R 來部署強大的 MapReduce/YARN 程式設計架構，以處理部署在 HDInsight 中之 Hadoop 叢集上的大量資料。

只有在建立叢集時，指令碼動作才可讓您執行指令碼以自訂叢集。如需詳細資訊，請參閱[使用指令碼動作自訂 HDInsight 叢集][hdinsight-cluster-customize]。
...
- [什麼是 R？](#whatIs)
- [如何安裝 R？](#install)
- [如何在 HDInsight 中執行 R 指令碼](#useR) 
- [使用 PowerShell 在 HDInsight Hadoop 叢集上安裝 R](#usingPS)
- [使用 .NET SDK 在 HDInsight Hadoop 叢集上安裝 R](#usingSDK)
- [另請參閱](#seeAlso)


## <a name="whatIs"></a>什麼是 R？

「 <a href="http://www.r-project.org/" target="_blank">R 專案」</a> 是一個用於統計計算的開放原始碼語言和環境。R 提供數百個內建的統計函式及它自己的程式設計語言，此語言結合了函式型和物件導向程式設計的層面。它也提供廣泛的圖形功能。R 是廣泛各種不同領域中，大多數專業統計人員和科學家慣用的程式設計環境。 
...
您可以從一個唯讀的 Azure 儲存體 Blob 取得在 HDInsight 叢集上安裝 R 的範例指令碼，網址為 [https://hdiconfigactions.blob.core.windows.net/rconfigactionv02/r-installer-v02.ps1](https://hdiconfigactions.blob.core.windows.net/rconfigactionv02/r-installer-v02.ps1)。本節提供有關如何在使用 Azure 管理入口網站佈建叢集時使用範例指令碼的指示。
...
2. 在精靈的 [**指令碼動作**] 頁面上，按一下 [**加入指令碼動作**] 提供有關指令碼動作的詳細資料，如下所示：
	...
	<table border='1'>
		...
		<tr><td>指令碼 URI</td>
			<td>指定要叫用來自訂叢集的指令碼的 URI。例如， <i>https://hdiconfigactions.blob.core.windows.net/rconfigactionv02/r-installer-v02.ps1</i></td></tr>
		<tr><td>節點類型</td>
			<td>指定執行自訂指定碼的節點。您可以選擇<b>所有節點</b>、<b>僅前端節點</b>或僅<b>背景工作節點</b>。
		<tr><td>參數</td>
			<td>如果指令碼要求，請指定參數。安裝 R 的指令碼不需要任何參數，因此您可以在此保留空白。</td></tr>
	</table>	

	您可以加入一個以上的指令碼動作，以在叢集上安裝多個元件。加入指令碼之後，按一下核取記號以開始佈建叢集。

您也可以使用指令碼在 HDInsight 上使用 PowerShell 或 HDInsight .NET SDK 安裝 R。本主題稍後會提供這些程序的指示。

## <a name="useR"></a>如何在 HDInsight 中執行 R 指令碼？
本節說明如何使用 HDInsight 在 Hadoop 叢集上執行 R 指令碼。

1. **建立與叢集的遠端桌面連線**：從「Azure 管理入口網站」，針對您所建立已安裝 R 的叢集啟用遠端桌面，然後從遠端連線至叢集。如需相關指示，請參閱 <a href="http://azure.microsoft.com/zh-tw/documentation/articles/hdinsight-administer-use-management-portal/#rdp" target="_blank">使用遠端桌面連線到 HDInsight 叢集</a>。

2. **開啟 R 主控台**：R 安裝會在前端節點的桌面上放置 R 主控台的連結。按一下該連結以開啟 R 主控台。

3. **執行 R 指令碼**：直接從 R 主控台執行 R 指令碼的方式是貼上它、選取它，然後按 **Enter**。以下是一個簡單的範例指令碼，此指令碼會產生數字 1 到 100，然後將它們乘以 2。
...
## <a name="usingPS"></a>使用 PowerShell 在 HDInsight 上安裝 R
...
3. 使用 **Add-AzureHDInsightScriptAction** Cmdlet 來叫用範例指令碼以安裝 R。 
...
10. 將下列程式碼附加至 Main() 函數中，以使用 [ScriptAction](http://msdn.microsoft.com/zh-tw/library/microsoft.windowsazure.management.hdinsight.clusterprovisioning.data.scriptaction.aspx) 類別來叫用自訂指令碼以安裝 R。
...
開啟 PowerShell 主控台、瀏覽至您儲存專案的位置、瀏覽至專案內的 \bin\debug 目錄，然後執行下列命令：
