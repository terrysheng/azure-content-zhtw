...
在本文件中，您將學習如何使用 Curl 在 HDInsight 叢集的 Hadoop 上執行 Hive 查詢。 
...
> [AZURE.NOTE] 如果您已熟悉如何使用以 Linux 為基礎的 Hadoop 伺服器，但剛接觸 HDInsight，請參閱 <a href="../hdinsight-hadoop-linux-information/" target="_blank">在以 Linux 為基礎的 HDInsight 上安裝 Hadoop 的須知事項</a>。

## <a id="prereq"></a>必要條件

若要完成本文中的步驟，您需要下列項目。

* HDInsight 叢集上的 Hadoop (以 Linux 或 Windows 為基礎)
...
> [AZURE.NOTE] 在使用 Curl 或與 WebHCat 進行任何其他 REST 通訊時，您必須提供 HDInsight 叢集系統管理員使用者名稱和密碼來驗證要求。您也必須在用來將要求傳送至伺服器的 URI 中使用叢集名稱。
> 
> 在本節的所有命令中，將 **USERNAME** 取代為用來驗證叢集的使用者，並將 **PASSWORD** 取代為使用者帳戶的密碼。將 **CLUSTERNAME** 取代為叢集名稱。
> 
> REST API 使用 <a href="http://en.wikipedia.org/wiki/Basic_access_authentication" target="_blank">基本驗證進行保護</a>。您應一律使用 HTTPS 來提出要求，以確保認證能夠安全地傳送到伺服器。
...
1. 從命令列中，使用下列命令來確認您可以連線到 HDInsight 叢集。
    ...
    * **-G** - 指出這是 GET 要求
    ...
    所有要求的 URL 開頭 **https://CLUSTERNAME.azurehdinsight.net/templeton/v1** 都一樣。**/status** 路徑表示伺服器的要求是要傳回 WebHCat (也稱為 Templeton) 的狀態。您也可以使用下列命令要求 Hive 的版本。
...
2. 使用下列命令建立新資料表 **log4jLogs**。
    ...
    * **-d** - 因為未使用 `-G`，要求會預設使用 POST 方法。`-d` 指定與要求一起傳送的資料值

        * **user.name** - 執行命令的使用者
        
        * **execute** - 要執行的 HiveQL 陳述式
        
        * **statusdir** - 這項工作的狀態所要寫入到的目錄

    這些陳述式將執行下列動作。

    * **DROP TABLE** - 若資料表已存在，刪除資料表和資料檔案
    ...
    * **ROW FORMAT** - 讓 Hive 知道資料的格式化方式。在此情況下，會以一個空格區隔每個記錄中的欄位
    
    * **STORED AS TEXTFILE LOCATION** - 將資料的儲存位置告訴 Hive (範例/資料目錄)，且資料是儲存為文字。
    
    * **SELECT** - 選擇其資料欄 **t4** 包含值 **[ERROR]** 的所有資料列計數。這應該會傳回值為 **3**，因為有三個資料列包含這個值
...
3. 若要檢查工作狀態，請使用下列命令。將 **JOBID** 取代為上一個步驟傳回來的值。例如，如果傳回值為  `{"id":"job_1415651640909_0026"}`，JOBID 就是 `job_1415651640909_0026`。

        curl -G -u USERNAME:PASSWORD -d user.name=USERNAME https://CLUSTERNAME.azurehdinsight.net/templeton/v1/jobs/JOBID | jq .status.state

	如果工作已完成，狀態會是「成功」。

    > [AZURE.NOTE] 這個 curl 要求傳回具有工作相關資訊的 JSON 文件；使用 jq 可僅擷取狀態值。 

4. 一旦工作的狀態變更為 [**成功**]，您就可以從 Azure Blob 儲存體擷取工作的結果。隨查詢一起傳送的  `statusdir` 參數包含輸出檔案的位置；在此案例中為 **wasb:///example/curl**。這個位址會將工作的輸出儲存在 HDInsight 叢集所用的預設儲存體容器的 **example/curl** 目錄中。

    您可以使用  <a href="../xplat-cli/" target="_blank">Azure 跨平台命令列介面 (xplat-cli)</a>列出並下載這些檔案。例如，若要列出 **example/curl** 中的檔案，請使用下列命令。
	...
	> [AZURE.NOTE] 您必須使用 `-a` 和 `-k` 參數指定包含 Blob 的儲存體帳戶名稱，或是設定 **AZURE\_STORAGE\_ACCOUNT** 和 **AZURE\_STORAGE\_ACCESS\_KEY** 環境變數。請參閱 <a href="../hdinsight-upload-data/" target="_blank" for more information.

6. 使用下列陳述式來建立新的 'internal' 資料表名為**錯誤記錄檔**。
    ...
    * **CREATE TABLE IF NOT EXISTS** - 建立還不存在的資料表。由於未使用 **EXTERNAL** 關鍵字，這是內部資料表，儲存在 Hive 資料倉儲並完全受 Hive 所管理。
    ...
    * <strong>SELECT \*</strong> - 從新的 **errorLogs** 資料表選取所有資料列。

7. 使用傳回的工作識別碼檢查工作的狀態。成功後，如先前所述使用 xplat-cli 下載並檢視結果。輸出應包含三行，其中都包含 **[ERROR]**。
...
其他可在 HDInsight 上使用 Hadoop 之方法的相關資訊。

* [在 HDInsight 上搭配 Hadoop 使用 Pig](../hdinsight-use-pig/)

* [在 HDInsight 上搭配 Hadoop 使用 MapReduce](../hdinsight-use-mapreduce/)
