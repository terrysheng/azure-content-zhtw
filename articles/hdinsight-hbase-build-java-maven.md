<properties title="使用 Maven 建置 HBase 應用程式" pageTitle="使用 Maven 建置 HBase 應用程式" description="了解如何使用 Apache Maven 建置 Java 型 Apache HBase 應用程式，然後部署至 Azure HDInsight" metaKeywords="Maven hbase hadoop, hbase hadoop, maven java hbase, maven java hbase hadoop, maven java hadoop, hbase hdinsight, hbase java hdinsight, maven hdinsight, maven java hdinsight, hadoop database, hdinsight database" services="hdinsight" solutions="big-data" documentationCenter="" authors="larryfr" videoId="" scriptId="" manager="paulettm" />
...
2.  將下列程式碼新增至 **pom.xml** 檔案。這必須在檔案中的 `<project>...</project>` 標籤內；例如，在 `</dependencies>` 和 `</project>` 之間。
    ...
    這會設定資源 (**conf\\hbase-site.xml**)，內含 HBase 的組態資訊。
...
4.  在 **hbaseapp** 目錄中建立名為 **conf** 的新目錄。在 **conf** 目錄中，建立名為 **hbase-site.xml** 的新檔案，並使用下列程式碼做為內容。
...
1.  移至 **hbaseapp\\src\\com\\microsoft\\examples** 目錄，將 app.java\_\_ 檔案重新命名為 **CreateTable.java**。

2.  開啟 **CreateTable.java** 檔案，以下列程式碼取代現有的內容。
    ...
    這是 **CreateTable** 類別，將會建立名為 **people** 的資料夾，並填入一些預先定義的使用者。

3.  儲存 **CreateTable.java** 檔案。

4.  在 **hbaseapp\\com\\microsoft\\examples** 目錄中，建立名為 **SearchByEmail.java** 的新檔案。使用下列程式碼做為此檔案的內容。
...
6.  在 **hbaseapp\\com\\microsoft\\examples** 目錄中，建立名為 **DeleteTable.java** 的新檔案。使用下列程式碼做為此檔案的內容。
    ...
    此類別只是用來清理此範例，作法是先停用再卸除 **CreateTable** 類別所建立的資料表。

7.  儲存 **CreateTable.java** 檔案。
...
3.  指令完成時，**hbaseapp\\target** 目錄將包含一個名為 **hbaseapp-1.0-SNAPSHOT.jar** 的檔案。
    ...
    > [WACOM.NOTE] **hbaseapp-1.0-SNAPSHOT.jar** 檔案是一個 uberjar (有時稱為 fatjar)，內含執行應用程式所需的所有相依項目。
...
3.  開啟新的 Azure PowerShell 視窗，切換至 **hbaseapp** 目錄，然後執行下列命令。
    ...
    將路徑變更為稍早建立的 **hbase-runner.psm1** 檔案的位置。這樣會針對此 PowerShell 工作階段來註冊模組。

4.  使用下列命令將 **hbaseapp-1.0-SNAPSHOT.jar** 上傳至 HDInsight 叢集。

        Add-HDInsightFile -localPath target\hbaseapp-1.0-SNAPSHOT.jar -destinationPath example/jars/hbaseapp-1.0-SNAPSHOT.jar -clusterName hdinsightclustername

    將 **hdinsightclustername** 換成您的 HDInsight 叢集名稱。然後，此命令會將 **hbaseapp-1.0-SNAPSHOT.jar** 上傳至 HDInsight 叢集的主要儲存體中的 **example/jars** 位置。

5.  上傳檔案之後，使用下列命令在 **hbaseapp** 中建立新的資料表

        Start-HBaseExample -className com.microsoft.examples.CreateTable -clusterName hdinsightclustername

    將 **hdinsightclustername** 換成您的 HDInsight 叢集名稱。

    此命令會在 HDInsight 叢集上建立名為 **people** 的新資料表。
...
6.  若要在資料表中搜尋項目，請使用下列命令。
    ...
    這會使用 SearchByEmail 類別來搜尋資料行系列 **contactinformation**、資料行 **email** 包含字串 **contoso.com** 的任何資料列。您應該會得到下列結果：
    ...
    使用 **fabrikam.com** 做為 `-emailRegex` 值會傳回電子郵件欄位中含有 **fabrikam.com** 的使用者。因為此搜尋是以規則運算式篩選器來實作，您也可以輸入像是 **^r** 這樣的規則運算式，將會傳回電子郵件以字母 'r' 開頭的項目。

## 刪除資料表

練習完範例之後，請從 PowerShell 工作階段中使用下列命令，以刪除此範例所使用的 **people** 資料表。
...
請使用 `-showErr` 參數，以檢視執行工作時所產生的 STDERR。
  ...
  [Maven 儲存機制搜尋]: http://search.maven.org/#artifactdetails%7Corg.apache.hbase%7Chbase-client%7C0.98.4-hadoop2%7Cjar
  [maven-shade-plugin]: http://maven.apache.org/plugins/maven-shade-plugin/
  [使用 RDP 連線到 HDInsight 叢集]: http://azure.microsoft.com/zh-tw/documentation/articles/hdinsight-administer-use-management-portal/#rdp
