<properties urlDisplayName="Get Started" pageTitle="開始在 HDInsight 中搭配使用 HBase 與 Hadoop | Azure" metaKeywords="" description="開始在 HDInsight 中搭配使用 HBase 與 Hadoop。了解如何建立 HBase 資料表並使用 Hive 進行查詢。" metaCanonical="" services="hdinsight" documentationCenter="" title="開始在 HDInsight 中搭配使用 HBase 與 Hadoop (英文)" authors="bradsev" solutions="big-data" manager="paulettm" editor="cgronlun" />
...
# 開始在 HDInsight 中搭配使用 HBase 與 Hadoop (英文)
...
> [WACOM.NOTE] HBase (0.98.0 版) 只能與 HDInsight 上的 HDInsight 3.1 叢集一起使用 (以 Apache Hadoop 和 YARN 2.4.0 為基礎)。如需版本資訊，請參閱 [HDInsight 在 Hadoop 叢集版本中提供的新功能？][HDInsight 在 Hadoop 叢集版本中提供的新功能？]
...
-   一個 Azure 儲存體帳戶。如需相關指示，請參閱[如何建立儲存體帳戶][如何建立儲存體帳戶]。
-   Visual Studio 的複本。
...
-   [在 Azure 入口網站中佈建 HBase 叢集][在 Azure 入口網站中佈建 HBase 叢集]
-   [從 HBase Shell 建立 HBase 範例資料表][從 HBase Shell 建立 HBase 範例資料表]
-   [使用 Hive 查詢 HBase 資料表][使用 Hive 查詢 HBase 資料表]
-   [使用 HBase C# API 建立 HBase 資料表，並從資料表擷取資料][使用 HBase C# API 建立 HBase 資料表，並從資料表擷取資料]
-   [摘要][摘要]
-   [後續步驟][後續步驟]
...
1.  遵循 [Marlin 的專案頁面][Marlin 的專案頁面] (英文) 中的「下載 Marlin 專案」所說明的建置步驟。將其解壓縮至本機目錄。
...
[HDInsight HBase 概觀][HDInsight HBase 概觀]：
HBase 是建置於 Hadoop 上的 Apache 開放原始碼 NoSQL 資料庫，可針對大量非結構化及半結構化資料，提供隨機存取功能和強大一致性。

[在 Azure 虛擬網路上佈建 HBase 叢集][在 Azure 虛擬網路上佈建 HBase 叢集]：
