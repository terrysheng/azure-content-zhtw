...
HDInsight 會透過新增次要前端節點 (Head Node1)，將此單一失敗點移除。已加入 [ZooKeeper][zookeeper] 節點 (ZK)，並使用於前端節點的領袖選擇，以確保背景工作節點和閘道 (GW) 知道當作用中前端節點 (Head Node0) 變成非作用中狀態時，容錯移轉至次要前端節點 (Head Node1) 的時機。
