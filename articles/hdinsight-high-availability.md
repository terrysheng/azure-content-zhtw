<properties 
	pageTitle="HDInsight 中的 Hadoop 叢集可用性 | Azure" 
	description="HDInsight 部署高可用性和可靠性的叢集。" 
	services="hdinsight" 
	editor="cgronlun" 
	manager="paulettm" 
	authors="bradsev" 
...
## 如何檢查作用中前端節點的服務狀態 ##
若要判斷哪個前端節點處於作用中狀態，並要檢查在該前端節點上執行的服務狀態，您必須使用遠端桌面通訊協定 (RDP) 連線到 Hadoop 叢集。依預設，系統會停用 Azure 中的遠端進入叢集功能，因此您必須首先將它啟用。如需如何在入口網站中執行此動作的指示，請參閱[使用遠端桌面連線到 HDInsight 叢集](../hdinsight-administer-use-management-portal/#rdp)
...
SDK 的情況十分類似。[使用 HDInsight .NET SDK](../hdinsight-provision-clusters/#sdk) 中記載使用 SDK 建立與佈建叢集的說明。超大型前端節點的組態要求將  `HeadNodeSize = NodeVMSize.ExtraLarge` 參數加入此程式碼中所使用的  `ClusterCreateParameters()` 方法。
...
- [ZooKeeper][zookeeper]
- [使用遠端桌面連線到 HDInsight 叢集](../hdinsight-administer-use-management-portal/#rdp)
