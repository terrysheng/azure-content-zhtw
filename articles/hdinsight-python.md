...
> [AZURE.NOTE] 本文中的步驟適用於 HDInsight 叢集 2.1、3.0 和 3.1 版。
...
2. 此  **SELECT TRANSFORM ...USING 'D:\Python27\python.exe streaming.py'** 陳述式從 **hivesampletable** 中選取資料，並將 clientid、devicemake 和 devicemodel 傳給 **streaming.py** 指令碼。

	> [AZURE.NOTE] **USING** 子句指定 python.exe 的完整路徑，因為它不在路徑中。
...
2. 執行串流處理時，有一行包含所有的值，而每個值之間是一個定位字元。因此， `string.split(line, "\t")` 可以在每一個定位字元處分割輸入，只傳回欄位。

3. 處理完成時，輸出必須以一行寫入 STDOUT，而每一個欄位之間是一個定位字元。這是利用 `print "\t".join([clientid, phone_label, hashlib.md5(phone_label).hexdigest()])` 來完成。

4. 這全部發生在  `while` 迴圈內，迴圈會一直重複到沒有讀取到  `line` 為止，此時， `break` 會結束迴圈，指令碼也就終止。
...
2. 使用  下列 PowerShell 指令碼，將 **streaming.py** 和 **jython.py** 檔案上傳至伺服器。在指令碼的前三行，替換您的 Azure HDInsight 叢集的名稱及 **streaming.py** 和 **jython.py** 檔案的路徑。
...
1. 上傳檔案之後，開啟瀏覽器並導覽至 https://YourClusterName.azurehdinsight.net/。提示您輸入認證時，請輸入您叢集的管理員使用者名稱和密碼。
	...
	> [AZURE.NOTE] 在 Azure 管理入口網站中，您也可以使用 HDInsight [**儀表板**] 底部的 [**管理叢集**] 連結來啟動 Hive 儀表板。

2. 使用 [**Hive 編輯器**]，以下列 HiveQL 來取代  `select * from hivesampletable` 這一行。
