<properties title="HDInsight 版本資訊" pageTitle="HDInsight 版本資訊 | Azure" description="HDInsight 版本資訊。" metaKeywords="hdinsight, hadoop, hdinsight hadoop, hadoop azure, release notes" services="HDInsight" solutions="" documentationCenter="" editor="cgronlun" manager="paulettm"  authors="bradsev" />
...
-   使用 Ambari 端點 "https://{clusterDns}.azurehdinsight.net/ambari/api/v1/clusters/{clusterDns}.azurehdinsight.net/services/{servicename}/components/{componentname}" 時，*host\_name* 欄位現在會傳回節點的完整網域名稱 (FQDN)，而不是只有主機名稱。例如，不是傳回 "**headnode0**"，而是傳回 FQDN “**headnode0.{ClusterDNS}.azurehdinsight.net**”。需要此變更，才能在一個虛擬網路 (VNET) 中部署多種叢集類型，例如 HBase 和 Hadoop。例如，使用 HBase 做為 Hadoop 的後端平台時就是這種情形。

-   我們為 HDInsight 叢集的預設部署提供新的記憶體設定。先前的預設記憶體設定並未充分考量到所部署的 CPU 核心數目。下表列舉預設 4 CPU 核心 (8 容器) HDInsight 叢集使用的新記憶體設定(括號中也附帶此版本之前使用的值)。

| 元件                              | 記憶體配置                       |
|-----------------------------------|----------------------------------|
| yarn.scheduler.minimum-allocation | 768MB (先前是 512MB)             |
| yarn.scheduler.maximum-allocation | 6144MB (不變)                    |
| yarn.nodemanager.resource.memory  | 6144MB (不變)                    |
| mapreduce.map.memory              | 768MB (先前是 512MB)             |
| mapreduce.map.java.opts           | opts=-Xmx512m (先前是 -Xmx410m)  |
| mapreduce.reduce.memory           | 1,536MB (先前是 1,024MB)         |
| mapreduce.reduce.java.opts        | opts=-Xmx1024m (先前是 -Xmx819m) |
| yarn.app.mapreduce.am.resource    | 768MB (先前是 1,024MB)           |
| yarn.app.mapreduce.am.command     | opts=-Xmx512m (先前是 -Xmx819m)  |
| mapreduce.task.io.sort            | 256MB (先前是 200MB)             |
| tez.am.resource.memory            | 1,536MB (不變)                   |

如需 HDInsight 使用的 Hortonworks Data Platform 上供 YARN 和 MapReduce 使用的記憶體組態設定的詳細資訊，請參閱＜[決定 HDP 記憶體組態設定][決定 HDP 記憶體組態設定]＞(英文)。Hortonworks 也提供工具來計算適當的記憶體設定。

HDInsight PowerShell/SDK 錯誤："*叢集未設定 Http 服務存取*」：

-   此錯誤稱為[相容性問題][相容性問題]，起因於 SDK/PowerShell 版本和叢集版本的差異。在 8/15 或之後建立的叢集支援佈建到虛擬網路的這項新功能。但舊版的 SDK/PowerShell 無法正確解譯此功能。結果造成某些工作提交作業失敗。如果您使用 SDK API 或 PowerShell Cmdlet 來提交工作 (**Use-AzureHDInsightCluster**、**Invoke-Hive**)，這些作業可能失敗並傳回錯誤訊息「*叢集 <clustername> 未設定 Http 服務存取*」，或者，根據作業而定，傳回其他錯誤訊息，例如「*無法連接到叢集*」。

-   最新版的 HDInsight SDK 和 Azure PowerShell 中已解決這些相容性問題。建議將 HDInsight SDK 更新到 1.3.1.6 版或更新版本，並將 Azure PowerShell Tools 更新到 0.8.8 版或更新版本。您可以從 [NuGet][NuGet] 取得最新的 HDInsight SDK，從＜[如何安裝和設定 Azure PowerShell][如何安裝和設定 Azure PowerShell]＞取得 Azure PowerShell Tools。
...
-   此版本以 Hortonworks Data Platform (HDP) 2.1.5 為基礎。如需此版本中修正的錯誤清單，請參閱 Hortonworks 網站的＜[此版本修正的項目][此版本修正的項目]＞頁面 (英文)。
...
最新版的 SDK 和 Azure PowerShell 中已解決這些相容性問題。建議將 HDInsight SDK 更新到 1.3.1.6 版或更新版本，並將 Azure PowerShell Tools 更新到 0.8.8 版或更新版本。您可以從 [nuget][nuget] 取得最新的 HDInsight SDK，使用 [Microsoft Web PI][Microsoft Web PI] 取得 Azure PowerShell Tools。
...
-   HDInsight 叢集 1.6 版採用以 [Hortonworks Data Platform 1.1][Hortonworks Data Platform 1.1] 為基礎的 Hadoop 散發。
  ...
  [決定 HDP 記憶體組態設定]: http://docs.hortonworks.com/HDPDocuments/HDP2/HDP-2.1-latest/bk_installing_manually_book/content/rpm-chap1-11.html
  [相容性問題]: https://social.msdn.microsoft.com/Forums/azure/en-US/a7de016d-8de1-4385-b89e-d2e7a1a9d927/hdinsight-powershellsdk-error-cluster-is-not-configured-for-http-services-access?forum=hdinsight
  ...
  [此版本修正的項目]: http://docs.hortonworks.com/HDPDocuments/HDP2/HDP-2.1.5/bk_releasenotes_hdp_2.1/content/ch_relnotes-hdp-2.1.5-fixed.html
