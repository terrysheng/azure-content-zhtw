...
3. 執行下列命令來定義 MapReduce 工作： 
	...
	*"-Dmapred.map.tasks=50"* 引數指定要建立來執行工作的 50 的對應。*"-Dmapred.reduce.tasks=25"* 引數指定要建立來執行工作的 25 個縮減工作。最後兩個引數指定輸入和輸出目錄。  
...
<h2><a id="java-code"></a>TeraSort MapReduce 程式的 Java 程式碼</h2>
