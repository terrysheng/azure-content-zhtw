...
> [AZURE.NOTE]此拓撲將預設為建立一個背景工作處理序，這對於示範目的已足夠。如果您要針對生產叢集採用此做法，則應該加入下列內容，然後調整成您想要建立的背景工作數目。
>
...
2. 開啟 **Program.cs** 檔案，並在 `TopologyBuilder topologyBuilder = new TopologyBuilder("EventHubReader");` 行後面緊接著加入下列程式碼：
	...
	`JavaComponentConstructor` 會定義如何在執行階段建構 Java Spout。在此案例中，您會使用稍早加入的事件中樞組態資訊來透過 <a href="http://storm.apache.org/documentation/Clojure-DSL.html" target="_blank">Apache Storm Clojure DSL</a> 設定 Spout。更具體來說，HDInsight 會在執行階段使用此程式碼執行下列動作：

	* 使用您所提供的事件中樞資訊，建立 **com.microsoft.eventhubs.spout.EventHubSpoutConfig** 的新執行個體。

	* 建立 **com.microsoft.eventhubs.spout.EventHubSpout** 的新執行個體，並傳入 **EventHubSpoutConfig** 執行個體。
...
5. 尋找下列程式碼：
	...
	這會指示拓樸使用先前步驟的 **JavaComponentConstructor** 做為 Spout，並將其命名為 "EventHubSpout"。這也可以將此元件的平行處理原則提示設定為事件中樞的資料分割數目。
...
建立 EventHubSpoutConfig 時有數個多載方法。請使用以下資訊來找出最適合您需求的方法。

* EventHubSpoutConfig(String PolicyName, String PolicyKey, String Namespace, String HubName, Int PartitionCount)

    * PolicyName：可從指定中樞讀取的共用存取原則名稱。

    * PolicyKey：共用存取原則的金鑰。

    * Namespace：中樞所存在的 ServiceBus 命名空間。

    * HubName：要從該處讀取的事件中樞名稱

    * PartitionCount：中樞的資料分割數目
...
* EventHubSpoutConfig(String PolicyName, String PolicyKey, String Namespace, String HubName, Int PartitionCount, String ZooKeeperConnection)
    ...
    * ZooKeeperConnection：ZooKeeper 節點的連接字串。針對 Storm on HDInsight 伺服器請保留為空白。
...
* EventHubSpoutConfig(String PolicyName, String PolicyKey, String Namespace, String HubName, Int PartitionCount, String ZooKeeperConnection, Int CheckPointIntervalInSeconds,Int ReceiverCredits)
    ...
    * CheckPointIntervalInSeconds：狀態保存到 Zookeeper 的頻率

    * ReceiverCredits：釋放到 Storm 拓撲之前，批次處理的事件數。
...
* EventHubSpoutConfig(String PolicyName, String PolicyKey, String Namespace, String HubName, Int PartitionCount, String ZooKeeperConnection, Int CheckPointIntervalInSeconds, Int ReceiverCredits, Int MaxPendingMsgsPerPartition, Long EnqueueTimeFilter)
    ...
    * MaxPendingMsgsPerPartition：從中樞擷取的事件數上限。預設為 1024。

    * EnqueueTimeFilter：根據事件加入佇列的時間戳記來過濾事件。
