<properties
	pageTitle="Apache Storm 教學課程：試用 Storm Starter 範例 | Microsoft Azure"
	description="在 HDInsight 上使用 Storm Starter 範例，開始透過 Apache Storm 分析巨量資料。了解如何使用 Apache Storm 即時處理資料。"
	services="hdinsight"
	documentationCenter=""
	authors="Blackmist"
	manager="paulettm"
...
# Apache Storm on HDInsight 教學課程：使用 Storm Starter 範例開始分析巨量資料 
...
2. 按一下左下角的 [新增]****，並指向 [資料服務]****，接著指向 [儲存體]****，然後按一下 [快速建立]****。
	...
	![Azure 入口網站，您可以在此入口網站中使用「快速建立」設定新的儲存體帳戶。](./media/hdinsight-storm-getting-started/HDI.StorageAccount.QuickCreate.png)
...
![含 Storm 儀表板連結的 Azure 入口網站](./media/hdinsight-storm-getting-started/dashboard-link.png)

> [AZURE.NOTE]連接至儀表板時，系統會提示您輸入使用者名稱和密碼。這是您建立叢集時使用的管理員名稱 (**admin**) 和密碼。

載入 Storm 儀表板後，您會看到**** [提交拓撲] (Submit Topology) 表單。

![含提交拓撲表單的 Storm 儀表板](./media/hdinsight-storm-getting-started/submit.png)
...
1. 選取 [Jar 檔案]**** 下拉式清單中的 [StormStarter - WordCount]****。此時應該會使用此範例的參數填入 [類別名稱]**** 和 [其他參數]**** 欄位。
	...
	![已選取 WordCount 的 Storm 儀表板](./media/hdinsight-storm-getting-started/submit.png)
...
2. 按一下 [提交]**** 按鈕。稍後 [結果]**** 欄位會顯示用於提交工作的命令，以及命令的結果。[錯誤]**** 欄位會顯示提交拓撲時發生的所有錯誤。
	...
	![[提交] 按鈕和結果](./media/hdinsight-storm-getting-started/submit-results.png)
...
1. 選取 Storm 儀表板頂端的 [Storm UI]****。這會顯示叢集和所有執行中拓撲的摘要資訊。
	...
	![顯示拓撲摘要的 Storm UI](./media/hdinsight-storm-getting-started/stormui.png)
