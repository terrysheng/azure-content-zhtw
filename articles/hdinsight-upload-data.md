...
Azure HDInsight 在 Azure Blob 儲存體之上提供全功能的 Hadoop 分散式檔案系統 (HDFS)。它設計為 HDFS 延伸模組，可讓 Hadoop 生態系統中的全部元件直接處理它所管理資料，客戶操作起來順暢無比。Azure Blob 儲存體和 HDFS 是不同的檔案系統，非常適合儲存資料和計算該資料。如需有關使用 Azure Blob 儲存體的優點，請參閱[在 HDInsight 上使用 Azure Blob 儲存體][在 HDInsight 上使用 Azure Blob 儲存體]。
...
可透過 [AzCopy][AzCopy]、[Azure PowerShell][Azure PowerShell]、[Azure Storage Client Library for .NET][Azure Storage Client Library for .NET] 或總管工具來存取 Azure Blob 儲存體。以下是一些可用的工具：
...
-   Azure HDInsight 叢集。如需指示，請參閱 [Azure HDInsight 使用者入門][Azure HDInsight 使用者入門]或[佈建 HDInsight 叢集][佈建 HDInsight 叢集]。

## 本文內容

-   [使用 AzCopy 將資料上傳至 Blob 儲存體][使用 AzCopy 將資料上傳至 Blob 儲存體]
-   [使用 Azure PowerShell 將資料上傳至 Blob 儲存體][使用 Azure PowerShell 將資料上傳至 Blob 儲存體]
-   [使用 Azure 儲存體總管將資料上傳至 Blob 儲存體][使用 Azure 儲存體總管將資料上傳至 Blob 儲存體]
-   [使用 Hadoop 命令列將資料上傳至 Blob 儲存體][使用 Hadoop 命令列將資料上傳至 Blob 儲存體]
-   [使用 Sqoop 將資料從 Azure SQL Database 匯入至 Blob 儲存體][使用 Sqoop 將資料從 Azure SQL Database 匯入至 Blob 儲存體]

## <span id="azcopy"></span></a>使用 AzCopy 將資料上傳至 Blob 儲存體

AzCopy 是一個命令列公用程式，可簡化在 Azure Storage 帳戶上進出傳送資料的工作。您可以將它當做獨立工具來使用，也可以將此公用程式納入現有的應用程式中。[下載 AzCopy][下載 AzCopy]。

AzCopy 語法如下：

    AzCopy <Source> <Destination> [filePattern [filePattern...]] [Options]

如需詳細資訊，請參閱 [AzCopy - 上傳/下載 Azure Blob 的檔案][AzCopy]

## <span id="powershell"></span></a>使用 Azure PowerShell 將資料上傳至 Blob 儲存體

Azure PowerShell 是功能強大的指令碼環境，可讓您在 Azure 中控制和自動化工作量的部署與管理。您可以使用 Azure PowerShell 將資料上傳至 Blob 儲存體，供 MapReduce 工作處理資料。如需有關設定工作站來執行 Azure PowerShell 的詳細資訊，請參閱[安裝和設定 Azure PowerShell][安裝和設定 Azure PowerShell]。

**將本機檔案上傳至 Blob 儲存體**

1.  根據[安裝和設定 Azure PowerShell][安裝和設定 Azure PowerShell] 的指示，開啟 Azure PowerShell 主控台視窗。
2.  在下列指令碼中設定前五個變數的值：
...
4.  按一下頁面頂端的 [設定]。
5.  如果尚未啟用遠端桌面，請按一下 [遠端桌面]，然後依指示進行。否則請跳到下一步。
6.  按一下頁面底部的 [連線]。
7.  按一下 [開啟]。
8.  輸入認證，然後按一下 [確定]。
9.  按一下 [是]。
10. 從桌面上，按一下 [Hadoop 命令列]。
11. 下列範例示範如何從 HDInsight 前端節點上的本機檔案系統中，將 davinci.txt 檔案複製到 /example/data 目錄。
...
12. 使用下列命令來列出已上傳的檔案清單：

        hadoop dfs -lsr /example/data

## <span id="sqoop"></span></a> 使用 Sqoop 將資料從 SQL Database/SQL Server 匯入至 HDFS

Sqoop 是一種專門在 Hadoop 和關聯式資料庫之間傳送資料的工具。此工具可讓您從 SQL、MySQL 或 Oracle 等關聯式資料庫管理系統 (RDBMS) 中，將資料匯入至 Hadoop 分散式檔案系統 (HDFS)，在 Hadoop 中使用 MapReduce 或 Hive 傳送資料，然後將資料匯回到 RDBMS。如需詳細資訊，請參閱＜[Sqoop 使用者指南][Sqoop 使用者指南]＞(英文)。

匯入資料之前，您必須知道 Azure SQL Database 伺服器名稱、資料庫帳戶名稱、帳戶密碼和資料庫名稱。

依預設，Azure SQL 資料庫接受來自 Azure 服務 (例如 Azure HDinsight) 的連線。如果此防火牆設定停用，您必須從 Azure 管理入口網站中啟用它。如需建立 SQL 資料庫和設定防火牆規則的相關指示，請參閱[建立和設定 SQL 資料庫][建立和設定 SQL 資料庫]。
...
3.  將指令碼貼到 Azure PowerShell 主控台視窗中來執行。有關從 Azure Blob 儲存體擷取資料檔的 PowerShell 範例，請參閱[開始使用 HDInsight][Azure HDInsight 使用者入門]。

如需有關使用 Sqoop 的詳細資訊，請參閱[在 HDInsight 上使用 Sqoop][在 HDInsight 上使用 Sqoop]。
...
-   [搭配 HDInsight 使用 Pig][搭配 HDInsight 使用 Pig]
  ...
  [Azure PowerShell]: http://msdn.microsoft.com/zh-tw/library/windowsazure/jj152841.aspx
  [Azure Storage Client Library for .NET]: /zh-tw/develop/net/how-to-guides/blob-storage/
  ...
  [Sqoop 使用者指南]: http://sqoop.apache.org/docs/1.4.4/SqoopUserGuide.html
