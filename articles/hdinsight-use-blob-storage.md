...
低成本 Azure Blob 儲存體是強大的一般用途 Hadoop 相容儲存體解決方案，其完美整合了 HDInsight。透過 Hadoop 分散式檔案系統 (HDFS) 介面，HDInsight 中的完整元件集可直接處理 Azure Blob 儲存體中的資料。在本教學課程中，您將了解如何設定 Blob 儲存體的容器，然後處理其中的資料。

將資料儲存在 Azure Blob 儲存體中，您便可安全地刪除用於計算的 HDInsight 叢集，而不會遺失使用者資料。
...
![HDInsight 中的 Hadoop 叢集可在雲端中存取符合成本效益、可擴充的 Hadoop 相容 Azure Blob 儲存體，並將巨量資料儲存在其中。](./media/hdinsight-use-blob-storage/HDI.ASVArch.png "HDInsight 儲存架構")
...
- **儲存體帳戶中連線至叢集的容器：**因為帳戶名稱和金鑰儲存在 *core-site.xml* 中，所以您對這些容器中的 Blob 具有完整存取權。
...
佈建程序及其金鑰中定義的儲存體帳戶會儲存在 %HADOOP_HOME%/conf/core-site.xml。HDInsight 的預設行為是使用 core-site.xml 檔案中定義的儲存體帳戶。因為叢集前端節點 (主要) 有可能會隨時重新安裝映像或進行移轉，屆時將會遺失對這些檔案所做的任何變更，所以我們不建議您編輯 core-site.xml 檔案。
...
Blob 儲存容器以機碼/值組來儲存資料，沒有目錄階層。但是，機碼名稱中可使用 ( / ) 斜線字元，使檔案變成好像儲存在目錄結構中一樣。例如，Blob 的機碼可能是 *input/log1.txt*。實際上，*input* 目錄並不存在，只是因為機碼名稱中有斜線字元，才形成檔案路徑的樣子。
...
##<a id="preparingblobstorage"></a>準備 Azure Blob 儲存體的容器
...
###使用 Azure 入口網站建立 HDInsight 的 Blob 容器
...
[Azure PowerShell][powershell-install] 可用來建立容器。以下 PowerShell 指令碼將示範如何在現有的 Azure 儲存體帳戶中建立 Blob 容器：
...
##<a id="addressing"></a>定址 Azure Blob 儲存體中的檔案

存取 Blob 儲存體中的檔案時，使用的 URI 配置如下：
...
如需關於在工作站安裝和設定 Azure PowerShell 的資訊，請參閱[安裝和設定 Azure PowerShell][powershell-install]。您可以使用 Windows PowerShell 主控台或 Windows PowerShell ISE 來執行 Azure PowerShell Cmdlet。
...
**使用 Azure PowerShell 存取未定義儲存體帳戶的範例**
...
在本文中，您學到如何搭配 HDInsight 使用 Azure Blob 儲存體，也了解 Azure Blob 儲存體是 HDInsight 的基本元件。這可讓您以 Azure Blob 儲存體建立可擴充的長期封存資料取得解決方案，並利用 HDInsight 來揭開儲存資料內的資訊。
