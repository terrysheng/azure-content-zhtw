...
> [WACOM.NOTE] HDInsight 叢集 3.0 版不支援 *asv://* 語法，而且未來版本也不會支援。這表示任何提交至 HDInsight 叢集 3.0 版，且明確使用 “asv://” 語法的工作都會失敗。請改用 *wasb://* 語法。另外，如果工作提交至任何以現有 metastore 建立的 HDInsight 叢集 3.0 版，且 metastore 中使用 asv:// 語法來明確參考資源，則也會失敗。必須使用 wasb:// 來定址資源，以重新建立這些 metastore。
...
## <span id="benefits"></span></a>Azure Blob 儲存體的優點
...
從 Azure 管理入口網站佈建 HDInsight 叢集時，有兩個選項：[快速建立] 和 [自訂建立]。快速建立選項需要事先建立 Azure 儲存體帳戶。相關指示請參閱[如何建立儲存體帳戶][Azure 儲存體帳戶]。
...
## <span id="addressing"></span></a>定址 Blob 儲存體中的檔案
...
\<BlobStorageContainerName\> 可識別 Blob 儲存體容器的名稱。
...
## <span id="powershell"></span></a>使用 Azure PowerShell 存取 Blob

如需有關在工作站安裝和設定 Azure PowerShell 的詳細資訊，請參閱[安裝和設定 Azure PowerShell][Azure PowerShell]。您可以使用 Azure PowerShell 主控台視窗或 PowerShell\_ISE 來執行 PowerShell Cmdlet。
