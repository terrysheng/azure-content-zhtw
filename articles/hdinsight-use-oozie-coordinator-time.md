...
將以時間為基礎的 Oozie 協調器用於 HDInsight
===========================================
...
1.  [什麼是 Oozie](#whatisoozie)
2.  [必要條件](#prerequisites)
3.  [定義 Oozie 工作流程檔案](#defineworkflow)
4.  [部署 Oozie 專案及進行教學課程前置工作](#deploy)
5.  [執行工作流程](#run)
6.  [後續步驟](#nextsteps)
...
1.  Hive 動作會執行一個 HiveQL 指令碼，以計算每個記錄層級類型在 log4j 記錄檔中的出現次數。每一個 log4j 記錄各由一列欄位組成，其中包含 [LOG LEVEL] 欄位以顯示類型和嚴重性。例如：
    ...
    如需 Hive 的詳細資訊，請參閱[在 HDInsight 上使用 Hive](/en-us/documentation/articles/hdinsight-use-hive/)。
...
-   已安裝並設定 Azure PowerShell 的**工作站**。如需指示，請參閱[安裝並設定 Azure PowerShell](/en-us/manage/install-and-configure-windows-powershell/)。若要執行 PowerShell 指令碼，您必須以系統管理員的身分執行 Azure PowerShell，並將執行原則設為 *RemoteSigned*。請參閱[執行 Windows PowerShell 指令碼](http://technet.microsoft.com/en-us/library/ee176949.aspx)。
-   **HDInsight 叢集**。如需建立 HDInsight 叢集的相關資訊，請參閱[佈建 HDInsight 叢集](/en-us/documentation/articles/hdinsight-provision-clusters/)或[開始使用 HDInsight](/en-us/documentation/articles/hdinsight-get-started/)。進行教學課程時，您將需要下列資料：
...
<tr data-morhtml="true"><td data-morhtml="true">HDInsight 叢集名稱</td><td data-morhtml="true">$clusterName</td><td data-morhtml="true"></td><td data-morhtml="true">您將用來執行此教學課程的 HDInsight 叢集。</td></tr>
<tr data-morhtml="true"><td data-morhtml="true">HDInsight 叢集使用者名稱</td><td data-morhtml="true">$clusterUsername</td><td data-morhtml="true"></td><td data-morhtml="true">HDInsight 叢集使用者的使用者名稱。 </td></tr>
<tr data-morhtml="true"><td data-morhtml="true">HDInsight 叢集使用者的密碼</td><td data-morhtml="true">$clusterPassword</td><td data-morhtml="true"></td><td data-morhtml="true">HDInsight 叢集使用者的密碼。</td></tr>
...
-   **Azure SQL Database**。您必須設定 SQL Database Server 的防火牆規則，以允許從您的工作站加以存取。如需建立 SQL 資料庫和設定防火牆的相關指示，請參閱[開始使用 Azure SQL Database](../sql-database-get-started/)。本文將提供建立本教學課程所需之 SQL Database 資料表的 PowerShell 指令碼。

    <table data-morhtml="true" border="1">
<tr data-morhtml="true"><th data-morhtml="true">SQL Database 屬性</th><th data-morhtml="true">PowerShell 變數名稱</th><th data-morhtml="true">值</th><th data-morhtml="true">說明</th></tr>
<tr data-morhtml="true"><td data-morhtml="true">SQL Database 伺服器名稱</td><td data-morhtml="true">$sqlDatabaseServer</td><td data-morhtml="true"></td><td data-morhtml="true">Sqoop 會將資料匯出到的 SQL Database 伺服器。 </td></tr>
<tr data-morhtml="true"><td data-morhtml="true">SQL Database 登入名稱</td><td data-morhtml="true">$sqlDatabaseLogin</td><td data-morhtml="true"></td><td data-morhtml="true">SQL Database 登入名稱。</td></tr>
<tr data-morhtml="true"><td data-morhtml="true">SQL Database 登入密碼</td><td data-morhtml="true">$sqlDatabaseLoginPassword</td><td data-morhtml="true"></td><td data-morhtml="true">SQL Database 登入密碼。</td></tr>
...
4.  **INSERT OVERWRITE 陳述式**可從 log4j Hive 資料表中計算每個記錄層級類型的出現次數，並將輸出儲存至 Azure 儲存體 - Blob (WASB) 位置。

Hive 路徑有已知問題。此問題會在您提交 Oozie 工作時發生。如需修正此問題的指示，請參閱 [TechNet Wiki](http://social.technet.microsoft.com/wiki/contents/articles/23047.hdinsight-hive-error-unable-to-rename.aspx)。
...
1.  建立含有下列內容的文字檔：
    ...
    <table data-morhtml="true" border="1">
<tr data-morhtml="true"><th data-morhtml="true">工作流程變數</th><th data-morhtml="true">說明</th></tr>
<tr data-morhtml="true"><td data-morhtml="true">${jobTracker}</td><td data-morhtml="true">指定 hadoop 工作追蹤器的 URL。在 HDInsight 叢集 2.0 和 3.0 版上請使用 <strong data-morhtml="true">jobtrackerhost:9010</strong>。</td></tr>
<tr data-morhtml="true"><td data-morhtml="true">${nameNode}</td><td data-morhtml="true">指定 hadoop 節點名稱的 URL。請使用預設檔案系統 WASB 位址。例如 <i data-morhtml="true">wasb://&lt;containerName&gt;@&lt;storageAccountName&gt;.blob.core.windows.net</i>。</td></tr>
<tr data-morhtml="true"><td data-morhtml="true">${queueName}</td><td data-morhtml="true">指定工作所將提交到的佇列名稱。請使用 [預設值]<strong data-morhtml="true"></strong>。</td></tr>
 </table>

    <table data-morhtml="true" border="1">
<tr data-morhtml="true"><th data-morhtml="true">Hive 動作變數</th><th data-morhtml="true">說明</th></tr>
<tr data-morhtml="true"><td data-morhtml="true">${hiveDataFolder}</td><td data-morhtml="true">Hive Create Table 命令的來源目錄。</td></tr>
<tr data-morhtml="true"><td data-morhtml="true">${hiveOutputFolder}</td><td data-morhtml="true">INSERT OVERWRITE 陳述式的輸出資料夾。</td></tr>
<tr data-morhtml="true"><td data-morhtml="true">${hiveTableName}</td><td data-morhtml="true">參考 log4j 資料檔案之 Hive 資料表的名稱。</td></tr>
 </table>

    <table data-morhtml="true" border="1">
<tr data-morhtml="true"><th data-morhtml="true">Sqoop 動作變數</th><th data-morhtml="true">說明</th></tr>
<tr data-morhtml="true"><td data-morhtml="true">${sqlDatabaseConnectionString}</td><td data-morhtml="true">SQL Database 連接字串。</td></tr>
<tr data-morhtml="true"><td data-morhtml="true">${sqlDatabaseTableName}</td><td data-morhtml="true">資料所將匯出到的 SQL Database 資料表。</td></tr>
<tr data-morhtml="true"><td data-morhtml="true">${hiveOutputFolder}</td><td data-morhtml="true">Hive INSERT OVERWRITE 陳述式的輸出資料夾。這是 Sqoop Export export-dir 的相同資料夾。</td></tr>
 </table>

    如需 Oozie 工作流程和使用工作流程動作的詳細資訊，請參閱 [Apache Oozie 4.0 文件](http://oozie.apache.org/docs/4.0.0/) (適用於 HDInsight 叢集 3.0 版) 或 [Apache Oozie 3.3.2 文件](http://oozie.apache.org/docs/3.3.2/) (適用於 HDInsight 叢集 2.1 版)。
...
1.  建立含有下列內容的文字檔：
    ...
    Variable | Description ------------------|------------ \${coordFrequency} | Job pause time.頻率一律以分鐘表示。\${coordStart} | Job start time.\${coordEnd} | Job end time.\${coordTimezone} | Oozie 會以未採用 DST 的固定時區 (通常為 UTC ) 處理協調器工作，此時區稱為「Oozie 處理時區」。\${wfPath} | workflow.xml 的路徑。如果工作流程檔案名稱不是預設檔案名稱 (workflow.xml)，您必須加以指定。
...
HDInsight 會使用 Azure Blob 儲存體來儲存資料。我們稱之為 *WASB* 或 *Windows Azure 儲存體 - Blob*。WASB 是 Microsoft 在 Azure Blob 儲存體上的 HDFS 實作。如需詳細資訊，請參閱[搭配 HDInsight 使用 Azure Blob 儲存體](/en-us/documentation/articles/hdinsight-use-blob-storage/)。

當您佈建 HDInsight 叢集時，會將一個 Azure 儲存體帳戶及其下的特定 Blob 儲存體容器指定為預設檔案系統，如同在 HDFS 中一般。除了此儲存體帳戶，您也可以在佈建過程中從相同 Azure 訂閱或不同 Azure 訂閱中新增其他儲存體帳戶。如需有關新增其他儲存體帳戶的詳細資訊，請參閱[佈建 HDInsight 叢集](/en-us/documentation/articles/hdinsight-provision-clusters/)。為簡化本教學課程中使用的 PowerShell 指令碼，所有檔案都會儲存在位於 */tutorials/useoozie* 的預設檔案系統容器中。根據預設，此容器的名稱會與 HDInsight 叢集名稱相同。WASB 語法如下：
...
-   CREATE TABLE 命令會將資料檔案移至預設容器上的 /hive/warehouse/ 資料夾。
...
如需詳細資訊，請參閱 [HDInsight：Hive 內部和外部資料表簡介](http://blogs.msdn.com/b/cindygross/archive/2013/02/06/hdinsight-hive-internal-and-external-tables-intro.aspx)。
...
2.  在底部窗格中執行下列命令，以連接到您的 Azure 訂閱：
    ...
    系統會提示您輸入 Azure 帳號認證。這種新增訂閱連線的方法會逾時，且在 12 小時後，您將必須重新執行 Cmdlet。
...
Azure PowerShell 目前並未提供任何用以定義 Oozie 工作的 Cmdlet。您可以使用 Invoke-RestMethod PowerShell Cmdlet 來叫用 Oozie Web 服務。Oozie Web 服務 API 是 HTTP REST JSON API。如需 Oozie Web 服務 API 的詳細資訊，請參閱 [Apache Oozie 4.0 文件](http://oozie.apache.org/docs/4.0.0/) (適用於 HDInsight 叢集 3.0 版) 或 [Apache Oozie 3.3.2 文件](http://oozie.apache.org/docs/3.3.2/) (適用於 HDInsight 叢集 2.1 版)。

**提交 Oozie 工作**

1.  開啟 Windows PowerShell ISE (在 Windows 8 的 [開始] 畫面上輸入 **PowerShell\_ISE**，然後按一下 **[Windows PowerShell ISE]**。請參閱[在 Windows 8 和 Windows 上啟動 Windows PowerShell](http://technet.microsoft.com/en-us/library/hh847889.aspx))。

2.  將下列指令碼複製到指令碼窗格中，然後設定前十四個變數 (請略過第 6 個變數 \$storageUri)。
...
若要進行工作流程的疑難排解，您可以從叢集前端節點，找出位於 C:\\apps\\dist\\oozie-3.3.2.1.3.2.0-05\\oozie-win-distro\\logs\\Oozie.log 的 Oozie 記錄檔。如需 RDP 的相關資訊，請參閱[使用管理入口網站管理 HDInsight 叢集](/en-us/documentation/articles/hdinsight-administer-use-management-portal/)。
...
-   [開發 HDInsight 的 C\# Hadoop 串流工作](/en-us/documentation/articles/hdinsight-hadoop-develop-deploy-streaming-jobs/)
