...
7. 選取 [節點定價層] 會顯示將針對此叢集建立之節點的相關資訊。背景工作節點數目預設會設為 __4__。請將此設為 __1__，因為這樣就足以用於本教學課程，也能減少叢集成本。您也可以在佈建叢集之後，變更叢集中的節點數目。
...
* __標記__ (![標記圖示](./media/hdinsight-administer-use-portal-linux/tags.png))：標記可讓您設定索引鍵/值組，以定義雲端服務的自訂分類。例如，您可能會建立名為 [專案] 的金鑰，然後使用與特定專案相關聯的所有服務之通用值。
