...
> [AZURE.NOTE]雖然本文章中的個別部分適用於以 Windows 為基礎的 HDInsight 叢集 (例如 Python 與 Hive)，本文件中的許多步驟仍只適用於以 Linux 為基礎的叢集。如需適用於以 Windows 為基礎的叢集的步驟，請參閱[在 HDInsight 中使用 Hive 分析航班延誤資料](hdinsight-analyze-flight-delay-data.md)
