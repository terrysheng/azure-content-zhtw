...
HDInsight 會提供 Hadoop、HBase、Storm 及 Spark 的叢集組態。或者，您可以<a href="http://azure.microsoft.com/documentation/articles/hdinsight-hadoop-customize-cluster/" target="_blank">使用指令碼動作自訂叢集</a>。
...
* **<a  target="_blank" href="http://spark.apache.org/">Apache Spark</a>**：一個開放原始碼平行處理架構，可支援記憶體內部處理，藉此大幅提升巨量資料分析應用程式的效能。請參閱 [Azure HDInsight 上的 Apache Spark](hdinsight-apache-spark-overview.md)。
...
* [使用自訂選項在 Linux 上佈建 HDInsight](hdinsight-hadoop-provision-linux-clusters.md)：了解如何使用自訂選項，透過 Azure 入口網站、Azure CLI 或 Azure PowerShell 在 Linux 上佈建 HDInsight Hadoop 叢集。
