...
1.  在 [Azure 入口網站預覽][Azure 入口網站預覽]中，按一下 [瀏覽]，然後按一下您有興趣監視的資源。
2.  [監視] 透鏡中含有每個 Azure 資源最重要的度量。例如，網站就有「要求」、「錯誤」、「[Web 測試][Web 測試]」和「[分析][分析]」。按一下 [今天的要求和錯誤] 組件，會顯示 [度量] 分頁。
...
![Web hosting plan][Web hosting plan]
