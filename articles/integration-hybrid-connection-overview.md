...
| 80         | HTTP 連接埠；用於憑證驗證。                                                |
| 443        | HTTPS 連接埠                                                               |
| 5671       | 用來連接到 Azure。如果 TCP 連接埠 5671 無法使用，則使用 TCP 連接埠 443。   |
| 9352       | 用來推送和提取資料。如果 TCP 連接埠 9352 無法使用，則使用 TCP 連接埠 443。 |

## 下一步

-   [建立和管理混合式連線][建立和管理混合式連線]
...
-   [用於管理 Azure 上之 BizTalk 服務的 REST API][用於管理 Azure 上之 BizTalk 服務的 REST API]
