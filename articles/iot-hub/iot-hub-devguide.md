...
* [Apache Storm Spout](../hdinsight/hdinsight-storm-develop-csharp-event-hub-topology.md)。您可以在[這裡](https://github.com/apache/storm/tree/master/external/storm-eventhubs)找到 Spout 原始檔的連結
...
IoT 中樞提供從應用程式後端 (服務或雲端) 至裝置 (以及相反方向) 通訊的傳訊基礎。我們將這些功能稱為[裝置到雲端](#d2c)和[雲端到裝置](#c2d)。
...
每個 IoT 中樞都會以特定 SKU 中特定數目的單位進行佈建 (如需詳細資訊，請參閱 [Azure IoT 中樞定價][lnk-pricing])。SKU 和單位數目可決定可以傳送的每日訊息配額上限以及身分識別登錄中的裝置身分識別數目上限。同時連接的裝置數目受限於登錄中的識別數目。
