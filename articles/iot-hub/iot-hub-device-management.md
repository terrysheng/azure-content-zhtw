...
遠端監視預先設定的解決方案用來建立裝置物件，然後將它傳送到服務的程式碼定義於 **Simulator/Simulator.WorkerRole/SimulatorCore/Devices/DeviceBase.cs** 中。查看 `SendDeviceInfo` 和 `GetDeviceInfo` 方法，您即可看到如何建立自我描述的裝置模型並將它傳送至 Azure IoT 中樞。

雲端服務用來處理裝置模型相關事件的程式碼位於 **EventProcessor/EventProcessor.WorkerRole/Processors/DeviceManagementProcessor.cs**。大部分涉及對傳送至預先設定之解決方案服務端的裝置模型相關訊息採取行動的工作會發生於 `ProcessJToken` 方法。

一旦收到裝置模型相關訊息之後，位於 **DeviceManagement/Infrastructure/BusinessLogic/DeviceLogic.cs** 的 `UpdateDeviceFromSimulatedDeviceInfoPacketAsync` 和 `UpdateDeviceFromRealDeviceInfoPacketAsync` 方法接著會負責更新裝置登錄。您可以在 **DeviceManagement/Web/WebApiControllers/DeviceApiController.cs** 中找到遠端監視預先設定的解決方案中的裝置登錄 API。
...
* **直接連接**︰若要透過直接連接到裝置 (例如 Linux 上的 SSH、Windows 上的遠端桌面，或透過遠端偵錯工具) 來啟用遠端控制，您必須能夠建立裝置的連線。假設在開放式網際網路中公開裝置會有安全性風險，建議使用轉送服務 (例如 Azure [服務匯流排轉送服務](service-bus-relay-overview.md)) 來進行裝置的連線和雙向流量傳輸。由於轉送連線是來自裝置的輸出連線，所以有助於限制裝置上開啟的 TCP 連接埠的受攻擊面。
...
系統管理員會將裝置健康情況界限定義為裝置操作資料 (通常移動快速) 和裝置中繼資料 (通常移動緩慢) 的功能。您可以使用規則來啟用裝置健康情況警示系統，其會透過串流處理引擎 (例如 [Azure 串流分析](https://azure.microsoft.com/services/stream-analytics/)) 而非輪詢策略來實作。
