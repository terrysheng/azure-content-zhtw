...
-   **IoT 通訊協定和擴充性**。如果您的解決方案不能使用裝置程式庫，Azure IoT 中樞會公開可讓裝置以原生方式使用 HTTP 1.1 和 AMQP 1.0 通訊協定的公用通訊協定。您也可以擴充 IoT 中樞，以利用 [Azure IoT 通訊協定閘道][protocol-gateway]開放原始碼元件，來提供支援給 MQTT v3.1.1。您可以在雲端或內部部署中執行 Azure IoT 通訊協定閘道，並將其擴充以支援自訂通訊協定。
...
-   **可靠的雲端到裝置傳訊 (或「命令」)。** 解決方案後端可以使用 IoT 中樞傳送訊息給個別裝置 (含至少一次的傳遞保證)。每個訊息都有個別的存留時間設定，且後端可以要求傳遞和到期的回條，以確保雲端到裝置訊息的生命週期有完整可見度。這樣您就可以實作包含在裝置上執行之作業的商務邏輯。
...
IoT 解決方案中的閘道通常是部署於雲端中的[通訊協定閘道][lnk-gateway]或使用您的裝置本機部署的[領域閘道][lnk-field-gateway]。通訊協定閘道會執行通訊協定轉譯，例如 AMQP 到 MQTT。領域閘道提供裝置的本機管理服務，可以是在現有硬體一部分上執行的特殊裝置或軟體。這兩種閘道類型可做為您的裝置與 IoT 中樞之間的媒介。
...
Azure IoT 中樞會在實作[服務輔助通訊][lnk-service-assisted-pattern]模式時，調節您的裝置與解決方案後端之間的互動。服務輔助通訊的目標是要在 IoT 中樞等控制系統，與部署在不受信任實體空間中的特殊用途裝置之間，建立可信任的雙向通訊路徑。該模式會建立下列原則：
...
行動產業已大規模成功使用服務輔助通訊模式來實作推播通知服務，例如 [Windows 通知服務][lnk-wns]、[Google 雲端通訊][lnk-google-messaging]和 [Apple Push Notification Service][lnk-apple-push]。
