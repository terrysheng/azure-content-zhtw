...
1.  在儀表板的左下角按一下 [加入裝置]。![][1]
2.  在自訂裝置上，按一下 [新增] 按鈕。

    ![][2]
3.  輸入名稱 (例如 realdevice1) 來選擇您自己的裝置識別碼，然後按一下 [檢查識別碼] 以確定該名稱尚未使用。![][3]

5. 請複製您提供的認證 (裝置識別碼、IoT 中樞主機名稱和裝置金鑰)。稍後在您的用戶端應用程式將會需要這些資訊，才能將裝置連接至方案。![][4]
...
5. 儲存您的變更並建置範例。若要建立您的範例，可以執行 **c/build\_all/linux** 目錄中的 build.sh。
...
1. 啟動 Visual Studio 2015 的新執行個體。開啟您的儲存機制本機副本 **c\\serializer\\build\\windows** 資料夾中的 **remote\_monitoring.sln** 方案。

2. 在 Visual Studio 中，於**方案總管**中瀏覽至 samples 資料夾。在 **remote\_monitoring** 專案中，開啟 **remote\_monitoring.c** 檔案。
...
8. 現在，前往命令與控制區段，以了解如何從遠端監視方案變更您的裝置上的溫度。

### 在 mbed 上執行您的裝置

下列指示描述將[啟用 mbed 的 Freescale FRDM-K64F](https://developer.mbed.org/platforms/FRDM-K64F/) 裝置連接至 Azure IoT 中心的步驟。
...
- 您可以在匯入這個專案的 mbed 編譯器中看到匯入的各種程式庫。某些是由 Azure IoT 小組提供和維護 ([azureiot\_common](https://developer.mbed.org/users/AzureIoTClient/code/azureiot_common/)、[iothub\_client](https://developer.mbed.org/users/AzureIoTClient/code/iothub_client/)、[iothub\_amqp\_transport](https://developer.mbed.org/users/AzureIoTClient/code/iothub_amqp_transport/)、[iothub\_http\_transport](https://developer.mbed.org/users/AzureIoTClient/code/iothub_http_transport/)、[proton-c-mbed](https://developer.mbed.org/users/AzureIoTClient/code/proton-c-mbed/))，而有些則是可以在 mbed 程式庫目錄中取得的第三方廠商程式庫。
...
將已註冊的裝置和資料視覺化。回到遠端監視方案儀表板。您應該會看到在 [裝置] 清單上，裝置的狀態已變更為「執行中」。![][18]
...
4. 開啟 shell (Linux) 或 Node.js 命令提示字元 (Windows) 並導覽至 **node\\samples** 資料夾。然後使用下列命令執行範例應用程式：
