<properties
 pageTitle="什麼是 Azure IoT 預先設定解決方案？ | Microsoft Azure"
...
您可以使用 Azure 訂用帳戶將實作常見物聯網 (IoT) 案例的預先設定解決方案部署至 Microsoft Azure。這些預先設定解決方案可以用來：- 做為 IoT 解決方案的起始點。- 了解 IoT 解決方案中設計和開發的常見模式。

每個預先設定解決方案都會實作一個常見 IoT 案例，且為完整的端對端實作。

除了部署並執行 Azure 中的預先設定解決方案之外，您也可以下載完整的原始程式碼來自訂和擴充解決方案，以符合特定需求。

可用的預先設定解決方案為：- 遠端監視 - 預測性維護 (即將推出)
...
此預先設定解決方案中的 IoT 中心執行個體與典型 IoT 解決方案架構中的「IoT 後端應用程式」相對應。

### Azure 串流分析
預先設定解決方案使用 Azure 串流分析工作來篩選來自冷卻器的事件資料流。其中一項工作會將所有的遙測資料傳送到冷儲存體的 Azure 儲存體 Blob。第二項工作會篩選命令回應訊息和裝置狀態更新訊息的事件資料流，並傳送這些特定訊息至 Azure 事件中樞端點。第三項工作會篩選已觸發的警示，並在解決方案入口網站之儀表板檢視中的警示歷程記錄資料表內顯示這些警示。
...
### 解決方案入口網站
解決方案入口網站是 Web 型的 UI，可讓您：- 在儀表板中檢視遙測和警示歷程記錄。- 佈建新裝置。- 管理並監視裝置。- 傳送命令至特定裝置。- 管理規則和動作。

注意：解決方案入口網站的裝置檢視會將 IoT 中心裝置身分識別登錄與 DocumentDB 資料庫中的豐富裝置狀態資訊維持同步。


## 後續步驟
若要開始使用 Azure IoT 預先設定解決方案，請探索下列資源：- [Azure IoT 預先設定解決方案概觀](iot-suite-overview.md) - [IoT 預先設定解決方案入門](iot-suite-getstarted-preconfigured-solutions.md)
