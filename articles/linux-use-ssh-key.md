...
2.  使用 `openssl` 來產生帶有 2048 位元 RSA 金鑰組的 X509 憑證。請回答 `openssl` 提示的一些問題 (您也可以將這些問題保留空白)。平台並未使用這些欄位的內容：
...
4.  建立 Linux 虛擬機器時上傳 `myCert.pem`。佈建程序會自動將這個憑證中的公開金鑰安裝到虛擬機器中指定使用者的 `authorized_keys` 檔案。
...
2.  使用 `ssh` 連接到 Linux 虛擬機器。第一次登入時，將提示您接受主機公開金鑰的指紋。
...
1.  從下列位置下載並安裝 GitHub for Windows：[http://windows.github.com/][2]
2.  從 [開始] 功能表 > [所有程式] > [GitHub, Inc] 執行 Git Shell

### 使用 cygwin

1.  從下列位置下載並安裝 Cygwin：[http://cygwin.com/][3]
...
1.  從下列位置下載並安裝 puttygen：[http://www.chiark.greenend.org.uk/~sgtatham/putty/download.html][4]
2.  執行 `puttygen.exe`
3.  按一下功能表：[File] > [Load a Private Key]
4.  找出您的私密金鑰，名稱為 `myPrivateKey.key`。您將需要變更檔案篩選，顯示**所有檔案 (*.*)**
5.  按一下 **開啟**。您將看見提示，看起來如下所示：
    
    ![linuxgoodforeignkey](./media/linux-use-ssh-key/linuxgoodforeignkey.png)

6.  按一下 **確定**。 7.  按一下以下螢幕擷取畫面強調顯示的 **Save Private Key**：
...
1.  從下列位置下載並安裝
    putty：[http://www.chiark.greenend.org.uk/~sgtatham/putty/download.html][4]
2.  執行 putty.exe
