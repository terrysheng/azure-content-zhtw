<properties 
   pageTitle="使用 Azure 入口網站在資源管理員中開始建立內部負載平衡器 | Microsoft Azure"
...
>[AZURE.NOTE] 如果看不到想要使用的虛擬網路，請檢查您為負載平衡器使用的 [位置]，並據以變更它。

6. 按一下 [子網路]，然後選取您要建立負載平衡器的子網路。
6. 在 [IP 位址指派] 下，按一下 [動態] 或 [靜態]，視您想要負載平衡器的 IP 位址固定 (靜態) 或不固定而定。
