...
-   在 ByRow 的述詞中，**s** 參數代表輸入層 Pixels 之節點矩形陣列的索引，而 **d** 參數代表隱藏層 ByRow 之節點陣列的索引。**s** 和 **d** 的類型為長度二之整數的 Tuple。在概念上，**s** 的範圍介於所有 0 \<= s[0] \< 10 與 0 \<= s[1][1] \< 20 的整數對之間，而 **d** 的範圍介於所有 0 \<= d[0] \< 10 與 0 \<= d[1][1] \< 12 的整數對之間。
...
-   每個核心的加權數為 1 + **KernelShape**[0] \* **KernelShape**[1][1] \* **KernelShape**[2] = 1 + 1 \* 5 \* 5 = 26，或是 26 \* 50 = 1300。
-   您可以用下列方式計算每個隱藏層中的節點數：

    -   **NodeCount**[0] = (5 - 1) / 1 + 1 = 5。
    -   **NodeCount**[1][1] = (13 - 5) / 2 + 1 = 5。
    -   **NodeCount**[2] = (13 - 5) / 2 + 1 = 5。
-   節點總數可使用該層的宣告維度 [50, 5, 5] 來計算，如下所示：**MapCount** \* **NodeCount**[0] \* **NodeCount**[1][1] \* **NodeCount**[1][1] = 10 \* 5 \* 5 \* 5
