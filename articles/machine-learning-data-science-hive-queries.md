...
使用者可以執行類似 `hive -e "<您的 Hive 查詢>;` 的查詢，在 Hadoop 命令列中直接提交簡單的 Hive 查詢。在下列範例中，紅色方塊框起來的是提交 Hive 查詢的命令，而綠色方塊框起來的則是 Hive 查詢的輸出。
...
使用者可以也在 Hadoop 命令列中執行 `hive` 命令，先進入 Hive 命令主控台，然後在 Hive 命令主控台中提交 Hive 查詢。範例如下。在此範例中，這兩個紅色方塊反白顯示的命令分別是用來進入 Hive 命令主控台，以及在 Hive 命令主控台中提交 Hive 查詢。綠色方塊反白顯示的是 Hive 查詢的輸出。 
...
在下列範例中，Hive 查詢的輸出會寫入 Hadoop 叢集預設容器內的 Blob 目錄 `queryoutputdir`。在此處，您只需提供目錄名稱，而不需提供 Blob 名稱。如果您同時提供目錄和 Blob 名稱，即會擲回錯誤，例如 `wasb:///queryoutputdir/queryoutput.txt`。 
...
使用者也可以在 Web 瀏覽器中輸入 URL `https://<Hadoop cluster name>.azurehdinsight.net/Home/HiveEditor` 來使用查詢主控台 (Hive 編輯器) (系統將要求您輸入 Hadoop 叢集認證以進行登入)，或者可以[使用 PowerShell 提交 Hive 工作](hdinsight-submit-hadoop-jobs-programmatically.md#hive-powershell)。 
...
> [AZURE.NOTE] Hive 查詢範例假設資料已上傳至 Azure HDInsight Hadoop 叢集中的 Hive 資料表。如果沒有，請依照[建立資料並載入 Hive 資料表](machine-learning-data-science-hive-tables.md) 執行，先將資料上傳至 Hive 資料表。
...
在此範例中，會設定變數 `smooth_param1` 和 `smooth_param2`，以減緩從資料計算得來的風險值。風險範圍介於 -Inf 和 Inf 之間。風險 > 0 代表目標等於 1 的機率大於 0.5。 
