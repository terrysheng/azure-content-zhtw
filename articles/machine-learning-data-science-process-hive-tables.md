...
。請注意，系統將要求您輸入 Hadoop 叢集認證以進行登入。或者，您可以[使用 PowerShell 提交 Hive 工作](hdinsight-submit-hadoop-jobs-programmatically.md#hive-powershell)。 
...
此查詢中所使用的欄位是上車與下車位置的 GPS 座標，其名稱為 pickup\_longitude、pickup\_latitude、dropoff\_longitude 及 dropoff\_latitude。計算上車與下車座標間直線距離的查詢如下：
