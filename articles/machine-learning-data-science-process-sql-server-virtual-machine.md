<properties 
	pageTitle="處理 SQL Azure 的資料 | Azure" 
...
您可以使用下列方式來將位置資訊功能化，以分隔出區域、位置及縣 (市) 資訊。請注意，其中一個也可以呼叫 REST 端點，例如，可在 `https://msdn.microsoft.com/library/ff701710.aspx` 上取得的 Bing Maps API，以取得區域或學區資訊。
...
新產生的功能可當成資料行新增至現有資料表或儲存於新的資料表中，並與原始資料表加以聯結以進行機器學習服務。如果已經建立功能，就可以使用 Azure ML 中的 *Reader Module* 來產生或存取功能，如下所示：
...
現在您可以利用[在資料科學環境中處理 Azure Blob 資料](machine-learning-data-science-process-data-blob.md)主題中所涵蓋的方式來使用 Pandas 資料框架。
