<properties 
	pageTitle="在 Azure 上 SQL Server 中進行資料取樣 | Azure" 
...
本文件將說明如何使用下列方式，對儲存於 Azure 上 SQL Server 中的資料進行取樣：

1. [使用 SQL](#sql)
2. [使用 Python 程式設計語言](#python) 

**注意**
>本文件中的 SQL 程式碼範例假設資料位於 Azure 上的 SQL Server 中。如果不是，請參閱雲端資料科學程序圖，以取得將資料移至 Azure 上的 SQL Server 中的指示。
...
本節示範如何使用 pyodbc 程式庫連接到 Python 中的 SQL Server 資料庫。資料庫連接字串如下所示：(使用您的設定來取代伺服器名稱、dbname、使用者名稱和密碼)：
...
您可以使用下列程式碼範例，將向下取樣的資料儲存至檔案，並將它上傳至 Azure Blob。Blob 中的資料可以使用 *Reader Module*，直接讀取至 Azure ML 實驗。步驟如下： 
