...
2.  開啟時，Management Studio 會顯示 [**連接至伺服器**] 對話方塊。在 [**伺服器名稱**] 方塊中，輸入虛擬機器的名稱，以利用物件總管連接 Database Engine。除了虛擬機器名稱之外，您還可以使用 **(本機)**，也可以將一個句點當作 **伺服器名稱**。選取 [**Windows 驗證**]，並保留 [**使用者名稱**] 方塊中的 ***your_VM_name*\\your_local_administrator**。按一下 [**連接**]。
