<properties 
	pageTitle="設定適用於資料科學的 Azure SQL Server 虛擬機器" 
...
# 設定適用於資料科學的 Azure SQL Server 虛擬機器
...
## <a name="Provision"></a>連接到 Azure 管理入口網站並佈建 SQL Server 虛擬機器

1.  使用您的帳戶登入 [Azure 管理入口網站](http://manage.windowsazure.com/)。 
	如果您沒有 Azure 帳戶，請造訪 [Azure 免費試用](http://www.windowsazure.com/pricing/free-trial/)。

2.  在 Azure 管理入口網站中，依序按一下網頁左下角的 [**+新增**]、[**運算**]、[**虛擬機器**] 和 [**從組件庫**]。

3.  在 [**建立虛擬機器**] 頁面上，根據您的資料需求選取含有 SQL Server 的虛擬機器映像，然後按一下頁面右下角的 [下一步] 箭號。如需 Azure 支援的 SQL Server 映像最新的資訊，請參閱 [Azure 虛擬機器中的 SQL Server](http://go.microsoft.com/fwlink/p/?LinkId=294719) 文件集內的 [開始使用 Azure 虛擬機器中的 SQL Server](http://go.microsoft.com/fwlink/p/?LinkId=294720) 主題。

	![Select SQL Server VM][1]

4.  在第一個 [**虛擬機器設定**] 頁面上，提供下列資訊：

    -   提供**虛擬機器名稱**。
    -   在 [**新增使用者名稱**] 方塊中，輸入 VM 本機系統管理員帳戶的唯一使用者名稱。
    -   在 [**新密碼**] 方塊中，輸入強式密碼。如需詳細資訊，請參閱 [增強式密碼](http://msdn.microsoft.com/library/ms161962.aspx)。
    -   在 [**確認密碼**] 方塊中，重新輸入密碼。
    -   從 [**大小**] 下拉式清單中選取適當的大小。

     > [AZURE.NOTE] 虛擬機器的大小可在佈建期間指定：我們建議生產工作負載採用 A2 大小。使用 SQL Server Enterprise Edition 時，虛擬機器的最小建議大小為 A3。當您使用 SQL Server Enterprise Edition 時，請選取 A3 或以上的大小。使用 SQL Server 2012 或 2014 Enterprise Optimized for Transactional Workloads 映像時，請選取 A4。使用 SQL Server 2012 或 2014 Enterprise Optimized for Data Warehousing Workloads 映像時，請選取 A7。選取的大小會限制可設定的資料磁碟數目。如需可用之虛擬機器大小和可連接至虛擬機器之資料磁碟數目的最新資訊，請參閱 [Azure 的虛擬機器大小](http://msdn.microsoft.com/library/azure/dn197896.aspx)。如需定價資訊，請參閱[虛擬機器定價](http://azure.microsoft.com/pricing/details/virtual-machines/)。

    按一下右下角的 [下一步] 箭頭以繼續操作。

    ![VM Configuration][2]

5.  在第二個 [**虛擬機器設定**] 頁面上，設定適用於網路、儲存體和可用性的資源：
...
6.	在 [**端點**] 區段中，按一下 [**名稱**] 底下的空白下拉式清單並選取 [**MSSQL**]，然後輸入 Database Engine 執行個體的連接埠號碼 (適用於預設執行個體的是 **1433**)。

7.  您的 SQL Server VM 也可以用來做為 IPython Notebook 伺服器 (將在稍後步驟中設定)。 
	新增端點以指定可供 IPython Notebook 伺服器使用的連接埠。在 [**名稱**] 資料行中輸入名稱、選取您為公用連接埠選擇的連接埠號碼，然後針對私用連接埠選擇 9999。

	按一下右下角的 [下一步] 箭頭以繼續操作。

	![Select MSSQL and IPython ports][3]

8.  接受已勾選預設的 [**安裝 VM 代理程式**] 選項，然後按一下精靈右下角的勾號，以完成 VM 佈建程序。
...
1.  佈建完成時，按一下虛擬機器的名稱以前往 [儀表板] 頁面。按一下頁面底部的 [**連接**]。

2.  選擇使用 Windows 遠端桌面程式 (`%windir%\system32\mstsc.exe`) 來開啟 rpd 檔案。

3.  在 [**Windows 安全性**] 對話方塊中，提供在先前步驟中指定的本機系統管理員帳戶密碼。(系統可能會要求您確認虛擬機器的認證。)

4.  當您第一次登入此虛擬機器時，可能需要完成數個程序，包括設定桌面、Windows 更新，以及完成 Windows 初始設定工作 (sysprep)。在 Windows sysprep 完成後，SQL Server 安裝程式就會完成設定工作。當這些工作完成時，會導致幾分鐘的時間延遲。要等到 SQL Server 安裝程式完成， `SELECT @@SERVERNAME` 才會傳回正確的名稱，而 SQL Server Management Studio 才會出現在起始畫面上。
...
- 以滑鼠右鍵按一下 Windows 開始圖示，然後按一下 [**命令提示字元 (系統管理員)**]
- 複製下列命令並貼至命令提示字元。 
...
- 您可以使用 `https://<virtual_machine_DNS_name>:<port>` 格式的 URL，從任何本機或遠端瀏覽器中存取並執行 IPython Notebook，其中的 port 是您在佈建虛擬機器時所選取的 IPython 公用連接埠。
...
若要連接其他資料磁碟，請依照[如何將資料磁碟連接至 Windows 虛擬機器](storage-windows-attach-disk.md)中所述的步驟，這將引導您逐步完成：
...
SQL Server Database Engine 須有網域環境才能使用 Windows 驗證。若要從另一部電腦連接 Database Engine，請設定 SQL Server 以進行混合模式驗證。混合模式驗證可允許 SQL Server 驗證和 Windows 驗證。需要有 SQL 驗證模式，才能在 [Azure Machine Learning Studio](https://studio.azureml.net) 中使用讀取程式模組，從您的 SQL Server VM 資料庫直接擷取資料。

1.  使用遠端桌面連接到虛擬機器時，請使用 Windows [**搜尋**] 窗格並輸入 **SQL Server Management Studio** (SMSS)。按一下以啟動 SQL Server Management Studio (SSMS)。您可能想要將捷徑新增到桌面上的 SSMS，以供日後使用。

    ![Start SSMS][5]

    當您首次開啟 Management Studio 時，它必須建立使用者 Management Studio 環境。這可能需要花費幾分鐘的時間。

2.  開啟時，Management Studio 會顯示 [**連接至伺服器**] 對話方塊。在 [**伺服器名稱**] 方塊中，輸入虛擬機器的名稱，以利用物件總管連接 Database Engine。除了虛擬機器名稱之外，您還可以使用 **(本機)**，也可以將一個句點當作 **伺服器名稱**。選取 [**Windows 驗證**]，並保留 [**使用者名稱**] 方塊中的 ***your\_VM\_name*\\your\_local\_administrator**。按一下 [**連接**]。

    ![Connect to Server][6]

	<br>

	 > [AZURE.TIP] 您可以使用 Windows 登錄機碼變更，或使用 SQL Server Management Studio，來變更 SQL Server 驗證模式。若要使用登錄機碼變更來變更驗證模式，請啟動 [**新增查詢**]，並執行下列指令碼：
...
4.  在 [**安全性**] 頁面的 [**伺服器驗證**] 下，選取 [**SQL Server 及 Windows 驗證模式**]，然後按一下 [**確定**]。

    ![Select Authentication Mode][8]

5.  在 [SQL Server Management Studio] 對話方塊中，按一下 [**確定**]，以確認重新啟動 SQL Server 的要求。
...
> [AZURE.TIP] 您也可以透過程式設計方式或使用 SQL Server Management Studio，來建立新的 SQL Server 登入。若要以程式設計方式使用 SQL 驗證來建立新的系統管理員使用者，請啟動 [**新增查詢**]，並執行下列指令碼。使用您選擇的使用者名稱和密碼來取代 <new user name\> 和 <new password\>。視需要調整密碼原則 (程式碼範例會關閉原則檢查及密碼到期日)。如需 SQL Server 登入的詳細資訊，請參閱 [建立登入](http://msdn.microsoft.com/library/aa337562.aspx)。  
...
5.  在 [**密碼**] 方塊中，輸入新使用者的密碼。在 [**確認密碼**] 方塊中，再次輸入該密碼。

6.  若基於複雜性和強制性因素而要強制執行密碼原則選項，請選取 [**強制執行密碼原則**] \(建議)。此為選取 SQL Server 驗證時的預設選項。

7.  若要針對逾期強制執行密碼原則選項，請選取 [**強制執行密碼逾期**] \(建議)。您必須選取強制執行密碼原則才能啟用此核取方塊。此為選取 SQL Server 驗證時的預設選項。

8.  若要強制使用者在首次登入後建立新密碼，請選取 [**使用者必須在下次登入時變更密碼**] \(如果此登入是供其他使用者使用，建議選取此選項。如果此登入是供您自己使用，請勿選取此選項。)您必須選取強制執行密碼逾期才能啟用此核取方塊。此為選取 SQL Server 驗證時的預設選項。
...
2.  在 [**連接至伺服器**] 或 [**連接到 Database Engine**] 對話方塊的 [**伺服器名稱**] 方塊中，輸入虛擬機器的 DNS 名稱 (在上一個工作中決定的) 和 *DNSName,portnumber* 格式的公用端點連接埠名稱 (例如 **tutorialtestVM.cloudapp.net,57500**)。

3.  在 [**驗證**] 方塊中，選取 [**SQL Server 驗證**]。

4.  在 [**登入**] 方塊中，輸入在先前工作中建立的登入名稱。

5.  在 [**密碼**] 方塊中，輸入在先前工作中建立的登入密碼。

6.  按一下 [**連接**]。

## <a name="amlconnect"></a>從 Azure Machine Learning 連接到 Database Engine

在雲端資料科學程序的後續階段中，您將使用 [Azure Machine Learning Studio](https://studio.azureml.net) 來建置和部署機器學習服務模型。若要將來自 SQL Server VM 資料庫的資料直接擷取至Azure Machine Learning 以用於訓練或計分，請在新的 [Azure Machine Learning Studio](https://studio.azureml.net) 體驗中使用讀取程式模組。您可以透過雲端資料科學程序圖的連結來找到更多有關本主題的詳細資訊。如需簡介，請參閱[什麼是 Azure Machine Learning Studio?](machine-learning-what-is-ml-studio.md)。

2.	在 [讀取程式模組](https://msdn.microsoft.com/library/azure/dn905997.aspx) 的 [**屬性**] 窗格中，從 [**資料來源**] 下拉式清單選取 [**Azure SQL Database**]。
...
Azure 虛擬機器的定價策略是「**只針對您使用的項目進行付費**」。若要確定您在不使用虛擬機器時不會被計費，其狀態必須是 [**已停止 (已取消配置)**] 狀態。

> [AZURE.NOTE] 從 VM 內部關閉虛擬機器 (使用 Windows 電源選項) 時，雖然 VM 已停止，但仍然處於已配置狀態。若要確保您不會繼續被計費，一律從 [Azure 管理入口網站](http://manage.windowsazure.com/)停止虛擬機器。您也可以藉由呼叫 ShutdownRoleOperation 搭配相當於 "StoppedDeallocated" 的 "PostShutdownAction"，透過 Powershell 來停止 VM。
