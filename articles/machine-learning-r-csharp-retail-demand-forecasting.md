<properties title="Forecasting - ETS + STL " pageTitle="預測 - ETS + STL  | Azure" description="Forecasting - ETS + STL " metaKeywords="" services="machine-learning" solutions="" documentationCenter="" authors="jaymathe" manager="paulettm" editor="cgronlun" videoId="" scriptId="" />
...
此 [Web 服務]( https://datamarket.azure.com/dataset/aml_labs/demand_forecast) 會實作季節性趨勢分解法 (STL) 和指數平滑法模型 (ETS)，以根據使用者所提供的歷史資料產生預測。今年的特定產品需求會增加嗎？為方便有效地規劃庫存，我可以預測聖誕節的產品銷售嗎？預測模型專門處理此類問題。有了過去的資料，這些模型可以檢查隱藏的趨勢和季節性來預測未來的趨勢。  
...
有多種方式能以自動的方式取用此服務 (範例應用程式[如下](http://microsoftazuremachinelearning.azurewebsites.net/StlEtsForecasting.aspx ))。
...
已在 Azure ML 中建立新的空白實驗。已使用預先定義的資料結構描述上傳範例輸入資料。連結至資料結構描述的是 [Execute R Script] 模組，此模組可使用 R 的 'stl'、'ets' 和 'forecast' 函數來產生季節性趨勢分解法 (STL) 和指數平滑法模型 (ETS) 預測模型。 
...
這是一個非常簡單的 ETS+STL 預測範例。從上面的範例程式碼可以看出，未實作錯誤攔截，且服務會假設所有變數都是連續/正數值，而頻率應該是大於 1 的整數。日期和值向量的長度應該相同，且時間序列的長度應該大於 2 倍頻率。日期變數應遵守 'mm/dd/yyyy' 格式。
