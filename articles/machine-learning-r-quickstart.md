<properties title="Quick start guide to R for Azure Machine Learning Studio" pageTitle="Azure Machine Learning Studio 的 R 快速入門指南" description="如何在 Azure Machine Learning Studio 中使用 R" metaKeywords="R azure, r machine learning, azure r machin elearning" services="machine-learning" solutions="" documentationCenter="" authors="larryfr" manager="paulettm" editor="cgronlun"  videoId="" scriptId="" />
...
Azure ML 包含許多功能強大的機器學習和資料操作模組。功能強大的 R 語言被描述為分析通用語言。好消息是，在 Azure ML 中，分析和資料操作皆可藉由使用 R 來加以擴充。這個組合利用 R 的彈性和深入分析，讓 Azure ML 更具延展性且更易於部署。

本文件將協助您使用 R 語言來快速開始擴充 Azure ML。本指南包含您在 Azure ML 內建立、測試及執行 R 程式碼時將會需要的資訊。在您隨著此快速入門指南進行的過程中，您將在 Azure ML 中使用 R 語言來建立一個完整的預測解決方案。  

###預測和資料集

預測是一個獲得廣泛採用且相當實用的分析方法。常見的用法範圍可從預測季節性項目的銷售額、判斷最佳的庫存量，一直到預測總體經濟變數。進行預測時通常是搭配時間序列模型。

時間序列資料係指其當中的值具有時間索引的資料。時間索引可以具規則性 (例如每個月或每分鐘) 或不具規則性。時間序列模型會根據時間序列資料。R 包含時間序列資料的彈性架構和廣泛分析。

在本快速入門指南中，我們將使用加州乳製品產量和定價資料。這些資料包括數項乳製品之產量及奶油 (基準商品) 價格的每月相關資訊。

本文中使用的資料以及 R 指令碼可[從這裡下載][download]。此資料原先是綜合自威斯康辛大學，網址為 http://future.aae.wisc.edu/tab/production.html。

###	組織

我們將循序進行數個步驟，讓您了解如何在 Azure ML 環境中建立、測試及執行分析和資料操作 R 程式碼。  

* 首先，我們將探討在 Azure ML Studio 環境中使用 R 語言的基本概念。

* 然後，我們將接著討論 Azure ML 環境中資料 I/O、R 程式碼及圖形的各個方面。
...
* [在 ML Studio 中與 R 互動](#mlstudio)

* [將資料輸入執行 R 指令碼模組及從此模組輸出](#scriptmodule)

* [資料篩選和轉換](#filtering)

* [時間序列物件和相互關聯分析](#timeseries)

* [時間序列範例：季節性預測](#seasonalforecasting)

* [附錄 A - RStudio 指南](#appendixa)

* [附錄 B - 進階閱讀](#appendixb)

##<a id="mlstudio"></a>在 ML Studio 中與 R 互動

本節將帶領您了解在 ML Studio 環境中與 R 互動的一些基本概念。R 語言提供一個功能強大的工具，可在 Azure ML 環境內建立自訂的分析和資料操作模組。

我將使用 RStudio 來進行小規模的 R 程式碼開發、測試及偵錯。然後，將此程式碼剪下並貼到 ML Studio 中已準備好要執行的 [執行 R 指令碼] 模組中。  

###執行 R 指令碼模組

在 ML Studio 中，R 指令碼是在 [執行 R 指令碼] 模組內執行。圖 1 顯示 ML Studio 中 [執行 R 指令碼] 模組的範例。

 ![The ML Studio environment showing the Execute R Script module selected][1]

*圖 1.顯示已選取 [執行 R 指令碼] 模組的 ML Studio 環境。*
...
使用複雜的 R 函式時，建議您在 RStudio 中進行編輯、測試及偵錯。與進行任何軟體開發相同，請以累加方式擴充您的程式碼，並在小型的簡單測試案例上進行測試。然後，將您的函式剪下並貼到 [執行 R 指令碼] 模組的 [R 指令碼] 視窗中。這個方法既可讓您控制 RStudio IDE，也可讓您控制 Azure ML 的強大功能。  
...
如果您正在使用 Azure ML 為 Web 服務開發 R 程式碼，您應該明確地規劃程式碼將如何處理非預期的資料輸入和例外狀況。為了清楚起見，在所示範的大多數程式碼中，並未包含太多有關檢查或例外狀況處理的部分。不過，隨著我們繼續進行，我將會提供您幾個使用 R 例外狀況處理功能的函式範例。  

如果您需要更完整的 R 例外狀況處理方式，建議您閱讀[附錄 B - 進階閱讀]列出之 Wickham 所著書籍中適用的小節。(#appendixb)。


####在 ML Studio 中進行 R 偵錯和測試

再次提醒您，建議您在 RStudio 中進行小規模的 R 程式碼測試和偵錯。不過，會有一些您將必須探究「執行 R 指令碼」本身 R 程式碼問題的情況。此外，在 ML Studio 中檢查結果也是相當好的做法。

R 程式碼的執行及在 Azure ML 平台上的執行所產生的輸出主要都在 output.log 中。有些其他資訊會顯示在 error.log 中。  

如果在執行 R 程式碼時，ML Studio 中發生錯誤，您的第一個行動方針應該是查看 error.log。error.log 可能包含可協助您了解並更正錯誤的實用錯誤訊息。若要檢視 error.log，請針對包含錯誤的 [執行 R 指令碼]，按一下 [屬性]**** 窗格上的 [檢視錯誤記錄檔]****。

例如，我執行了 [執行 R 指令碼] 模組中含有未定義之變數 y 的 下列 R 程式碼：
...
Azure ML 附有超過 350 個預先安裝的 R 封裝。您可以使用 [執行 R 指令碼] 模組中的下列程式碼，來擷取預先安裝的封裝清單。
...
如果您目前對此程式碼的最後一行還不了解，請往下閱讀。在本文件的其餘部分，我們將大量討論如何在 Azure ML 環境中使用 R。

###	RStudio 簡介

RStudio 是一個廣泛使用、適用於 R 的整合式開發環境 (IDE)。我將使用 RStudio 對本《快速入門指南》中所用的 R 程式碼進行編輯、測試及偵錯。測試並備妥 R 程式碼之後，您只要從 RStudio 編輯器剪下並貼到 ML Studio 的 [執行 R 指令碼] 模組中即可。  

如果您的桌上型電腦上並未安裝 R，建議您現在安裝。您可以從 Comprehensive R Archive Network (或稱為 CRAN) 免費下載開放原始碼 R，網址為 http://www.r-project.org/。有提供適用於 Windows、MacOS 及 Linux/UNIX 的下載項目。請選擇附近的鏡像，然後依照下載指示進行。   此外，CRAN 也包含大量實用的分析和資料操作封裝。
...
https://support.rstudio.com/hc/zh-tw/sections/200107586-Using-RStudio 有提供 RStudio 的教學課程簡介。
...
###在 ML Studio 中載入和檢查資料

####<a id="loading"></a>載入資料集

我們將從把 **csdairydata.csv** 檔案載入到 Azure ML Studio 中開始。

- 啟動您的 Azure ML Studio 環境。

- 按一下您畫面左下方的 +，然後選取 [資料集]****。

- 使用 [瀏覽]**** 來選取檔案。

- 確定您已經選取 [含標頭的一般 CSV 檔案 (.csv)]****。
...
既然我們在 ML Studio 中已經有一些資料，我們需要建立一個實驗來執行分析。  
...
- 將 [csdairydata.csv 資料集] 的輸出**** 連接到 [執行 R 指令碼]**** 最左邊的輸入 ([資料集1]****)。
...
在這個檢視中，我們會看到許多有用的資訊。[功能類型]**** 資料列顯示 Azure ML Studio 指派給資料集內各資料行的資料類型。我們也會看到該資料集的前幾列。擁有一個類似這樣的快速檢視，對於開始執行任何正式工作來說，是相當好的執行前例行性檢查。

###	第一個 R 指令碼

讓我們建立第一個要在 Azure ML Studio 中實驗的簡單 R 指令碼。我已在 RStudio 中建立並測試下列指令碼：  
...
現在，我需要將此指令碼轉移到我的 Azure ML Studio 中。我可以只利用剪下並貼上。不過，在此案例中，我將透過 Zip 檔案轉移我的 R 指令碼。

###	執行 R 指令碼模組的資料輸入

讓我們看看 [執行 R 指令碼] 模組的輸入。在此範例中，我們將會把加州乳製品資料讀入 [執行 R 指令碼] 模組中。  

[執行 R 指令碼] 模組有三個可能的輸入。視您的應用方式而定，您可以使用這當中的任何一個或所有輸入。使用不接受任何輸入的 R 指令碼也是十分合理的。  

讓我們從左到右看看這當中的每一個輸入。您可以將游標放到輸入上方並閱讀工具提示，來查看每個輸入的名稱。  

####	指令碼組合

「指令碼組合」輸入可讓您將 Zip 檔案的內容傳入到 [執行 R 指令碼] 模組中。您可以使用下列其中一個命令將 Zip 檔案的內容讀入到 R 程式碼中：
...
> [WACOM.NOTE] Azure ML 會將 Zip 中的檔案視為在 src/ 目錄中，因此您必須在您的檔案名稱前面加上此目錄名稱。  

我們已經在[載入資料集]中討論過如何載入資料集(#loading)。在您建立並測試上一節中所示的 R 指令碼之後，請執行下列作業：

1. 將 R 指令碼儲存成 .R 檔案。我將我的指令碼檔案稱為 "simpleplot.R"。  

2.  建立一個 Zip 檔案，然後將您的指令碼複製到此 Zip 檔案。

3.	將您的檔案新增到 ML Studio 中的「資料集」****，將類型指定為 **zip**。您現在應該會在您的資料集內看到的此 Zip 檔案。

4.	將此 Zip 檔案從「資料集」****拖放到「ML Studio 畫布」****上。

5.	將 [Zip 資料]**** 圖示的輸出連接到 [執行 R 指令碼]**** 模組的 [指令碼組合]**** 輸入。

6.	在 [執行 R 指令碼]**** 模組的程式碼視窗中，輸入含有您 Zip 檔案名稱的 `source()` 函式。在我的案例中，我輸入了 `source("src/SimplePlot.R")`。  

7.	確定按一下 [儲存]****。

完成這些步驟之後，[執行 R 指令碼] 模組就會在實驗執行時，執行 Zip 檔案中的 R 指令碼。此時，您的實驗應該會看起來像圖 5。
...
您可以使用 [資料集1] 輸入將矩形資料表傳遞給您的 R 程式碼。在我們的簡單指令碼中，`maml.mapInputPort(1)` 函式會從連接埠 1 讀取資料。這些資料會接著被指派給您程式碼中的資料框架變數名稱。在我們的簡單指令碼中，第一行程式碼會執行這項指派。

```r
cadairydata <- maml.mapInputPort(1)
```

請按一下 [執行]**** 按鈕來執行您的實驗。執行完成時，請按一下 [執行 R 指令碼] 模組，然後按一下 [屬性] 窗格上的 [檢視輸出記錄檔]****。您的瀏覽器中應該會出現一個新頁面，當中顯示 Output.log 檔案的內容。當您向下捲動時，您應該會看到類似下列的內容：
...
> [WACOM.NOTE] 從 [執行 R 指令碼] 模組的 [R 裝置] 輸出可以便利地取得這個相同的列印輸出。我們將在下一節中討論「執行 R」模組的輸出。  
...
[執行 R 指令碼] 模組的 [裝置] 輸出包含訊息和圖形輸出。來自 R 的標準輸出和標準錯誤訊息都會傳送到 [R 裝置] 輸出連接埠。  
...
R 是動態指定類型的語言，這表示會視需要強制將資料類型從一種類型轉換成另一種類型。R 中不可部分完成的資料類型包括數值、邏輯及字元。因素類型可用來簡潔地儲存分類資料。您可以在[附錄 B - 進階閱讀]的參考內容中，找到更多有關資料類型的資訊(#appendixb)。
...
我已經刪除建立散佈圖矩陣的程式碼行，並新增將 'Month' 資料行轉換成因素的程式碼行。在我的實驗中，我將只是把 R 程式碼剪下並貼到 [執行 R 指令碼] 模組的程式碼視窗中。您也可以更新 Zip 檔案，然後將它上傳到 Azure ML Studio，但這需要數個步驟。  
...
R 資料框架支援強大的篩選功能。藉由在資料列或資料行使用邏輯篩選，可將資料集再細分成子集。在許多情況下，將會需要複雜的篩選條件。[附錄 B - 進階閱讀]中的參考內容(#appendixb) 包含大量的篩選資料框架範例。  
...
> [WACOM.NOTE] 從現在起，在本節中，我將只會示範要在 [執行 R 指令碼] 模組中新增的額外程式碼。我會在 `str()` 函式「之前」****新增每個新程式碼行。我會使用此函式在 Azure ML Studio 中確認我的結果。

我在 [執行 R 指令碼] 模組的 R 程式碼中新增下列程式碼行。
...
為了協助組織程式碼，我們將建立我們的第一個簡單函式 `num.month()`。然後，我們會套用此函式在資料框架中建立新資料行。新程式碼如下：
...
在本節中，我們將對資料框架之某些資料行中的值執行一些簡單的轉換。R 語言幾乎支援任何一種值轉換。[附錄 B - 進階閱讀]中的參考內容(#appendixb) 包含大量的範例。
...
針對我們的預測模型，我們使用乘法模型來進行這些資料的趨勢和季節性調整。對數轉換可讓我們使用線性模型，以簡化此程序。我們可以在套用乘數的相同函式中套用對數轉換。

在下列程式碼中，我定義了新函式 `log.transform()`，並將它套用至包含數值的資料列。R `Map()` 函式可用來將 `log.transform()` 函式套用至資料框架中的所選資料行。`Map()` 與 `apply()` 類似，但可允函式有多個引數清單。請注意，乘數清單會提供 `log.transform()` 函式的第二個引數。`na.omit()` 函式是用來進行一點清除，以確保我們在資料框架中沒有遺失或未定義的值。
...
1. 定義包含四個訊息的向量，用來傳達一些可能在此程式碼發生的錯誤和例外狀況的相關資訊。
...
此時會清除我們的資料，而我們已經準備好進行一些建立模型工作。看看我們 [執行 R 指令碼] 模組之 [結果資料集] 輸出的視覺化摘要，您會看到 'Month' 資料行是 'Categorical' 並具有 12 個唯一值，這又再次是我們所想要的。
...
讓我們開始進行我們的範例。請將「新的執行 R 指令碼模組」****拖放到您的實驗中。將現有 [執行 R 指令碼] 模組的 [結果資料集 1] 輸出連接埠連接到新 [執行 R 指令碼] 模組的 [結果集 1] 輸入連接埠。
...
這些資料的類型和格式皆如預期。請注意，'Month' 資料行的類型是因素，並且具有預期的層級數目。
...
我們已經在[值轉換]中討論過防禦型程式設計的範例(#valuetransformations)。兩個計算區塊皆包裝在 `tryCatch()` 中。就某些錯誤而言，傳回原始輸入向量相當合理，而在其他情況下，我會傳回零向量。  
...
請注意，資料列名稱會在資料框架的資料行中。這麼做可在從 [執行 R 指令碼] 輸出資料列名稱時，保留這些資料列名稱。

執行此程式碼時，會在我將 [結果資料集] 連接埠的輸出「視覺化」****時，產生如圖 29 所示的輸出。資料列名稱如預期般在第一個資料行中。
...
首先，請將「新的執行 R 指令碼模組」****新增到您的實驗中。將現有 [執行 R 指令碼] 模組的 [結果資料集]**** 輸出連接到新模組的 [資料集1]**** 輸入。結果應該會看起來像圖 20。
...
在資料規模較小的情況下，我會在 RStudio 中為趨勢建置模型，再將產生的模型剪下並貼到 Azure ML 中。RStudio 針對這種互動式分析提供了互動式環境。
...
讓我們繪製顯示趨勢曲線的加州乳製品產量資料時間序列圖，來做為例行性測試。我已經在 Azure ML [執行 R 指令碼] 模型 (非 RStudio) 中新增下列程式碼，以建立模型並繪圖。
...
讓我們繪製另一張加州乳製品產量資料的時間序列圖，看看季節性模型運作得如何。我已經在 Azure ML [執行 R 指令碼] 中新增下列程式碼，以建立模型並繪圖：
...
在 Azure ML 中執行此程式碼會產生如圖 24 所示的圖。
...
有一些衡量標準可用來衡量時間序列模型的表現。在我們的案例中，我們將使用均方根誤差 (或稱 RMS 誤差)。下列函式會計算兩個數列間的 RMS 誤差：  
...
1.	建立專案
	...
	您可以使用 RStudio 以專案方式組織和管理您的 R 程式碼。您可以在下列網址找到使用專案的文件：https://support.rstudio.com/hc/zh-tw/articles/200526207-Using-Projects。

	建議您依照這些指示，為本文件中的 R 程式碼範例建立專案。  

2.	編輯和執行 R 程式碼

	RStudio 提供一個可編輯和執行 R 程式碼的整合式環境。您可以在下列網址找到相關文件：https://support.rstudio.com/hc/zh-tw/articles/200484448-Editing-and-Executing-Code。

3.	偵錯

	RStudio 包含強大的偵錯功能。下列網址提供這些功能的相關文件：https://support.rstudio.com/hc/zh-tw/articles/200713843-Debugging-with-RStudio。

	下列網址提供中斷點疑難排解功能的相關文件：https://support.rstudio.com/hc/zh-tw/articles/200534337-Breakpoint-Troubleshooting。

##<a id="appendixb"></a>附錄 B - 進階閱讀

如果您不熟悉 R，CRAN 有提供兩本簡介。
...
- Robert Ka
- bacoff 所著的《R in Action》是另一本實用的簡介書籍。'Quick R' 網站是相當實用的資源，網址為 http://www.statmethods.net/。
...
- 如果您想要深入探索 R 中的進階主題，可以看看 Hadley Wickham 即將推出的新書《Advanced R》。這本書的線上版本提供免費下載，網址為 http://adv-r.had.co.nz/。

您可以在「CRAN 工作檢視：時間序列分析」找到 R 時間序列封裝目錄：: http://cran.r-project.org/web/views/TimeSeries.html. For information on specific time series object packages, you should refer to the documentation for that package.
