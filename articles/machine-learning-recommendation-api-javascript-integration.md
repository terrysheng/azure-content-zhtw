<properties 
	pageTitle="Azure Machine Learning Recommendations - JavaScript 整合" 
...
* includeMetadata (布林值，選擇性) – 如果設為 ‘true’，代表結果中必須填入中繼資料欄位。
* 處理函式 – 將會處理所傳回建議的函式。資料是以陣列形式傳回：
	* Item – 項目的唯一識別碼 
	* name – 項目名稱 (如果存在於目錄中) 
	* rating – 建議評等 
