<properties 
	pageTitle="機器學習應用程式：異常偵測服務 | Azure " 
...
2. 動態範圍的值增加：舉例來說，監視服務所擲回的例外狀況時，若動態範圍的值有任何增加，可能表示服務的健康情況不穩定，以及

3. 尖峰和下降：例如，監視服務的登入失敗數目或電子商務網站中的取出數目時，若有尖峰或下降可能表示異常的行為。
...
此服務提供可以不同方式使用透過 HTTPS 的 REST 型 API，包括 Web 或行動應用程式、R、Python、Excel 等等。我們的 [Azure Web 應用程式](http://anomalydetection-aml.azurewebsites.net/)有助於對您的資料執行異常偵測 Web 服務，並將結果以具體方式呈現。 
