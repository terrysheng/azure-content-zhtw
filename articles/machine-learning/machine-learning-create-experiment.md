...
1. 我們可以將我們的資料分割成個別的訓練集和測試集，用來進行訓練和測試。選取 [[分割][split]] 模組並拖曳到實驗畫布，然後將其連接到最後一個 [[Project Columns][project-columns]] 模組的輸出。將 [Fraction of rows in the first output dataset] 設為 0.75。如此，我們將使用百分之 75 的資料來訓練模型，並保留百分之 25 做為測試之用。
...
如果您對模型感到滿意，您可以將其部署為 Web 服務，用以透過新資料預測汽車價格。如需詳細資訊，請參閱[部署 Azure 機器學習 Web 服務][publish]。
