...
在本教學課程中，我們將引導您使用 SQL 資料倉儲 (SQL DW)，針對可公開使用的資料集 ([NYC 計程車車程](http://www.andresmh.com/nyctaxitrips/)資料集) 建置和部署機器學習服務模型。所建構的二元分類模型可預測是否已針對某趟車程支付小費，並且也會討論多元分類模型和迴歸模型。

此程序遵循 [Cortana Analytics 程序 (CAP)](https://azure.microsoft.com/documentation/learning-paths/cortana-analytics-process/) 工作流程。我們會示範如何設定資料科學環境、如何將資料載入到 SQL DW，以及如何瀏覽 SQL DW 和 IPython Notebook 中的資料和工程功能。然後，我們會示範如何在 Azure Machine Learning 中建置和部署模型。
...
聯結 trip\_data 和 trip\_fare 的唯一索引鍵是由下列三個欄位所組成：medallion、hack\_license 和 pickup\_datetime。
...
建立自己的 **Azure Blob 儲存體帳戶**。本逐步解說中所使用的 NYC 計程車資料會在 Azure 的公用 Blob 儲存體容器中以 .csv 格式分享使用。在本逐步解說中，我們會先將該資料複製到您自己的 Azure Blob 儲存體，然後再將資料上傳至 Azure SQL DW。**公用 Blob 儲存體**位於***美國中南部***。

- 當您在佈建自己的 Azure Blob 儲存體時，請為 Azure Blob 儲存體選擇最接近美國中南部的地理位置。該資料會從公用 Blob 儲存體容器複製到您自己的儲存體帳戶中的容器。您的 Azure Blob 儲存體越接近美國中南部，就能越快完成這項工作 (步驟 4)。 
...
確定您可以使用 Visual Studio 連接到 Azure SQL DW。如需指示，請參閱[使用 Visual Studio 連接到 Azure SQL 資料倉儲](sql-data-warehouse-get-started-connect.md)。
...
- 如果尚未安裝 AzCopy，請下載並安裝 AzCopy。
- 使用 AzCopy 將公用 Blob 中的資料複製到私人 Blob 儲存體帳戶。
- 將私人 Blob 儲存體帳戶中的資料載入到 Azure SQL DW。
	- 為 Blob 儲存體帳戶上的 NYC 計程車資料集建立外部資料表。
	- 在 SQL DW 上建立資料表 (trip 和 fare 資料表) 以儲存 NYC 計程車資料集。
	- 將外部資料表中的 NYC 計程車資料集匯入到 SQL DW 資料表。
...
- 如果資料位於內部部署電腦的現實應用程式中，您仍可以使用 AzCopy 將內部部署資料上傳至私人 Azure Blob 儲存體。您只需要在 AzCopy 命令中，將 PowerShell 指令碼檔案中的 **Source** 位置變更為本機目錄。	
- 如果資料已位於私人 Azure Blob 儲存體的現實應用程式中，您可以略過 PowerShell 指令碼中的 AzCopy 步驟，並直接將資料上傳到 Azure SQL DW。 
...
- 使用 Visual Studio 以 SQL DW 登入名稱和密碼連接到 Azure SQL DW。
...
此範例可找出在指定期間內完成超過 100 趟車程的圓形徽章 (計程車號碼)。資料分割資料表存取的條件是以 **pickup\_datetime** 資料分割配置為依據，因為可為查詢帶來好處。查詢完整資料集也會使用資料分割資料表及 (或) 索引掃描。
