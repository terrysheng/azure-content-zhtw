...
本逐步解說的下一步驟，是在 Machine Learning Studio 中建立使用我們上傳之資料集的新實驗。
...
我們可以使用 [中繼資料編輯器][][metadata-editor] 模型來新增資料行標題。使用 [[中繼資料編輯器][metadata-editor]] 模組來變更與資料集相關聯的中繼資料。透過這種方式，可提供資料行標題更易記的名稱。

若要使用 [[中繼資料編輯器][metadata-editor]]，您必須先指定要修改的資料行 (在此案例中為全部)，接著指定要在這些資料行執行的動作 (在此案例中為變更資料行標題)。

1.	在模組調色盤的 [搜尋] 方塊中，輸入 "metadata"。您會在模組清單中看見 [[中繼資料編輯器][metadata-editor]]。
2.	按一下將 [[中繼資料編輯器][metadata-editor]] 模組拖曳到畫布上，放在我們稍早新增的資料集下。
3.	將資料集連接到 [[中繼資料編輯器][metadata-editor]]：按一下資料集的輸出連接埠 (資料集底部的小圓圈)，拖曳到 [[中繼資料編輯器][metadata-editor]] 的輸入連接埠 (模組頂端的小圓圈)，然後放開滑鼠按鈕。即使您在畫布上移動資料集或模組，這兩者仍會保持連線。
    ...
    > [AZURE.TIP] 您可以按兩下模組並輸入文字，為模組新增註解。這有助於您快速檢視模組在您實驗中的執行情況。在此案例中，按兩下 [[中繼資料編輯器][metadata-editor]] 模組，然後輸入註解「新增資料行標題」。按一下畫布上其他的任何地方來關閉文字方塊。按一下模組的向下箭號以顯示註解。

4.	選取 [[中繼資料編輯器][metadata-editor]]，接著在畫布右邊的 [屬性] 窗格中，按一下 [啟動資料行選取器]。
5.	在 [**選取資料行**] 對話方塊中，將 [**開始於**] 欄位設定為 [所有資料行]。
6.	[開始於] 下方的資料列可讓您加入或排除[中繼資料編輯器][metadata-editor]要修改的特定資料行。由於我們要修改「所有」資料行，因此請按一下資料列右側的減號 ("-") 來刪除此資料列。對話方塊應該會看起來如下：![已選取所有資料行的資料行選取器][4]
7.	按一下 [**確定**] 核取記號。
...
> [AZURE.TIP] 如果您想要確認資料行標題，請執行實驗 (按一下實驗畫布下方的 [執行])。當執行結束 (綠色的核取記號會出現在 [[中繼資料編輯器][metadata-editor]] 上) 時，按一下 [[中繼資料編輯器][metadata-editor]] 模組的輸出連接埠，然後選取 [視覺化]。您可以用相同方式檢視任何模組的輸出，以檢視資料在實驗中的執行進度。
...
1.	找出 [[資料分割][split]] 模組，將它拖曳到畫布上，然後連接到最後一個 [[中繼資料編輯器][metadata-editor]] 模組。
