...
[中繼資料編輯器][metadata-editor]模組可用來變更與資料集相關聯的中繼資料。透過這種方式，可提供資料行標題更易記的名稱。為了達成目的，我們將引導[中繼資料編輯器][metadata-editor]對所有資料行執行，然後提供要加入資料行的名稱清單。
...
6.	[**開始於**] 下方的資料列可讓您包含或排除[中繼資料編輯器][metadata-editor]要修改的特定資料行。由於我們想要修改所有資料行，因此請按一下資料列右側的減號 ("-") 刪除這個資料列。對話方塊應該會看起來如下：
    ![已選取所有資料行的資料行選取器][4]
7.	按一下 [**確定**] 核取記號。 
...
> [AZURE.TIP]如果您要確認資料行標題，請執行實驗 (按一下實驗畫布下方的 [**執行**])，按一下[中繼資料編輯器][metadata-editor]模組的輸出連接埠，然後選取 [**檢視結果**]。您可以用相同方式檢視任何模組的輸出，以檢視資料在實驗中的執行進度。
...
1.	尋找[執行 R 指令碼][execute-r-script]模組並拖曳到實驗畫布，並且將[分割][split]模組的左輸出連接埠連接至[執行 R 指令碼][execute-r-script]模組的第一個輸入連接埠 ("Dataset1")。
...
3.	將此 [執行 R 指令碼][execute-r-script] 模組的第一個輸入連接埠連接到右輸出連接埠的 [分割][split] 模組。  
