...
我們可以使用 [中繼資料編輯器][][metadata-editor] 模型來新增資料行標題。
使用 [[中繼資料編輯器][metadata-editor]] 模組來變更與資料集相關聯的中繼資料。透過這種方式，可提供資料行標題更易記的名稱。
...
6.	[開始於] 下方的資料列可讓您加入或排除[中繼資料編輯器][metadata-editor]要修改的特定資料行。由於我們要修改「所有」資料行，因此請按一下資料列右側的減號 ("-") 來刪除此資料列。對話方塊應該會看起來如下：
	![已選取所有資料行的資料行選取器][4]
7.	按一下 [**確定**] 核取記號。
