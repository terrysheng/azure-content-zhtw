...
3.	將 [評分模型][score-model] 模組的右側輸入埠連接到右邊 [Execute R Script][execute-r-script] 模組的輸出。 
...
7.	將[評分模型][score-model]模組的右側輸入連接埠連接到[標準化資料][normalize-data]模組的輸出。  
...
當所有模組都出現核取記號時，表示實驗執行完成。若要檢查結果，請按一下 [評估模型][evaluate-model] 模組的輸出埠，然後選取 [**檢視結果**]。
...
按一下 [**已評分的資料集**] 或 [**要比較的已評分資料集**]，以醒目提示相關聯的曲線，並在下方顯示相關聯的度量。在曲線的圖例中，"Scored dataset" 對應至 [評估模型][evaluate-model] 模組的左側輸入埠 - 在我們的案例中，這是推進式決策樹模型。「要比較的已計分資料集」會對應至右側輸入連接埠 (在本例中為 SVM 模型)。按一下其中一個標籤時，該模型的曲線會反白顯示，並在下方顯示對應的度量。
