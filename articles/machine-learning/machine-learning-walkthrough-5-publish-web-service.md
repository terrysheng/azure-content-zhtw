...
我們還必須對實驗執行額外的步驟。當 Machine Learning Studio 移除[分割][split]模組時，會移除其中一個[執行 R 指令碼][execute-r-script]模組，但留下另一個[執行 R 指令碼][execute-r-script]模組。由於該模組只用於訓練和測試 (提供樣本資料的加權函式)，因此我們現在可以將其移除，並將[中繼資料編輯器][metadata-editor]連線到[評分模型][score-model]。
