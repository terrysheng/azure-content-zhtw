...
為了讓其他人可以使用此預測模型，我們將它發佈到 Azure 上當作 Web 服務。

截至目前為止，我們一直在試驗如何定型模型。但發佈後的服務不會再進行訓練 - 而是會給使用者的輸入評分。因此，我們將做些準備，再將這項實驗發佈為運作正常的 Web 服務，以供使用者存取。使用者將可以傳送一組信用貸款申請資料給服務，而服務會傳回信用風險的預測。
...
但首先，我們需要更精簡這項實驗。我們的實驗目前有兩個不同的模型，但現在需要選取一個要發佈的模型。
...
幸好上述三個步驟只要按一下實驗畫布底部的 [**建立評分實驗**]，即可完成。

當您按一下 [**建立評分實驗**] 時，會發生幾件事：
...
我們還必須對實驗執行額外的步驟。當 Machine Learning Studio 移除[分割][split]模組時，會移除其中一個[執行 R 指令碼][execute-r-script]模組，但留下另一個[執行 R 指令碼][execute-r-script]模組。由於該模組只用於定型和測試 (提供樣本資料的加權函數)，因此我們現在可以將其移除，並將[中繼資料編輯器][metadata-editor]連接到[評分模型][score-model]。
...
最後一次執行實驗 (按一下 [執行])。如果要驗證模型仍然有效，請按一下 [[評分模型][score-model]] 模組，並選取 [**檢視結果**]。您會看到原始資料出現，也會看到信用風險值 ("Scored Labels") 和評分機率值 ("Scored Probabilities").

##發佈 Web 服務

若要從實驗中發佈 Web 服務，請按一下畫布下方的 [**PUBLISH WEB SERVICE**]。Machine Learning Studio 會將實驗當做 Web 服務發佈，並帶您前往服務儀表板。

> [AZURE.TIP]您可以在發佈 Web 服務之後進行更新。例如，如果想要變更模型，只要編輯定型實驗、調整模型參數，然後按一下 [**發佈 Web 服務**]。重新發佈實驗時，將會取代 Web 服務 (現在是使用已更新的模型)。
