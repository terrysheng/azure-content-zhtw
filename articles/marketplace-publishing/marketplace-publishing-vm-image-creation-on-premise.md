<properties
   ...
   description="了解及執行建立內部部署 VM 映像，並且部署至 Azure Marketplace 供他人購買的步驟。"
...
我們強烈建議您使用 RDP 直接在雲端中開發 Azure VHD。不過如果需要，可以使用內部部署基礎結構下載 VHD 並進行開發。

如果是內部部署開發，您必須下載已建立好之 VM 的作業系統 VHD。這些步驟會在前述的步驟 3.3 進行。
...
1.	移至 [瀏覽] > [VM]，選取已部署的 VM。
2.	在 [設定] 之下，選取 [磁碟] 磚，開啟 [磁碟] 刀鋒視窗。

  ![繪圖](media/marketplace-publishing-vm-image-creation-on-premise/img01.png)

3.	選取 [作業系統磁碟]。這會開啟另一個刀鋒視窗顯示磁碟屬性，包括 VHD 的位置。
4.	複製此 Blob URL。

  ![繪圖](media/marketplace-publishing-vm-image-creation-on-premise/img02.png)

5.	現在，刪除已部署的 VM 而不要刪除備份磁碟。您也可以選擇停止 VM，而不是刪除。不要在 VM 執行時下載作業系統 VHD。

  ![繪圖](media/marketplace-publishing-vm-image-creation-on-premise/img03.png)

### 下載 VHD
知道 Blob URL，您就可以使用 [Azure 管理入口網站](http://manage.windowsazure.com/)或 PowerShell 下載 VHD。
> [AZURE.NOTE]本指南建立時，新的 Microsoft Azure 入口網站還沒有下載 VHD 的功能。

**透過目前的 [Azure 管理入口網站](http://manage.windowsazure.com/)下載作業系統 VHD**
...
除了使用 Azure 管理入口網站，還可以使用 [Save-AzureVhd](http://msdn.microsoft.com/library/dn495297.aspx) Cmdlet 下載作業系統 VHD：
...
> [AZURE.NOTE]*Save-AzureVhd* 也有 NumberOfThreads 選項，可以用來增加平行處理來善用下載可用的頻寬。

## 上傳 VHD 到 Azure 儲存體帳戶
如果您準備好內部部署的 VHD，必須將它們上傳到 Azure 中的儲存體帳戶。這個步驟會在建立內部部署 VHD 之後，但是在取得 VM 映像認證之前進行。

### 建立儲存體帳戶和容器
如先前所述，我們建議將 VHD 上傳到美國境內區域的儲存體帳戶中。單一 SKU 的所有 VHD 都應該放在單一儲存體帳戶內的單一容器中。
...
7.	在 [容器] 刀鋒視窗中，選取 [新增]，輸入容器名稱和容器權限。為容器權限選取 [私人]。

> [AZURE.TIP]建議您為每個您計畫要發佈的 SKU 都建立一個容器。
...
然後您可以使用 [NewAzureStorageContainer](http://msdn.microsoft.com/library/dn495291.aspx) Cmdlet 在該儲存體帳戶內建立容器：

        New-AzureStorageContainer -Name “containername” -Permission “Off”

> [AZURE.NOTE]這些命令假設目前的儲存體帳戶內容已在 PowerShell 中設定。請參閱[設定 Azure PowerShell](marketplace-publishing-powershell-setup.md) 了解 PowerShell 安裝程式的詳細資訊。
