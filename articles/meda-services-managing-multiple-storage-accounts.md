...
媒體服務在建置串流內容的 URL 時使用 **IAssetFile.Name** 屬性的值 (例如，http://{WAMSAccount}.origin.mediaservices.windows.net/{GUID}/{IAssetFile.Name}/streamingParameters.) 基於這個理由，不允許 percent-encoding。名稱屬性的值不能有下列任何 [percent-encoding 保留字元](http://en.wikipedia.org/wiki/Percent-encoding#Percent-encoding_reserved_characters)：!*'();:@&=+$,/?%#[]"。而且，副檔名只能有一個 ‘.’。
