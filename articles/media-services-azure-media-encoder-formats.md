...
轉碼器有兩個元件：一個用來壓縮數位媒體檔案以進行傳輸，另一個用來解壓縮數位媒體檔案以進行播放。有壓縮和解壓縮音訊的音訊轉碼器，和壓縮和解壓縮視訊的視訊轉碼器。轉碼器可以使用無失真壓縮或失真壓縮。無失真轉碼器在壓縮時會保留所有資訊。解壓縮檔案時，結果會是與輸入媒體完全相同的檔案，使得無失真轉碼器非常適合用來封存和儲存。失真轉碼器在編碼時會遺失部分資訊，並產生較小的檔案 (與原始檔案相比)，但會犧牲視訊品質，非常適合用來透過網際網路進行串流。Azure Media Encoder 用來編碼的兩個主要轉碼器分別為 H.264 和 VC-1。我們合作夥伴的編碼器生態系統可能提供其他的轉碼器。
...
<tr><td>DVD 傳輸資料流 (TS) 檔案</td><td>.ts</td></tr>
