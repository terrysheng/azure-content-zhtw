...
本教學課程涵蓋 API。對於播放程式開發，Microsoft 強裂建議使用 [Player Framework for Windows 8](http://playerframework.codeplex.com/)。播放程式架構可讓應用程式建置起來更容易，並可提供許多其他功能。 
...
如需開發 Windows 市集應用程式的詳細資訊，請參閱[開發 Windows 8 適用的好用應用程式](http://msdn.microsoft.com/windows/apps/br229512.aspx)。 
本課程包含下列程序：
...
6. 在 MainPage 類別中，找到 UI 按鈕並按一下事件地區，再新增以下函式定義：
