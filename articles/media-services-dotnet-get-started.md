...
# 使用 Media Services SDK for .NET 傳遞點播視訊 

[AZURE.INCLUDE [media-services-selector-get-started](../includes/media-services-selector-get-started.md)]


>[AZURE.NOTE]
> 若要完成此教學課程，您需要 Azure 帳戶。如果您沒有帳戶，只需要幾分鐘的時間就可以建立免費試用帳戶。如需詳細資訊，請參閱 <a href="http://azure.microsoft.com/pricing/free-trial/?WT.mc_id=A8A8397B5" target="_blank">Azure 免費試用版</a>。

本教學課程會逐步完成使用 Azure Media Services (AMS) SDK for .NET 實作點播視訊 (VoD) 內容傳遞應用程式。 

本教學課程介紹基本媒體服務工作流程，以及媒體服務開發所需的最常用程式設計物件和工作。完成本教學課程時，您將能夠串流處理或漸進式下載您已上傳、編碼和下載的範例媒體檔。  

若要實作 VoD 內容傳遞應用程式，您可以使用不同的技術 (例如，.NET、REST 或 Java) 或工具 (Azure 管理入口網站或 Azure 媒體服務總管) 或兩者的組合。 

本教學課程會使用 Azure 管理入口網站和 Media Services SDK for .NET 來達成下列工作：     


1.  [使用入口網站建立媒體服務帳戶](#create_ams)。
2.  [使用入口網站設定串流單元](#configure_streaming_units)。
3.  [建立和設定 Visual Studio 專案](#configure_VS)
4.  [使用 .NET 上傳、編碼和傳遞內容](#use_dotnet)
	5.  [連線到媒體服務帳戶](#connect)。
	1.  [建立新資產並上傳視訊檔案](#upload)。
	1.  [將來源檔案編碼為一組調適性位元速率 MP4 檔案](#encode)。
	1.  [設定編碼資產的傳遞原則](#configure_delivery_method)。
	2.  [(選擇性) 設定動態內容保護](#configure_content_protection)。 
	1.  [發佈資產並取得串流和漸進式下載 URL](#publish_get_urls)。 
1.  [播放內容](#play)。 
...
6. 按一下表單底部的 [**快速建立**]。
	...
	在頁面底部，會出現 [**管理金鑰**] 按鈕。按一下此按鈕時，會顯示含媒體服務帳戶名稱以及主要和次要金鑰的對話方塊。您需要有帳戶名稱和主要金鑰資訊，才能以程式設計方式存取媒體服務帳戶。 
...
使用 Azure 媒體服務時，最常見的一個案例是將調適性位元速率串流傳遞給用戶端。運用調適性位元速率串流，用戶端可以切換到較高或較低的位元速率資料流，因為是根據目前網路頻寬、CPU 使用率和其他因素來顯示視訊。媒體服務支援下列調適性位元速率串流技術：HTTP 即時資料流 (HLS)、Smooth Streaming、MPEG DASH 和 HDS (僅適用於 Adobe PrimeTime/Access 被授權人)。 

媒體服務提供動態封裝，可讓您以媒體服務所支援的串流格式 (MPEG DASH、HLS、Smooth Streaming、HDS) 傳遞調適性位元速率 MP4 或 Smooth Streaming 編碼內容，而不需要重新封裝為這些串流格式。 

若要利用動態封裝，您需要執行下列動作：

- 將夾層 (來源) 檔案編碼或轉碼為一組調適性位元速率 MP4 檔案或調適性位元速率 Smooth Streaming 檔案 (本教學課程稍後會示範編碼步驟)、  
- 取得想要從中傳遞內容之串流端點的至少一個隨選串流單元。

運用動態封裝，您只需要以單一儲存體格式儲存和播放檔案，而且媒體服務會根據來自用戶端的要求來建置並提供適當的回應。 

請注意，除了能夠使用動態封裝功能之外，隨選串流保留單元提供您可以 200 Mbps 為增量單位加以購買的專用傳出功能。依預設，隨選串流是以共用執行個體模型來設定，可與其他所有使用者共用伺服器資源 (例如，運算、流出容量等)。若要改進隨選串流輸送量，建議購買隨選串流保留單元。
...
4. 按 [儲存] 以儲存您的變更。
	...
	>
	> 計算成本時會使用 24 小時內指定的最大單元數。如需定價詳細資料的詳細資訊，請參閱[媒體服務定價詳細資料](http://go.microsoft.com/fwlink/?LinkId=275107)。
...
## <a id="use_dotnet"></a>使用 .NET 上傳、編碼和傳遞內容 
...
### <a id="connect"></a>連線到媒體服務帳戶

搭配使用媒體服務與 .NET 時，您必須將 **CloudMediaContext** 類別用於大部分的媒體服務程式設計工作：連線到媒體服務帳戶；建立、更新、存取和刪除下列物件：資產、資產檔案、工作、存取原則、定位器等。 
 
將預設 Program 類別覆寫為下列程式碼。此程式碼示範如何讀取 App.config 檔案中的連線值以及如何建立 CloudMediaContext 物件來連線到媒體服務。如需連線到媒體服務的詳細資訊，請參閱[使用 Media Services SDK for .NET 連線到媒體服務](http://msdn.microsoft.com/library/azure/jj129571.aspx)。
...
### <a id="upload"></a>建立新資產並上傳視訊檔案

在媒體服務中，您可以將數位檔案上傳 (或內嵌) 到資產。**Asset** 實體可以包含視訊、音訊、影像、縮圖集合、文字播放軌和隱藏式輔助字幕檔案 (和這些檔案的中繼資料。)上傳檔案之後，您的內容會安全地儲存在雲端，以進一步進行處理和串流處理。資產中的檔案稱為**資產檔案**。
...
將資產內嵌到媒體服務之後，可以先將媒體編碼、轉碼多工處理、加上浮水印等，再傳遞給用戶端。這些活動會排程並針對多種背景角色執行個體執行，以確保高效能與可用性。這些活動稱為「工作 (Job)」，而每個工作 (Job) 都是由原子工作 (Task) 構成，這些原子工作負責執行資產檔案的實際工作。 

如稍早所提及，使用 Azure 媒體服務時，其中一個最常見的案例是將調適性位元速率串流傳遞給用戶端。媒體服務可以將一組調適性位元速率 MP4 檔案動態封裝為下列其中一種格式：HTTP 即時資料流 (HLS)、Smooth Streaming、MPEG DASH 和 HDS (僅適用於 Adobe PrimeTime/Access 被授權人)。 

若要利用動態封裝，您需要執行下列動作：

- 將夾層 (來源) 檔案編碼或轉碼為一組調適性位元速率 MP4 檔案或調適性位元速率 Smooth Streaming 檔案、  
- 取得想要從中傳遞內容之串流端點的至少一個隨選串流單元。 
...
- [傳遞儲存體加密內容](http://msdn.microsoft.com/library/azure/dn783451.aspx)

### <a id="configure_delivery_method"></a>設定編碼資產的傳遞原則

媒體服務內容傳遞工作流程中的其中一個步驟是設定資產傳遞原則。資產傳遞原則組態所含的一些項目：可用來傳遞資產的通訊協定 (例如，MPEG DASH、HLS、HDS、Smooth Streaming 或全部)、是否要動態加密資產，以及方法 (信封或一般加密)。 
...
進行此傳遞組態之後，就可以使用下列格式來要求 Smooth、HLS 或 MPEG DASH 資料流：
...
### <a id="publish_get_urls"></a>發佈資產並取得串流和漸進式下載 URL

若要串流處理或下載資產，您必須先建立定位器來「發佈」它。定位器可以存取資產中所含的檔案。媒體服務支援兩種類型的定位器：OnDemandOrigin 定位器用來串流處理媒體 (例如，MPEG DASH、HLS 或 Smooth Streaming)，而存取簽章 (SAS) 定位器用來下載媒體檔。

建立定位器之後，就可以建置用來串流處理或下載檔案的 URL。 
...
若要測試 HLS，請使用 iOS 或 Safari 裝置或者 [3ivx-hls-player](http://apps.microsoft.com/windows/zh-tw/app/3ivx-hls-player/f79ce7d0-2993-4658-bc4e-83dc182a0614)。 
