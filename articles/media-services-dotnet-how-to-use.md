...
您也可以使用 OData 型 REST API 對媒體服務進行程式設計。您可以使用 .NET 語言或其他程式設計語言，建置對媒體服務進行 REST API 呼叫的應用程式。如需有關使用 Media Services REST API 進行程式設計的完整文件系列，請參閱[使用 Azure Media Services REST API 建立應用程式](http://go.microsoft.com/fwlink/?linkid=252967)。 
...
下列提供可用之用戶端 SDK 和播放程式架構的清單。如需這些和其他計劃性 SDK 與播放程式架構以及其支援之功能的詳細資訊，請參閱媒體服務論壇中的[媒體服務用戶端開發][] (英文)。 
...
##後續步驟
您已概略了解媒體服務，現在請移至[設定媒體服務的電腦](../media-services-set-up-computer/) 主題。
  ...
  [設定媒體服務開發]: #setup-dev
  [作法：以程式設計方式連接到媒體服務]: #connect
  [作法：建立加密資產並上傳至儲存體]: #create-asset
  [作法：取得媒體處理器執行個體]: #get-mediaproc
  [作法：檢查工作進度]: #check-job-progress
  ...
  [媒體服務預覽：支援的功能]: http://social.msdn.microsoft.com/Forums/en-US/MediaServices/thread/eb946433-16f2-4eac-834d-4057335233e0
  [媒體服務即將發行的版本：規劃的功能支援]: http://social.msdn.microsoft.com/Forums/en-US/MediaServices/thread/431ef036-0939-4784-a939-0ecb31151ded
