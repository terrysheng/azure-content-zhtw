...
「非輪詢方法」的命名模式如下：Send*OperationName*Operation (如 SendCreateOperation)。Send*OperationName*Operation 方法會傳回 **IOperation** 物件；傳回的物件含有可用來追蹤作業的資訊。Send*OperationName*OperationAsync 方法會傳回 **Task<IOperation>**。

以下是目前支援非輪詢方法的類別：**Channel**、**StreamingEndpoint** 及 **Program**。

若要輪詢作業狀態，請針對 **OperationBaseCollection** 類別使用 **GetOperation** 方法。請使用下列間隔來檢查作業狀態：對於 **Channel** 和 **StreamingEndpoint** 作業，使用 30 秒；對於 **Program** 作業，使用 10 秒。
