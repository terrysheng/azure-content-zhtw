...
資產中的檔案稱為**資產檔案**。**AssetFile** 執行個體和實際媒體檔是兩個不同的物件。AssetFile 執行個體會包含媒體檔的中繼資料，而媒體檔包含實際的媒體內容。
...
## 使用 Media Services .NET SDK 上傳檔案  
...
- 	建立 **AccessPolicy** 執行個體，以定義存取資產所需的權限和期間。
 	
- 	建立 **Locator** 執行個體，以存取資產。
