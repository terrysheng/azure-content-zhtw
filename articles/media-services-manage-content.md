<properties urlDisplayName="How to manage media content" pageTitle="如何管理媒體內容 - Azure 媒體服務" metaKeywords="" description="了解如何在 Azure 媒體服務中管理您的媒體內容。" metaCanonical="" services="media-services" documentationCenter="" title="如何管理媒體服務中的內容" authors="juliako" solutions="" manager="dwrede" editor="" />
...
1.  在[管理入口網站][管理入口網站]中按一下 [媒體服務]，然後按一下媒體服務帳戶名稱。
2.  選取 [內容] 頁面。
...
4.  在 Azure Media Encoder 對話方塊中，從其中一個一般或進階編碼預設格式中進行選擇。
    ...
    **一般預設格式**
    ...
    -   **使用 PlayReady 內容保護進行編碼**。此預設格式會產生以 PlayReady 內容保護編碼的資產。若要讓此預設格式運作，您必須啟用 PlayReady 授權傳遞服務。若要這麼做，請移至 [內容保護] 索引標籤，然後在 [Branding Reporting] 資料表中新增一個資料列。媒體服務 PlayReady 授權服務會在您按 [儲存] 後的幾分鐘內啟用。
        ...
        依預設會使用媒體服務 PlayReady 授權服務。若要指定其他可讓用戶端從中取得授權以播放 PlayReady 加密內容的服務，請使用 REST 或媒體服務 .NET SDK API。如需詳細資訊，請參閱[使用靜態加密保護您的內容][使用靜態加密保護您的內容]，並在媒體編碼程式預設中設定 **licenseAcquisitionUrl** 屬性。或者，您可以使用動態加密，並依照[使用 PlayReady 動態加密和授權傳遞服務][使用 PlayReady 動態加密和授權傳遞服務]來設定 **PlayReadyLicenseAcquisitionUrl** 屬性。

        請注意，只有在您註冊 PlayReady 內容保護預覽功能後，才會顯示此選項。若要註冊預覽功能，您必須完成下列頁面上說明的程序：[Microsoft Azure 預覽功能][Microsoft Azure 預覽功能]。

    **進階預設格式**

    -   [Azure Media Encoder 的工作預設字串][Azure Media Encoder 的工作預設字串]主題說明 [進階預設格式] 清單中每個預設格式的意義。

    ![EncoderDialog][EncoderDialog]

    入口網站目前不支援 Media Encoder 所支援的所有編碼格式。也不支援媒體資產加密\\解密。您可以透過程式設計方式執行這些工作，如需詳細資訊，請參閱[使用 Media Services SDK for .NET 建立應用程式][使用 Media Services SDK for .NET 建立應用程式]和 [Azure Media Encoder 的工作預設字串][Azure Media Encoder 的工作預設字串]。
...
<!-- Images -->
  ...
  [使用靜態加密保護您的內容]: 
  [使用 PlayReady 動態加密和授權傳遞服務]: http://go.microsoft.com/fwlink/?LinkId=507720
  [Microsoft Azure 預覽功能]: http://azure.microsoft.com/zh-tw/services/preview/
