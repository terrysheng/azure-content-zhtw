...
3. 在 [**監視**] 中，設定 Blob 的監視層級和資料保留原則：

-  若要設定監視等級，請選取下列其中一項：

      **最小** - 收集度量，例如入口流量/出口流量、可用性、延遲和成功百分比，這些是針對 Blob、資料表和佇列服務彙總。

      **詳細資訊** - 除了最小度量之外，在 Azure 儲存體服務 API 中也針對每個儲存體作業收集相同的度量集合。詳細資訊度量可供進一步分析在應用程式運作期間發生的問題。 

      **關閉** - 關閉監視。現有的監視資料會儲存到保留期間結束。

- 若要設定資料保留原則，在 [**保留期限 (以天為單位)**] 中，輸入要保留資料的天數，範圍從 1 到 365 天。如果不想設定保留原則，則請輸入零。如果沒有保留原則，您可以決定是否刪除監視資料。我們建議根據要保留帳戶的儲存體分析資料多久來設定保留原則，以便系統可以不花費成本地刪除舊的無用分析資料。
