...
-   使用 [windowsazure.mediaservices Nuget][windowsazure.mediaservices Nuget] (英文) 封裝來安裝 Azure SDK for .NET。下一節顯示如何使用 **Nuget** 來安裝 Azure SDK。
...
2.  確實將目標架構設為 .NET Framework 4。若要執行此動作，請以滑鼠右鍵按一下 Visual Studio 專案，然後選取 [屬性]。在 [應用程式] 索引標籤中，為目標架構設定 .NET Framework 4。
3.  加入 **System.Configuration** 組件的參考。若要加入 **System.Configuration** 的參考，請在 [方案總管] 中，以滑鼠右鍵按一下 [參考] 節點，然後選取 [加入參考...]。在 [管理參考] 對話方塊中，選取 [System.Configuration]，然後按一下 [確定]。
4.  使用 [windowsazure.mediaservices Nuget][windowsazure.mediaservices Nuget] 套件，將參考新增至 **Azure SDK for .NET**。(所有其他相依組件也會安裝)。
