...
2. 指定通道名稱，描述欄位是選擇性的。在 [通道設定] 下，對 [即時編碼] 選項選取 [標準]，並將輸入通訊協定設定為 **RTP (MPEG-TS)**。您可以將所有其他設定保留現狀。
...
2. 一旦建立新事件，請向下捲動至輸出群組並新增 **UDP/TS** 輸出群組。

3. 選取 [新增串流] 然後按一下 [加入輸出] 來建立新的輸出。
	
	![Elemental](./media/media-services-elemental-live-encoder/media-services-elemental13.png)
	
	>[AZURE.NOTE]建議將 Elemental 事件的時間代碼設定為「系統時鐘」，以協助編碼器在串流失敗時重新連接。

4. 現在輸出已建立，請按一下 [加入串流]。現在可以設定輸出設定。
...
6. 取得通道的輸入 URL。
	...
	瀏覽回到 AMSE 工具，並檢查通道的完成狀態。一旦狀態從**啟動中**變更為**執行中**，您可以取得輸入 URL。
	  
	通道執行時，以滑鼠右鍵按一下通道名稱，向下瀏覽以暫留在 [複製輸入 URL 到剪貼簿]，然後選取 [主要輸入 URL]。
	
	![Elemental](./media/media-services-elemental-live-encoder/media-services-elemental6.png)
	
1. 在 Elemental 的 [主要目的地]欄位貼上這項資訊。所有其他設定可以保留預設值。
...
>[AZURE.IMPORTANT]在您在 Elemental Live Web 介面上按一下 [開始] 之前，您**必須**確保通道已就緒。此外，請確保不要讓通道處於就緒狀態而無任何事件超過 15 分鐘。
...
2. 命名程式，並視需要調整**封存時間長度** (預設為 4 小時)。您也可以指定儲存體位置，或保留為預設值。
3. 勾選 [現在啟動程式] 方塊。
4. 按一下 [建立程式]。  
...
請參閱[疑難排解](media-services-troubleshooting-live-streaming.md)主題以取得指引。
