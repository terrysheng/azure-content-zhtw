...
1. 將輸入檔 (稱為夾層檔) 上傳到資產。例如，H.264、 MP4 或 WMV。
1. 編碼資產，使其變成調適性位元速率 MP4。
1. 發佈資產。 
2. 使用 [動態封裝](http://msdn.microsoft.com/library/azure/jj889436.aspx)，將以下任何格式的內容傳遞給用戶端：MPEG DASH、 Apple HLS 或 Smooth Streaming。 

本主題簡介主要[概念](../media-services-deliver-content.md#concepts) 並提供一些連結，讓您瞭解如何執行內容傳遞相關[作業](../media-services-deliver-content.md#tasks)。
...
調適性位元速率技術可讓視訊播放器應用程式判斷網路狀況，並由數種位元速率中進行選擇。當網路通訊降級時，用戶端會選擇較低的位元速率，讓播放器能夠持續以較低的視訊品質來播放視訊。當網路狀況改善時，用戶端會切換至較高的位元速率，以提高視訊品質。Azure 媒體服務支援下列調適性位元速率技術：HTTP 即時資料流 (HLS)、Smooth Streaming、MPEG DASH 和 HDS。
...
若要漸進式下載內容，請使用 OnDemandOrigin 類型的定位器。下列範例顯示的 URL 採用的是 OnDemandOrigin 類型的 locator.r：
