...
此外，您可以藉由調整縮放單位 (也稱為串流單位)，控制串流端點服務處理成長頻寬需求的產能。建議您為生產環境中的應用程式配置一個或多個縮放單位。利用縮放單位，我們可以購買專用的流出容量 (以 200 Mbps 為單位逐次增加) 以及其他包含以下幾點的功能：[動態封裝](https://msdn.microsoft.com/library/azure/jj889436.aspx)、CDN 整合以及進階組態。
...
1. 若要指定串流單元數目，請選取 [調整] 索引標籤，然後移動 [**保留容量**] 滑動軸。

	![調整頁面](./media/media-services-manage-origins/media-services-origin-scale.png)
...
## <a id="configure_streaming_endpoints"></a>設定資料流端點
...
- 串流端點必須至少是一個串流 (調整) 單位。如果以後想要將調整單位設定為 0，請先停用 CDN 整合。 

- 串流端點必須處於停止狀態。啟用 CDN 之後，您可以啟動串流端點。 

您可能需要等候 90 分鐘的時間，才能啟用 Azure CDN 整合。   

一旦啟用後，就會停用下列組態：**自訂主機名稱**和**存取控制**。

並非所有的資料中心都支援 Azure CDN 整合。如果您使用 Azure 管理入口網站，只要您的資料中心有 Azure CDN，就會看到 [**啟用 CDN**] 選項。 

![資料流端點啟用 CDN][streaming-endpoint-enable-cdn]

如果您使用.NET SDK 或 REST API，您需要檢查資料中心是否有可用的 Azure CDN 整合。 

若要檢查，請執行下列作業：

1. 試著將 CdnEnabled 設定為 true。
1. 到「HTTP 錯誤碼 412 (PreconditionFailed)」以及附帶的「串流端點 CdnEnabled 屬性無法設定為 true，因為目前的區域沒有 CDN 功能」訊息中檢查傳回的結果。 

	如果顯示這一則錯誤訊息，表示資料中心不支援它。您應該嘗試另一個資料中心。
