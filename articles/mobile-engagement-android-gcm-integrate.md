...
> [AZURE.IMPORTANT] 按照本指南進行前，您必須先遵循「如何在 Android 上整合 Engagement」文件中所述的整合程序。
>
...
以下資訊供你參考，Engagement 只使用 [Send-to-Sync] 訊息搭配  `engagement.tickle` 摺疊金鑰。
...
請只進行該程序中在您的帳戶上啟用 GCM 的部分。當您到達＜**取得 API 金鑰**＞章節時，請停止閱讀並返回此頁面，不要再遵循任何進一步的 Google 程序。

此程序說明**專案編號**可做為 **GCM 傳送者識別碼**，在此程序稍後需要用到此識別碼。

> [AZURE.IMPORTANT] **專案編號**不應與**專案識別碼**混淆。專案識別碼現在與專案編號不同 (在新專案上是一個名稱)。您需要在 Engagement SDK 中整合的是**專案編號**，此編號顯示於 [[Google 開發人員主控台]] 的 [**概觀**] 功能表中。
