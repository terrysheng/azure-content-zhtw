<properties 
   pageTitle="Azure Mobile Engagement 作法指南" 
   description="Azure Mobile Engagement 作法指南" 
...
# Azure Mobile Engagement - 作法指南

## 簡介

下列 Azure Mobile Engagement 使用者介面的作法指南假設您熟悉 Azure Mobile Engagement 的基本 [概念][Link 6] 且只會在您將 Azure Mobile Engagement [SDK] 整合[Link 5] 到應用程式之後才能運作。這些逐步解說著重於標準使用者如何使用 [使用者介面] [Link 1] 而不是開發人員使用者如何使用 HTTP REST 型 [API][Link 4]。如果您對於任何一項 [逐步解說] 有困難，[Link 3] 請參閱 Azure Mobile Engagement [疑難排解指南][Link 2]。

## <a name="#First">進行第一個推送通知活動</a>
...
> 警告：如果未定義使用者的應用程式資訊標記，使用者將不會收到推送。
...
## <a name="#Differentiate">區分您的推送通知 (在應用程式內外)</a>
...
> 注意：建立時沒有結束日期的活動會將推送本機存放在裝置上，並在下次開啟應用程式時顯示它 - 即使已手動結束活動。

## <a name="#TextView">使用文字檢視增強推送通知</a>
...
- 僅限通知：它是簡單的標準通知。這表示，如果使用者按一下它，不會出現任何其他檢視，而只會發生與它關聯的動作。
- 文字公告：它是吸引使用者看一下文字檢視的通知。
- 網頁公告：它是吸引使用者看一下網頁檢視的通知。
選取「網頁公告」內容。

> 注意：當您推送網頁檢視時，它一律是先有通知。
