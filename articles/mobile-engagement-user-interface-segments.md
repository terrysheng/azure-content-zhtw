<properties 
   ...
   description="Azure Mobile Engagement [使用者分佈] 區段的使用者介面概觀" 
...
- [API 文件集 - 使用者分佈 API][Link 4]、[疑難排解指南 - 分析][Link 2]

您可以根據 [分析] 區段中過去最多 60 天的特定期間及最多 10 個準則來建立使用者分佈。 
例如，您可以根據過去 10 天內在您的應用程式中檢視某些頁面或搜尋特定內容的人員，建立使用者分佈。您也可以在 [分析] 區段取得此資訊。因此，您可以使用這項資訊來建立使用者分佈，然後再針對這一類使用者設定推送通知，讓他們再回到應用程式。 
 
> 注意：使用者分佈一旦計算之後，就無法進行編輯；它只能複製或終結 (刪除)。使用者分佈可以在同一個應用程式 (具有相同的 AppID) 內複製，也可以複製到其他應用程式 (具有不同的 AppID)。 
