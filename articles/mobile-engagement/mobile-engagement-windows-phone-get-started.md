...
3. 在顯示的快顯視窗中，輸入以下資訊：
	...
	- **應用程式資源名稱**：這是此應用程式可透過 API 與 URL 存取時的名稱。您只可以使用傳統 URL 自字元。自動產生的名稱應該能提供您強健的基礎。建議您附加平台名稱，以避免發生名稱衝突，因為名稱必須是唯一的。
...
5. 按一下 [連接資訊]**** 來顯示要置入您行動應用程式中 SDK 整合的連接設定。
...
本節說明如何使用 Mobile Engagement 的即時監視功能來檢查您的應用程式是否連接至 Mobile Engagement 後端。
