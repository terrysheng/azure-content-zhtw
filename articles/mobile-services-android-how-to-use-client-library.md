...
## <a name="lookup"></a><span class="short-header">查閱資料</span>作法：查閱特定項目
...
## <a name="binding"></a><span class="short-header">繫結資料</span>作法：將資料繫結至使用者介面
...
2.  在活動類別的 **onCreate** 方法中，於建立 `MobileServiceClient` 物件的程式碼後面加入下列程式碼行：我們假設 `MobileServiceClient` 物件的參考是 *mClient*。
...
要將用戶端資料表名稱對應至不同的行動服務資料表名稱並不難，只要使用
[getTable()][第二個多載] 函數的其中一項覆寫即可，如下列程式碼所示。
