<properties pageTitle="伺服器端授權 (Android) | 行動開發人員中心" description="了解如何在 Azure 行動服務的 .NET 後端為使用者授權。" services="mobile-services" documentationCenter="android" authors="ggailey777" manager="dwrede" editor=""/>
...
2. 從 [**執行**] 功能表中，按一下 [**執行**] 以在 Android 模擬器中啟動專案。

   	請注意，雖然已有先前教學課程中在 TodoItem 資料表中輸入的項目，依然不會傳回任何項目。發生此情況是因為之前的項目插入時沒有 userId 資料行，而現在有 Null 值。
...
5. 回到[管理入口網站][Azure 管理入口網站]的 **todoitem** 資料表中，按一下 [**瀏覽**] 並驗證每個新增的項目現在是否有相關聯的 userId 值。
...
[行動服務 Android 作法概念性參考]: /zh-tw/documentation/articles/mobile-services-android-how-to-use-client-library/
