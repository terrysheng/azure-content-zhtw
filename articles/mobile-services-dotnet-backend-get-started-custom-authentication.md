...
2.  在您的行動服務後端專案中，新增名為 CustomRegistrationController 的新自訂控制器，並貼到下列程式碼中：
...
接著，您將建立供使用者登入的端點。系統會根據資料庫檢查您所接收到的使用者名稱和密碼；方法是套用使用者的加料 (salt)、將密碼雜湊化，然後確定傳入的值符合資料庫的值。如果符合，您即可建立 `ClaimsIdentity`，並將其傳至 `CustomLoginProvider`。接著，用戶端應用程式將會接收使用者識別碼和驗證權杖，以供進一步存取您的行動服務之用。
...
2.  新增名為 `CustomLoginController` 的新自訂控制器，並貼到下列程式碼中：
