<properties urlDisplayName="Troubleshoot the Mobile Services .NET Backend" pageTitle="行動服務 .NET 後端的疑難排解 - Azure 行動服務" metaKeywords="" description="了解如何使用 .NET 後端診斷和修正行動服務的問題" metaCanonical="" services="" documentationCenter="Mobile" title="Troubleshoot the Mobile Services .NET Backend" authors="mahender" solutions="" manager="dwrede" editor="mollybos" />
...
1. 首先，在 **Visual Studio 2013 Update 2** 或更新版本中開啟行動服務伺服器專案。如果您尚無此專案，您可以選取 [**檔案**]、[**新增**]、[**專案**]，然後依序選取 [**雲端**] 節點和 [**Microsoft Azure 行動服務**] 範本，以建立專案。
2. 點按 **F5**，以建置並執行專案。在開始頁面上，選取 [**立即試用**]。 

    >[WACOM.NOTE] 
    > 如果是在本機主控服務，則在按一下連結後將會導向至下一頁。但如果是在雲端中主控，則系統會提示您提供一組認證。這是為了確保讓未經授權的使用者無法存取您 API 和裝載的相關資訊。若要檢視頁面，您必須使用**空白使用者名稱**，並以您的**應用程式金鑰**作為密碼，來進行登入。您可以在 **Azure 管理入口網站**中導覽至行動服務的 [**儀表板**] 索引標籤，並選取 [**管理金鑰**]，以取得應用程式金鑰。
    >
...
4. 在程式碼片段中要進行偵錯之處，設定一個中斷點。例如，在 Visual Studio 中的行動服務專案範本隨附之 **TodoItemController** 的 **GetAllTodoItems()** 方法中，設定一個中斷點。
...
您的行動服務處理來自客戶的要求時，會產生各種實用的診斷資訊，並擷取任何發生的例外狀況。除此之外，您也可以使用每個 [**TableController**](http://msdn.microsoft.com/library/microsoft.windowsazure.mobile.service.tables.tablecontroller.aspx) 的 [[**服務**](http://msdn.microsoft.com/library/microsoft.windowsazure.mobile.service.tables.tablecontroller.services.aspx)] 屬性上提供的 [[**記錄**](http://msdn.microsoft.com/library/microsoft.windowsazure.mobile.service.apiservices.log.aspx)] 屬性，以其他記錄來檢測您的控制器程式碼。

在本機上偵錯時，記錄會顯示在 Visual Studio 的 [**輸出**] 視窗。
...
在 **Azure 管理入口網站**上，也可以從行動服務的 [**記錄**] 索引標籤上取得相同的記錄。
...
當您將行動服務發行至 Azure 時，此服務會由行動服務主控環境載入，以確保能夠順利地升級和修補至主控控制器程式碼的 HTTP 管線。其中包括 [.NET 後端 NuGet 封裝](http://www.nuget.org/packages?q=%22mobile+services+.net+backend%22)所參考的所有組件：相關團隊會持續更新服務，以使用最新版的組件。 

有時候版本會因為參考了必要組件的*不同主要版本*而發生衝突 (允許不同的*次要*版本)。這常發生在 NuGet 提示您升級至行動服務 .NET 後端所使用的其中一個封裝的最新版本時。 

>[WACOM.NOTE] 行動服務目前只有與 ASP.NET 5.1 相容；目前不支援 ASP.NET 5.2。在部署之後，將您的 ASP.NET NuGet 封裝升級至 5.2.* 可能會產生錯誤。
...
使用附有 SQL Database 的行動服務 .NET 後端時，會以 Entity Framework (EF) 作為資料存取技術，讓您能夠查詢資料庫及保存物件。EF 代表開發人員處理的重要面向之一，是資料庫資料行 (也稱為*結構描述*) 如何隨著程式碼中指定的模型類別而變更。此程序稱為 [Code First 移轉](http://msdn.microsoft.com/zh-tw/data/jj591621)。
