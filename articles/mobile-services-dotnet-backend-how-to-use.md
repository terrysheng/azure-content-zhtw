...
此程式設計模型的根本是 [**TableController<T>**](http://msdn.microsoft.com/library/dn643359.aspx) 類別，此類別只是針對 CRUD 資料存取模式而自訂的一般 Web API [**ApiController**](http://msdn.microsoft.com/library/system.web.http.apicontroller.aspx)。**TableController** 可以使用各種資料存放區，包括 SQL (透過 [Entity Framework](http://msdn.microsoft.com/data/ef.aspx))、[Azure 資料表儲存體](http://azure.microsoft.com/documentation/services/storage/)、[MongoDB](http://www.mongodb.org)，或您自己的自訂存放區。
