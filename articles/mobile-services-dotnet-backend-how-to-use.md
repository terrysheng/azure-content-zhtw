...
行動服務 .NET 後端可讓您使用 [ASP.NET Web API][] 和您慣用的 .NET 語言，撰寫豐富的後端商業邏輯。行動服務可將小型程式設計模型介面公開為 [NuGet 封裝][]，以便您從 Windows、Android、iOS 的跨平台行動服務用戶端 SDK 順暢地存取您的服務。這些 API 也可確保驗證和推播通知支援也可盡可能簡單地新增。但大部分的程式設計模型皆以 Web API 為基礎，開發人員十分熟悉，因此應能得心應手。
...
此程式設計模型的根本是 [**TableController<t>**][] 類別，此類別只是針對 CRUD 資料存取模式而自訂的一般 Web API [**ApiController**][]。**TableController** 可使用多種資料存放區，包括 SQL (透過 [Entity Framework][])、[Azure 資料表儲存體][]、[MongoDB][] 或您本身的自訂存放區。
