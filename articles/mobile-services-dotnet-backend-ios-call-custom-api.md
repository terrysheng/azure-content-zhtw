<properties pageTitle="從 iOS 應用程式呼叫自訂 API | 行動服務" metaKeywords="" description="了解如何定義自訂 API 然後從使用 Windows Azure 行動服務的 iOS 應用程式呼叫它。" metaCanonical="" services="mobile-services" documentationCenter="Mobile" title="Call a custom API from the client" authors="krisragh"  solutions="" writer="krisragh" manager="dwrede" editor=""  />
