...
## <a name="test-locally-hosted"></a>對本機代管的服務測試 Windows Phone 應用程式
...
1.  在 Visual Studio 中開啟 App.xaml.cs 檔案，將您最近貼在檔案中的 `MobileService` 定義註解化。新增新的定義，以根據您在工作站上設定的 IP 位址和連接埠建立連線。接著，請儲存檔案。程式碼應會如下所示：
