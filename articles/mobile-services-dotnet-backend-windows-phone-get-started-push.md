...
本教學課程會以行動服務快速入門為基礎。在開始本教學課程之前，您必須先完成[將行動服務新增至現有的應用程式]，將專案連接到行動服務。

>[AZURE.NOTE]此教學課程的目標是 Windows Phone 8.1 "Silverlight" 應用程式。如果您要改為建立 Windows Phone 8.1 市集應用程式，請參閱 [Windows Store appWindows 市集應用程式](mobile-services-dotnet-backend-windows-store-dotnet-get-started-push) 本教學課程的版本。如需 Windows Phone Silverlight 應用程式及其與 Windows Phone 市集應用程式比較的相關資訊，請參閱 [Windows Phone Silverlight 8.1 應用程式]。 
...
3. 按一下 [**推播**] 索引標籤，核取 [**啟用未驗證的推播通知**]，然後按一下 [**儲存**]。
...
此教學課程示範如何讓 Windows Phone 應用程式能夠使用行動服務和通知中樞來傳送推播通知的基本步驟。接下來，請考慮閱讀下個教學課程[將推播通知傳送給驗證的使用者]，此課程說明如何使用標籤將行動服務中的推播通知指定傳送給驗證的使用者。
...
[將行動服務新增至現有的應用程式]: /zh-tw/documentation/articles/mobile-services-dotnet-backend-windows-phone-get-started-data
