...
在 Visual Studio 2013 Update 2 中，ASP.NET Web 應用程式專案此時會包含 Windows Azure 行動服務的範本。請選取此範本，然後按一下 [確定]。
...
以滑鼠右鍵按一下 Controllers 資料夾、選取 [新增]，然後選取 [新增 Scaffolded 項目]。
...
2.  在 [資料內容類別] 下，選取 [LeaderboardContext]。
3.  將控制器命名為 “PlayerContoller”。
4.  按一下 [新增]。
...
在 `PlayerRankController` 類別中，將 `LeaderboardContext` 變數從建構函式移至類別變數：
...
當服務的非同步作業擱置時，`IsPending` 屬性會是 true。`ErrorMessage` 屬性會包含任何來自服務的錯誤訊息。
...
<!-- Anchors. --> <!-- Images. --> <!-- URLs. -->
  ...
  [Overview]: #overview
  [關於範例應用程式]: #about-the-sample-app
  [新增資料模型]: #add-data-models
  [新增 Web API 控制器]: #add-web-api-controllers
  [使用 DTO 傳回相關實體]: #use-a-dto-to-return-related-entities
  [定義用來提交分數的自訂 API]: #define-a-custom-api-to-submit-scores
  [建立 Windows 市集應用程式]: #create-the-windows-store-app
  [新增模型類別]: #add-model-classes
  [建立檢視模型]: #create-a-view-model
  [新增 MobileServiceClient 執行個體]: #add-a-mobileserviceclient-instance
  [建立主頁面]: #create-the-main-page
