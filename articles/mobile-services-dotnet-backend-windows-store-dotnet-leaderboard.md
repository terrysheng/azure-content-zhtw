...
請注意，這兩個類別皆衍生自 **EntityData** 類別。從 **EntityData** 衍生，可方便應用程式取用資料，而將跨平台用戶端程式庫用於 Azure 行動服務。**EntityData** 也可方便應用程式[處理資料庫寫入衝突](http://azure.microsoft.commobile-services-windows-store-dotnet-handle-database-conflicts.md)。
