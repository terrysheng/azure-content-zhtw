...
本主題說明如何從您的 Windows 市集應用程式，在 Azure 行動服務中驗證使用者。在本教學課程中，您將使用行動服務支援的身分識別提供者，將驗證加入快速入門專案。由行動服務成功驗證並授權之後，就會顯示使用者識別碼值。
...
1.  在 Visual Studio 2013 中，開啟您完成[開始使用行動服務][1]教學課程時所建立的專案。
...
<!-- URLs. -->
  ...
  [1]: /zh-tw/documentation/articles/mobile-services-dotnet-backend-windows-store-dotnet-get-started/
  ...
  [行動服務使用者的伺服器端授權]: /zh-tw/documentation/articles/mobile-services-dotnet-backend-windows-store-javascript-authorize-users-in-scripts
