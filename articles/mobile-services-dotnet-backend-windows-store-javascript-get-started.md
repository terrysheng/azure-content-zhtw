...
1.  在共用程式碼專案中，開啟 default.js 檔案，找出建立 [WindowsAzure.MobileServiceClient][WindowsAzure.MobileServiceClient] 執行個體的程式碼，使用 *localhost* 將建立此用戶端的程式碼註解化，然後使用遠端行動服務 URL，將建立用戶端的程式碼取消註解，如下所示：
