...
> [WACOM.NOTE] 如果您想要提供用戶端導向的驗證，以使用 Azure Active Directory 進行單一登入 (SSO)，請參閱[使用 Active Directory Authentication Library 單一登入來驗證您的應用程式][使用 Active Directory Authentication Library 單一登入來驗證您的應用程式]教學課程。
