...
> [WACOM.NOTE]Visual Studio 2013 可讓您輕鬆地向行動服務註冊您的 Windows 市集應用程式封裝。如需詳細資訊，請參閱 Windows 開發人員中心的[快速入門：新增行動服務的推播通知][快速入門：新增行動服務的推播通知]。

用戶端管理的驗證可讓您透過 Live Connect，在 Windows 裝置上執行單一登入。如果您使用 Live Connect API，您即無須完成此主題中的步驟。如需詳細資訊，請參閱[使用 Live Connect 單一登入驗證 Windows 市集應用程式][使用 Live Connect 單一登入驗證 Windows 市集應用程式]。

> [WACOM.NOTE].NET 後端行動服務尚未支援使用 Live Connect 的用戶端管理的驗證。
