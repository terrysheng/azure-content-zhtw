...
<div class="dev-center-tutorial-subselector"><a href="/zh-tw/documentation/articles/mobile-services-dotnet-backend-how-to-use/" title=".NET backend">.NET 後端</a> | <a href="/zh-tw/documentation/articles/mobile-services-how-to-use-server-scripts/"  title="JavaScript backend" class="current">JavaScript 後端</a></div>
...
+ [資料表作業]
	...
	+ [做法：覆寫預設的錯誤處理]
	+ [做法：新增自訂參數]
	+ [做法：處理資料表使用者][做法：處理使用者]
+ [自訂 API][自訂 API 錨點]
	+ [做法：定義自訂 API]
	+ [做法：實作 HTTP 方法]
	+ [做法：以 XML 傳送及接收資料]
	+ [做法：在自訂 API 中使用 user 和 header]
	+ [做法：在自訂 API 中定義多個路由]
+ [工作排程器]
	+ [做法：定義排程工作指令碼]
+ [原始檔控制、共用程式碼及協助程式函數]
...
+ [排程的作業][作業排程器]。
+ [在自訂 API 中定義的 HTTP 方法][自訂 API 錨點]。

伺服器指令碼中主要函數的特徵依指令碼的使用情境而異。您也可以將常用指令碼定義為 nodes.js 模組，由指令碼共用。如需詳細資訊，請參閱[原始檔控制和共用程式碼][原始檔控制、共用程式碼及協助程式函數]。
...
資料表作業指令碼是伺服器指令碼，註冊至資料表的作業 -- 插入、讀取、更新或刪除 (*del*)。指令碼的名稱必須符合註冊之作業的種類。一個指定資料表作業只能註冊一個指令碼。每次 REST 要求叫用該指定作業時，就會執行其指令碼&mdash;例如，收到要在資料表中插入項目的 POST 要求時。行動服務不會保留指令碼執行之間的狀態。因為指令碼每次執行都會建立新的全域內容，指令碼中定義的所有狀態變數都會重新初始化。如果您想儲存兩個不同要求之間的狀態，請在行動服務中建立資料表，讀取狀態並將狀態寫入資料表。如需詳細資訊，請參閱 [如何：從指令碼存取資料表]。
...
每個伺服器指令碼都有主要函數，可能也有選擇性的協助程式函數。雖然伺服器指令碼可能是為了特定資料表而建立，它也可以參考相同資料庫中的其他資料表。您也可以將常用函數定義為模組，讓所有指令碼共用。如需詳細資訊，請參閱[原始檔控制和共用程式碼][原始檔控制、共用程式碼及協助程式函數]。

###<a name="register-table-scripts"></a>做法：註冊資料表指令碼

您可以用下列方式之一定義註冊到資料表作業的伺服器指令碼：

+ 在 [Azure 管理入口網站][管理入口網站]。可在指定資料表的 [**指令碼**] 索引標籤中存取資料表作業的指令碼。以下為註冊到  `TodoItem` 資料表之插入指令碼的預設程式碼。您可用自己的自訂業務邏輯覆寫此程式碼。

	![1][1]
	
	若要知道怎麼做，請參閱[使用伺服器指令碼驗證及修改行動服務中的資料]。  

+ 使用原始檔控制。當您啟用原始檔控制時，只要在 Git 儲存機制的 .\service\table 子資料夾中建立名為 <em>`<table>`</em>.<em>`<operation>`</em>.js 的檔案，其中 <em>`<table>`</em> 是資料表的名稱，而 <em>`<operation>`</em> 是所註冊的資料表作業。如需詳細資訊，請參閱[原始檔控制和共用程式碼][原始檔控制、共用程式碼及協助程式函數]。
...
###<a name="override-response"></a>做法：覆寫預設回應
...
###<a name="override-success"></a>做法：覆寫執行成功
...
###<a name="override-error"></a>做法：覆寫預設的錯誤處理
...
###<a name="access-headers"></a>做法：存取自訂參數
...
[request 物件]的 **parameters** 屬性會將這些自訂的查詢參數當做 JSON 值存取。行動服務會提供 **request** 物件給任何已註冊至資料表作業的函數。下列用於插入作業的伺服器指令碼，會先檢查  `duplicateText` 參數的值，再執行插入作業：
...
###<a name="work-with-users"></a>做法：使用 user

在 Azure 行動服務中，您可以使用身分識別提供者來驗證使用者。如需詳細資訊，請參閱[開始使用驗證]。當已驗證使用者叫用資料表作業時，行動服務會使用 [user 物件]將使用者資訊提供給註冊的指令碼函數。**userId** 屬性可用於儲存及擷取使用者特定資訊。以下範例會根據已驗證使用者的 userId 設定項目的 owner 屬性：
...
自訂 API 是行動服務中的端點，被一或多個標準 HTTP 方法存取：GET、POST、PUT、PATCH、DELETE。可以為自訂 API 支援的每個 HTTP 方法另外定義一個 export 函數，全部放在同一個指令碼檔案中。當使用指定方法的自訂 API 收到要求，會叫用註冊的指令碼。如需相關資訊，請參閱[自訂 API]。

當行動服務執行階段呼叫自訂 API 函數時，會提供[request][request 物件]和 [response][response 物件] 物件。這些物件公開 [express.js library] 的功能，而您的指令碼可加以利用。以下名為 **hello** 的自訂 API 是一個很簡單的範例，會傳回 _Hello, world!_ 回應 POST 要求：
...
###<a name="define-custom-api"></a>做法：定義自訂 API

您可以用下列方式之一，定義註冊到自訂 API 端點中 HTTP 方法的伺服器指令碼：

+ 在 [Azure 管理入口網站][管理入口網站]。在 [**API**] 索引標籤中建立及修改自訂 API 指令碼。伺服器指令碼位於指定自訂 API 的 [**指令碼**] 索引標籤下。以下是 POST 要求對  `CompleteAll` 自訂 API 端點叫用的指令碼。 

	![2][2]
	
	在 [權限] 索引標籤中指派對自訂 API 方法的存取權限。若要了解此自訂 API 是如何建立的，請參閱[從用戶端呼叫自訂 API]。  

+ 使用原始檔控制。啟用原始檔控制後，只要在 git 儲存機制的 .\service\api 子資料夾中建立名為 <em>`<custom_api>`</em>.js 的檔案，其中 <em>`<custom_api>`</em> 是所註冊之自訂 API 的名稱。此指令碼檔案包含 _exported_ 函數，用於自訂 API 公開的每個 HTTP 方法。權限是在伴隨的 .json 檔案中定義。如需詳細資訊，請參閱[原始檔控制和共用程式碼][原始檔控制、共用程式碼及協助程式函數]。

+ 從命令提示字元使用 Azure 命令列工具。如需詳細資訊，請參閱[使用命令列工具]。

###<a name="handle-methods"></a>做法：實作 HTTP 方法
...
###<a name="api-return-xml"></a>做法：以 XML 傳送及接收資料
...
###<a name="get-api-user"></a>做法：在自訂 API 中使用 user 和 header

在 Azure 行動服務中，您可以使用身分識別提供者來驗證使用者。如需詳細資訊，請參閱[開始使用驗證]。當已驗證使用者要求自訂 API 時，行動服務會使用 [user 物件]將有關使用者的資訊提供給自訂 API 程式碼。要從 [request 物件]的 user 屬性存取 [user 物件]。**userId** 屬性可用於儲存及擷取使用者特定資訊。 
...
###<a name="api-routes"></a>做法：在自訂 API 中定義多個路由
...
###<a name="scheduler-scripts"></a>做法：定義排程工作指令碼
...
+ 在 [Azure 管理入口網站][管理入口網站]中，位於排程器的 [**指令碼**] 索引標籤中：
...
###<a name="modules-helper-functions"></a>做法：運用模組
...
###<a name="shared-code-source-control"></a>做法：使用原始檔控制共用程式碼

您可以使用原始檔控制搭配 Node.js 封裝管理員 (npm) 來控制您的行動服務可使用哪些模組。作法有二：

+ 針對發行至 npm 以及由 npm 安裝的模組，使用 package.json 檔案宣告哪些封裝要由您的行動服務安裝。這麼做時，您的服務一律有權存取所需封裝的最新版本。package.json 檔案留存在  `.\service` 目錄中。如需詳細資訊，請參閱 [Azure 行動服務對 package.json 的支援]。
...
###<a name="helper-functions"></a>做法：使用協助程式函數
...
將指令碼檔案上傳至共用目錄的方式包括使用[原始檔控制][作法：使用原始檔控制共用程式碼]，或使用[命令列工具][使用命令列工具]。

###<a name="app-settings"></a>做法：使用應用程式設定
...
###<a name="access-tables"></a>做法：從指令碼存取資料表
...
###<a name="bulk-inserts"></a>做法：執行大量插入
...
您可以使用以下指令碼，設定平行插入記錄的批次大小。建議您設定較小的記錄數目。當非同步插入批次完成時，**insertItems** 函數會遞迴呼叫它自己。結尾的 for 迴圈會一次插入一筆記錄，並且在成功時呼叫 **insertComplete**，在發生錯誤時呼叫 **errorHandler**。**insertComplete**控制是否會在下一個批次遞迴呼叫 **insertItems**，或工作是否完成並且應該結束指令碼。
...
在這篇[部落格文章](http://blogs.msdn.com/b/jpsanders/archive/2013/03/20/server-script-to-insert-table-items-in-windows-azure-mobile-services.aspx)中可以找到完整的程式碼範例以及相關的討論。如果您使用此程式碼，可以根據您的特定情況調整，並徹底測試。

###<a name="JSON-types"></a>做法：對應 JSON 類型至資料庫類型
...
當您編寫使用 [insert]、[update]、[read] 或 [delete] 函數的伺服器指令碼時，可以存取您的資料的 JavaScript 表示法。行動服務使用 Node.js 的還原序列化函數 ([JSON.parse](http://es5.github.io/#x15.12)) 線上將 JSON 轉換為 JavaScript 物件。不過，行動服務會進行轉換，以從 ISO 8601 字串擷取 **Date** 物件。
...
- **query**︰執行由 TSQL 字串指定的查詢；結果傳回給在 **options** 物件上的 **success** 回呼。若有  *params* 參數，則 query 可以包含參數。
...
####<a name="static-query"></a>做法：執行靜態查詢
...
####<a name="dynamic-query"></a>做法：執行動態參數化查詢
...
####<a name="raw"></a>做法：執行查詢並傳回 *raw* 的結果
...
####<a name="connection"></a>做法：存取資料庫連線

您可以使用 **open** 方法取得對資料庫連線的存取權。這麼做的可能原因之一是您需要使用資料庫交易。

**open** 執行成功會將資料庫連線當做參數傳遞給 **success** 函數。您可以在 **connection** 物件上叫用以下任何函數： *close*、 *queryRaw*、 *query*、 *beginTransaction*、 *commit* 和  *rollback*。
...
###<a name="write-to-logs"></a>做法：將輸出寫入行動服務記錄檔
...
[簡介]: #intro
[資料表作業]: #table-scripts
[做法：註冊資料表作業]: #register-table-scripts
[做法：定義資料表指令碼]: #execute-operation
[做法：覆寫預設回應]: #override-response
[做法：修改作業]: #modify-operation
[做法：覆寫成功和錯誤]: #override-success-error
[做法：覆寫執行成功]: #override-success
[做法：覆寫預設的錯誤處理]: #override-error
[做法：從指令碼存取資料表]: #access-tables
[如何：從指令碼存取資料表]: #access-tables
[作法：新增自訂參數]: #access-headers
[做法：新增自訂參數]: #access-headers
[做法：處理使用者]: #work-with-users
[做法：定義排程工作指令碼]: #scheduler-scripts
[做法：修改資料表的存取權]: #authorize-tables
[使用 Transact-SQL 存取資料表]: #TSQL
[做法：執行靜態查詢]: #static-query
[做法：執行動態查詢]: #dynamic-query
[做法：執行查詢並傳回 *raw* 的結果]: #raw
[做法：存取資料庫連線]: #connection
[做法：聯結關聯式資料表]: #joins
[做法：執行大量插入]: #bulk-inserts
[做法：對應 JSON 類型至資料庫類型]: #JSON-types
[做法：載入 Node.js 模組]: #modules-helper-functions
[做法：將輸出寫入行動服務記錄檔]: #write-to-logs
[原始檔控制、共用程式碼及協助程式函數]: #shared-code
[使用命令列工具]: #command-prompt
[使用資料表]: #working-with-tables
[自訂 API 錨點]: #custom-api
[做法：定義自訂 API]: #define-custom-api
[作法：使用原始檔控制共用程式碼]: #shared-code-source-control
[做法：使用原始檔控制共用程式碼]: #shared-code-source-control
[如何：使用原始檔控制共用程式碼]: #shared-code-source-control
[做法：使用協助程式函數]: #helper-functions
[偵錯與疑難排解]: #debugging
[做法：實作 HTTP 方法]: #handle-methods
[做法：在自訂 API 中使用 user 和 header]: #get-api-user
[做法：存取自訂 API 要求標頭]: #get-api-headers
[作業排程器]: #scheduler-scripts
[工作排程器]: #scheduler-scripts
[做法：在自訂 API 中定義多個路由]: #api-routes
[做法：以 XML 傳送及接收資料]: #api-return-xml
[做法：使用應用程式設定]: #app-settings
...
[Windows Phone 推播]: /zh-tw/develop/mobile/tutorials/get-started-with-push-wp8/
