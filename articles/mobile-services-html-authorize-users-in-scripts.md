<properties pageTitle="伺服器端授權 (HTML) | 行動開發人員中心" metaKeywords="" description="了解如何在 Azure 行動服務的 JavaScript 後端授權給使用者。" metaCanonical="" services="" documentationCenter="Mobile" title="Service-side authorization of Mobile Services users" authors="glenga" solutions="" manager="dwrede" editor="" />
...
4. 使用下列函式取代現有的指令碼，然後按一下 [儲存]****。
	...
	<p>首次執行插入指令碼時，必須啟用動態結構描述。啟用動態結構描述後，行動服務會在第一次執行時自動將 <strong>userId</strong> 資料行新增至 <strong>TodoItem</strong> 資料表。依預設會啟用新行動服務的動態結構描述，您應先加以停用，再發佈應用程式。</p>
    </div>
...
3. 在應用程式的 [Enter new task]**** 中輸入文字，然後按一下 [新增]****。
