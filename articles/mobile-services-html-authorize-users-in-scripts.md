<properties pageTitle="伺服器端授權 (HTML) | 行動開發人員中心" description="了解如何在 Azure 行動服務的 JavaScript 後端為使用者授權。" services="mobile-services" documentationCenter="" authors="ggailey777" manager="dwrede" editor=""/>
...
4. 使用下列函式取代現有的指令碼，然後按一下 [**儲存**]。
    ...
    > [AUZRE.IMPORTANT] 首次執行插入指令碼時，必須啟用動態結構描述。在已啟用動態結構描述的情況下，行動服務會在首次執行時將 **userId** 資料欄自動新增至 **TodoItem** 資料表。依預設會啟用新行動服務的動態結構描述，您應先加以停用，再發佈應用程式。
...
1. 在網頁瀏覽器中導覽至應用程式的 index.html 頁面，然後以您選擇的身分識別提供者登入。 
   	...
   	請注意，雖然已有先前教學課程中在 TodoItem 資料表中輸入的項目，依然不會傳回任何項目。發生的原因是因為插入之前的項目時並沒有 userId 資料行，但現在這些項目有 Null 值。
