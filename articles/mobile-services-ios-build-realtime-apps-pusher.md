...
1.  在專案總管的專案根目錄中，按一下 [Build Phases]，然後按一下 [Add Build Phase] 及 [Add Copy Files]。

2.  從專案總管將 **libPusher-combined.a** 檔案拖曳至新的組建階段。

3.  將 [目的地] 變更為 [Frameworks]，並按一下 [Copy only when installing]。

    ![][1]

4.  在 [Link Binary With Libraries] 區域中，新增下列程式庫：
...
5.  最後，在 [組建設定] 中，找出目標組建設定 [Other Linker Flags]，並新增 **-all\_load** 旗標。
...
## <a name="install-scripts"></a>安裝服務指令碼
