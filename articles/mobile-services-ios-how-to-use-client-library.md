...
本指南說明如何使用適用於 Azure 行動服務的 iOS 用戶端執行一般案例。相關範例是以 objective-C 撰寫並需要[行動服務 SDK](https://go.microsoft.com/fwLink/p/?LinkID=266533)。本教學課程也需要 [iOS SDK](https://developer.apple.com/xcode)。涵蓋的案例包括查詢資料、插入、更新和刪除資料、驗證使用者和處理錯誤。如果您不熟悉行動服務，您應考慮首先完成[行動服務快速入門](/en-us/develop/mobile/tutorials/get-started-ios)。快速入門教學課程可協助您設定帳戶，並建立您的第一個行動服務。
...
如果這是您第一次建立 iOS 應用程式，請務必在應用程式的 **[Link Binary With Libraries]** 設定中新增 `WindowsAzureMobileServices.framework`。
...
-   *totalCount*：所有查詢頁面中的項目總計數，而不只是目前頁面中所傳回的這些項目。除非您在要求中明確要求總計數，否則此值會被設為 -1。如需詳細資訊，請參閱[以分頁方式傳回資料](#paging)。
...
這些參數會以 `myKey1=value1&myKey2=value2` 方式附加到查詢 URI。如需詳細資訊，請參閱 [如何：存取自訂參數](/en-us/develop/mobile/how-to-guides/work-with-server-scripts#access-headers)。

插入資料作法：將資料插入行動服務
--------------------------------

若要將新資料列插入資料表中，您可以建立新的 [NSDictionary 物件](http://go.microsoft.com/fwlink/p/?LinkId=301965)，並將該物件傳遞至 insert 函數。下列程式碼會將新的 todo 項目插入資料表中：
...
行動服務支援資料表識別碼的唯一自訂字串值。這可讓應用程式使用自訂值，例如行動服務資料表識別碼資料行的電子郵件地址或使用者名稱。例如，如果您想要根據電子郵件地址來識別每筆記錄，您可以使用下列 JSON 物件。
...
您也可以使用伺服器指令碼來設定識別碼值。以下指令碼範例將產生自訂的 GUID，並將它指派為新記錄的識別碼。如果您未傳入記錄的識別碼值，此值會與行動服務產生的識別碼值類似。
...
如需設定識別提供者的詳細資訊，請參閱[開始使用驗證](/en-us/develop/mobile/tutorials/get-started-with-users-ios)。
...
下列範例會使用 [Live Connect SDK](http://go.microsoft.com/fwlink/p/?LinkId=301960) 來啟用 iOS 應用程式的單一登入。
...
權杖屬於敏感資料且必須加密儲存，以防在裝置遺失或遭竊時洩漏。

使用快取權杖時，使用者在權杖到期之前無需重新登入。當使用者嘗試使用過期的權杖登入時，系統便會傳回 401 未經授權的回應。此時，使用者必須重新登入以取得新的權杖，並重新快取權杖。您可以使用篩選來避免每當應用程式呼叫行動服務，就得撰寫程式碼以處理過期權杖的情況。篩選可讓您攔截行動服務的呼叫和來自行動服務的回應。篩選中的程式碼會測試 401 的回應、觸發登入程序 (如果權杖已過期)，然後重試產生 401 的要求。如需詳細資料，請參閱[處理過期權杖](http://go.microsoft.com/fwlink/p/?LinkId=301955) (英文)。
...
發生錯誤時，您可以透過在程式碼中包含 MSError.h 檔案來取得更多資訊。此檔案會定義下列可用來存取 `[error userInfo]` 其他資料的常數：
...
如需執行驗證和處理任何資料的範例，請參閱[使用伺服器指令碼驗證及修改行動服務中的資料](/en-us/develop/mobile/tutorials/validate-modify-and-augment-data-ios)。在此主題中，伺服器端驗證是透過伺服器指令碼進行實作的。提交無效資料時，系統會傳回錯誤回應，且用戶端會處理此回應。
