...
如同其他隨行動服務提供的身分識別提供者，Azure Active Directory (AAD) 提供者也支援豐富的 [Graph API]，可用於透過程式設計來存取目錄。在本教學課程中，您會根據使用 [Graph API] 從目錄中擷取的其他使用者資訊來更新 ToDoList 應用程式，以個人化已驗證的使用者應用程式體驗。
...
在本節中，您將建立 GetUserInfo 自訂 API，以使用 [Graph REST API] 從 AAD 擷取使用者的其他相關資訊。

如果您未曾搭配使用自訂 API 與行動服務，您應考慮先參閱[自訂 API 教學課程]，再開始執行本節步驟。

1. 在 [Azure 管理入口網站]中，為您的行動服務建立新的 GetUserInfo 自訂 API，然後將 get 方法的權限設為 [**僅限通過驗證的使用者**]。
...
3. 為 `getAADToken` 函數新增下列定義。
    ...
    在給定 *tenant_domain*、整合的應用程式*用戶端識別碼*和應用程式*金鑰*的情況下，此函數會提供用來讀取目錄資訊的 Graph 存取權杖。

4. 新增下列會使用 Graph API 傳回使用者資訊的 `getUser` 函數。
    ...
    此函數是 Graph REST API 的[取得使用者]端點的精簡型包裝函式。它會使用 Graph 存取權杖和使用者的物件識別碼，以取得使用者資訊。
...
[Graph REST API]: http://msdn.microsoft.com/zh-tw/library/azure/hh974478.aspx
