...
>[AZURE.NOTE] 本教學課程的目的是要擴充驗證知識以加入授權實務作法。您應先使用 Azure Active Directory 驗證提供者完成[將驗證加入行動服務應用程式]教學課程。本教學課程接著將更新[將驗證加入行動服務應用程式]教學課程中使用的 TodoItem 應用程式。
...
* 使用 Azure Active Directory 驗證提供者完成[將驗證加入行動服務應用程式]教學課程。
...
在進行[將驗證加入行動服務應用程式]教學課程期間，您已在完成[如何向 Azure Active Directory 註冊]步驟時，為整合的應用程式建立註冊。在本節中，您將產生在使用該整合的應用程式用戶端識別碼讀取目錄資訊時所將使用的金鑰。 

如果您已瀏覽[存取 Azure Active Directory 圖形資訊]教學課程，代表您已經完成此步驟，並可略過本節。
...
2. 將下列指令碼新增至定義  `getAADToken` 函數的檔案頂端。在給定  *tenant_domain*、整合的應用程式 *client id*和應用程式 *金鑰*的情況下，此函數會提供用來讀取目錄資訊的圖形存取權杖。

    >[AZURE.NOTE] 您應快取權杖，而不要為每個存取檢查建立一個新權杖。接著，請在嘗試使用 401 Authentication_ExpiredToken 回應中的權杖結果 [圖形 API 錯誤的參考] 時重新整理快取。為求單純性，下方的程式碼中並不會說明此作業，但這將可以減輕對您 Active Directory 的額外網路流量。 
...
3. 將下列程式碼加入  *rbac.js*，以定義  `getGroupId` 函數。此函數會使用存取權杖，根據篩選器中使用的群組名稱取得群組識別碼。
...
4. 將下列程式碼加入  *rbac.js*，以定義會呼叫圖形 REST API 之 [IsMemberOf] 端點的  `isMemberOf` 函數。
...
7. 將下列匯出的  `checkGroupMembership` 函數加入  *rbac.js*。  
...
當您完成[將驗證加入行動服務應用程式]教學課程，您應該已將資料表作業設定為需要驗證，如下所示。
...
<!-- URLs. -->
[將驗證加入行動服務應用程式]: mobile-services-javascript-backend-windows-universal-dotnet-get-started-users.md
...
[註冊以使用 Azure Active Directory 登入]: mobile-services-how-to-register-active-directory-authentication.md
[圖形 API]: http://msdn.microsoft.com/library/azure/hh974478.aspx
[圖形 API 錯誤的參考]: http://msdn.microsoft.com/library/azure/hh974480.aspx
[IsMemberOf]: http://msdn.microsoft.com/library/azure/dn151601.aspx
[存取 Azure Active Directory 圖形資訊]: mobile-services-javascript-backend-windows-store-dotnet-aad-graph-info.md
