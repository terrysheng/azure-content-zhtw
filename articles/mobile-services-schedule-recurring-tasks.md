...
<a href="/zh-tw/documentation/articles/mobile-services-dotnet-backend-schedule-recurring-tasks/" title=".NET 後端">.NET 後端</a> | <a href="/zh-tw/documentation/articles/mobile-services-schedule-recurring-tasks/"  title="JavaScript 後端" class="current">JavaScript 後端</a>
</div>
...
6.  按 [上一步] 按鈕，再依序按一下 [資料]、[Updates] 資料表、[瀏覽]，並驗證 Twitter 資料是否已插入資料表中。

    ![][6]

7.  按 [上一步] 按鈕，再按一下 [排程器]、選取 [getUpdates]，然後按一下 [啟用]。
