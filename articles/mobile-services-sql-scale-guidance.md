...
6.  設定您的行動服務以使用您剛剛建立的資料庫

    -   如果您要建立新的行動服務，請選取 [+新增]、[運算]、[行動服務]、[建立]。在下一個畫面上填入相關值，在 [資料庫] 欄位中選取 [Use an existing SQL database]，然後選取 [下一步]。在下一個畫面上確實選取您在步驟 5 中建立的資料庫，然後選取 [確定]。
    -   如果您已有行動服務，請導覽至該服務的 [設定] 索引標籤，然後在工具列中選取 [變更資料庫]。在下一個畫面上，在 [SQL 資料庫] 欄位中選取 [Use an existing SQL database]，然後選取 [下一步]。在下一個畫面上確實選取您在步驟 5 中建立的資料庫，然後選取 [確定]。
...
> [WACOM.NOTE]
> 請在伺服器的**主要**資料庫上執行此查詢，**sys.resource\_stats** 檢視只會出現在該資料庫上。
