...
1.  [在行動服務中啟用原始檔控制](#enable-source-control)。
2.  [安裝 Git 與建立本機儲存機制](#clone-repo)。
3.  [將更新指令碼檔案部署到行動服務](#deploy-scripts)。
4.  [在伺服器指令碼中運用共用程式碼和 Node.js 模組](#use-npm)。

若要完成本教學課程，您必須已經建立行動服務，方法是完成[開始使用行動服務](/en-us/develop/mobile/tutorials/get-started)或[開始使用資料](/en-us/develop/mobile/tutorials/get-started-with-data-dotnet)教學課程。

啟用原始檔控制在行動服務中啟用原始檔控制
----------------------------------------
...
2.  按一下 **[儀表板]** 索引標籤，然後按一下 **[Quick glance]** 下的 **[Set up source control]**，並按一下 **[是]** 以確認。
    ...
    > [WACOM.NOTE] 原始檔控制是一項預覽功能。即使這些檔案儲存在行動服務中，我們仍然建議您定期備份指令碼檔案。
...
複製儲存機制安裝 Git 與建立本機儲存機制
---------------------------------------

1.  在您的本機電腦上安裝 Git。

    安裝 Git 所需的步驟會因作業系統而有所不同。如需作業系統特定的發佈和安裝指引，請參閱[安裝 Git](http://git-scm.com/book/en/Getting-Started-Installing-Git)。

    > [WACOM.NOTE] 在某些作業系統上，可提供 Git 的命令列和 GUI 兩種版本。本文提供的指示將使用命令列版本。

2.  開啟命令列，例如 **GitBash** (Windows) 或 **Bash** (Unix Shell)。在 OS X 系統上，您可以透過 **[終端機]** 應用程式來存取命令列。

3.  在命令列中，切換至您要儲存指令碼的目錄。例如，`cd SourceControl`。

4.  使用下列命令建立新 Git 儲存機制的本機複本，以行動服務的 Git 儲存機制 URL 取代 `<your_git_URL>`：
...
共用程式碼和模組在伺服器指令碼中運用共用程式碼和 Node.js 模組
-------------------------------------------------------------

行動服務提供整組核心 Node.js 模組的存取權，透過 **require** 函數，您可以在您的程式碼中使用這些模組。行動服務也可以使用不是核心 Node.js 封裝一部分的 Node.js 模組，您甚至可以將自己的共用程式碼定義為 Node.js 模組。如需關於建立模組的詳細資訊，請參閱 Node.js API 參考文件中的[模組](http://nodejs.org/api/modules.html) (英文)。

接著，您可以在行動服務中新增 [node-uuid](https://npmjs.org/package/node-uuid) Node.js 模組，方法是使用原始檔控制和 Node.js 封裝管理員 (NPM)。接著便可使用此模組，來為插入項目上的 **uuid** 屬性產生新的 GUID 值。

1.  如果您尚未這麼做，請依照 [Node.js 網站](http://nodejs.org/) (英文) 上的步驟，在您的本機電腦上安裝 Node.js。

2.  瀏覽至本機 Git 儲存機制的 `.\service` 資料夾，然後在命令提示字元中執行下列命令：

         npm install node-uuid

    NPM 會在目前位置中建立 `node_modules` 目錄，並在 `\node-uuid` 子目錄中安裝 [node-uuid](https://npmjs.org/package/node-uuid) 模組。

    **注意**

    若目錄階層中已有 `node_modules`，NPM 將改為建立在 `\node-uuid` 子目錄中，而不是在儲存機制中建立新的 `node_modules`。在此情況下，只要刪除現有的 `node_modules` 目錄。
...
-   [定義支援提取通知的自訂 API](/en-us/develop/mobile/tutorials/create-pull-notifications-dotnet)
    說明如何使用自訂 API，支援可更新 Windows 市集應用程式中動態磚的定期通知。
