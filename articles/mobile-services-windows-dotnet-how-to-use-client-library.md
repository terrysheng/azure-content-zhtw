...
此為具類型的序列化模型，請參閱上面的[不具類型的序列化模型][作法：使用不具類型的資料]討論。
...
下列程式碼說明如何在查詢中包含 `Where` 子句，以篩選資料。它會傳回 `Complete` 屬性等於 `false` 之 `todoTable` 中的所有項目。`Where` 函數會套用資料列篩選述詞來查詢資料表。
...
## <a name="customizing"></a><span class="short-header">自訂用戶端</span>作法：自訂用戶端
