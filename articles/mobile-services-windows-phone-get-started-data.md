<properties urlDisplayName="Get Started with Data" pageTitle="開始使用資料 (WP8) - Azure 行動服務" metaKeywords="" description="了解如何開始從 Windows Phone 8 應用程式使用 Azure 行動服務。" metaCanonical="" services="mobile-services" documentationCenter="Mobile" title="Get started with data in Mobile Services" authors="glenga" solutions="" manager="dwrede" editor="" />
...
本教學課程需要 Windows 8 上執行的 Visual Studio 2012 Express for Windows Phone 8 和 [Windows Phone 8 SDK]。若要完成此教學課程來建立 Windows Phone 8.1 應用程式，您必須使用 Visual Studio 2013 Update 2 或更新的版本。
...
1. 在 [**方案總管**] 中以滑鼠右鍵按一下專案名稱，然後選取 [**管理 NuGet 封裝**]。

2. 在左窗格中選取 [**線上**] 類別，搜尋 `WindowsAzure.MobileServices`，在 [**Azure 行動服務**] 封裝上按一下 [**安裝**]，然後閱讀並接受授權合約。
...
6. 在此相同檔案中，依序取消註解定義 **MobileService** 變數的程式碼，並在 **MobileServiceClient** 建構函式中，依序提供行動服務的 URL 和應用程式金鑰。
...
7. 針對定義現有 **items** 集合的程式碼行進行註解，然後取消註解下列程式碼行：
   	...
   	此程式碼會建立行動服務感知繫結集合 (**items**)，和 SQL Database 資料表 **TodoItem** 的 Proxy 類別 (**todoTable**)。 
...
9. 在 **UpdateCheckedTodoItem** 方法中，將 **async** 修正因子新增至方法，然後取消註解下列程式碼行：
...
Windows Phone 8 的**開始使用資料**教學課程到此結束。

## <a name="next-steps"> </a>後續步驟

本教學課程已示範啟用 Windows Phone 8 應用程式，進而使用行動服務資料的基本概念。接下來，請考慮完成本教學課程中採用 GetStartedWithData 應用程式所建立的下列教學課程：
