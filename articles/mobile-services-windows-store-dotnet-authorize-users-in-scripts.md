...
[.NET 後端](/zh-tw/documentation/articles/mobile-services-dotnet-backend-windows-store-dotnet-authorize-users-in-scripts/ ".NET 後端") | [JavaScript 後端](/zh-tw/documentation/articles/mobile-services-windows-store-dotnet-authorize-users-in-scripts/ "JavaScript 後端")

本主題說明如何授權經驗證的使用者，從 Windows 市集應用程式存取 Azure 行動服務中的資料。在本教學課程中，您將在行動服務中註冊指令碼，根據經驗證使用者的 userId 篩選查詢，確保每位使用者只能看到他們自己的資料。

按一下右方的迴紋針可以觀看本教學課程的影片版。

[觀賞教學課程](http://channel9.msdn.com/Series/Windows-Azure-Mobile-Services/Windows-Store-app-Authenticate-and-Authorize-users-with-Server-Scripts-in-Windows-Azure-Mobile-Servi) [播放視訊](http://channel9.msdn.com/Series/Windows-Azure-Mobile-Services/Windows-Store-app-Authenticate-and-Authorize-users-with-Server-Scripts-in-Windows-Azure-Mobile-Servi) 13:52

本教學課程會以行動服務快速入門為基礎，並依據上一個[開始使用驗證](/en-us/develop/mobile/tutorials/get-started-with-users-dotnet)教學課程建立。開始此教學課程之前，您必須先完成[開始使用驗證](/en-us/develop/mobile/tutorials/get-started-with-users-dotnet)。
...
1.  在 Visual Studio 2012 Express for Windows 8 中，開啟您在完成[開始使用驗證](/en-us/develop/mobile/tutorials/get-started-with-users-dotnet)教學課程時所修改的專案。

2.  按 F5 鍵執行應用程式，然後以您選擇的識別提供者登入。

	請注意，即使 TodoItem 資料表中仍有上一個教學課程的項目，但這次並不會傳回任何項目。發生這個情況是因為先前插入的項目並沒有 userId 資料行，而現在具有 null 值。

3.  在應用程式的 **[Insert a TodoItem]** 中輸入文字，然後按一下 **[儲存]**。

	![][3]

	這會在行動服務的 TodoItem 資料表中插入文字和 userId。由於新項目有正確的 userId 值，行動服務會將它傳回並顯示在第二個資料行中。
...
[Get started with data]: /zh-tw/documentation/articles/mobile-services-windows-store-dotnet-get-started-data/
