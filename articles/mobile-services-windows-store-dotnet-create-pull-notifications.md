...
<a href="/zh-tw/develop/mobile/tutorials/create-pull-notifications-dotnet" title="Windows 市集 C#" class="current">Windows 市集 C#</a><a href="/zh-tw/develop/mobile/tutorials/create-pull-notifications-js" title="Windows 市集 JavaScript">Windows 市集 JavaScript</a>
</div>

本主題說明如何使用自訂 API 來支援 Windows 市集應用程式中的定期通知。在啟用定期通知的情況下，Windows 將定期存取您的自訂 API 端點，並使用傳回的 XML (採用磚的特定格式) 來更新開始功能表上的應用程式磚。如需詳細資訊，請參閱[定期通知][定期通知]。

將此功能新增至您在完成[開始使用行動服務][開始使用行動服務]或[開始使用資料][開始使用資料]教學課程時所建立的應用程式。若要達到此目的，您將需要完成下列步驟：

1.  [定義自訂 API][定義自訂 API]
2.  [更新應用程式以開啟定期通知][更新應用程式以開啟定期通知]
3.  [測試應用程式][測試應用程式]

本教學課程會以行動服務快速入門為基礎。在開始本教學課程之前，您必須先完成[開始使用行動服務][開始使用行動服務]或[開始使用資料][開始使用資料]。
