<properties pageTitle="伺服器端授權 (Windows 市集) | 行動開發人員中心" metaKeywords="" description="了解如何在 Azure 行動服務的 JavaScript 後端授權給使用者。" metaCanonical="" services="" documentationCenter="Mobile" title="Service-side authorization of Mobile Services users" authors="glenga" solutions="" manager="dwrede" editor="" />
...
4. 使用下列函式取代現有的指令碼，然後按一下 [儲存]****。
	...
	<p>首次執行插入指令碼時，必須啟用動態結構描述。在已啟用動態結構描述的情況下，行動服務會在首次執行時將 <strong>userId</strong> 資料欄自動新增至 <strong>TodoItem</strong> 資料表。依預設，新行動服務的動態結構描述為已啟用，您應該先將它停用之後再將應用程式發行至 Windows 市集。</p>
    </div>
...
3. 在應用程式中，於 [Insert a TodoItem]**** 中輸入文字，然後按一下 [Save]****。

   	![][3]

   	這樣會在行動服務中的 TodoItem 資料表中插入文字與 userId。因為新項目有正確的 userId 值，它會由行動服務傳回並顯示在第二個資料欄中。
