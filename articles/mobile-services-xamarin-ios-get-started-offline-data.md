...
<a href="/zh-tw/documentation/articles/mobile-services-xamarin-ios-get-started-offline-data" title="Xamarin.iOS" class="current">Xamarin.iOS</a>
...
> [WACOM.NOTE] 本教學課程旨在協助您深入了解如何透過行動服務，使用 Azure 儲存並擷取 Windows 市集應用程式中的資料。因此，本主題將逐步說明已在行動服務快速入門中完成的許多步驟。如果這是您第一次接觸行動服務，請考慮首先完成教學課程[開始使用行動服務][]。
>
...
-   [Azure 行動服務 SDK 1.3.0-alpha3 版 (或更新版本)][]
-   [Azure 行動服務 SQLite Store 1.0.0-alpha2 版 (或更新版本)][]

> [WACOM.NOTE] 下列指示假設您使用附有 Xamarin 延伸的 Visual Studio 2012 或更新版本。如果您在 OS X 上使用 Xamarin Studio，則指示大致上都相同，但您應同時安裝 [NuGet Addin for Xamarin][]，以便能輕鬆地將搶鮮版行動服務 NuGet 封裝新增至您的專案。
...
1.  在 Visual Studio 中，開啟您在[開始使用行動服務][]或[開始使用資料][]教學課程中完成的專案。在 [方案總管] 中，移除對 [元件] 中的 [Azure 行動服務 SDK] 的參考。
...
編輯類別 `QSTodoService`，以啟用 SQLite 本機存放區的行動服務離線功能。
...
4.  在 `QSTodoService` 的建構函式中新增對 `SQLitePCL.CurrentPlatform.Init()` 的呼叫，作為程式碼的第二行：
...
5.  在類別 `QSTodoService` 中，定義新的方法 `InitializeAsync`：
...
6.  在類別 `QSTodoService` 中，定義新的方法 `SyncAsync`：
...
修改 `QSTodoListViewController`，以在使用者執行重新整理動作時呼叫新的 `SyncAsync` 方法。

1.  在 `todoService` 初始化後，在 `ViewDidLoad()` 中新增對 `InitializeStoreAsync` 的呼叫：
...
2.  修改方法 `AddRefreshControl`，以在呼叫 `RefreshAsync` 之前先呼叫 `SyncAsync`：
...
5.  在應用程式中拉下項目清單，以執行重新整理動作。這會使應用程式依序呼叫 `MobileServiceClient.SyncContext.PushAsync` 和 `IMobileServiceSyncTable.PullAsync()`，然後呼叫 `RefreshTodoItems` 以使用本機存放區中的項目重新整理應用程式。
