<properties urlDisplayName="Using Offline Data" pageTitle="使用行動服務 (Xamarin iOS) 中的離線資料 | 行動開發人員中心 " metaKeywords="" description="了解如何使用 Azure 行動服務在您的 Xamarin iOS 應用程式中快取及同步處理離線資料" metaCanonical="" disqusComments="1" umbracoNaviHide="1" documentationCenter="Mobile" title="Using offline data in Mobile Services" authors="donnam" editor="wesmc" manager="dwrede"/>
...
>[WACOM.NOTE] 下列指示假設您使用附有 Xamarin 延伸的 Visual Studio 2012 或更新版本。如果您是在 OS X 上使用 Xamarin Studio
...
[此處](https://github.com/Azure/mobile-services-samples/tree/master/TodoOffline/Xamarin.iOS)可以取得此教學課程中已完成的專案。
...
2. 將成員 `todoTable` 類型由 `IMobileServiceTable` 變更為 `IMobileServicesSyncTable`
...
4. 在 'QSTodoService' 的建構函式中，新增 'SQLitePCL.CurrentPlatform.Init()' 的呼叫做為程式碼的第二行：
...
6. 在類別 'QSTodoService' 中，定義新方法 'SyncAsync'：
...
1. 初始化 `todoService` 之後，在 `ViewDidLoad()` 中新增 `InitializeStoreAsync` 的呼叫。
...
2. 呼叫 `RefreshAsync` 之前，修改方法 `AddRefreshControl` 以呼叫 `SyncAsync`：
...
5. 在應用程式中拉下項目清單，以執行重新整理動作。這會使應用程式呼叫 `MobileServiceClient.SyncContext.PushAsync` 和 `IMobileServiceSyncTable.PullAsync()`，然後呼叫 `RefreshTodoItems` 以使用本機存放區中的項目來重新整理應用程式。 
...
您可以在我們的 [GitHub 範例存放庫](https://github.com/Azure/mobile-services-samples/tree/master/TodoOffline/Xamarin.iOS)中下載此教學課程的完整版本。
