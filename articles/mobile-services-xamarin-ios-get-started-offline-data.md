<properties pageTitle="使用行動服務 (Xamarin iOS) 中的離線資料 | 行動開發人員中心" description="Learn how to use Azure Mobile Services to cache and sync offline data in your Xamarin iOS application" documentationCenter="xamarin" authors="lindydonna" editor="wesmc" manager="dwrede" services=""/>
...
1. 在 Visual Studio 中，開啟您在[開始使用行動服務]教學課程中完成的專案。開啟  `QSTodoService.cs` 檔案。
...
3. 必須先初始化本機存放區，才能執行資料表作業。此動作可用  `InitializeStoreAsync` 方法來完成：
...
5. 在  `QSTodoService` 類別中，會在修改資料、 `InsertTodoItemAsync()` 和  `CompleteItemAsync` 的作業之後呼叫  `SyncAsync()` 方法。此外也會從  `RefreshDataAsync()` 呼叫此方法，讓使用者在每次執行重新整理動作時都能取得最新的資料。應用程式也會在啟動時執行同步處理，因為  `QSTodoListViewController.ViewDidLoad()` 會呼叫  `RefreshDataAsync()`。
...
1. 開啟  `QSTodoService.cs`。編輯  `InsertTodoItemAsync()` 和  `CompleteItemAsync()` 方法，並註解化  `SyncAsync()` 的呼叫。
...
3. 若要變更的 UI 的初始化，使其不會在應用程式啟動時同步處理，請編輯  `QSTodoListViewController.ViewDidLoad()` 方法。在方法結尾處移除  `RefreshAsync()` 的呼叫，並取代為  `ReloadData()` 的呼叫：
