...
本主題說明如何透過 Apple 推播通知服務 (APNS)，使用 Azure 行動服務將推播通知傳送至 iOS 應用程式。在本教學課程中，您使用 Azure 通知中樞啟用推播通知到[快速入門專案](mobile-services-ios-get-started.md/)。完成後，行動服務就會在每次插入記錄時傳送推播通知。
