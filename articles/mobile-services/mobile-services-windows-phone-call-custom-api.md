<properties 
	pageTitle="從 Windows Phone 用戶端呼叫自訂 API - 行動服務" 
...
您會將此功能新增至您完成[將行動服務新增至現有的應用程式](mobile-services-windows-phone-get-started-data.md) 教學課程時所建立的應用程式。若要達到此目的，您將需要完成下列步驟：

1. [定義自訂 API]
2. [更新應用程式以呼叫自訂 API]
3. [測試應用程式]

本教學課程的基礎是 GetStartedWithData 範例 (簡單的 TodoList 應用程式)。在開始本教學課程之前，您必須先完成[將行動服務新增至現有的應用程式](mobile-services-windows-phone-get-started-data.md)。
