...
1. 建立新目錄，然後在此目錄中建立 **server.js** 檔案。**server.js** 檔案應該包含下列內容：
...
2. 建立新網站並記下網站的名稱。例如，以下使用 [Azure 命令列工具] 建立新的 Azure 網站 (名稱為 **mywebsite**)，然後啟用該網站的 Git 儲存機制。

		azure site create mywebsite --git

3. 建立名稱為 **bin** 的新目錄，作為 **server.js** 檔案所在目錄的子目錄。

4. 下載要與應用程式搭配使用的特定 **node.exe** 版本 (Windows 版本)。例如，以下使用 **curl** 下載 0.8.1 版：

		curl -O http://nodejs.org/dist/v0.8.1/node.exe

	將 **node.exe** 檔案儲存到先前建立的 **bin** 資料夾中。

5. 在與 **server.js** 檔案相同的目錄中建立 **iisnode.yml** 檔案，然後在 **iisnode.yml** 檔案中新增下列內容：

		nodeProcessCommandLine: "D:\home\site\wwwroot\bin\node.exe"

	在將您的應用程式發行到 Azure 網站後，專案中的 **node.exe** 檔案將會位在這個路徑中。

6. 發行您的應用程式。例如，由於我稍早是使用 --git 參數建立新的網站，下列命令會將應用程式檔案新增到我的本機 Git 儲存機制，然後將這些檔案推播到網站儲存機制：
