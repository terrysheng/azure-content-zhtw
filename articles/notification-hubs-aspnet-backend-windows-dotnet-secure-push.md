...
> [AZURE.NOTE] 本教學課程假設您已如[開始使用通知中心 (Windows 市集)][開始使用通知中心 (Windows 市集)] 中所述建立並設定通知中心。
> 另請注意，Windows Phone 8.1 需要 Windows (非 Windows Phone) 認證，且該背景工作無法在 Windows Phone 8.0 或 Silverlight 8.1 上運作。若是 Windows 市集應用程式，只有當應用程式已啟用鎖定畫面 (按一下 Appmanifest 中的核取方塊) 時，您才可以透過背景工作接收通知。
