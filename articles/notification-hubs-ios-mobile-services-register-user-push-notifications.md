...
1.  在 Xcode 中，在您完成必要的教學課程[開始使用驗證](/en-us/develop/mobile/tutorials/get-started-with-users-ios/)時所建立的專案中開啟 QSTodoService.h 檔案，然後新增下列 **deviceToken** 屬性：

         @property (nonatomic) NSData* deviceToken;

	此屬性會儲存裝置權杖。

1.  在 QSTodoService.m 檔案中，新增下列 **getDeviceTokenInHex** 方法：
...
2.  在 QSAppDelegate.m 檔案中，將下列程式碼行新增至 **didFinishLaunchingWithOptions** 方法中：

             [[UIApplication sharedApplication] registerForRemoteNotificationTypes:UIRemoteNotificationTypeAlert | UIRemoteNotificationTypeBadge | UIRemoteNotificationTypeSound];

    這會在您的應用程式中啟用推播通知。

3.  在 QSAppDelegate.m 檔案中新增下列方法：
...
4.  (選用) 在 QSAppDelegate.m 檔案中，新增下列處理常式方法：
...
1.  在 QSTodoListViewController.m 檔案中，新增 **registerForNotificationsWithBackEnd** 方法：
    ...
    此方法會建構包含裝置權杖的 json 裝載。接著，它會呼叫您行動服務中的自訂 API，以進行通知的註冊。此方法會建立推播通知的裝置權杖，並將其連同裝置類型傳送至會在通知中心建立註冊的自訂 API 方法。此自訂 API 定義於[使用通知中心來通知使用者](/en-us/manage/services/notification-hubs/notify-users)中。

2.  最後，在使用者成功驗證後，在 **viewDidAppear** 方法中將此 API 的呼叫新增至新的 **registerForNotificationsWithBackEnd** 方法中，如下列範例所示：
