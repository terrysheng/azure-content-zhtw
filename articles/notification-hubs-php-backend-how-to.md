...
-   依照 [開始使用教學課程][]，針對您所選的行動平台，在 PHP 中實作後端部分。
...
另外，如果您要的話，您可以使用 [PHP REST 包裝函式範例][]中的程式碼，直接進入[完成教學課程][]一節。

您可以在 [MSDN][] 上找到所有實作完整 REST 包裝函式的詳細資料。在本節中，我們將針對存取通知中心 REST 端點所需主要步驟的 PHP 實作進行說明：
