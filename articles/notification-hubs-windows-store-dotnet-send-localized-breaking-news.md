...
3.  [建置 Windows 市集用戶端應用程式][建置 Windows 市集用戶端應用程式]
4.  [從後端傳送通知][從後端傳送通知]
...
範本的功能非常強大，您可以在[通知中心指引][通知中心指引]一文中了解詳情。[Windows 市集的通知中心作法][Windows 市集的通知中心作法]則提供了範本運算式語言的參考。
...
1.  在您的 Notifications 類別中，將地區設定參數新增至 *StoreCategoriesAndSubscribe* 和 *SubscribeToCateories* 方法。
    ...
    請注意，我們並未呼叫 *RegisterNativeAsync* 方法，而是呼叫 *RegisterTemplateAsync*：我們要註冊範本依存於地區設定的特定通知格式。我們也為範本提供名稱 ("newsTemplate")，因為我們可能會想註冊多個範本 (例如，一個供快顯通知使用，一個供磚使用)，而且我們必須為其命名，才能加以更新或刪除。
