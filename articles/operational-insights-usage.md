...
Operational Insights 會收集資料，並定期將資料傳送給 Operational Insights 服務。您可以使用 [**使用量**] 儀表板，檢視正在傳送給 Operational Insights 服務的資料量。[**使用量**] 儀表板也會顯示智慧套件每日正在傳送的資料量，及您的管理群組正在傳送資料的頻率。
...
如果您已超過每日使用量上限，或如果接近限制，您可以選擇性地移除智慧套件來減少傳送至 Operational Insights 服務的資料量。如需有關移除智慧套件的詳細資訊，請參閱[使用圖庫來新增或移除智慧套件](operational-insights-add-intelligence-packs.md).
...
3. 如果得知有不必要使用大量配額的智慧套件，您可以考慮移除該智慧套件。
