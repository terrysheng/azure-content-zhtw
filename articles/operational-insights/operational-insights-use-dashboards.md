...
本指南可幫助您了解 Operational Insights 儀表板如何以視覺效果顯示所有儲存的搜尋，提供單一的方式檢視您的環境。
...
在儀表板中，磚是由您儲存的搜尋所形成。Operational Insights 內建許多預先儲存的搜尋，讓您可以隨時開始。您會看到下列圖示大綱，說明如何開始。

![圖示](./media/operational-insights-use-dashboards/pictorial.png)

在 [我的儀表板] 檢視中，只要按一下頁面底部的 [自訂] 齒輪，就可以進入自訂模式。在頁面右側開啟的面板會顯示工作區所有儲存的搜尋。

![新增磚 1](./media/operational-insights-use-dashboards/add-tile1.png)

若要以磚的視覺效果顯示儲存的搜尋，只要將搜尋拖曳到左側的空白區域即可。當您拖曳時，搜尋就會變成磚。
...
顯示已儲存搜尋之結果的時間線，或是根據您的搜尋是否根據欄位彙總結果來依欄位顯示結果清單。

**度量** <p> ![度量](./media/operational-insights-use-dashboards/metric.png)

在磚中以數字顯示搜尋結果總數。度量磚可讓您設定一個閾值，當達到該閾值時就讓磚發亮顯示。
