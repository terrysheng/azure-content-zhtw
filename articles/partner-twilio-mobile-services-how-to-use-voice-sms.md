...
Azure 客戶享有[特別優惠][special_offer]：升級 Twilio 帳戶時，可獲贈價值 $10 的 Twilio 點數。此 Twilio 點數可用來折抵任何 Twilio 使用量 ($10 點數相當於最多傳送 1,000 則簡訊，或最多接收 1000 分鐘的撥入語音，視電話號碼所在地點或通話目的地而定)。請至[ ahoy.twilio.com/azure][special_offer] 兌換 Twilio 點數來開始使用。

Twilio 是隨收隨付的服務。不需要設定費，隨時都可結清帳戶。如需詳細資訊，請參閱 [Twilio 定價][twilio_pricing]。  

<h2><a id="Concepts"></a>概念</h2>
Twilio API 是一套為應用程式提供語音和簡訊功能的 RESTful API。用戶端程式庫有多種語言版本，相關清單請參閱 [Twilio API 程式庫] [twilio_libraries]。您可參閱關於使用 Twilio 搭配以下列撰寫之任何 Azure 應用程式的教學課程：[.NET][azure_twilio_howto_dotnet]、[node.js][azure_twilio_howto_node]、[Java][azure_twilio_howto_java]、[PHP][azure_twilio_howto_php]、[Python][azure_twilio_howto_python] 或 [Ruby][azure_twilio_howto_ruby]。
...
3. 將 Twilio 封裝參考新增至**「相依性」**物件後，package.json 檔案的外觀應與下列類似：
	...
	>[AZURE.NOTE]應將 Twilio 的相依性新增為 `"twilio": "~1.7.0"` (包含 (~))：不支援包含插入號 (^) 的參考。 
...
下列指令碼將說明如何使用 **makeCall** 函數從您的行動服務起始對話通話。此程式碼也使用 Twilio 提供的網站來傳回 Twilio 標記語言 (TwiML) 回應。使用您的值取代 **From** 與 **To** 電話號碼，並確定您已先驗證 Twilio 帳戶的 **From** 電話號碼，再執行程式碼。
...
當應用程式開始呼叫 Twilio API 時 (例如，透過 client.InitiateOutboundCall 方法)，Twilio 會傳送要求至 URL，然後應該會傳回 TwiML 回應。做法中的範例：撥出電話中的範例使用 Twilio 提供的 URL http://twimlets.com/message 來傳回回應。
