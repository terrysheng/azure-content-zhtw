...
如需詳細資訊，請參閱 [][2]<https://github.com/twilio/twilio-php/blob/master/README.md></a>。
...
-   **注意**：若要對 SSL 憑證驗證錯誤進行疑難排解，請參閱 [][3]<http://readthedocs.org/docs/twilio-php/en/latest/usage/rest.html></a>。
...
當您的應用程式開始呼叫 Twilio API 時，Twilio 會將要求傳送至 URL，然後應該會傳回 TwiML 回應。前述範例使用 Twilio 提供的 URL [][4]<http://twimlets.com/message></a>。(雖然 TwiML 是設計給 Twilio 使用的，但您也可以在瀏覽器中檢視 TwiML。例如，按一下 [][4]<http://twimlets.com/message></a> 可查看空白的 `<Response>` 元素，又例如，按一下 [][5]<http://twimlets.com/message?Message%5B0%5D=Hello%20World></a> 可查看包含 `<Say>` 元素的 `<Response>` 元素。)
...
如需 TwiML 的詳細資訊，請參閱 [][6]<https://www.twilio.com/docs/api/twiml></a>。
