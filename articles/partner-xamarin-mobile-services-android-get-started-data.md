...
本教學課程以 [GetStartedWithData 應用程式][下載 Xamarin.Android 應用程式專案] (Xamarin.Android 應用程式) 為基礎而建立。此應用程式的 UI 與 Mobile Services Android 快門入口所產生的應用程式相同，差別在於新增的項目會儲存在本機的記憶體。
