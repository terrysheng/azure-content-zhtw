...
3. 按一下 [**執行**] 啟動應用程式，然後在文字方塊中輸入超過 10 個字元的文字，再按一下 [**新增**] 按鈕。
...
2. 在 [**執行**] 功能表中，按一下 [**執行**] 啟動應用程式，並在文字方塊中輸入文字 (少於 10 個字元)，然後按一下 [**新增**]。
...
1. 在 **TodoItem** 類別的私人欄位定義中新增下列程式碼：
    ...
    <div class="dev-callout"><b>注意</b>
	<p><code>DataMember's Name</code> 註解會指示用戶端，將應用程式中的新 <code>CreatedAt</code> 屬性對應至 TodoItem 資料表中所定義、且具有不同名稱的 <code>createdAt</code> 資料行。使用此註解，您的應用程式便可擁有物件上的屬性名稱，且該名稱會與 SQL Database 中的資料欄名稱不同。若是沒有此註解，則會因大小寫差異而發生錯誤。</p>
    </div>

2. 在 GetView 方法中，將下列程式碼新增至將 <code>checkBox.Text</code> 設為 <code>currentItem.Text</code> 的目前程式碼上方：
