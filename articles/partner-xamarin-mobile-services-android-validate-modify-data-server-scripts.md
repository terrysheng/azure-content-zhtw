...
1.  在 **TodoItem** 類別的私人欄位定義中新增下列程式碼：
    ...
    **注意**
    ...
    `DataMember's Name` 註解會指示用戶端，將應用程式中的新 `CreatedAt` 屬性對應至 TodoItem 資料表中所定義、且具有不同名稱的 `createdAt` 資料行。使用此註解，您的應用程式便可擁有物件上的屬性名稱，且該名稱會與 SQL Database 中的資料欄名稱不同。若是沒有此註解，則會因大小寫差異而發生錯誤。
