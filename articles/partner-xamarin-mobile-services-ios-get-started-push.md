...
2. 按一下 [**Keychain Access**]，展開 [**憑證助理**]，然後按一下 [**Request a Certificate from a Certificate Authority...**]。
...
4. 在 [**另存新檔**] 中輸入憑證簽署要求 (CSR) 檔案的名稱，在 [**位置**] 中選取儲存位置，然後按一下 [**儲存**]。
...
按一下 [**提交**] 之後，您將會看見 [**註冊完成**] 畫面，如下所示。按一下 [**完成**]。
...
按一下應用程式 ID 將會顯示應用程式和應用程式 ID 上的詳細資料。按一下 [**設定**] 按鈕。
...
3. 接著，從 [**應用程式識別碼**] 下拉式清單中，選取行動服務快速入門應用程式的應用程式識別碼，然後按一下 [**繼續**]。
...
2. 開啟 **TodoItem** 類別，並新增下列屬性：

        [JsonProperty(PropertyName = "deviceToken")]
        public string DeviceToken { get; set; }

3. 在 **QSTodoService** 中，覆寫現有的用戶端宣告以做為：
...
這會註冊新的 insert 指令碼，它會使用 [apns 物件] 將推播通知 (插入的文字) 傳送至插入要求中提供的裝置。 
    ...
    <div class="dev-callout"><b>注意</b>
   <p>此指令碼會延遲通知的傳送，讓您有時間關閉應用程式來接收快顯通知。</p>
   </div> 
...
1. 按 [**執行**]  按鈕以建置專案，並在可執行 iOS 的裝置上啟動應用程式，然後按一下 [**確定**] 以接受推播通知
    ...
    <div class="dev-callout"><b>注意</b>
    <p>您必須在應用程式中確實接受推播通知。只有在應用程式第一次執行時，才會發生此要求。</p>
    </div>
