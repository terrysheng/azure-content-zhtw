...
> [AZURE.NOTE] 如果您已經安裝 PowerShell 資源庫的 Azure 模組，安裝程式會自動移除該模組。這是為了防止混淆您已安裝的模組與其所在的位置。PowerShell 資源庫模組通常安裝於 **%ProgramFiles%\\WindowsPowerShell\\Modules**。相反地，WebPI 安裝程式會在 **%ProgramFiles%\\Microsoft SDKs\\Azure\\PowerShell** 中安裝 Azure 模組。**PowerShellGet** 會將模組解除安裝，如果在解除安裝時載入模組相依性，則會留下鎖定的 .dll 與其資料夾。如果在安裝期間發生錯誤，請移除 **%ProgramFiles%\\WindowsPowerShell\\Modules** 資料夾中的 [Azure*] 資料夾。

如果您已經透過 PowerShell 資源庫安裝 Azure PowerShell，但卻想要使用 WebPI 安裝，則 WebPI 安裝將會自動移除已從資源庫安裝的 Cmdlet。

> [AZURE.NOTE] 從 WebPI 安裝時，PowerShell **$env:PSModulePath** 會發生一個已知的問題。如果您的電腦因為系統更新或其他安裝而需要重新啟動，可能會導致 **$env:PSModulePath** 不包含 Azure PowerShell 的安裝路徑。您可以藉由重新啟動電腦來解決這個問題。
...
- **Install-Module AzureRM** 會為 AzureRM 模組安裝啟動程序模組。此模組包含的 Cmdlet 有助於以安全且一致的方式來更新、解除安裝及匯入 AzureRM 模組。AzureRM 模組包含必要的模組和版本範圍 (最小和最大) 清單，以確保 AzureRM 主要版本不會有重大變更。如需有關語意版本設定的詳細資訊，請參閱 [semver.org](http://semver.org)。這表示您可以使用特定的 AzureRM 版本來撰寫 Cmdlet，並得知所有透過啟動載入程式安裝的模組將不會引入重大變更。
- **Install-AzureRM** 會安裝在啟動程序模組中宣告的所有模組。
...
- 在至少執行 Windows 8 或 Windows Server 2012 的電腦上，可使用內建的 [搜尋]。請在 [開始] 畫面輸入「power」。這會傳回目標範圍內的應用程式清單，其中包括 Windows PowerShell。若要開啟主控台，請按一下任一應用程式。(如要將應用程式釘選到 [開始] 畫面，請用滑鼠右鍵按一下圖示)。

- 在 Windows 版本早於 Windows 8 或 Windows Server 2012 的電腦上，請使用 [開始] 功能表。請按一下 [開始] 功能表的 [所有程式]、按一下 [附屬應用程式]、按一下 [Windows PowerShell] 資料夾，然後按一下 [Windows PowerShell]。
...
您可以有多個帳戶和訂閱供 Azure PowerShell 使用。只要執行 **Add-AzureRmAccount** 數次，就能新增多個帳戶。
