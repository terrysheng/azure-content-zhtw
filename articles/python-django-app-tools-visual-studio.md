<properties urlDisplayName="Django with Visual Studio" pageTitle="使用 Visual Studio (Python) 的 Django - Azure 教學課程" metaKeywords="Azure Django web app, Azure Django virtual machine" description="本教學課程說明如何建置在 Azure 虛擬機器中主控的 Django Web 應用程式。" metaCanonical="" services="cloud-services" documentationCenter="Python" title="使用 Python Tools for Visual Studio 1.5 建立 Django 應用程式" authors="huvalo" solutions="" manager="wpickett" editor="" />
...
**注意：**本教學課程也已製作成 [Youtube 影片][Youtube 影片]。

**注意：**目前已針對 PTVS 2.0 Beta 設計了[更新、內容更豐富的教學課程][更新、內容更豐富的教學課程]。

透過可用工具，即可輕鬆開發 Azure。
本教學課程假設您先前沒有使用 Azure 的經驗。
完成本指南之後，您將在雲端啟動並執行 Django 應用程式。
...
Web 應用程式。應用程式將託管於 Web 角色的執行個體中，
而此執行個體在 Azure 中執行時，本身將託管於專用的虛擬機器 (VM) 中。
...
在開始開發 Azure 應用程式之前，您需要先取得工具，並設定開發環境。如需取得及安裝 Azure SDK for Python 的詳細資訊，請參閱[如何安裝 Python][如何安裝 Python]。
