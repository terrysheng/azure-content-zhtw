...
本文說明 **2014-10-20-Preview** 版本的 Azure 搜尋服務 REST API。這是 API 的原型版本，隨時都會變更。請勿在生產程式碼中採用此 API 上的相依性。

> [AZURE.NOTE]由於這是實驗性版本，因此，並不是本文件中描述的所有功能都可在 Azure 入口網站使用者介面中使用。具體來說，目前僅在 API 中公開的是，在欄位中透過 `analyzer` 選項支援多種語言、新的 `tag` 評分函式，以及新的 `suggesters` 功能。如果您使用此 API 來啟用這其中一個功能，就能在入口網站中檢視索引定義，但無法更新它們，而且您看不到已視覺化的新功能。

與此版本相關的其他 API 內容包含下列項目：

- [評分設定檔 (Azure 搜尋服務 REST API：2014-10-20-Preview)](../search-api-scoring-profiles-2014-10-20-preview/)
...
服務操作的端點是您佈建的 Azure 搜尋服務 URL：https://*your-service-name*.search.windows.net。
...
您可以針對指定的索引資源，透過簡單的 HTTP 要求 (POST、GET、PUT、DELETE)，在 Azure 搜尋服務中建立與管理索引。若要建立索引，您必須先 POST 一份 JSON 文件來說明索引結構描述。結構描述可定義索引欄位、其資料類型，以及使用它們的方式 (例如，用於全文檢索搜尋、篩選器、排序、面向設定或建議)。它也會定義評分設定檔、建議工具，以及可用來設定索引行為的其他屬性。
...
您可以使用簡單的 HTTP POST 要求，在 Azure 搜尋服務中建立新的索引。要求的本文是一份 JSON 文件，可用來指定索引名稱、欄位、屬性以控制查詢行為、為結果進行評分、建議工具，以及 CORS 選項。
...
建立索引時，可以設定下列屬性。如需評分和評分設定檔的詳細資訊，請參閱[評分設定檔 (Azure 搜尋服務 REST API：2014-10-20-Preview)](../search-api-scoring-profiles-2014-10-20-preview/)。
...
`suggestions` - 設定欄位是否可用於自動完成自動提示的動作。這只能針對類型 `Edm.String` 或 `Collection(Edm.String)` 的欄位進行設定。`suggestions` 預設是 `false`，因為它在您的索引中需要額外的空間。**注意**：請考慮使用版本 2014-10-20-Preview 中引進的 `suggesters` 屬性，而不使用此選項來提供建議。在未來版本中，`suggestions` 屬性會被淘汰，有利於使用個別的 `suggesters` 規格。
...
`scoringProfiles` - 定義自訂評分行為，讓您能夠影響搜尋結果中哪些項目的出現機率會比較高。評分設定檔是由加權欄位和函式所組成。如需評分設定檔中所使用的屬性詳細資訊，請參閱[評分設定檔 (Azure 搜尋服務 REST API：2014-10-20-Preview)](../search-api-scoring-profiles-2014-10-20-preview/)。
...
建議工具會在搜尋中啟用自動完成功能。通常會在使用者輸入期間將部分搜尋字串傳送到建議 API，而此 API 會傳回一組建議的片語。

Azure 搜尋服務正在轉換為新的建議工具 API。版本 2014-07-31-Preview 提供一個可縮小建議範圍的 API，其會將某個欄位標記為 `"suggestions": true`，然後就能執行簡短字串的前置建議 (3-25 個字元)。從版本 2014-10-20-Preview 開始，Azure 搜尋服務就開始根據本節所述的「建議工具」來提供功能更強大的建議版本。這個新的實作可以進行前置和中置比對，並對搜尋字串中的錯誤提供更好的容錯能力。從版本 2014-10-20-Preview 開始，強烈建議您使用新的建議工具 API。

目前的建議工具支援在用來建議特定文件 (而非鬆散的字詞/片語) 時的效果最好。針對此類型的建議工具，最佳的來源欄位是標題、名稱，以及可識別它們的其他相關簡短片語。通常不太有效的欄位種類範例是很容易重複的欄位 (例如，類別和標記) 或非常長的欄位 (例如，說明或註解欄位)。

做為索引定義的一部分，您可以將單一建議工具新增到 `suggesters` 集合。定義建議工具的屬性如下：

- `name`：建議工具的名稱。您可以在呼叫 `suggest` API 時使用建議工具名稱。
- `searchMode`：用來搜尋候選片語的策略。目前唯一支援的模式是 `analyzingInfixMatching`，其可在句子開頭或中間執行彈性的片語比對。
- `sourceFields`：用來提供建議之內容來源的一或多個欄位清單。只有類型 `Edm.String` 和 `Collection(Edm.String)` 的欄位可以用來做為提供建議的來源。在目前的建議工具預覽版本中，只能使用尚未設定自訂分析器的欄位。
...
您可以在 Azure 搜尋服務中，使用 HTTP PUT 要求來更新現有的索引。在公開預覽版本中，更新包括將欄位新增到現有的結構描述、修改 CORS 選項，以及修改評分設定檔 (請參閱[評分設定檔 (Azure 搜尋服務 REST API：2014-10-20-Preview)](../search-api-scoring-profiles-2014-10-20-preview/))。您可以在要求 URI 上指定要更新的索引名稱：

    PUT https://[search service url]/indexes/[index name]?api-version=[api-version]
    Content-Type: application/json
    api-key: [admin key]

**重要：**對於索引結構描述更新的支援會限制為不需重建搜尋索引的操作。目前不支援任何需要重新編製索引的結構描述更新 (例如，變更欄位類型)。儘管無法變更或刪除現有欄位，但可隨時新增欄位。這同樣適用於建議工具。新欄位可能會在新增欄位時新增到建議工具，但是您無法從建議工具中移除欄位，也無法將現有欄位新增到建議工具。
