...
<p>此範例將說明 Adventure Works 資源回收公司的搜尋應用程式。完成本教學課程後，您將取得附有豐富搜尋使用性的線上產品目錄，其中包括 Facet 導覽、多個可用於搜尋結果的排序選項，以及預先輸入查詢建議。
...
<h2 id="sub-2">建立 Azure Search 索引</h2>

在此步驟中，您將根據 AdventureWorks SQL Server Database 中的資料，使用 **CatalogIndex** 新建名為 "catalog" 的 Azure Search 索引。

1.	在 Visual Studio 中，開啟名為 **AdventureWorksCatalog.sln** 的 Azure Search 示範方案。  
...
> [WACOM.NOTE] 如果出現包含「金鑰 'attachdbfilename' 的值無效」的錯誤，或是其他資料庫附加錯誤，表示可能有 UAC 衝突發生。為進行此示範，請執行下列動作以解決這些錯誤： 
> 將方案複製到可供已驗證的使用者存取的新資料夾或現有資料夾 (如 Temp)。 
> 使用 [以系統管理員身分執行]**** 啟動 Visual Studio。 
> 開啟方案、建置方案，然後按 **F5** 來建立索引。

若要驗證索引建立和文件上傳是否成功，請移至 [Azure Preview 入口網站](https://portal.azure.com)中的 [搜尋服務] 儀表板。在 [Usage] 中，索引計數應該會增加一個，而您應有 294 個文件，資料庫中的每項產品各一個。
...
<h2 id="sub-3">探索 CatalogIndexer</h2>
...
<h2 id="sub-4">使用 Azure Search 建置 MVC4 應用程式</h2>
...
5.	按 **F5** 鍵啟動專案。如果您收到建置錯誤，請依照下列[疑難排解](#err-mvc) 步驟執行。 
...
<h2 id="sub-5">探索 AdventureWorksWeb</h2>
...
2. **HomeController.cs** 內有兩個主要函數：
	...
	**Search**：當使用者按一下搜尋按鈕或選擇 Facet 時，即會呼叫此函數，以擷取結果並將其傳回至要顯示的 [索引] 檢視。

	**Suggest**：當使用者在 [搜尋] 方塊中輸入時，即會呼叫此函數，以根據您的 Azure Search 索引內容傳回建議清單。

我們將深入詳細探索這兩個函數。  

3.	在 **HomeController.cs** `Search` 函數中，您會看見 `_catalogSearch.Search` 的呼叫，其中包含您 Azure Search 服務的連線物件。當您呼叫位於 **CatalogSearch.cs** 中的 Search 函數時，您可以看見此函數使用這些參數建置 Azure Search 查詢。查詢的結果會儲存在名為 `result` 的 JSON 物件中，然後傳回至會在網頁中剖析及顯示結果的 `Index` 檢視。
...
1. 開啟 Package Manager Console：**[工具** | NuGet 封裝管理員**** | 封裝管理員主控台****]
