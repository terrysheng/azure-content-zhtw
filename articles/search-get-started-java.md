...
輸入搜尋字詞，讓搜尋引擎運作一下。試著輸入地區名稱。"Roger Williams" 是羅德島州的第一任州長。許多公園、建築物和學校都是以他的姓名命名。
...
如果您已有一些 Azure 搜尋服務的背景知識，可以利用此範例做為進一步實驗的跳板，例如擴充[搜尋頁面](../search-pagination/)或實作[多面向導覽](../search-faceted-navigation/)。您也可以新增計數和批次處理文件，讓使用者可以逐頁查看結果，藉此改進搜尋結果頁面。
