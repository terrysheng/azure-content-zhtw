<properties title="Get started with Azure Search Management REST API" pageTitle="開始使用 Azure 搜尋管理 REST API" description="Get started with Azure Search Management REST API" metaKeywords="" services="Azure Search" solutions="" documentationCenter="" authors="Heidist" manager="mblythe" videoId="" scriptId="" />
...
在完成本教學課程的過程中，將會佈建兩項服務：Azure Search 和 Azure Active Directory (AD)。此外，您會建立 AD 應用程式，以建立起 Azure 中用戶端應用程式與資源管理員端點之間的信任 。

您需要有 Azure 帳戶才能完成此教學課程。


<h2 id="download">下載範例應用程式</h2>

本教學課程是以 C# 撰寫的 Windows 主控台應用程式為基礎，您可以在 Visual Studio 2012 或 2013年中進行編輯和執行

您可以在 Codeplex 的 [Azure Search Management API Demo](https://azuresearchmgmtapi.codeplex.com/) 上尋找此用戶端應用程式。
...
4. 按一下 [**新增**]，開啟 [**App 服務** | **Active Directory**]。在此步驟中，您會建立新的 Active Directory 服務。這項服務會裝載您從現在起要定義一些步驟的 AD 應用程式。建立新的服務有助於從您可能已在 Azure 中裝載的其他應用程式區隔本教學課程。
...
4. 選擇 [**Add an application my organization is developing**]。此選項會對未發行至應用程式庫的應用程式提供註冊設定。由於用戶端應用程式不是應用程式庫的一部分，所以這是此教學課程的正確選擇。
...
11. 向下捲動至 [權限] 並選取 [**Windows Azure 管理 API**]。在此步驟中，您會指定用戶端應用程式需要存取的 API (在此例中為 Azure 資源管理員 API)，以及所需的存取層級。

12. 在 [委派權限] 中，按一下下拉式清單並選取 [**存取 Azure 服務管理 (Preview)**]。
...
如果您尚未[從 Codeplex 下載範例應用程式](linkTBD)，您在此步驟中會需要它。
...
6. 提供 `SubscriptionID`。
	- 移至主要入口網站頁面。
	- 按一下左側導覽窗格底部的 [**設定**]。
	- 從 [訂閱] 索引標籤，複製訂閱 ID 並將它貼到 Program.cs 中。
