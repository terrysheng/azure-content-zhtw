...
身為管理員，您可以使用新的 [Azure 預覽入口網站][Azure 預覽入口網站]，將搜尋新增到現有的 Azure 訂用帳戶中。只有管理員可在訂用帳戶中新增功能。設定服務時，有 2 種價格層可供選擇。
...
當您準備好要登入時，請參閱[在 Azure 預覽入口網站中設定搜尋][在 Azure 預覽入口網站中設定搜尋]。
...
1.  登入 [Azure 預覽入口網站][Azure 預覽入口網站]。
2.  按一下 [瀏覽] | [全部] | [搜尋服務]。
3.  按一下您的搜尋服務名稱以開啟儀表板。
...
由於 API 版本並沒有顯示在入口網站頁面中，因此您無法提供這項資訊。如需目前和舊版的 API，請參閱 [Azure 搜尋 REST API][Azure 搜尋 REST API]。
...
使用搜尋服務 API 可取得文件和索引的計數。根據價格層不同，會有些與這些計數相關聯的固定限制。如需詳細資訊，請參閱[限制條件][限制條件]。
...
若要協助進行未來規劃，您可能需要檢查儲存體 (使用[取得索引統計資料][取得索引統計資料]) 以了解實際上您可以使用的空間大小。
