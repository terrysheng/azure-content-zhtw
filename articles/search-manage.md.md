...
Azure 搜尋是一種雲端架構服務和 HTTP 架構 API，可用來自訂搜尋應用程式。搜尋服務提供的引擎具備全文檢索文字分析、進階搜尋功能、儲存體及查詢命令語法。

本文說明如何在 Microsoft Azure 中管理搜尋服務。

如前所述，管理工作需要搭配新的預覽入口網站。Azure 搜尋不適用於較舊版本的入口網站。很快地，管理 API 將可使用指令碼撰寫一般管理工作。
...
身為管理員，您可以使用新的 [Azure 預覽入口網站][]，將搜尋新增到現有的 Azure 訂用帳戶中。只有管理員可在訂用帳戶中新增功能。設定服務時，有 2 種價格層可供選擇。

您可以使用共用服務 (我們並不會向現有訂戶收費)，並建議您將其用於學習、概念證明測試，以及小型開發專案上。共用服務的限制為 50 MB 儲存體、3 個索引及有限的文件份數 (無論佔用的儲存體是否少於完整的 50 MB，皆固定限制 10,000 份文件)。共用服務並沒有效能保證，因此如果您要建置生產搜尋應用程式，可以考慮使用標準搜尋。
...
當您準備好要登入時，請參閱[在 Azure 預覽入口網站中設定搜尋][]。

## 管理工作

雖然有些服務允許擁有共同管理員，但在 Azure 搜尋服務上，每個訂用帳戶只會有一個管理員。您必須是管理員才能執行本節中描述的工作。
除了將搜尋新增到訂用帳戶中，管理員還需負責以下其他工作：
...
由於 API 版本並沒有顯示在入口網站頁面中，因此您無法提供這項資訊。如需目前和舊版的 API，請參閱 [Azure 搜尋 REST API][]。
...
使用搜尋服務 API 可取得文件和索引的計數。根據價格層不同，會有些與這些計數相關聯的固定限制。如需詳細資訊，請參閱[限制條件][]。
...
若要協助進行未來規劃，您可能需要檢查儲存體 (使用[取得索引統計資料][]) 以了解實際上您可以使用的空間大小。
...
<!--Link references-->
  ...
  [限制條件]: http://msdn.microsoft.com/en-us/library/dn798934.aspx
  ...
  [Azure 搜尋 REST API]: http://go.microsoft.com/fwlink/p/?LinkdID=509922
  [1]: ./media/search-manage/Azure-Search-Manage-2-Keys.png
  [取得索引統計資料]: http://msdn.microsoft.com/en-us/library/dn798942.aspx
  [文件計數]: http://msdn.microsoft.com/en-us/library/dn798924.aspx
