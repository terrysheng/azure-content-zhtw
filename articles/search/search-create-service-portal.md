...
## 將 Azure 搜尋服務加入您的訂用帳戶中  

身為管理員，如果選擇使用共用的服務，您可以免費將 Azure 搜尋服務加入現有的訂用帳戶中，如果選擇使用專用資源，則可按照標準費率來加入。
...
許多客戶一開始會使用免費服務，但是當查詢效能或儲存體不足以支應所需執行的工作負載時，就會升級至標準層。標準層會在 Azure 資料中心內提供專屬於您的專用資源。Azure 搜尋服務作業同時需要儲存體和服務複本。當您註冊標準搜尋時，您可以將服務設定最佳化，以使用更多對您的狀況來說最重要的任何資源。
