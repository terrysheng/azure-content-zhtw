...
2. 開啟 Azure 搜尋服務的服務儀表板。以下提供一些尋找儀表板的方法。
	- 在動態工具列中，按一下 [搜尋服務]。Jumpbar 列出您的訂用帳戶中佈建的每項服務。如果已定義搜尋服務，您會看到清單中的**搜尋服務**。
	- 在動態工具列中，按一下 [瀏覽]，然後在搜尋方塊中鍵入 "search" 來產生您訂用帳戶中建立的所有搜尋服務的清單。
...
有數種方法能夠建立索引。如果您的資料位於 Azure 搜尋服務可以搜耙的存放區 (例如 Azure SQL Database、Azure VM 上的 SQL Server 或 DocumentDB) 中有可使用的資料，您可以使用索引子非常輕鬆地建立及填入索引。

若要讓這項工作以入口網站為基礎，我們假設可以透過 [匯入資料] 精靈使用索引子搜耙 DocumentDB 中的資料。

繼續之前，請建立[範例 DocumentDB 資料庫](#apdx-sampledata)以搭配本教學課程使用，再回到本節執行下列步驟。
...
2. 在精靈中，按一下 [資料來源] > [DocumentDB] > [名稱]，輸入資料來源的名稱。資料來源是 Azure 搜尋服務中可以搭配其他索引子使用的連接物件。建立資料來源後，它就會成為您的服務中可用的「現有資料來源」。
...
若要檢查進度，請回到服務儀表板，向下捲動，然後按兩下 [索引子] 磚即可開啟索引子清單。您應該會在清單中看到您剛建立的索引子，您應該會看到指出「進行中」或成功的狀態，以及索引編製到 Azure 搜尋服務中的文件數目。
...
在您執行精靈一次之後，您可以返回並檢視或修改個別的元件︰索引、索引子或資料來源。在索引上不允許進行某些編輯 (例如變更欄位資料類型)，但大部分的屬性和設定都可修改。若要檢視個別的元件，請按一下儀表板上的 [索引]、[索引子] 或 [資料來源] 圖格以顯示現有的物件清單。
...
1. [按一下這裡](https://github.com/HeidiSteen/azure-search-get-started-sample-data)以下載包含音樂市集 JSON 資料檔案的 ZIP 檔。我們為此資料集提供 246 個 JSON 文件。
...
3. 按一下 [Document Explorer]。
4. 按一下 [新增文件]。
5. 在 [新增文件] 中，分批上傳 JSON 檔案 (一批 100 個或更少檔案)。
	- 386\.json
	- 387\.json
	- . . .
6. 按一下 [查詢總管] 以確認資料已上傳，進而符合 Document Explorer 的上傳需求。

若要這麼做，最簡單的方式就是修改預設查詢，以便它選取前 300 個 (總計小於此資料集中的 300 個項目)，或者您可以撰寫 `select * from musicstorecoll`，然後按一下 [執行查詢]。
