...
`mergeOrUpload` | 如果含有指定索引鍵的文件已經存在於索引中，則此動作的行為會類似 `merge`。如果文件不存在，其行為會類似新文件的 `upload`。| 索引鍵以及其他任何您想要定義的欄位 |- `delete` | 從索引中移除指定的文件。| 僅索引鍵 | 索引鍵欄位以外的所有指定欄位都將遭到忽略。如果您想要從文件中移除個別欄位，請改用 *merge* 並直接將該欄位明確設為 null。
...
假設此 "hotels" 索引範例已填入幾份文件。請留意我們在使用 `merge` 時是如何不必指定所有可能的文件欄位，以及在使用 `delete` 時如何僅指定文件索引鍵 (`hotelId`)。
...
如需文件動作和成功/錯誤回應的詳細資訊，請參閱[此頁面](https://msdn.microsoft.com/library/azure/dn798925.aspx)。如需失敗時可能傳回的其他 HTTP 狀態碼的詳細資訊，請參閱[這篇文章](https://msdn.microsoft.com/library/azure/dn798925.aspx)。

## 下一步
在填入 Azure 搜尋服務索引後，您就可以開始發出查詢來搜尋文件。
