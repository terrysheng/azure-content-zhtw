...
- **基本 (預覽)**會針對生產工作負載提供規模較小的專用計算資源。此層目前為預覽狀態，並享有[優惠費率](https://azure.microsoft.com/pricing/details/search/)。
- **標準**執行的所在專用機器僅限您的服務使用，在各層級具有更多的儲存和處理容量，包括最小組態。標準有兩個層級 (S1 和 S2)。 

## 層級限制

[AZURE.INCLUDE [azure-search-limits](../../includes/azure-search-limits-all.md)]

> [AZURE.NOTE] 每秒查詢 (QPS) 會隨時改變 (尤其在共用服務中)，因為輸送量會因可用頻寬和系統資源爭用而異。備份共用服務的 Azure 計算和儲存體資源由多個訂閱者共用，因此解決方案的 QPS 會因有多少其他工作負載在同時執行而隨時不同。在標準層級中，由於可控制較多的參數，所以能更準確地估計 QPS。請參閱＜[管理搜尋解決方案](search-manage.md)＞中的最佳作法一節，取得如何計算工作負載 QPS 的指引。
