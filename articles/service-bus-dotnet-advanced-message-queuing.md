...
本作法指南說明如何使用服務匯流排 .NET API 從 .NET 應用程式使用服務匯流排代理傳訊功能 (佇列和發佈/訂閱主題)。我們另備有說明如何使用標準 Java 訊息服務 (JMS) API 來進行相同操作的附屬作法指南。您可以搭配使用這兩部指南來瞭解使用 AMQP 1.0 的跨平台傳訊。

開始使用開始使用服務匯流排
--------------------------

本指南假設您已具有服務匯流排命名空間，且其中包含名為 "queue1" 的佇列。如果您沒有服務匯流排命名空間，可以使用 [Azure 管理入口網站](http://manage.windowsazure.com)來建立命名空間和佇列。如需建立服務匯流排命名空間和佇列的詳細資訊，請參閱標題為[如何使用服務匯流排佇列](https://www.windowsazure.com/en-us/develop/net/how-to-guides/service-bus-queues/) (英文) 的作法指南。

下載 SDK下載服務匯流排 SDK
--------------------------

服務匯流排 SDK 2.1 或更新版本提供 AMQP 1.0 支援。您可以從 NuGet (<http://nuget.org/packages/WindowsAzure.ServiceBus/> (英文)) 下載最新版本的 SDK。

編寫 .NET 應用程式的程式碼編寫 .NET 應用程式的程式碼
----------------------------------------------------
...
目前的版本中有幾項在使用 AMQP 時不支援的 API 功能。這些不支援的功能列示於後續的＜不支援的功能和限制＞一節中。在使用 AMQP 時，某些進階組態設定亦有不同的意義。本簡短的作法指南未使用上述任何設定，不過如果您需要詳細資料，請參閱[服務匯流排 AMQP 1.0 開發人員指南](http://msdn.microsoft.com/en-us/library/windowsazure/jj841071.aspx) (英文)。
...
如需使用服務匯流排和 AMQP 1.0 進行跨平台傳訊之詳細資料的詳細資訊，請參閱[服務匯流排 AMQP 1.0 開發人員指南](http://msdn.microsoft.com/en-us/library/windowsazure/jj841071.aspx) (英文)。

### 從 JMS 到 .NET

示範從 JMS 到 .NET 的傳訊操作：

-   在不搭配任何命令列引數的情況下啟動 .NET 範例應用程式。
-   啟動 Java 範例應用程式並搭配 "sendonly" 命令列引數。在此模式中，應用程式將不會接收來自佇列的訊息，它只會傳送訊息。
-   在 Java 應用程式主控台內按幾次 **Enter**，如此能導致訊息傳送。
-   .NET 應用程式將能接收到這些訊息。
...
如需詳細資訊，請參閱[服務匯流排 AMQP 1.0 開發人員指南](http://msdn.microsoft.com/en-us/library/windowsazure/jj841071.aspx) (英文)。該主題含有不支援之 API 的詳細清單。
...
您也可以從 Java、C、Python 及 PHP 等其他語言使用服務匯流排 AMQP 1.0。使用這些語言組建的元件可使用服務匯流排中的 AMQP 1.0 可靠而真實地交換訊息。如需詳細資訊，請參閱[服務匯流排 AMQP 1.0 開發人員指南](http://msdn.microsoft.com/en-us/library/windowsazure/jj841071.aspx) (英文)。
