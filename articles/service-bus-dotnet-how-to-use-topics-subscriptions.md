...
## <span class="short-header">如何設定連接字串</span>如何設定服務匯流排連接字串
...
例如，在您的服務定義 (`*.csdef`) 檔案中新增一個設定，如下所示：
...
## <span class="short-header">如何建立主題</span>如何建立主題
...
**CreateQueue** 方法的超載可讓您調整主題的屬性(例如，針對要在傳送至主題的訊息所套用的 [存留時間] 設定預設值)。您可以使用
**TopicDescription** 類別來套用這些設定。下列範例將示範如何使用大小上限為 5 GB 和預設訊息存留時間為 1 分鐘的設定，來建立名為 "TestTopic" 的佇列。
...
## <span class="short-header">如何建立訂閱</span>如何建立訂閱

您也可以使用 **NamespaceManager** 類別來建立主題訂閱。訂閱是經過命名，且能包含選擇性篩選器，以用來限制傳遞至訂閱的虛擬佇列的訊息集合。
...
## <span class="short-header">傳送訊息至主題</span>如何傳送訊息至主題
...
自訂閱接收訊息的最簡單方式是使用 **SubscriptionClient** 物件。**SubscriptionClient** 物件可以在兩種不同模式下運作：
**ReceiveAndDelete** 和 **PeekLock**。
...
以下範例將示範如何使用預設的 **PeekLock** 模式來接收與處理訊息。若要指定其他 **ReceiveMode** 值，您可以使用 **CreateFromConnectionString** 的另一個超載。此範例會建立一個無限迴圈，並在訊息抵達 "HighMessages" 訂閱時處理訊息。請意，至 "HighMessages"訂閱的路徑會以 "\<*主題路徑*\>/subscriptions/\<*訂閱名稱*\>" 的格式提供。
...
## <span class="short-header">應用程式當機與無法讀取的訊息</span>如何處理應用程式當機與無法讀取的訊息

服務匯流排提供一種功能，可協助您從應用程式的錯誤或處理訊息的問題中順利復原。如果接收應用程式因為某些原因無法處理訊息，它可以呼叫所接收訊息上的 **Abandon** 方法(而不是 **Complete** 方法)。這將導致服務匯流排將訂閱中的訊息解除鎖定，讓此訊息可以被相同取用應用程式或其他取用應用程式重新接收。
...
## <span class="short-header">刪除主題和訂閱</span>如何刪除主題和訂閱
...
-   建立一個可行的應用程式，往返傳送或
  ...
  [服務匯流排代理訊息 .NET 教學課程]: http://msdn.microsoft.com/en-us/library/hh367512.aspx
