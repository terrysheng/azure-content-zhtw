...
本指南將示範如何使用服務匯流排佇列。這些範例均以 JavaScript 撰寫並使用 Node.js Azure 模組。所涵蓋的案例包括**建立佇列**、**傳送並接收訊息**，以及**刪除佇列**。如需佇列的詳細資訊，請參閱[後續步驟](#next-steps)一節。
...
-   [作法：自佇列接收訊息](#receive-messages)
-   [作法：應用程式當機與無法讀取的訊息](#handle-crashes)
-   [後續步驟](#next-steps)
...
建立空白的 Node.js 應用程式。如需建立 Node.js 應用程式的指示，請參閱[建立並部署 Node.js 應用程式至 Azure 網站](/en-us/develop/nodejs/tutorials/create-a-website-(mac)/)、[Node.js 雲端服務](/zh-tw/documentation/articles/cloud-services-nodejs-develop-deploy-app/) (使用 Windows PowerShell) 或 [使用 WebMatrix 的網站](/en-us/develop/nodejs/tutorials/web-site-with-webmatrix/)。

設定應用程式使用服務匯流排
--------------------------

若要使用 Azure 服務匯流排，您需要下載並使用 Node.js azure 封裝。這包含一組便利程式庫，能與服務匯流排 REST 服務通訊。
...
使用記事本或其他文字編輯器，將以下內容新增至應用程式的 **server.js** 檔案頂端：

    var azure = require('azure');

### 設定 Azure 服務匯流排連接

azure 模組會讀取環境變數 AZURE\_SERVICEBUS\_NAMESPACE 及 AZURE\_SERVICEBUS\_ACCESS\_KEY，以取得連接 Azure 服務匯流排所需的資訊。如果未設定這些環境變數，則呼叫 **createServiceBusService** 時必須指定帳戶資訊。
...
**ServiceBusService** 物件讓您能使用佇列。下列程式碼會建立 **ServiceBusService** 物件。在靠近 **server.js** 檔案頂端的 azure 陳述式之後加入它來匯入模組：
...
**createServiceBusService** 也支援其他選項，讓您覆寫預設佇列設定，例如訊息存留時間或佇列大小上限。下列範例顯示將佇列大小上限設定為 5GB 而且存留時間為 1 分鐘：
...
可以將選用的篩選作業套用到使用 **ServiceBusService** 執行的作業。篩選操作可包括記錄、自動重試等等。篩選器是以簽章實作方法的物件：
...
Azure SDK for Node.js 包含了實作重試邏輯的兩個篩選器：**ExponentialRetryPolicyFilter** 和 **LinearRetryPolicyFilter**。以下會建立使用 **ExponentialRetryPolicyFilter** 的 **ServiceBusService** 物件：
...
下列範例示範如何使用 **sendQueueMessage** 將測試訊息傳送至名稱為「myqueue」的佇列：
...
如果 **isPeekLock** 參數是設定為 **true**，接收會變成兩階段作業，因此可以支援無法容許遺漏訊息的應用程式。當服務匯流排收到要求時，它會尋找要取用的下一個訊息、將其鎖定以防止其他取用者接收此訊息，然後將它傳回應用程式。在應用程式完成處理訊息 (或可靠地儲存此訊息以供未來處理) 之後，它可呼叫 **deleteMessage** 方法，並提供將刪除的訊息做為參數，來完成接收程序的第二個階段。**deleteMessage** 方法會將訊息標示為已取用，並將它從佇列中移除。
