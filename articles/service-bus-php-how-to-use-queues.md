...
本指南將說明如何搭配使用服務匯流排佇列與 PHP。這些範例均是以 PHP 撰寫，並使用 [Azure SDK for PHP](http://go.microsoft.com/fwlink/?LinkId=252473) (英文)。本文說明的案例包括**建立佇列**、**傳送並接收訊息**，以及**刪除佇列**。
...
> [WACOM.NOTE]
> 您的 PHP 安裝也必須已安裝並啟用 [OpenSSL 延伸](http://php.net/openssl)。
...
若要傳送訊息至服務匯流排佇列，應用程式會呼叫 **ServiceBusRestProxy-\>sendQueueMessage** 方法。下列程式碼將說明如何將訊息傳送至我們先前在 `MySBNamespace` 服務命名空間內建立的 `myqueue` 佇列。
