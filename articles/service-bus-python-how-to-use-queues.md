...
本指南將說明如何使用服務匯流排佇列。相關範例是以 Python 撰寫的，並使用 Python Azure 模組。所涵蓋的案例包括**建立佇列、傳送並接收訊息**，以及**刪除佇列**。如需佇列的詳細資訊，請參閱[後續步驟](#next-steps)一節。
...
**create\_queue** 也支援其他選項，而可讓您覆寫訊息存留時間或佇列大小上限等預設佇列設定。下列範例說明將佇列大小上限設為 5GB，將存留時間設為 1 分鐘的設定：
...
當您讀取訊息後，訊息便會從佇列中刪除，但您可以將選用參數 **Peek\_Lock** 設為 **true**，藉此讀取 (查看) 並鎖定訊息，且避免訊息在佇列中遭到刪除。
