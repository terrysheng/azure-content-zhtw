<properties 
	...
	description="了解如何在 Azure 使用服務匯流排佇列。程式碼範例以 Ruby 撰寫。" 
...
本指南將說明如何使用服務匯流排佇列。這些範例均以 Ruby 撰寫，並使用 Azure gem。本文說明的案例包括**建立佇列、傳送並接收訊息**和
**刪除佇列**。如需佇列的詳細資訊，請參閱[後續步驟](#next-steps) 一節。

## 什麼是服務匯流排佇列？

服務匯流排佇列支援**代理訊息通訊**模型。使用佇列時，分散式應用程式的元件彼此不直接通訊，而是透過扮演中繼角色的佇列來交換訊息。訊息產生者 (傳送者) 會將訊息遞交給佇列，然後繼續其處理工作。
