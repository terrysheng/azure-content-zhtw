...
**注意**訂閱是持續性的，它們會持續存在，直到本身或相關的主題遭到刪除為止。如果應用程式含有建立訂閱的邏輯，它應該會先使用 getSubscription 方法檢查訂閱是否存在。
...
**注意**由於預設篩選器會自動套用至所有新訂閱，因此您必須先移除預設篩選器，否則 **MatchAll** 會覆寫您指定的其他任何篩選器。您可以使用 **Azure::ServiceBusService** 物件的 **delete\_rule()** 方法移除預設規則。
...
以下範例將示範如何使用 **receive\_subscription\_message()** 來接收與處理訊息。此範例會先使用設為 **false** 的 **:peek\_lock** 來接收及刪除來自 "low-messages" 訂閱的訊息，然後再接收另一個來自 "high-messages" 的訊息，接著使用 **delete\_queue\_message()** 刪除訊息：
