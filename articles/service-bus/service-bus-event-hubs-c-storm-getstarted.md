<properties 
	pageTitle="開始使用事件中心" 
	description="遵循此教學課程，開始使用 Azure Event Hubs 以 C 傳送事件並且在 Apache Storm 叢集中接收這些事件。" 
...
6. 按一下頁面頂端的 [**設定**] 索引標籤上，新增名為 **SendRule** 且具有 *Send* 權限的規則，再新增另一個名為 **ReceiveRule** 且具有 *Listen* 權限的規則，然後按一下 [**儲存**]。
...
- [使用 Storm 和 HDInsight 分析感應器資料]是一個完整案例教學課程，其使用事件中心、Storm 和 HBase 擷取 Hadoop 叢集中的感應器資料。
