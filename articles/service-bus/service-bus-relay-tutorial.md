...
1. 將 `System.ServiceModel.dll` 的參考新增至專案︰在方案總管中，以滑鼠右鍵按一下專案資料夾下的 [參考]資料夾，然後按一下 [加入參考]。選取 [加入參考] 對話方塊中的 [.NET] 索引標籤，並向下捲動直到您看到 **System.ServiceModel**。請加以選取，然後按一下 [確定]。
...
1. 將連線模式設定為 [自動偵測]。在用戶端應用程式的 `Main()` 方法中加入下列程式碼。
