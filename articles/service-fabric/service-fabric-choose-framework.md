...
Azure Service Fabric 提供兩個高階架構來建置服務：Reliable Actors API 和 Reliable Services API。雖然兩者都建置於相同的 Service Fabric 核心，但會在並行處理、資料分割以及通訊方面的簡易性和彈性之間進行不同的權衡。了解這兩個模型相當實用，以便您可以在應用程式中選擇特定服務的適當架構。
...
|您想要使用單一執行緒的物件，同時仍能夠調整和維護一致性。|您想要使用 Reliable Collections (例如 .NET Reliable Dictionary 和 Reliable Queue) 來儲存和管理您的狀態。|
