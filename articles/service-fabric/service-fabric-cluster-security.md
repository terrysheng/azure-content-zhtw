...
如要設定安全的 Service Fabric 叢集，您需要至少一個伺服器 x509 憑證。您接著會將該憑證上傳至 Azure 金鑰保存庫並將它使用於叢集建立程序
...
**步驟 2.2**：確定您的電腦已安裝 Azure PS 1.0+。如果您還沒安裝，我們強烈建議您依照[如何安裝和設定 Azure PowerShell](../powershell-install-configure.md) 一文中所述的步驟來進行。

**步驟 2.3**：安裝完畢之後，請開啟 Powershell 視窗，並匯入 ServiceFabricRPHelpers.psm (這就是您在步驟 2.1 所下載的模組)。
...
**步驟 2.4**：如果您要使用之前已取得的憑證，請依照下列步驟進行。否則，請跳至步驟 2.5。步驟 2.5 將討論如何建立自我簽署的憑證，以及如何將該憑證部署至 KeyVault 的步驟。

登入您的 Azure 帳戶。如果這個 PowerShell 因某種原因而無法運作，請查看 Azure PowerShell 是否已正確安裝。

```
Login-AzureRmAccount
```

下列指令碼會建立新的資源群組和/或保存庫 (如果原本不存在)。
...
指令碼順利執行完畢之後，您會看到類似下列內容的輸出。請記下輸出，以便在步驟 #3 (設定安全的叢集) 中使用。
...
您提供給指令碼的 OutputPath，將包含已由指令碼上傳至 KeyVault 的新自我簽署憑證。
...
**憑證指紋**：64881409F4D86498C88EEC3697310C15F8F1540F **SourceVault** /金鑰存放庫的資源識別碼：/subscriptions/35389201-c0b3-405e-8a23-9f1450994307/resourceGroups/chackdankeyvault4doc/providers/Microsoft.KeyVault/vaults/chackdankeyvault4doc **憑證 URL** /能前往金鑰存放庫中憑證位置的 URL：https://chackdankeyvalut4doc.vault.azure.net:443/secrets/chackdantestcertificate3/fvc8df6300834326a95d05d90e0720ea
...
>[AZURE.NOTE] 
1\.需要使用的憑證會指定於 [安全性組態] 下的 NodeType 層級。2.您必須為您的叢集中的每個節點類型指定此項目。3.雖然這份文件會逐步解說如何使用入口網站執行這項作業，但您可以使用 ARM 範本執行相同的作業。

![Azure 入口網站中的安全性組態螢幕擷取畫面][SecurityConfigurations_01]

必要參數：

- **安全性模式。** 請選取 [X509 憑證]。這會對 Service Fabric 指出您想要設定安全的叢集。
...
Service Fabric 允許您指定兩個憑證 (主要和次要)。您在建立時指定的憑證會預設為主要憑證。若要新增其他憑證，您需要將該憑證部署到叢集中的 VM。本文件的步驟 #2 (請見上文) 概述如何將新的憑證上傳至 KeyValult。您在這裡可以使用同一個 KeyValult，就跟您處理第一個憑證的方式相同。如需詳細資訊，請參閱[Deploy certificates to VMs from a customer-managed Key Vault (將憑證從客戶管理的金鑰保存庫部署到 VM)](http://blogs.technet.com/b/kv/archive/2015/07/14/vm_2d00_certificates.aspx)。

一旦完成該作業，請使用入口網站或資源管理員，對 Service Fabric 指出您具有的次要憑證也可以使用。您只需要指紋。

**新增憑證來供叢集使用的程序**：前往入口網站、瀏覽至您想要新增此憑證的叢集資源、按一下憑證設定、輸入次要憑證指紋，然後按下 [儲存]。將會開始進行部署，而在成功完成該部署時，您目前可以使用主要或次要憑證在叢集上執行管理作業。

![入口網站中的憑證指紋螢幕擷取畫面][SecurityConfigurations_02]

**移除舊憑證來讓叢集無法使用的程序**：前往入口網站、瀏覽至您的叢集安全性設定，然後移除其中一個憑證。請務必在移除憑證後按下 [儲存]，如此就會開始進行新的部署。一旦完成該部署，您所移除的憑證便無法再用來連接到叢集。
...
X509 數位憑證通常用來驗證用戶端與伺服器，以及加密及數位簽署訊息。如需有關這些憑證的詳細資料，請前往 MSDN Library 的[使用憑證](http://msdn.microsoft.com/library/ms731899.aspx)。

>[AZURE.NOTE]
1\.執行生產工作負載的叢集中所使用的憑證，應利用已正確設定的 Windows Server 憑證服務來建立，或是透過已核准的 [Certificate Authority (CA，憑證授權單位)](https://en.wikipedia.org/wiki/Certificate_authority) 來取得。2.絕對不要在生產環境中使用透過 MakeCert.exe 等工具建立的暫存或測試憑證。3.對於用於測試的叢集，您可以選擇使用自我簽署憑證。
...
下列文章說明如何產生具有主體別名 (SAN) 的憑證：[如何為安全的 LDAP 憑證新增主體別名](http://support.microsoft.com/kb/931351)。
