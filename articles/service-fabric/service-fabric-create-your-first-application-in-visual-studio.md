...
Service Fabric SDK 包含的 Visual Studio 增益集提供用來建立、偵錯和部署 Service Fabric 應用程式的範本和工具。本主題逐步解說在 Visual Studio 中建立第一個應用程式的程序。
...
5. 以滑鼠右鍵按一下 [本機叢集管理員] 系統匣應用程式並選擇 [管理本機叢集]，以啟動 Service Fabric 總管。
  ...
  Service Fabric 總管提供了叢集的視覺表示法，包括部署至叢集合的應用程式集以及構成叢集的實體節點集合。
