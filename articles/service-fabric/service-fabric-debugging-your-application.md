...
8. 偵錯服務類似於偵錯任何其他應用程式。中斷點通常可以透過 Visual Studio 進行設定以輕鬆偵錯。即使可靠集合會在多個節點上複寫，它們仍會實作 IEnumerable。這表示您可以在 Visual Studio 中使用 [結果檢視]，同時進行偵錯以查看已在內部儲存的內容。您可以在程式碼中的任何位置輕鬆設定中斷點。
