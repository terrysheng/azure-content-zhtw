...
2. 在 [連接端點] 區段中，指定本機或遠端 Service Fabric 叢集的發佈端點。若要新增或變更連接端點，請選擇 [選取...] 按鈕。[選取 Service Fabric 叢集] 對話方塊會顯示根據您的 Azure 訂用帳戶，您可以對其發佈的可用 Service Fabric 叢集連接端點。請注意，如果您尚未登入 Visual Studio，系統將會提示您登入。
...
Visual Studio 發佈體驗已針對發佈至遠端叢集 (與您的其中一個 Azure 訂用帳戶相關聯) 最佳化。不過，可以發佈至任意端點 (例如 Service Fabric 合作對象叢集)，方法是直接編輯發行設定檔 XML。如上所述，預設會提供兩個發行設定檔 (**Local.xml** 和 **Cloud.xml**)，但是您可以針對不同的環境建立其他設定檔。例如，您可能想要建立設定檔以發佈至合作對象叢集，也許命名為 **Party.xml**。
