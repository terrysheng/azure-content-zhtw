...
Azure Service Fabric 中的資源平衡器可提供數種機制來描述叢集。在執行階段期間，資源平衡器會使用這些資訊來放置服務，以確保在叢集中執行之服務的高可用性，同時確保叢集資源的最大使用率。資源平衡器用來描述叢集的功能包括容錯網域、升級網域、節點屬性和節點容量。除此之外，資源平衡器有一些可以調校其效能的組態選項。

## 重要概念

### 容錯網域

容錯網域可讓叢集管理員定義可能在相同時間因為共用實體的相依性 (例如電源和網路來源) 而發生錯誤的實體節點。容錯網域通常代表這些共用相依性的相關階層，包括可能從容錯網域樹狀目錄中的較高點一起發生錯誤的更多節點。下圖顯示依據階層式容錯網域來架構的數個節點 (以資料中心、機架和刀鋒視窗排序)。

![透過容錯網域組織的節點][Image1]

 在執行階段期間，Azure 資源管理員會考慮叢集中的容錯網域，並嘗試針對指定的服務分散複本，以免將它們全都放在不同的容錯網域中。在任何一個容錯網域發生錯誤時，此程序有助確保該服務和其狀態的可用性沒有遭到破壞。下圖顯示即使有足夠的空間將服務複本集中在一個或兩個網域中，仍會將其分散於數個容錯網域中。
...
節點屬性也可用來指定較抽象的屬性，以協助放置原則決策。例如，您可將「顏色」指派給叢集內的數個節點，做為分割成不同叢集區段的依據。程式碼範例顯示，節點的節點屬性是透過叢集資訊清單設為節點類型定義的一部分。節點屬性之後可套用到叢集內的多個節點。
...
### 節點容量
節點容量是一種索引鍵/值組，用來定義特定節點可以使用的特定資源名稱和數量。下面的程式碼範例顯示某個節點，它具有名為 "MemoryInMB" 的計量容量，而且預設有 2048 MB 的可用記憶體。容量是透過叢集資訊清單來定義，就像節點屬性一樣。
...
由於在節點上執行的服務，可以透過報告負載來更新其容量需求，因此資源平衡器會定期檢查節點是否達到或超過其計量的任何容量。如果是，資源平衡器可將服務移至負載較少的節點，以減少資源爭用並提升整體效能和使用率。

請注意，雖然節點類型的屬性區段中也會列出指定的計量，但如果是可以在執行階段期間使用之節點的屬性，則最好列為容量。將指定計量另外列為屬性時，可讓依據「最低硬體需求」的服務查詢含有放置約束限制的節點。如果在執行階段期間，其他服務也會使用資源的話，就可能需要這麼做。我們建議您也將其納為容量，讓資源平衡器可以採取其他動作。

新的服務建立時，Service Fabric 資源平衡器會使用現有節點的容量及現有服務的使用量相關資訊，判斷是否有足夠的容量可容納整個新服務。如果容量不足，就會拒絕「建立服務」要求並顯示錯誤訊息，說明容量問題。


### 資源平衡器組態

在叢集資訊清單中，下列組態值可定義資源平衡器的整體行為。

在資源平衡器回應之前，*平衡臨界值*會控制叢集與特定計量間的不平衡程度。平衡臨界值是指資源平衡器重新平衡叢集之前，允許存在的最常使用節點與最少使用節點之比率上限。

下圖顯示兩個範例，其中針對指定度量的平衡臨界值是 10。

![平衡臨界值][Image5]

請注意，在這個階段中，節點上的「使用率」並不會考慮節點容量所判斷的節點大小。而只會參照針對特定計量目前報告的節點絕對使用量。

下列程式碼範例顯示，計量的平衡臨界值是透過叢集資訊清單內的 FabricSettings 元素，針對每個計量進行設定。
...
*活動臨界值*可限制資源平衡器執行的頻率，方法是限制資源平衡器在出現大量絕對負載時的回應情況。如此一來，如果以特定的計量來說，叢集不是非常忙碌，資源平衡器就不會執行，即使叢集內有少量計量非常不平衡亦然。這種方法可防止進行實質效益很低的叢集重新平衡時帶來的資源浪費。下圖顯示度量的平衡臨界值為 4，且活動臨界值為 1536。

![活動臨界值][Image6]

請注意，活動和平衡臨界值都必須超過相同的計量，才會執行資源平衡器。觸發兩個不同的計量之一時，並不會執行資源平衡器。

下列程式碼範例顯示，如同平衡臨界值，活動臨界值是透過叢集資訊清單內的 FabricSettings 元素，針對每個計量進行設定。
...
- PLBRefreshInterval--控制資源平衡器掃描資訊且必須檢查條件約束違規的頻率。條件約束違規包含不符合的放置約束限制、所需執行個體或複本數量不足的服務、超過部分計量容量的節點、超載的容錯或升級網域，或叢集失衡。資源平衡器會使用平衡和活動臨界值以及叢集節點的目前負載檢視來識別條件約束違規。重新整理間隔的定義是以秒為單位，而且預設值為 1。或者，可以使用 MinConstraintCheckInterval 和 MinPlacementInterval 這兩個新的間隔來控制條件約束檢查與放置的頻率。如果定義這些新的參數，就無法使用與定義 PLBRefreshInterval。

- MinConstraintCheckInterval--控制資源平衡器掃描資訊且必須檢查條件約束違規的頻率。條件約束違規包含不符合的放置約束限制、所需執行個體或複本數量不足的服務、超過部分計量容量的節點、超載的容錯或升級網域，或叢集失衡。資源平衡器會使用平衡和活動臨界值以及叢集節點的目前負載檢視來識別條件約束違規。條件約束檢查間隔的定義是以秒為單位。如果未定義條件約束檢查間隔，其預設值會等於 PLBRefreshInterval 的值 (兩個值不能同時指定)。

- MinPlacementInterval--控制資源平衡器檢查是否有需要放置的新執行個體或複本並嘗試加以放置的頻率。放置間隔的定義是以秒為單位。如果未定義條件放置間隔，其預設值會等於 PLBRefreshInterval 的值 (兩個值不能同時指定)。
...
請注意，這些值都比較積極，但如果平衡和活動的臨界值符合指定的計量，負載平衡整體來說，只會發生在叢集中。下列程式碼範例顯示特定的叢集是否不需要精確和作用中的負載平衡；您可以透過 FabricSettings 元素中的組態，將這些值設為比較保守。在此範例組態中，兩個條件約束檢查之間最短的時間為 10 秒，放置的最短時間為 5 秒，而負載平衡會每隔 5 分鐘發生一次。請注意，在此情況下不需定義 PLBRefreshInterval。
