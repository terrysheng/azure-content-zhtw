...
本文說明使用 Visual Studio Team Services (VSTS) 為 Service Fabric 應用程式設定持續整合的步驟，以確保能夠自動建置、封裝及部署您的應用程式。請注意，這些指示每次都會從頭重新建立叢集。
...
若要安裝 Azure PowerShell，請依照上一節**安裝 Azure PowerShell 並登入**中的步驟進行。略過**登入 Azure PowerShell** 小節。
...
2.	執行憑證管理員。
    ...
    a.開啟 Windows 控制台。以滑鼠右鍵按一下 [開始] 按鈕並選擇 [控制台]。
...
3.	授與本機服務帳戶權限以使用自動化憑證。
    ...
    a.在 [憑證 - 本機電腦] 底下，展開 [個人]，然後選擇 [憑證]。

    b.在清單中尋找憑證。

    c.以滑鼠右鍵按一下您的憑證，然後選擇 [所有工作] > [管理私密金鑰]。

    d.選擇 [新增] 按鈕，然後輸入 [本機服務] 並且按一下 [檢查名稱]。

    e.選擇 [確定] 按鈕，然後關閉憑證管理員。
...
1.	下載 agent.zip。作法：
    ...
    a.登入您的 Team 專案，例如 ****https://[your-VSTS-account-name].visualstudio.com**。

    b.選擇畫面右上角的齒輪圖示。

    c.從控制台中選擇 [代理程式集區] 索引標籤。

    d.選擇 [下載代理程式] 以下載 agent.zip 檔案。
...
4.  請確認您的組建代理程式已註冊。作法：
    ...
    a.返回網頁瀏覽器 (應該會在 `https://[your-VSTS-account-name].visualstudio.com/_admin/_AgentPool`)，重新整理頁面。

    b.選擇您稍早執行 ConfigureAgent.ps1 時選取的代理程式集區。
...
1.	建立空白組建定義。作法：
    ...
    f.選取您註冊組建代理程式的代理程式佇列，並勾選 [連續整合] 核取方塊。
...
### 新增「組建」步驟
@<Author GitHub alias> – 請檢閱您的文章的審稿校對、解決我在註解中輸入的任何問題，並讓我知道我是否已變更任何地方的技術意義。
...
4.	選擇 [方案] 欄位旁的 [...] 按鈕，然後選取應用程式專案的 .sfproj 檔案。
