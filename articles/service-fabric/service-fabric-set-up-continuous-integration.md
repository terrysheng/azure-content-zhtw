...
在您可以設定組建電腦之前，必須建立組建代理程式用來向 Azure 進行驗證的[服務主體](../resource-group-create-service-principal-portal.md)。您也需要建立憑證，並將其上載至金鑰保存庫，因為 Azure 金鑰保存庫不支援服務主體驗證。您可以從任何電腦執行這些步驟。您的開發電腦是不錯的選擇。
...
1.	在系統管理 PowerShell 命令提示字元中，將目錄變更為您解壓縮 `Manual.zip` 所在的位置。

2.	執行 PowerShell 指令碼 `CreateAndUpload-Certificate.ps1` 與下列參數。

    |參數|值|
    |---|---|
    |KeyVaultLocation|任何值。必須符合您計劃建立叢集的位置。|
    |CertificateSecretName|任何值。|
    |SecureCertificatePassword|任何值。當您在組建電腦上匯入憑證時使用。|
...
2.	如果您還沒有電腦，可以使用預先安裝的 Visual Studio 2015 快速佈建 Azure 虛擬機器 (VM)。作法：
    ...
    6.	選擇映像 **Visual Studio Enterprise 2015 Update 1 With Azure SDK 2.8 on Windows Server 2012 R2**。
...
3. 輸入 `PSModulePath` 做為名稱，輸入 `%PROGRAMFILES%\WindowsPowerShell\Modules` 做為值。
	...
	1. 將 `%PROGRAMFILES%` 取代為 `PROGRAMFILES` 環境變數的值。
...
1.	將憑證匯入您的組建電腦。作法：
    ...
    2. 使用您稍早傳遞至 `CreateAndUpload-Certificate.ps1` 的密碼開啟系統管理 PowerShell 命令提示字元，並執行下列命令：
...
3.	授與本機服務帳戶權限以使用自動化憑證。
    ...
    4.	選擇 [新增] 按鈕，然後輸入 [本機服務] 並且按一下 [檢查名稱]。
...
1.	下載 agent.zip。作法：
    ...
    3.	從控制台中選擇 [代理程式集區] 索引標籤。

    4.	選擇 [下載代理程式] 以下載 agent.zip 檔案。

    5.	將 agent.zip 複製到您稍早建立的組建電腦。

    6.	將 agent.zip 解壓縮至組建電腦上的 `C:\agent` (或具有簡短路徑的任何位置)。
...
4. 請確認您的組建代理程式已註冊。作法：
    ...
    2. 選擇您稍早執行 ConfigureAgent.ps1 時選取的代理程式集區。

    3. 確認您的組建代理程式出現在清單中，並且具有綠色狀態反白顯示。若強調顯示是紅色，表示組建代理程式在連接到 Team Services 時發生問題。
...
1.	將 [ServiceFabricContinuousIntegrationScripts.zip](https://gallery.technet.microsoft.com/Set-up-continuous-f8b251f6) 解壓縮到您電腦上的任一資料夾。將 `Powershell\Automation` 的內容複製到原始檔控制中的任一資料夾。
...
1.	建立空白組建定義。作法：
    ...
    6.  選取您註冊組建代理程式的代理程式佇列，並勾選 [連續整合] 核取方塊。
...
>[AZURE.NOTE] 這個步驟與上一個 [移除叢集資源群組] 步驟之間的差異在於：這個步驟應核取 [永遠執行]。
...
6.	在 [控制選項] 底下，核取 [永遠執行] 核取方塊。

7.	儲存組建定義。

### 試試看！

按一下 [佇列組建] 開始一個組建。組建也會在推送/簽入時觸發。
...
1.	透過 Azure 管理入口網站或 Azure PowerShell 手動建立測試叢集。您可以參考 `ProvisionAndDeploy-SecureCluster.ps1` 指令碼做為參考。
