<properties
   ...
   description="取得如何使用 Visual Studio Team Services 設定 Service Fabric 應用程式之連續整合的概觀。"
...
# 使用 Visual Studio Team Services 為 Azure Service Fabric 應用程式設定持續整合

本文章將帶領您完成使用 Visual Studio Team Services 為 Service Fabric 應用程式設定持續整合，讓您可以自動化您應用程式的建置、封裝及部署。請注意，本文件反映目前的經驗且預期隨著開發進度變更。此外，這些指示會每次從頭重新建立叢集。

## 先決條件

開始時，請先在 Visual Studio Team Services 上設定您的專案：

1. 若還沒有專案，請使用您的 [Microsoft 帳戶](http://www.microsoft.com/account)建立 Team Services 帳戶。
...
在您可以設定組建電腦之前，必須建立自動化用來向 Azure 進行驗證的「服務主體」。您也需要建立憑證，並將其上載至金鑰保存庫，因為 Azure 金鑰保存庫不支援服務主體驗證。您可以從任何電腦執行這些步驟。您的開發電腦是不錯的選擇。
...
2.	安裝和更新 AzureRM 模組。
    ...
    2.  啟動 PowerShell 命令提示字元。
    3.	使用命令 `Install-Module AzureRM` 安裝 AzureRM 模組。
    4.	使用命令 `Update-AzureRM` 更新 AzureRM 模組。
...
4.	登入 Azure PowerShell。
    ...
    2. 在顯示的對話方塊中，輸入您的 Azure 認證。
    3. 執行命令 `Get-AzureRmSubscription`。
    4. 找出您想要使用的訂用帳戶然後執行命令 `Select-AzureRmSubscription -SubscriptionId <id for your subscription>`。

### 建立服務主體

1.	將 [ServiceFabricCIManualScripts.zip](http://go.microsoft.com/fwlink/?LinkId=703773) 解壓縮到這台電腦上的資料夾。
2.	在系統管理 PowerShell 命令提示字元中，將目錄變更為您解壓縮 ServiceFabricCIManualScripts.zip 所在的位置。
...
2.	執行 PowerShell 指令碼 CreateAndUpload-Certificate.ps1 與下列參數：
...
2.	如果您還沒有電腦，可以使用預先安裝的 Visual Studio 2015 快速佈建 Azure 虛擬機器 (VM)。作法：
    ...
    5.	選擇 [計算] > [虛擬機器] > [從資源庫]。
    6.	選擇映像 **Visual Studio Enterprise 2015 With Azure SDK 2.7 on Windows Server 2012 R2**。

        >[AZURE.NOTE]Azure SDK 不是必要的元件，但是沒有任何可用的映像僅安裝 Visual Studio 2015。

    7.	依照對話方塊中的指示建立您的 VM。(建議您選擇 D 系列 VM 以獲得最佳的磁碟和 CPU 效能)。
...
若要安裝 Azure PowerShell，請依照上一節**安裝 Azure PowerShell 並登入**中的步驟進行。跳過步驟 4：**登入 Azure PowerShell**。
...
3. 授與本機服務帳戶權限以使用自動化憑證。
    ...
    2.	在清單中尋找憑證。
    3.	以滑鼠右鍵按一下您的憑證，然後選擇 [所有工作] > [管理私密金鑰]。
    4.	選擇 [新增] 按鈕、輸入 [本機服務]，然後選擇 [檢查名稱]。
    5.	選擇 [確定] 按鈕，然後關閉憑證管理員。

![Windows 憑證管理員 UI 的圖片](media/service-fabric-set-up-continuous-integration/windows-certificate-manager.png)

### 註冊組建代理程式

1.	下載 agent.zip。作法：
    1.	登入您的 Team 專案，例如 ****https://[your-Team-Services-account-name].visualstudio.com**。
1.	選擇您的畫面右上角的「齒輪」圖示。
    ...
    1.	將 agent.zip 解壓縮至組建電腦上的 `C:\agent` (或具有簡短路徑的任何位置)。
        ...
        >[AZURE.NOTE]如果您打算建置 ASP.NET 5 Web 服務，建議您為此資料夾選擇可能的最短名稱，以避免在部署期間發生 **PathTooLongExceptions** 錯誤。

2.	從系統管理 PowerShell 命令提示字元中，執行 `C:\agent\ConfigureAgent.ps1`。指令碼會提示您下列參數：

    |參數|值|
    |---|---|
    |代理程式名稱|接受預設值，`Agent-[machine name]`。)
    |TFS Url|輸入您 Team 專案的 URL，例如 `https://[your-Team-Services-account-name].visualstudio.com`。
    |代理程式集區|輸入代理程式集區的名稱。(如果您尚未建立代理程式集區，接受預設值。)|
    |工作資料夾|接受預設值。這是組建代理程式實際建置您的應用程式的資料夾。注意：如果您打算建置 ASP.NET 5 Web 服務，建議您為此資料夾選擇可能的最短名稱，以避免在部署期間發生 PathTooLongExceptions 錯誤。|
    |安裝為 Windows 服務？|預設值為 N。將值變更為 Y。|
    |執行服務的使用者帳戶|接受預設值，`NT AUTHORITY\LocalService)`。|
    |取消設定現有的代理程式？|接受預設值，**N**。|

3. 系統將會提示您輸入認證。輸入具有您的小組專案權限的 Microsoft 帳戶的認證。
4. 請確認您的組建代理程式已註冊。作法：

    1. 返回網頁瀏覽器 (頁面應是 `https://[your-Team-Services-account-name].visualstudio.com/_admin/_AgentPool`)，然後重新整理該頁面。
    1. 選擇您稍早執行 ConfigureAgent.ps1 時選取的代理程式集區。
    1. 確認您的組建代理程式出現在清單中，並且具有綠色狀態反白顯示。若強調顯示是紅色，表示組建代理程式在連接到 Team Services 時發生問題。

![控制台的 [代理程式集區] 索引標籤](media/service-fabric-set-up-continuous-integration/vso-configured-agent.png)
...
3.	在 [觸發程序] 索引標籤中，選取 [連續整合] 和 [批次變更] 選項。
4.	在 [一般] 索引標籤上，選擇您註冊組建代理程式的佇列。
5.	儲存組建定義並且給予名稱。(您想要的話可以在稍後變更此名稱。)
...
如果前一個組建未在本身之後清除 (例如，如果組建已在可以清除之前取消)，現有資源群組可能會與新的資源群組發生衝突。為避免衝突，在建立新的資源群組之前，清除任何剩餘的資源群組 (和其相關聯的資源)：
...
現在您已經完成使用暫存叢集，您應該清除它。如果您不這麼做，將會繼續針對暫存叢集向您計費。這個程序會移除資源群組，如此會移除群組中的叢集與其他所有資源：

1.	在 [組建] 索引標籤中，選擇 [新增組建步驟…] 命令。
2.	選擇 [公用程式] > [PowerShell]。
3.	選擇組建步驟的名稱旁的鉛筆圖示，然後將其重新命名為 [移除叢集資源群組]。
4.	選擇 [指令碼檔案名稱] 旁邊的 [...] 按鈕。瀏覽至您解壓縮自動化指令碼的位置，然後選擇 **RemoveClusterResourceGroup.ps1**。
5.	針對 [引數]，輸入 `-ServicePrincipalPassword "$(ServicePrincipalPassword)`。
