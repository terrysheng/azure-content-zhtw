...
Service Fabric 是分散式系統平台，可讓您輕鬆封裝、部署及管理可調整和可信賴的微服務。

## 重要術語概念
本節詳細說明 Service Fabric 中所使用的術語，以利您了解文件中的其他處使用的詞彙。
...
**映像存放區**：每個 Service Fabric 叢集皆維護一個映像存放區。您必須將應用程式封裝的內容複製到映像存放區，然後在該應用程式封裝內註冊該應用程式類型。佈建應用程式類型後，您可以建立其具名應用程式。只有在刪除應用程式類型的所有具名應用程式之後，才可以從映像存放區取消註冊該應用程式類型。

如需有關部署至映像存放區的詳細資訊，請閱讀[部署應用程式](service-fabric-deploy-remove-applications.md)一文。
...
如需詳細資訊，請參閱[分割 Service Fabric Reliable Services](service-fabric-concepts-partitioning.md)。

**程式設計模型**：有兩種 .NET Framework 程式設計模型可讓您建置 Service Fabric 服務：
...
如需詳細資訊請參閱[為服務選擇程式設計模型](service-fabric-choose-framework.md)一文。
