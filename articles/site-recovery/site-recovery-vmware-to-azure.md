...
**公用網際網路：**內部部署伺服器 (處理序伺服器、受保護的伺服器) 與 Azure 基礎結構元件伺服器 (設定伺服器、主要目標伺服器) 之間的通訊和資料複寫，會透過安全 SSL/TLS 連線從內部部署對設定伺服器與主要目標伺服器上的公用端點進行。(唯一的例外狀況是 TCP 連接埠 9080 上處理序伺服器與主要目標伺服器之間的連線未加密。在此連接上只有與用來設定複寫相關的複寫通訊協定相關的控制資訊會加以交換。)

![部署圖表網際網路](./media/site-recovery-vmware-to-azure/ASRVmware_deploymentInternet.png)

**VPN：**內部部署伺服器 (處理序伺服器、受保護的伺服器) 與 Azure 基礎結構元件伺服器 (設定伺服器、主要目標伺服器) 之間的通訊和資料複寫，會透過 VPN 連線在內部部署網路與部署組態伺服器和主要目標伺服器所在的 Azure 虛擬網路之間進行。確定您的內部部署網路透過 ExpressRoute 連線或站對站 VPN 連線連接到 Azure 虛擬網路。
...
5. 在 **[區域]** 中，選取保存庫的地理區域。若要查看支援的區域，請參閱 [Azure Site Recovery 價格詳細資料](pricing/details/site-recovery/) (英文) 中的＜各區域上市情況＞。
...
5. 當您按一下 [**確定**]，會在您的訂用帳戶中為設定伺服器建立以 Azure Site Recovery Windows Server 2012 R2 資源庫映像為基礎的標準 A3 虛擬機器。它會建立為新的雲端服務中的第一個執行個體。如果您將網路連線類型指定為 [公用網際網路]，則會使用保留的公用 IP 位址建立雲端服務。您可以在 [**工作**] 索引標籤中監視進度。

	![監視進度](./media/site-recovery-vmware-to-azure/ASRVMWare_MonitorConfigServer.png)

6.  **僅在您的連線類型是 [公用網際網路] 時，此步驟才適用。** 部署設定伺服器之後，請在 Azure 入口網站的 [**虛擬機器**] 頁面中注意指派給此伺服器的公用 IP 位址。然後在 [**端點**] 索引標籤上注意對應至私人連接埠 443 的公用 HTTPS 連接埠。稍後當您在主要目標與處理序伺服器上註冊此設定伺服器的時候，您會需要這個資訊。這個設定伺服器利用這些端點進行部署：
...
1. 在 [**快速啟動**] 頁面上，按一下 [**準備目標資源**] > [**下載註冊金鑰**]。金鑰檔案會隨即自動產生。該金鑰在產生後會維持 5 天有效。將它複製到設定伺服器。
2. 在 [**虛擬機器**] 中，從虛擬機器清單選取設定伺服器。開啟 [**儀表板**] 索引標籤，然後按一下 [**連接**]。[**開啟**] 下載的 RDP 檔案以使用遠端桌面登入設定伺服器。如果設定伺服器是部署在 VPN 網路上，則使用設定伺服器的內部 IP 位址 (這是當您部署設定伺服器時指定的 IP 位址，並且也可在設定伺服器虛擬機器的虛擬機器儀表板頁面上看到)，以從您的內部部署網路進行遠端桌面到它。當您第一次登入時，Azure Site Recovery 設定伺服器安裝精靈會自動執行。
...
5. 在 [**網際網路設定**] 中，指定設定伺服器將連線到網際網路的方式。請注意：
...
8. 如果您在執行 Linux：
	...
	3. 執行下列項目以從執行了 gzip 的安裝程式解壓縮檔案：**tar –xvzf Microsoft-ASR\_UA\_8.4.0.0\_RHEL6-64**\* ![Linux 主要目標伺服器](./media/site-recovery-vmware-to-azure/ASRVMWare_TSLinuxTar.png)
...
4. 在 [**變更處理序伺服器**] 對話方塊中，於 [**目標處理序伺服器**] 中選取新伺服器，然後選取您想要複寫到新伺服器的虛擬機器。按一下伺服器名稱旁的資訊圖示，以取得其相關資訊，包括可用空間、使用的記憶體。將每個選取的虛擬機器複寫到新的處理序伺服器所需的平均空間就會顯示，以協助您進行負載的決策。![變更處理序伺服器 2](./media/site-recovery-vmware-to-azure/ASRVMware_ChangePS2.png)
