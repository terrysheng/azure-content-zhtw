...
5. 在 [**伺服器**] 中，選取 [**新增 SQL Database 伺服器**]。在其他欄位中使用預設值。
...
7. 您現在可以將此值貼到程式碼檔案中，以建立下列格式的連接字串。以您在上一個步驟中所複製的文字取代 *your_server* (共有兩個位置)，並以在建立 SQL Database 帳戶時所指定的密碼值取代 *your_password*。(而如果您未使用 **gettingstarted** 和 **MySQLAdmin**，則分別取代指派給 **database=** 和 **user=** 的值。) 
...
3. 將 **Microsoft JDBC Driver for SQL Server** 新增至您的組建路徑。
   ...
   如果使用 Eclipse：
    ...
    2. 在 [**內容**] 對話方塊的左窗格中，按一下 [**Java 組建路徑**]。
    3. 按一下 [**程式庫**] 索引標籤，然後按一下 [**新增程式庫**]。
    4. 在 [**新增程式庫**] 對話方塊中選取 [**Microsoft JDBC Driver 4.0 for SQL Server**]，按 [**下一步**]，然後按一下 [**完成**]。
    5. 按一下 [**確定**] 以關閉 [**內容**] 對話方塊。

    如果不是使用 Eclipse，則將 Microsoft JDBC Driver 4.0 for SQL Server JAR 新增至您的類別路徑。如需相關資訊，請參閱 [使用 JDBC 驅動程式](http://msdn.microsoft.com/library/ms378526.aspx)。
...
下列程式碼說明如何使用 **PersonID** 資料行，在 **Person** 資料表上建立名為 **index1** 的索引。
...
3. 在 [**新增組件指示詞**] 對話方塊中，按一下 [**Java 組建路徑項目**]，然後按 [**下一步**]。
4. 選取 [**Microsoft JDBC Driver 4.0 SQL Server**]，然後按一下 [**完成**]。
5. 按一下 [**確定**] 以關閉 [**內容**] 對話方塊。
6. 將專案的 WAR 檔案匯出至您的 approot 資料夾，並根據[使用 Azure Plugin for Eclipse with Java 建立 Hello World 應用程式 (由 Microsoft Open Technologies 提供)](http://msdn.microsoft.com/library/windowsazure/hh690944.aspx) 中所記載的步驟重建 Azure 專案。該主題也說明如何在運算模擬器和 Azure 中執行應用程式。
...
若要深入了解 Microsoft JDBC Driver for SQL Server，請參閱 [JDBC 驅動程式概觀](http://msdn.microsoft.com/library/ms378749.aspx)。若要深入了解 SQL Database，請參閱 [SQL Database 概觀](http://msdn.microsoft.com/library/windowsazure/ee336241.aspx)。
