...
- **控制節點**：以任何開發、載入或商業智慧工具搭配使用 SQL 資料倉儲時連接至控制項節點。在 SQL 資料倉儲中，計算節點是 SQL Database，當連接時它的外觀像是標準 SQL Database。不過，在介面底下，它會協調系統中發生的所有資料移動和計算。當命令發出至控制節點時，它會將其細分為一組查詢，傳遞至服務的計算節點。
...
- **資料移動服務**：SQL 資料倉儲中將所有項目融會在一起的最後一塊拼圖是我們的資料移動服務。資料移動服務可讓控制節點進行通訊並將資料傳遞給所有計算節點。這可讓計算節點在彼此之間傳遞資料，給予它們其他計算節點上的資料的存取權，並且讓它們取得完成聯結和彙總所需的資料。
...
## 可調整
SQL 資料倉儲的架構引進分隔的儲存體和計算功能，可單獨進行調整。SQL Database 快速又簡單的部署結構允許即時的額外計算。輔助這項功能就是使用 Azure 儲存體 Blob。Blob 不只提供穩定、複寫儲存體，同時也提供低成本的輕鬆擴充基礎結構。SQL 資料倉儲運用雲端規模儲存體和 Azure 計算的組合，讓您只在需要時支付查詢效能儲存體的費用。變更計算數量就像是在 Azure 入口網站中將滑桿向左或向右移動一樣簡單，但是也可以使用 T-SQL 和 PowerShell 排定或新增至工作負載。
