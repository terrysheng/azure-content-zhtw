...
Azure SQL 資料倉儲是一種企業級分散式資料庫，可處理大量的關聯式與非關聯式資料。它是業界首見的雲端資料倉儲，結合經實證的 SQL 功能，能在幾秒內增加、縮減和暫停。SQL 資料倉儲也深植在 Azure 中，而且數秒之內即可輕鬆部署。除此之外，此服務完全受管理，免除您花時間修補和維護軟體及備份的麻煩。SQL 資料倉儲的自動、內建備份可支援容錯功能和自助式還原。

在建立 SQL 資料倉儲時，我們著重於幾個關鍵屬性，以確保我們完全利用 Azure，並且建立可以符合任何企業工作負載的資料倉儲。

## 最佳化

### 資料倉儲架構

SQL 資料倉儲的核心使用 Microsoft 的巨量平行處理 (MPP) 架構，設計初衷是要執行一些最大的內部部署企業資料倉儲。此架構會利用內建資料倉儲效能改進，同時也可讓 SQL 資料倉儲輕鬆地向外延展和平行處理複雜的 SQL 查詢計算。此外，SQL 資料倉儲的架構被設計為利用在 Azure 中的目前狀態。結合這兩個層面，架構會分成 4 個主要元件：

![SQL 資料倉儲架構][1]

- **控制節點：**以任何開發、載入或商業智慧工具，搭配使用 SQL 資料倉儲時連接至控制節點。在 SQL 資料倉儲中，控制節點是 SQL Database，而且連接它的外觀及操作就像是標準的 SQL Database。不過，在介面底下，它會協調系統中發生的所有資料移動和計算。當命令發出至控制節點時，它會將其細分為一組查詢，傳遞至服務的計算節點。
...
- 將叢集資料行存放區索引包含至 SQL 資料倉儲也是達成快速查詢效能的關鍵。與傳統的資料列導向儲存體相比，使用資料行儲存體的 SQL 資料倉儲最多可讓壓縮速度提升 5 倍，查詢效能提升 10 倍。資料倉儲查詢適合使用資料行存放區索引，因為該查詢通常會掃描整個資料表或資料表的整個分割，並且將查詢步驟的移動資料影響降至最低。
