<properties 
	pageTitle="開始使用 Azure SQL Database Elastic Scale" 
	description="Azure SQL Database Elastic Scale 功能的基本解說，包括易於執行的範例應用程式。" 
...
範例會建立簡易的分區化應用程式，並探討 Elastic Scale 的主要功能。若要下載並執行應用程式，請遵循下方或視訊 [Elastic Scale - 開始使用](http://go.microsoft.com/?linkid=9862983) 中顯示的步驟。 
...
* 請參閱 [開始使用 Microsoft Azure SQL Database](http://azure.microsoft.com/documentation/articles/sql-database-get-started/)。

##下載及執行範例應用程式
[**Azure SQL Database Elastic Scale - 開始使用**] 範例應用程式會解說使用 Azure SQL DB Elastic Scale 開發分區化應用程式時最重要的使用經驗層面。範例應用程式著重在 [分區對應管理](http://go.microsoft.com/?linkid=9862595)、[資料相依路由](http://go.microsoft.com/?linkid=9862596) 和 [多分區查詢](http://go.microsoft.com/?linkid=9862597) 的主要使用案例。若要下載及執行範例，請遵循下列步驟： 
...
3. 然後，按一下 [**範例**] 下的 [**Visual C#**]。
    ...
    ![Click Visual C#][3]
4. 在搜尋方塊中，輸入 **Elastic Scale** 以搜尋範例。此時會出現「**Azure SQL Database Elastic Scale - 開始使用**」標題。
...
**注意**   如果您沒有 SQL Server Management Studio，請參閱 [使用 SQL Server Management Studio 管理 Azure SQL Database](http://azure.microsoft.com/documentation/articles/sql-database-manage-azure-ssms/)，其中包含取得此工具的指示。  
...
2. **合併現有分區**︰分區合併也是使用 [**分割/合併服務**] 來執行的。如需詳細資訊，請參閱：[分割/合併服務](http://go.microsoft.com/?linkid=9862795)。   
