<properties title="Using Elastic Scale with Entity Framework" pageTitle="使用 Elastic Scale 與 Entity Framework" description="Elastic Scale makes it easy to scale, Entity Framework is easy to use for coding databases " metaKeywords="Using Elastic Scale with Entity Framework, Azure SQL Database sharding, elastic scale, Entity Framework and Elastic Scale" services="sql-database" documentationCenter="" manager="jhubbard" authors="sidneyh@microsoft.com"/>
...
* 在 [新增專案] 對話方塊中，瀏覽至 **Visual C#**] 的 [**線上範例**]，然後在右上方的搜尋方塊中輸入 "elastic scale"。
...
下列指導方針指示如何使用 Azure SQL Database Elastic Scale，以滿足 Code First 應用程式的這些需求。 
...
* 資料庫已經存在，而且已在 Elastic Scale 分區對應中註冊。 
* 應用程式的結構描述已經部署到資料庫 (如下所述)。 
* 由 Elastic Scale 分區對應代理資料庫的資料相依路由連接。 

整合 **DbContexts** 與資料相依路由以相應放大：

1. 透過分區對應管理員的 Elastic Scale 介面，建立實體資料庫連接， 
...
* 基底類別建構函式的呼叫會繞道至靜態方法，以執行資料相依路由所需的所有步驟。 
    * 它會在分區對應上使用 Elastic Scale 介面的 **OpenConnectionForKey** 呼叫，以建立開啟的連接。 
...
透過 EF 移轉的結構描述部署在**未開啟的連接**上表現最好。這與資料相依路由的情況相反，它依賴 Elastic Scale 所提供的已開啟連接。另一個差異是一致性需求：雖然希望確保所有資料相依路由連接的一致性，以防止並行分區對應操作，但這無關於最初將結構描述部署到尚未在分區對應中註冊且尚未配置來保留 Shardlet 的新資料庫。因此，我們在此案例中可依賴一般資料庫連接，而不是資料相依路由。  

這會形成一種方法，使得透過 EF 移轉的結構描述部署，與新的資料庫在應用程式的分區對應中註冊為分區緊密相關。這依賴下列必要條件： 

* 已建立資料庫。 
* 資料庫是空 - 不含任何使用者結構描述與任何使用者資料。
* 還不能透過資料相依路由的 Elastic Scale API 存取資料庫。 
...
這個範例示範 **RegisterNewShard** 方法，此方法會在 Elastic Scale 中註冊分區、透過 EF 移轉部署結構描述，並將分區化索引鍵的對應儲存至分區。它依賴 **DbContext** 子類別 (範例中的 **ElasticScaleContext**) 的建構函式 (接受 SQL 連接字串做為輸入)。這個建構函式的程式碼簡單易懂，如下列範例所示： 
...
* 使用 **LocalDb** 的 EF 應用程式必須先移轉至一般 SQL Server 資料庫，才能使用 Elastic Scale。使用**LocalDb** 時，無法透過分區化與 Elastic Scale 來相應放大應用程式。請注意，仍可使用 **LocalDb** 來開發。 
...
* 提出要求時，將會假定所有資料庫處理都包含在要求所提供的分區化索引鍵所識別的單一分區內。不過，這項假設不一定永遠成立。例如，有時無法提供分區化索引鍵。為了解決這個問題，Elastic Scale 程式庫提供 **MultiShardQuery** 類別，此類別實作連接抽象來查詢數個分區。學習搭配 EF 來使用 **MultiShardQuery** 已超出本文的範圍
