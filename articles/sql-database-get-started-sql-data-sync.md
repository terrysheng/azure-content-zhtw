...
遵循[安裝 SQL 資料同步 (預覽) 用戶端代理程式](http://msdn.microsoft.com/en-us/library/jj823137.aspx)中的指示，以安裝代理程式。
...
安裝用戶端代理程式之後，註冊您要納入代理程式之同步群組中的每個內部部署 SQL Server 資料庫。若要向代理程式註冊資料庫，請遵循[向用戶端代理程式註冊 SQL Server 資料庫](http://msdn.microsoft.com/en-us/library/jj823138.aspx)中的指示。
