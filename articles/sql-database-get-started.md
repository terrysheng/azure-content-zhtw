...
若要查看代管您剛才建立的 **School** 資料庫的伺服器名稱，請依序按一下左側瀏覽窗格的 [**SQL Databases**]，以及 [**SQL Databases**] 清單檢視中的 **School** 資料庫。在 [**快速啟動**] 頁面中，向下捲動即可查看伺服器名稱。
...
**注意：** 此結構描述和資料值取自 [MSDN 文章](http://msdn.microsoft.com/zh-tw/library/windowsazure/ee621790.aspx "MSDN article")，且已為和 SQL Database 搭配使用而進行修改。
...
4. SQL Database 管理入口網站會在新的瀏覽器視窗中開啟，您即可連線至 [**主要**]。
...
<h2 id="ClientConnection">步驟 9：從其他應用程式連線</h2>
...
3.  在下一個頁面中選擇 **School** 資料庫，然後選擇 **Person**。按一下 [**完成**]。如果系統提示您輸入登入詳細資料，請在輸入後按一下 [**確定**]。

4. [匯入資料] 對話方塊會隨即出現，並提示您選取要匯入資料的方式和位置。在選取預設選項的情況下，按一下 [**確定**]。

	![Navigation pane][Image19]

5. 在工作表中，您應會在 Person 資料表中看到 34 列的結果集，包括 PersonID、LastName、FirstName、HireDate 以及 EnrollmentDate，如同步驟 7 的查詢結果一般。 
