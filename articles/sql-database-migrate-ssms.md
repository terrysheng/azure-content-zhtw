...
![替代文字](./media/sql-database-migrate-ssms/01SSMSDiagram.png)
...
2. 在 SSMS 物件總管中找出來源資料庫，然後執行 [部署資料庫至 Windows Azure SQL Database...] 工作。

	![替代文字](./media/sql-database-migrate-ssms/02MigrateusingSSMS.png)

3.	在部署精靈中，設定與上述步驟中佈建之目標 Azure SQL Database 的連接。
4.	輸入資料庫 [名稱]，並設定 [版本] (服務層) 和 [服務目標] (效能層級)。如需關於進行這些設定的詳細資訊，請參閱＜選擇資料庫效能層級/定價層以進行移轉＞。 

	![替代文字](./media/sql-database-migrate-ssms/03MigrateusingSSMS.png)
...
2.	在 SSMS 物件總管中找出來源資料庫，然後選取 [部署資料庫至 Windows Azure SQL Database...] 工作。

	![替代文字](./media/sql-database-migrate-ssms/04MigrateusingSSMS.png)
...
3. 在匯出精靈中設定匯出，將 BACPAC 檔案儲存在本機上。匯出的 BACPAC 一律會包含完整的資料庫結構描述，以及預設之所有資料表的資料。如果您想要排除部分或所有資料表的資料，請使用 [進階] 索引標籤。比方說，您可能會選擇只匯出參考資料表的資料。
	...
	![替代文字](./media/sql-database-migrate-ssms/05MigrateusingSSMS.png)

4.	一旦建立 BACPAC 檔案後，請連接至步驟 1 中建立的伺服器，以滑鼠右鍵按一下 [資料庫] 資料夾，然後選取 [匯入資料層應用程式...]。

	![替代文字](./media/sql-database-migrate-ssms/06MigrateusingSSMS.png)

5.	在匯入精靈中，選取剛才匯出的 BACPAC 檔案，即可在 Azure SQL Database 中建立新的資料庫 。

	![替代文字](./media/sql-database-migrate-ssms/07MigrateusingSSMS.png)

6.	輸入資料庫名稱，然後設定 [版本] (服務層) 和 [服務目標] (效能層級)。
	 
7.	完成精靈以匯入 BACPAC 檔案，然後在 Azure SQL Database 中建立資料庫。

	![替代文字](./media/sql-database-migrate-ssms/08MigrateusingSSMS.png)
 
##替代項目
您也可以使用命令列公用程式 sqlpackage.exe 部署資料庫或匯出和匯入 BACPAC。Sqlpackage.exe 和 SSMS 同樣使用 DAC 技術，因此結果會是一樣的。如需詳細資訊，請移至[這裡](https://msdn.microsoft.com/library/hh550080.aspx)。
