...
##<a name="platform"></a>Microsoft 資料平台
...
- **Azure VM 中執行的 SQL Server** 已經過最佳化，可在混合式案例中將現有的內部部署 SQL Server 應用程式延伸到 Azure，或在移轉案例或開發/測試案例中將現有的應用程式部署到 Azure。其中一個混合式案例是透過 [Azure 虛擬網路](http://msdn.microsoft.com/library/azure/jj156007.aspx)，在 Azure 中保留一份次要資料庫複本。有了 Azure VM 中的 SQL server，您即擁有專用 SQL Server 執行個體和雲端架構 VM 的完整系統管理權限。當組織擁有可用來維護虛擬機器的 IT 資源時，此選項會是最佳選擇。有了 VM 中的 SQL Server，您可以建立高度自訂的系統，以解決應用程式的特定效能和可用性需求。
...
**Azure SQL Database** 會以服務 (而非授權) 的形式銷售給客戶，而 Azure VM 中的 SQL Server 則需要傳統的 SQL Server 授權。
...
**重要事項：**目前，Azure SQL Database 不支援 SQL Server 的所有功能。如需詳細的比較資訊，請參閱 [Azure SQL Database 方針和限制](http://msdn.microsoft.com/library/azure/ff394102.aspx)。當您因為需要額外預算來因應資料庫重新設計計劃，而想要將現有資料庫移至 Azure SQL Database 時，請別忘了上面說的這點。Azure SQL Database 是 Microsoft 的平台即服務產品。當您將現有內部部署 SQL Server 應用程式移轉至 Azure SQL Database 時，建議您更新應用程式，以便能夠利用平台即服務產品的所有優點。例如，開始在應用程式層上使用 [Azure 網站](http://azure.microsoft.com/documentation/services/websites/)或 [Azure 雲端服務](http://azure.microsoft.com/services/cloud-services/)，以增加成本效益。此外，請針對不同的 Azure SQL Database 服務層驗證您的應用程式，看看哪一個最符合您的應用程式需求。此程序可協助您達到更好的效能結果並將成本降到最低。如需詳細資訊，請參閱 [Azure SQL Database 服務層和效能層級](http://msdn.microsoft.com/library/azure/dn741336.aspx)。
...
相反地，您可能有內部專業人員，並想保持只由機器本身控制資料庫位置。有了 **Azure VM 中執行的 SQL Server**，您可以完全掌控作業系統和 SQL Server 執行個體組態。有了 VM，您可以決定何時更新/升級作業系統與資料庫軟體，以及何時安裝任何其他軟體 (例如防毒和備份工具)。此外，您還可以控制 VM 的大小、磁碟的數目及其儲存體組態。例如，Azure 可讓您視需要變更正在執行的 VM 大小。如需詳細資訊，請參閱 [Azure 的虛擬機器和雲端服務大小](http://msdn.microsoft.com/library/azure/dn197896.aspx)。
