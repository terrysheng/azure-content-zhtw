...
現有的 **Azure SQL Database 伺服器**。若要建立伺服器，您必須先建立至少一個資料庫。當您建立資料庫時，您可以選擇在新的伺服器或現有伺服器上建立資料庫。如需有關建立新資料庫 (在新伺服器上) 的指示，請參閱[建立您的第一個 Azure SQL Database](sql-database-get-started.md)。
