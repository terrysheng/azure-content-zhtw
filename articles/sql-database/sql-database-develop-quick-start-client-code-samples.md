<properties 
	pageTitle="SQL Database 的用戶端開發和快速入門程式碼範例" 
	description="列出用戶端程式可用於連接到 Azure SQL Database 或 Microsoft SQL Server 之每個驅動程式的最低版本號碼。所提供的驅動程式相關版本資訊連結，是由社群而不是由 Microsoft 所發行。"
...
- 各種程式設計語言。
- 您的用戶端程式碼執行所在的 Windows 和 Linux 作業系統。
- 簡短的快速入門程式碼範例，以及處理自動重試邏輯之暫時性錯誤的較長範例。
- 將關聯式結果集轉換成物件導向格式的程式碼範例。


> [AZURE.NOTE]截至 2015 年 5 月 19 日為止，將會準備更多語言版本的程式碼範例，並將在本主題中新增這些範例的連結。
