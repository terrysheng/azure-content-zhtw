...
> [AZURE.NOTE]當您選取 [加入集區] 命令時，必須選取 [預覽版條款]，並完成 [預覽版條款] 刀鋒視窗，以接受預覽版的條款。各個訂用帳戶只需要接受條款一次。
...
SQL Database 服務會評估使用量歷程記錄，並在比使用單一資料庫更符合成本效益時，建議您使用一或多個彈性資料庫集區。星號 (![星號][10]) 代表根據您的資料庫工作負載而建議的價格層。
...
| **eDTU MIN** - 每個資料庫的 eDTU 保證 | 每個資料庫的 eDTU 保證是集區中單一資料庫能夠保證的 eDTU 數。例如，在 Standard 集區中您可以將這項保證設定為 0、10、20、50 或 100 個 eDTU，或者您可以選擇不提供保證給群組中的資料庫 (eDTU MIN = 0)。<br> **我應該將每個資料庫的 eDTU 保證設定為？** <br>通常，每個資料庫的 eDTU 保證 (eDTU MIN) 設為 0 和 ([每個資料庫的平均使用量]) 之間的任何數量。每個資料庫的 eDTU 保證是全域設定，會設定集區中所有資料庫的 eDTU 保證。 |
