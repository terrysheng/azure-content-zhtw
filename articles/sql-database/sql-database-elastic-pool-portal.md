...
# 建立和管理 Azure SQL Database 彈性資料庫集區
...
本文將說明如何使用 [Azure Preview 入口網站](https://portal.azure.com)建立彈性資料庫集區。
...
- Azure SQL Database V12 伺服器。如果沒有 V12 伺服器，請遵循這篇文章中的步驟建立一個：[建立您的第一個 Azure SQL Database](sql-database-get-started.md)，或[將現有伺服器升級至 V12](sql-database-v12-upgrade.md)。
...
2.	選取 [SQL Server] 刀鋒視窗頂端的 [新增集區]，以建立集區。
    ...
    如果您看到訊息說明此伺服器有建議的集區，請按一下該集區，即可輕鬆地檢閱並建立最適合您伺服器資料庫的集區。如需詳細資訊，請參閱[建議的彈性資料庫集區](sql-database-elastic-pool-portal.md#recommended-elastic-database-pools)。
...
> [AZURE.NOTE]當您選取 [新增集區] 命令時，必須選取 [預覽條款]，並完成 [預覽條款] 刀鋒視窗，以接受預覽版的條款。各個訂用帳戶只需要接受條款一次。
...
- 選取 [設定集區] 以變更集區 eDTU 設定和每個資料庫的 eDTU 設定。
- 選取 [建立工作]，然後建立彈性工作以管理集區中的資料庫。彈性工作有助於對集區中任意數目的資料庫執行 Transact-SQL 指令碼。如需詳細資訊，請參閱[彈性資料庫工作概觀](sql-database-elastic-jobs-overview.md)。
- 選取 [管理工作] 以管理現有的彈性工作。
