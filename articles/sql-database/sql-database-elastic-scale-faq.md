...
# Azure SQL Database 彈性延展常見問題集 

####如果我的每一分區各有單一租用戶，卻沒有分區化索引鍵，我要如何填入結構描述資訊的分區化索引鍵？
結構描述資訊物件只是用來分割合併案例。如果應用程式原本就是單一租用戶，則其並不需要分割合併服務，因此就不需要填入結構描述資訊物件。

#### 我已佈建資料庫並已擁有分區對應管理員，要如何將這個新資料庫註冊為分區？
請參閱 **[新增分區至彈性延展應用程式](http://azure.microsoft.com/documentation/articles/sql-database-elastic-scale-add-a-shard/)**。 

#### 彈性延展需要多少成本？
使用彈性延展程式庫不會產生任何成本。只有您使用於分區和分區對應管理員的 Azure SQL Database 和您為分割合併服務佈建的 Web/背景工作角色才會產生成本。
...
可以，分區是個別的資料庫，因此可以有一個分區是「高階」版，另一個是「標準」版。此外，在分區存留期間，分區的版本可以多次擴大或縮小。

#### 分割合併服務會在分割或合併作業期間佈建 (或刪除) 資料庫嗎？ 
