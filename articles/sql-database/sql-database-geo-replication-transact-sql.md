...
連接到 SQL Database 需要您知道在 Azure 上的伺服器名稱，並且已為您要使用 Management Studio 連接之用戶端的 IP 位址建立防火牆規則。您可能需要登入入口網站才能取得此資訊及完成此工作。

1.  登入 [Azure 管理入口網站](http://manage.windowsazure.com)。

2.  在左窗格中，按一下 [SQL Database]。

3.  在 SQL Databases 首頁上，按一下頁面頂端的 [伺服器]，以列出與訂用帳戶相關的所有伺服器。尋找想要連線的伺服器名稱，並將它複製到剪貼簿上。
...
2.  在 [連線到伺服器] 中，將完整伺服器名稱指定為 <MyLocalServer>.database.windows.net。在 Azure 上，伺服器名稱是由英數字元組成的自動產生字串。
...
3. 使用下列 **ALTER DATABASE** 陳述式，使異地複寫資料庫成為具有 <ElasticPool2> 中 <MySecondaryServer4> 上可讀取次要資料庫。
...
3. 使用下列 **ALTER DATABASE** 陳述式，使 <MyLocalDB> 成為具有 <ElasticPool2> 中 <MySecondaryServer4> 上可讀取次要資料庫的異地複寫主要複本。
