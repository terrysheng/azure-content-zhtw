...
請參閱 [SQL Database 服務層文章](sql-database-service-tiers.md)，以取得服務層和儲存體資訊。

使用 [In-Memory](sql-database-in-memory.md) 時，記憶體最佳化資料表中的資料和資料表變數位於 XTP 記憶體內部儲存體。每個進階服務層都有最大的記憶體內部儲存體大小。一旦超過此限制，插入和更新作業可能會開始出錯 (錯誤碼 41805)。屆時您將需要刪除資料以回收記憶體，或升級您的資料庫的效能層。
...
##### 進階服務層的最大 XTP 記憶體內部儲存體：



您可以在 Azure [入口網站](http://portal.azure.com/)監視記憶體內部儲存體使用量。
