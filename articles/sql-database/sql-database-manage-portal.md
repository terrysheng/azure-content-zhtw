...
[Azure 管理入口網站][Management Portal]可讓您建立、監視和管理 Azure SQL Database 與伺服器。本文會強調顯示可使用管理入口網站來完成的資料庫作業。
...
Azure SQL Database 可以設定為以非同步方式，將已認可的交易複寫至次要資料庫。在管理入口網站上的異地複寫部分，可讓您選取想要放置次要資料庫的 Azure 區域。
