...
如果資料庫結構描述已與 Azure SQL Database 相容，那麼您將能輕鬆進行移轉作業。如果不需要進行任何結構描述修改，您只需將資料庫匯入 Azure 即可進行移轉作業。這只需要一個步驟即可完成，亦即使用 SSMS 將資料庫「部署」至 Azure SQL Database；或者也可分為兩個步驟，先匯出目前資料庫的 BACPAC，然後再將 BACPAC 匯入 Azure 以建立新的 Azure SQL Database。

此外，您可以將匯出的 BACPAC 上傳至 Azure 儲存體，然後使用入口網站將 BACPAC 匯入為資料庫。在 Azure 中執行匯入作業可減少匯入過程中的延遲，藉此改善大型資料庫移轉作業的效能及可靠性。
...
3.	在部署精靈中，設定與您佈建之目標 Azure SQL Database 的連接。
4.	輸入資料庫 [名稱]，並設定 [版本] (服務層) 和 [服務目標] (效能層級)。如需設定這些設定的詳細資訊，請參閱 [Azure SQL Database 服務層](sql-database-service-tiers.md)。 

	![匯出設定](./media/sql-database-migrate-ssms/03MigrateusingSSMS.png)

5.	完成精靈以移轉資料庫。移轉時間取決資料庫部署的大小及複雜性，可能需要數分鐘到數小時。如果錯誤指出資料庫結構描述與 SQL Database 不相容，請使用不同的移轉選項。
