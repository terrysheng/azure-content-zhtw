...
以下範例示範如何使用 **SQLSRV** 和 **PDO_SQLSRV** 擴充功能連接名為 `testdb` 的 SQL Database。如需 **SQLSRV** 和 **PDO_SQLSRV** API 的相關資訊，請參閱 [Microsoft Drivers for PHP for SQL Server 文件][driver-docs]。您將需要從上述區段取得的資訊。將 `SERVER_ID` 取代為 10 位數的伺服器 ID (自上述區段取得之 SERVER 值的前 10 個字元)，然後將正確的值 (您的使用者名稱和密碼) 指派給 `$user` 和 `$pwd` 變數。
