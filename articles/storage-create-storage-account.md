...
您也可以設定與儲存體帳戶搭配使用的自訂網域名稱。如需詳細資料，請參閱[針對儲存體帳戶中的 Blob 資料設定自訂網域名稱](../storage-custom-domain-name/)。
...
> [AZURE.NOTE] 建議您避免將儲存體帳戶存取金鑰透露給其他任何人。若要允許在不出示存取金鑰的情況下存取儲存體資源，您可以使用「共用存取簽章」**。共用存取簽章可在您定義的間隔期間內，使用您所指定的權限，來存取帳戶中的資源。如需詳細資訊，請參閱[共用存取簽章教學課程](../storage-dotnet-shared-access-signature-part-1/)。
