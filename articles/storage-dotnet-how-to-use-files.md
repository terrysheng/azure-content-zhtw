...
-   **儲存體帳戶：**所有的 Azure 儲存體存取都可透過儲存體帳戶完成。如需有關儲存體帳戶容量的詳細資訊，請參閱 [Azure 儲存體延展性和效能目標](http://msdn.microsoft.com/library/azure/dn249410.aspx)。

-   **共用：**檔案儲存體共用是指在 Azure 中共用的 SMB 2.1 檔案。
    所有的目錄和檔案必須在上層共用中建立。帳戶可包含無限制數目的共用，而共用可儲存無限制數目的檔案，最多可達儲存體帳戶的容量限制。

-   **目錄：**選擇性的目錄階層。

-	**檔案：女**共用中的檔案。檔案的大小可能高達 1 TB。
...
若要準備使用 PowerShell，請下載並安裝 Azure PowerShell Cmdlet。請參閱 [如何安裝和設定 Azure PowerShell](install-configure-powershell.md) ，取得安裝點及安裝指示。
...
<li>檢視與檔案服務相關的 Azure 儲存體團隊部落格文章：
  ...
  <ul>
    ...
    <li>使用<a href="/documentation/articles/storage-dotnet-how-to-use-queues/">佇列儲存體</a>可靠地保管訊息。</li>
