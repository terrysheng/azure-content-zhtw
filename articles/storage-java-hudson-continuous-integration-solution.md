...
- **URL 格式**：可利用下列 URL 格式來定址 Blob：
    ...
    在上述格式中， `storageaccount` 代表您儲存體帳戶的名稱， `container_name` 代表您容器的名稱，而  `blob_name` 代表您 Blob 的名稱。容器名稱中可以有多個路徑，這些路徑彼此以正斜線 (**/**) 分隔。本教學課程中的範例容器名稱為 **MyJob**，使用的共同虛擬路徑是 **${BUILD\_ID}/${BUILD\_NUMBER}**，所產生之 Blob 的 URL 格式如下：
