<properties 
	pageTitle="管理 Azure 儲存體資源的存取" 
	description="了解您可用來管理 Azure 儲存體資源存取的不同方式。" 
	services="storage" 
	documentationCenter="" 
	authors="micurd" 
	manager="jahogg" 
...
#管理 Azure 儲存體資源的存取
根據預設，只有儲存體帳戶的擁有者可以存取該帳戶內的 Blob、資料表和佇列。如果您的服務或應用程式需要將這些資源提供給其他用戶端使用，而不共用存取金鑰，則您有下列選項來允許存取：
...
根據預設，可能只有儲存體帳戶的擁有者能存取容器及其內部的任何 Blob。如果您想要為匿名使用者授與容器及其 Blob 的讀取權限，您可以設定容器權限以允許公用存取。匿名使用者可以讀取可公開存取之容器內的 Blob，而不需驗證要求。
...
| REST 作業                                         | .NET 方法                                      | 具有完整公開讀取權限 | 僅對 Blob 有公開讀取權限 |
|--------------------------------------------------------|--------------------------------------------------|-----------------------------------------|---------------------------------------------------|
| 列出容器                                        | ListContainers                                   | 只有擁有者                              | 只有擁有者                                        |
| 建立容器                                       | 建立                                           | 只有擁有者                              | 只有擁有者                                        |
| 取得容器屬性                               | 屬性                                       | 全部                                     | 只有擁有者                                        |
| 取得容器中繼資料                                 | 中繼資料                                         | 全部                                     | 只有擁有者                                        |
| 設定容器中繼資料                                 | 中繼資料                                         | 只有擁有者                              | 只有擁有者                                        |
| 取得容器 ACL                                      | BlobContainerPermissions                         | 只有擁有者                              | 只有擁有者                                        |
| 設定容器 ACL                                      | BlobContainerPermissions                         | 只有擁有者                              | 只有擁有者                                        |
| 刪除容器                                       | 刪除                                           | 只有擁有者                              | 只有擁有者                                        |
| 列出 Blob                                             | ListBlobs                                        | 全部                                     | 只有擁有者                                        |
| 放置 Blob                                               | 建立 CloudPageBlob 或 CloudBlockBlob 的方法 | 只有擁有者                              | 只有擁有者                                        |
| 取得 Blob                                               | DownloadByteArray 等項目                     | 全部                                     | 全部                                               |
| 取得 Blob 屬性                                    | 屬性                                       | 全部                                     | 全部                                               |
| 設定 Blob 屬性                                    | SetProperties                                    | 只有擁有者                              | 只有擁有者                                        |
| 取得 Blob 中繼資料                                      | 中繼資料                                         | 全部                                     | 全部                                               |
| 設定 Blob 中繼資料                                      | 中繼資料                                         | 只有擁有者                              | 只有擁有者                                        |
| 放置區塊                                              | PutBlock                                         | 只有擁有者                              | 只有擁有者                                        |
| 取得區塊清單 (僅限認可區塊)                 | DownloadBlockList                                | 全部                                     | 全部                                               |
| 取得區塊清單 (僅限未認可的區塊或所有區塊) | DownloadBlockList                                | 只有擁有者                              | 只有擁有者                                        |
| 放置區塊清單                                         | PutBlockList                                     | 只有擁有者                              | 只有擁有者                                        |
| 刪除 Blob                                            | 刪除                                           | 只有擁有者                              | 只有擁有者                                        |
| 複製 Blob                                              | CopyFromBlob                                     | 只有擁有者                              | 只有擁有者                                        |
| 快照集 Blob                                          | CreateSnapshot                                   | 只有擁有者                              | 只有擁有者                                        |
| 租用 Blob                                             | 租用                                            | 只有擁有者                              | 只有擁有者                                        |
| 放置頁面                                               | WritePages                                       | 只有擁有者                              | 只有擁有者                                        |
| 取得頁面範圍                                        | GetPageRanges                                    | 全部                                     |                                                   |

##建立和使用共用存取簽章
共用存取簽章是一個 URI，可針對特定的時間間隔，授與容器、Blob、佇列和資料表有限的存取權限。藉由為用戶端提供共用存取簽章，您可以讓他們存取您儲存體帳戶中的資源，而不需與他們共用您的帳戶金鑰。

>[AZURE.NOTE]如需共用存取簽章的深入概念性概觀和教學課程，請參閱[共用存取簽章](http://azure.microsoft.com/documentation/articles/storage-dotnet-shared-access-signature-part-1/)。
