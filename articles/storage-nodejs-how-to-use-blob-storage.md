...
**BlobService** 物件讓您能使用容器及 blob。下列
程式碼會建立 **BlobService** 物件。將下列內容新增至
接近 **server.js**的頂端：
...
如果建立容器，`result` 為 true。如果容器已存在，`result` 為 false。`response` 將包含操作的相關資訊，包括容器的 [ETag](http://en.wikipedia.org/wiki/HTTP_ETag) 資訊。
...
> [WACOM.NOTE] Page blobs consist of 512-byte 'pages'.如果上傳的資料大小不是 512 的倍數，可能會發生錯誤。
