<properties 
	pageTitle="如何使用 Node.js 的 Blob 儲存體 | Microsoft Azure" 
...
2.  在命令視窗中輸入 **npm install azure-storage**，該命令應能產生
    下列輸出：
...
azure 模組會讀取環境變數 `AZURE_STORAGE_ACCOUNT` 及 `AZURE_STORAGE_ACCESS_KEY` 或 `AZURE_STORAGE_CONNECTION_STRING`，以取得連接 Azure 儲存體帳戶所需的資訊。如果未設定這些環境變數，則呼叫 **createBlobService** 時必須指定帳戶資訊。
...
> [AZURE.NOTE] 您可以使用 **createBlobServiceAnonymous** 並提供主機位址，以匿名存取 Blob。例如， `var blobSvc = azure.createBlobServiceAnonymous('https://myblob.blob.core.windows.net/');`.
...
如果建立容器，則 `result` 為 true。如果容器已存在，則 `result` 為 false。 `response` 將包含操作的相關資訊，包括容器的 [ETag](http://en.wikipedia.org/wiki/HTTP_ETag) 資訊。
...
> [AZURE.NOTE] 分頁 Blob 由 512 位元組的 'pages' 組成。如果上傳的資料大小不是 512 的倍數，可能會發生錯誤。
...
這個 `result` 將包括 `entries` 集合，這是描述每個 Blob 的物件陣列。若無法傳回所有 Blob，則 `result` 也會提供 `continuationToken`，可作為第二個參數來擷取更多項目。
...
這個 `result` 會包含 Blob 的相關資訊，包括 **ETag** 資訊。
...
**myblob** 的後續操作必須提供 `options.leaseId` 參數。租用識別碼會從 **acquireLease** 作為 `result.id` 傳回 。
...
-   請參閱 [Azure Storage SDK for Node API 參考][]
