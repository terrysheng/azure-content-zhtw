...
。這些範例使用
Node.js API 撰寫。所涵蓋的案例包括**「建立和刪除
資料表」以及「在資料表中插入及查詢實體」**。如需
資料表的詳細資訊，請參閱[後續步驟][]一節。
...
在命令視窗中2.  輸入 **npm install azure-storage**，該命令應能產生以下輸出：
...
可以將選用的篩選作業套用到使用 **TableService** 執行的作業。篩選作業可包括記錄、自動重試等等。篩選器是以簽章實作方法的物件：
...
若要查詢資料表，請使用 **TableQuery** 物件並利用下列子句來
建置查詢運算式：
...
若成功，`result.entries` 將包含符合查詢的實體陣列。若查詢無法傳回所有實體，`result.continuationToken` 可作為 **queryEntities** 的第三個參數來擷取更多結果。在初始查詢中，第二個參數應為 *null*。

###如何查詢實體屬性的子集

一項資料表查詢可以只擷取實體的少數欄位。
這可減少頻寬並提高查詢效能 (尤其是對大型實體而言)。請使用 **select** 子句並傳遞要傳回的欄位名稱。例如，下列查詢只會傳回 **description** 例如，下列查詢只會傳回 **dueDate** 欄位。
...
您可以使用實體的資料分割和資料列索引鍵來刪除實體。在這個
範例中，**task1** 物件包含要刪除之實體的 **RowKey** 和 
**PartitionKey** 值。然後物件會
傳給 **deleteEntity** 方法。
...
-   請造訪 GitHub 上的 [Azure Storage SDK for Node][] 儲存機制 (英文)。
  ...
  [OData.org]: http://www.odata.org/ (英文)
