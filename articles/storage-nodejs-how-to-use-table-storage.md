...
本指南將示範如何使用 Azure 資料表服務執行一般案例。這些範例使用 Node.js API 撰寫。涵蓋的案例包括**「建立和刪除資料表」、「在資料表中插入及查詢實體」**。如需資料表的詳細資訊，請參閱[後續步驟][]一節。
...
以下是定義實體的範例。請注意，**dueDate** 定義為 **Edm.DateTime** 的類型。可選擇是否指定型別，若未指定，則會推斷型別。
...
> [WACOM.NOTE] 每一筆記錄也都有 **Timestamp** 欄位，這是在插入或更新實體時由 Azure 設定。
...
如果作業成功，`result` 將會包含插入記錄的 [ETag](http://en.wikipedia.org/wiki/HTTP_ETag) ，而 `response` 會包含作業的相關資訊。
...
> [WACOM.NOTE] 依預設，更新實體並不會檢查正在更新的資料先前是否被另一個程序修改過。若要支援並行更新：
> 
> 1.取得正在更新之物件的 ETag。這會放在任何實體相關作業的 `response` 中傳回，可透過 `response['.metadata'].etag` 擷取。
> 
...
使用 **updateEntity** 和 **mergeEntity** 時，如果正在更新的實體不存在，則更新作業會失敗。因此，如果您要儲存一個實體，而不管它是否已存在，您應該改用 **insertOrReplaceEntity** 或 **insertOrMergeEntity**。

在 `result` 更新作業成功時將包含已更新之實體的 **Etag**。
...
批次作業成功時，`result` 將包含批次中每個操作的相關資訊。

###處理批次操作

若要檢查新增至批次的作業，您可以檢視 `operations` 屬性。您也可以使用下列方法來處理操作。

* **clear** - 清除批次中的所有作業。

* **getOperations** - 從批次取得作業。

* **hasOperations** - 若批次包含作業，則傳回 true。
...
當此作業完成時，`result` 將包含實體。

## <a name="query-set-entities"> </a>如何查詢一組實體

若要查詢資料表，請使用 **TableQuery** 物件和下列子句建置查詢
運算式：
...
由於未使用 **select**，所以會傳回所有欄位。若要對資料表執行查詢，請使用 **queryEntities**。下列範例使用此查詢從 'mytable' 傳回實體。
...
一項資料表查詢可以只擷取實體的少數欄位。
這可以減少頻寬並提高查詢效能 (尤其是對大型實體而言)。使用 **select** 子句並傳遞要傳回的欄位名稱。例如，下列查詢只會傳回 **description** 和 **dueDate** 欄位。
...
用戶端應用程式接著會以 **TableServiceWithSAS** 使用 SAS，對資料表執行操作。下列範例會連線到資料表並執行查詢。
...
下列範例會取得 **hometasks** 資料表的目前 ACL，然後使用 **setTableAcl** 新增原則。此方法允許：
