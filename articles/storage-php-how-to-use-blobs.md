...
本指南將示範如何使用 Azure Blob 服務執行一般案例。這些範例均是以 PHP 撰寫，並使用 [Azure SDK for PHP](http://go.microsoft.com/fwlink/?LinkID=252473) (英文)。所涵蓋的案例包括**「上傳」**、**「列出」**、**「下載」**及**「刪除」** Blob。如需 Blob 的詳細資訊，請參閱[後續步驟](#NextSteps)一節。
...
若要將檔案當作 Blob 上傳，請使用 **BlobRestProxy-\>createBlockBlob** 方法。如果 Blob 不存在，此作業將予以建立，若已存在，則予以覆寫。下列程式碼範例假設已建立容器，並使用 [fopen](http://www.php.net/fopen) (英文) 將檔案當作串流開啟。
...
請注意，上述範例會以串流方式上傳 Blob。不過，也可以使用 [file\_get\_contents](http://php.net/file_get_contents) 之類的函數將 Blob 當作字串上傳。若要這麼做，請將上述範例中的 `$content = fopen("c:\myfile.txt", "r");` 變更為 `$content = file_get_contents("c:\myfile.txt");`。
...
請注意，以上範例會以串流資源形式 (預設行為) 取得 Blob。不過，您可以使用 [stream\_get\_contents](http://www.php.net/stream_get_contents) 函數將傳回的串流轉換成字串。
