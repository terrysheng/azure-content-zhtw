...
下列範例會將 **task1.txt** 檔的內容上傳至 **myblob** Blob。

	blob_service.put_block_blob_from_path('mycontainer', 'myblob', 'task1.txt')

## <a name="list-blob"> </a>作法：列出容器中的 Blob

若要列出容器中的 Blob，請使用 **list\_blobs** 方法與
**for** 迴圈，顯示容器中每個 Blob 的名稱。下列
程式碼將容器中每個 Blob 的 **name** 和 **url** 輸出到主控台
。
