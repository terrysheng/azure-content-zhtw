...
本節強調資料表服務的某些重要功能，特別是關於效能和延展性的設計。如果您是 Azure 儲存體和資料表服務的新手，請先閱讀 [Microsoft Azure 儲存體簡介](../storage-introduction/) 和[如何使用 .NET 的資料表儲存體]，(../storage-dotnet-how-to-use-tables/) 然後再閱讀本文的其餘部分。雖然本指南的焦點是資料表服務，但仍會討論 Azure 佇列和 Blob 服務，以及如何在方案中將它們與資料表服務搭配使用。  
...
根據預設，資料表服務會在個別實體的層級上，針對**插入**、**合併**和**刪除**作業實作開放式並行存取檢查，但用戶端也可以強制資料表服務略過這些檢查。如需資料表服務如何管理並行存取的詳細資訊，請參閱 Microsoft Azure 網站上的[管理 Microsoft Azure 儲存體中的並行存取](../storage-concurrency/) 。  
...
如需搭配使用 SAS 權杖與資料表服務的詳細資訊，請參閱[共用存取簽章，第 1 部分：了解 SAS 模型](../storage-dotnet-shared-access-signature-part-1/)。  
