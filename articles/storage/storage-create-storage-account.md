...
5. 指定要建立的儲存體帳戶類型。儲存體帳戶類型會決定儲存體帳戶如何複寫，以及是否為標準儲存體帳戶或進階儲存體帳戶。

	預設儲存體帳戶類型是 **Standard-RAGRS**，它是具有讀取存取異地備援複寫的標準儲存體帳戶，。這種類型的儲存體帳戶會從主要區域複寫到數百哩之外的次要地區，並提供次要位置的讀取權限。
...
8. 指定新的資源群組，或選取現有的資源群組。如需資源群組的詳細資訊，請參閱[使用 Azure 入口網站管理 Azure 資源](../resource-group-portal.md)。
...
建議您應定期變更儲存體帳戶的存取金鑰，保護儲存體連線的安全。指派了兩個存取金鑰，因此您可以在重新產生一個存取金鑰的同時，使用另一個存取金鑰維持儲存體帳戶連線。
