...
本文提供匯入/匯出服務的概觀，並說明如何透過 [Azure 入口網站](portal.azure.com)使用匯入/匯出服務。如需 REST API 的相關資訊，請參閱 [Azure 匯入/匯出服務 REST API 參考](http://go.microsoft.com/fwlink/?LinkID=329099)。
...
1.	備妥磁碟機後，請在 [Azure 入口網站](portal.azure.com)中瀏覽至您的儲存體帳戶，然後檢視 [儀表板]。在 [快速概覽] 之下，按一下 [建立匯入工作]。 
...
您可以在 [Azure 入口網站](portal.azure.com)中追蹤匯入或匯出工作的狀態。在管理入口網站中瀏覽至您的儲存體帳戶，然後按一下 [匯入/匯出] 索引標籤。頁面上將會顯示您的工作清單。您可以依照工作狀態、工作名稱、工作類型或追蹤號碼篩選此清單。
