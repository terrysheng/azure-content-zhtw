...
若要建立 Azure 資料表，請在上述程式碼中新增呼叫到 `CreateIfNotExistsAsync()`。
...
##取得單一實體
您可以撰寫查詢來取得單一特定實體。下列程式碼使用 `TableOperation` 物件來指定名為 'Ben Smith' 的客戶。此方法只會傳回一個實體而非一個集合，且 `TableResult.Result` 中的傳回值為 `CustomerEntity` 物件。若要從表格服務中擷取單一實體，最快的方法是在查詢中同時指定資料分割索引鍵和資料列索引鍵。
