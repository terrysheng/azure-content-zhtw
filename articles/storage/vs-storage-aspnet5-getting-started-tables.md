<properties 
	...
	description="如何在 Visual Studio 的 ASP.NET 5 專案中開始使用 Azure 資料表儲存體" 
	services="storage" 
	documentationCenter="" 
	authors="patshea123" 
	manager="douge" 
...
本文描述如何在您使用 Visual Studio 的 [**新增連接的服務**] 對話方塊，建立或參考了 ASP.NET 5 專案中的 Azure 儲存體帳戶之後開始在 Visual Studio 使用 Azure 資料表儲存體。
...
若要建立 Azure 資料表，請新增呼叫到 `CreateIfNotExistsAsync()`。
...
實體的資料表作業是利用您之前在「存取程式碼中的資料表」中建立的 `CloudTable` 物件所完成。 `TableOperation` 物件代表要完成的作業。下列程式碼範例顯示如何建立 `CloudTable` 物件及 `CustomerEntity` 物件。為了準備這項作業，建立 `TableOperation` 以便將資料表中插入至客戶實體。最後，呼叫 CloudTable.ExecuteAsync 來執行操作。
...
您可以在單一寫入操作中將多個項目插入至資料表。下列程式碼範例會建立兩個實體物件 ("Jeff Smith" 和 "Ben Smith")，並利用 Insert 方法將其加入至 `TableBatchOperation` 物件，然後透過呼叫 CloudTable.ExecuteBatchAsync 開始啟動作業。
...
##取得單一實體
您可以撰寫查詢來取得單一特定實體。下列程式碼使用 `TableOperation` 物件來指定名為 'Ben Smith' 的客戶。此方法只會傳回一個實體而非一個集合，且 `TableResult.Result` 中的傳回值為 `CustomerEntity` 物件。若要從「資料表」服務中擷取單一實體，最快的方法是在查詢中同時指定資料分割索引鍵和資料列索引鍵。
