...
若要開始，首先您必須在儲存體帳戶中建立資料表。我們將顯示如何從 Visual Studio 的 [**伺服器總管**] 建立 Azure 資料表。如果您喜歡，也會顯示如何在程式碼中建立資料表。

我們也將顯示如何執行基本的資料表和實體作業，例如新增、修改、讀取和讀取資料表實體。這些範例均以 C# 程式碼撰寫，並使用 Azure Storage Client Library for .NET。
...
涉及實體的資料表操作是利用您稍早在「存取程式碼中的資料表」中建立的 **CloudTable** 物件來執行。 **TableOperation** 物件要執行的操作。下列程式碼範例顯示如何建立 **CloudTable** 物件及 **CustomerEntity** 物件。為了準備這項操作，其建立了 **TableOperation**，以便在資料表中插入客戶實體。最後，呼叫 CloudTable.ExecuteAsync 來執行操作。
