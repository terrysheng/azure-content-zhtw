<properties urlDisplayName="Website with MongoDB on MongoLab" pageTitle="建立使用 MongoLab 與 MongoDB 的網站 (.NET)" metaKeywords="" description="Learn how to create an Azure website that stores data in MongoDB hosted by MongoLab." metaCanonical="" services="web-sites" documentationCenter=".NET" title="Create a C# ASP.NET Application on Azure with MongoDB using the MongoLab Add-On" authors="eric@mongolab.com" solutions="" manager="mongolab" editor="mollybos" />
...
1. 針對 [**名稱**]，請輸入 MONGOLAB\_URI。
1. 針對 [**值**]，請貼上我們在上一節中取得的連線字串。
1. 在 [類型] 下拉式清單中選取 [**自訂**] (而不是預設的 **SQLAzure**)。
1. 在 Visual Studio 中，選取 **[工具] > [Library Package Manager] > [Package Manager Console]** 以安裝 Mongo C# 驅動程式。在 PM 主控台上，輸入 **Install-Package mongocsharpdriver**，並按下 **Enter**。
...
如需利用 MongoDB C# 驅動程式的詳細訊息，請參閱 mongodb.org 的 [CSharp 驅動程式快速入門](http://www.mongodb.org/display/DOCS/CSharp+Driver+Quickstart "CSharp Driver Quickstart")。

### 新增建立檢視
您現在將新增建立新記事的檢視。

1. 在 [方案總管] 中以滑鼠右鍵按一下 **[檢視] > [首頁]** 項目，然後選取 **[新增] > [檢視]**。將這個新檢視命名為 **Create**，然後按一下 [**新增**]。
1. 將針對此檢視 (**Create.cshtml**) 自動產生的程式碼取代為以下內容：  
...
1. 在 **[檢視] > [首頁]** 下開啟 **Index.cshtml**，然後將其中的內容取代為以下內容：  
...
1. 在 [方案總管] 的 [**控制器**] 下開啟 **HomeController.cs**，然後將其中的內容取代為以下內容：  
...
1. 在 [**快速概覽**] 下按一下 [**下載發行設定檔**]，然後將 .PublishSettings 檔案儲存到所選的目錄。  
