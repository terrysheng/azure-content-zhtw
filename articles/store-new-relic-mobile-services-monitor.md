...
1.  [使用 Azure 市集註冊 New Relic](#sign-up)。
2.  [安裝 New Relic 模組](#install-module)。
3.  [啟用行動服務的 New Relic 開發人員分析](#enable-service)。
4.  [在 New Relic 儀表板中監視行動服務](#monitor)。
...
1.  如果您尚未這麼做，請依照[在原始檔控制中儲存伺服器指令碼](/en-us/develop/mobile/tutorials/store-scripts-in-source-control/)教學課程中的步驟進行，以啟用行動服務的原始檔控制、複製儲存機制，及安裝 [Node Package Manager (NPM)](http://nodejs.org/)。

2.  瀏覽至本機 Git 儲存機制的 `.\service` 資料夾，然後在命令提示字元中執行下列命令：

         npm install newrelic

    NPM 會在 `\newrelic` 子目錄中安裝 [New Relic 模組](https://npmjs.org/package/newrelic)。
...
接著，您將在[管理入口網站](https://manage.windowsazure.com/)中啟用 New Relic 監視行動服務。

啟用行動服務的 New Relic 開發人員分析
-------------------------------------

1.  在[管理入口網站](https://manage.windowsazure.com/)中，選取您的行動服務，然後按一下 **[設定]** 索引標籤。
...
2.  等候幾分鐘的時間來完成資料處理，然後瀏覽到 New Relic 儀表板。
    ...
    當以附加元件方式購買 New Relic 訂閱時，請在[管理入口網站](https://manage.windowsazure.com/)中選取此訂閱，並按一下 **[管理]**。
...
-   如需定價資訊，請參閱＜[Azure 市集的 New Relic 頁面](/en-us/gallery/store/new-relic/new-relic/)＞。
-   如需有關使用 New Relic 的詳細資訊，請參閱 New Relic 文件中的[應用程式概觀](https://docs.newrelic.com/docs/applications-dashboards/applications-overview) (英文)。
