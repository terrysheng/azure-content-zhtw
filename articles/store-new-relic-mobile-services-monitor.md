<properties 
	pageTitle="在原始檔控制中儲存伺服器程式碼 - Azure 行動服務" 
...
3. 開啟 Git 命令列工具 (例如 **GitBash** (Windows) 或 **Bash** (Unix Shell))，然後在 Git 命令提示字元中輸入下列命令： 
...
接著，您將在[管理入口網站][Azure 管理入口網站]中啟用 New Relic 監視行動服務。 

##<a name="enable-service"></a>啟用行動服務的 New Relic 開發人員分析

1. 在[管理入口網站][Azure 管理入口網站]，選取您的行動服務，然後按一下 **[設定]** 索引標籤。
...
2. 等候幾分鐘的時間來完成資料處理，然後瀏覽到 New Relic 儀表板。
	...
	當以附加元件方式購買 New Relic 訂閱時，請在[管理入口網站][Azure 管理入口網站]中選取此訂閱，並按一下 **[管理]**。
...
+ 如需有關使用 New Relic 的詳細資訊，請參閱 New Relic 文件中的[應用程式概觀]。 
