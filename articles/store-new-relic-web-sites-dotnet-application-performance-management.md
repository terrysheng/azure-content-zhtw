<properties urlDisplayName="New Relic App Performance Management" pageTitle="Azure 上的 New Relic 應用程式效能管理" metaKeywords="new relic Azure, performance azure" description="了解如何對 Azure 使用 New Relic 的效能監視。" metaCanonical="" services="web-sites" documentationCenter=".NET" title="New Relic Application Performance Management on Azure Websites" authors="new relic" solutions="" manager="carolz" editor="" />
...
本指南說明如何將 New Relic 的世界級效能監視加入 Azure 網站。我們將討論以快速又簡單的程序將 New Relic 加入至應用程式，並介紹一些 New Relic 功能。如需有關使用 New Relic 的詳細資訊，請參閱[使用 New Relic](#using-new-relic)。
...
4. 選取 [**New Relic for Azure Websites**] 項目，然後按一下 [**安裝**]。
...
您在前一個步驟中新增至應用程式的 New Relic 套件，是以新增至 Azure 網站的 [**應用程式設定**] 所設定的。執行下列步驟以新增這些設定。

1. 登入 [Azure 管理入口網站](https://manage.windowsazure.com) 然後導覽至您的網站。
...
3. 在 [**開發人員分析**] 中指定授權之後，按一下 [**儲存**]。一旦儲存操作完成後，下列值便會新增至頁面的 [**應用程式設定**] 區段以支援 New-Relic：
	...
	<div class="dev-callout"> 
	<strong>注意</strong> 
	<p>新的 [應用程式設定] 最多 <strong>可能需要 30 秒的時間</strong> 才能生效。若要強制設定立即生效，請重新啟動網站。</p> 
	</div>
...
<table border="1">
  ...
  <tbody>
    ...
    <tr>
       ...
       <td>從 [概觀] 的 <b>[瀏覽器]</b> 檢視，指向 [全球 Apdex] 地圖上的任一處。<br /><b>秘訣：</b> 若要直接移至選取的應用程式 <a href="https://newrelic.com/docs/site/geography" target="_blank">[地理位置]</a>儀表板，請按一下 <b>[全球 Apdex]</b> 標題，或按一下 [全球 Apdex] 地圖上的任一處。</td>
    </tr>
    <tr>
    ...
    <tr>
       <td>檢視 <a href="https://newrelic.com/docs/site/errors" target="_blank">[誤差]</a> 儀表板</td>
       <td>按一下 [應用程式概觀] 儀表板上的 [誤差率圖表] 標題。<br /><b>秘訣：</b> 您可以從 <b>[應用程式]</b> &gt; (您的應用程式) &gt; [事件] &gt; [誤差] 來檢視 [誤差] 儀表板。</td>
    </tr>
    <tr>
 ...
 * [實際使用者監控](https://newrelic.com/docs/features/real-user-monitoring)：RUM 如何詳細說明使用者瀏覽器載入網頁的所需時間、 
