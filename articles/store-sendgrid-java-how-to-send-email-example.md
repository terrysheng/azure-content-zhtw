...
此外，強烈建議您熟悉[在 Eclipse 建立 Azure 的 Hello World 應用程式](http://msdn.microsoft.com/en-us/library/windowsazure/hh690944)上的資訊，或熟悉其他在 Azure 中代管 Java 應用程式的技術 (如果您未使用 Eclipse)。
...
-   您可以使用 `RoleEnvironment.getConfigurationSettings`，從部署的組態設定中擷取 SendGrid 使用者名稱和密碼，而不 Web 表單擷取這些值。如需 `RoleEnvironment` 類別的相關資訊，請參閱[在 JSP 中使用 Azure Service Runtime Library](http://msdn.microsoft.com/en-us/library/windowsazure/hh690948)，以及 &lt;http://dl.windowsazure.com/javadoc\> 上的 Azure Service Runtime 封裝文件。
