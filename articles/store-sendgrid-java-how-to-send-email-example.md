...
-   您可以使用 `RoleEnvironment.getConfigurationSettings`，從部署的組態設定中擷取 SendGrid 使用者名稱和密碼，而不 Web 表單擷取這些值。如需 `RoleEnvironment` 類別的相關資訊，請參閱[在 JSP 中使用 Azure Service Runtime Library](http://msdn.microsoft.com/zh-tw/library/windowsazure/hh690948)，以及 &lt;http://dl.windowsazure.com/javadoc\> 上的 Azure Service Runtime 封裝文件。
