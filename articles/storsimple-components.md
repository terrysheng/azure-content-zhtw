<properties 
   ...
   description="說明 StorSimple 裝置、服務與管理技術。" 
   services="storsimple" 
...
Microsoft Azure StorSimple 裝置會是內部部署混合式儲存體陣列，可提供主要儲存體和 iSCSI 以存取儲存在其他位置的資料。它會管理雲端儲存體之間的通訊，並協助確保所有儲存在 Microsoft Azure StorSimple 方案的資料安全性和機密性。
