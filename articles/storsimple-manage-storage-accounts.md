...
- **服務訂用帳戶中的儲存體帳戶** – 這些是與相同服務訂用帳戶相關聯的 Azure 儲存體帳戶。若要深入了解如何建立這些儲存體帳戶，請參閱[關於 Azure 儲存體帳戶](storage-create-storage-account.md)。 
...
1. 在服務登陸頁面上選取您的服務，然後按兩下該服務。這會將您帶領到 \[快速入門\]**** 頁面。 
2. 存取 \[設定\]**** 頁面。
3. 按一下 \[新增/編輯儲存體帳戶\]****。
4. 在 \[新增/編輯儲存體帳戶\]**** 對話方塊中，執行下列動作：
  1. 按一下 \[新增\]****。
  2. 提供儲存體帳戶的名稱。
  3. 提供 Microsoft Azure 儲存體帳戶的「儲存體帳戶存取金鑰」****。
  4. 選取 \[啟用 SSL 模式\]**** 核取方塊來建立裝置與雲端之間網路通訊的安全通道。只有當您在私人雲端內進行操作時，才需清除 \[啟用 SSL 模式\]**** 核取方塊。
  5. 按一下核取圖示 ![核取圖示](./media/storsimple-manage-storage-accounts/HCS_CheckIcon.png)。在成功建立儲存體帳戶之後，系統將會通知您。

    ![新增儲存體帳戶](./media/storsimple-manage-storage-accounts/HCS_AddStorageAccount.png)

5. 新建立的儲存體帳戶將顯示於 \[設定\]**** 頁面上的 \[儲存體帳戶\]**** 下方。按一下 \[儲存\]**** 以儲存新建立的儲存體帳戶。系統提示您進行確認時，按一下 \[確定\]****。
...
深入了解 [StorSimple 安全性](storsimple-security.md) <!--HONumber=52-->
