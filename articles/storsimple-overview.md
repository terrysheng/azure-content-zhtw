<properties 
   ...
   description="說明 StorSimple 功能、架構和元件。" 
   services="storsimple" 
...
- **透明整合** - Microsoft Azure StorSimple 會使用 Internet Small Computer System Interface (iSCSI) 通訊協定，以無形的方式連結資料儲存設備。這可確保儲存在雲端、在資料中心或在遠端伺服器上的資料會看似儲存在單一位置。
- **降低儲存體成本** - Microsoft Azure StorSimple 會配置足夠符合目前需求的本機或雲端儲存體，並在必要時擴充雲端儲存體。它可以進一步地降低儲存體需求和支出，方法是消除相同資料的備援版本 (重複資料刪除)，並使用壓縮。
- **簡化的儲存體管理** - Microsoft Azure StorSimple 提供系統管理工具，可用來設定和管理在內部部署、在遠端伺服器上和在雲端中儲存的資料。此外，您可以從 Microsoft Management Console (MMC) 嵌入式管理單元管理備份和還原功能。StorSimple 提供一個單獨的選用介面，可用來將 StorSimple 管理和資料保護服務延伸到儲存在 SharePoint 伺服器上的內容。 
- **改良的災害復原和提高合規性** - Microsoft Azure StorSimple 不需要延長回復時間。相反地，它會在需要時還原資料。這表示正常作業可以在最少干擾的情況下繼續執行。此外，您還可以設定原則以指定備份排程和資料保留。
- **資料行動力** - 您可以從其他站台存取上傳至 Microsoft Azure 雲端服務的資料，以供復原和移轉使用。此外，您可以使用 StorSimple，在 Microsoft Azure 中執行的虛擬機器 (VM) 上設定 StorSimple 虛擬裝置。VM 然後可以使用虛擬裝置來存取儲存的資料，以供測試或復原使用。 
...
- **StorSimple Adapter for SharePoint** - 可將 Microsoft Azure StorSimple 儲存體和資料保護明確延伸至 SharePoint 伺服器陣列，並可讓您從 SharePoint 管理入口網站檢視和管理 StorSimple 儲存體的工具。
