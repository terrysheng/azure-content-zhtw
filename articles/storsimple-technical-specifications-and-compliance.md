...
下表列出各種 StorSimple 裝置機型的耗電量資料。
 
| 條件 | 240 V AC | 110 V AC |
| ---------- | -------- | -------- |
| 風扇慢速，磁碟機閒置 | 1\.45 0.31 kW | 3\.19 A 0.34 kW |
| 風扇慢速，磁碟機存取中 | 1\.54 A 0.33 kW | 3\.27 A 0.36 kW |
| 風扇快速，磁碟機閒置，兩個 PSU 已運轉 | 2\.14 A 0.49 kW | 4\.99 A 0.54 kW |
| 風扇快速，磁碟機閒置，一個 PSU 已運轉、一個 PSU 閒置 | 2\.05 A 0.48 kW | 4\.58 A 0.5 kW |
| 風扇快速，磁碟機存取中，兩個 PSU 已運轉 | 2\.26 A 0.51 kW | 4\.95 A 0.54 kW |
| 風扇快速，磁碟機存取中，一個 PSU 已運轉、一個 PSU 閒置 | 2\.14 A 0.49 kW | 4\.81 A 0.53 kW |
...
如果是 10 GbE 網路介面、DATA 2 和 DATA3，可在以下連結中找到支援的網路纜線和模組清單：[http://www.mellanox.com/page/cables?mtag=cable\_overview](http://www.mellanox.com/page/cables?mtag=cable_overview)
