...
維護模式更新是干擾性更新，例如，磁碟韌體升級或 USM 韌體升級。這些更新需要使裝置進入維護模式。如需詳細資訊，請參閱[變更裝置模式](#change-device-modes)。您無法使用管理入口網站來安裝維護模式更新。您必須改用 Windows PowerShell for StorSimple。
...
連接到主控台之後，判斷是否有需要安裝的更新，並進入維護模式來安裝這些更新。
...
## 透過 Windows PowerShell 或 StorSimple 安裝 Hotfix
...
深入了解 [Windows PowerShell for StorSimple](https://https://msdn.microsoft.com/library/azure/dn772425.aspx)。
