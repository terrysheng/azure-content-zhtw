...
主控台視窗頂端的功能表列包含 [[**檔案**](#file-menu)]、[[**動作**](#action-menu)]、[[**檢視**](#view-menu)]、[[**我的最愛**](#favorites-menu)]、[[**視窗**](#window-menu)]，以及 [[**說明**](#help-menu)] 功能表。

按一下功能表列上的任何項目，即可查看可用的命令清單。下列範例顯示功能表列上選取的 [**檢視**] 功能表。
...
| 開啟 | 按一下 [**開啟**]，可開啟現有的主控台。 |
| 儲存 | 按一下 [**儲存**]，可儲存目前的主控台。 |
| 另存新檔 | 按一下 [**另存新檔**]，可建立目前主控台的新的、重新命名的執行個體。使用 [**另存新檔**] 選項，可自訂檢視並加以儲存，以供稍後擷取。例如，您可以建立 StorSimple Snapshot Manager 嵌入式管理單元，指向特定伺服器。 |
| 新增/移除嵌入式管理單元 | 按一下 [**新增/移除嵌入式管理單元**]，可新增或移除嵌入式管理單元，以及組織 [**範圍**] 窗格中的節點。如需詳細資訊，請移至《[新增、移除和組織 MMC 3.0 中的嵌入式管理單元和延伸模組](https://technet.microsoft.com/library/cc722035.aspx)》。 |
...
使用 [**動作**] 功能表，可選取可用的動作。您可以使用的項目取決於您在 [**範圍**] 窗格或 [**結果**] 窗格中所做的選擇。
...
- [**說明**] 欄描述如何使用 [**動作**] 功能表或 [動作] 窗格上的每個動作，並解釋其用途。
...
| ![向上圖示](./media/storsimple-use-snapshot-manager/HCS_SSM_Up.png) | 按一下向上圖示，可在主控台樹狀目錄中往上一層 ([**範圍**] 窗格)。 |
...
磁碟區節點代表對應至主機所掛接之磁碟區的磁碟機，包括透過 iSCSI 探索到的磁碟機，以及透過裝置探索到的磁碟機。請使用這個節點，來檢視可用的磁碟區清單，並將個別磁碟區指派給磁碟區群組。
...
備份原則是本機和雲端快照的作業排程。使用 [**備份原則**] ，可指定建立備份的頻率，以及應該保留備份多長時間。
...
[**備份目錄**] 節點包含 Azure StorSimple 磁碟區的現場和異地備份清單。此節點是依磁碟區群組組織，而且每個磁碟區群組容器包含本機快照 (本機快照節點) 和雲端快照 (雲端快照集節) 的個別結構。展開時，每個磁碟區群組容器會列出以互動方式或透過已設定的原則所取得的所有成功備份。
...
[**本機快照**] 節點會列出特定磁碟區群組的本機快照。節點位於 [**範圍**] 窗格中的 [**備份目錄**] 之下。本機快照是儲存在 Azure StorSimple 裝置之磁碟區資料的時間點複本。一般而言，可以快速建立和還原這種類型的備份。您可以如同使用本機備份複本一般使用本機快照。
...
**雲端快照**節點會列出特定磁碟區群組的雲端快照集。節點位於 [**範圍**] 窗格中的 [**備份目錄**] 之下。雲端快照是儲存在雲端之磁碟區資料的時間點複本。雲端快照相當於在不同的異地儲存體系統上複寫的快照。雲端快照在災害復原案例中特別有用。
...
|:----------------|:------------------------------------|
| V | 開啟 [**檢視**] 功能表。 |
| W | 開啟新的 StorSimple Snapshot Manager 主控台，著重於 [**作業**] 節點。|
| F | 更新 StorSimple Snapshot Manager 主控台。|
| L | 開啟 [**匯出清單**] 頁面。 |
| H | 開啟線上說明 |
