<properties 
	pageTitle="Azure Stream Analytics Power BI 儀表板 |Azure" 
	description="了解如何在即時 Power BI 儀表板中填寫 Azure Stream Analytics 工作的資料。" 
...
# Azure Stream Analytics & Power BI：資料流即時分析上的即時儀表板

Azure Stream Analytics 一個常見的用途就是即時分析大量的資料流以及掌握即時儀表板中的數據 (一種可以即時更新，不用使用者重新整理瀏覽器內容的儀表板)。[Microsoft Power BI](https://powerbi.com/) 最適合用來建立即時儀表板。[以下是案例解說的範例影片](https://www.youtube.com/watch?v=SGUpT-a99MA)。在這篇文章中，了解如何利用 Power BI 當做  Azure Stream Analytics 工作的輸出。請注意，Azure Stream Analytics 目前已可使用，Power BI 輸出是 Azure Stream Analytics 的預覽功能。

## 必要條件

* 使用組織識別碼的 Microsoft Azure  帳戶 (限 Power BI 搭配組織識別碼。組織識別碼是公司或企業的電子郵件地址，例如 xyz@mycompany.com。xyz@hotmail.com 此類的個人電子郵件，則不是組織識別碼。[您可以在這裡深入了解組織識別碼](https://www.arin.net/resources/request/org.html))。
* ASA (Azure Stream Analytics) 工作使用其中資料的來源輸入資料流。此時，ASA 接受來自 Azure Eventhub 或 Azure Blob 存放區的輸入。  

## 建立 Azure Stream Analytics 工作
...
> [AZURE.NOTE]注意 - Power BI 輸出僅適用於使用組織識別碼的 Azure 帳戶。如果您的 Azure 帳戶不使用組織識別碼 (例如，即時識別碼/個人 Microsoft 帳戶)，就不會看到 Power BI 輸出選項。
...
4.  在這個步驟中，必須小心使用您用於  ASA  工作的相同組織識別碼。目前，Power BI 輸出必須和 ASA 工作使用相同的組織識別碼。如果 Power BI 帳戶已經使用相同的組織識別碼，請選取 [立即授權]。如果沒有，請在申請 Power BI 時，選擇 [立即註冊]，然後使用和 Azure 帳戶相同的組織識別碼。[下面的部落格文章為您逐步解說如何註冊 Power BI](http://blogs.technet.com/b/powerbisupport/archive/2015/02/06/power-bi-sign-up-walkthrough.aspx)。
5.  接下來您會看到一個畫面，如下所示：
...
*	**資料表名稱** - 提供 Power BI 輸出資料集的資料表名稱。暫時稱之為 "pbidemo"。目前，ASA  工作的 Power BI 輸出中，一個資料集只能有一個資料表。

>	[AZURE.NOTE] 注意 - 您不應該在您的 Power BI 帳戶中明確建立此資料集與資料表。它們將會在您開始您的 ASA 工作，且工作開始對 Power BI 輸出內容之後自動建立。如果您的 ASA 工作查詢沒有傳回任何結果，將不會建立資料集與資料表。

*	依序按一下 [確定]、[測試連線]，輸出組態就已經完成。

>	[AZURE.WARNING] 也請您注意，如果 Power BI 已經有資料集與資料表，且其名稱與您在此 ASA 工作中提供的名稱相同，將會複寫現有的資料。.
...
移至 [Powerbi.com](https://powerbi.com)，然後輸入組織識別碼登入如果 ASA 工作查詢輸出任何結果，就會看到您的資料集已經建立完成：
...
現在按一下 ASA 工作建立的資料集 (就是目前範例中的 pbidemo)。系統會開啟一個頁面並在這個資料集上方建立一張圖表。以下是您可以建立的報表範例：
...
請注意，本教學課程示範如何為一個資料集建立一種圖表類型。不過，Power BI 的用途可是千變萬化。如需其他 Power BI 儀表板範例，請觀賞[開始使用 Power BI](https://youtu.be/L-Z_6P56aas?t=1m58s) 視訊。
