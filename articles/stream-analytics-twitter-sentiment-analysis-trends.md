<properties
   pageTitle="分析 Twitter 感受度和即時趨勢 |Microsoft Azure"
   description="了解如何使用 Stream Analytics 即時分析 Twitter 的感受度和趨勢。本教學課程包括從事件產生到即時儀表板資料的一切相關步驟。"
...
# 社交媒體分析：即時 Twitter 感受度分析

在這個教學課程中，您將學習如何將 Twitter 事件帶入事件中樞、編寫 Stream Analytics 查詢來分析資料，然後儲存結果或使用儀表板即時提供深入解析，以此制定一套解決方案。

## 案例

有一個新聞媒體網站提供與讀者立即有關的內容，試圖打敗競爭對手。
透過即時分析 Twitter 資料，他們針對與讀者相關之主題使用了社交媒體深入解析。具體來說，為了找出熱門話題，他們需要即時分析熱門話題的推文數量和感受度。

## 必要條件
1.	本教學課程需要 Twitter 帳戶。  
2.	這個逐步教學課程利用 GitHub 上的事件產生器。請到[這裡](https://github.com/streamanalytics/samples/tree/master/TwitterClient)下載，然後按照下列步驟設定您的解決方案。
...
6.	啟動應用程式。您會看到推文事件連同 CreatedAt、Topic 和 SentimentScore 值，一起傳送到事件中樞：
	...
	![將 SentimentScore 值傳送到事件中樞 。](./media/stream-analytics-twitter-sentiment-analysis-trends/stream-analytics-twitter-sentiment-output-to-event-hub.png)

## 建立 Stream Analytics 工作

現在，我們已經有了一連串的 Tweet 事件，我們可以設定 Stream Analytics 工作並即時分析這些事件。
...
4.	在第三頁上輸入或選取下列值：
	...
	* **輸入別名**：輸入此工作輸入的易記名稱，例如 TwitterStream。請注意，此名稱將用於後續的查詢。**事件中樞**：如果您建立的事件中樞與 Stream Analytics 工作位於相同的訂閱中，請選取事件中樞所在的命名空間。

		如果事件中心位於不同的訂閱中，請選取 **Use Event Hub from Another Subscription**, 儲存體帳戶 **SERVICE BUS NAMESPACE**, **EVENT HUB NAME**, **EVENT HUB POLICY NAME**, **EVENT HUB POLICY KEY**, 和 **EVENT HUB PARTITION COUNT**.
...
使用 [Azure 儲存體總管](https://azurestorageexplorer.codeplex.com/)或 [Azure 總管](http://www.cerebrata.com/products/azure-explorer/introduction)此類的工具，即時檢視您工作的輸出。從這裡開始，您可以擴充應用程式，讓它在輸出時包含自訂的儀表板，例如下圖使用 [Power BI](https://powerbi.com/) 的儀表板。

![Power BI 儀表板中的 Stream Analytics 輸出。](./media/stream-analytics-twitter-sentiment-analysis-trends/stream-analytics-output-power-bi.png)
