...
| 訂閱 | 您可以從自己的訂用帳戶選擇資料庫，或提供連接資訊給另一個訂用帳戶中的其中一個資料庫 |
| 資料庫 | 您傳送輸出的資料庫名稱 |
...
若要將大量非結構化資料儲存於雲端，Blob 儲存體提供具有成本效益且可擴充的解決方案。如需 Azure Blob 儲存體和其使用方式的簡介，請參閱[如何使用 Blob](./articles/storage-dotnet-how-to-use-blobs.md) 中的文件。
...
<tr>
<td>訂閱</td>
<td>您可以從自己的訂用帳戶選擇 blob 儲存體帳戶，或提供連接資訊給另一個訂用帳戶中的其中一個資料庫</td>
</tr>
<tr>
<td>儲存體帳戶</td>
<td>您傳送輸出的儲存體帳戶名稱。</td>
...
<tr>
<td>分隔符號</td>
<td>串流分析可支援多種序列化 CSV 資料常用的分隔符號。支援的值是逗號、分號、空格、索引標籤和分隔號。</td>
</tr>
<tr>
<td>格式</td>
...
[事件中樞](https://azure.microsoft.com/services/event-hubs/)是可充分調整的發佈-訂閱事件擷取器。它每秒可以收集數百萬個事件。事件中樞做為輸出的其中一個使用時機，就是當串流分析工作的輸出將成為另一個串流工作的輸入時。
...
| 訂閱 | 您可以從自己的訂用帳戶選擇事件中樞，或提供連接資訊給另一個訂用帳戶中的其中一個資料庫 |
...
| 分隔符號 | 串流分析可支援多種以 CSV 格式序列化資料常用的分隔符號。支援的值是逗號、分號、空格、索引標籤和分隔號。 |
| 格式 | 僅適用於 JSON 類型。分隔的行會指定輸出的格式化方式為利用新行分隔每個 JSON 物件。陣列會指定輸出將會格式化為 JSON 物件的陣列。 |
## Power BI

可以使用 [Power BI](https://powerbi.microsoft.com/) 做為串流分析工作的輸出，藉此提供串流分析使用者豐富的視覺體驗。這項功能可以用於可作業的儀表板、報告的產生及度量驅動的報告。

### 授權 Power BI 帳戶

1.	選取 Power BI 做為輸出時，您將會收到提示授權現有的 Power BI 使用者或建立新的 Power BI 帳戶。  
...
| 訂閱 | 如果儲存體帳戶來自另一個訂用帳戶，您可以從自己的訂用帳戶選擇資料表儲存體帳戶或提供連接資訊 |
| 儲存體帳戶 | 您傳送輸出的儲存體帳戶名稱。 |
...
| 訂閱 | 如果命名空間來自另一個訂用帳戶，您可以從自己的訂用帳戶選擇服務匯流排命名空間或提供連接資訊 |
...
| 訂閱 | 如果命名空間來自另一個訂用帳戶，您可以從自己的訂用帳戶選擇服務匯流排命名空間或提供連接字串資訊 |
| 服務匯流排 命名空間 | 服務匯流排命名空間是一個容器，包含一組訊息實體。建立新的事件中樞時，也會建立服務匯流排命名空間 |
