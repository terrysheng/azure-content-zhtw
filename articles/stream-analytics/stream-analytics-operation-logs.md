...
串流分析工作相對應的記錄檔連結會顯示在該工作的 [儀表板] 索引標籤上。按一下該連結時，其會以能夠顯示該特定工作最新記錄檔的方式來設定篩選器。
  ...
  ![選取管理服務](./media/stream-analytics-operation-logs/01-stream-analytics-operation-logs.png)
...
在 Azure 傳統入口網站中，按一下視窗底部的 [**詳細資料**] 按鈕來檢視所選事件的詳細資訊。
...
在 Azure 傳統入口網站中，按一下 [搜尋] 圖示並鍵入 ‘failed’。這樣可以找出所有包含失敗項目的記錄檔。
...
萬一您需要連絡[支援服務](http://azure.microsoft.com/support/options/)或透過 [MSDN 論壇](https://social.msdn.microsoft.com/Forums/zh-TW/home?forum=AzureStreamAnalytics)將資訊提供給團隊，請注意作業詳細資料，特別是**相互關聯識別碼**。
