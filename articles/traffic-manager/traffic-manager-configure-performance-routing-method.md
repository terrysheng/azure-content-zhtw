...
為了能夠路由位於全球不同資料中心 (也稱為區域) 的雲端服務和網站 (端點) 的流量，您可以將連入流量從要求用戶端導向具有最低延遲的端點。通常，具有最低延遲的資料中心會對應到最短地理距離的資料中心。效能流量路由方法可讓您根據最低延遲進行散發，但無法考量在網路組態或負載中的即時變更。如需 Azure 流量管理員所提供之不同流量路由方法的詳細資訊，請參閱[關於流量管理員流量路由方法](traffic-manager-load-balancing-methods.md)。
