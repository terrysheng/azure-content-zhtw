...
在流量管理員中可以使用的負載平衡方法有三種。雖然您可以隨時為設定檔選取不同的負載平衡方法，但每個流量管理員設定檔一次只能使用一種負載平衡方法。

請務必注意所有的負載平衡方法都包括端點監視。設定您的流量管理員設定檔並指定最適合您需求的負載平衡方法之後，請進行監視設定。已正確設定監視之後，流量管理員會監視包含雲端服務和網站的端點狀態，並且不會將流量傳送到認定無法使用的端點。如需流量管理員監視的相關資訊，請參閱 [關於流量管理員監視] (traffic-manager-monitoring.md)。 
...
為了能夠負載平衡位於全球不同資料中心內的端點，您可以根據要求用戶端與端點之間的最低延遲，將連入流量導向最靠近的端點。通常，「最靠近」的端點會直接對應到最短的地理距離。[效能] 負載平衡方法可讓您根據位置和延遲進行分配，但不會將網路組態或負載中的即時變更納入考量。
