...
在指定外部端點時：- 必須使用 'Target' 參數指定端點網域名稱。- 如果使用「效能」流量路由方法，則 'EndpointLocation' 為必要項目，否則為選用項目。此值必須是[有效的 Azure 區域名稱](http://azure.microsoft.com/regions/)。- 和用於 Azure 端點時一樣，'Weight' 和 'Priority' 是選用項目。
