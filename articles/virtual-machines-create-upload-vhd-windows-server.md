...
本文說明如何使用作業系統上傳虛擬硬碟 (VHD)，以便用它做為映像，根據該映像建立虛擬機器。如需 Microsoft Azure 中磁碟和映像的詳細資訊，請參閱[關於 Azure 中的磁碟和映像](http://msdn.microsoft.com/library/windowsazure/jj672979.aspx)。

> [AZURE.NOTE] 當您依據映像建立虛擬機器時，您可以為您計畫在虛擬機器上執行的應用程式自訂適當的作業系統設定。此設定是針對該虛擬機器所儲存，因此不會影響映像。如需指示，請參閱[如何建立自訂虛擬機器](http://azure.microsoft.com/documentation/articles/virtual-machines-windows-tutorial/)。

##必要條件##
本文假設您具有下列項目：

1. **Azure 訂用帳戶** - 如果您沒有帳戶，只需要幾分鐘的時間就可以建立免費試用帳戶。如需詳細資訊，請參閱[建立 Azure 帳戶](http://azure.microsoft.com/develop/php/tutorials/create-a-windows-azure-account/)。  

2. **Microsoft Azure PowerShell** - 您已安裝 Microsoft Azure PowerShell 模組並設定為使用您的訂用帳戶。若要下載此模組，請參閱 [Microsoft Azure 下載](http://azure.microsoft.com/downloads/)。安裝及設定模組的教學課程位於[此處](http://azure.microsoft.com/documentation/articles/install-configure-powershell/)。您可以使用 [Add-AzureVHD](http://msdn.microsoft.com/library/azure/dn495173.aspx) Cmdlet 來上傳 VHD。

3. **支援的 Windows 作業系統並儲存在 .vhd 檔案中** - 您已將支援的 Windows Server 作業系統安裝在虛擬硬碟中。有多項工具可用來建立 .vhd 檔案。您可以使用虛擬化解決方案 (例如 Hyper-V) 建立虛擬機器並安裝作業系統。如需指示，請參閱[安裝 Hyper-V 角色及設定虛擬機器](http://technet.microsoft.com/library/hh846766.aspx)。
...
在將映像上傳到 Azure 之前，您必須使用 Sysprep 命令將其一般化。如需使用 Sysprep 的詳細資訊，請參閱[如何使用 Sysprep：簡介](http://technet.microsoft.com/library/bb457073.aspx)。
...
您必須有 Azure 中的儲存體帳戶，才能上傳可在 Azure 中用來建立虛擬機器的 .vhd 檔案。您可以使用 Azure 管理入口網站來建立儲存體帳戶。
...
4. 依照下列方式填入欄位：
	...
	- 選擇儲存體帳戶的位置或同質群組。同質群組可讓您將雲端服務與儲存體放在相同的資料中心。
		 
	- 決定儲存體帳戶是否要使用異地複寫。依預設會開啟地理區域複寫。此選項可讓您免費將資料複寫至次要位置，使您在遇到主要位置發生重大錯誤時，可將儲存體容錯移轉至該位置。次要位置會自動指派，且無法變更。如果您因為法律規定或組織原則要求的緣故，而必須更嚴密地掌控雲端儲存體的位置，您可以關閉地理複寫。但請注意，如果您後續又開啟地理區域複寫，在您將現有的資料複寫至次要位置時，將會產生一次性的資料傳輸費用。不含地理區域複寫的儲存服務會有相對的折扣。管理儲存體帳戶地理區域複寫的詳細資訊提供於：[建立、管理或刪除儲存體帳戶](../storage-create-storage-account/#replication-options)。
...
1. 依照此文章的指示，開啟 Azure PowerShell 主控台：[做法：安裝 Microsoft Azure PowerShell](#Install)。
...
4. 輸入： 
	...
	如需安裝及設定 PowerShell 的詳細資訊，請參閱[如何安裝及設定 Microsoft Azure PowerShell](http://azure.microsoft.com/documentation/articles/install-configure-powershell/) 
...
4. 在 [從 VHD 建立映像] 中，執行下列動作：
	...
	- 指定 [名稱]
	- 指定 [描述]
	- 若要指定 VHD 的 URL，請按一下資料夾按鈕來開啟以下視窗：
...
5. **選擇性：**您可以使用 Add-AzureVMImage Cmdlet (不要使用「管理入口網站」) 來將您的 VHD 新增為映像。	在 Azure PowerShell 主控台中，輸入：
...
6. 完成前面的步驟之後，當您選擇 [映像] 索引標籤時就會列出新的映像。 
	...
	當您建立虛擬機器時，這個新的映像現在已在 [我的映像] 底下提供。如需指示，請參閱[建立執行 Windows Server 的虛擬機器](http://azure.microsoft.com/documentation/articles/virtual-machines-windows-tutorial/)。

	![create VM from custom image](./media/virtual-machines-create-upload-vhd-windows-server/create_vm_custom_image.png)

	> [AZURE.TIP] 如果您在嘗試建立 VM 時發生錯誤，如果錯誤訊息是 [VHD https://XXXXX... 的虛擬大小 YYYY 位元組不受支援。大小必須是整數 (MB)]，這表示您的 VHD 不是整數的 MB，且必須是固定大小的 VHD。請嘗試使用 Add-AzureVMImage PowerShell Cmdlet (不要使用「管理入口網站」) 來新增映像 (請參閱上面的步驟 5)。Azure Cmdlet 可確保 VHD 符合 Azure 需求。
	
## 後續步驟 ##
 

建立虛擬機器之後，嘗試建立 SQL Server 虛擬機器。如需指示，請參閱[在 Microsoft Azure 上佈建 SQL Server 虛擬機器](http://azure.microsoft.com/documentation/articles/virtual-machines-provision-sql-server/)。 
