<properties urlDisplayName="SharePoint on Azure" pageTitle="在 Azure 虛擬機器上部署 SharePoint 2010" metaKeywords="" description="了解在 Azure 虛擬機器上使用 SharePoint 2010 的支援案例。" metaCanonical="" services="virtual-machines" documentationCenter="" title="在 Azure 虛擬機器上部署 SharePoint" authors="josephd" solutions="" manager="timlt" editor="" />
...
Azure 虛擬機器可為 SharePoint 案例提供運算功能、記憶體和儲存體，而其價格通常以資源用量為基礎。組織僅需就使用的部分付費，且服務會提供所有執行 SharePoint 基礎結構所需的功能。如需定價和計費的詳細資訊，請參閱 [Azure 定價詳細資料][Azure 定價詳細資料]。請注意，從內部部署網路將 Azure 雲端中的儲存體和資料移出時，須收取少許費用。但 Azure 不會對上傳資料收費。
...
本小節將討論 Azure 的基本架構。Azure 中的**映像庫**會提供預先配置的可用 VM 清單。使用者可將 SharePoint Server、SQL Server、Windows Server 和其他 ISO/VHD 發佈至映像庫。為簡化 VM 的建立作業，系統會建立基礎映像，並將其發佈至映像庫。授權使用者可使用這些映像產生所需的 VM。如需詳細資訊，請移至 Azure 網站上的[建立執行 Windows Server 2008 R2 的虛擬機器][建立執行 Windows Server 2008 R2 的虛擬機器]。圖 2 顯示使用 Azure 管理入口網站建立 VM 的基本步驟：

圖 2：VM 建立步驟概觀

![azure-sharepoint-wp-2][azure-sharepoint-wp-2]

使用者也可在 Azure 管理入口網站上傳已執行過 Sysprep 的映像。如需詳細資訊，請移至[建立和上傳虛擬硬碟][建立和上傳虛擬硬碟]。圖 3 顯示上傳映像以建立 VM 的基本步驟：
...
<ol>
<li>
透過您的 Azure 訂閱帳戶，登入 [Azure 管理入口網站][Azure 管理入口網站]。

-   如果您沒有 Azure 帳戶，請[註冊 Azure 的免費試用版][註冊 Azure 的免費試用版]。

</li>
<li>
若要建立採用基礎作業系統的 VM，在 Azure 管理入口網站上，依序按一下 [新增]、[計算]、[虛擬機器] 和 [從組件庫]。

</li>
<li>
[選擇映像] 對話方塊隨即出現。按一下 [Windows Server 2008 R2 SP1] 平台映像，然後按一下向右箭頭。

</li>
<li>
[虛擬機器組態]**** 對話方塊隨即出現。請提供下列資訊：

-   輸入 **[虛擬機器名稱]**。
-   選取適當的 **[大小]**。
    -   在實際執行環境 (SharePoint 應用程式伺服器和資料庫) 中，建議使用 [A3] *(4 核心、7GB 記憶體)*。
-   在 [新使用者名稱] 中，輸入本機系統管理員帳戶名稱。
-   在 **[新密碼]** 方塊中，輸入強式密碼。
-   在 [確認] 方塊中重新輸入密碼，然後按一下向右箭頭。

<li>
第二個 [虛擬機器組態] 對話方塊隨即出現。請提供下列資訊：

<ul>
<li>
在 [雲端服務] 方塊中，選擇下列其中一項：

-   **建立新的雲端服務**：在這種情況下，您還必須提供雲端服務 DNS 名稱。
-   選取現有的雲端服務。
...
-   您的儲存體帳戶限制為 20 個。
-   如需詳細資訊，請移至[在 Azure 中建立儲存體帳戶][在 Azure 中建立儲存體帳戶]。

</li>
<li>
在 [可用性設定組] 方塊中，選取 [(無)]****，然後按一下向右箭頭。
...
<li>
在第三個 [虛擬機器組態] 對話方塊中，按一下核取記號以建立 VM。
...
-   依照前述的步驟 1 到 7 來建立 SQL Server 2012 VM，但**步驟 3** 除外；請使用 SQL Server 2012 映像，而不要使用 Windows Server 2008 R2 SP1 映像。如需詳細資訊，請移至[在 Azure 上佈建 SQL Server 虛擬機器][在 Azure 上佈建 SQL Server 虛擬機器]。
...
-   使用 SQL Server 系統準備 (SysPrep) 工具，在基礎作業系統的 VM 上安裝 SQL Server (如前述步驟 1 到 7 所說明)。如需詳細資訊，請移至[使用 SysPrep 安裝 SQL Server 2012][使用 SysPrep 安裝 SQL Server 2012]。
-   使用命令提示字元安裝 SQL Server。如需詳細資訊，請移至[使用命令提示字元安裝 SQL Server 2012][使用命令提示字元安裝 SQL Server 2012]。
-   使用支援的 SQL Server 媒體和您的授權金鑰，在基礎作業系統的 VM 上安裝 SQL Server (如前述步驟 1 到 7 所說明)。

</li>
<li>
使用下列子步驟建置 SharePoint 伺服器陣列：
...
-   子步驟 2：以基礎作業系統建立另一個 VM (如前述步驟 1 到 7 所說明)，以佈建 SharePoint 伺服器。若要在此 VM 上建置 SharePoint 伺服器，請選擇下列其中一項：
    -   使用 SharePoint GUI 進行佈建：
        -   若要建立及佈建 SharePoint 伺服器陣列，請移至[建立 Microsoft SharePoint Server 伺服器陣列][建立 Microsoft SharePoint Server 伺服器陣列]。
        -   若要在伺服器陣列中新增 Web 或應用程式伺服器，請移至[新增 Web 或應用程式伺服器至伺服器陣列 (SharePoint Server 2010)][新增 Web 或應用程式伺服器至伺服器陣列 (SharePoint Server 2010)]。
        -   若要在現有的伺服器陣列中新增資料庫伺服器，請移至[新增資料庫伺服器至現有的伺服器陣列][新增資料庫伺服器至現有的伺服器陣列]。

            -   若要將 SQL Server 2012 用於您的 SharePoint 伺服器陣列，您必須在安裝應用程式並選擇不設定伺服器之後，安裝 SharePoint Server 2010 的 Service Pack 1。如需詳細資訊，請移至 [SharePoint Server 2010 的 Service Pack 1][SharePoint Server 2010 的 Service Pack 1]。
            -   若要使用 SQL Server BI 功能，建議您將 SharePoint Server 安裝為伺服器陣列，而不是獨立伺服器。如需詳細資訊，請移至[安裝 SQL Server 2012 Business Intelligence 功能][安裝 SQL Server 2012 Business Intelligence 功能]。

    -   使用 Microsoft Windows PowerShell 進行佈建：您可以將 Psconfig 命令列工具作為替代介面，用以執行幾項控制 SharePoint 2010 產品之佈建方式的作業。如需詳細資訊，請移至 [Psconfig 命令列參照][Psconfig 命令列參照]。
-   子步驟 3：設定 SharePoint。在每個 SharePoint VM 都處於就緒狀態後，請使用下列其中一項，在每部伺服器上設定 SharePoint Server：
    -   從 GUI 設定 SharePoint。
    -   使用 Windows PowerShell 設定 SharePoint。如需詳細資訊，請移至[使用 Windows PowerShell 安裝 SharePoint Server 2010][使用 Windows PowerShell 安裝 SharePoint Server 2010]。
        -   您也可以使用 CodePlex 專案的 AutoSPInstaller；其中包含 Windows PowerShell 指令碼、一個 XML 輸入檔，和一個標準 Microsoft Windows 批次檔。AutoSPInstaller 會根據 Windows PowerShell 提供適用於 SharePoint 2010 安裝指令碼的架構。如需詳細資訊，請移至 [CodePlex：AutoSPInstaller][CodePlex：AutoSPInstaller]。 **注意**：請務必設定管理入口網站端點的安全性，並在 VM 的 Windows 防火牆上設定輸入連接埠。接著，請以系統管理員認證開啟 Windows PowerShell 工作階段，以確認您可以對其中一個 SharePoint 應用程式伺服器啟動遠端 Windows PowerShell 工作階段。

</li>
<li>
指令碼執行完成後，請使用 VM 儀表板連接到 VM。

</li>
<li>
驗證 SharePoint 組態：登入 SharePoint 伺服器，然後使用「管理中心」來驗證組態。
...
1.  建立具有 Hyper-V 功能的映像：使用 Hyper-V 管理員建立具有 Hyper-V 功能的 VHD。如需詳細資訊，請移至[建立虛擬硬碟][建立虛擬硬碟]。
2.  在 Azure 中建立儲存體帳戶：必須要有 Azure 中的儲存體帳戶，才能上傳可用來建立 VM 的 VHD 檔案。此帳戶可使用 Azure 管理入口網站來建立。如需詳細資訊，請移至[在 Azure 中建立儲存體帳戶][建立和上傳虛擬硬碟]。
3.  準備要上傳的映像：映像在上傳至 Azure 之前，必須使用 SysPrep 命令進行一般化。如需詳細資訊，請移至[如何使用 SysPrep：簡介][如何使用 SysPrep：簡介] (英文)。
4.  將映像上傳至 Azure：若要上傳 VHD 檔案中包含的映像，您必須建立並安裝管理憑證。取得憑證的指模和訂閱 ID。設定連線，並使用 CSUpload 命令列工具上傳 VHD 檔案。如需詳細資訊，請移至[將映像上傳至 Azure][建立和上傳虛擬硬碟]。
...
1.  *佈建*：首先，請使用 Azure 虛擬網路佈建內部部署與 Azure 之間的 VPN 連線。(由於此處不會使用 Active Directory，因此需要有 VPN 通道。)如需詳細資訊，請移至 [Azure 虛擬網路 (設計考量和安全連線情境)][Azure 虛擬網路 (設計考量和安全連線情境)]。接著，請在管理入口網站上使用映像庫中儲存的映像佈建新的 VM。
    -   您可以將內部部署的 SharePoint 開發與測試 VM 上傳至您的 Azure 儲存體帳戶，並透過映像庫參照這些 VM，以建置必要的環境。
    -   您可以使用 SQL Server 2012 映像，而不使用 Windows Server 2008 R2 SP1 映像。如需詳細資訊，請移至[在 Azure 上佈建 SQL Server 虛擬機器][在 Azure 上佈建 SQL Server 虛擬機器]。

2.  *安裝*：使用遠端桌面連線，在 VM 上安裝 SharePoint Server、Visual Studio 和 SQL Server。
    -   選擇 SharePoint Server 的安裝選項：
        -   使用 SharePoint 2010 的檢視設定指令碼 (Easy Setup Script) 建置 SharePoint 開發人員機器。如需詳細資訊，請移至 [SharePoint 2010 簡易設定指令碼][SharePoint 2010 簡易設定指令碼]。
        -   使用 Windows PowerShell。如需詳細資訊，請移至[使用 Windows PowerShell 安裝 SharePoint Server 2010][使用 Windows PowerShell 安裝 SharePoint Server 2010]。
        -   使用 CodePlex 專案的 AutoSPInstaller。如需詳細資訊，請移至 [CodePlex：AutoSPInstaller][CodePlex：AutoSPInstaller]。
    -   安裝 Visual Studio。如需詳細資訊，請移至 [Visual Studio 安裝][Visual Studio 安裝]。
    -   安裝 SQL Server。如需詳細資訊，請移至[使用 SysPrep 安裝 SQL Server][使用 SysPrep 安裝 SQL Server 2012]。
        -   參考為 SharePoint 伺服器陣列部署建立及設定 SQL Server 2012 的實際操作實驗室。[為 Azure 中的 SharePoint 設定 SQL Server 2012][為 Azure 中的 SharePoint 設定 SQL Server 2012]。
        -   參考藉由設定 Active Directory 及使用單一 SQL Server 資料庫來建立 SharePoint 伺服器陣列的實際操作實驗室。[使用 Azure 虛擬機器部署 SharePoint 伺服器陣列][使用 Azure 虛擬機器部署 SharePoint 伺服器陣列]。
...
除此之外，使用 Azure 虛擬機器也可讓組織輕鬆設定執行於 VM 的執行環境和實際執行環境。在 Azure 中建立的 SharePoint 公用端 VM 可備份至虛擬儲存體。此外，可供災害復原之用的「連續地理區域複寫」功能，可讓組織將執行於某個資料中心的 VM 自動備份至遠處的另一個資料中心。(如需地理區域複寫的詳細資訊，請移至 [Azure 儲存體的地理區域複寫簡介][Azure 儲存體的地理區域複寫簡介])。
...
1.  *部署 Active Directory*：在 Azure 虛擬機器上部署 Active Directory 的基本需求，類似於 (但不等同於) 在內部部署 VM (和某些實體機器) 上的部署。如需其差異的詳細資訊，以及相關指引和其他注意事項，請移至[在 Azure 虛擬機器上部署 Active Directory 的指引][在 Azure 虛擬機器上部署 Active Directory 的指引]。若要在 Azure 中部署 Active Directory：
    -   定義及建立可讓 VM 指派至特定子網路的虛擬網路。如需詳細資訊，請移至[設定虛擬網路][設定虛擬網路]。
    -   使用管理入口網站，在 Azure 中的新 VM 上建立及部署網域控制站。如需詳細資訊，請移至[部署及建立網域控制站][設定虛擬網路]。
        -   您也可以參照 Windows PowerShell 指令碼，使用 Azure 虛擬機器和虛擬網路在雲端中部署獨立網域。如需詳細資訊，請移至[在 Azure 中部署 Active Directory (Windows PowerShell)][在 Azure 中部署 Active Directory (Windows PowerShell)]。
        -   如需使用 Azure 虛擬網路在 VM 上建立新 Active Directory 樹系的詳細資訊，請移至[在 Azure 中安裝新的 Active Directory 樹系][在 Azure 中安裝新的 Active Directory 樹系]。

2.  *佈建 VM*：使用管理入口網站，從映像庫中儲存的映像佈建新的 VM。
3.  *部署 SharePoint 伺服器陣列*：
    -   使用新佈建的 VM 來安裝 SharePoint 及產生可重複使用的映像。如需安裝 SharePoint Server 的詳細資訊，請移至[使用 Windows PowerShell 安裝及設定 SharePoint Server 2010][使用 Windows PowerShell 安裝 SharePoint Server 2010] 或 [CodePlex：AutoSPInstaller][CodePlex：AutoSPInstaller]。
...
將現有的 BI 應用程式移轉至 Azure，可達到更有效的調整。藉由 SSAS、SSRS 和 SharePoint Server 的支援，組織將可建立功能強大、並且可向上或向下調整的 BI 和報告應用程式與儀表板。這些應用程式和儀表板也將可更安全地與內部部署資料和應用程式整合。Azure 可確保資料中心符合並支援 ISO 27001。如需詳細資訊，請移至 [Azure 信任中心][Azure 信任中心]。
...
1.  *佈建*：
    -   使用 Azure 虛擬網路，佈建內部部署與 Azure 之間的 VPN 連線。如需詳細資訊，請移至 [Azure 虛擬網路 (設計考量和安全連線情境)][Azure 虛擬網路 (設計考量和安全連線情境)]。
...
2.  *安裝*：如果您的組織沒有預先建置的 SharePoint Server 或 SQL Server BI 元件映像，請使用遠端桌面連線在 VM 上安裝 SharePoint Server 和 SQL Server。
    -   如需安裝 SharePoint 的詳細資訊，請移至[使用 Windows PowerShell 安裝 SharePoint Server 2010][使用 Windows PowerShell 安裝 SharePoint Server 2010] 或 [CodePlex：AutoSPInstaller][CodePlex：AutoSPInstaller]。
    -   如需安裝 SQL Server 的詳細資訊，請移至[使用 SysPrep 安裝 SQL Server][使用 SysPrep 安裝 SQL Server 2012]。
    -   參考為 SharePoint 伺服器陣列部署建立及設定 SQL Server 2012 的實際操作實驗室。[為 Azure 中的 SharePoint 設定 SQL Server 2012][為 Azure 中的 SharePoint 設定 SQL Server 2012]。
    -   參考藉由設定 Active Directory 及使用單一 SQL Server 資料庫來建立 SharePoint 伺服器陣列的實際操作實驗室。[使用 Azure 虛擬機器部署 SharePoint 伺服器陣列][使用 Azure 虛擬機器部署 SharePoint 伺服器陣列]。
...
2.  *部署 SQL Server*：
    -   使用管理入口網站，從映像庫中儲存的映像佈建新的 VM。
    -   在 VM 上設定 SQL Server。如需詳細資訊，請移至[使用 SysPrep 安裝 SQL Server][使用 SysPrep 安裝 SQL Server 2012]。
    -   將 VM 加入新建立的 Active Directory 網域中。

3.  *部署多伺服器 SharePoint 伺服器陣列*：
    -   建立虛擬網路。如需詳細資訊，請移至 [Azure 虛擬網路 (設計考量和安全連線情境)][Azure 虛擬網路 (設計考量和安全連線情境)]。
        -   在部署 SharePoint VM 時，您必須為 SharePoint Server 提供子網路，如此，本機 Active Directory 方塊中的 DNS 位址在佈建期間才可供使用。
    -   使用管理入口網站建立 VM。
    -   在此 VM 上安裝 SharePoint Server，並產生可重複使用的映像。如需安裝 SharePoint Server 的詳細資訊，請移至[使用 Windows PowerShell 安裝及設定 SharePoint Server 2010][使用 Windows PowerShell 安裝 SharePoint Server 2010] 或 [CodePlex：AutoSPInstaller][CodePlex：AutoSPInstaller]。
    -   設定要建立的 SharePoint VM，並使用 [Join-SharePointFarm][Join-SharePointFarm] 命令連接到 SharePoint 伺服器陣列。
    -   使用管理入口網站設定負載平衡：
        -   設定 VM 端點，選取要對現有端點的流量進行負載平衡的選項，然後指定負載平衡 VM 的名稱。
            -   如需在 Azure 虛擬機器上部署 SharePoint 伺服器陣列的詳細資訊，請檢視此 [TechEd North America 2012 視訊][TechEd North America 2012 視訊]。
...
-   適用於不同作業系統的命令列工具和 PowerShell Cmdlet
    ...
    [https://www.windowsazure.com/zh-tw/manage/downloads/][https://www.windowsazure.com/zh-tw/manage/downloads/]

-   使用說明和最佳作法文件

    [https://www.windowsazure.com/zh-tw/manage/windows/][https://www.windowsazure.com/zh-tw/manage/windows/]
