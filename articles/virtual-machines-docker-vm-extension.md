...
+ [如何透過 Azure 跨平台介面 (xplat-cli) 使用 Docker VM 延伸模組]
+ [如何使用 Docker VM 延伸模組與 Azure 入口網站]
...
|   功能      |Hypervisor| 容器  |
| :------------- |-------------| ----------- |
|程序隔離|至少會完成一部分|如果取得根，容器主機可能會洩漏|
|磁碟上必須有記憶體|完整作業系統和應用程式|僅限應用程式需求|
|啟動所需時間|明顯較長：作業系統的根和應用程式載入|明顯較短：只有應用程式需要啟動，因為核心已在執行中|
|容器自動化 |隨作業系統和應用程式而有明顯不同| [Docker 映像庫](https://registry.hub.docker.com/)；其他 
...
[如何使用 Docker VM 延伸模組與 Azure 入口網站]: http://azure.microsoft.com/documentation/articles/virtual-machines-docker-with-portal/
