...
+ [建立 Docker 憑證]
+ [新增 Docker VM 延伸模組]
+ [測試 Docker 用戶端與 Azure Docker 主機]
+ [後續步驟]

> [AZURE.NOTE] 本主題說明如何從 Azure 入口網站建立 Docker VM。若要查看如何在命令列建立 Docker VM，請參閱[如何從 Azure 跨平台介面 (xplat-cli) 使用 Docker VM 擴充程式](http://azure.microsoft.com/documentation/articles/virtual-machines-docker-with-xplat-cli/)。若要查看容器及其優點的高層級討論，請參閱 [Docker 高層級白板](http://channel9.msdn.com/Blogs/Regular-IT-Guy/Docker-High-Level-Whiteboard)。 

## <a id='createvm'>從映像庫建立新的 VM</a>
...
根據[使用 https 執行 Docker] 來建立 Docker 通訊的憑證和金鑰檔案，並將其置於用戶端電腦上的 **`~/.docker`** 目錄。 
...
若要新增 Docker VM 延伸模組，找到您所建立的 VM 執行個體，並向下捲動至 [**延伸模組**]，然後按一下 [延伸模組] 以開啟 VM 延伸模組，如下所示。
> [AZURE.NOTE] 此功能僅在預覽入口網站受到支援: https://portal.azure.com/

![](./media/virtual-machines-docker-with-portal/ClickExtensions.png)
### 新增擴充程式
按一下 [**+新增**] 以顯示可新增至此 VM 的可用 VM 延伸模組。 

![](./media/virtual-machines-docker-with-portal/ClickAdd.png)
### 選取 Docker VM 擴充程式
選擇 Docker VM 延伸模組，這會開啟 Docker 說明和重要連結，然後按一下底部的 [**建立**] 以開始安裝程序。
...
## <a id='testclientandserver'>測試 Docker 用戶端和 Azure Docker 主機</a>
尋找並複製 VM 的網域名稱，並在用戶端電腦的命令列中，輸入 `docker --tls -H tcp://`*dockerextension*`.cloudapp.net:4243 info` (其中， *dockerextension* 會被您 VM 的子網域取代)。 
...
您現在已可移至 [Docker 使用者指南]，並使用 Docker VM。如果您想透過命令列介面在 Azure VM 上自動建立 Docker主機，請參閱[如何從 Azure 跨平台介面 (xplat-cli) 使用 Docker VM 延伸模組](http://azure.microsoft.com/documentation/articles/virtual-machines-docker-with-xplat-cli/)
...
<!--Link references-->
[如何透過 Azure 跨平台介面 (xplat-cli) 使用 Docker VM 延伸模組]: http://azure.microsoft.com/documentation/articles/virtual-machines-docker-with-xplat-cli/
