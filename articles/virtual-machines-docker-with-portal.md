...
> [AZURE.NOTE] 本主題說明如何從 Azure 入口網站建立 Docker VM。若要查看如何在命令列建立 Docker VM，請參閱[如何從 Azure 跨平台介面 (xplat-cli) 使用 Docker VM 擴充程式]。若要查看容器及其優點的高層級討論，請參閱 [Docker 高層級白板](http://channel9.msdn.com/Blogs/Regular-IT-Guy/Docker-High-Level-Whiteboard)。 
...
您現在已可移至 [Docker 使用者指南]，並使用 Docker VM。如果您想透過命令列介面在 Azure VM 上自動建立 Docker主機，請參閱[如何從 Azure 跨平台介面 (xplat-cli) 使用 Docker VM 延伸模組]
