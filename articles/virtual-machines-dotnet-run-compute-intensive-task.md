...
本教學課程假設您知道如何建立 .NET 主控台應用程式。未假設您具備任何 Azure 相關知識。 
...
10. 按一下 [**建立虛擬機器**]。系統隨即會開始建立虛擬機器。您可以在管理入口網站的 [**虛擬機器**] 區段中監視狀態。當狀態顯示為 [**作用中**] 時，您即可登入該虛擬機器。
...
5.  確定命名空間名稱可用之後，請選擇要代管命名空間的區域 (務必使用要代管虛擬機器的相同區域)。
    ...
    <strong>重要事項</strong>
    <p>挑選您使用或打算使用於虛擬機器的**相同區域**。這樣可以獲得最佳效能。</p>
    </div>
...
4.  在此對話方塊中，找出 [**預設簽發者**] 和 [**預設金鑰**] 項目。請記下這些值，接下來會利用此資訊對命名空間執行作業。 

<h2>如何建立 .NET 應用程式以便執行需密集運算的工作</h2>

1. 在您的開發電腦 (不一定要是您所建立的虛擬機器) 上，下載 [Azure SDK for .NET](http://www.windowsazure.com/zh-tw/develop/net/)。
...
6. 您將需要在執行 TSP 求解器排列之前，先建立服務匯流排佇列。請執行下列命令來建立服務匯流排佇列：
...
除了使用 TSPSolver 建立或刪除佇列，您也可以使用 [Azure 管理入口網站]來建立或刪除佇列(https://manage.windowsazure.com)。請造訪管理入口網站的服務匯流排區段，存取用於建立或刪除佇列的使用者介面，以及擷取連接字串、發行者和存取金鑰。您也可以檢視服務匯流排佇列的儀表板，進而檢視連入和連出訊息的度量。 
