...
6.  修改 **your\_service\_bus\_namespace**、**your\_service\_bus\_owner** 和 **your\_service\_bus\_key** 預留位置，可分別使用您的服務匯流排 **[namespace]**、**[Default Issuer]** 和 **[Default Key]** 值。
...
1.  在開發電腦上，以 **TSPClient** 作為專案名稱來建立 .NET 主控台應用程式。確定已針對 **NET Framework 4** (而非 **.NET Framework 4 用戶端設定檔**) 設定目標 Framework。執行下列動作，即可在您建立專案後設定此目標 Framework：在 Visual Studio 的功能表中，依序按一下 **[專案]**、**[屬性]**、**[應用程式]** 索引標籤，然後設定 **[目標 Framework]** 的值。
2.  加入 Microsoft ServiceBus 程式庫中。在 Visual Studio 方案總管中，於 **[TSPSolver]** 上按一下滑鼠右鍵，按一下 **[新增參考]**、按一下 **[瀏覽]** 索引標籤，瀏覽至 **C:\\Program Files\\Microsoft SDKs\\Windows Azure.NET SDK\\2012-06\\ref** 並選取 **Microsoft.ServiceBus.dll** 作為參考。
...
5.  修改 **your\_service\_bus\_namespace**、**your\_service\_bus\_owner** 和 **your\_service\_bus\_key** 預留位置，可分別使用您的服務匯流排 **[namespace]**、**[Default Issuer]** 和 **[Default Key]** 值。
...
執行需密集運算的應用程式，首先建立佇列，然後解決 Traveling Saleseman Problem，以將目前最佳路由新增至服務匯流排佇列。當需密集運算的應用程式正在執行時 (或執行後)，執行用戶端以顯示服務匯流排佇列的結果。
...
4.  利用下列內容建立名為 **c:\\TSP\\cities.txt** 的檔案：
...
5.  在命令提示字元中，將目錄變更為 c:\\TSP。
6.  您必須在執行 TSP solver 排列之前，先建立服務匯流排佇列。執行下列命令以建立服務匯流排佇列：

         TSPSolver createqueue

7.  現在已建立佇列，您即可執行 TSP solver 排列。例如，執行下列命令以對 8 個城市執行 solver。

         TSPSolver 8

 如果您未指定數字，此 solver 將會針對 10 個城市執行。當 solver 找到目前最短路由時，便會將這些路由新增至佇列。

Solver 將會持續執行，直到其完成所有路由的檢查為止。

> [WACOM.NOTE] 您指定的數字愈大，Solver 執行的時間就會愈長。例如，針對 14 個城市執行可能需要幾分鐘的時間，而針對 15 個城市執行可能需要幾小時的時間。增加至 16 個 (含) 以上的城市會導致數天的執行時間 (最終是數週、數月和數年)。這是由於 Solver 所評估的排列數目會隨著城市數目的增加而急速增加。

### 如何執行監控用戶端應用程式

1.  登入您將執行用戶端應用程式的電腦。這不一定是執行 **TSPSolver** 應用程式的同一部電腦，但是有此可能。
2.  建立您將執行應用程式的資料夾。例如，**c:\\TSP**。
...
5.  執行以下命令：
    ...
    或者，您可以傳入命令列引數，以指定檢查佇列之間的睡眠分鐘數。檢查佇列的預設睡眠期間為 3 分鐘，若未將任何命令列引數傳遞至 **TSPClient**，便會使用預設值。如果想要使用不同的睡眠間隔值 (例如，一分鐘)，請執行：

         TSPClient 1

    用戶端將會持續執行，直到其看見 [完成] 佇列訊息為止。請注意，如果您在未執行用戶端的情況下，多次執行 Solver，則必須執行用戶端很多次才能完全清空佇列。此外，您可以刪除佇列，然後重新建立一次。若要刪除佇列，請執行下列 **TSPSolver** (而非 **TSPClient**) 命令：
