...
本文說明如何建立及上傳包含 FreeBSD 作業系統的虛擬硬碟 (VHD)，以便用它做為您自己的映像，在 Azure 中建立虛擬機器。 

##必要條件##
本文假設您具有下列項目：

- **Azure 訂用帳戶** - 如果您沒有帳戶，只需要幾分鐘的時間就可以建立免費試用帳戶。如需詳細資訊，請參閱[建立 Azure 帳戶](http://azure.microsoft.com/documentation/articles/php-create-account/)。 
...
2. **啟用 SSH**
    ...
    SSH 從光碟片安裝之後會預設為啟用。如果不是，或您直接使用 FreeBSD VHD，請輸入：
...
6. **安裝 Azure 代理程式**
    ...
    最新版的 Azure 代理程式一律可在 [github](https://github.com/Azure/WALinuxAgent/releases) 上找到。2.0.10 版和更新版本正式支援 FreeBSD 10 和更新的版本。
...
1. 從您在上一個步驟使用的 Azure PowerShell 視窗中，輸入：
    ...
    **重要事項**：現在請使用 Linux 作為作業系統，因為目前的 Azure PowerShell 版本只接受 "Linux" 或 "Windows" 做為參數。
