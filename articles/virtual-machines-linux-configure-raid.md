...
2.  **SLES 和 openSUSE** - 啟用 boot.md 並建立 mdadm.conf
...
5.  選用參數
    ...
    許多散發套件 (包括 `nobootwait` 或 `nofail`) 掛接參數可能會被新增至 /etc/fstab 檔案。這些參數容許發生掛接特定檔案系統失敗，並容許 Linux 系統繼續開機，即使它無法正確地掛接 RAID 檔案系統。請查閱散發套件的文件，以取得這些參數的相關資訊。
