...
-   [準備執行 Azure 的 CentOS 6.x 虛擬機器][準備執行 Azure 的 CentOS 6.x 虛擬機器]
-   [準備執行 Azure 的 CentOS 7.0+ 虛擬機器][準備執行 Azure 的 CentOS 7.0+ 虛擬機器]

## 必要條件

本文假設您已將 CentOS (或類似的衍生物件) Linux 作業系統安裝到虛擬硬碟。有多項工具可用來建立 .vhd 檔案，例如 Hyper-V 的虛擬化解決方案。如需相關指示，請參閱[安裝 Hyper-V 角色及設定虛擬機器][安裝 Hyper-V 角色及設定虛擬機器]。
...
-   因為 Linux Kernel 2.6.37 以下的版本的一個錯誤，較大的 VM 不支援 NUMA。這個問題主要會影響使用上游 Red Hat 2.6.32 Kernel 的散發套件。手動安裝 Azure Linux 代理程式 (waagent) 將會自動停用 Linux Kernel GRUB 組態中的 NUMA。您可以在以下步驟中找到與此有關的詳細資訊。
...
8.  **僅限 CentOS 6.3**：安裝 Linux Integration Services 的驅動程式
    ...
    **重要事項：此步驟僅適用於 CentOS 6.3 和較舊的版本。**在 CentOS 6.4+ 中，Linux Integration Services *已是核心中的可用項目*。
    ...
    a) 從 [Microsoft 下載中心][Microsoft 下載中心]取得包含 Linux Integration Services 驅動程式的 .iso 檔案。
...
16. 在 Hyper-V 管理員中，依序按一下 [動作] -\> [關閉]。您現在可以將 Linux VHD 上傳至 Azure。
  ...
  [Microsoft 下載中心]: http://www.microsoft.com/zh-tw/download/details.aspx?id=41554
