<properties urlDisplayName="Upload a Linux VHD" pageTitle="在 Azure 中建立及上傳 Linux VHD" metaKeywords="Azure VHD, uploading Linux VHD" description="Learn to create and upload an Azure virtual hard disk (VHD) that contains a Linux operating system." metaCanonical="" services="virtual-machines" documentationCenter="" title="Creating and Uploading a Virtual Hard Disk that Contains a Linux Operating System" authors="szarkos" solutions="" manager="timlt" editor="tysonn" />
...
**重要事項**：只有在使用其中一個[背書散發套件]時，Azure 平台 SLA 才適用於執行 Linux OS 的虛擬機器(../virtual-machines-linux-endorsed-distributions)。Azure 映像庫中提供的所有 Linux 散發套件，皆為使用必要組態的背書散發套件。
...
基於這個原因，我們建議您盡可能從其中一個 [Azure 背書散發套件上的 Linux] 開始著手(../linux-endorsed-distributions) 。下列文章會逐步引導您如何準備 Azure 支援的各種 Linux 背書散發套件：
...
如果您打算執行 Red Hat Enterprise Linux 版本 **6.0-6.3** 的變體，則您必須安裝 Hyper-V 的最新 LIS 驅動程式。您可以[在此位置]找到這些驅動程式(http://go.microsoft.com/fwlink/p/?LinkID=254263&clcid=0x409)。從 RHEL **6.4+** (及衍生物件) 開始，核心已隨附 LIS 驅動程式，因此無需額外的安裝套件，即可在 Azure 上執行這些系統。
...
[Azure Linux 代理程式](../virtual-machines-linux-agent-user-guide) (waagent) 在要於 Azure 中正確佈建 Linux 虛擬機器時，是必要選項。您可以在 [Linux 代理程式 GitHub 儲存機制]中取得最新的版本、檔案問題或提交提取要求(https://github.com/Azure/WALinuxAgent)。
...
- 安裝 Azure Linux 代理程式
	...
	如需在 Azure 上佈建 Linux 映像，您需要 Azure Linux 代理程式。許多散發套件以 RPM 或 Deb 套件 (此套件通常稱為 'WALinuxAgent' 或 'walinuxagent') 的形式提供代理程式。您也可以遵循 [Linux 代理程式指南]中的步驟來手動安裝代理程式(../virtual-machines-linux-agent-user-guide)。
