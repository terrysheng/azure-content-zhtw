...
Azure 映像庫上的配送映像是由下列合作夥伴提供，而我們仍繼續與不同的 Linux 社群合作來提供更多經過背書的配送映像。同時，針對映像庫中未提供的散發套件，您可以隨時依照[本頁][本頁]上的方針，帶自己的 Linux 來上班。
...
下表顯示不同的配送版本、Linux Integration Services (LIS) 驅動程式，以及經測試過可在 Azure 上運作的 Azure Linux 代理程式版本。依預設，LIS 驅動程式會內建於散發套件的核心，或您可以自[此處][此處]取得。Linux 代理程式版本可從散發套件的封裝儲存機制或在 [Github][Github] 上取得。

有些散發套件/核心版本需要有 [Linux 核心相容性修補程式][Linux 核心相容性修補程式]才能在 Azure 中適當運作，在此情況下，表中也包含這類修補程式的連結。
...
<td>
[只有 12.04 或 12.04.01 才需要][只有 12.04 或 12.04.01 才需要]

</td>
<td>
套件：在 walinuxagent 下的封裝儲存機制中
 來源:[GitHub][GitHub]
...
<td>
CentOS 6.3：[LIS 驅動程式][此處]；CentOS 6.4+ 驅動程式：在核心中

</td>
<td>
[只有 6.3 才需要][只有 6.3 才需要]

</td>
<td>
封裝：在 walinuxagent 下的 [Open Logic 封裝儲存機制][Open Logic 封裝儲存機制] (英文) 中
 來源:[GitHub][GitHub]
...
<td>
套件：在 [Cloud:Tools][Cloud:Tools] (英文) 存放庫中，名稱：WALinuxAgent
 原始程式碼：[GitHub][GitHub]
...
<sup>1</sup> **注意：**Azure 上的 CoreOS 目前已經進入開發人員預覽階段 (*alpha*)。
