<properties pageTitle="登入在 Azure 中執行 Linux 的虛擬機器" description="了解如何登入執行 Linux 的 Azure 虛擬機器，方法是使用安全殼層 (SSH) 用戶端。" services="virtual-machines" documentationCenter="" authors="KBDAzure" manager="timlt" editor=""/>
...
4. 使用您在建立機器時指定的帳戶登入虛擬機器。如需建立使用使用者名稱和密碼之虛擬機器的詳細資訊，請參閱[建立執行 Linux 的虛擬機器](http://azure.microsoft.com/zh-tw/documentation/articles/virtual-machines-linux-tutorial/)。 
