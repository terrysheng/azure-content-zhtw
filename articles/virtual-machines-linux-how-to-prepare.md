...
-   **以 .vhd 檔案安裝的 Linux 作業系統。** - 您已將支援的 Linux 作業系統安裝至虛擬硬碟。有多項工具可用來建立 .vhd 檔案。您可以使用虛擬化解決方案 (例如 Hyper-V) 建立 .vhd 檔案，並安裝作業系統。如需指示，請參閱[安裝 Hyper-V 角色及設定虛擬機器](http://technet.microsoft.com/en-us/library/hh846766.aspx)。
    ...
    **重要事項**：Azure 不支援較新的 VHDX 格式。您可以使用 Hyper-V 管理員或 convert-vhd Cmdlet，將磁碟轉換為 VHD 格式。
    ...
    如需背書散發套件清單，請參閱 [Linux on Azure 背書散發套件](../linux-endorsed-distributions)。或者，請參閱本文最後一節中的[非背書散發套件的資訊](#nonendorsed)。
...
Azure Linux 代理程式 (Waagent) 與 NetworkManager 不相容。網路組態應使用 ifcfg-eth0 檔案，且應可透過 ifup/ifdown 指令碼來控制。如果偵測到 NetworkManager 封裝，Waagent 將會拒絕安裝。

由於 2.6.37 以下的 Linux 核心版本有錯誤，因此不支援 NUMA。若安裝 waagent，將會在 Linux 核心命令列的 GRUB 組態中自動停用 NUMA。

要使用 Azure Linux 代理程式，必須要安裝 python-pyasn1 封裝。

建議您不要在安裝期間建立 SWAP 磁碟分割。您可以使用 Azure Linux 代理程式來設定 SWAP 空間。此外，也建議您不要對 Azure 虛擬機器使用未在 [Microsoft 網站](http://go.microsoft.com/fwlink/?LinkID=253692&clcid=0x409) 上提供修補程式的主流 Linux 核心 (許多目前的散發套件/核心可能以包含此修正程式)。
...
7.  CentOS 6.2 或 6.3：安裝 Linux Integration Services 的驅動程式

    **注意：**此步驟僅適用於 CentOS 6.2 和 6.3。在 CentOS 6.4+ 中，Linux Integration Services 已是核心中的可用項目。

    a) 從[下載中心](http://www.microsoft.com/en-us/download/details.aspx?id=34603)取得包含 Linux Integration Services 之驅動程式的 .iso 檔案。
...
9.  將其 /etc/yum.repos.d/CentOS-Base.repo 檔案取代為下列文字
...
10. 將下列幾行新增至 /etc/yum.conf

        http_caching=packages
        exclude=kernel*

11. 編輯檔案 "/etc/yum/pluginconf.d/fastestmirror.conf" 以停用 yum 模組 "fastestmirror"，然後在 [main] 下方輸入下列內容

        set enabled=0

12. 執行下列命令，以清除目前的 yum 中繼資料：

        yum clean all

13. 針對 CentOS 6.2 和 6.3，執行下列命令以更新執行中 VM 的核心
...
14. 在 grub 中修改核心開機行，以加入下列參數。這樣也可確保主控台訊息能傳送至第一個序列埠，以協助 Azure 支援偵錯問題：

        console=ttyS0 earlyprintk=ttyS0 rootdelay=300

15. 確定所有掛接在核心中的 SCSI 裝置都具有 300 秒或更長的 I/O 逾時。

16. 在 /etc/sudoers 中註解化以下一行 (如果存在)：

        Defaults targetpw

17. 確定 SSH 伺服器已安裝，並設定成在開機時啟動

18. 執行下列命令以安裝 Azure Linux 代理程式

        yum install WALinuxAgent

19. 請不要在 OS 磁碟上建立交換空間
...
3.  取代您的映像中目前的儲存機制，以使用具有核心的 azure 儲存機制，以及您升級 VM 時所需的代理程式封裝。此步驟會隨著 Ubuntu 版本而略有不同。
    ...
    在編輯 /etc/apt/sources.list 前，建議先備份 sudo cp /etc/apt/sources.list /etc/apt/sources.list.bak
...
6.  在 Grub 或 Grub2 中修改核心開機行，以加入下列參數。這樣也可確保主控台訊息能傳送至第一個序列埠，以協助 Azure 支援偵錯問題：
...
### 準備 SUSE Linux Enterprise Server 11 SP2 和 SP3 作業系統

**注意：** [SUSE Studio](http://www.susestudio.com) 可讓您輕鬆建立及管理 Azure 和 Hyper-V 的 SLES/opeSUSE 映像。此外，您也可以將 SUSE Studio Gallery 中的下列正式映像下載或複製到您本身的 SUSE Studio 帳戶，以便進行自訂：

> -   [SUSE Studio Gallery 上的 SLES 11 SP2 for Azure](http://susestudio.com/a/02kbT4/sles-11-sp2-for-windows-azure)
...
3.  在 Grub 中修改核心開機行，以加入下列參數。這樣也可確保主控台訊息能傳送至第一個序列埠，以協助 Azure 支援偵錯問題：
...
### 準備 openSUSE 12.3 作業系統

**注意：** [SUSE Studio](http://www.susestudio.com) 可讓您輕鬆建立及管理 Azure 和 Hyper-V 的 SLES/opeSUSE 映像。此外，您也可以將 SUSE Studio Gallery 中的下列正式映像下載或複製到您本身的 SUSE Studio 帳戶，以便進行自訂：

> -   [SUSE Studio Gallery 上的 openSUSE 12.3 for Azure](http://susestudio.com/a/02kbT4/opensuse-12-3-for-windows-azure)
...
4.  在 Shell 上執行命令 '`zypper lr`'。如果此命令傳回
...
7.  在 Grub 中修改核心開機行，以加入下列參數。這樣也可確保主控台訊息能傳送至第一個序列埠，以協助 Azure 支援偵錯問題：

        console=ttyS0 earlyprintk=ttyS0 rootdelay=300

     然後，在 /boot/grub/menu.lst 中，移除核心命令列中的下列參數 (如果存在)：

        libata.atapi_enabled=0 reserve=0x1f0,0x8

1.  建議您將 /etc/sysconfig/network/dhcp 或對等項目從 DHCLIENT\_SET\_HOSTNAME="yes" 改設為 DHCLIENT\_SET\_HOSTNAME="no"

2.  在 /etc/sudoers 中註解化以下一行 (如果存在)：

        Defaults targetpw

3.  確定 SSH 伺服器已安裝，並設定成在開機時啟動
...
5.  執行下列命令，以取消佈建虛擬機器，並準備將其佈建於 Azure 上：

        waagent -force -deprovision
        export HISTSIZE=0
        logout

6.  確定 Azure Linux 代理程式會在啟動時執行：

        systemctl enable waagent.service

7.  在 Hyper-V 管理員中按一下 **[關閉]**。
...
2.  按一下命令列上的 **[新增]**。
    ...
    ![建立儲存體帳戶](./media/virtual-machines-linux-how-to-prepare/create.png)
...
正因如此，建議您從我們的其中一個[合作夥伴背書映像](https://www.windowsazure.com/en-us/manage/linux/other-resources/endorsed-distributions/)開始著手進行。

下列清單取代了讓您自行建立 VHD 之程序的步驟 1：

1.  您必須確定您所執行的核心包含 Hyper-V 的最新 LIS 驅動程式，或是您已成功加以編譯 (這些驅動程式採用開放原始碼)。驅動程式位於[此位置上](http://go.microsoft.com/fwlink/p/?LinkID=254263&clcid=0x409)
...
7.  您必須依照 [Linux 代理程式指南](https://www.windowsazure.com/en-us/manage/linux/how-to-guides/linux-agent-guide/)中的步驟安裝 Azure Linux 代理程式。此代理程式已在 Apache 2 授權下發行，您可以在[代理程式 GitHub 位置](http://go.microsoft.com/fwlink/p/?LinkID=250998&clcid=0x409)上取得最新版本
