...
[MySQL][] 是一種很受歡迎的開放原始碼 SQL 資料庫。在 [Azure 管理入口網站][]中，您可以從映像庫建立執行 OpenSUSE Linux 的虛擬機器。接著，就可以在虛擬機器上安裝和設定 MySQL 資料庫。
...
在本教學課程中，您已了解如何建立 OpenSUSE Linux 虛擬機器，以及使用 SSH 或 PuTTY 從遠端連接到此虛擬機器。您也學到如何在 Linux 虛擬機器上安裝和設定 MySQL。如需 MySQL 的詳細資訊，請參閱 [MySQL 文件][]。
