...
3. 按一下管理入口網站中的  *Capture*，或使用 Powershell 或 CLI 工具，將虛擬機器擷取為映像。
...
每個虛擬機器都會連接一個暫時性的本機 *resource disk* 。因為資源磁碟上的資料在重新開機之後就會消失，通常供虛擬機器中執行的應用程式和處理程序**暫時**儲存資料。也用來儲存作業系統的分頁檔或交換檔。
