<properties title="在 Linux 上使用負載平衡集合將 MySQL 叢集化" pageTitle="在 Linux 上使用負載平衡集合將 MySQL 叢集化" description="本文說明在 Azure 上設定負載平衡、高可用性 Linux 叢集的模式，使用 MySQL作為範例" metaKeywords="mysql, linux, cluster, azure, ha, high availability, corosync, pacemaker, drbd, heartbeat" services="virtual-machines" solutions="" documentationCenter="" authors="jparrel" videoId="" scriptId="" manager="timlt" />
...
如果您在這兩個 VM 上檢查 /proc/drbd (`sudo cat /proc/drbd`) 的內容，您應可在 `hadb01` 上看到 `Primary/Secondary`，以及在 `hadb02` 上看到 `Secondary/Primary`，直到這個階段都會與解決方案一致。客戶可免費透過 10.10.10.0/24 網路同步處理這個 5 GB 磁碟。
...
如果您不打算現在容錯移轉 DRBD，雖然第一個選擇可以說是較為繁瑣，但相對是比較容易的。安裝完成後，您可以開始使用 MySQL 資料庫。在 `hadb02` (或其中任何一個對 DRBD 而言為作用中伺服器) 上：
...
如果您想要在 VM 外進行查詢，您還必須啟用 MySQL 的網路功能，這正是本指南的目的。在這兩個 VM 上，開啟 `/etc/mysql/my.cnf` 並瀏覽至 `bind-address`，將它從 127.0.0.1 變更為 0.0.0.0。儲存檔案後，在您目前主要上發佈 `sudo service mysql restart`。
...
-   VM 互連中的寫入效能將會取決於虛擬開關，因為虛擬開關是 DRBD 用來複寫裝置的機制。
  ...
  [STONITH]: #stonith
  [限制]: #limitations
  [第 9 頻道]: http://channel9.msdn.com/Blogs/Open/Load-balancing-highly-available-Linux-services-on-Windows-Azure-OpenLDAP-and-MySQL
