...
使用者在 Linux 虛擬機器上可以透過 `sudo` 命令而以提高的權限來執行命令。不過，根據系統如何佈建而定，操作過程可能不同。

1.  **SSH 金鑰和密碼，或僅密碼** - 虛擬機器是以憑證 (`.CER` 檔案) 和密碼來佈建，或只以使用者名稱和密碼來佈建。在此情況下，執行命令之前，`sudo` 會提示輸入使用者的密碼。

2.  **僅 SSH 金鑰** - 虛擬機器是以憑證 (`.cer` 或 `.pem` 檔案) 來佈建，未使用密碼。在此情況下，執行命令之前，`sudo` **不會**提示輸入使用者的密碼。
...
在此例子中，將**不會**提示使用者輸入密碼。按下 `<enter>` 之後，`sudo` 會以 `root` 權限來執行命令。
