...
2. 使用 'openssl' 以 2048 位元 RSA 金鑰組來產生 X509 憑證。請回答 'openssl' 提示的一些問題 (您也可以將這些問題保留空白)。平台並未使用這些欄位的內容：
...
4.	建立 Linux 虛擬機器時上傳 `myCert.pem`。佈建程序會針對虛擬機器中的指定使用者，自動將此憑證中的公開金鑰安裝至 `authorized_keys` 檔案中。
