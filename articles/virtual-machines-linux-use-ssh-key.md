...
1.  在某些情況下，Linux 虛擬機器的 SSH 端點可設定為除了預設連接埠 22 以外的連接埠。您可以在管理入口網站的 VM 儀表板 (在 [SSH 詳細資料] 下) 上找到正確的連接埠號碼。
...
1.  從下列位置下載並安裝 msysgit：<http://msysgit.github.com/>
2.  從安裝的目錄執行 `msys` (範例：c:\\msysgit\\msys.exe)
3.  透過輸入 `cd bin` 來變更至 `bin` 目錄。

### 使用 GitHub for Windows

1.  從下列位置下載並安裝 GitHub for Windows：<http://windows.github.com/>
2.  從 [開始] 功能表 \> [所有程式] \> [GitHub, Inc] 執行 Git Shell

### 使用 cygwin

1.  從下列位置下載並安裝 Cygwin：<http://cygwin.com/>
...
1.  從下列位置下載並安裝 Puttygen：<http://www.chiark.greenend.org.uk/~sgtatham/putty/download.html>
...
1.  從下列位置下載並安裝 putty：<http://www.chiark.greenend.org.uk/~sgtatham/putty/download.html>
2.  執行 putty.exe
