...
當您建立 Linux 虛擬機器時，您可以自行選擇使用者名稱，或接受預設值 *azureuser*。在多數情況中，這位新使用者不會存在於基本映像中，而會在佈建程序中建立。若此使用者存在於基本 VM 映像，則 Azure Linux 代理程式只會在建立 VM 時，根據您在建立 VM 時所提供的資訊，為該使用者設定密碼 (及/或 SSH 金鑰)。

**但是**，Linux 會定義一組不應使用的使用者名稱。如果您嘗試使用現有的系統使用者 (定義為具有 UID 0-99 的使用者) 佈建 Linux VM，佈建程序將會**失敗**。常見的範例是 `root` 使用者，其 UID 為 0。
