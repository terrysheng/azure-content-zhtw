<properties
   pageTitle="在 Azure PowerShell 中使用服務管理模式時的一般 VM 工作"
   description="提供一些命令，處理當您在 Azure PowerShell 中使用服務管理模式時的一般 VM 工作"
...
# 在 Azure PowerShell 中使用服務管理模式時的一般 VM 工作
...
當您進行 VM 的日常管理時，可以在服務管理模式中使用 Azure PowerShell Cmdlet，許多的工作就會自動完成。這篇文章提供了幾個簡單工作的範例命令，另外也提供顯示用來完成更複雜的工作之命令的文章連結。
