...
3. 在 [**建立新的虛擬機器**] 頁面中選取含有 SQL Server 的虛擬機器映像，然後按一下頁面右下角的 [下一步] 箭頭。如需 Azure 支援的 SQL Server 映像最新的資訊，請參閱 [Azure 虛擬機器中的 SQL Server](http://go.microsoft.com/fwlink/p/?LinkId=294719) 文件集內的 [開始使用 Azure 虛擬機器中的 SQL Server](http://go.microsoft.com/fwlink/p/?LinkId=294720) 主題。 
...
4. 當您第一次登入這個虛擬機器時，可能需要完成數個程序，包括設定桌面、Windows Update，以及完成 Windows 初始組態工作 (sysprep)。待 Windows sysprep 完成後，SQL Server 安裝程式會完成組態工作。這些工作完成時，會導致幾分鐘的時間延遲。SQL Server 安裝程式完成時， `SELECT @@SERVERNAME` 才會傳回正確的名稱，而且 SQL Server Management Studio 才會出現在起始頁面。
...
7. 在 [**設定檔**] 對話方塊中選取 [**公用**]，然後按 [**下一步**]。 
    ...
    **安全性注意事項：**選取 [**公用**] 允許透過網際網路存取。請盡可能選取較嚴格的設定檔。
...
4. 在詳細資料窗格中以滑鼠右鍵按一下 [TCP]。依預設，組件庫映像的 TCP 應該是 [已啟用]。對於自訂映像，按一下 [**啟用**] (如果它的狀態是 [已停用])。

	![Enable TCP][Image10]

5. 在主控台窗格中，按一下 [**SQL Server 服務**]。(Database Engine 重新啟動可延至下個步驟完成時再進行。)

6. 在詳細資料窗格中，以滑鼠右鍵按一下 **SQL Server (_instance name_)** (預設執行個體是 **SQL Server (MSSQLSERVER)**)，然後按一下 [**重新啟動**]，以停止並重新啟動 SQL Server 執行個體。 
...
1. 在使用遠端桌面連接虛擬機器的情況下，於 [開始] 功能表中依序按一下 [**所有程式**]、[**Microsoft SQL Server _version_**] 及 [**SQL Server Management Studio**]。 

	![Start SSMS][Image18]

	當您首次開啟 Management Studio 時，它必須建立使用者 Management Studio 環境。這可能需要花費幾分鐘的時間。

2. Management Studio 開啟時，會出現 [**連接到伺服器**] 對話方塊。在 [**伺服器名稱**] 方塊中，輸入虛擬機器的名稱，以利用物件總管連接 Database Engine。除了虛擬機器名稱之外，您還可以使用 **(本機)**，也可以將一個句點當作 **伺服器名稱**。選取 [**Windows 驗證**]，然後在 [**使用者名稱**] 方塊中保留 **_your_VM_name_\your_local_administrator**。按一下 [**連接**]。
...
4. 在 [**登入**] 方塊中，輸入於先前工作中建立的登入名稱。

5. 在 [**密碼**] 方塊中，輸入於先前工作中建立的登入密碼。
