...
-   [連接 Azure 管理入口網站並從組件庫佈建虛擬機器][]
-   [使用遠端桌面開啟虛擬機器及完成設定][]
-   [完成在另一部電腦上使用 SQL Server Management Studio 連接虛擬機器的組態步驟][]
-   [後續步驟][]

## <span id="Provision"></span>連接 Azure 管理入口網站並從組件庫佈建虛擬機器</a>

1.  使用您的帳戶登入 [Azure 管理入口網站][]。如果您沒有 Azure 帳戶，請造訪 [Azure 免費試用][]。

2.  在 Azure 管理入口網站中，依序按一下網頁左下角的 [+NEW]、[運算]、[虛擬機器] 和 [從組件庫]。

3.  在 [Create a New Virtual Machine] 頁面中選取含有 SQL Server 的虛擬機器映像，然後按一下頁面右下角的 [下一步] 箭頭。如需 Azure 支援之 SQL Server 映像的最新資訊，請參閱 [Azure 虛擬機器中的 SQL Server][] 文件集內的[開始使用 Azure 虛擬機器中的 SQL Server][] 主題。

    > [WACOM.NOTE] 如果您擁有以 SQL Server Evaluation Edition 平台映像建立的虛擬機器，該虛擬機器無法升級為組件庫內以分鐘計費之版本的映像。您可以在以下兩個選項中選擇其中一項：

    > -   您可以使用組件庫內以分鐘計費的 SQL Server 版本來建立新虛擬機器，然後再遵循[如何使用資料磁碟在 Azure 虛擬機器之間移轉 SQL Server 資料庫檔案和結構描述][]中的步驟，將資料庫檔案移轉到這部新虛擬機器。**或**，

    > -   您可以遵循[升級為不同的 SQL Server 2014 版本 (安裝程式)][] 中的步驟，在 [Azure 上透過軟體保證的授權流動性][]協定的規範下，將現有 SQL Server Evaluation Edition 執行個體升級為不同版本的 SQL Server。如需如何購買授權版本之 SQL Server 的詳細資訊，請參閱[如何購買 SQL Server][]。

4.  在 [虛擬機器組態] 頁面，提供下列資訊：

    -   提供 [虛擬機器名稱]。
    -   在 [新使用者名稱] 方塊中，輸入 VM 本機系統管理員帳戶的唯一使用者名稱。
    -   在 **[新密碼]** 方塊中，輸入強式密碼。如需詳細資訊，請參閱[增強式密碼][]。
    -   在 **[確認密碼]** 方塊中，重新輸入密碼。
    -   從 [大小] 下拉式清單中選取適當的大小。
    ...
    > 為資料倉儲的映像使用 SQL Server 2012 Enterprise 時，請選取 A6。
    > 為資料倉儲的映像使用 SQL Server 2014 時，請選取 A7。
    > 選取的大小會限制您可以設定的資料磁碟數。如需可用之虛擬機器大小和可連接至虛擬機器之資料磁碟數目的最新資訊，請參閱[虛擬機器][]。
...
視需要為其他元件開啟額外的連接埠。如需詳細資訊，請參閱[設定 Windows 防火牆以允許 SQL Server 存取][]。
...
SQL Server Database Engine 須有網域環境才能使用 Windows 驗證。若要從另一部電腦連接 Database Engine，請設定 SQL Server 以進行混合模式驗證。混合模式驗證可允許 SQL Server 驗證和 Windows 驗證。如果您已設定 Azure 虛擬網路，可能就不需要設定混合模式驗證。如需詳細資訊，請參閱 [Azure 虛擬機器中的 SQL Server][2] 文件集內的 [Azure 虛擬機器中的 SQL Server 連接性考量][]。
...
如需 SQL Server 登入的詳細資訊，請參閱[建立登入][]。
...
如需詳細資訊，請參閱[如何疑難排解 SQL Server Database Engine 連線][] (英文)。

## <span id="Optional"></span>後續步驟</a>

您已了解如何使用平台映像來建立及設定 Azure 虛擬機器上的 SQL Server。在 Azure 虛擬機器中使用 SQL Server 時，建議您遵循文件庫中 [Azure 虛擬機器中的 SQL Server][] 文件的詳細指引。此文件集含有一系列提供詳細指引的文章和教學課程。這一系列的內容包括以下小節：

[Azure 虛擬機器中的 SQL Server][]

[開始使用 Azure 虛擬機器中的 SQL Server][]
...
[Azure 虛擬機器中 SQL Server 適用的技術文件][]

-   [白皮書：Azure 虛擬機器中的 SQL Server 應用程式模式和開發策略][]

-   [白皮書：在 Azure 虛擬機器中部署 SQL Server Business Intelligence][]

-   [白皮書：Azure 虛擬機器中的 SQL Server 效能指引][]

-   [白皮書：Reporting Services 報告檢視器控制項和 Microsoft Azure 虛擬機器型報表伺服器][]
  ...
  [按一下 新增端點]: ./media/virtual-machines-provision-sql-server/28Add-Endpoint.png
  ...
  [有端點的 VM]: ./media/virtual-machines-provision-sql-server/31VM-Connect.png
