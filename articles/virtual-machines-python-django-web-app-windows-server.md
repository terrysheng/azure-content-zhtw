<properties urlDisplayName="Web with Django (Windows)" pageTitle="搭配 Django 的 Python Web 應用程式 - Azure 教學課程" metaKeywords="Azure Django web app, Azure Django virtual machine" description="本教學課程會教您如何使用執行 Windows Server 2012 R2 Datacenter 的虛擬機器，在 Azure 上裝載 Django 型網站。" metaCanonical="" services="virtual-machines" documentationCenter="Python" title="Django Hello World Web 應用程式" authors="huvalo" solutions="" manager="wpickett" editor="" />
...
本教學課程說明如何使用執行 Windows Server 的虛擬機器，在 Microsoft
Azure 上裝載 Django 型網站。本教學課程假設您先前沒有使用 Azure 的經驗。完成本指南之後，您將在雲端啟動並執行 Django 型應用程式。
...
依照本教學課程的步驟，您將建置一個簡單的 Hello World
Web 應用程式。該應用程式將裝載於 Azure 虛擬機器中。
...
1.  按照[此處][此處] (英文) 所提供的指示，建立 *Windows Server 2012 R2 Datacenter* 散發套件的 Azure 虛擬機器。
...
## <span id="setup"></span> </a>設定 Python 和 Django

**注意：** 為了要使用 Internet Explorer 進行下載，您可能必須設定 IE ESC 設定 ([開始]/[系統管理工具]/[伺服器管理員]/[本機伺服器]，然後按一下 [IE 增強式安全性設定]，將其設為 [關閉])。

1.  安裝 [Web Platform Installer][Web Platform Installer]。
2.  使用 Web Platform Installer 安裝 Python 和WFastCGI。這將會在您的 Python 指令碼資料夾中安裝 wfastcgi.py。

    1.  啟動 Web Platform Installer。
    2.  在搜尋列中輸入 WFastCGI。
    3.  在您要使用的 Python 版本 (2.7 或 3.4) 中選取 WFactCGI 項目。請注意，這會將 Python 作為 WFastCGI 的相依性安裝。

3.  如果您已安裝 Python 2.7，則請[遵循這些指示手動安裝 pip][遵循這些指示手動安裝 pip] (Python 3.4 內含已安裝的 pip)。
4.  使用 pip 安裝 Django。
...
4.  設定 PYTHONPATH，讓 Python 解譯器能夠找到您的 Django 應用程式。
...
5.  告訴 FastCGI 到 WSGI 的閘道要使用哪個 WSGI 處理常式。
...
完成本教學課程時，請關閉並/或移除新建立的 Azure 虛擬機器釋出資源供其他教學課程使用，並避免產生 Azure 使用的費用。
  ...
  [遵循這些指示手動安裝 pip]: https://pip.pypa.io/en/latest/installing.html
