...
[觀看教學課程](http://go.microsoft.com/fwlink/?LinkID=254535&clcid=0x409) [播放影片](http://go.microsoft.com/fwlink/?LinkID=254535&clcid=0x409) 5:22
...
按照[此處](/en-us/manage/linux/tutorials/virtual-machine-from-gallery/)提供的指示建立 *OpenSUSE* 或 *Ubuntu* 散發套件。本教學課程使用 OpenSUSE 12.3 及 Ubuntu Server 13.04。我們假設預設使用者名稱是 *azureuser*。

### Windows VM

按照[此處](/en-us/manage/windows/tutorials/virtual-machine-from-gallery/)提供的指示建立 *Windows Server 2012 Datacenter* 散發套件的虛擬機器。在本教學課程中，我們假設使用者名稱是 *azureuser*。
...
若要建立端點，請移至 VM 儀表板，按一下 [端點] ，並按一下 [新增端點] ，然後建立新端點 (在此範例中稱為 `ipython_nb`)。選擇 TCP 通訊協定，為公用連接埠選擇 443，並為私人連接埠選擇 9999：
...
安裝 [NumPy](http://www.numpy.org/ "NumPy")、[Matplotlib](http://matplotlib.sourceforge.net/ "Matplotlib")、[Tornado](http://www.tornadoweb.org/ "Tornado") 以及 IPython 的其他相依性：
...
1.  從 [python.org](http://www.python.org/download) 安裝 Python 2.7.5 (32 位元)。您也需要將 `C:\Python27` 及 `C:\Python27\Scripts` 新增至您的 `PATH` 環境變數中。

2.  從 [python-distribute.org](http://python-distribute.org/) 下載 **distribute\_setup.py** 檔案，然後執行以下命令來安裝散發套件：

        python distribute_setup.py

3.  執行以下命令來安裝 [Tornado](http://www.tornadoweb.org/ "Tornado") 和 [PyZMQ](https://github.com/zeromq/pyzmq "PyZMQ")：

        easy_install tornado
        easy_install pyzmq

4.  使用這些網站上提供的 `.exe` 二進位安裝程式，下載並安裝 [NumPy](http://www.numpy.org/ "NumPy")。截至撰寫時為止，最新版為 **numpy-1.7.1-win32-superpack-python2.7.exe**。

5.  使用這些網站上提供的 `.exe` 二進位安裝程式，下載並安裝 [Matplotlib](http://matplotlib.sourceforge.net/ "Matplotlib")。截至撰寫時為止，最新版為 **matplotlib-1.2.1.win32-py2.7.exe**。

6.  下載並安裝 OpenSSL。您需要從 <http://slproweb.com/products/Win32OpenSSL.html> 

    -  安裝 **Win32 OpenSSL v1.0.1e Light** 和 **Visual C++ 2008 Redistributable**。
 

    - 您也需要將 `C:\OpenSSL-Win32\bin` 新增至您的 `PATH` 環境變數中。
...
2.  開啟 Windows 防火牆中的連接埠。在 Windows Server 2012 上，防火牆預設會阻擋傳入的連線。若要開啟連接埠 9999，請依照下列步驟執行：
...
您現在應該能夠透過位址 `https://[此處為您選擇的名稱].cloudapp.net` 存取您的 IPython Notebook。
...
標記有 `In []:` 提示的區域是輸入區域，您可以在其中輸入任何有效的 Python 程式碼，而且它會在您按 `Shift-Enter` 或按一下 [播放] 圖示 (工具欄中的向右三角形) 時執行。
...
Notebook 本身對曾經使用 Python 和文字處理器的任何人而言應該感覺相當自然，因為它在某些方面是這兩者的結合：您可以執行 Python 程式碼區塊，但是也可以使用工具列中的下拉式功能表，將儲存格的樣式從 [程式碼] 更改為 [Markdown] 來保留記事及其他文字：
...
如果您進入 IPython 原始程式碼儲存庫，將可找到提供 [Notebook 範例](https://github.com/ipython/ipython/tree/master/examples/notebooks)的整個目錄，您可以下載範例，然後在自己的 Azure IPython VM 上進行試驗。只需從網站下載 `.ipynb` 檔案，並將這些檔案上傳至您的 Notebook Azure VM 儀表板即可 (也可以將這些檔案直接下載至 VM)。
...
例外，也可以透過 [Python Tools for Visual Studio](http://pytools.codeplex.com) (PTVS) 在 Visual Studio 中使用 IPython 的核心功能。PTVS 是 Microsoft 提供的免費開放原始碼外掛程式，可以將 Visual Studio 轉變為進階 Python 開發環境，其中包括具有 IntelliSense、偵錯、剖析和平行運算整合功能的進階編輯器。
