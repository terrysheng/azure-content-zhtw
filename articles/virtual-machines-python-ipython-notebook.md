...
按照 [此處][portal-vm-windows] 提供的指示建立 *Windows Server 2012 R2 Datacenter* 散發套件。在本教學課程中，我們假設使用者名稱是 *azureuser*。
...
端點] 建立新的端點 (在此範例中稱為 `ipython_nb`)。選擇
...
安裝 [NumPy][numpy]、[Matplotlib][matplotlib]、[Tornado][tornado] 以及 IPython 的其他相依性，方法是進行：
...
1.  下載並安裝最新 32 位元版本的 [Python 2.7][]。您需要加入 `C:\Python27` and `C:\Python27\Scripts` 到您的 `PATH` 環境變數。

1.  安裝 [Tornado][tornado] 和 [PyZMQ][pyzmq] 以及其他 IPython 的相依性，方法是：
...
1.  使用可在網站上取得的 `.exe` 二進位安裝程式下載並安裝 [NumPy][numpy]。截至撰寫時為止，最新版為 **numpy-1.9.1-win32-superpack-python2.7.exe**。
...
1.  下載並安裝 [OpenSSL][]。
	...
	* 您需要加入 `C:\OpenSSL-Win32\bin` 到您的 `PATH` 環境變數。
