...
您將使用 Azure VM 中代管的 MySQL 找到 *World* 的替代，擴展 [Django Hello World][現代化商務的雲端] 範例。接著透過 MySQL 支援的 Django *counter* 應用程式決定替代。與 Hello World 範例一樣，Azure 虛擬機器將再次代管這個 Django 應用程式。
 ...
 * 在[這裡][MySQL Community Server]執行 *Windows (x86、64 位元) MSI 安裝程式*連結，並且從最鄰近的下載鏡像下載適當的 MSI 安裝程式。實用提示如下：
    -   選取*完成* 設定類型。
...
1.  按照 [Django Hello World][現代化商務的雲端] 教學課程中提供的指示，在 Django 中建立一個不起眼的「Hello World」Web 應用程式。
...
1.  取代 **C:\\django\\helloworld\\helloworld\\views.py** 的內容。以下新實作的 *hello* 函數搭配先前安裝的個別範例資料庫 *world* 使用 *Counter* 模型，產生 "*World*" 字串的適當取代：
...
1.  切換回 Windows PowerShell 視窗，並輸入下列命令，以對外公開部署 Django 網站：
    ...
    **runserver** 參數會指示 Django 在 TCP 連接埠 *80* 上執行 *helloworld* 網站。此命令的結果應該如下所示：
...
1.  若要停止 Django 代管網站，只要切換回 PowerShell 視窗並按下 **CTRL-C** 鍵即可。
