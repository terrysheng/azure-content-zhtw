...
本教學課程說明如何使用 Linux 虛擬機器，在 Azure 上代管 Ruby on Rails 型網站。本教學課程假設您先前沒有使用 Azure 的經驗。完成本教學課程後，您將在雲端啟動並執行 Ruby on Rails 型應用程式。
...
1.  在開發環境中安裝 Ruby。端視您的作業系統而定，步驟可能不同。
    ...
    -   **Apple OS X** - OS X 有多個 Ruby 散發套件。本教學課程使用 [Homebrew](http://brew.sh/) 安裝 **rbenv** 及 **ruby-build**，在 OS X 通過驗證。在 <https://github.com/sstephenson/rbenv/> 可找到安裝資訊。
...
3.  若要建立將儲存部落格貼文的資料庫，請使用下列命令：
    ...
    這將使用 Rails 的預設資料庫提供者，也就是 [SQLite3 Database](http://www.sqlite.org/)。雖然您可能會想要對於生產應用程式使用不同的資料庫，不過 SQLite 對於本教學課程的目的而言已經夠用。
...
本教學課程中的步驟是在代管 Ubuntu 12.10 的 Azure 虛擬機器上執行的。如果您使用不同的 Linux 散發套件，可能需要不同的步驟才能完成相同的工作。
...
1.  從命令列或終端機工作階段中，使用下列命令以 SSH 連線至虛擬機器：
    ...
    **注意**
    ...
    如果您使用 Windows 做為開發環境，您可以使用 **PuTTY** for SSH 功能之類的公用程式。PuTTY 可自 [PuTTY 下載頁面](http://www.chiark.greenend.org.uk/~sgtatham/putty/download.html) (英文) 下載取得。
...
2.  從 SSH 工作階段中，使用下列命令在 VM 上安裝 Ruby：
    ...
    這應該會傳回虛擬機器上安裝的 Ruby 版本，可能與 1.9.1 不同。例如，**ruby 1.9.3p194 (2012-04-20 revision 35410) [x86\_64-linux]**。
...
如果您使用 Windows 做為開發環境，您可以使用 **pscp** for scp 功能之類的公用程式。Pscp 可自 [PuTTY 下載頁面](http://www.chiark.greenend.org.uk/~sgtatham/putty/download.html) (英文) 下載取得。
...
4.  在瀏覽器中，瀏覽至 [Azure 管理入口網站](https://manage.windowsazure.com/)並選取虛擬機器。
...
6.  在 **[新增端點]** 對話方塊中，按一下右下方的箭頭繼續進入第二頁，並在表單中輸入下列資訊：
    ...
    這將建立公用連接埠 80，將流量傳送至 Rails 伺服器接聽的私人連接埠 300。

    ![新節點對話方塊](./media/virtual-machines-ruby-rails-web-app-linux/newendpoint.png)

7.  按一下核取記號來儲存端點。

8.  訊息應該會出現，顯示**更新進行中**。這個訊息消失後，端點便在作用中。您現在可以瀏覽至虛擬機器的 DNS 名稱，測試您的應用程式。網站應該類似如下範例：

    ![預設 rails 頁面](./media/virtual-machines-ruby-rails-web-app-linux/basicrailscloud.png)

    將 **/posts** 附加到 URL 之後應該會顯示樣板命令所產生的頁面。

    ![貼文頁面](./media/virtual-machines-ruby-rails-web-app-linux/blograilscloud.png)
