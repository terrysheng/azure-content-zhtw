...
您可以使用 Azure 跨平台命令列介面 (xplat-cli) 取得訂閱 ID 和管理憑證。如需安裝及設定 xplat-cli 的相關資訊，請參閱[安裝與設定 Azure 跨平台命令列介面](http://azure.microsoft.com/manage/install-and-configure-cli/) (英文)。
...
建立新的 Ruby 應用程式。本文件使用的範例可在單一 **.rb** 檔案中實作。
...
2. 使用下列命令安裝 azure gem：
	...
	> [AZURE.NOTE] 如果出現權限相關錯誤，請改用 <code>sudo gem install azure</code>。
...
## <a name="setup-connection"> </a>做法：連線到服務管理
...
## <a name="virtual-machine"> </a>做法：使用虛擬機器
...
* **:ssh\_private\_key\_file** - 包含私密金鑰的檔案，將用來保護對 Linux VM 的 SSH 存取。在選取 HTTPS 傳輸時，也會使用此檔案來指定用以保護 WinRM 的憑證。如果 **:ssh\_private\_key\_file** 和 **:ssh\_certificate\_file** 會省略，SSH 將只會使用密碼驗證

* **:ssh\_certificate\_file** - 包含憑證檔案的檔案，將用來保護對 Linux VM 的 SSH 存取。在選取 HTTPS 傳輸時，也會使用此檔案來指定用以保護 WinRM 的憑證。如果 **:ssh\_private\_key\_file** 和 **:ssh\_certificate\_file** 會省略，SSH 將只會使用密碼驗證
...
* **:winrm_transport** - 可用於 WinRM 的傳輸陣列。有效的傳輸為  'http' 和  'https'。如果指定  'https' 做為傳輸，您也必須使用 **:ssh\_private\_key\_file** 和 **:ssh\_certificate\_file** 來指定用來保護 HTTPS 通訊安全的憑證。
...
> [AZURE.WARNING] **delete_virtual_machine** 方法會刪除與虛擬機器相關聯的雲端服務和任何磁碟。
...
## <a name="vm-images"> </a>做法：使用虛擬機器映像和磁碟

虛擬  機器映像作業可使用 **Azure::VirtualMachineImageService** 類別來執行。磁碟作業可使用 **Azure::VirtualMachineImageManagement::VirtualMachineDiskManagementService** 類別來執行。
...
若要列出 Azure 訂閱使用的磁碟，請使用 **list\_virtual\_machine\_disks** 方法。此方法會傳回 **Azure::VirtualMachineImageManagement::VirtualMachineDisk** 物件的陣列。
...
## <a name="cloud-services"> </a>做法：使用雲端服務
...
## <a name="storage-services"> </a>做法：使用儲存服務
