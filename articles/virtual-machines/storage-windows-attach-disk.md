...
如果您需要其他資料磁碟，可將空磁碟或現有的資料磁碟連接到 VM。在這兩種情況下，磁碟實際上是需要位於 Azure 儲存體帳戶中的 .vhd 檔案。如果是新的磁碟，則在連接磁碟之後，您也需要將它初始化，使其可供 Windows VM 使用。
...
4. [磁碟] 區段會列出磁碟。在大部分情況下，會有磁碟 0、磁碟 1 和磁碟 2。磁碟 0 是作業系統磁碟、磁碟 1 是暫存磁碟 (不應用於資料儲存體)，而磁碟 2 是您剛連接到虛擬機器的資料磁碟。新的資料磁碟會將磁碟分割列為 [未知]。使用滑鼠右鍵按一下磁碟，然後選取 [初始化]。
