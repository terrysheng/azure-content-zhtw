...
本文說明如何擷取執行 Windows 的 Azure 虛擬機器，以便用它做為範本建立其他虛擬機器。此範本包括作業系統磁碟和任何連結至虛擬機器的資料磁碟。其不包含網路組態，因此您將需要在建立其他使用該範本的虛擬機器時加以設定。
...
7.	Sysprep 會將虛擬機器關機，並會在 [Azure 入口網站](http://manage.windowsazure.com)中，將虛擬機器的狀態變更為 [**已停止**]。
...
##後續步驟##
映像已可用來做為範本，建立虛擬機器。若要這麼做，您將需要使用 [**從資源庫**] 功能表項目並選取您剛建立的映像，以便建立自訂的虛擬機器。如需指示，請參閱《[如何建立自訂虛擬機器][]》。
