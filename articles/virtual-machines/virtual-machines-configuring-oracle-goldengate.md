...
- 您已經在 Azure 入口網站上，針對站台 A 將虛擬機器名稱設定為 “MachineGG1”，並針對站台 B 將虛擬機器名稱設定為 “MachineGG2”。
...
首先，透過 Azure 入口網站，使用遠端桌面功能連至站台 A 和站台 B。開啟 Windows 命令提示字元，並建立適用於 Oracle GoldenGate 安裝檔案的主目錄：
...
您需要在站台 A 和站台 B 上建立擷取和資料幫浦程序。透過 Azure 入口網站，使用遠端桌面功能連至站台 A 和站台 B。開啟 GGSCI 命令直譯器視窗。在站台 A 上執行下列命令：
