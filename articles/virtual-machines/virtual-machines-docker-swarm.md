...
> [AZURE.NOTE]本主題將 docker 與 swarm 和 Azure CLI 搭配使用，而「不」使用 **docker-machine**，以示範不同工具如何共同運作但仍保持獨立。**docker-machine** 具有 **--swarm** 參數，可讓您使用 **docker-machine** 直接將節點新增到 swarm。如需範例，請參閱 [docker-machine](https://github.com/docker/machine) 文件。如果您錯過了在 Azure VM 上執行的 **docker-machine**，請參閱[如何搭配 Azure 使用 docker-machine](virtual-machines-docker-machine.md)。
