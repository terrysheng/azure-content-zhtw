<properties 
	pageTitle="Azure 上適用於 Linux 的 Docker 虛擬機器擴充程式" 
...
# Azure 上適用於 Linux 的 Docker 虛擬機器擴充程式
...
+ [如何從Azure 跨平台介面 (xplat-cli) 使用 Docker VM 擴充程式]
+ [如何搭配 Azure 入口網站使用 Docker VM 延伸模組]
...
如需有關 Docker 及其實際運作方式的詳細資訊，請參閱[什麼是 Docker？](https://www.docker.com/whatisdocker/)
...
+ [如何從 Azure 跨平台介面 (xplat-cli) 使用 Docker VM 擴充程式]
+ [如何搭配 Azure 入口網站使用 Docker VM 擴充程式]

## <a id='Virtual Machine Extensions for Linux and Windows'>適用於 Linux 及 Windows 的虛擬機器擴充程式</a>
Azure 的 Docker VM 擴充程式只是許多提供特殊行為的 VM 擴充程式的其中一個，並且還有許多程式正在開發中。例如，[許多 Linux VM 代理程式的延伸模組](http://azure.microsoft.com/documentation/articles/virtual-machines-linux-agent-user-guide/)功能可讓您修改和管理虛擬機器，包括安全性功能及核心與網路功能等。例如，VMAccess 延伸模組可讓您重設管理員密碼或 SSH 金鑰。

如需完整清單，請參閱 [Azure VM 延伸模組](http://msdn.microsoft.com/library/azure/dn606311.aspx)。
...
[如何搭配 Azure 入口網站使用 Docker VM 擴充程式]: http://azure.microsoft.com/documentation/articles/virtual-machines-docker-with-portal/
...
[如何搭配 Azure 使用 Docker VM 擴充程式]: #How-to-use-the-Docker-VM-Extension-with-Azure
