...
這篇文章提供以自訂指令碼延伸模組撰寫 Azure 資源管理員範本，用來啟動 Linux 或 Windows VM 上工作負載的概觀。

如需自訂指令碼延伸模組的概觀請參閱文章<a href="https://azure.microsoft.com/zh-tw/documentation/articles/virtual-machines-extensions-customscript/" target="_blank">這裡</a>。

自從自訂指令碼延伸模組可供使用後，已廣泛使用於在 Windows 和 Linux VM 上設定工作負載。隨著 Azure 資源管理員範本的引入，使用者現在可以建立單一的範本，不只可用於佈建 VM，也可在 VM 上設定工作負載。
...
為了部署範本，我們使用 Azure 服務管理 API 中可用之相同版本的自訂指令碼延伸模組。本延伸模組支援將檔案上傳至 Azure 儲存體帳戶或 Github 位置的相同參數和案例。搭配範本使用時，主要差異在於應該指定正確版本的延伸模組，而不是以 majorversion.* 格式指定版本。
 ...
 ## 用於在 Linux VM 上自訂指令碼延伸模組的範本程式碼片段
...
## 用於在 Windows VM 上自訂指令碼延伸模組的範本程式碼片段
...
如需在 VM 上使用自訂指令碼延伸模組來設定應用程式的完整範例，請參閱下面範例。
