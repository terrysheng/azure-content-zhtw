...
|VM 擴充功能名稱|功能描述|相關資訊
|---|---|---|
|VMAccessAgent (Windows)|建立、更新和重設使用者資訊以及 RDP 與 SSH 連線組態。|[Windows](https://msdn.microsoft.com/library/dn606308.aspx)
|VMAccessForLinux (Linux)|建立、更新和重設使用者資訊以及 RDP 與 SSH 連線組態。|[Linux](http://azure.microsoft.com/blog/2014/08/25/using-vmaccess-extension-to-reset-login-credentials-for-linux-vm/)
...
|**Visual Studio 發行管理員** (以 DSC 擴充功能為基礎)|支援使用 Visual Studio 的持續部署。|[使用版本管理自動化部署](https://msdn.microsoft.com/library/dn217874.aspx)|
|**CentosChefClient**|||
...
|**McAfeeEndpointSecurity**|保護您的 VM 防禦惡意軟體。|[McAfee](http://www.mcafeeasap.com/)|
|**TrendMicroDSA**|可讓 TrendMicro 的深層安全性平台支援提供入侵偵測和防止、防火牆、反惡意程式碼、Web 信譽、記錄檢查和完整性監視。|[如何在 Azure VM 上安裝和設定 Trend Micro Deep Security as a Service](http://virtual-machines-install-trend.md)|
...
|**SymantecEndpointProtection**|Symantec Endpoint Protection 12.1.4 可跨越實體和虛擬系統達成安全性和效能。|[如何在 Azure VM 上安裝和設定 Symantec Endpoint Protection](http://virtual-machines-install-symantec.md)
...
|**HpcVmDrivers**|在大小為 A8 或 A9 的虛擬機器上安裝、設定和維護下列網路裝置驅動程式，讓 VM 可以存取 Azure 遠端直接記憶體存取 (RDMA) 網路。|[HpcVmDrivers 擴充功能](https://msdn.microsoft.com/library/dn690126.aspx)
