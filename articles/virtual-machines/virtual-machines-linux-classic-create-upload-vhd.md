...
Azure 虛擬機器執行的作業系統，根據您在建立期間所選擇的映像而定。這些映像會以 VHD 格式，儲存在儲存體帳戶中的 .vhd 檔案。如需詳細資訊，請參閱 [Azure 中的磁碟](virtual-machines-linux-about-disks-vhds.md)和 [Azure 中的映像](virtual-machines-linux-classic-about-images.md)。


在建立虛擬機器時，您可以自訂部分作業系統設定，使用它們適合您要執行的應用程式。如需指示，請參閱[如何建立自訂虛擬機器](virtual-machines-windows-classic-createportal.md)。
