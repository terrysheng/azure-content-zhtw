...
>[AZURE.NOTE]Azure Linux RDMA 目前支援在 SUSE Linux Enterprise Server 12 (SLES 12) 上執行的 Intel MPI Library 5.0 版。
>
> Azure 也提供 A10 和 A11 大量計算執行個體，包含與 A8 和 A9 執行個體相同的處理能力，但不含 RDMA 後端網路的連接。若要在 Azure 中執行 MPI 工作負載，使用 A8 和 A9 執行個體通常可獲得最佳效能。
...
* **HPC Pack** - 您可以在 Azure 中建立的 Microsoft HPC Pack 叢集，並加入執行受支援 Linux 散發套件 (支援在 HPC Pack 2012 R2 Update 2 中啟動) 的計算節點。某些 Linux 節點可以設定為存取 RDMA 網路。請參閱＜[HPC Pack 文件](http://go.microsoft.com/fwlink/?LinkId=617894)＞以開始使用。

* **Azure CLI 指令碼** - 您可以使用適用於 Mac、Linux 和 Windows 的 [Azure 命令列介面](../xplat-cli.md) (CLI)，建置自己的指令碼以部署虛擬網路和其他必要元件以便建立 Linux 叢集。在 Azure 服務管理 (asm) 模式中，CLI 將循序部署叢集節點，因此如果您正在部署許多計算節點，則可能需要花費幾分鐘才能完成部署。如需範例，請參閱本文剩餘部分中的步驟。

* **Azure 資源管理員範本** - 藉由建立簡單的 Azure 資源管理員 JSON 範本檔案和執行 Azure CLI arm 模式命令，或使用 Azure Preview 入口網站，您可以部署多個的 A8 和 A9 Linux VM，以及定義虛擬網路、靜態 IP 位址、DNS 設定和其他資源，以便建立可利用 RDMA 網路和執行 MPI 工作負載的計算叢集。您可以[建立自己的範本](../resource-group-authoring-templates.md)，或檢查 [[Azure 快速入門範本](https://azure.microsoft.com/documentation/templates/)] 頁面，取得由 Microsoft 或社群貢獻的範本以部署想要的方案。資源管理員範本通常會提供最快速且最可靠的方式來部署 Linux 叢集。
...
* **用戶端電腦** - 您將需要 Mac、Linux 或 Windows 用戶端電腦才能與 Azure 進行通訊。
...
* **Azure CLI** - [安裝](../xplat-cli-install.md) Azure CLI 並[進行設定](../xplat-cli-connect.md)，以便在用戶端電腦上使用您的 Azure 訂用帳戶。


### 佈建 SLES 12 VM

使用 Azure CLI 登入 Azure 之後，執行 `azure config list` 命令來確認 CLI 處於 Azure 服務管理 (asm) 模式。如果不是，請執行此命令來設定模式：
...
若要查看 Azure 中公開可用的 SLES 12 HPC 映像，如果您的 Shell 環境支援 **grep**，請執行類似下列的命令：
...
* 外部 SSH 通訊埠編號 (在此範例中為 10004) 是任何有效的連接埠編號；內部 SSH 通訊埠編號將設定為 22

* 系統將在位置指定的 Azure 區域中建立新的雲端服務；指定一個位置例如「美國西部」，其中 A8 和 A9 執行個體可提供使用

* 目前的映像名稱可以是 `b4590d9e3ed742e4a1d46e5424aa335e__suse-sles-12-hpc-v20150708` (免費) 或 `b4590d9e3ed742e4a1d46e5424aa335e__suse-sles-12-hpc-priority-v20150708` 以取得 SUSE 優先支援 (將收取費用)

### 自訂 VM

VM 完成佈建之後，使用 VM 的外部 IP 位址 (或 DNS 名稱) 以及您設定和自訂的外部連接埠編號來設定 VM 的 SSH。如需連接詳細資料，請參閱[如何登入執行 Linux 的虛擬機器](virtual-machines-linux-how-to-log-on.md)。

>[AZURE.NOTE]Microsoft Azure 不會提供 Linux VM 的根目錄存取權。若要在以使用者身分連線時取得系統管理存取權，您可以使用 `sudo –s`。

**更新** - 您可以使用 **zypper**，以及 NFS 公用程式來安裝更新。
...
**鎖定記憶體** - 如需使用 MPI 程式碼防止 RDMA 使用記憶體，您需要在 /etc/security/limits.conf 檔案中加入或變更下列設定：
...
**SSH 金鑰** - 建立使用者名稱和密碼的信任，以便用來在叢集中的所有計算節點之間建立此 VM。使用以下命令建立 SSH 金鑰：

```
$ ssh-keygen
```

將公用金鑰儲存在預設位置，並記住所輸入的複雜密碼。
...
或者，您可以列出叢集中每個 VM 的私人網路 IP 位址，如下所示：
...
>[AZURE.NOTE]如果未如上所示指定特定 IP 位址或範圍，則設定 `StrictHostKeyChecking no` 可能會造成潛在的安全性風險。

**應用程式** - 複製此 VM 上需要的任何應用程式，或在擷取映像之前執行其他自訂項目。

### 擷取映像

若要擷取映像，請先在 Linux VM 中執行下列命令：
...
針對您的環境，使用適當的值修改下列指令碼，並從用戶端電腦執行。因為 ASM 部署方法會循序部署 VM，系統將花費幾分鐘部署這段指令碼中建議的 8 個 A9 VM。
...
若要在 Azure Linux RDMA 上執行 MPI 應用程式，您需要設定 Intel MPI 專用的特定環境變數。以下範例指令碼，可供您設定變數並執行應用程式。
...
主機檔案的格式如下所示。針對叢集中的每個節點加入一行指令碼。從之前定義的 VNet 指定私人 IP 位址，而非 DNS 名稱。
