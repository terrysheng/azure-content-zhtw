...
* **Azure 資源管理員範本** - 藉由建立簡單的「Azure 資源管理員」JSON 範本檔案並執行「資源管理員」的 Azure CLI 命令，或藉由使用 Azure Preview 入口網站，可部署多個 A8 和 A9 Linux VM，以及定義虛擬網路、靜態 IP 位址、DNS 設定和其他資源，以建立可利用 RDMA 網路來執行 MPI 工作負載的運算叢集。您可以[建立自己的範本](../resource-group-authoring-templates.md)，或檢查 [Azure 快速入門範本](https://azure.microsoft.com/documentation/templates/) 頁面，取得由 Microsoft 或社群貢獻的範本以部署想要的方案。資源管理員範本通常會提供最快速且最可靠的方式來部署 Linux 叢集。
