...
- [傳統部署]：[如何：上傳您自己的散發套件映像](virtual-machines-linux-classic-create-upload-vhd.md) (以及有關使用[經 Azure 背書的散發套件](virtual-machines-linux-endorsed-distros.md)的指示)
- [注意：在 Azure 中執行的一般 Linux 需求](virtual-machines-linux-create-upload-generic.md)
- [傳統部署]：[注意：Azure 上的 Linux 一般簡介](virtual-machines-linux-intro-on-azure.md)
...
1. [做法：上傳您自己的 Ubuntu 映像](virtual-machines-linux-create-upload-ubuntu.md)
2. [做法：Ubuntu LAMP 堆疊](virtual-machines-linux-install-lamp-stack.md)
3. [傳統部署]：[如何：MySQL 叢集](virtual-machines-linux-classic-mysql-cluster.md)
4. [傳統部署]：[如何：Node.js 和 Cassandra](virtual-machines-linux-classic-cassandra-nodejs.md)
5. [資源管理員部署]：[如何：IPython Notebook](virtual-machines-linux-jupyter-notebook.md)
...
2. [映像庫](https://azure.microsoft.com/marketplace/partners/OpenLogic/)
3. [做法：針對 Azure 準備自訂 CentOS 型 VM](virtual-machines-linux-create-upload-centos.md)
4. [傳統部署]：[Blog: How to Deploy a CentOS VM Image from OpenLogic](https://azure.microsoft.com/blog/2013/01/11/deploying-openlogic-centos-images-on-windows-azure-virtual-machines/) (部落格：如何從 OpenLogic 部署 CentOS VM 映像)
6. [傳統部署]：[如何：安裝適用於 AMQP 和服務匯流排的 Apache Qpid Proton-C](../service-bus/service-bus-amqp-apache.md/)
...
11. [傳統部署]：[如何：安裝和執行 MySQL](virtual-machines-linux-classic-mysql-on-opensuse.md)
12. [做法：準備自訂 SLES 或 openSUSE VM](virtual-machines-linux-suse-create-upload-vhd.md)  
...
11. [傳統部署]：[如何：在 Azure 上使用 CoreOS](virtual-machines-linux-classic-coreos-howto.md)
12. [傳統部署]：[如何：開始在 Azure 上的 CoreOS 使用 Fleet 和 Docker](virtual-machines-linux-classic-coreos-fleet-get-started.md)


#### [Oracle Linux](https://azure.microsoft.com/marketplace/?term=Oracle+Linux)
  2. [準備執行 Azure 的 Oracle Linux 虛擬機器](virtual-machines-linux-oracle-create-upload-vhd.md)

### FreeBSD

12. [MSOpenTech VM Depot](https://vmdepot.msopentech.com/List/Index?sort=Date&search=FreeBSD)
13. [傳統部署]：[Blog: Running FreeBSD in Azure](https://azure.microsoft.com/blog/2014/05/22/running-freebsd-in-azure/) (部落格：在 Azure 中執行 FreeBSD)
14. [傳統部署]：[Blog: Easy Deploy FreeBSD](http://msopentech.com/blog/2014/10/24/easy-deploy-freebsd-microsoft-azure-vm-depot/) (部落格：輕鬆部署 FreeBSD)
...
6. [傳統部署]：[基本概念：使用 Azure 傳統入口網站登入 Linux VM](virtual-machines-linux-classic-log-on.md)
7. [基本概念：SSH](virtual-machines-linux-ssh-from-linux.md)
8. [傳統部署]：[基本概念：如何重設 Linux 的密碼或 SSH 屬性](virtual-machines-linux-classic-reset-access.md)
9. [基本概念：使用 Root](virtual-machines-linux-use-root-privileges.md)
10. [傳統部署]：[基本概念：將資料磁碟連接至 Linux VM](virtual-machines-linux-classic-attach-disk.md)
11. [傳統部署]：[基本概念：從 Linux VM 卸離資料磁碟](virtual-machines-linux-classic-detach-disk.md)
...
14. [傳統部署]：[基本概念：擷取 Linux VM 來製作範本](virtual-machines-linux-classic-capture-image.md)
...
17. [傳統部署]：[基本概念：將自訂資料插入 VM 中以搭配 Cloud-init 使用](virtual-machines-windows-classic-inject-custom-data.md)
18. [傳統部署]：[Blogging the basics: Building Highly Available Linux on Azure in 12 Steps](http://blogs.technet.com/b/keithmayer/archive/2014/10/03/quick-start-guide-building-highly-available-linux-servers-in-the-cloud-on-microsoft-azure.aspx) (基本概念部落格：以 12 個步驟在 Azure 上建置高可用性 Linux)
19. [傳統部署]：[Blogging the basics: Automate Provisioning Linux on Azure with Azure CLI, node.js, jhawk](http://blogs.technet.com/b/keithmayer/archive/2014/11/24/step-by-step-automated-provisioning-for-linux-in-the-cloud-with-microsoft-azure-xplat-cli-json-and-node-js-part-1.aspx) (基本概念部落格：透過 Azure CLI、node.js、jhawk 在 Azure 上自動佈建 Linux)
20. [基本概念：Azure Docker VM 延伸模組](virtual-machines-linux-dockerextension.md)
...
3. [做法：使用 Azure 傳統入口網站在 Linux 上設定 Tomcat7](virtual-machines-linux-classic-setup-tomcat.md)
...
5. [部落格：開始使用 Java 的 Azure 管理程式庫](https://azure.microsoft.com/blog/2014/09/15/getting-started-with-the-azure-java-management-libraries/)
...
8. [映像：WebSphere](https://azure.microsoft.com/marketplace/partners/msopentech/was-8-5-was-8-5-5-3/)
9. [映像：WebLogic](https://azure.microsoft.com/marketplace/?term=weblogic)
...
5. [Azure 服務管理 SDK：Ruby](https://github.com/MSOpenTech/azure-sdk-for-ruby)
    - [做法：安裝 Ruby on Rails](virtual-machines-linux-classic-ruby-rails-web-app.md)
6. [Azure 服務管理 SDK：Python](https://github.com/Azure/azure-sdk-for-python)
    - [做法：Django Hello World Web 應用程式 (Mac-Linux)](virtual-machines-linux-python-django-web-app.md)
...
1. Docker
	- [Azure 上 Linux 的 Docker VM 延伸模組](virtual-machines-linux-dockerextension.md)
	...
	- [如何在 Azure 上搭配 swarm 使用 docker](virtual-machines-linux-docker-swarm.md)
	- [在 Azure 上開始使用 Docker 和 Compose](virtual-machines-linux-docker-compose-quickstart.md)
...
10. Chef
	- [Chef 和虛擬機器](virtual-machines-windows-chef-automation.md)
