...
在本文中，我們將使用 Azure CLI 部署簡單的 LAMP 應用程式至 Ubuntu。

## 必要條件

針對下一個範例，先建立兩個執行 Ubuntu 14.04 的 Azure VM。VM 稱為 *script-vm* 和 *lamp-vm*。建立 VM 時請使用唯一名稱。其中一個用來執行 CLI 命令，而另一個用來部署 LAMP 應用程式。
...
下一個指令碼範例會將 LAMP 堆疊安裝到 Ubuntu (包括設定 MySQL 的無訊息安裝)、寫入簡單的 PHP 檔案，並啟動 Apache。
...
前一個命令會名為 *lamp-vm* 的 VM 上下載並執行 *lamp\_install.sh* 指令碼。
