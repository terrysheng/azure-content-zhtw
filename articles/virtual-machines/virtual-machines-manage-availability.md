...
### 避免可用性設定組中只有一部執行個體虛擬機器
避免一個可用性設定組中只有一部執行個體虛擬機器。此組態中的虛擬機器不符合 SLA 的保證，且會在 Azure 規劃的維護事件期間發生停機狀況。此外，若您在某個可用性設定組中部署單一虛擬機器執行個體，則不會收到平台維護的事先警告或通知。在此組態中，您的單一虛擬機器執行個體會在發生平台維護時，沒有事先警告就重新啟動。
