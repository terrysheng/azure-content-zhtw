...
## 區域配對
Azure 會組織一組區域配對，並保證該配對中只有一個區域將進行計劃性維護。Azure 將不會在計劃性維護期間同時首度發行已配對區域的更新。如需目前區域配對的相關資訊，請參閱下表：
...
例如，在計劃性維護首度發行期間，如果美國東部在該時候正在進行維護，Azure 將不會首度發行美國西部的更新。不過，像是北歐等其他地區可以和美國東部一同進行維護。
