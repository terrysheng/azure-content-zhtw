...
![顯示列出貼文的瀏覽器][blog-rails-cloud]


##<a id="setup"></a>設定開發環境

1. 在開發環境中安裝 Ruby。視作業系統而定，步驟可能有所不同。

	* **Apple OS X** - Ruby 有許多 OS X 版的散發套件。本教學課程已在 OS X 上利用 [Homebrew](http://brew.sh/) 來安裝 **rbenv** 和 **ruby-build** 而完成驗證。如需相關安裝資訊，請參閱 [https://github.com/sstephenson/rbenv/](https://github.com/sstephenson/rbenv/)。
...
2. 開啟新的命令列或終端機工作階段，並輸入下列命令安裝 Ruby on Rails：
	...
	> [AZURE.NOTE]此命令可能需要某些作業系統的管理員或根權限。如果您執行此命令, try using 'sudo' as follows:
	>
	>````` 
...
2. 開啟您的瀏覽器並瀏覽至 http://localhost:3000/。您應該會看到如下所示的頁面：

	![預設 rails 頁面][default-rails]

	此頁面是靜態的歡迎畫面。若要檢視由樣板命令產生的表單，請瀏覽至 http://localhost:3000/posts。您應該會看到如下所示的頁面：
...
* **user@vmdns**：將檔案複製到其中以及登入使用者帳戶所使用的機器位址
...
如果要深入了解 Ruby on Rails，請參閱 [Ruby on Rails 指南][rails-guides] (英文)。
