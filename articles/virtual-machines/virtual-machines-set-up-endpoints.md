<properties
	...
	description="了解如何在 Azure 入口網站中設定端點，以允許與 Azure 中的虛擬機器進行通訊。"
...
在 Azure 入口網站中建立虛擬機器後，遠端桌面、Windows PowerShell 遠端和安全殼層 (SSH) 的端點也會自動建立。建立虛擬機器或日後有需要時，您可以設定其他端點。
...
當您使用 Azure入口網站來建立端點時，系統會提供 IP 通訊協定的預設值，以及已知網路通訊協定的 TCP 或 UDP 連接埠。針對自訂端點，您必須指定正確的 IP 通訊協定 (TCP 或 UDP) 以及公用和私人連接埠。若要將連入流量隨機分散到多部虛擬機器，您必須建立負載平衡的集合，其中包含多個端點。
...
5.	在 [將端點加入至虛擬機器] 頁面上，選擇端點的類型。
	...
	- 如果您要建立的新端點不屬於負載平衡集，或是新負載平衡集內的第一個端點，請選擇 [加入獨立端點]，然後按一下向左箭號。
	- 否則，請選擇 [將端點加入至現有的負載平衡集]、選取負載平衡集的名稱，然後按一下向左箭號。在 [指定端點的詳細資料] 頁面上，輸入端點的名稱，然後按一下核取記號以建立端點。
