...
下表說明可以為自動備份設定的選項。實際的設定步驟會依據您是使用 Azure Preview 入口網站或 Azure Windows PowerShell 命令而有所不同。
...
## 在 Azure Preview 入口網站中設定自動備份

在建立新的 SQL Server 2014 虛擬機器時，您可以透過 Azure Preview 入口網站設定自動備份。
...
下列 Azure Preview 入口網站螢幕擷取畫面顯示 [**選用組態**] | [**SQL 自動備份**] 下的選項。

![Azure Preview 入口網站中的 SQL 自動備份組態](./media/virtual-machines-sql-server-automated-backup/IC778483.jpg)

針對現有的 SQL Server 2014 虛擬機器，在虛擬機器屬性的 [**組態**] 區段中，選取 [**自動備份**] 設定。在 [**自動備份**] 視窗中，您可以啟用功能、設定保留期限、選取儲存體帳戶及設定加密。如下列螢幕擷取畫面所示。

![Azure Preview 入口網站中的自動備份組態](./media/virtual-machines-sql-server-automated-backup/IC792133.jpg)

>[AZURE.NOTE]當您第一次啟用自動備份時，Azure 會在背景中設定 SQL Server IaaS 代理程式。在此期間，Azure Preview 入口網站不會顯示已設定自動備份。請等候幾分鐘的時間來安裝及設定代理程式。之後，Azure Preview 入口網站將會反映新的設定。
