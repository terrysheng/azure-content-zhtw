<properties 
	pageTitle="設定 Azure 中 AlwaysOn 可用性群組的外部接聽程式"
...
請注意，當您使用雲端服務公用 VIP 位址進行部署時，Azure 中的可用性群組接聽程式具有下列限制：
...
- 每個雲端服務僅支援一個可用性群組接聽程式，因為接聽程式設定為使用雲端服務 VIP 位址。請注意，雖然 Azure 現在支援在指定的雲端服務中建立多個 VIP 位址中，這項限制仍為有效。
