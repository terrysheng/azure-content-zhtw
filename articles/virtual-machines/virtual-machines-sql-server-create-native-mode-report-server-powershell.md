...
1. 在下一個頁面中，編輯下列欄位：
	...
	- [端點]：保留 [遠端桌面] 和 [PowerShell] 端點，然後根據您的環境新增 HTTP 或 HTTPS 端點。
		...
		- [HTTPS]：預設的公用和私人連接埠都是 **443**。安全性最佳作法就是變更私用連接埠，並將防火牆和報表伺服器設為使用私用連接埠。如需有關端點的詳細資訊，請參閱[如何設定與虛擬機器的通訊](virtual-machines-set-up-endpoints.md)。請注意，如果您使用的連接埠不是 443，請變更 HTTPS 指令碼中的 **$HTTPsport = 443** 參數。
