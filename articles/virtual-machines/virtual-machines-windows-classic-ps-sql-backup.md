...
![Azure 入口網站中的 SQL 自動備份設定](./media/virtual-machines-windows-classic-ps-sql-backup/IC778483.jpg)

針對現有的 SQL Server 2014 虛擬機器，在虛擬機器屬性的 [**組態**] 區段中，選取 [**自動備份**] 設定。在 [**自動備份**] 視窗中，您可以啟用功能、設定保留期限、選取儲存體帳戶及設定加密。如下列螢幕擷取畫面所示。

![Azure 入口網站中的自動備份設定](./media/virtual-machines-windows-classic-ps-sql-backup/IC792133.jpg)
...
您可以在下列主題中找到 Azure VM 上 SQL Server 的其他備份和還原指引：[Azure 虛擬機器中的 SQL Server 備份和還原](virtual-machines-windows-classic-sql-br.md)。

Azure 中的 SQL Server VM 相關功能為 [Azure 虛擬機器中的 SQL Server 自動修補](virtual-machines-windows-classic-ps-sql-patch.md)。

請檢閱其他[在 Azure 虛擬機器中執行 SQL Server 的資源](virtual-machines-windows-classic-sql-overview.md)。
