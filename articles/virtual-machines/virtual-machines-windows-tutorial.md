<properties
	...
	description="了解如何使用 Azure Preview 入口網站中的 Azure Marketplace，建立執行 Windows 的 Azure 虛擬機器"
...
本教學課程會使用資源管理員部署模型來建立虛擬機器。我們建議您使用上述模組，不要使用以服務管理 API 為基礎的傳統部署模型。如需資源管理員的詳細資料，請參閱 [Azure 資源管理員概觀](resource-group-overview.md)。若要深入了解使用虛擬機器資源管理員的優點，請參閱 [Azure 資源管理員提供的 Azure 運算、網路和儲存提供者](virtual-machines-azurerm-versus-azuresm.md)。
