...
> [AZURE.NOTE]因為這些虛擬機器用於內部網路應用程式，所以未獲指派公用 IP 位址或 DNS 網域名稱標籤，也不會公開到網際網路。不過，這也表示您無法從 Azure Preview 入口網站連線到它們。當您檢視虛擬機器的屬性時，[連線] 按鈕將無法使用。使用「遠端桌面連線」配件或其他遠端桌面工具，來使用虛擬機器的私人 IP 位址或內部網路 DNS 名稱連接該虛擬機器。
...
9.	在 [登入名稱] 中輸入 *domain*\\sqladmin in，其中 *domain* 是指在[第 2 階段](virtual-machines-workload-high-availability-LOB-application-phase2.md)中建立 sqladmin 帳戶所在網域的名稱。 
...
1.	使用在[第 2 階段](virtual-machines-workload-high-availability-LOB-application-phase2.md)中建立的 sqladmin 帳戶來登入主要 SQL Server 虛擬機器。
...
若要繼續設定此工作負載，請前往[第 4 階段：設定 Web 伺服器](virtual-machines-workload-high-availability-LOB-application-phase4.md)。
