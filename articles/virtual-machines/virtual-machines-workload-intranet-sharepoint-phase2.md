...
8.	在 [**使用者名稱**] 中，輸入 VM 名稱以及使用該 VM 建立的本機系統管理員帳戶名稱 (本機電腦帳戶)。  
...
2.	使用 **ping** 命令來 ping 您組織網路中各個資源的名稱和 IP 位址。
...
1.	在 Azure 入口網站的左側窗格中按一下 [**網路**]，然後按一下您虛擬網路的名稱 (資料表 V – 項目 1 – 值資料行)。
2.	按一下 [設定]。
...
4.	在 **[DNS 伺服器**] 中，新增兩個具有易記名稱以及下列兩個資料表項目之 IP 位址的項目：
 ...
 - 資料表 V – 項目 7 – 值資料行
5.	在底部的命令列中按一下 [**儲存**]。
6.	在 Azure 入口網站的左側窗格中按一下 [**虛擬機器**]，然後按一下第一個網域控制站名稱旁邊的 [**狀態**] 資料行。
7.	在命令列中按一下 [**重新啟動**]。
