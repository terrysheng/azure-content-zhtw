...
它們啟動之後，使用[使用遠端桌面連線程序登入虛擬機器](virtual-machines-workload-intranet-sharepoint-phase2.md#logon)四次 (每一部 SharePoint Server 一次)，以使用 [網域]\\sp\_farm\_db 帳戶認證登入。您已在[第 2 階段：設定網域控制站](virtual-machines-workload-intranet-sharepoint-phase2.md)中建立了這些認證。
...
5.	在 [指定設定資料庫設定] 頁面中：
 ...
 - 在 [使用者名稱] 中，輸入 [網域]**\\sp\_farm\_db** (已在[第 2 階段：設定網域控制站](virtual-machines-workload-intranet-sharepoint-phase2.md)中建立)。還記得 sp\_farm\_db 帳戶具有資料庫伺服器的系統管理員權限。
...
- 依照[第 5 階段](virtual-machines-workload-intranet-sharepoint-phase5.md)指示繼續此工作負載的設定。
