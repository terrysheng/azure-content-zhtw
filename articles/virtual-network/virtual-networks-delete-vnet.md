...
1. **儲存設定 (選擇性) –** 如果您要將虛擬網路設定儲存至本機檔案，則必須先匯出組態檔才能刪除虛擬網路。請參閱＜[將虛擬網路設定匯出至網路組態檔](virtual-networks-using-network-configuration-file.md)＞以取得詳細資訊。儲存設定可讓您未來在需要時重新建立 VNet。
