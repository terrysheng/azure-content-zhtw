...
您可能熟悉如何使用防火牆和存取控制清單 (ACL) 來篩選傳送到網路區段、個別電腦和甚至是電腦中的網路介面卡 (NIC) 的網路流量。您也可以在 Azure 中以類似的方式篩選網路資料流量，如下所示。

- **端點 ACL**
	- 僅能篩選輸入的流量。
	- 僅能用在公開到網際網路或透過內部負載平衡器的端點上。
	- 限制在每個端點的 50 個 ACL 規則。
	- **不**需要 VNet (傳統部署)。
- **網路安全性群組 (NSG)**
	- 根據方向、通訊協定、來源位址和連接埠與目的地位址和連接埠來允許或拒絕流量。
	- 可以控制在 VM 或角色執行個體 (傳統部署)、NIC (資源管理員部署) 和子網路 (所有部署) 上的輸入和輸出流量。這包括連接到子網路 (例如雲端服務和 AppService 環境) 的任何資源。
	- 僅可以套用至連線到區域 VNet 的資源。
	- **不**需要管理防火牆應用裝置。
	- 限制在 100 個 NSG，每個區域各 200 個規則。
- **防火牆應用裝置**
	- 在您的 Azure 網路中實作為 VM。
	- 根據方向、通訊協定、來源位址和連接埠與目的地位址和連接埠來允許或拒絕流量。
	- 視使用的防火牆應用裝置而定提供額外的功能。

本文的重點在於 NSG。如需其他流量篩選選項的詳細資訊，請瀏覽以下提供的連結。

- [ACL 文件](./virtual-networks-acl.md)。
- [使用 NSG 和防火牆應用裝置建置 DMZ](virtual-networks-dmz-nsg-fw-udr-asm.md)。

## NSG 如何運作？

NSG 包含兩種類型的規則，即**輸入**和**輸出**。當流量流入裝載 VM 或角色執行個體的 Azure 伺服器時，主機會根據流量的方向載入所有輸入或輸出 NSG 規則。然後主機會依優先順序檢查每個規則。如果有規則符合主機正在分析的封包，會套用該規則的動作 (允許或拒絕)。如果沒有規則符合該封包，則會捨棄封包。下圖顯示這個決策流程。

![NSG ACL](./media/virtual-network-nsg-overview/figure3.png)

>[AZURE.NOTE]因為您可以建立 NSG 與 VM (傳統部署)、NIC (資源管理員部署) 或子網路 (所有部署) 的關聯，套用至指定 VM 或角色執行個體的規則可以來自多個 NSG。[關聯 NSG](#Associating-NSGs) 一節涵蓋如何根據流量的方向套用來自多個 NSG 的規則。
...
|**存取**|如果規則符合，要套用的存取類型|allow (允許) 或 deny (拒絕)|請注意，如果找不到封包的允許規則，則會捨棄封包|
...
|規則|存取|優先順序|來源位址範圍|來源連接埠|目的地連接埠範圍|目的地連接埠|通訊協定|
|---|---|---|---|---|---|---|---|
