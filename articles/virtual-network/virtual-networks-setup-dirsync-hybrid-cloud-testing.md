...
2.	開啟 Internet Explorer 並移至 ****http://fasttrack.office.com**。
3.	按一下 [開始使用 FastTrack]。
4.	在 [開始使用 FastTrack] 頁面的 [首先，註冊 Office 365 試用] 下，按一下 [企業在此註冊]。
5.	在 [步驟 1] 頁面上填寫內容，在 [公司電子郵件地址] 中指定新的 Microsoft 帳戶，然後按一下 [下一步]。
...
7.	在 [步驟 3] 頁面上，鍵入可收發簡訊的手機或智慧型手機的電話號碼，然後按一下 [傳簡訊給我]。
8.	手機收到簡訊後，鍵入驗證碼，然後按一下 [建立我的帳戶]。 
9.	Office 365 建立完您的帳戶時，按一下 [您準備繼續]。
10.	您現在應該會看見 Office 365 入口網站主頁面。在最上方的功能區中，按一下 [系統管理]，然後按一下 [Office 365]。Office 365 系統管理中心頁面隨即出現。讓 CLIENT1 保持這個頁面開啟。
...
1.	執行 Internet Explorer，在網址列中鍵入 ****http://go.microsoft.com/fwlink/?LinkID=278924**，然後按 ENTER。系統提示執行 dirsync.exe 時，請按一下 [儲存] 旁邊的箭號，再按一下 [另存新檔]，然後再按一下 [儲存] 將檔案儲存於 [下載] 資料夾。如需安裝工具的詳細資訊，請參閱[安裝或升級目錄同步作業工具](http://technet.microsoft.com/library/jj151800)。
2.	開啟 [下載] 資料夾，以滑鼠右鍵按一下 **dirsync** 檔案，然後按一下 [以系統管理員身分執行]。
3.	在 Active Directory 同步安裝精靈的 [歡迎] 頁面上，按一下 [下一步]。 
4.	在 [授權條款] 頁面上，按一下 [我接受]，然後按一下 [下一步]。
5.	在 [選取資料夾安裝] 頁面上，按一下 [下一步]。這可能需要幾分鐘才能完成安裝。
6.	在 [完成] 頁面上，取消選取 [立即啟動組態精靈]，然後按一下 [完成]。
7.	在開始畫面中，按一下 [user1]，然後按一下 [登出]。

接著，對於 Office 365 FastTrack 試用啟用 Directory 同步處理。

1.	在 CLIENT1 上，於 [Office 365 系統管理中心] 頁面的左窗格中，按一下 [使用者]，然後按一下 [作用中使用者]。
2.	針對 **Active Directory 同步處理**，按一下 [設定]。
3.	在 [設定和管理 Active Directory 同步處理] 頁面的步驟 3 中，按一下 [啟動]。
4.	出現 [您想要啟動 Active Directory 同步處理？] 的提示時，按一下 [啟動]。執行此動作後，步驟 3 將顯示 [Active Directory 同步處理已啟動]。
5.	在 CLIENT1 上保持開啟 [設定和管理 Active Directory 同步處理] 頁面。

接著，以 CORP\\User1 帳戶登入 DC1，並開啟系統管理員層級 Windows PowerShell 命令提示字元。逐一執行這些命令，以建立名為 contoso\_users 的新組織單位以及 Marci Kaufman 和 Lynda Meyer 兩個新的使用者帳戶。
...
6.	在 [Active Directory 認證] 頁面上，於 [使用者名稱] 中鍵入 **CORP\\User1**，並在 [密碼] 中輸入 User1 的帳戶密碼。按 [下一步]。
7.	在 [混合式部署] 頁面上，選取 [啟用混合式部署]，然後按一下 [下一步]。
8.	在 [密碼同步處理] 頁面上，選取 [啟用密碼同步處理]，然後按一下 [下一步]。
9.	[組態] 頁面會顯示。組態完成時，按一下 [下一步]。
10.	在 [完成] 頁面上，按一下 [完成]。出現提示時，按一下 **[確定]**。

接著，確認 CORP 網域中的使用者帳戶已同步處理至 Office 365。請注意，可能需要經過數小時才會進行同步處理。

在 CLIENT1 上，於 [設定和管理 Active Directory 同步處理] 頁面中，按一下此頁面步驟 6 中的**使用者**連結。如果 Directory 同步處理順利進行，您應該會看見如下所示的內容。
...
7.	以 Lynda Meyer 的 Office 365 認證登入。她的使用者名稱將是 lyndam@<*虛構名稱*>.onmicrosoft.com。密碼是 Lynda Meyer Active Directory 使用者帳戶密碼。
