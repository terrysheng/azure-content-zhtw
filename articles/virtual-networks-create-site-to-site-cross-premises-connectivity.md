...
7.	下載 VPN 裝置組態指令碼範本：按一下儀表板上的 [**下載 VPN 裝置指令碼**]。

8.	在 [**下載 VPN 裝置組態指令碼**] 對話方塊中，選取公司 VPN 裝置的廠商、平台及作業系統。按一下核取記號按鈕並儲存檔案。 
