...
本教學課程將逐步引導您完成在[虛擬網路][虛擬網路]上，從虛擬機器 (VM) 之企業 Active Directory 樹系安裝額外網域控制站的步驟。在本教學課程中，VM 的虛擬網路已連接公司網路。如需在 Azure 虛擬網路上安裝 Active Directory 網域服務 (AD DS) 的概念指引，請參閱[在 Azure 虛擬機器中部署 Windows Server Active Directory 的指導方針][在 Azure 虛擬機器中部署 Windows Server Active Directory 的指導方針]。

## 目錄

-   [必要條件][必要條件]
-   [步驟 1：驗證 YourPrimaryDC 的靜態 IP 位址][步驟 1：驗證 YourPrimaryDC 的靜態 IP 位址]
-   [步驟 2：安裝企業樹系][步驟 2：安裝企業樹系]
-   [步驟 3：建立子網路和站台][步驟 3：建立子網路和站台]
-   [步驟 4：在 CloudSite 中安裝額外的網域控制站][步驟 4：在 CloudSite 中安裝額外的網域控制站]
-   [步驟 5：驗證安裝][步驟 5：驗證安裝]
-   [步驟 6：佈建在開機時便已加入網域的虛擬機器][步驟 6：佈建在開機時便已加入網域的虛擬機器]
-   [步驟 7：備份網域控制站][步驟 7：備份網域控制站]
-   [步驟 8：測試驗證和授權][步驟 8：測試驗證和授權]

## <span id="Prerequisites"></span></a>必要條件

-   在 Azure 虛擬網路與公司網路之間設定的[管理入口網站中設定站對站 VPN][管理入口網站中設定站對站 VPN]。
-   在虛擬網路中建立雲端服務。
-   在屬於虛擬網路的雲端服務中部署兩部 VM (指定要放置 VM 的子網路)。如需詳細資訊，請參閱[將虛擬機器新增至虛擬網路][將虛擬機器新增至虛擬網路]。其中一部 VM 的大小必須為 L 或更大，以便連接兩個資料磁碟。需要資料磁碟用來儲存：
    -   Active Directory 資料庫和記錄。
    -   系統狀態備份。
-   具備兩部 VM (YourPrimaryDC 和 FileServer) 的企業網路。
-   如果您需要讓外部使用者解析 Active Directory 中帳戶的名稱，則必須部署網域名稱系統 (DNS) 基礎結構。在此情況下，將 DNS 伺服器安裝在網域控制站上之前，您應該先建立 DNS 區域委派，或允許 Active Directory 網域服務安裝精靈建立委派。如需建立 DNS 區域委派的詳細資訊，請參閱[建立區域委派][建立區域委派]。
...
## <span id="installforest"></span></a>步驟 2：安裝企業樹系

1.  在 VM 的 RDP 工作階段中，按一下 [開始] 並輸入 **dcpromo**，然後按 ENTER。
...
4.  在 [選擇部署組態] 頁面中按一下 [在新樹系內建立新網域]，然後再按 [下一步]。
...
## <span id="subnets"></span></a>步驟 3：建立子網路和站台

1.  在 YourPrimaryDC 上依序按一下 [開始]、[系統管理工具] 及 [Active Directory 站台及服務]。
2.  按一下 [站台]，以滑鼠右鍵按一下 [子網路]，然後再按一下 [新增子網路]。

    ![CreateSubnetsandSites1][CreateSubnetsandSites1]

3.  在 [Prefix::] 中輸入 **10.1.0.0/24**，選取 [Default-First-Site-Name] 站台物件，然後按一下 [確定]。

    ![CreateSubnetsandSites2][CreateSubnetsandSites2]

4.  以滑鼠右鍵按一下 [站台]，然後再按一下 [新增站台]。

    ![CreateSubnetsandSites3][CreateSubnetsandSites3]

5.  在 [名稱] 中輸入 **CloudSite**，選取 [DEFAULTIPSITELINK]，然後再按一下 [確定]。
...
8.  在 [Prefix::] 中輸入 **10.4.2.0/24**，選取 [CloudSite] 站台物件，然後按一下 [確定]。

    ![CreateSubnetsandSites7][CreateSubnetsandSites7]

## <span id="cloudsite"></span></a>步驟 4：在 CloudSite 中安裝額外的網域控制站

1.  登入 YourVMachine，按一下 [開始] 並輸入 **dcpromo**，然後按 ENTER。
...
4.  在 [選擇部署組態] 頁面中依序按一下 [現有樹系]、[新增現有網域的網域控制站] 及 [下一步]。
...
## <span id="validate"></span></a>步驟 5：驗證安裝

1.  重新連接 VM。

2.  按一下 [開始]，以滑鼠右鍵按一下 [命令提示字元]，然後按一下 [以系統管理員身分執行]。

3.  輸入以下命令，然後按 ENTER：'Dcdiag /c /v'

4.  確認測試是否成功執行。

在設定 DC 之後，執行以下 Windows PowerShell Cmdlet 以佈建額外的虛擬機器，並讓它們在佈建時自動加入網域。在佈建 VM 時，您必須設定 VM 的 DNS 用戶端解析程式設定。請替換為正確的網域名稱、VM 名稱等。

如需使用 Windows PowerShell 的詳細資訊，請參閱 [Azure PowerShell][Azure PowerShell] 和 [Azure 管理 Cmdlet][Azure 管理 Cmdlet] (英文)。

## <span id="provisionvm"></span></a>步驟 6：佈建在開機時便已加入網域的虛擬機器
...
## <span id="test"></span></a>步驟 8：測試驗證和授權
...
-   [Windows Azure IT Pro IaaS：(01) 虛擬機器基礎][Windows Azure IT Pro IaaS：(01) 虛擬機器基礎] (英文)

-   [Windows Azure IT Pro IaaS：(05) 建立虛擬網路及跨單位連線][Windows Azure IT Pro IaaS：(05) 建立虛擬網路及跨單位連線] (英文)
