...
接著，註冊新的 Microsoft 帳戶。移至 **http://outlook.com** 使用 user123@outlook.com 之類的電子郵件地址建立帳戶。您將使用這個帳戶註冊 Office 365 FastTrack 試用。

接著，註冊新的 Office 365 FastTrack 試用。

1.	以 CORP\User1 帳戶認證登入 CLIENT1。
2.	開啟 Internet Explorer 移至 **http://fasttrack.office.com**。
...
6.	在 [步驟 2] 頁面上，輸入初始 Office 365 帳戶的名稱 (在第一個欄位中)、虛構的公司名稱和密碼。記錄產生的電子郵件地址 (例如 user123@contoso123.onmicrosoft.com) 和密碼並儲存於安全之處。您將需要這些資訊，才能在階段 3 完成 Active Directory 同步處理工具組態精靈。按 [**下一步**]。
...
1.	執行 Internet Explorer，並且在網址列中輸入 **http://go.microsoft.com/fwlink/?LinkID=278924**，然後按下 ENTER。提示執行 dirsync.exe，按一下 [**儲存**] 旁邊的箭號，並按一下 [**另存新檔**]，然後按一下 [**儲存**] 將檔案儲存於 [下載] 資料夾。如需安裝工具的詳細資訊，請參閱 [安裝或升級 Directory 同步處理工具](http://technet.microsoft.com/library/jj151800)。
2.	開啟 [**下載**] 資料夾，並且以滑鼠右鍵按一下 **dirsync** 檔，然後按一下 [**以系統管理員身分執行**]。
...
1.	在 CLIENT1 個 [**作用中使用者**] 頁面上，選取 **Lynda Meyer** 帳戶。
...
7.	使用 Lynda Meyer 的 Office 365 認證登入。她的使用者名稱將是 lyndam@<*Your Fictional Name*>.onmicrosoft.com。密碼是 Lynda Meyer Active Directory 使用者帳戶密碼。
