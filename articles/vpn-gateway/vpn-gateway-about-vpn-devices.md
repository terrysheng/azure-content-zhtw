...
| 階段 2 安全性關聯 (SA) 存留期 (時間) | 3,600 秒 | - | | 階段 2 安全性關聯 (SA) 存留期 (輸送量) | 102,400,000 KB | - | | IPsec SA 加密及驗證提供項目 (依喜好順序) | 1.ESP-AES256 2.ESP-AES128 3.ESP-3DES 4.N/A | 請參閱＜路由式閘道 IPsec安全性關聯 (SA) 提供項目＞ (下方)| | 完整轉寄密碼 (PFS) | 否 | 是 (DH Group1、2、5、14、24) | | 停用的對等偵測 (DPD) | 不支援 | 支援 |
