...
本文將逐步引導您使用 Azure 資源管理員部署模型建立虛擬網路以及內部部署網路的站對站 VPN 連線。

>[AZURE.IMPORTANT]請務必了解 Azure 目前使用兩種部署模型：資源管理員模型和傳統模型。開始您的組態之前，請確定您瞭解部署模型和工具。如需部署模型的資訊，請參閱 [Azure 部署模型](../azure-classic-rm.md)。

您可以使用上方的索引標籤，選取部署模型和部署工具的文章。例如，如果您想要使用 Azure 入口網站中的傳統部署模型，而不是使用資源管理員模型建立站對站 VPN 閘道連線，按一下 [ Azure 入口網站] 索引標籤 (上方) 來開啟[在 Azure 入口網站中建立站對站 VPN 連線](vpn-gateway-site-to-site-create.md)。
...
- 最新版的 Azure PowerShell Cmdlet。您可以從[下載頁面](http://azure.microsoft.com/downloads/)的 Windows PowerShell 區段下載並安裝最新版本。 
...
接下來，切換至 Azure 資源管理員模式。在不久的將來，將不需要變更為此模式，而這兩種模式的 cmdlet 會自動提供。
...
有時候會變更您的本機站台首碼。若要修改您的 IP 位址首碼，採取的步驟取決於您是否已建立 VPN 閘道連接。請參閱[修改本機站台的 IP 位址首碼](#modify-ip-address-prefixes-for-a-local-site)。
...
- 閘道器類型為 *Vpn*。
- VpnType 可以 RouteBased* (在某些文件中稱為動態閘道器)，或以*原則為基礎* (在某些文件中稱為靜態閘道器)。如需 VPN 閘道類型的詳細資訊，請參閱[關於 VPN 閘道](vpn-gateway-about-vpngateways.md)。 	
...
- **若要新增**其他位址首碼到您建立的本機站台，但還沒有 VPN 閘道連線，請使用下列範例。
