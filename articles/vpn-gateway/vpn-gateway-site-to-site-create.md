...
您可以建立站台對站台 VPN 連線，來連接您的內部部署位置與虛擬網路。此程序將逐步引導您建立虛擬網路，並在新建立的 VNet 與您的內部部署位置之間建立站台對站台 VPN 連線。您可以使用此程序，建立跨部署與混合式虛擬網路組態。


## 開始之前

- 請確認您想要使用的 VPN 裝置符合建立跨部署虛擬網路連線所需的需求。如需詳細資訊，請參閱＜[關於虛擬網路連線能力的 VPN 裝置](https://msdn.microsoft.com/library/azure/jj156075.aspx)＞。
...
輸入以下資訊。如需詳細資料頁面上設定的詳細資訊，請參閱〈[虛擬網路詳細資料頁面](https://msdn.microsoft.com/library/azure/09926218-92ab-4f43-aa99-83ab4d355555#BKMK_VNetDetails)〉。
...
## DNS 伺服器和 VPN 連線能力頁面
輸入下列資訊，然後按一下右下角的 [下一步] 箭頭。如需此頁面上設定的詳細資訊，請參閱 〈[DNS 伺服器和 VPN 連線能力頁面](https://msdn.microsoft.com/library/azure/09926218-92ab-4f43-aa99-83ab4d355555#BKMK_VNETDNS)〉。
...
輸入下列資訊，然後按一下 [下一步] 箭頭。如需此頁面上設定的詳細資訊，請參閱＜[站台對站台連線能力頁面](https://msdn.microsoft.com/library/azure/09926218-92ab-4f43-aa99-83ab4d355555#BKMK_VNETSITE)＞。
...
## 虛擬網路位址空間頁面
指定您想要用於虛擬網路的位址範圍。這些是將指派給 VM 的動態 IP 位址 (DIP)，以及指派給您部署至此虛擬網路之其他角色執行個體的動態 IP 位址 (DIP)。有相當多關於虛擬網路位址空間的規則，因此如需詳細資訊，請參閱＜[虛擬網路位址空間頁面](https://msdn.microsoft.com/library/azure/09926218-92ab-4f43-aa99-83ab4d355555#BKMK_VNET_ADDRESS)＞。
...
接著，您將設定**虛擬網路閘道**，以便建立安全的站台對站台 VPN 連線。請參閱＜[設定管理入口網站中的虛擬網路閘道](https://msdn.microsoft.com/library/azure/jj156210.aspx)＞。

## 後續步驟

您可以進一步了解本文中虛擬網路的跨單位連線：〈[關於虛擬網路安全的跨單位連線](https://msdn.microsoft.com/library/azure/dn133798.aspx)〉

如果您想要設定點對站台 VPN 連線，請參閱＜[設定點對站台 VPN 連線](vpn-gateway-point-to-site-create.md)＞
