<properties
   pageTitle="利用編輯及重新整理功能來修改在本機 Docker 容器中執行的線上 ASP.NET 5 Web 應用程式 | Microsoft Azure"
...
# 利用編輯及重新整理功能來修改在本機 Docker 容器中執行的線上 ASP.NET 5 Web 應用程式
...
## 設定及測試 Docker 用戶端 
本節將引導您完成各個步驟，來確認 Docker 機器的預設執行個體已成功設定，且正在執行。

1. 在命令提示字元發出下列命令，來建立預設的 Docker 主機執行個體。

		docker-machine create --driver virtualbox default
 
1. 在命令提示字元發出下列命令，來確認預設的執行個體已成功設定，且正在執行中。(您應該會看到名為「default」的執行個體正在執行。

		docker-machine ls 
		
	![][0]
 
1. 在命令提示字元發出下列命令，來將「default」執行個體設定成目前的主機。
...
1. 下列命令應該會顯示執行中之作用中容器的空白回應。

		docker ps

	![][1]
 
> [AZURE.NOTE] 每次您將開發用機器重新開機時，就需要重新啟動您的本機 Docker 主機。方法是在命令提示字元發出下列命令：`docker-machine start default`
...
1. 在 [新增專案] 對話方塊的 [範本] 區段下方，選取 [Visual C#] > [Web]。
...
1. 當容器映像已建置，且正在 Docker 容器中執行時，PowerShell 主控台會嘗試在您的預設瀏覽器中啟動 Web 應用程式。如果您在使用 Microsoft Edge 瀏覽器，請參閱[疑難排解](#troubleshooting)一節。
...
- **建置：無法建置映像、查看 TLS 連線時發生錯誤：主機並未執行**
	...
	請確認預設的 Docker 主機正在執行。請參閱[設定 Docker 用戶端](#configuring-the-docker-client)一節。

- **找不到磁碟區對應**

	根據預設，VirtualBox 會把 `C:\Users` 當做 `c:/Users` 來共用。如果專案不在 `c:\Users` 中，請手動把專案新增到 VirtualBox 的[共用資料夾](https://www.virtualbox.org/manual/ch04.html#sharedfolders)中。

- **把 Microsoft Edge 當做預設瀏覽器**

	如果您在使用 Microsoft Edge 瀏覽器，網站可能不會開啟，原因是 Edge 認為該 IP 位址不安全。如要修正這個問題，請執行下列步驟：1.在 Windows [執行] 方塊輸入 `Internet Options`。2.當 [網際網路選項] 出現時，點選該選項。2.點選 [安全性] 索引標籤。3.選取 [近端內部網路] 區域。4.點選 [網站]。5.將您虛擬機器 (在這個案例中為 Docker 主機) 的 IP 位址新增到清單中。6.在 Edge 中重新整理網頁，此時您應該會看到網站已正常運作。7.如需有關這個問題的詳細資訊，請瀏覽 Scott Hanselman 的部落格文章：[Microsoft Edge can't see or open VirtualBox-hosted local web sites (Microsoft Edge 無法看到或開啟裝載在 VirtualBox 上的本機網站)](http://www.hanselman.com/blog/FixedMicrosoftEdgeCantSeeOrOpenVirtualBoxhostedLocalWebSites.aspx)。
