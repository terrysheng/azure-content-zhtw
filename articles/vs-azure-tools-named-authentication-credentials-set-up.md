...
- 當您從 [發佈 Azure 應用程式] 精靈發佈 Azure 雲端服務時，選擇 [選擇訂用帳戶] 清單中的 **<Manage>**，再選擇 [憑證] 索引標籤，然後選擇 [新增] 或 [編輯] 按鈕。

下列程序假設 [新增訂用帳戶] 對話方塊已開啟。

### 在 Visual Studio 中設定驗證認證

1. 在 [選取現有憑證以進行驗證] 清單中選擇憑證。

1. 選擇 [複製完整路徑] 按鈕。憑證 (.cer 檔案) 的路徑便會複製到剪貼簿。

    >[AZURE.IMPORTANT]若要從 Visual Studio 發佈 Azure 應用程式，您必須將此憑證上傳至管理入口網站。

1. 若要將憑證上傳至管理入口網站，請選擇 **Azure 入口網站**連結。

    管理入口網站隨即開啟。

1. 選擇 **Azure 入口網站**連結。

    [管理入口網站](http://go.microsoft.com/fwlink/?LinkID=213885)隨即開啟。

1. 使用您的 Microsoft 帳戶登入管理入口網站，然後選擇 [雲端服務] 按鈕。

1. 選擇您感興趣的雲端服務。該服務的頁面隨即開啟。
