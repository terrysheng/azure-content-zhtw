...
1. 在下拉式清單中，選擇 **<Create>**，或從清單中選擇現有的憑證。
    ...
    1. 提供新憑證的易記名稱，然後選擇 [確定] 按鈕。新的憑證會出現在下拉式清單方塊中。
    ...
    1. 提供所有必要的資訊之後，選擇 [確定] 按鈕。
    
        啟用遠端存取服務的數個設定會加入至 .cscfg 和 .csdef 檔案。

1. 在 [Microsoft Azure 發佈設定] 精靈中，當您準備好要發佈雲端服務時，選擇 [確定] 按鈕。

    如果您未準備好要發佈，請選擇 [取消] 按鈕。將會儲存組態設定，您可以稍後再發佈雲端服務。

## 使用遠端桌面連接到 Azure 角色

在 Azure 上發佈雲端服務之後，您可以使用 [伺服器總管] 來登入 Azure 所裝載的虛擬機器。

1. 在 [伺服器總管] 中展開 [Azure] 節點，然後展開雲端服務的節點及其中一個角色，以顯示執行個體的清單。
