...
- 您也必須擁有 Azure 帳戶並登入，才能在 [雲端總管] 中檢視 Azure 資源。如需詳細資訊，請參閱[建立 Azure 帳戶並啟用預覽功能](../php-create-account/)。
...
1. 選取您想要瀏覽的訂用帳戶的核取方塊，然後選擇 [套用] 按鈕。
...
根據 Azure 資源而定，也可能出現其他動作和屬性值。例如，除了 [在入口網站中開啟]，Web 應用程式和邏輯應用程式還有 [在瀏覽器中開啟] 和 [附加偵錯工具] 動作。當您選擇儲存體帳戶 blob、佇列或資料表時，將會出現開啟編輯器的動作。Azure 應用程式具有 [URL] 和 [狀態] 屬性，而儲存體資源具有索引鍵和連接字串屬性。
