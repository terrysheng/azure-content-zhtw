...
**Web 通訊端**。設為 [開啟] 以啟用 WebSocket 通訊協定；例如，如果您的 Web 應用程式會使用 [ASP.NET SignalR](http://www.asp.net/signalr) 或 [socket.io](web-sites-nodejs-chat-app-socketio.md)。
...
如果是 .NET 應用程式，這些連接字串就會在執行階段插入 .NET 設定的 `connectionStrings` 設定，並覆寫索引鍵相當於所連結資料庫名稱的現有項目。 
...
例如，如果已將 MySql 連接字串命名為 `connectionstring1`，則可透過環境變數 `MYSQLCONNSTR_connectionString1` 加以存取。
...
**失敗的要求追蹤**。將失敗的要求記錄於 XML 檔案中。檔案會儲存在 /LogFiles/W3SVC*xxx* 之下，其中 xxx 是唯一識別碼。此資料夾內含有一個 XSL 檔案和一或多個 XML 檔案。請務必下載 XSL 檔案，因為 XSL 檔案可提供格式化和篩選 XML 檔案內容的功能。
...
您可在基本或標準模式中上傳自訂網域的 SSL 憑證。如需更多資訊，請參閱[啟動網頁應用程式的 HTTPS](web-sites-configure-ssl-certificate.md)。 
