...
> [AZURE.NOTE] 如為連續的 WebJob，建議您為 Web 應用程式啟用 [永遠開啟]。「永遠開啟」功能提供 [基本] 和 [標準] 兩種模式，它能預防 Web 應用程式遭到卸載，即使它們已閒置一段時間亦是如此。如果您的 Web 應用程式隨時處於載入狀態，則連續執行的 WebJob 將能夠以更可靠的方式執行。 

## <a name="CreateScheduled"></a>建立排定的 WebJob

Azure 管理入口網站尚未具備建立排定之 WebJob 的能力，但在加入該功能之前，您可以使用[舊的入口網站](http://manage.windowsazure.com)來執行這個動作。
...
您可以在[舊的入口網站](http://manage.windowsazure.com)的 Azure 排程器頁面中，進一步設定排定的工作。
