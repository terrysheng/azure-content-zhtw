<properties 
	pageTitle="在 Microsoft Azure 網站中使用 WebJobs 執行背景工作" 
	description="了解如何在 Microsoft Azure 網站中執行背景工作。" 
	services="web-sites" 
	documentationCenter="" 
	authors="cephalin" 
	writer="timamm" 
...
Azure 網站可讓您利用以下三種方法，在網站中執行程式或指令碼：依需要、連續或根據排程。使用 Microsoft Azure WebJobs 不會產生額外的費用。

本文說明如何使用 Azure 管理入口網站來部署 WebJobs。如需如何使用 Visual Studio 或連續傳遞程序進行部署的相關資訊，請參閱[如何將 Azure WebJobs 部署至 Azure 網站](http://azure.microsoft.com/documentation/articles/websites-dotnet-deploy-webjobs)。
...
3. 在 **[內容 (zip 檔案 - 最大 100 MB)]** 方塊中，瀏覽至含有指令碼的 zip 檔案。該 zip 檔案應包含您的可執行檔 (.exe .cmd .bat .sh .php .py .js)，以及執行程式或指令碼所需的任何支援檔案。
	
4. 在 **[如何執行]** 方塊中選擇 **[依需要執行]**。
...
1. 若要建立連續執行的工作，請遵循建立執行一次之工作的步驟，但是 **[如何執行]** 方塊中選取 **[連續執行]**。
	
	![新增連續工作][NewContinuousJob]
	
2. 若要啟動或停止連續執行的工作，請在清單中選取工作，然後在命令列中按一下 **[啟動]** 或 **[停止]**。

> [AZURE.NOTE] 如果您的網站執行多個執行個體，連續執行的工作會在所有執行個體上執行。隨選和排程的工作會在 Microsoft Azure 為負載平衡而選取的單一執行個體上執行。
...
2. 按一下連結可開啟工作的網站工作詳細資料。此頁面能顯示執行之命令的名稱、上次執行時間及成功或失敗。按一下 **[最近的工作執行]** 下的時間可查看進一步的詳細資料。
	
	![WebJobDetails][WebJobDetails]
	
3. **[WebJob 執行詳細資料]** 頁面隨即出現。按一下 **[切換輸出]** 可查看記錄的文字內容。輸出記錄將會是文字格式。 
	
	![Web 工作執行詳細資料][WebJobRunDetails]
...
5. 頁面頂端的 **[WebJobs]** 連結可讓您以便利的方法在歷程記錄儀表板中取得網站工作的清單。
	...
	![連結至網站工作清單][WebJobsLinkToDashboardList]
	
	![歷程記錄儀表板中的工作清單][WebJobsListInJobsDashboard]
