...
1.  在 [WebJobs] 頁面中，按一下工作的 [排程] 連結以瀏覽至 Azure 排程器入口網站頁面。
    ...
    ![連結至 Azure 排程器][連結至 Azure 排程器]

2.  在 [排程器] 頁面中按一下工作。

    ![排程器 入口網站頁面中的工作][排程器 入口網站頁面中的工作]

3.  [工作動作] 頁面隨即開啟，您可以在其中進一步設定工作。

    ![工作動作 PageInScheduler][工作動作 PageInScheduler]
...
如需詳細資訊，請參閱教學課程[開始使用 Microsoft Azure WebJobs SDK][開始使用 Microsoft Azure WebJobs SDK] (英文)。
...
Visual Studio 2013 含有將主控台應用程式專案自動部署為 WebJob 的功能。如需詳細資訊，請參閱[如何將 Azure WebJob 部署至 Azure 網站][如何將 Azure WebJob 部署至 Azure 網站] (英文)。

您也可以使用 FTP、Git 或 Web Deploy 部署 WebJob。如需詳細資訊，請參閱[使用 WebJob 透過 Git 將 .NET 主控台應用程式部署到 Azure][使用 WebJob 透過 Git 將 .NET 主控台應用程式部署到 Azure] (英文) 和[如何部署 Windows Azure WebJob][如何部署 Windows Azure WebJob] (英文)。
