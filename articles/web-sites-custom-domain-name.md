...
<div class="dev-center-tutorial-selector sublanding">
  <a href="/documentation/articles/web-sites-custom-domain-name" title="Custom Domain" class="current">自訂網域</a>
  <a href="/documentation/articles/web-sites-godaddy-custom-domain-name" title="GoDaddy">GoDaddy</a>
  <a href="/documentation/articles/web-sites-network-solutions-custom-domain-name" title="Network Solutions">網路解決方案</a>
...
<div class="dev-center-tutorial-subselector">
  <a href="/documentation/articles/web-sites-custom-domain-name/" title="Websites" class="current">Web 應用程式</a> | <a href="/documentation/articles/web-sites-traffic-manager-custom-domain-name/" title="Web Apps using Traffic Manager">使用流量管理員的 Web 應用程式</a>
</div>
...
若是實際執行的 Web 應用程式，您可能想讓使用者看到自訂網域名稱。本文說明如何使用 [App Service Web Apps](http://go.microsoft.com/fwlink/?LinkId=529714) 設定自訂網域。(本文提供所有網域註冊機構的一般指示。本文最上方的索引標籤為特定註冊機構的部分文章連結。)
...
1. 將網域名稱加入 [Azure 入口網站](http://go.microsoft.com/fwlink/?LinkId=529715)中。
...
當您建立或編輯記錄時，可使用欄位將網域名稱對應至 IP 位址 (針對 A 記錄)，或對應至其他網域 (針對 CNAME 記錄)。若為 CNAME 記錄，您的對應方向是 *from* 自訂網域 *to* 您的 azurewebsites.net 子網域。
...
<h2 id="awverify">建立 "awverify" 記錄 (只有 A 記錄)</h2>

若您建立 A 記錄，Web 應用程式也會要求特定的 CNAME 記錄，用於確認您是否擁有嘗試使用的網域。此 CNAME 記錄必須具有下列形式。

- *若 A 記錄對應至根網域或萬用字元網域：*建立 CNAME 記錄，對應方向從 **awverify.&lt;yourdomain&gt;** 到 **awverify.&lt;yourwebappname&gt;.azurewebsites.net**。例如，針對 **contoso.com** 的 A 記錄，建立 **awverify.contoso.com** 的 CNAME 記錄。
- *若 A 記錄對應至特定子網域：*建立 CNAME 記錄，對應方向從 **awverify.&lt;subdomain&gt;** 到 **awverify.&lt;yourwebappname&gt;.azurewebsites.net**。例如，針對 **blogs.contoso.com** 的 A 記錄，建立 **awverify.blogs.contoso.com** 的 CNAME 記錄。
...
[在 Web 應用程式上啟用網域名稱]: #enable-the-domain-name-on-your-web-app
