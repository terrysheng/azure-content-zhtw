<properties 
	pageTitle="設定 Azure 網站的自訂網域名稱" 
	description="了解如何使用自訂網域名稱搭配 Azure 網站" 
	services="web-sites" 
	documentationCenter="" 
	authors="blackmist" 
	manager="wpickett" 
...
當您建立網站時，Azure 會將它指派給 azurewebsites.net 的子網域。例如，如果您的網站名為 **contoso**，則 URL 是 **contoso.azurewebsites.net**。Azure 也會指派虛擬 IP 位址。
...
> [AZURE.NOTE] 若您刪除網站後再重新建立，或是將網站模式變更回免費，則 IP 位址可能會變更。
...
當您建立或編輯記錄時，可使用欄位將網域名稱對應至 IP 位址 (針對 A 記錄)，或對應至其他網域 (針對 CNAME 記錄)。若為 CNAME 記錄，您要*from*自訂網域對應*to*azurewebsites.net 子網域。 
...
- *若 A 記錄會對應根網域或萬用字元網域：*請建立從 **awverify.&lt;yourdomain&gt;** 對應至 **awverify.&lt;yourwebsitename&gt;.azurewebsites.net** 的 CNAME 記錄。例如，若 A 記錄屬於 **contoso.com**，請為 **awverify.contoso.com** 建立 CNAME 記錄。
- *若 A 記錄會對應特定的子網域：*請建立從 **awverify.&lt;subdomain&gt;** 對應至 **awverify.&lt;yourwebsitename&gt;.azurewebsites.net** 的 CNAME 記錄。例如，若 A 記錄屬於 **blogs.contoso.com**，請為 **awverify.blogs.contoso.com** 建立 CNAME 記錄。
