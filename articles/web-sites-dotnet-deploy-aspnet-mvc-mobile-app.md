...
在本教學課程中，您要將行動功能加入 [入門專案][StarterProject] (英文) 裡提供的簡單會議清單應用程式。如同在 Internet Explorer 11 F12 開發人員工具的瀏覽器模擬器中所見，以下螢幕擷取畫面示範完成的應用程式中的 ASP.NET 工作階段。

![][FixedSessionsByTag]

您可以使用 Internet Explorer 11 F12 開發人員工具和 [Fiddler 工具][Fiddler] (英文)，來協助偵錯您的應用程式。
...
1. 若要安裝 Azure SDK for .NET，請按一下底下連結：如果您尚未安裝 Visual Studio 2013，按下該連結會進行安裝。本教學課程需要安裝 Visual Studio 2013。[Azure SDK for Visual Studio 2013][AzureSDKVs2013] (英文)
1. 在 [Web Platform Installer] 視窗中，按一下 [**安裝**] 並繼續進行安裝。

	![Web Platform Installer - Azure SDK for .NET][WebPIAzureSdk23NetVS13]

您還需要一個行動瀏覽器模擬器。下列任一項目都可
使用：

-   [Internet Explorer 11 F12 開發人員工具][EmulatorIE11]中的瀏覽器模擬器 (使用於所有行動
...
選擇 [**解除封鎖**] 按鈕。(解除封鎖會防止
在您嘗試使用從 Web 下載的.*.zip* 檔案時，出現
安全性警告。)

4.	在 *Mvc5Mobile.zip* 檔案上按一下滑鼠右鍵，然後選取 [**解壓縮​​全部​​**] 可
解壓縮檔案。 

5. 	在 Visual Studio 中，開啟 *Mvc5Mobile.sln* 檔案。

6.  在 [方案總管] 中，於專案上按一下滑鼠右鍵，再按一下 [**發佈**]。

	![][DeployClickPublish]

7.	按一下 [發佈 Web] 中的 [**Windows Azure 網站**]。
...
13.	[發佈 Web] 對話方塊會填入新網站的網站設定。按一下 [**發佈**]。
...
>[WACOM.NOTE] 當您在 Visual Studio 中偵錯 MVC 5 應用程式時，可以再次將網站發佈到 Windows Azure，以直接從行動瀏覽器或瀏覽器模擬器確認作用中的網站。

該顯示內容在行動裝置上非常清楚易讀。您也可以看到一些 Bootstrap CSS 架構所套用的視覺效果。按一下 [**ASP.NET**] 連結。

![][SessionsByTagASP.NET]

ASP.NET 標籤檢視會縮放至適合螢幕的大小，而這是 Bootstrap 自動為您執行的效果。但您可以改善此檢視，使其更適合行動瀏覽器。例如，[**日期**] 欄難以閱讀。您將在稍後的教學課程中變更 *AllTags* 檢視，使其更適合行動用途。
...
*bootstrap.js* 和 *bootstrap.css* 的程式碼行。下列程式碼會顯示 `RegisterBundles` 方法變更之後的最後
兩個陳述式：
...
您會發現可摺疊的導覽列現在只是一般的未排序清單。再次按一下 [**依標籤瀏覽**]，然後按一下 [**ASP.NET**]。在行動裝置模擬器檢視中，您可以看到它不再是縮放至適合螢幕的大小，而且您必須左右捲動才能查看右側資料表。
...
Bootstrap 並非專屬於 ASP.NET MVC 5，您也可以在所有 Web 應用程式中利用這些功能。但它目前內建於 ASP.NET MVC 5 專案範本中，所以 MVC 5 Web 應用程式可根據預設利用 Bootstrap。
	...
	如需 Bootstrap 的詳細資訊，請移至 [Bootstrap][BootstrapSite] (英文) 網站。
...
若要開始，請將 *Views\\Shared\\\_Layout.cshtml* 複製到 *Views\\Shared\\\_Layout.Mobile.cshtml*。開啟 *\_Layout.Mobile.cshtml*，並將標題從 **MVC5 Application** 變更為 **MVC5 Application (Mobile)**。

在每個導覽列的 `Html.ActionLink` 呼叫當中，將每個 *ActionLink* 連結中的 "Browse by" 移除。下列程式碼會顯示行動配置檔案的已完成 `<ul class="nav navbar-nav">` 標籤。
...
開啟 *Global.asax* 檔案，並將下列程式碼新增至 `Application_Start` 方法的底部。
...
>[WACOM.NOTE] 新增行動瀏覽器專用的顯示模式時，例如 iPhone 和 Android，請務必將第一個引數設為 `0` (插入於清單的頂端)，才能確保瀏覽器的專用模式會優先於行動範本 (*.Mobile.cshtml)。若位於清單頂端的是行動範本，則會優先選取該行動範本，而不是您想要的顯示模式 (第一個相符的會成功，而行動範本符合所有行動瀏覽器)。 

在程式碼中，以滑鼠右鍵按一下 `DefaultDisplayMode`，選擇 [**解析**]，
然後選擇 [`using System.Web.WebPages`]。這會新增
`System.Web.WebPages` 命名空間的參考，您可以在此命名空間
定義 `DisplayModeProvider` 和 `DefaultDisplayMode` 類型。
...
儲存變更。將 *Views\\Shared\\\_Layout.Mobile.cshtml* 檔案複製到 *Views\\Shared\\\_Layout.iPhone.cshtml*。開啟新的檔案，然後將標題從 `MVC5 Application (Mobile)` 變更為 `MVC5 Application (iPhone)`。

將 *Views\\Home\\AllTags.Mobile.cshtml* 檔案複製到 *Views\\Home\\AllTags.iPhone.cshtml*。在新檔案中，將 `<h2>` 元素從 "Tags (M)" 變更為 "Tags (iPhone)"。

執行應用程式。執行行動瀏覽器模擬器，請確認其使用者代理程式設為 "iPhone"，然後瀏覽至 *AllTags* 檢視。如果使用 Internet Explorer 11 F12 開發人員工具中的模擬器，請將模擬設為以下內容：
...
在行動瀏覽器中，選取 [**演講者**] 連結。因為沒有行動檢視 (*AllSpeakers.Mobile.cshtml*)，請使用行動配置檢視 (*\_Layout.Mobile.cshtml*) 呈現預設的演講者檢視 (*AllSpeakers.cshtml*)。如下所示，標題 **MVC5 Application (Mobile)** 會在 *\_Layout.Mobile.cshtml* 中加以定義。

![][AllSpeakers_LayoutMobile]

您可以在 *Views\\\_ViewStart.cshtml* 檔案中，將 `RequireConsistentDisplayMode` 設定設為 true，以全域停用預設 (非行動) 檢視，使其無法在行動配置內顯示，如下所示：
...
將 `RequireConsistentDisplayMode` 設為 `true` 時，行動配置 (*\_Layout.Mobile.cshtml*) 僅適用於行動檢視 (也就是當檢視檔案屬於 ***ViewName**.Mobile.cshtml* 格式時)。若行動配置不適用於您的非行動檢視，您可能會想要將 `RequireConsistentDisplayMode` 設為 `true`。以下螢幕擷取畫面顯示將 `RequireConsistentDisplayMode` 設為 `true` 時，[*演講者*] 頁面會如何呈現 (頂端導覽列中沒有 "(Mobile)" 字串)。

![][AllSpeakers_LayoutMobileOverridden]

您可以在檢視檔案中將 `RequireConsistentDisplayMode` 設為 `false`，以停用特定檢視中的一致顯示模式。*Views\\Home\\AllSpeakers.cshtml* 檔案中的下列標記會將 'RequireConsistentDisplayMode' 設為 'false'：
...
在本節中，我們知道如何建立行動配置和檢視，以及如何建立特定裝置 (例如 iPhone) 的配置和檢視。不過，Bootstrap CSS 架構的主要優點是回應靈敏的配置，這表示單一樣式表可以套用到桌面、電話和平板電腦瀏覽器中，並建立一致的外觀及操作。您將在下一節看到如何利用 Bootstrap 建立適合行動的檢視。
...
`<div>` 標籤中的 `class="list-group"` 屬性會將
Bootstrap 清單樣式套用到每個連結，而 `class="input-group-item"` 屬性
會將 Bootstrap 清單項目樣式套用到每個連結。

重新整理行動瀏覽器。更新的檢視外觀如下所示：
...
雖然已經改善行動瀏覽器檢視，但要瀏覽冗長的演講者清單還是很不方便。Bootstrap 並沒有現成的搜尋篩選功能，但您可以用數行程式碼來新增此功能。首先要先將搜尋方塊新增至檢視，然後與 JavaScript 程式碼連結，以取得篩選功能。在 *Views\\Home\\AllSpeakers.cshtml* 中，在 \<h2\> 標籤後面加上 \<form\> 標籤，如下所示：
...
請注意，`<form>` 和 `<input>` 標籤都已套用 Bootstrap 樣式。`<span>` 元素會將 Bootstrap [glyphicon][] 加入 search box.
...
您也需要在註冊的套件中包含 filter.js。開啟 *App\_Start\\BundleConfig.cs*，並變更第一組套件。變更第一個 `bundles.Add` 陳述式 (用於 **jquery** 套件) 以包含 *Scripts\\filter.js*，如下所示： 

     bundles.Add(new ScriptBundle("~/bundles/jquery").Include(
                "~/Scripts/jquery-{version}.js",
                "~/Scripts/filter.js"));

**jquery** 套件已經由預設的 *\_Layout* 
...
和 *Speakers* 檢視一樣，*Tags* 檢視可以讀取，但連結
不但小，而且很難在行動裝置上點選。若您使用先前描述的程式碼變更 (但在 *Views\\Home\\AllTags.cshtml* 中包含下列 `Html.ActionLink` 方法語法)，您就可以像修正 *Tags* 檢視一樣修正 *Speakers* 檢視：
...
*Tags* 檢視一樣 (若您使用先前描述的程式碼變更，但在 *Views\\Home\\AllDates.cshtml* 中包含下列 `Html.ActionLink` 方法語法)：
...
您可以進一步改善 *Dates* 檢視，方法是按日期組織日期時間
值。這可透過 Bootstrap 完成
[面板][]樣式。將
*Views\\Home\\AllDates.cshtml* 檔案的內容取代為
下列程式碼：
...
此程式碼會為清單中的每個不同日期建立個別的 `<div class="panel panel-primary">` 標籤
，並在個別連結中和以前一樣使用 [連結清單群組][]
。以下是執行此程式碼時的行動瀏覽器外觀
：
...
開啟 *Views\\Home\\SessionByCode.cshtml*，並將內容取代為
下列標記：
