...
建立可連接至在 Azure 的虛擬機器上執行之 MongoDB 的 Azure 網站
=============================================================

使用 Git，即可將 ASP.NET 應用程式部署至 Azure 網站。在本教學課程中，您將建置簡易的前端 ASP.NET MVC 工作清單應用程式，以連接至在 Azure 的虛擬機器中執行的 MongoDB 資料庫。[MongoDB](http://www.mongodb.org) (英文) 是受歡迎的高效能開放原始碼 NoSQL 資料庫。在部署電腦上執行和測試 ASP.NET 應用程式之後，您將使用 Git 將此應用程式上傳至 Azure 網站。
...
-   MongoDB 的 C\# 驅動程式。如需針對 MongoDB 開發 C\# 應用程式的詳細資訊，請參閱 MongoDB＜[CSharp Language Center](http://docs.mongodb.org/ecosystem/drivers/csharp/)＞(英文)。
-   ASP .NET Web 應用程式架構。您可以在 [ASP.net 網站](http://www.asp.net/)了解此架構。
-   ASP .NET MVC Web 應用程式架構。您可以在 [ASP.NET MVC 網站](http://www.asp.net/mvc)了解此架構。
-   Azure。您可以在 [Azure](http://www.windowsazure.com) 開始進行讀取。
...
-   若要建立 Windows 虛擬機器和安裝 MongoDB，請參閱＜[在 Azure 中執行 Windows Server 的虛擬機器上安裝 MongoDB](/en-us/manage/windows/common-tasks/install-mongodb/)＞。
-   此外，若要建立 Linux 虛擬機器和安裝 MongoDB，請參閱＜[在 Azure 中執行 CentOS Linux 的虛擬機器上安裝 MongoDB](/en-us/manage/linux/common-tasks/mongodb-on-a-linux-vm/)＞。

在 Azure 中建立虛擬機器並安裝 MongoDB 之後，請務必記住虛擬機器的 DNS 名稱 (例如，"testlinuxvm.cloudapp.net") 以及您在端點中指定之 MongoDB 的外部連接埠。稍後在教學課程中需要此資訊。

### 安裝 Visual Studio

從安裝和執行 [Visual Studio Express 2013 for Web](http://www.microsoft.com/visualstudio/eng/2013-downloads#d-2013-express) 或 [Visual Studio 2013](http://www.microsoft.com/visualstudio/eng/2013-downloads) 著手。
...
若要變更頁面最上方的標題，請在 **[方案總管]** 中開啟 *Views\\Shared\\\_Layout.cshtml* 檔案，並以 "My Task List Application" 取代導覽列中的 "Application name"，使其看起來如下：
...
以執行您在本教學課程的[建立虛擬機器和安裝 MongoDB](#virtualmachine) 步驟中建立之 MongoDB 的虛擬機器 DNS 名稱取代 `<vm-dns-name>`。若要尋找虛擬機器的 DNS 名稱，請移至 Azure 管理入口網站，選取 **[虛擬機器]**，然後尋找 **[DNS 名稱]**。
...
如需 MongoDB 連接字串的詳細資訊，請參閱＜[連接](http://www.mongodb.org/display/DOCS/Connections)＞(英文)。
...
將應用程式部署至 Azure 網站將 ASP.NET 應用程式部署至 Azure 網站
---------------------------------------------------------------

在這一節中，您將建立網站並使用 Git 部署 My Task List ASP.NET 應用程式。

### 建立 Azure 網站   
在這一節中，您將建立 Azure 網站。

1.  開啟網頁瀏覽器並瀏覽至 [Azure 管理入口網站](http://manage.windowsazure.com)。使用您的 Azure 帳戶進行登入。
...
### 使用 Git 將 ASP.NET 應用程式部署至網站 在這一節中，您將使用 Git 部署 My Task List 應用程式。
...
您現在已成功將 ASP.NET 應用程式部署至 Azure 網站。若要檢視網站，請按一下 **[儀表板]** 頁面的 **[網站 URL]** 欄位。如需針對 MongoDB 開發 C\# 應用程式的詳細資訊，請參閱＜[CSharp Language Center](http://docs.mongodb.org/ecosystem/drivers/csharp/)＞(英文)。
