...
## <a name="sitemanagement"></a>網站組態與管理

Visual Studio 可讓您存取管理入口網站中可用的網站管理功能與組態設定功能的子集。本節將說明可用的項目。

1.  如果您尚未在 Visual Studio 中登入 Azure，按一下 [伺服器總管] 中的 [連線到 Azure] 按鈕。

    替代方式為安裝可讓您存取帳戶的管理憑證。管理憑證讓 [伺服器總管] 得以存取其他的 Azure 服務 (SQL 資料庫與行動服務)。如果您選擇安裝憑證，請以滑鼠右鍵按一下 [伺服器總管] 中的 [Azure] 節點，然後按一下內容功能表中的 [管理訂閱]。在 [管理 Azure 訂用帳戶] 對話方塊中，按一下 [憑證] 索引標籤，再按一下 [匯入]。請依照指示下載，然後匯入您 Azure 帳戶的訂用帳戶檔案 (亦稱為 *.publishsettings* 檔案)。
...
4.  在 `ViewBag.Message` 字行中[設定中斷點][在偵錯模式中執行]。
...
10. 針對 Visual Studio 2012 含 Update 4：<span id="vs2012"></span></a>
    ...
    -   當系統提示您輸入認證時，請輸入具有發行網站權限的使用者名稱與密碼。若要取得這些認證，請前往管理入口網站中網站的 [儀表板] 索引標籤，然後按一下 [下載發行設定檔]。在文字編輯器中開啟檔案，這時您會在第一個出現的 **userName=** 與 **userPWD=** 的後面看到使用者名稱與密碼。
...
-   不建議在生產環境中執行偵錯模式。如果您的生產網站並未擴充為多個伺服器執行個體，則偵錯程序會佔用通往您的唯一 Web 伺服器執行個體的流量。如果您不具備多個 Web 伺服器執行個體，當您連結至偵錯工具時，會取得一個隨機產生的執行個體，而且您將無法確認瀏覽器要求是否會通往該執行個體。此外，通常我們不會將偵錯組建部署到生產環境中，而且版本組建的編譯器最佳化作業將會使系統無法逐行顯示您的原始程式碼中所發生的事情。針對生產環境問題的疑難排解，您的最佳資源將是應用程式追蹤與 Web 伺服器記錄。
...
## <a name="apptracelogs"></a><span class="short-header">應用程式記錄</span>建立並檢視應用程式追蹤記錄

您將在本節執行下列工作：

-   將追蹤陳述式新增至您在[稍早的教學課程][開始使用 Azure 和 ASP.NET]中所建立的 Web 專案。
-   當您在本機上執行專案時檢視記錄。
...
2.  開啟應用程式 Web.config 檔 (位於專案資料夾中的檔案) 並將 `<system.diagnostics>` 元素加入檔案結尾 `</configuration>` 元素關閉處前：
...
5.  在瀏覽器視窗的網址列中，將 *trace.axd* 新增至 URL，然後按 Enter 鍵 (URL 類似於 <http://localhost:53370/trace.axd>)。
...
6.  在 [應用程式追蹤] 頁面上，按一下 [檢視詳細資料]。
    ...
    根據預設，`trace.axd` 只能在本機使用。如果您想從遠端網站使用它，可以將 `localOnly="false"` 加入 *Web.config* 檔案中的 `trace` 元素，如以下範例所示：
...
## <a name="webserverlogs"></a><span class="short-header">Web 伺服器記錄</span>檢視 Web 伺服器記錄
...
## <a name="detailederrorlogs"></a><span class="short-header">錯誤記錄</span>檢視詳細的錯誤訊息記錄
...
## <a name="storagelogs"></a><span class="short-header">儲存體記錄</span>檢視儲存體記錄
...
4.  在 Visual Studio 的 [Azure 網站] 視窗中，按一下 [記錄] 索引標籤，然後按一下 [設定記錄]。
    ...
    這麼做會在管理入口網站中開啟您的網站的 [設定] 索引標籤。要到達此頁面的另一個方法，就是依序按一下 [網站] 索引標籤和您的網站，然後按一下 [設定] 索引標籤。
...
## <a name="failedrequestlogs"></a><span class="short-header">失敗要求記錄</span>檢視失敗要求追蹤記錄
