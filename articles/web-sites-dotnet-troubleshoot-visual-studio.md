...
# 在 Visual Studio 中疑難排解 Azure 網站

在 Web 應用程式的開發與測試期間，您可以[在偵錯模式中執行][1]或是使用 [IntelliTrace][2] 來進行疑難排解。您可以在 IIS Express 本機上或是在遠端的 Azure 網站於偵測模式中執行。但是對於只在生產環境中出現的錯誤，最佳的偵錯方式應該是檢閱應用程式碼或 Web 伺服器所建立的記錄。本教學課程將示範如何使用可協助偵錯在 Azure 網站中執行之應用程式的 Visual Studio 工具，方法是藉由自遠端在偵錯模式中執行，或檢視應用程式與 Web 伺服器記錄。

您將了解：

* Visual Studio 中所提供的 Azure 網站管理功能有哪些。
* 如何使用 Visual Studio 遠端檢視對遠端網站進行快速變更。
* 如何在 Azure 網站中還在執行專案時，從遠端執行偵測模式。
...
2.  [網站組態與管理](#sitemanagement)
3.  [遠端檢視](#remoteview)
4.  [遠端偵錯](#remotedebug)
5.  [診斷記錄概觀](#logsoverview)
6.  [建立並檢視應用程式追蹤記錄](#apptracelogs)
7.  [檢視 Web 伺服器記錄](#webserverlogs)
8.  [檢視詳細的錯誤訊息記錄](#detailederrorlogs)
9.  [下載檔案系統記錄](#downloadlogs)
10. [檢視儲存體記錄](#storagelogs)
11. [檢視失敗要求記錄](#failedrequestlogs)
12. [後續步驟](#nextsteps)

<h2><a name="prerequisites"></a><span  class="short-header">必要條件</span></h2>


本教學課程可運用於開發環境、Web 專案與您在[開始使用 Azure 和 ASP.NET](/en-us/develop/net/tutorials/get-started/) 中所設定的 Azure 網站。本教學課程中所提供的程式碼範例適用於 C# MVC Web 應用程式，但是疑難排解程序則是與 Visual Basic 和 Web Form 應用程式一樣。
...
1.  如果您尚未在 Visual Studio 中登入 Azure，按一下 **伺服器總管** 中的 [連線到 Azure]**** 按鈕。
    ...
    替代方式為安裝可讓您存取帳戶的管理憑證。管理憑證讓 [伺服器總管] 得以存取其他的 Azure 服務 (SQL 資料庫與行動服務)。如果您選擇安裝憑證，請以滑鼠右鍵按一下 **伺服器總管** 中的 **Azure** 節點，然後按一下內容功能表中的 **管理訂閱**。在 **管理 Azure 訂閱** 對話方塊中，按一下 **憑證** 索引標籤，再按一下 **匯入**。請依照指示下載，然後匯入您 Azure 帳戶的訂閱檔案 (亦稱為 *.publishsettings* 檔案)。


   
    > [WACOM.NOTE] 如果您下載了訂閱檔案，請將其儲存到原始程式碼目錄以外的資料夾 (例如在 Downloads
    > 資料夾)，然後在匯入完成後刪除該檔案。惡意使用者一旦能夠存取此訂閱檔案，就能夠編輯、建立和刪除您的 Azure 服務。
    
    如需從 Visual Studio 連線至 Azure 資源的詳細資訊，請參閱[管理帳戶、訂閱和系統管理角色][3]。

2.  在 **伺服器總管** 中，展開 **Azure**，然後展開 **網站**。

3.  以滑鼠右鍵按一下您在[開始使用 Azure 和 ASP.NET](/en-us/develop/net/tutorials/get-started/) 中建立的網站節點，然後按一下 **檢視設定**。
    ...
    **Azure 網站** 索引標籤會顯示，您會看到 Visual Studio 中所提供的網站管理與組態工作。
    ...
    如需此視窗中的應用程式設定與連接字串方塊的詳細資訊，請參閱 [Azure 網站：應用程式字串與連接字串的運作方式][4] (英文)。
    
    如果您想要執行無法在此視窗中完成的網站管理工作，可以按一下 **Full Web Site Settings** 來開啟管理入口網站的瀏覽器視窗。如需詳細資訊，請參閱[如何設定網站](/en-us/manage/services/web-sites/how-to-configure-websites/#howtochangeconfig)。

<h2><a name="remoteview"></a>遠端檢視</h2>


部署網站時，通常會將 Web.config 檔案中的 `customErrors` 旗標設定為 `On` 或 `RemoteOnly`，這表示出現問題時，您將收不到有用的錯誤訊息。對許多錯誤而言，您只會看到如下列之一的頁面。

**'/' 應用程式中有伺服器錯誤：** ![Unhelpful error page](./media/web-sites-dotnet-troubleshoot-visual-studio/genericerror.png)

**發生錯誤：** ![Unhelpful error page](./media/web-sites-dotnet-troubleshoot-visual-studio/genericerror1.png)

**網站無法顯示該網頁** ![Unhelpful error page](./media/web-sites-dotnet-troubleshoot-visual-studio/genericerror2.png)

要找到錯誤原因最簡單的方式，往往就是啟用詳細的錯誤訊息，而以上第一個螢幕擷取畫面說明的是其做法。該做法需要在部署的 Web.config 檔案中進行變更。您可以編輯專案中的 *Web.config* 檔案，然後重新部署專案，或是建立一個 [Web.config 轉換]**5]，並部署偵錯組建，但是以下方式更快：在 [方案總管** 中，您可以使用*遠端檢視*功能，直接檢視與編輯遠端網站上的檔案。
...
本節說明如何使用您在[開始使用 Azure 和 ASP.NET](/en-us/develop/net/tutorials/get-started/) 中所建立的專案進行遠端偵錯。

1.  開啟您在[開始使用 Azure 和 ASP.NET](/en-us/develop/net/tutorials/get-started/) 中建立的 Web 專案。

2.  開啟 *Controllers\\HomeController.cs*。

3.  刪除 `About()` 方法，然後插入下列程式碼加以取代。
...
4.  在 `ViewBag.Message` 字行中[設定中斷點][1]。

5.  在 **方案總管** 中，於專案上按一下滑鼠右鍵，再按一下 **發行**。

6.  在 **設定檔** 下拉式清單中，選取您在[開始使用 Azure 和 ASP.NET](/en-us/develop/net/tutorials/get-started/) 中使用的相同設定檔。
...
10. 針對 Visual Studio 2012 含 Update 4：
    ...
    * 當系統提示您輸入認證時，請輸入具有發行網站權限的使用者名稱與密碼。若要取得這些認證，請前往管理入口網站中的網站 [儀表板] 索引標籤，然後按一下 **Download the publish profile**。在文字編輯器中開啟檔案，這時您會在第一個出現的 **userName=** 與 **userPWD=** 的後面看到使用者名稱與密碼。
...
13. 為 `currentTime` 變數輸入新的值，例如「現在於 Azure 中執行」。
...
* 不建議在生產環境中執行偵錯模式。如果您的生產網站並未擴充為多個伺服器執行個體，則偵錯程序會佔用通往您的唯一 Web 伺服器執行個體的流量。如果您不具備多個 Web 伺服器執行個體，當您連結至偵錯工具時，會取得一個隨機產生的執行個體，而且您將無法確認瀏覽器要求是否會通往該執行個體。此外，通常我們不會將偵錯組建部署到生產環境中，而且版本組建的編譯器最佳化作業將會使系統無法逐行顯示您的來源程式碼中所發生的事情。針對生產環境問題的疑難排解，您的最佳資源將是應用程式追蹤與 Web 伺服器記錄。

* 進行遠端偵錯時，避免長時間在中斷點停止運作。Azure 會將停止超過幾分鐘的處理序視為無回應的處理序，並將其關閉。

* 在偵錯期間，伺服器會將資料傳送至 Visual Studio，進而影響頻寬付費情況。如需關於頻寬費率的詳細資訊，請參閱 [Azure 定價](/en-us/pricing/calculator/)。
...
* 如果您發現偵錯工具無法逐步執行您要偵錯的程式碼，可能需要變更「Just My Code」設定。如需詳細資訊，請參閱[將逐步執行限制於 Just My Code][6]。

* 當您啟用遠端偵錯功能時，伺服器上會啟動計時器，並在 48 小時後自動關閉此功能。此 48 小時的限制是為了安全與效能起見而設計的功能。若需要，您可以輕鬆開啟這項功能，次數不限。當您不需要偵錯時，建議您將其保持為停用。

* 您可以將此偵錯工具手動連結到任何處理序。您不只可以偵錯網站處理序 (w3wp.exe)，還可以偵錯其他處理序，例如 [WebJobs][7]。如需如何在 Visual Studio 中使用偵錯模式的詳細資訊，請參閱[Visual Studio
  偵錯][8]。

<h2><a name="logsoverview"></a><span  class="short-header">記錄概觀</span>診斷記錄概觀</h2>


在 Azure 網站中執行的 ASP.NET 應用程式，可建立下列各種記錄：

* **應用程式追蹤記錄**  
  此應用程式會呼叫 [System.Diagnostics.Trace][9]
  類別的方法來建立這些記錄。
* **Web 伺服器記錄**  
...
記錄會寫入網站內檔案系統中 *LogFiles* 資料夾內的檔案。Web 伺服器記錄與應用程式記錄也可以寫入至 Azure 儲存體帳戶。您可以在儲存體帳戶中保留大於檔案系統可容納的記錄檔數量。當您使用檔案系統時，最大的記錄檔大小為 100 MB。(檔案系統記錄僅適用於短期保留之用。Azure 會在達到此上限之後刪除舊的記錄檔以騰出空間給新的記錄檔使用。)

<h2><a name="apptracelogs"></a><span  class="short-header">應用程式記錄</span>建立並檢視應用程式追蹤記錄</h2>


您將在本節執行下列工作：

* 將追蹤陳述式新增至您在[稍早的教學課程](/en-us/develop/net/tutorials/get-started/)中所建立的 Web 專案。
* 當您在本機上執行專案時檢視記錄。
...
1.  按 F5 以在偵錯模式中執行應用程式。
    ...
    預設的追蹤接聽程式會將所有追蹤輸出連同其他「偵錯」輸出，一起寫入到 **輸出** 視窗。下圖顯示您新增至 `Index` 方法的追蹤陳述式的輸出。
...
2.  開啟應用程式 Web.config 檔 (專案資料夾中的那個) 並將 `<system.diagnostics>` 元素新增至檔案結尾 `</configuration>` 元素關閉處前：
  		...
  		<system.diagnostics>
		    <trace>
		      <listeners>
		        <a dd name="WebPageTraceListener"
...
1.  在 Web.config 檔案的 `<system .web>` 下方，新增[追蹤元素][10]，如以下範例所示：
    
         <trace  enabled="true" writeToDiagnosticsTrace="true" mostRecent="true" pageOutput="false" />

2.  按 CTRL+F5 執行應用程式。

3.  在瀏覽器視窗的網址列中，將 *trace.axd* 新增至 URL，然後按 Enter (URL 類似於 http://localhost:53370/trace.axd)。

4.  在 **應用程式追蹤** 頁面上，按一下 **檢視詳細資料**。
    
    ![trace.axd](./media/web-sites-dotnet-troubleshoot-visual-studio/tws-traceaxd1.png)
    
    **要求詳細資訊** 頁面隨即顯示，而且在 **追蹤資訊** 區段中，您會看到先前新增至 `Index` 方法的追蹤陳述式輸出。
...
3.  在 **伺服器總管** 中，以滑鼠右鍵按一下網站，然後選取 **View Streaming Logs in Output Window**。
...
5.  在 **伺服器總管** 中，以滑鼠右鍵按一下網站，然後按一下 **檢視設定** (如同稍早所做的動作)。
...
<h2><a name="webserverlogs"></a><span  class="short-header">Web 伺服器記錄</span>檢視 Web 伺服器記錄</h2>
...
<h2><a name="detailederrorlogs"></a><span  class="short-header">錯誤記錄</span>檢視詳細的錯誤訊息記錄</h2>


針對導致錯誤回應代碼 (400 或以上) 之 HTTP 要求，詳細的錯誤訊息記錄可提供部分額外資訊。為了在 **輸出** 視窗中檢視記錄，您必須在網站上啟用記錄，然後告訴 Visual Studio 您想要監視記錄。
...
4.  在瀏覽器視窗的網址列中，為 URL 新增額外的字元以引發 404 錯誤 (例如，`http://localhost:53370/Home/Contactx`)，然後按 Enter。
...
<h2><a name="downloadlogs"></a><span  class="short-header">下載記錄</span>下載檔案系統記錄</h2>
...
2.  將 *.zip* 檔案解壓縮後，您會看到下列資料夾結構：
    ...
    * Web 伺服器記錄位於 *LogFiles\\http\\RawLogs* 資料夾的 *.log* 檔案中。您可以使用[記錄檔剖析器][11] (英文) 之類的工具來檢視與操作這些檔案。
...
<h2><a name="storagelogs"></a><span  class="short-header">儲存體記錄</span>檢視儲存體記錄</h2>


應用程式追蹤記錄也可以傳送至 Azure 儲存體帳戶，以便您在 Visual Studio 中加以檢視。若要這麼做，請建立一個儲存體帳戶，並在管理入口網站中啟用儲存體記錄，然後就可透過 **Azure 網站** 視窗的 **記錄** 索引標籤加以檢視。
...
資料表可讓您輕鬆地在線上檢視記錄的詳細資料，而且資料表還支援串流；您可以查詢資料表中的記錄，並即時看到正在建立的新記錄。Blob 則可讓您輕鬆地將記錄下載到檔案，並運用 HDInsight 加以分析，因為 HDInsight 知道如何處理 Blob 儲存體。如需詳細資訊，請參閱[資料儲存體選項 (運用 Azure 建構真實的雲端應用程式)][12] 中的 **Hadoop 與
MapReduce** (英文)。
...
1.  在 **伺服器總管** 中，以滑鼠右鍵按一下網站，然後按一下 **在管理入口網站中開啟**。
...
4.  在 Visual Studio 的 **Azure 網站** 視窗中，按一下 **記錄** 索引標籤，然後按一下 **設定記錄**。
    ...
    ![下載的檔案](./media/web-sites-dotnet-troubleshoot-visual-studio/tws-configlogging.png)
    
    這麼做會在管理入口網站中開啟您的網站的 **設定** 索引標籤。要到達此頁面的另一個方法，就是按一下 **網站** 索引標籤，按一下您的網站，然後按一下 **設定** 索引標籤。

5.  在管理入口網站的 **設定** 索引標籤中，向下捲動至應用程式診斷區段，然後將 [Application Logging (Table Storage)]**** 變更為 **開啟**。
...
9.  在管理入口網站的 **設定** 索引標籤中，按一下 **儲存**。

10. 在顯示該應用程式網站的瀏覽器視窗中，依序按一下 **首頁**、**關於**、**連絡人**。
...
12. 按一下 **View all application logs**。
    ...
    此檢視會顯示在其他任何檢視中都不會看到的額外欄位。此檢視還可讓您藉由特殊的查詢產生器 UI 來篩選記錄，以便建構查詢。如需詳細資訊，請參閱[使用伺服器總管瀏覽儲存體資源][13]中的「使用表格資源 - 篩選實體」。
...
<h2><a name="failedrequestlogs"></a><span  class="short-header">失敗要求記錄</span>檢視失敗要求追蹤記錄</h2>
...
2.  在顯示該網站的瀏覽器視窗網址列中，將額外字元新增至 URL 並按一下 Enter 以引發 404 錯誤。
    
    這麼做會讓系統建立失敗要求追蹤記錄，以下步驟將說明如何檢視或下載記錄。

3.  在 Visual Studio 中，按一下 **Azure 網站** 視窗之 **組態** 索引標籤中的 **在管理入口網站中開啟**。
...
7.  在新瀏覽器視窗中，前往位於您的網站之管理入口網站 **儀表板** 索引標籤中的 **FTP 主機名稱** 下方的 URL。**FTP 主機名稱** 位於 **Quick Glance** 區段中的 [Deployment / FTP User]**** 附近。

8.  使用您先前建立的 FTP 認證登入 (包括該使用者名稱的網站名稱前置詞)。
...
在了解 Visual Studio 如何讓您輕鬆地檢視 Azure 網站所建立的記錄之後，您可能想要進一步了解疑難排解 Azure 網站、追蹤 ASP.NET 應用程式，以及分析 Web 伺服器記錄。

### Azure 網站疑難排解

如需疑難排解 Azure 網站 (WAWS) 的詳細資訊，請參閱下列資源：

* [疑難排解 Azure](/en-us/develop/net/best-practices/troubleshooting/) (英文) 
  
	這份基本簡介的白皮書有簡短介紹 WAWS。

* [疑難排解網站](/en-us/develop/net/best-practices/troubleshooting-web-sites/) (英文)  這份簡介重點在 WAWS。
...
* [如何監視網站](/en-us/manage/services/web-sites/how-to-monitor-websites/)  
  [設定診斷與下載記錄](/en-us/manage/services/web-sites/how-to-monitor-websites/#howtoconfigdiagnostics)一節提供了疑難排解文件所未能納入的寶貴資訊。

如需特定疑難排解問題的說明，請在下列任一個論壇中開啟一段討論串：

* [ASP.NET 網站上的 Azure 論壇][14]。
* [MSDN 上的 Azure 論壇][15]。
* [StackOverflow.com][16]。

### 在 Visual Studio 中偵錯

如需如何在 Visual Studio 中使用偵錯模式的詳細資訊，請參閱[在 Visual Studio 中偵錯][8] MSDN 主題與 [Visual Studio 2010 的偵錯秘訣][17]。

如果您的網站使用 Azure Web API 或是行動服務後端，而您需要加以偵錯，請參閱[在 Visual Studio 中對 .NET 後端進行偵錯][18]。

### 在 ASP.NET 應用程式中追蹤

網際網路上找不到關於 ASP.NET 追蹤功能詳盡且具時效性的說明。您所能做的，就是從專為 Web Form 所撰寫的一些舊有簡介資料下手，因為
MVC 是最近才問世的技術，並以著重在特定議題的較新的部落格文章來做為補充。以下資源是您開始了解這項技術的一些好去處：

* [監視與遙測 (運用 Azure 建構真實的雲端應用程式)][19] (英文)。  
  針對追蹤 Azure 雲端應用程式所建議的電子書章節。
* [ASP.NET 追蹤][20] (英文)   
   舊有但仍是該主題的基本簡介的良好資源。
* [追蹤接聽程式][21] (英文)   
  內含有關追蹤接聽程式的資訊，但是沒有提到
  [WebPageTraceListener][22]。
* [逐步介紹：整合 ASP.NET 追蹤功能與系統診斷追蹤功能][23] (英文)   
  同樣為舊有的資料，但是內含簡介文章沒有提到的一些額外資訊。
* [追蹤 ASP.NET MVC Razor 檢視][24] (英文)   
  除了追蹤 Razor 檢視之外，該文同時說明了如何建立錯誤篩選條件以便記錄 MVC
  應用程式所出現的所有未處理的例外。如需如何記錄 Web Form 應用程式中所有未處理的例外項目的詳細資訊，請參閱 MSDN
  上[完整的錯誤處理常式範例][25] (英文) 的 Global.asax 範例。無論是 MVC 還是 Web
  Form，如果您想要記錄特定例外，但是讓預設的架構處理功能生效，則您可以如以下範例所示捕捉並重新擲回這些例外：
...
* [從 Azure 命令列串流診斷追蹤記錄 (加上 Glimpse！)][26] (英文) 如何使用命令列來執行此教學課程所示範的 Visual Studio 步驟。[Glimpse][27] (英文) 工具可供您偵錯 ASP.NET 應用程式。
* [使用 Azure 網站記錄與診斷功能 - 與 David Ebbo 一起][28] (英文) 與[從 Azure 網站串流記錄 - 與 David Ebbo 一起][29](英文) 影片，由 Scott Hanselman 與 David Ebbo 共同錄製。

針對錯誤記錄，做為撰寫自己的追蹤程式碼的替代方法，便是使用開放原始碼的記錄架構，例如 [ELMAH][30]。如需詳細資訊，請參閱 [Scott Hanselman 關於 ELMAH 的部落格文章][31] (英文)。
...
* [使用 LogParser 疑難排解 IIS 效能問題或應用程式錯誤][33] (英文)   
   此篇介紹可以用來分析 Web 伺服器記錄的 Log Parser 工具。
* [Robert McMurray 關於使用 LogParser 的部落格文章][34]  
  
* [IIS 7.0、IIS 7.5 與 IIS 8.0 中的 HTTP 狀態碼][35]

### 分析失敗要求追蹤記錄

Microsoft TechNet 網站內的[使用失敗要求追蹤][36] (英文) 小節可能有助您了解如何使用這些記錄。不過，本文主要著重在 IIS 內設定失敗要求追蹤功能，這是您無法在 Azure 網站中執行的功能。

### 偵錯雲端服務

如果您想對 Azure 雲端服務 (而非網站) 進行偵錯，請參閱[偵錯雲端服務][37] (英文)。
...
[6]: http://msdn.microsoft.com/zh-tw/library/vstudio/y740d9d3.aspx#BKMK_Restrict_stepping_to_Just_My_Code
[7]: http://www.asp.net/aspnet/overview/developing-apps-with-windows-azure/getting-started-with-windows-azure-webjobs
[8]: http://msdn.microsoft.com/zh-tw/library/vstudio/sc65sadd.aspx
[9]: http://msdn.microsoft.com/zh-tw/library/system.diagnostics.trace.aspx
[10]: http://msdn.microsoft.com/zh-tw/library/vstudio/6915t83k(v=vs.100).aspx
...
[13]: http://msdn.microsoft.com/zh-tw/library/windowsazure/ff683677.aspx
...
[20]: http://msdn.microsoft.com/zh-tw/library/ms972204.aspx
[21]: http://msdn.microsoft.com/zh-tw/library/4y5y10s7.aspx
[22]: http://msdn.microsoft.com/zh-tw/library/system.web.webpagetracelistener.aspx
[23]: http://msdn.microsoft.com/zh-tw/library/b0ectfxd.aspx
