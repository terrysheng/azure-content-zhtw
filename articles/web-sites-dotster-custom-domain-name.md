<properties 
	pageTitle="設定 Azure 網站的 Dotster 網域名稱" 
	description="了解如何設定 Azure 網站使用向 Dotster 註冊的網域名稱" 
	services="web-sites" 
	documentationCenter="" 
	authors="blackmist" 
	manager="wpickett" 
...
5. 然後指定該記錄的 [主機]**** 和 [Points to]**** (指向) 欄位。完成時，請按一下 [新增]**** 按鈕。
    ...
    * 新增 CNAME 記錄時，您必須將 [主機]**** 欄位設定為您要使用的子網域。例如，**www**。您必須將 [Points to]**** (指向) 欄位設定為 Azure 網站的 **.azurewebsites.net** 網域名稱。例如，**contoso.azurwebsites.net**。

    * 新增 A 記錄時，您必須將 [主機]**** 欄位設定為 **@** (這代表根網域名稱，例如 **contoso.com**)，或您要使用的子網域 (例如 **www**)。您必須將 [Points to]**** (指向) 欄位設定為您 Azure 網站的 IP 位址。

		> [AZURE.NOTE] 如果您使用 A 記錄，則也必須利用以下其中一個組態來新增 CNAME 記錄：
		> 
		> * **www** 的 [主機]**** 值會 [Points to]**** (指向) **&lt;yourwebsitename&gt;.azurewebsites.net** 的值。
		> 
		> 或
		> 
		> * **awverify.www** 的 [主機]**** 值會 [Points to]**** (指向) **awverify.&lt;yourwebsitename&gt;.azurewebsites.net** 的值。
		> 
