...
> [AZURE.NOTE] 本文使用 [Azure 入口網站](http://go.microsoft.com/fwlink/?LinkId=529715)、Azure PowerShell 及 Azure 跨平台命令列介面來處理診斷記錄。如需使用 Visual Studio 來處理診斷記錄的相關資訊，請參閱[在 Visual Studio 中疑難排解 Azure](troubleshoot-web-sites-in-visual-studio.md)
...
> [AZURE.NOTE] 儲存在 [資料表儲存體] 或 [Blob 儲存體] 內的資訊只能透過儲存用戶端，或是能夠直接使用這些儲存系統的應用程式來存取。例如，Visual Studio 2013 內含的 [儲存體總管] 可用來探索資料表或 Blob 儲存體，而 HDInsight 則可存取儲存在 Blob 儲存體內的資料。您也可以使用任一個 [Azure SDK](/downloads/#)，來撰寫可存取 Azure 儲存體的應用程式。
...
* **檔案系統** - 將應用程式診斷資訊儲存至 Web 應用程式檔案系統。這些檔案可透過 FTP 存取，或是使用 Azure PowerShell 或 Azure 命令列工具下載為 Zip 封存。
...
> [AZURE.NOTE] 您也可以在 Azure PowerShell 中使用 **Set-AzureWebsite** Cmdlet 來啟用診斷功能。如果您尚未安裝 Azure PowerShell，或者尚未將其設定為使用您的 Azure 訂用帳戶，請參閱[如何使用 Azure PowerShell](/develop/nodejs/how-to-guides/powershell-cmdlets/)。

##<a name="download"></a>作法：下載記錄

儲存在 Web 應用程式檔案系統中的診斷資訊，可透過 FTP 直接存取，或是使用 Azure PowerShell 或 Azure 命令列工具下載為 Zip 封存。

儲存這些記錄的目錄結構如下所示：

* **應用程式記錄** - /LogFiles/Application/。此資料夾內含有一或多個文字檔案，這些檔案涵蓋應用程式記錄所產生的資訊。

* **失敗的要求追蹤** - /LogFiles/W3SVC#########/。此資料夾內含有一個 XSL 檔案和一或多個 XML 檔案。請確保會將 XSL 檔案下載至 XML 檔案所在的相同目錄，因為 XSL 檔案可提供功能，讓您在 Internet Explorer 中檢視時能夠格式化和篩選 XML 檔案內容。
...
> [AZURE.NOTE] 如果您尚未安裝 Azure PowerShell，或者尚未將它設定為使用 Azure 訂用帳戶，請參閱[如何使用 Azure PowerShell](/develop/nodejs/how-to-guides/powershell-cmdlets/)。
...
> [AZURE.NOTE] 如果您尚未安裝 Azure 命令列工具，或是尚未將其設定為使用您的 Azure 訂閱，請參閱[如何使用 Azure 命令列工具](xplat-cli.md)。
...
失敗的要求追蹤會儲存在名為 __fr######.xml__ 的 XML 檔案中。為了讓您更輕鬆地檢視記錄資訊，系統會在 XML 檔案所屬的相同目錄中，提供名為 __freb.xsl__ 的 XSL 樣式表。在 Internet Explorer 中開啟其中一個 XML 檔案會使用 XSL 樣式表，提供格式化的追蹤資訊顯示。此資訊類似以下內容：
...
- [在 Visual Studio 中疑難排解 Azure Web 應用程式](/zh-tw/develop/net/tutorials/troubleshoot-web-sites-in-visual-studio/)
