...
如需建立新網站的詳細步驟說明，請參閱[如何建立及部署網站](/en-us/manage/services/web-sites/how-to-create-websites/)。
...
-   從原始檔控制部署。
-   從 Visual Studio 發行。
-   從 [WebMatrix](http://www.microsoft.com/web/webmatrix/) 發行。
...
在此許多部署工作都必須使用到 Azure 管理入口網站內的資訊。前往您的網站並選取 **[儀表板]** 索引標籤，然後尋找 **[Quick Glance]** 區段。以下螢幕擷取畫面顯示數個選項。
...
您也可以選擇透過原始檔控制管理服務 (如 GitHub 或 TFS Online) 來進行部署。按一下 **[設定從原始檔控制進行部署]** 選項。接著遵循您所選的原始檔控制系統或服務指示進行。如需從本機 Git 儲存機制發行的逐步說明，請參閱[從原始檔控制發行至 Azure 網站](/en-us/develop/net/common-tasks/publishing-with-git/)。

如果您打算使用 Visual Studio 來建立與管理網站，可以選擇直接從 Visual Studio 發行。方法之一就是按一下 **[Download the publish profile]** 選項。此舉可讓您儲存 publishsettings 檔案，以便匯入 Visual Studio 供 Web 發行之用。

**注意**

請務必確保 *publishsettings* 檔案安全且不受到原始檔控制影響，因為此檔案包含了部署與任何連結之資料庫連接字串所需的使用者名稱與密碼。

您也可以直接將訂閱資訊匯入 Visual Studio。舉例來說，假設 Visual Studio 裡有一個本機 ASP.NET 專案。以滑鼠右鍵按一下該 Web 專案，然後選取 **[發行]**。**[Publish Web]** 對話方塊裡的 **[匯入]** 按鈕 可讓您匯入內含 Azure 訂閱設定的檔案，或是您從網站儀表板中下載的 publishsettings 檔案。以下螢幕擷取畫面顯示這些選項。
...
如需此選項的詳細資訊，請參閱[使用 Microsoft WebMatrix 開發與部署網站](/en-us/develop/net/tutorials/get-started/)。

儘管這些步驟提供您部署 .COM 網站所需，您還是應該建立管理持續進行的內容發行週期的計畫。這些選項包括導入自訂解決方案、定期重新部署偶爾改變的網站，乃至於功能完整的內容管理系統 (CMS)。如果您是建立新的網站，請注意組件庫裡有些選項可供您使用現有的 CMS 架構，例如 [Drupal](https://drupal.org/) 或 [Umbraco](http://umbraco.com/)。
...
在 Azure 管理入口網站中，確認您在 **[共用]** 或是 **[標準]** 模式下 **[擴充]** 索引標籤上執行 (**免費**網站不支援自訂網域)。接著前往 **[設定]** 索引標籤，並按一下 **[管理網域]** 按鈕。此舉可讓您將網站與自訂網域名稱產生關聯。

![GlobalWebWebMatrix](./media/web-sites-global-web-presence-solution-overview/GlobalWeb_WebMatrix.png)

在您將自訂網域放入清單之前，首先必須前往 DNS 提供者處並為您的自訂網域 (www.contoso.com) 建立 CNAME 記錄以便指向您的 Azure 網站 URL (contoso.azurewebsites.net)。等到此記錄散佈之後，您就可在上一個螢幕擷取畫面所示的對話方塊中輸入自訂網域。當 www.contoso.com 存在指向此網站之 CNAME 記錄時，可確保您具有使用此網站之網域名稱的權限。此時，您可以在對話方塊下方建立內含 IP 位址的 A 記錄。
...
如需詳細資訊，請參閱[設定 Azure 網站的自訂網域名稱](/en-us/develop/net/common-tasks/custom-dns-web-site/)。
...
不過，如果您是使用自訂網域名稱，則您必須採取步驟上傳憑證，並透過 Azure 管理入口網站，為您的網站啟用 SSL。以下步驟提供此程序摘要，不過您可以透過以下主題找到更詳細的說明：[設定 Azure 網站的 SSL 憑證](/en-us/develop/net/common-tasks/enable-ssl-web-site/)。
...
從憑證授權單位取得憑證後，您不能原封不動地直接上傳到 Azure。您必須使用 oenssl 命令，產生 .pfx 檔案。openssl 命令是 OpenSSL 專案的一部分。原始檔分佈在 [OpenSSL 網站](http://www.openssl.org/)上，但一般而言，您也可以在網際網路找到此工具預先編譯的版本。在以下範例中，我們使用憑證 myserver.crt 與私有金鑰檔 myserver.key 來建立 .pfx 檔案。

    openssl pkcs12 -export -out myserver.pfx -inkey myserver.key -in myserver.crt

若要將憑證上傳至 Azure，請先移至 **[擴充]** 索引標籤，然後確認您正在 **[標準]** 模式下執行。**[免費]** 或 **[共用]** 模式皆不支援在自訂網域上使用 SSL。在 **[設定]** 索引標籤上，按一下 **[上傳憑證]** 按鈕。
...
**[IP SSL]** 選項是將公用的專屬 IP 位址對應至網域名稱的傳統方式。所有瀏覽器皆適用此方式。**[SNI SSL]** 選項可讓多個網域共用相同的 IP 位址，而且針對每個網域具有不同的相關聯 SSL 憑證。SNI SSL 無法在某些舊版的瀏覽器上運作 (如需相容性詳細資訊，請參閱 [Wikipedia 上的 SNI SSL 項目](http://en.wikipedia.org/wiki/Server_Name_Indication))。每個 SSL 憑證都必須按月繳交使用費用 (按小時比例分配)，而定價則是依據您選擇的是 IP SSL 或是 SNI SSL 而定。如需定價資訊，請參閱[網站定價詳細資料](/en-us/pricing/details/web-sites/#service-ssl)。如需此程序詳細資訊，請參閱[設定 Azure 網站的 SSL 憑證](/en-us/develop/net/common-tasks/enable-ssl-web-site/)。
...
如果您發現網站需要擴充，可以在 [擴充] 索引標籤中手動完成，或是透過 [自動擴充] 預覽方式完成****。[擴充] 索引標籤提供您向上擴充 (容量更大的專屬機器) 或是向外擴充 (增加額外的共用執行個體或是相同大小的專屬執行個體)。不過，[自動擴充] 預覽功能僅支援向外擴充。如需此選項的詳細資訊，請參閱「如需網站監視的詳細資訊，請參閱[數位行銷活動](/en-us/manage/services/web-sites/digital-marketing-campaign-solution-overview)案例中的＜隨使用者需求擴充＞區段。另請參閱[如何監視網站](/en-us/manage/services/web-sites/how-to-monitor-websites/)。
