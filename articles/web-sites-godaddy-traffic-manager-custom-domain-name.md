<properties 
	pageTitle="使用流量管理員設定 Azure 網站的 GoDaddy 網域名稱" 
...
4. [Zone File Editor]**** 會依每個記錄類型被分為不同區段，從作為第一個區段的 A 記錄開始 (列為 [A (主機)]****)，接著是 CNAME 記錄 (列為 [CNAME (別名)]****)。若要新增項目，請使用對應區段下方的 [快速新增]**** 按鈕。若要編輯現有項目，請選取該項目並修改現有資訊。

	![zone file editor](./media/web-sites-custom-domain-name/godaddy-quickaddcname.png)

	> [AZURE.NOTE] 在將項目新增至區域檔案之前，請注意，GoDaddy 已為熱門子網域 (在編輯器中稱為 [主機]****) 建立 DNS 記錄，例如 [電子郵件]****、[檔案]****、郵件****及其他。如果您要使用的名稱已存在，請修改現有記錄，而非建立新記錄。
...
5. 當您完成新增或修改記錄時，請按一下 [儲存區域檔案]**** 以儲存變更。
