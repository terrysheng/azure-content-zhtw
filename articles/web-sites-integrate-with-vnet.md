...
如果您的 Web 應用程式不在標準的 App Service 方案中，UI 會讓您知道應該要升級，並授與您存取定價層的權限。
...
## 連線到既存的網路 ##
若要將 Web 應用程式連接到虛擬網路，請前往 Web 應用程式的刀鋒視窗、在 [網路] 區段中按一下虛擬網路磚，然後選取任一個既存的網路。
...
##管理虛擬網路連線##
您可以造訪 App Service 方案的刀鋒視窗，查看目前與 App Service 方案內之 Web 應用程式相關聯的所有虛擬網路清單。您最多可以擁有 5 個與 'Standard' App Service 方案相關聯的網路。

如果您將 App Service 方案調整為「免費」、「共用」或「基本」等較低階的方案，該方案中 Web 應用程式所用的虛擬網路連線將遭到停用。如果您將方案調整回標準方案，系統將會重新建立先前的網路連線。
