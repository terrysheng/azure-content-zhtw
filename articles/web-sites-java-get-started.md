...
本教學課程示範如何在使用 Java 的 Microsoft Azure 上，使用 Azure Marketplace 或 [App Service Web Apps](http://go.microsoft.com/fwlink/?LinkId=529714) 設定 UI，來建立 Web 應用程式。 

如果您不打算使用這些技術 (例如，如果您要自訂應用程式容器)，請參閱[將自訂的 Java Web 應用程式上傳至 Azure](web-sites-java-custom-upload.md)。
...
>如果您想要在註冊 Azure 帳戶之前開始使用 Azure App Service，請移至[試用 App Service](http://go.microsoft.com/fwlink/?LinkId=523751)，讓您能夠在 App Service 中立即建立短期的入門 Web 應用程式。不需要信用卡，無需承諾。
...
2. 按一下頁面左下方的 [新增]。
3. 按一下 [Web + 行動] 刀鋒視窗。
4. 按一下 [Web + 行動] 刀鋒視窗底部的 [Azure Marketplace]。
5. 按一下 [Web]。
...
幾分鐘內即可建立您的 Web 應用程式。若要檢視 Web 應用程式，可在 Azure 管理入口網站內的 [Web 應用程式] 刀鋒視窗中，按一下您最新建立的 Web 應用程式，然後按一下該 Web 應用程式的 URL。

現在，您已建立含有應用程式容器的 Web 應用程式，如需將應用程式上傳至該 Web 應用程式的相關資訊，請參閱＜後續步驟＞一節。

# 使用 Azure 設定 UI 建立 Java Web 應用程式

本資訊說明如何使用 Azure 設定 UI，為您的 Web 應用程式選取 Java 應用程式容器 (Apache Tomcat 或 Jetty)。
