...
您可以使用這兩組認證之一。FTP 和 FTPS 主機名稱會列於 [快速概覽] 下方的儀表板中。
...
部署認證與 Microsoft 帳戶相關聯。如果您變更使用者名稱或密碼，這項變更會套用至與該帳戶相關聯的所有網站中。如果 Azure 訂閱擁有多個管理員，則每個管理員都會有自己的認證。
...
查詢含有 `publishMethod="FTP"` 的設定檔。使用者名稱列於 `userName` 下方，而密碼則列於 `userPWD` 下方。
