...
在 [Azure 入口網站](https://portal.azure.com)中，您可以透過一系列的 *blades* (其中包含適用於不同元件的容器) 來管理 Web 應用程式(如需 Azure 入口網站的概觀，請參閱[入口網站](http://go.microsoft.com/fwlink/?LinkId=529715)變更指南)。 

若要檢視 Web 應用程式的刀鋒視窗，依序按一下 [首頁] 和應用程式名稱，或者，若要查看您的所有 Azure 資源，可按一下 [瀏覽]。

![](./media/web-sites-manage/manage01.png)

刀鋒視窗的最上層組件具有適用於一些常見動作的控制項：
...
[基本功能] 會列出有關應用程式的基本資訊，包括 URL、位置、[App Service](http://go.microsoft.com/fwlink/?LinkId=529714) 方案，以及目前狀態 (執行中、已停止等)。

位於 [基本功能] 下方的是可用來監視、管理和部署應用程式的各個組件。若要自訂刀鋒視窗的版面配置，可使用滑鼠右鍵按一下並選取 [自訂] 或 [新增組件]。本主題的其餘部分將說明預設版面配置中的組件。
...
- **串流記錄** (A)。按一下以檢視應用程式記錄。若要啟用記錄，請移至 [設定] 並開啟 [診斷記錄] 刀鋒視窗。 
