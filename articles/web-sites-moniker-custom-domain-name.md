<properties 
	pageTitle="設定 Azure 網站的 Moniker 網域名稱" 
	description="了解如何設定 Azure 網站使用向 Moniker 註冊的網域名稱" 
	services="web-sites" 
	documentationCenter="" 
	authors="blackmist" 
	manager="wpickett" 
...
此文章提供有關使用從 [Moniker.com](https://moniker.com) 購買的自訂網域名稱搭配 Azure 網站的指示。
...
5. 接著，建立 DNS 記錄，方法是先選取 [Record Type]**** (記錄類型)。然後填入 [Hostname]**** (主機名稱) 和 [Address]**** (位址)。 
    ...
    * 新增 CNAME 記錄時，您必須將 [Hostname]**** (主機名稱) 欄位設定為您要使用的子網域。例如 **www**。您必須將 [Address]**** (位址) 欄位設定為您 Azure 網站的 **.azurewebsites.net** 網域名稱。例如，**contoso.azurwebsites.net**。

    * 新增 A 記錄時，您必須將 [Hostname]**** (主機名稱) 欄位設定為 **@** (這代表根網域名稱，例如 **contoso.com**) ，或您要使用的子網域 (例如 **www**)。您必須將 [Address]**** (位址) 欄位設定為您 Azure 網站的 IP 位址。

		> [AZURE.NOTE] 如果您將使用 A 記錄，則也必須利用以下其中一個組態來新增 CNAME 記錄：
		> 
		> * [Hostname]**** (主機名稱) 值為 **www** 且 [Address]**** (位址) 的值為 **&lt;您的網站名稱&gt;.azurewebsites.net**。
		> 
		> 或
		> 
		> * [Hostname]**** (主機名稱) 值為 **awverify.www** 且 [Address]**** (位址) 的值為 **awverify.&lt;您的網站名稱&gt;.azurewebsites.net**。
		> 
