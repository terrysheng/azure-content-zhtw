...
1.  開啟網站的 [儀表板] 管理頁面。
2.  在 [usage overview] 區段下方，[Data Out]、[CPU 時間] 及 [File System Storage] 的使用量配額均顯示於此。每個資源顯示的綠色長條代表網站目前耗用訂用帳戶之資源使用量配額的程度；每項資源顯示的灰色長條代表與您網站訂用帳戶相關之其他所有共用模式網站目前耗用訂用帳戶之資源使用量配額的程度。

資源使用量配額能預防超用以下資源：

-   **資料輸出** - 在目前的配額間隔 (24 小時) 內，以**共用**模式運作之網站傳送到用戶端的資料量量值。
-   **CPU 時間** - 在目前的配額間隔內，以**共用**模式運作之網站使用的 CPU 時間量。
-   **檔案系統儲存體** - 以**共用**模式運作之網站使用的檔案系統儲存體數量。

當超過訂閱的使用量配額時，Azure 會採取動作來停止資源超用。此舉是為了避免訂閱者耗盡資源，損害其他訂閱者的權益。

## <a name="resourceusage"></a>作法：減少資源使用量

由於 Azure 會在 24 小時的配額間隔內量測訂用帳戶之共用模式網站所用的資源，藉此計算資源使用量配額，因此請將以下事項納入考量：

-   當設定為以共用模式執行的網站數目增加時，很有可能會超過共用模式的資源使用量配額。
    若超過資源使用量配額，請考慮減少設定為以共用模式執行的網站數目。
-   同樣地，以共用模式執行的網站執行個體數增加時，也很有可能會超過共用模式的資源使用量配額。
    若超過資源使用量配額，請考慮縮減共用模式網站的額外執行個體。

## <a name="exceeded"></a>超過資源使用量配額時的後續動作

在配額間隔 (24 小時) 內超出訂閱的資源使用量配額時，Azure 會採取以下措施：

-   **資料輸出** - 在超出此項配額時，Azure 會在目前配額間隔的剩餘時間內停止訂用帳戶之所有已設定為以**共用**模式執行的網站。Azure 會在下一個配額間隔開始時啟動網站。

-   **CPU 時間** - 在超出此項配額時，Azure 會在目前配額間隔的剩餘時間內停止訂用帳戶之所有已設定為以**共用**模式執行的網站。Azure 會在下一個配額間隔開始時啟動網站。

-   **檔案系統儲存體** - 當部署會導致檔案系統儲存體使用量配額超量時，Azure 會禁止部署訂用帳戶之任何已設定為以共用模式執行的網站。當檔案系統儲存體資源達到配額允許的上限時，讀取操作仍可存取檔案系統儲存體，不過所有寫入操作 (包括一般網站活動所需的寫入操作) 都將遭到封鎖。當這種情況發生時，您可以將一或多個以共用網站模式執行的網站設定為以標準網站模式執行，抑或是減少檔案系統儲存體的使用量，使其低於檔案系統儲存體的使用量配額。
...
-   **Application Logging (File System)** - 開啟應用程式產生之資訊的記錄功能。[Logging Level] 欄位決定要記錄 [錯誤]、[警告] 還是 [資訊] 層級的資訊。您也可以選取 [詳細資訊] 來記錄應用程式產生的所有資訊。
    ...
    此項設定所產生的記錄會儲存於網站的檔案系統中，因此您可以使用下文＜下載網站的記錄檔＞一節中所述的步驟予以下載。
...
<div class="dev-callout"> 
<b>注意</b> 
<p>您可以同時啟用<b>應用程式診斷 (檔案系統)</b>、<b>應用程式診斷 (資料表儲存體)</b> 及<b>應用程式診斷 (Blob 儲存體)</b>，且它們可以擁有各自的記錄層級組態。例如，您也許會想要將錯誤和警告記錄在儲存體並做為長期的記錄解決方案，同時在檢測應用程式程式碼後啟用檔案系統記錄並搭配詳細資訊層級，以供疑難排解問題之用。</p> </div>
...
-   **詳細錯誤資訊：**包括錯誤的相關資訊，如 [模組]、[處理常式]、[錯誤碼] 及 [要求的 URL]。
...
運作時間是透過 HTTP 回應碼來加以監視，而回應時間的測量單位是毫秒。當回應時間少於 30 秒，且 HTTP 狀態碼小於 400 時，運作時間可視為 100%。當回應時間超過 30 秒，或 HTTP 狀態碼大於 400 時，運作時間為 0%。
...
3.  移至 [監視] 區段以輸入端點設定。
4.  輸入端點的名稱。
5.  輸入要監視之服務的 URL。例如 [][]<http://contoso.cloudapp.net></a>。
6.  在清單中選取一或多個地理位置。
...
-   [Keeping Azure Web Sites up plus Endpoint Monitoring - with Stefan Schackow][]
  ...
  [Keeping Azure Web Sites up plus Endpoint Monitoring - with Stefan Schackow]: http://www.windowsazure.com/zh-tw/documentation/videos/azure-web-sites-endpoint-monitoring-and-staying-up/
