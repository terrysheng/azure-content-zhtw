<properties 
	pageTitle="如何監視網站 - Azure 服務管理" 
	description="了解如何使用「管理入口網站」中的 [監視] 頁面監視 Azure 網站。" 
	services="web-sites" 
	documentationCenter="" 
	authors="cephalin" 
	manager="wpickett" 
...
1. 在 [Azure 管理入口網站中](http://manage.windowsazure.com/)，從網站的 [管理] 頁面按一下 [監視]**** 索引標籤以顯示 [監視]**** 管理頁面。根據預設，[監視]**** 頁面上圖表顯示的度量與 [儀表板]**** 頁面的圖表相同。 
...
在「標準」****網站模式下，您可以接收以網站監視度量為基礎的警示。您必須先設定監視所用的 Web 端點才能使用警示功能，您可以在 [設定]**** 頁面的 [監視]**** 區段中加以設定。在 Azure 管理入口網站的 [設定]**** 頁面中，您可以接著建立規則以在選擇的度量達到指定值時觸發警示。您也可以選擇在觸發警示時傳送電子郵件。如需詳細資訊，請參閱[作法：在 Azure 中接收警示通知及管理警示規則](http://go.microsoft.com/fwlink/?LinkId=309356)。  

##<a name="howtoviewusage"></a>作法：檢視網站的使用量配額

在網站的 [調整規模]**** 管理頁面中，您可以將網站設定為以「共用」****或「標準」****網站模式運作。在「共用」****網站模式下，每個 Azure 訂用帳戶都能存取針對執行每個區域最多 100 個網站而提供的資源集區。基於此目的而供應給每個網站訂用帳戶的資源集區，將由相同地理區域中設定為以「共用」****模式執行的其他網站共用。由於這些資源需與其他網站共用，因此所有訂用帳戶在使用這些資源時均會受到限制。套用在訂用帳戶上的資源使用限制，會以列於各網站 [儀表板]**** 管理頁面之 [使用量概觀] 區段下的使用量配額來表示。

>[AZURE.NOTE] 當網站設定為以「標準」****模式執行時，系統會配置等同於 [Azure 的虛擬機器和雲端服務大小][vmsizes]文章中表格所示的「小型」****(預設)、「中型」****或「大型」****專屬資源。對於以「標準」****模式運作的網站，訂用帳戶可使用的資源並沒有限制。不過，每個區域容許建立的「標準」****模式網站數目只有 500 個。
 
### 作法：檢視設定為共用網站模式之網站的使用量配額 ###
若要判斷網站影響資源使用量配額的程度，請遵循以下步驟：

1. 開啟網站的 [儀表板]**** 管理頁面。
2. 個別虛擬主機方案的使用量配額會顯示在 [使用量概觀]**** 區段之下，它是下列項目的子集：
...
配額與效能或成本無關，但這是 Azure 管理多租用戶環境中資源使用量的方法，以免租用戶過度使用共用資源。因為超過配額即表示您的網站停機或功能降低，如果您希望在即將達到配額時讓網站持續運作，請考慮下列各項：

- 將網站移到較高層級的虛擬主機方案，以便利用較大的配額。例如，「基本」****和「標準」****方案的唯一配額為檔案系統儲存體。 
...
[設定]**** 管理頁面的 [應用程式診斷]**** 區段能控制應用程式產生之資訊的記錄，在記錄發生於應用程式內部的事件時，此項功能便能派上用場。例如，當應用程式內部發生錯誤時，您也許會希望能向使用者呈現易於理解的錯誤，以及在記錄中寫入更詳細的錯誤資訊，以供日後分析之用。

您可以啟用或停用以下應用程式診斷：

- [應用程式記錄 (檔案系統)]**** - 開啟應用程式產生之資訊的記錄功能。[記錄層次]**** 欄位決定要記錄「錯誤」、「警告」或「資訊」層級的資訊。您也可以選取 [詳細資訊] 來記錄應用程式產生的所有資訊。

	此設定所產生的記錄檔會儲存在網站的檔案系統上，因此您可以使用下文＜下載網站的記錄檔＞****一節中所述的步驟下載。
...
> 
> - **.NET** - [啟用 Azure 網站的診斷記錄功能](/zh-tw/develop/net/common-tasks/diagnostics-logging-and-instrumentation/)
> - **Node.js** - [如何針對 Azure 網站中的 Node.js 應用程式進行偵錯](/zh-tw/develop/nodejs/how-to-guides/Debug-Website/)
> 
...
- **Web 伺服器記錄** - 開啟 Web 伺服器記錄功能以使用 W3C 擴充記錄檔案格式儲存網站記錄。Web 伺服器記錄功能會產生涵蓋所有傳入網站之要求的記錄，其中包括用戶端 IP 位址、要求的 URI、回應的 HTTP 狀態碼及用戶端的使用者代理程式字串等資訊。您可以將記錄儲存在 Azure 儲存體帳戶或檔案系統中。

 若要將 Web 伺服器記錄儲存在 Azure 儲存體帳戶，請選擇 [儲存體]****，然後選擇 [管理儲存體]**** 以指定儲存體帳戶和用來保存記錄的 Azure Blob 容器。如需 Azure 儲存體帳戶的詳細資訊，請參閱[如何管理儲存體帳戶](https://www.windowsazure.com/zh-tw/manage/services/storage/how-to-manage-a-storage-account/)。

   若要將 Web 伺服器記錄儲存在檔案系統中，請選擇 [檔案系統]****。如此能啟用 [配額]**** 方塊以供您設定容納記錄檔的磁碟空間上限。大小的下限為 25MB，上限為 100MB。預設大小為 35MB。

 依預設，系統永遠不會刪除 Web 伺服器記錄。若要指定自動刪除記錄的時間，請選取 [設定保留]**** 並在 [保留週期]**** 方塊內輸入記錄的保留天數。此設定適用於 Azure 儲存體和檔案系統選項。

- [詳細錯誤訊息]**** - 開啟詳細錯誤記錄功能以記錄有關 HTTP 錯誤 (大於 400 的狀態碼) 的額外資訊。

- **失敗要求的追蹤** - 開啟失敗要求追蹤功能可擷取失敗的用戶端要求資訊，如 400 系列的 HTTP 狀態碼。「失敗要求的追蹤」功能會產生 XML 文件，其中包括要求在 IIS 中經過哪些模組的追蹤、模組傳回的詳細資料及叫用模組的時間。這些資訊可用來隔離出發生失敗的元件。


啟用網站診斷後，按一下 [設定]**** 管理頁面底部的 [儲存]**** 圖示可套用您設定的選項。

> [AZURE.IMPORTANT] 記錄和追蹤等功能會使網站的資源需求大增。建議您在重新產生要疑難排解的問題後關閉記錄和追蹤功能。
...
1. 開啟網站的 [儀表板]**** 管理頁面，接著記下 [診斷記錄]**** 下列示的 FTP 站台和 [部署使用者]**** 下列示的帳戶。FTP 站台是記錄檔的所在位置，而列示於 [部署使用者] 下的帳戶可用來通過 FTP 站台的驗證。
...
2. 在 Azure PowerShell 提示字元中使用下列命令來下載記錄檔：
	...
	如此可下載 **websitename** 所指定之網站的記錄檔，並將它們儲存到目前目錄中的 **log.zip** 檔案。
	
	您也可以使用下列命令來檢視記錄事件的即時資料流：
...
開啟新的命令提示字元、PowerShell、Bash 或終端機工作階段，然後使用下列命令來下載記錄檔：
...
- 位置 /LogFiles/Application/。此資料夾內含有一或多個文字檔案，這些檔案涵蓋應用程式記錄所產生的資訊。所記錄的資訊包括日期和時間、應用程式的處理程序識別碼 (PID)，以及應用程式檢測所產生的值。

- 檔案讀取方法：了解應用程式產生之值的文字編輯器或剖析器。

**記錄檔類型：失敗要求的追蹤**

- 位置：/LogFiles/W3SVC#########/. 此資料夾內含有一個 XSL 檔案和一或多個 XML 檔案。請務必將 XSL 檔案下載至 XML 檔案所在的相同目錄，因為 XSL 檔案可提供格式設定功能，讓您在 Internet Explorer 中檢視時能夠篩選 XML 檔案內容。 

- 檔案讀取方法：Internet Explorer

**記錄檔類型：詳細的錯誤記錄**

- 位置：/LogFiles/DetailedErrors/. /LogFiles/DetailedErrors/ 資料夾包含一或多個 .htm 檔案，內含已經發生之任何 HTTP 錯誤的詳細資訊。 
...
- 位置：/LogFiles/http/RawLogs. 儲存在檔案中的資訊是使用 [W3C 擴充記錄檔格式](http://go.microsoft.com/fwlink/?LinkID=90561)加以格式化。Azure 網站未使用 s-computername、s-ip 及 cs-version 等欄位。
...
9.	在左邊的服務列中，按一下 [管理服務]****。
10.	按一下底部的 [新增規則]****。
11.	在 [服務類型]**** 中，選取 [網站]****，然後選取您先前設定端點監視的網站。按一下 [下一步]****。
