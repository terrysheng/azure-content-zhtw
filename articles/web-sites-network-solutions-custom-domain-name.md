...
<div class="dev-center-tutorial-subselector"><a href="/zh-tw/documentation/articles/web-sites-network-solutions-custom-domain-name/" title="Websites" class="current">網站</a> | <a href="/zh-tw/documentation/articles/web-sites-network-solutions-traffic-manager-custom-domain-name/" title="Website using Traffic Manager">使用流量管理員的網站</a></div>
...
若要將您的自訂網域與 Azure 網站相關聯，您必須使用由 Network Solutions 所提供的工具，在 DNS 表格中為您的自訂網域新增項目。使用下列步驟來尋找並使用 networksolutions.com 的 DNS 工具
...
4. [Update Advanced DNS]**** (更新進階 DNS) 頁面包含每個記錄類型的區段，每個區段底下都有 [Edit]**** (編輯) 按鈕。
	
	* 對於 A 記錄，請使用 [IP Address (A Records)]**** (IP 位址 (A 記錄)) 區段。
	* 對於 CNAME 記錄，請使用 [Host Alias (CNAME Records)]**** (主機別名 (CNAME 記錄)) 區段。
...
5. 按一下 [Edit]**** (編輯) 按鈕時會出現表單，可讓您用來修改現有的記錄或新增記錄。 
	...
	* 新增 CNAME 記錄時，您必須將 [Alias]**** (別名) 欄位設定為您要使用的子網域。例如 **www**。您必須選取 [Other host]**** (其他主機) 欄位旁的圓形欄位，並將 [Other host]**** (其他主機) 設定為 Azure 網站的 **.azurewebsites.net** 網域名稱。例如，**contoso.azurwebsites.net**。將 [Refers to Host Name]**** (參照主機名稱) 保留為 [Select]**** (選取)，因為在建立與 Azure 網站搭配使用的 CNAME 記錄時不需要此欄位。
	
		![cname form](./media/web-sites-custom-domain-name/ns-cname.png)

		> [AZURE.NOTE] 如果您將使用 A 記錄，則也必須利用以下其中一個組態來新增 CNAME 記錄：
		> 
		> * [Alias]**** (別名) 的值為 **www** ，且 [Other host]**** (其他主機) 的值為 **&lt;您的網站名稱&gt;.azurewebsites.net**。
		> 
		> 或
		> 
		> * [Alias]**** (別名) 的值為 **awverify.www** ，且 [Other host]**** (其他主機) 的值為 **awverify.&lt;您的網站名稱&gt;.azurewebsites.net**。
		> 
		> Azure 會使用 CNAME 記錄來驗證您擁有 A 記錄所述的網域。

	* 新增 A 記錄時，您必須將 [Host]**** (主機) 欄位設定為 **@** (這代表根網域名稱，例如 **contoso.com**)、* (符合多個子網域的萬用字元) ，或您要使用的子網域 (例如 **www**)。您必須將 [Numeric IP]**** (數值 IP) 欄位設定為 Azure 網站的 IP 位址。
