...
Socket.IO 使用 WebSocket，提供 node.js 伺服器與用戶端之間的即時通訊。此外，它還能支援遞補其他適用於舊版瀏覽器的傳輸 (例如長輪詢)。本教學課程將逐步引導您裝載 Socket.IO 型聊天應用程式以做為 Azure Web 應用程式，並示範如何為應用程式[調整規模](#scale-out) (使用 [Azure Redis 快取](http://azure.microsoft.com/documentation/services/cache))。如需 Socket.IO 的詳細資訊，請參閱 [http://socket.io/][socketio]。
...
1.  下載 Socket.IO 專案的 [ZIP 或 GZ 壓縮版本][release] (此文件使用 1.3.5 版)。
...
1.  在文字編輯器中開啟 **app.js** 檔案。變更包含 `var io = require('../..')(server);` 的程式碼行，如下所示：
...
1. 安裝 Azure 跨平台命令列介面 (xplat-cli)，並連接到您的 Azure 訂用帳戶。請參閱[安裝與設定 Azure 跨平台命令列介面](xplat-cli)。

2. 如果這是您第一次在 Azure 中設定儲存機制，就需要建立登入認證。從 xplat-cli 中，輸入下列命令：
...
3. 切換至 **\\node\chat** 目錄，然後使用下列命令來建立新的 Azure Web 應用程式和本機 Git 儲存機制。這個命令也會建立名為 'azure' 的 Git 遠端。
	...
	您必須使用 Web 應用程式的唯一名稱來取代 'mysitename'。
...
4. Socket.IO 使用 WebSockets (在 Azure 上依預設不會啟用)。若要啟用 Web Sockets，請使用下列命令：
	...
	>[AZURE.NOTE]
	> 'azure site set -w' 命令僅適用於 Azure 跨平台命令列介面 0.7.4 版或更新版本。您也可以使用 [Azure 入口網站](https://portal.azure.com)來啟用 WebSocket 支援。
	>
...
Socket.IO 應用程式可以使用__配接器__橫向擴充，以在多個應用程式執行個體之間散佈訊息和事件。由於市面上已有數種配接器，因此可以輕鬆使用 [socket.io-redis](https://github.com/automattic/socket.io-redis) 配接器來搭配 Azure Redis 快取功能。
...
2. 修改 __app.js__ 檔案，在 ' var io = require('socket.io')(server);' 之後立即加入下列程式碼行
	...
	使用 Redis 快取的主機名稱和金鑰來取代 __redishostname__ 和 __rediskey__。
...
* **嘗試限制傳輸僅使用 WebSockets**
	...
	為了讓 Socket.IO 使用 WebSockets 作為訊息傳輸，伺服器和用戶端都必須支援 WebSockets。如果其中一方不支援 WebSockets，則 Socket.IO 將會交涉其他傳輸，例如長輪詢。Socket.IO 所使用的傳輸預設清單為 ` websocket, htmlfile, xhr-polling, jsonp-polling`。您可以將下列程式碼加入至 **app.js** 檔案中包含 `, nicknames = {};` 的程式碼行後面，強制讓 Socket.IO 只能使用 WebSockets。
...
* **使用 SSL**
	...
	若要修改此範例以啟用此設定，您可以將下列程式碼加入至 **app.js** 檔案中包含 `, nicknames = {};` 的程式碼行後面。
...
* **確認 web.config 設定**
	...
	裝載 Node.js 應用程式的 Azure Web 應用程式會使用 **web.config** 檔案，將連入要求路由傳送至 Node.js 應用程式。若要讓 WebSockets 能搭配 Node.js 應用程式正確運作，**web.config** 必須包含下列項目。
	...
	> [AZURE.NOTE] 只有當您的應用程式未提供 **web.config** 檔案時，Azure 網站才會自動產生此檔案。如果您在應用程式專案的根目錄中提供 **web.config** 檔案，Azure Web Apps 將會使用此檔案。
	...
	如果沒有顯示此項目，或此項目設定為 `true` 的值，那麼您應該在 Node.js 應用程式的根目錄中建立一個 **web.config**，然後指定 `false` 的值。以下即為某個應用程式的預設 **web.config**，這種應用程式是使用 **app.js** 作為進入點，供您參考。
		...
		<configuration>
		  <system.webServer>
		    ...
		    <!--
		      您可以控制如何使用下列選項，將 Node 裝載於 IIS 上：
		        * watchedFiles：以分號分隔的檔案清單，系統將監看此清單的變更來重新啟動伺服器
		        * node_env：將傳播至節點以做為 NODE_ENV 環境變數
		        * debuggingEnabled - 控制是否要啟用內建的偵錯工具
		
		      如需完整的選項清單，請參閱 https://github.com/tjanczuk/iisnode/blob/master/src/samples/configuration/web.config
		    -->
