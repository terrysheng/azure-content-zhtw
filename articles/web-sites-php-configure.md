...
本指南將說明您如何在 Azure 網站中設定內建 PHP 執行階段、提供自訂 PHP 執行階段，並且在 Azure 網站中啟用擴充功能。若要使用 Azure 網站，請註冊[免費試用][免費試用]。若要充分利用本指南，您應該先在 Azure 網站中建立 PHP 網站 (請參閱 [PHP 開發人員中心教學課程][PHP 開發人員中心教學課程])。如需在 Azure 網站中設定網站的一般資訊，請參閱[如何設定網站][如何設定網站]。

## 目錄

-   [什麼是 Azure 網站？][什麼是 Azure 網站？]
-   [作法：變更預設 PHP 組態][作法：變更預設 PHP 組態]
-   [作法：在內建 PHP 執行階段中啟用擴充][作法：在內建 PHP 執行階段中啟用擴充]
-   [作法：使用自訂 PHP 執行階段][作法：使用自訂 PHP 執行階段]
-   [後續步驟][後續步驟]
...
您建立 Azure 網站時，預設會安裝 PHP 5.4，而且可立即供您使用。若要查看可用的修訂版、其預設組態及啟用的擴充，最好的方法是部署呼叫 [phpinfo()][phpinfo()] 函數的指令碼。

PHP 5.5 也可供使用，不過並未預設啟用。若要將它啟用，請依照下列步驟執行：

1.  在 Azure 入口網站中瀏覽至您網站的儀表板，並按一下 [設定]。

    ![網站儀表板上的設定索引標籤][網站儀表板上的設定索引標籤]
...
對於任一個內建 PHP 執行階段，您可以按照下列步驟，變更並非僅系統層級指示詞的任何組態選項。(如需僅系統層級指示詞的資訊，請參閱 [php.ini 指示詞的清單][php.ini 指示詞的清單] (英文)。)

1.  將 [.user.ini][.user.ini] 檔案新增至根目錄。
...
除了使用 `.user.ini` 檔案之外，您也可以在指令碼中使用 [ini\_set()][ini\_set()] 函數，設定非系統層級指示詞的組態選項。

## <a name="EnableExtDefaultPHP"></a>作法：在預設 PHP 執行階段中啟用擴充

如同上一個小節所述，若要查看預設 PHP 版本、其預設組態及啟用的擴充，最好的方法是部署呼叫 [phpinfo()][phpinfo()] 函數的指令碼。若要啟用擴充，請依照下列步驟執行。
...
2.  為您的執行階段修改 `php.ini` 檔案。請注意，Azure 網站將忽略僅系統層級指示詞的任何組態設定。(如需僅系統層級指示詞的資訊，請參閱 [php.ini 指示詞的清單][php.ini 指示詞的清單] (英文))。
...
-   [在 Azure 中設定、監視和調整網站][在 Azure 中設定、監視和調整網站]
