...
本指南將說明您如何在 Azure 網站中設定內建 PHP 執行階段、提供自訂 PHP 執行階段，並且在 Azure 網站中啟用擴充。若要使用 Azure 網站，請註冊[免費試用](https://www.windowsazure.com/en-us/pricing/free-trial/)。若要充分利用本指南，您應該先在 Azure 網站中建立 PHP 網站 (請參閱[PHP Developer Center 教學課程](https://www.windowsazure.com/en-us/develop/php/tutorials/) (英文))。如需在 Azure 網站中設定網站的一般資訊，請參閱[如何設定網站](https://www.windowsazure.com/en-us/manage/services/web-sites/how-to-configure-websites/)。
...
Azure 網站可以讓您在 Azure 上建立高擴充性網站。您很容易就能夠將網站快速部署至高擴充性雲端環境，讓您在一開始時先建立小規模的網站，往後隨著流量增加時擴充。Azure 網站使用您選擇的語言和開放原始碼，並支援使用 Git、FTP 和 TFS 進行部署。您很容易就能夠整合其他服務，例如 MySQL、SQL Database、快取、CDN 和儲存。

作法：變更內建 PHP 組態
-----------------------

您建立 Azure 網站時，預設會安裝 PHP 5.4，而且可立即供您使用。若要查看可用的修訂版、其預設組態及啟用的擴充，最好的方法是部署呼叫 [phpinfo()](http://php.net/manual/en/function.phpinfo.php) 函數的指令碼。
...
3.  按一下頁面底部的 **[儲存]**。
	...
	對於任一個內建 PHP 執行階段，您可以按照下列步驟，變更並非僅系統層級指示詞的任何組態選項。(如需僅系統層級指示詞的資訊，請參閱 [php.ini 指示詞的清單](http://www.php.net/manual/en/ini.list.php) (英文)。)

1.  將 [.user.ini](http://www.php.net/manual/en/configuration.file.per-user.php) 檔案新增至根目錄。
...
3.  部署應用程式。
4.  重新啟動網站。(必須重新啟動，因為 PHP 讀取 `.user.ini` 檔案的頻率是由 `user_ini.cache_ttl` 設定所控制，這是系統層級設定，預設為 300 秒 (5 分鐘)。重新啟動網站將使得 PHP 讀取 `.user.ini` 檔案中的新設定。)

除了使用 `.user.ini` 檔案之外，您也可以在指令碼中使用 [ini\_set()](http://www.php.net/manual/en/function.ini-set.php) 函數，設定非系統層級指示詞的組態選項。

作法：在預設 PHP 執行階段中啟用擴充
-----------------------------------

如同上一個小節所述，若要查看預設 PHP 版本、其預設組態及啟用的擴充，最好的方法是部署呼叫 [phpinfo()](http://php.net/manual/en/function.phpinfo.php) 函數的指令碼。若要啟用擴充，請依照下列步驟執行。
...
5.  在 **[應用程式設定]** 區段中，建立 **PHP\_EXTENSIONS** 索引鍵和 **bin\\your-ext-file** 值。若要啟用多個擴充，請加入逗號區隔的 `.dll` 檔案清單。
...
2.  為您的執行階段修改 `php.ini` 檔案。請注意，Azure 網站將忽略僅系統層級指示詞的任何組態設定。(如需僅系統層級指示詞的資訊，請參閱 [php.ini 指示詞的清單](http://www.php.net/manual/en/ini.list.php) (英文))。
