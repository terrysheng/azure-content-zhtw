...
*作者：[Ben Lobaugh](http://ben.lobaugh.net)，[Microsoft Open Technologies Inc.](http://msopentech.com)*

本教學課程中，您將了解如何將透過 Azure 的組件庫所建立的現有 WordPress 網站，轉換成 WordPress 多網站安裝。此外，您也將了解如何將自訂網域指派給安裝內的每一個子網站。

本文假設您目前已安裝 WordPress。如果沒有，請依照＜[從 Azure 中的組件庫建立 WordPress 網站](https://www.windowsazure.com/en-us/develop/php/tutorials/website-from-gallery/)＞所提供的指引進行。

將現有的 WordPress 單一網站安裝轉換成多網站通常相當簡單，以下許多初始步驟都直接取材自 [WordPress Codex](http://codex.wordpress.org) 的＜[建立網站](http://codex.wordpress.org/Create_A_Network)＞頁面 (英文)。
...
使用您選擇的編輯器開啟 `wp-config.php` 檔案，將下列程式碼插入 `/* That's all, stop editing!Happy blogging.*/` 上方。
...
如需子網域和子目錄設定兩者的詳細資訊，請參閱 WordPress Codex 上的＜[多網站網路的類型](http://codex.wordpress.org/Before_You_Create_A_Network#Types_of_multisite_network)＞文章 (英文)。
...
在 [Network Setup]** 頁面按一下 **[Install]** 按鈕後，WordPress 將嘗試更新 `wp-config.php` 和 `web.config` 檔案。不過，一定要檢查檔案，以確定更新成功。如果失敗，此畫面會顯示必要的更新。編輯並儲存檔案。
...
[WordPress MU Domain Mapping](http://wordpress.org/extend/plugins/wordpress-mu-domain-mapping/) 外掛程式可讓您輕鬆地將自訂網域加入至網路中的任何網站。為了讓外掛程式正常運作，您需要在入口網站上執行其他一些設定，也需要在網域註冊機構上這樣做。
...
WordPress 多網站目前沒有內建的方法可對應自訂網域。不過，有一個稱為 [WordPress MU Domain Mapping](http://wordpress.org/extend/plugins/wordpress-mu-domain-mapping/) 的外掛程式可為您增加此功能。請登入網站的 [Network Admin] 部分，並安裝 **WordPress MU Domain Mapping** 外掛程式。
