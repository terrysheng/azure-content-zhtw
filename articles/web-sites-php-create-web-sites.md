...
本文說明如何使用 [Azure 管理入口網站][Azure 管理入口網站]、[適用於 Mac 和 Linux 的 Azure 命令列工具][適用於 Mac 和 Linux 的 Azure 命令列工具]或 [Azure PowerShell Cmdlet][Azure PowerShell Cmdlet]，在 [Azure 網站][Azure 網站]中建立 PHP 網站。

大致上，在 Azure 網站中建立 PHP 網站和建立*任何*網站並無不同。依預設，所有網站都會啟用 PHP。如需有關設定 PHP (或提供您自己的自訂 PHP 執行階段) 的詳細資訊，請參閱＜[如何在 Azure 網站中設定 PHP][如何在 Azure 網站中設定 PHP]＞。

下述每一個選項說明如何在共用託管環境中免費建立網站，但 CPU 使用量和頻寬使用量有所限制。如需詳細資訊，請參閱＜[Azure 網站定價][Azure 網站定價]＞。如需有關如何升級和調整網站的詳細資訊，請參閱[如何調整網站][如何調整網站]。

## 目錄

-   [使用 Azure 管理入口網站來建立網站][使用 Azure 管理入口網站來建立網站]
-   [使用適用於 Mac 和 Linux 的 Azure 命令列工具來建立網站][使用適用於 Mac 和 Linux 的 Azure 命令列工具來建立網站]
-   [使用 Azure PowerShell Cmdlet 來建立網站][使用 Azure PowerShell Cmdlet 來建立網站]

## <a name="portal"></a>使用 Azure 管理入口網站來建立 PHP 網站

在 Azure 管理入口網站中建立網站時有三個選項：[快速建立]、[Create with Database] 和 [從組件庫]。以下指示適用於 [快速建立] 選項。如需其他兩個選項的詳細資訊，請參閱＜[建立 PHP-MySQL Azure 網站並使用 Git 部署][建立 PHP-MySQL Azure 網站並使用 Git 部署]＞和＜[從 Azure 中的組件庫建立 WordPress 網站][從 Azure 中的組件庫建立 WordPress 網站]＞。
...
2.  按一下頁面底部的 [新增]，然後依序按一下 [運算]、[網站] 和 [快速建立]。提供網站的 [URL]，並選取網站的 [地區]。最後，按一下 [建立網站]。
    ...
    ![Select Quick Create web site][Select Quick Create web site]

## <a name="XplatTools"></a>使用適用於 Mac 和 Linux 的 Azure 命令列工具來建立 PHP 網站

若要使用適用於 Mac 和 Linux 的 Azure 命令列工具來建立 PHP 網站，請執行下列動作：

1.  依照此處的指示來安裝 Azure 命令列工具：[如何安裝適用於 Mac 和 Linux 的 Azure 命令列工具][如何安裝適用於 Mac 和 Linux 的 Azure 命令列工具]。

2.  依照此處的指示來下載和匯入發行設定檔：[如何下載和匯入發行設定][如何下載和匯入發行設定]。
...
如需其他選項的相關資訊，請參閱＜[如何建立和管理 Azure 網站][如何建立和管理 Azure 網站]＞。

## <a name="PowerShell"></a>使用 Azure PowerShell Cmdlet 來建立 PHP 網站

若要使用 Azure PowerShell Cmdlet 來建立 PHP 網站，請執行下列動作：

1.  依照此處的指示來安裝 Azure PowerShell Cmdlet：[開始使用 Azure PowerShell][開始使用 Azure PowerShell]。

2.  依照此處的指示來下載和匯入發行設定檔：[作法：匯入發行設定][作法：匯入發行設定]。
...
如需其他選項的相關資訊，請參閱＜[作法：建立和管理 Azure 網站][作法：建立和管理 Azure 網站]＞。
...
-   [如何設定網站][如何設定網站]
-   [如何在 Azure 網站中設定 PHP][如何在 Azure 網站中設定 PHP]
-   [如何管理網站][如何管理網站]
-   [如何監視網站][如何監視網站]
-   [如何調整網站][如何調整網站]
-   [使用 Git 來發行][使用 Git 來發行]

如需完整的教學課程系列，請參閱＜[PHP 開發人員中心 - 教學課程][PHP 開發人員中心 - 教學課程]＞頁面 (英文)。
