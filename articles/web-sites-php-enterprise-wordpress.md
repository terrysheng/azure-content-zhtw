<properties 
	pageTitle="Azure 網站上的企業級 WordPress" 
	description="了解如何在 Azure 網站上託管企業級 WordPress 網站" 
	services="web-sites" 
	documentationCenter="" 
	authors="tfitzmac" 
	manager="wpickett" 
...
Azure 網站針對關鍵的大規模 [WordPress 網站][wordpress]，提供可擴充、安全又容易使用的環境。Microsoft 本身執行企業級網站，例如 [Office][officeblog] 和 [Bing][bingblog] 部落格。本文件說明如何使用 Azure 網站，來建立與維護可應付大量訪客的企業級雲端架構 WordPress 網站。
...
* **MySQL 資料庫** - 可透過 [Azure 市集中的 ClearDB][cdbnstore] 取得，或您可以在使用 [Windows][mysqlwindows] 或 [Linux][mysqllinux] 的 Azure 虛擬機器上管理您自己的 MySQL 安裝。
...
使用 [Azure 流量管理員][trafficmanager]，您可以在多個地理區域中調整 WordPress 網站規模，但只提供一個 URL 給訪客。所有透過流量管理員進入的訪客，會根據負載平衡組態接著被路由傳送至某個區域。
...
依預設，Blob 儲存體會在不同區域中異地發佈，因此您無需擔心在所有網站中複寫檔案。您也可以為 Blob 儲存體啟用 Azure [內容發佈網路 (CDN)][cdn]，這會將檔案發佈至靠近訪客的端點。
...
**部署網站** | [部署 Azure 網站][deploy]

####可用性和災難復原

作法...| 目的...
------------------------|-----------
**對網站進行負載平衡**或**對網站進行異地發佈** | [使用 Azure 流量管理員路由傳送流量][trafficmanager]
**備份與還原** | [備份 Azure 網站][backup]與 [還原 Azure 網站][restore]
...
* **手動移轉** - [備份您的網站][wordpressbackup]與[資料庫][wordpressdbbackup]，然後手動將它還原成 Azure 網站和相關聯 MySQL 資料庫，以移轉高度自訂的網站並避免冗長的手動外掛程式、主題或其他自訂安裝。
...
2. 依照[從 Azure 中的組件庫建立 WordPress 網站][createwordpress]中的步驟來建立新的 WordPress 網站。建立網站時，請選取 [使用現有的 MySQL 資料庫]****， 然後選取在步驟 1 中所建立的資料庫。
...
3. 登入您在 Azure 網站上的 WordPress 網站，然後依序按一下 [外掛程式]**** -> [新增]****。搜尋並安裝 **WordPress Importer** 外掛程式。
...
3. 建立新的資料庫並還原備份。
	...
	2. 使用MySQL 用戶端 (例如 [MySQL Workbench][workbench]) 連線到新的資料庫，並匯入您的 WordPress 資料庫。
