...
Azure 網站針對關鍵的大規模 [WordPress][WordPress] 網站，提供可擴充、安全又容易使用的環境。Microsoft 本身執行企業級網站，例如 [Office][Office] 和 [Bing][Bing] 部落格。本文件說明如何使用 Azure 網站，來建立與維護可應付大量訪客的企業級雲端架構 WordPress 網站。

## 本文內容

-   [架構與規劃][架構與規劃] - 請了解架構、需求和效能考量之後，再建立網站
...
-   **MySQL 資料庫** - 可透過 [Azure 市集中的 ClearDB][Azure 市集中的 ClearDB] 取得，或您可在 Azure 虛擬機器上，使用 [Windows][Windows] 或 [Linux][Linux] 來管理自己的 MySQL 安裝。

    > [WACOM.NOTE] ClearDB 提供數個 MySQL 組態，每個組態具有不同的效能特性。請參閱 [Azure 市集][Azure 市集中的 ClearDB]，以取得透過 Azure 市集所提供的產品相關資訊，請參閱 [ClearDB 定價][ClearDB 定價]，以取得 ClearDB 直接提供的產品。

-   **PHP 5.2.4 或更高版本** - Azure 網站目前提供 [PHP 5.3、5.4 和 5.5 版][PHP 5.3、5.4 和 5.5 版]。
...
使用 Azure [流量管理員][流量管理員]，您可以在多個地理區域中調整 WordPress 網站，但只提供一個 URL 給訪客。所有透過流量管理員進入的訪客，會根據負載平衡組態接著被路由傳送至某個區域。
...
您可以使用 ClearDB 的 [CDBR 高可用性路由器][CDBR 高可用性路由器] (如左側所示) 或 [MySQL 叢集 CGE][MySQL 叢集 CGE]，進行複寫和路由傳送至多個 MySQL 資料庫。

### 包含媒體儲存體和快取的多重區域部署

如果網站接受上傳或主機媒體檔案，請使用 Azure Blob 儲存體。如果您需要快取，請考慮 [Redis cache][Redis cache]、[Memcache Cloud][Memcache Cloud]、[MemCachier][MemCachier] 或 [Azure 市集][Azure 市集]中的其中一個快取產品。
...
依預設，Blob 儲存體會在不同區域中異地發佈，因此您無需擔心在所有網站中複寫檔案。您也可以為 Blob 儲存體啟用 Azure [內容發佈網路 (Content Distribution Network，CDN)][內容發佈網路 (Content Distribution Network，CDN)]，這會將檔案發佈至靠近訪客的端點。
...
<td align="left"><a href="http://azure.microsoft.com/zh-tw/gallery/store/sendgrid/sendgrid-azure/">SendGrid</a> 和<a href="http://wordpress.org/plugins/sendgrid-email-delivery-simplified/">使用 SendGrid 的 WordPress 外掛程式</a></td>
</tr>
<tr class="odd">
...
| **備份與還原**                     | [備份 Azure 網站][備份 Azure 網站]和[還原 Azure 網站][還原 Azure 網站]        |
...
| 作法...                  | 目的...                                                                                                                                                               |
|--------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **了解網站執行個體功能** | [定價詳細資料，包括網站大小和模式的功能][定價詳細資料，包括網站大小和模式的功能]                                                                                                                            |
| **快取資源**             | [Redis cache][Redis cache]、[Memcache Cloud][Memcache Cloud]、[MemCachier][MemCachier] 或 [Azure 市集][Azure 市集]中的其中一個快取產品。                                                                           |
| **調整應用程式**         | [調整 Azure 網站][調整 Azure 網站]和 [ClearDB 高可用性路由處理][CDBR 高可用性路由器] (英文)。如果您選擇主控與管理自己的 MySQL 安裝，您應考量可橫向擴充的 [MySQL 叢集 CGE][MySQL 叢集 CGE] (英文)。 |

#### 移轉

將現有的 WordPress 網站移轉成 Azure 網站的方法有兩種。

-   **[WordPress 匯出][WordPress 匯出]** - 這將匯出您的部落格內容，並接著使用 [WordPress 匯入工具外掛程式][WordPress 匯入工具外掛程式]將其匯入 Azure 上的新 WordPress 網站。

    > [WACOM.NOTE] 此程序可讓您移轉內容，但它無法移轉任何外掛程式、主題或其他自訂。您必須手動重新安裝這些項目。

-   **手動移轉** - [備份您的網站][備份您的網站]和[資料庫][資料庫] (英文)，然後手動將它還原成 Azure 網站和相關聯 MySQL 資料庫，以移轉高度自訂的網站並避免冗長的手動外掛程式、主題或其他自訂安裝。

## 作法

### <span id="create"></span></a>建立新的 WordPress 網站

1.  使用 [Azure 市集][Azure 市集中的 ClearDB]，在您將主控網站的區域中建立 MySQL 資料庫，其大小為您在[架構與規劃][架構與規劃]區段中所指定的大小。

2.  依照[從 Azure 中的映像庫建立 WordPress 網站][從 Azure 中的映像庫建立 WordPress 網站]中的步驟來建立新的 WordPress 網站。建立網站時，請選取 [Use an existing MySQL Database]，然後選取在步驟 1 中所建立的資料庫。

如果您打算移轉現有的 WordPress 網站，在建立新網站後，請參閱[移轉現有的 WordPress 網站][移轉現有的 WordPress 網站]。

### <span id="migrate"></span></a>將現有的 WordPress 網站移轉至 Azure

如[架構與規劃][架構與規劃]區段中所提到的，移轉 WordPress 網站的方法有兩種。
...
1.  使用 [WordPress 匯出][WordPress 匯出]來匯出您的現有網站。

2.  使用[建立新的 WordPress 網站][建立新的 WordPress 網站]區段中的步驟建立新網站。
...
7.  在完成所有這些步驟後，請在 [Azure 管理入口網站][Azure 管理入口網站]的網站儀表板中重新啟動網站。
...
| **映像/媒體連結** | 若要更新新位置的連結，請使用 [Velvet Blues Update URLs 外掛程式][Velvet Blues Update URLs 外掛程式] (搜尋和取代工具)，或手動在資料庫中更新 |
...
3.  建立新的資料庫並還原備份。
    ...
    1.  從 [Azure 市集][Azure 市集中的 ClearDB]中購買新的資料庫，或在 [Windows][Windows] 或 [Linux][Linux] VM 上設定 MySQL 資料庫。

    2.  使用 MySQL 用戶端 (如 [MySQL Workbench][MySQL Workbench]) 連線到新的資料庫，並匯入您的 WordPress 資料庫。

    3.  更新資料庫，將網域項目變更成新的 Azure 網站網域。例如，mywordpress.azurewebsites.net。使用[搜尋與取代 WordPress 資料庫指令碼][搜尋與取代 WordPress 資料庫指令碼] 以安全的方式變更所有執行個體。

4.  建立新網站，並發佈 WordPress 備份。

    1.  在 [Azure 管理入口網站][Azure 管理入口網站]中，使用 [新增] -\> [網站] -\> [自訂建立]，來建立新網站和資料庫。這將會建立一個空白網站
...
<li><p>啟用快取。<a href="http://msdn.microsoft.com/zh-tw/library/azure/dn690470.aspx">Redis cache</a> (預覽版) 可與 <a href="https://wordpress.org/plugins/redis-object-cache/">Redis 物件快取 WordPress 外掛程式</a> 搭配使用，或使用 <a href="http://azure.microsoft.com/zh-tw/gallery/store/">Azure 市集</a>的其中一個快取產品。</p></li>
...
<td align="left"><ol>
<li><p><a href="http://azure.microsoft.com/zh-tw/gallery/store/sendgrid/sendgrid-azure/">使用 Azure 市集啟用 SendGrid</a></p></li>
...
-   [WordPress 最佳化][WordPress 最佳化]

-   [將 WordPress 網站轉換成多網站][將 WordPress 網站轉換成多網站]

-   [適用於 Azure 的 ClearDB 升級精靈][適用於 Azure 的 ClearDB 升級精靈]

-   [在 Azure 網站的子資料夾中主控 WordPress][在 Azure 網站的子資料夾中主控 WordPress]

-   [逐步說明：使用 Azure 建立 WordPress 網站][逐步說明：使用 Azure 建立 WordPress 網站]

-   [在 Azure 上主控現有的 WordPress 部落格][在 Azure 上主控現有的 WordPress 部落格]

-   [在 WordPress 中啟用 Pretty Permalinks][在 WordPress 中啟用 Pretty Permalinks]

-   [如何在 Azure 網站上移轉與執行 WordPress 部落格][如何在 Azure 網站上移轉與執行 WordPress 部落格]

-   [如何在 Azure 網站上免費執行 WordPress][如何在 Azure 網站上免費執行 WordPress]

-   [在 2 分鐘內完成在 Azure 上安裝與執行 WordPress][在 2 分鐘內完成在 Azure 上安裝與執行 WordPress]

-   [將 WordPress 部落格移至 Azure - 第 1 部分：在 Azure 上建立 WordPress 部落格][將 WordPress 部落格移至 Azure - 第 1 部分：在 Azure 上建立 WordPress 部落格]

-   [將 WordPress 部落格移至 Azure - 第 2 部分：傳輸內容][將 WordPress 部落格移至 Azure - 第 2 部分：傳輸內容]

-   [將 WordPress 部落格移至 Azure - 第 3 部分：設定自訂網域][將 WordPress 部落格移至 Azure - 第 3 部分：設定自訂網域]

-   [將 WordPress 部落格移至 Azure - 第 4 部分：Pretty Permalinks 和 URL Rewrite 規則][將 WordPress 部落格移至 Azure - 第 4 部分：Pretty Permalinks 和 URL Rewrite 規則]

-   [將 WordPress 部落格移至 Azure - 第 5 部分：從子資料夾移至根目錄][將 WordPress 部落格移至 Azure - 第 5 部分：從子資料夾移至根目錄]

-   [如何在 Azure 帳戶中設定 WordPress 網站][如何在 Azure 帳戶中設定 WordPress 網站]

-   [在 Azure 上維持 WordPress][在 Azure 上維持 WordPress]

-   [在 Azure 上的 WordPress 秘訣][在 Azure 上的 WordPress 秘訣]
  ...
  [Redis cache]: http://msdn.microsoft.com/zh-tw/library/azure/dn690470.aspx
  [Memcache Cloud]: http://azure.microsoft.com/zh-tw/gallery/store/garantiadata/memcached/
  [MemCachier]: http://azure.microsoft.com/zh-tw/gallery/store/memcachier/memcachier/
  ...
  [定價詳細資料，包括網站大小和模式的功能]: https://azure.microsoft.com/zh-tw/pricing/details/web-sites/
