<properties 
	pageTitle="採用 SQL Database 和 WebMatrix 的 PHP 網站 - Azure" 
	description="示範如何使用免費的 WebMatrix IDE 建立和部署 PHP 網站將資料儲存在 SQL Database 中的教學課程。" 
	services="web-sites" 
	documentationCenter="" 
	authors="tfitzmac" 
	manager="wpickett" 
...
* 如何使用管理入口網站建立 Azure 網站和 SQL Database。由於預設會在 Azure 網站上啟用 PHP，因此您不需要執行任何特殊步驟就能執行 PHP 程式碼。
...
4. 輸入資料庫的 [名稱]**** 值，然後選取 [新的 SQL Database 伺服器]****。輸入伺服器登入名稱和密碼 (並確認密碼)。選擇要建立新 SQL Database 伺服器的地區。
	...
	建立網站後，您會看到「[網站名稱] 網站建立成功****」的字樣。接下來，您要取得資料庫連接資訊。
...
1. 在 WebMatrix 中開啟網站之後，按一下 [新增現有的]**** 新增您的應用程式檔案。
...
4. 最後，瀏覽至 http://[您的網站名稱].azurewebsites.net/index.php 啟動應用程式。
