...
本教學課程示範如何建立 PHP 網站，以及在後端使用 Azure 資料表儲存體服務。本教學課程假定您的電腦已經安裝 [PHP](http://www.php.net/manual/en/install.php) 與一部 Web 伺服器。本教學課程裡的說明可運用在包括 Windows、Mac 與 Linux 的任何作業系統上。完成本指南的步驟後，您將擁有一台在 Azure 上執行的 PHP 網站，並能夠存取資料表儲存體服務。
...
-   取得 `Entity` 後，讀取資料的模型為 `Entity->getPropertyValue('[name]')`：
...
6.  請記下您剛建立的儲存體帳戶名稱以及主要金鑰。
    ...
    ![Select Manage Keys](./media/web-sites-php-storage/storage-access-keys.png)
...
3.  依序按一下 [運算]、**[網站]** 及 **[快速建立]**。
    ...
    ![Custom Create a new web site](./media/web-sites-php-storage/website-quick-create.png)

    輸入一個 **URL** 值，然後在 **[區域]** 下拉式清單中為您的網站選取資料中心。按一下對話方塊底部的 **[建立新網站]** 按鈕。

    ![Fill in web site details](./media/web-sites-php-storage/website-quick-create-details.png)

    建立網站後，您會看到 **[Creation of Web Site '[SITENAME]' completed successfully]** 的字樣。現在，您可以啟用 Git 發佈。

4.  按一下網站清單中顯示的網站名稱，以開啟該網站的 **[QUICKSTART]** 儀表板。
...
7.  若要啟用 Git 發佈，您必須提供使用者名稱和密碼。請寫下您建立的使用者名稱與密碼(如果您之前設定過 Git 儲存機制，系統將會略過此步驟。)
...
8.  一旦 Git 儲存機制就緒，系統會顯示 Git 命令的使用指示，以便設定本機儲存機制並將檔案發送至 Azure。
    ...
    ![Git deployment instructions returned after creating a repository for the web site.](./media/web-sites-php-storage/git-instructions.png)
...
3.  瀏覽至 **http://[your web site domain]/createtable.php** 以為應用程式建立資料表。
4.  瀏覽至 **http://[your web site domain]/index.php** 開始使用應用程式。
...
3.  瀏覽至 **http://[your web site domain]/index.php** 以查看您的變更。
