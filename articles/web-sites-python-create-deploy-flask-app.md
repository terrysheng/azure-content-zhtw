...
您將建立使用 Flask Web 架構的應用程式 (請參閱本教學課程適用於 [Django](../web-sites-python-create-deploy-django-app) 與 [Bottle] 的其他版本(../web-sites-python-create-deploy-bottle-app))。您會從 Azure 組件庫建立網站、設定 Git 部署，並於本機複製儲存機制。然後您會在本機執行應用程式、進行變更、認可和推送至 Azure。本教學課程示範如何從 Windows 或 Mac/Linux 執行這項操作。
