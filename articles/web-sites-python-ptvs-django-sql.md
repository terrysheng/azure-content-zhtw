<properties 
	...
	description="了解如何使用 Python Tools for Visual Studio 建立 Django 應用程式，以在 SQL 資料庫儲存個體中儲存資料並佈署到網站。 
	services="" 
...
在這一節中，我們會將應用程式設定為使用我們剛才建立的 SQL Database。我們將可了解如何從 Azure 入口網站取得連線設定。我們也將安裝搭配使用 SQL 資料庫與 Django 時所需的其他 Python 封裝。然後會在本機執行此應用程式。
