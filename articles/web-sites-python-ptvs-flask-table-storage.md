...
在本教學課程中，我們將使用其中一個 PTVS 範例範本來建立簡單的民調應用程式。本教學課程也提供教學[影片](https://www.youtube.com/watch?v=qUtZWtPwbTk)。
...
1.  (選用) 您可以啟用異地複寫。利用異地複寫，Azure 儲存體現在可將您的資料持續放在兩個位置。在這兩個位置中，Azure 儲存體會不斷為您的資料維護多份健全複本。
...
1.  實作 Azure 資料表儲存體存放庫的程式碼位於 **models/azuretablestorage.py** 中。如需有關如何從 Python 使用資料表服務的詳細資訊，請參閱[文件]。

1.  使用 <kbd>F5</kbd> 執行應用程式。使用 [Create Sample Polls]**** 建立的民調以及投票所提交的資料將會在Azure 資料表儲存體中序列化。
