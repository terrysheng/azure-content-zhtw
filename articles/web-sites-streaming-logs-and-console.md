...
可依每個網站為基礎啟用診斷。從[入口網站](https://portal.azure.com)內按一下左功能表列上的 [瀏覽] 按鈕，然後按一下 [Web 應用程式] 取得所有 Web 應用程式的清單。  

![][BrowseSitesScreenshot]

按一下要設定的 Web 應用程式名稱。接著按一下名為 [診斷記錄] 的部分，再將 [Application Logging (Filesystem)] 參數轉換成 [開] 設定。接著出現 [等級] 選項，可讓您變更要擷取之追蹤的嚴重性等級。如果您才剛熟悉此功能，應該將此選項設定為 [詳細資訊]，因此這可確保記錄追蹤陳述式。

按一下刀鋒視窗頂端的 [儲存]，隨後可準備檢視記錄。

若要從入口網站中檢視串流記錄，請按一下 Web 應用程式刀鋒視窗上的 [串流記錄] 部分。如果您的應用程式主動撰寫追蹤陳述式，則您應在結果視窗中近乎即時地看到這些追蹤陳述式。
...
若要取得主控台，請如上節所述瀏覽至 Web 應用程式。按一下 [主控台] 部分，主控台隨即開啟。
