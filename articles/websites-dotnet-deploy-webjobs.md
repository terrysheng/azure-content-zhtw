<properties urlDisplayName="Deploy Azure WebJobs to Azure Websites" pageTitle="將 Azure WebJobs 部署至 Azure 網站 " metaKeywords="Azure 網站, WebJobs, 背景工作" description="Learn how to deploy Azure WebJobs to Azure Websites using Visual Studio." metaCanonical="" services="web-sites" documentationCenter="" title="Deploy Azure WebJobs to Azure Websites" authors="tdykstra"  solutions="" manager="wpickett" editor="mollybos" />
...
本主題說明如何使用 Visual Studio，將主控台應用程式專案作為 [Azure WebJob](http://go.microsoft.com/fwlink/?LinkId=390226) 部署至 Azure 網站。部署 WebJobs 的另一個方法是使用 Azure 管理入口網站；如需該方法的相關資訊，請參閱[在 Microsoft Azure 網站中使用 WebJobs 執行背景工作](/zh-tw/documentation/articles/web-sites-create-web-jobs/)。
...
* 將執行階段檔案複製到 Azure 網站中的適當資料夾 (若是連續 WebJobs，則是 *App_Data/jobs/continuous*，若是排程和隨選 WebJobs，則是 *App_Data/jobs/triggered*)。
...
2.  完成 [[Add Azure WebJob](#configure)] 對話方塊，然後按一下 [**確定**]。
	...
	此時會出現 [**發佈 Web**] 精靈。如果您不打算立即發行，請關閉精靈。您所輸入的設定會被儲存下來，以供[部署專案](#deploy)時使用。
...
1. 依序按一下 [**檔案**] > [**新增專案**]，然後再依序按一下 [**新增專案**] 對話方塊中的 [**雲端**] > [**Microsoft Azure WebJob**]。

	![New Project dialog showing WebJob template](./media/websites-dotnet-deploy-webjobs/np.png)
	
2. 請依照稍早的指示，[將主控台應用程式專案設定成獨立的 WebJobs 專案](#convertnolink)。
...
此對話方塊上的欄位會對應至 Azure 管理入口網站中 [**新增工作**] 對話方塊上的欄位。如需詳細資訊，請參閱[在 Microsoft Azure 網站中使用 WebJobs 執行背景工作](/zh-tw/documentation/articles/web-sites-create-web-jobs/)。

若是排程 WebJob (不適用於連續 WebJobs)，則 Visual Studio 會建立 [Azure 排程器](/zh-tw/services/scheduler/)工作集合 (如果尚未存在)，然後在集合中建立工作：
...
>*如果設定的是 [**週期性工作**]，並設定週期頻率的分鐘數，Azure 排程器不是免費提供的服務。其他頻率 (時數、天數等) 可免費提供。
...
您可以編輯此檔案目錄，而 Visual Studio 會提供 IntelliSense。檔案結構描述會儲存在 [http://schemastore.org](http://schemastore.org/schemas/json/webjob-publish-settings.json)，您可以在那裡進行檢視。  
...
您可以編輯此檔案目錄，而 Visual Studio 會提供 IntelliSense。檔案結構描述會儲存在 [http://schemastore.org](http://schemastore.org/schemas/json/webjobs-list.json)，您可以在那裡進行檢視。
...
若是獨立的 WebJob，則 Web 專案所使用的相同 [**發佈 Web**] 精靈會隨即出現，但其中幾個設定可以變更。
