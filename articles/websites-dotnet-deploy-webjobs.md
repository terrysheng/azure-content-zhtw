...
### <span id="convertlink"></span></a> 透過 Web 專案啟用自動 WebJobs 部署
...
## <span id="create"></span></a>建立具有 WebJobs 功能的新專案
...
## <span id="configure"></span></a>新增 Azure WebJob 對話方塊
...
> [WACOM.NOTE]
>
> -   使用 MSBuild 的命令列部署並不會設定排程 WebJobs 的排程。此功能只有在使用 Visual Studio 部署時才能運作。
...
您可以編輯此檔案目錄，而 Visual Studio 會提供 IntelliSense。檔案結構描述會儲存在 [][]<http://schemastore.org></a>，您可以在那裡進行檢視。
...
您可以編輯此檔案目錄，而 Visual Studio 會提供 IntelliSense。檔案結構描述會儲存在 [][1]<http://schemastore.org></a>，您可以在那裡進行檢視。
...
如需 Azure WebJobs 和 WebJobs SDK 的詳細資訊，請參閱 [Azure WebJobs - 建議使用的資源][]。
  ...
  [Azure WebJobs - 建議使用的資源]: http://www.asp.net/aspnet/overview/developing-apps-with-windows-azure/azure-webjobs-recommended-resources
