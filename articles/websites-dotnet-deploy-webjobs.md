...
* 將執行階段檔案複製到 Web 應用程式中的適當資料夾 (如為連續執行的 WebJob，請複製到 *App_Data/jobs/continuous*，如為排定與依需求執行的 WebJob，則複製到 *App_Data/jobs/triggered*)。
...
* [webjob-publish-settings.json](#publishsettings) 檔案。 
...
2.  完成 [[新增 Azure WebJob]](#configure) 對話方塊，然後按一下 [確定]。
	...
	此時會出現 [發行 Web] 精靈。如果您不打算立即發行，請關閉精靈。您所輸入的設定會被儲存下來，以便在[部署專案](#deploy)時使用。
...
* 排程器工作集合會命名為 *WebJobs-{regionname}*，其中 *{regionname}* 是指裝載 Web 應用程式的區域。例如：WebJobs-WestUS。
* 排程器工作會被命名為 *{websitename}-{webjobname}*。例如：MyWebSite-MyWebJob。 
 
>[AZURE.NOTE]
> 
>* 如需命令列部署的相關資訊，請參閱[啟用 Azure WebJob 的命令列或連續傳遞](/blog/2014/08/18/enabling-command-line-or-continuous-delivery-of-azure-webjobs/)
