...
9. 在 [在 Microsoft Azure 上建立網站]**** 對話方塊的 [網站名稱]**** 方塊中輸入唯一的名稱。

	完整 URL 將包含您在此處輸入的內容，並加上 .azurewebsites.net (如 [網站名稱]**** 文字方塊旁邊所示)。例如，如果網站名稱是 ContosoAds，則 URL 會是 ContosoAds.azurewebsites.net。
...
7. 在 [伺服器總管]**** 中，以滑鼠右鍵按一下您在 [網站]****節點下的網站，然後按一下 [檢視設定]****。

	就會在 [組態]**** 索引標籤中開啟 [Azure 網站]**** 視窗。
...
12. 網暫停止之後，再以滑鼠右鍵按一下網站然後按一下 [啟動網站]****。

	發行時，WebJob 便會自動啟動，但在您進行組態變更時便會停止。若要重新啟動，您可以重新啟動網站，或在 Azure 管理入口網站中重新啟動 WebJob。組態變更後，一般的建議是重新啟動網站。 
...
在此範例應用程式中，網站活動的優先順序一律高於建立佇列訊息，因此如果網站進入休眠以及因長時間閒置而中止時，並不會發生問題。當要求傳入時，網站會被喚醒並重新啟動 WebJob。

針對即使網站長時間閒置您也想要持續執行的 WebJobs，您可以使用 Azure 網站的 [AlwaysOn](http://weblogs.asp.net/scottgu/archive/2014/01/16/windows-azure-staging-publishing-support-for-web-sites-monitoring-improvements-hyper-v-recovery-manager-ga-and-pci-compliance.aspx) 功能。
...
5. 在 [新增 ASP.NET 專案]**** 對話方塊中，選擇 MVC 範本，並取消選取在 [Microsoft Azure]**** 底下的 [雲端主機]**** 核取方塊。
	...
	選取 [雲端主機]**** 可讓 Visual Studio 自動建立新的 Azure 網站和 SQL Database。由於您稍早已建立這些項目，您現在無需在建立專案時執行此作業。如果您要建立一個新的，請勾選此核取方塊。您可以接著設定新網站和 SQL Database，就好像您稍早部署應用程式的方法一樣。
