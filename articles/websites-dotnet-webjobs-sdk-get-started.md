<properties pageTitle="開始使用 Azure WebJobs SDK " metaKeywords="Azure 教學課程, Azure WebJobs 教學課程, Azure 多層次教學課程, MVC 教學課程, Azure blob 教學課程, Azure 佇列教學課程, Azure 儲存體教學課程" description="了解如何使用 ASP.NET MVC 和 Azure 建立多層式應用程式。前端在網站中執行，後端則以 WebJob 的形式執行。該應用程式使用 Entity Framework、SQL Database 及 Azure 儲存體佇列和 Blob。" metaCanonical="" services="web-sites,storage" documentationCenter=".NET" title="Get Started with the Azure WebJobs SDK" authors="tdykstra" solutions="" manager="wpickett" editor="mollybos" />
