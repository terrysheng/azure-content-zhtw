<properties 
	pageTitle="如何使用 WebJobs SDK 來使用 Azure 服務匯流排" 
	description="了解如何使用 WebJobs SDK 來使用 Azure 服務匯流排佇列和主題。" 
...
除了儲存體連接字串之外，您也必須設定 AzureWebJobsServiceBus 連接字串。
...
下列程式碼範例會讀取包含字串的佇列訊息，並將該字串寫入 WebJobs SDK 儀表板。
...
**注意：**如果您正在未使用 WebJobs SDK 的應用程式中建立佇列訊息，請務必將 [BrokeredMessage.ContentType](http://msdn.microsoft.com/zh-tw/library/microsoft.servicebus.messaging.brokeredmessage.contenttype.aspx) 設為 "text/plain"。
...
如需示範如何使用 POCO 的屬性，在同一個函數中使用 Blob 和資料表的程式碼範例，請參閱[本文的儲存體佇列版本](../websites-dotnet-webjobs-sdk-storage-queues-how-to/#pocoblobs).
...
針對 POCO 類型參數，一律會在函數結束時建立佇列訊息；如果參數為 Null，SDK 就會建立佇列訊息，在收到訊息並將它還原序列化時傳回 Null。針對其他類型，如果參數為 Null，就不會建立任何佇列訊息。
