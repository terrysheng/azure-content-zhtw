<properties 
	...
	description="了解如何使用 Azure WebJob 和 Azure WebJobs SDK 的建議資源。" 
...
本主題連結至有關如何使用 Azure WebJobs 和 Azure WebJobs SDK 的文件資源。Azure WebJob 可讓您輕鬆地在 [App Service Web Apps](http://go.microsoft.com/fwlink/?LinkId=529714) 上以背景處理序的方式執行指令碼或程式。您可以上傳並執行可執行檔，例如 cmd、bat、exe (.NET)、ps1、sh、php、py、js 和 jar。這些程式會依照排程 (cron) 或持續以 WebJobs 的方式執行。
...
* [Azure WebJobs 簡介](http://www.hanselman.comblog.md/IntroducingWindowsAzureWebJobs.aspx)
* [Azure WebJob 功能](/blog/2014/10/22/webjobs-goes-into-full-production/)
...
另請參閱[部署 WebJobs](#deploy) 與[測試及偵錯 WebJobs](#debug)的以下各節。

##<a name="deploy"></a>部署 WebJobs

* [如何將 Azure WebJob 部署至 Azure 網站](websites-dotnet-deploy-webjobs.md)
* [如何使用 Azure 管理入口網站部署 WebJob](web-sites-create-web-jobs.md)
...
* [遠端偵錯 WebJob](web-sites-dotnet-troubleshoot-visual-studio.md#remotedebugwj)
...
* [第一個要求的 Azure 網站和雲端服務緩慢](http://wp.sjkp.dk/windows-azure-websites-and-cloud-services-slow-on-first-request/)。示範如何使用 WebJobs 來模擬僅適用於標準網站層的 AlwaysOn 功能。
* [WebJobs 順利關機](http://blog.amitapple.com/post/2014/05/webjobs-graceful-shutdown/#.U72Il_5OWUl)。對於 WebJobs SDK 順利關機，請參閱[順利關機](websites-dotnet-webjobs-sdk-storage-queues-how-to.md#graceful).)
* 影片
...
* [Azure 網站論壇](http://social.msdn.microsoft.com/Forums/azure/home?forum=windowsazurewebsitespreview)
