...
## 案例
以下是常見的 VM 操作，您可以在 asm 和 arm 模式中使用 Azure CLI 命令來執行這類操作。使用許多 arm 模式命令時，您需要傳遞現有的資源群組名稱。
...
將 VM 存取延伸模組新增至 VM | 未提供 | `azure vm reset-access [options] <resource-group> <name>`
...
將 Chef 延伸模組新增至 VM | `azure  vm extension get-chef [options] <vm-name>` | 未提供
停用 VM 延伸模組 | `azure  vm extension set [options] –b <vm-name> <extension-name> <publisher-name> <version>` | 未提供
...
顯示 VM 映像 | `azure vm image show [options]` | 未提供
