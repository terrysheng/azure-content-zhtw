...
xplat-cli 是以 JavaScript 撰寫的，需要搭配 Node.js 一起使用。使用 Azure SDK for Node.js 來實作，並以 Apache 2.0 授權來發行。專案儲存機制位於 [][]<https://github.com/WindowsAzure/azure-sdk-tools-xplat></a>。
...
對於 Linux 系統，您必須先安裝 Node.js，並使用 **npm** 依以下所述安裝 xplat-cli，或是從原始碼加以建立。原始碼位於 [][1]<http://go.microsoft.com/fwlink/?linkid=253472&clcid=0x409></a>。如需從原始碼建立的詳細資訊，請參閱內含在封存中的 INSTALL 檔案。
...
> [WACOM.NOTE] 您可能必須使用 `sudo`，才能順利執行 **npm** 命令。
...
> [WACOM.NOTE] Node.js 可從 <http://nodejs.org/> 進行安裝。

## <span id="Configure"></span></a>如何連線至您的 Azure 訂閱
...
> [WACOM.NOTE] 如果這是您第一次使用這些認證登入，系統會提示您確認是否要快取驗證權杖。如果您先前曾經使用過下列所述的 `azure logout` 命令，也會出現此提示。如果要在自動化作業中略過此提示，請在`azure login` 命令中中使用 `-q` 參數。
>
> [WACOM.NOTE] 當您使用組織帳戶進行驗證時，存取您 Azure 訂閱所需的資訊，會存放在您 `user` 目錄的 `.azure` 目錄中。您的 `user` 目錄會受到作業系統的保護，但建議您採取額外步驟，加密您的 `user` 目錄。作法如下：
...
>
> 在使用下載頁面或是造訪 Azure 管理入口網站來選定之後，所選的 Active Directory 會成為入口網站與下載頁面使用的預設值。建立好預設值之後，您會在下載頁面上方看到 [click here to return to the selection page]。請使用提供的連結，返回選取頁面。

接下來請執行下列命令匯入 `.publishsettings` 檔案，然後將`[path to .publishsettings file]` 變更為 `.publishsettings` 檔案的路徑：

    azure account import [path to .publishsettings file]

> [WACOM.NOTE] 當您匯入發行設定時，存取您 Azure 訂閱所需的資訊，會存放在您 `user` 目錄的 `.azure` 目錄中。您的 `user` 目錄會受到作業系統的保護，但建議您採取額外步驟，加密您的 `user` 目錄。作法如下：
>
...
如果您擁有多重 Azure 訂閱，登入動作將會針對與您的認證相關聯的所有訂閱授予存取權限。如果是使用發行設定檔案，`.publishsettings` 檔案將包含所有訂閱的資訊。無論是哪種方法，都會選取一項訂閱作為 xplat-cli 執行作業時所使用的預設訂閱。您可以檢視訂閱及預設的訂閱，但必須使用 `azure account list` 命令。此命令會傳回類似以下的資訊：
...
在上列清單中，[Current] 資料欄代表目前的預設訂閱，例如 Azure-sub-1。如果要變更預設訂閱，請使用 `azure account set` 命令，並指定您要設為預設的訂閱。例如：
...
如果您要 xplat-cli 使用非預設訂閱，但又不想變更目前的預設，可以使用 `--subscription` 選項，並提供作業所要使用的訂閱名稱。

## <span id="use"></span></a>如何使用 Azure 跨平台命令列介面
...
以上所列的頂層命令包含可與特定 Azure 區域搭配使用的命令。以 `azure account` 命令為例，其便包含了與您 Azure 訂閱相關的命令，例如先前曾經使用的 `download` 與 `import` 設定。

大多數命令都會採用 `azure <command> <operation> [parameters]` 的格式，並對服務或物件 (例如您的帳戶組態) 執行作業。其他命令會提供子命令，並採用下列格式：`azure <command> <subcommand> <operation> [parameters]`。以下列舉可與您的帳戶組態搭配運作的命令範例：
...
`--help` 或`-h` 參數可用於檢視特定命令的說明。或者，您也可使用 `azure help [command] [options]` 格式傳回相同的資訊。例如，以下命令全部都會傳回相同的資訊：
...
當您不確定命令所需的參數時，可使用 `--help`、`-h` 或 `azure help [command]` 來查閱說明。

### 設定組態模式

xplat-cli 可讓您對個別的「資源」(即使用者管理的實體) 執行作業，例如資料庫伺服器、資料庫或網站。這是 xplat-cli 預設的操作模式，我們稱為「Azure 服務管理」。不過，當您使用內含多個資源的複雜解決方案時，能夠將整個解決方案當成同一個單元來管理會讓您輕鬆不少。

為支援以單一邏輯單元或是*資源群組*的方式來管理一組資源，我們推出了 [資源管理員] 的預覽版，做為管理 Azure 資源的全新方式。

> [WACOM.NOTE] 「資源管理員」目前為預覽版，且無法提供與 Azure 服務管理相同等級的管理功能。

為能支援新的資源管理員，xplat-cli 可以讓您使用 `azure config mode` 命令切換這些管理模式。
...
1.  使用下列命令建立新的 Azure 網站。將 **mywebsite** 取代為唯一的名稱。

        azure site create mywebsite

    您會收到提示要求您指定網站的建立區域。請選取地理位置上距離您較近的區域。當此命令完成之後，該網站便便列在 <http://mywebsite.azurewebsites.net> 中 (請將 **mywebsite** 變更為您所指定的名稱)。

    > [WACOM.NOTE] 如果您使用 Git 控制專案來源，可以指定 `--git` 參數，在 Azure 上為此網站建立 Git 儲存機制。這麼做也會在執行該命令的目錄中初始化 Git 儲存機制 (如果還沒有儲存機制的話)。這也會建立名為 **azure** 的 Git remote，可用來使用 `git push azure master` 命令，將部署推送至 Azure 網站。

    > [WACOM.NOTE] 如果遇到錯誤指出 'site' 不是 Azure 命令，則 xplat-cli 很可能處於資源群組模式。如果要切換回資源模式，請使用 `azure config mode asm` 命令。
...
3.  使用下列命令可停止該網站。將 **mywebsite** 取代為網站的名稱。

        azure site stop mywebsite

    在命令完成後，您可以重新整理瀏覽器以確認網站已經停止。

4.  使用下列命令可啟動該網站。將 **mywebsite** 取代為網站的名稱。

        azure site start mywebsite

    在命令完成後，您可以重新整理瀏覽器以確認網站已經啟動。

5.  使用下列命令可刪除該網站。將 **mywebsite** 取代為網站的名稱。

        azure site delete mywebsite

    當此命令完成之後，請使用 `azure site list` 命令確認該網站不再存在。

## <span id="script"></span></a>如何撰寫 Azure 跨平台命令列介面指令碼

您不但可以使用 xplat-cli 手動發出命令，還可以運用您的命令列轉譯器與系統提供的其他命令列公用程式等各項功能，建立複雜的自動化工作流程。例如下列命令可停止所有執行中的 Azure 網站：

    azure site list | grep 'Running' | awk '{system("azure site stop "$2)}'

此範例會輸送一份網站清單給 `grep` 命令，由其檢查每一行有無字串 "Running"。所有符合的字行都會輸送給 `awk` 命令，由其呼叫 `azure site stop` 並使用傳遞給它的第二個資料欄 (執行中之網站的名稱)，做為要停止的網站名稱。
...
建立指令碼時，通常您需要擷取命令輸出並將其傳遞至另一個命令，或是對輸出執行操作，例如檢查特定值。xplat-cli 會將輸出產生至 STDOUT 與 STDERR。標以前置詞 `info:` 的字行為資訊狀態訊息；標以 `data:` 的字行為服務相關的傳回資料；您也可使用 `--verbose` 或 `-v` 參數，指示 xplat-cli 傳回更多的詳細資訊。這會傳回更多的資訊，並標以前置詞 `verbose:`。
...
> [WACOM.NOTE] `--json` 參數會造成無法產生狀態或資料資訊 (前置詞為 `info:` 及 `data:`)。例如搭配使用 `--json` 參數與 `azure site create`，將不會傳回任何輸出，因此此命令只會傳回 `info:` 的資料。
...
-   如需 Azure 的詳細資訊，請參閱 [][2]<http://azure.microsoft.com/></a>。
  ...
  [Windows 安裝程式]: http://go.microsoft.com/?linkid=9828653&clcid=0x409
