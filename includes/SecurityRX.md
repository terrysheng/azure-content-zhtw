...
開發 Azure 的應用程式時，身分識別與存取是您必須謹記於心的主要安全性考量。本主題說明雲端中與身分識別和存取相關的主要安全性考量，以及您如何為雲端應用程式提供最佳保護。

## 概觀

應用程式的安全性是其介面的功能之一。應用程式公開的介面越多，安全性疑慮也就越高。例如，從安全性角度來看，以自動批次處理方式執行之應用程式的公開介面會比公開可用的網站還要少。

移往雲端時，您無需擔心基礎結構和網路，因為資料中心會使用世界級安全性作法、工具、技術來管理基礎結構和網路。另一方面，雲端在本質上會為您的應用程式公開較多的介面區域，因而可能會遭到攻擊者所利用。這是因為許多雲端技術和服務會以端點 (而非記憶體內部元件) 的方式公開。您可透過線路上的端點來存取 Azure 儲存體、服務匯流排、SQL Database (之前稱為 SQL Azure) 和許多其他服務。

在雲端應用程式中，應用程式開發人員會擔負更多設計、開發及維護其雲端應用程式以符合高安全性標準的重責大任，以將攻擊者阻擋於千里之外。請細想下圖 (出處源自 J.D. Meier 的 [Azure 安全性注意事項 PDF][])：請注意，雲端提供者 (在範例中是 Azure) 會負責解決基礎結構的部分，而更多的安全性工作會留給應用程式開發人員：

![保護應用程式的安全][]

好消息是您已經熟悉的所有安全性開發作法、原則和技術仍都適用於雲端應用程式的開發。請考量下列必須解決的主要區域：

-   所有輸入都會經過類型、長度、範圍及字串模式的驗證以避免插入式攻擊，而應用程式回應的所有資料也會受到妥善處理。
-   在靜止狀態和在線路上的敏感性資料應受到保護，以減輕資訊洩漏與資料遭到竄改等威脅。
-   稽核和記錄必須正確實作，才能減輕非否認性的威脅。
-   驗證與授權必須使用平台所提供之已經實證的機制來實作，
    以防止身分詐騙和提升權限等威脅。

如需威脅、攻擊、弱點及對策的完整清單，請參閱模式與作法的[速查表：Web應用程式安全性框架][]和[應用程式索引的安全性指導方針][]。

雲端的驗證和存取控制機制，與內部部署應用程式所使用的驗證和存取控制機制有極大的差異。雲端應用程式有更多的驗證和存取選項，這些驗證和存取選項有可能會引起混淆，最終讓實作出現瑕疵。在定義什麼是雲端應用程式時則會產生更多的誤解。例如，您可以在雲端中部署應用程式，但其驗證機制可能是由 Active Directory 所提供。另一方面，您也可在內部部署中部署應用程式，但將驗證機制放在雲端 (例如，透過 Azure Active Directory Access Control (先前稱為存取控制服務或 ACS) 進行驗證)。

## 威脅、弱點及攻擊

威脅是您想要避免的潛在不良結果，例如，洩漏敏感性資訊或服務無法使用。使用縮寫字 "STRIDE" 來將威脅分類是常見的作法：
...
弱點是指開發人員引進程式碼的錯誤，使得應用程式容易受到攻擊者的利用。例如，以純文字方式傳送敏感性資料，流量探查攻擊會讓資訊洩漏威脅成為可能。攻擊是指這些弱點遭人利用，而對應用程式造成傷害。例如，跨網站指令碼或 XSS 是指入侵未淨化輸出的攻擊。另一個範例是竊聽線路，擷取以純文字方式傳送的認證。這些攻擊都會演變成您我所熟知的身分詐騙威脅。簡言之，請將威脅、弱點和攻擊看成是壞事。將下圖視為與部署到 Azure 之 Web 應用程式相關壞事的上層檢視 (出處源自 J.D.Meier 的 [Azure 安全性注意事項 PDF][Azure 安全性
注意事項 PDF])：
...
-   MSDN Code Gallery 上的 [Windows Identity Foundation 4.5 範例][]。
-   MSDN Code Gallery 上的[適用於 Visual Studio 11 Beta 的 Windows Identity Foundation 4.5 工具][]。
-   MSDN 上的 [Windows Identity Foundation 執行階段 (.Net 3.5/4.0)][]。
-   MSDN 上的 [Windows Identity Foundation 3.5/4.0 範例及 Visual Studio 2008/2010 範本][]。
...
本節將簡短概述本主題所涵蓋的重要案例。請將它當作對應表，為您的應用程式找出適合的身分識別解決方案。
...
-   [作法：使用 WIF 與 ACS 在宣告感知 ASP.NET 應用程式中實作宣告授權][]
-   [作法：使用 WIF 與 ACS 在宣告感知 ASP.NET 應用程式中實作以角色為基礎的存取控制 (RBAC)][]
-   [作法：使用 X.509 憑證設定 ACS 與 ASP.NET Web 應用程式之間的信任][]
-   [程式碼範例：ASP.NET 簡易表單][]

### 使用服務身分識別的 WCF (SOAP) 服務

在此應用程式架構案例中，您的 WCF (SOAP) 服務可能會部署到 Azure 或內部部署。Web 應用程式或甚至另一個 Web 服務會將此服務當作下游服務來存取。您必須使用應用程式特定的身分識別來控制對此服務的存取。把它想像成您在 IIS 中使用的應用程式集區帳戶類型，即使技術不同，但使用應用程式範圍帳戶或使用者帳戶來存取服務的方法卻很類似。

在 Azure AD 存取控制中使用服務身分識別功能。這類似將應用程式部署到 Windows Server 和 IIS 時所使用的 IIS 應用程式集區帳戶。設定 Azure AD 存取控制，簽發將由 WCF (SOAP) 服務中的 WIF 進行處理的 SAML 權杖。

![WCF (SOAP) 服務][]

若要實作此案例，請參閱下列資源：

-   [作法：使用 X.509 憑證、密碼或對稱金鑰新增服務身分識別][]
-   [作法：以用戶端憑證驗證受 ACS 保護的 WCF 服務][]
-   [作法：以使用者名稱與密碼驗證受 ACS 保護的 WCF 服務][]
-   [程式碼範例：WCF 憑證驗證][]
-   [程式碼範例：WCF 使用者名稱驗證][]

### 使用同盟驗證 (Active Directory 中的身分識別) 的 WCF (SOAP) 服務

在此應用程式架構案例中，您的 WCF (SOAP) 服務可能會部署到 Azure 或內部部署。您需要使用由企業 Windows Server Active Directory (AD) 所管理的身分識別，來控制對此服務的存取。

使用針對 Windows Server AD FS 同盟所設定的 Azure AD 存取控制。在此情況中，您不需要使用 Azure AD 存取控制來設定服務身分識別。需要存取 WCF (SOAP) 服務的代理程式會提供 AD FS 認證，並在驗證成功時收到由 AD FS 簽發的權杖。此權杖接著會提交到 Azure AD 存取控制，然後重新簽發還給該代理程式。該代理程式會使用此權杖來提交要求給 WCF (SOAP) 服務。
...
在此案例中，您的 WCF (REST) 服務可能會部署到 Azure 或內部部署。Web 應用程式或另一個 Web 服務會將此服務當作下游服務來存取。您必須使用應用程式特定的身分識別來控制對此服務的存取。把它想像成您在 IIS 中使用的應用程式集區帳戶類型，即使技術不同，但使用應用程式範圍帳戶或使用者帳戶來存取服務的方法卻很類似。

在 Azure AD 存取控制中使用服務身分識別功能。設定 Azure AD 存取控制，以簽發簡式 Web 權杖 (SWT) 權杖。若要在 REST 服務端處理 SWT 權杖，您可以實作自訂權杖處理常式並將它插入 WIF 管線，或者您可以在不使用 WIF 基礎結構的情況下「手動」剖析權杖。
...
-   [作法：使用對稱金鑰設定 ACS 與 WCF 服務之間的信任][]
-   [作法：使用 ACS 驗證部署到 Azure 的 REST WCF 服務][]
-   [程式碼範例：ASP.NET Web 服務][]
-   [程式碼範例：Windows Phone 7 應用程式][程式碼範例：ASP.NET Web 服務]
-   [使用由 Azure 存取控制服務 (ACS) 簽發之 SWT 權杖的 REST WCF][]
...
使用 Azure AD 存取控制來簽發 SWT 權杖。若要在 REST 服務端處理 SWT 權杖，您可以實作自訂權杖處理常式並將它插入 WIF 管線，或者您可以在不使用 WIF 基礎結構的情況下「手動」剖析權杖。

要注意的重點是，為了實作此案例，應用程式必須使用網頁瀏覽器控制項來收集使用者認證。如此一來，它便不適用於從 ASP.NET Web 應用程式存取 REST 服務的案例。它只適用於由使用者用戶端應用程式 (例如 Windows Phone 7 應用程式或豐富型桌面用戶端) 存取 REST 服務的案例。快顯網頁瀏覽器控制項的主要原因是，網際網路身分識別並不直接支援主動式設定檔案例 (Web 服務案例)。網際網路身分識別主要支援依賴瀏覽器重新導向的被動式設定檔案例 (Web 應用程式)：在此案例中，網頁瀏覽器控制項就能派上用場。
...
-   [作法：將 Yahoo! 設定為身分識別提供者][]
-   [程式碼範例：Windows Phone 7 應用程式][程式碼範例：ASP.NET Web 服務]
-   [使用由 Azure 存取控制服務 (ACS) 簽發之 SWT 權杖的 REST WCF][]

### 使用共用 SWT 權杖，從 ASP.NET Web 應用程式到 REST WCF 服務

在此案例中，您擁有使用前端 ASP.NET Web 應用程式和下游 REST 服務的分散式應用程式，而且您想要跨實體層來維護使用者的內容。在下游 REST 服務中實作授權邏輯或根據使用者的身分識別登入時，這一點有時是必要的。

設定 Azure AD 存取控制來簽發 SWT 權杖。SWT 權杖會簽發給前端 ASP.NET Web 應用程式，然後與下游 REST 服務共用。在此情況中，只會在 Azure AD 存取控制中設定一個信賴憑證者。但是，請注意以下幾點情況：

-   由於 WIF 不提供現成的 SWT 權杖處理常式，
    您必須實作自訂權杖處理常式，才能與 ASP.NET Web 應用程式搭配使用。
    您應該依賴 WIF 執行繁重工作，
    以支援依賴瀏覽器重新導向的 WS-同盟通訊協定，而非自己進行實作。
-   實作 SWT 自訂權杖處理常式時，
    請確保會處理啟動程序權杖以確實保留該權杖。
    否則，您將無法共用它並將它傳送至下游 REST 服務。
-   您不需在 REST 服務上使用 WIF；更確切地說，
    您可以「手動」剖析權杖，因為此案例沒有必要處理重新導向。
...
-   [作法：將 Facebook 設定為身分識別提供者][]
-   [作法：將 Yahoo! 設定為身分識別提供者][]
-   [使用共用 SWT 權杖，從 ASP.NET Web 應用程式到 REST WCF 服務委派][]

### 在宣告感知應用程式與服務中以角色為基礎的存取控制 (RBAC)

在此案例中，您需要在 Web 應用程式或服務中根據使用者角色實作授權：具備所需角色的使用者會取得存取權，而不具所需角色的使用者則會遭到拒絕。簡言之，您的應用程式必須回答這個簡單的問題 - 使用者的角色是否為 X？
...
-   [作法：使用 WIF 與 ACS 在宣告感知 ASP.NET 應用程式中實作以角色為基礎的存取控制 (RBAC)][]
-   [作法：使用規則實作權杖轉換邏輯][]
-   [在宣告感知 (WIF) ASP.NET Web 應用程式中使用 RoleManager 進行授權][]
-   程式碼範例：在 [Windows Identity Foundation SDK][]
    的 IsInRole 中使用宣告。
...
-   [安全性方針和限制 (SQL Database)][]
-   [作法：使用 sqlcmd 連線到 SQL Database][]
-   [作法：使用 ADO.NET 連線到 SQL Database][]
-   [作法：透過 ASP.NET 連線到 SQL Database][]
-   [作法：透過 WCF 資料服務連線到 SQL Database][]
-   [作法：使用 PHP 連線到 SQL Database][]
-   [作法：使用 JDBC 連線到 SQL Database][]
-   [作法：使用 ADO.NET Entity Framework 連線到 SQL Database][]
...
-   [作法：透過程式設計方式設定 Azure 快取的快取用戶端][]
-   [作法：使用應用程式組態檔設定 Azure 快取的快取用戶端][]
-   [Azure 服務匯流排和快取範例][] (＜快取範例＞一節)
...
-   [在您的 Marketplace 應用程式中使用 HTTP 基本驗證][]
...
下列是 WIF 的主要安全性旋鈕。以下資訊摘錄自 [WIF 設計考量][]和[Windows Identity Foundation (WIF) Security適用於 ASP.NET Web 應用程式的 Windows Identity Foundation (WIF) 安全性 - 威脅與對策][]
。
...
請考量在 Azure AD 存取控制部署中的下列安全性旋鈕。以下資訊是摘錄自 [ACS 安全性指導方針][]和[憑證與金鑰管理指導方針][]。
...
-   [存取控制服務][]
-   [ACS 的作法][]
-   [使用存取控制服務 v2.0 來保護 Azure Web 角色 ASP.NET Web 應用程式的安全][]
-   [Azure AD 存取控制服務 (ACS) 學術視訊][]
-   [Microsoft 安全性開發生命週期][]
-   [SDL 威脅模型化工具 3.1.8][]
-   [安全性與隱私權部落格][]
...
-   [安全性開發人員中心 (MSDN)][]
  ...
  [應用程式索引的安全性指導方針]: http://msdn.microsoft.com/en-us/library/ff650760.aspx
  ...
  [Windows Identity Foundation 執行階段 (.Net 3.5/4.0)]: http://www.microsoft.com/en-us/download/details.aspx?id=17331
  [Windows Identity Foundation 3.5/4.0 範例及 Visual Studio 2008/2010 範本]: http://www.microsoft.com/en-us/download/details.aspx?displaylang=en&id=4451
  [存取控制服務 2.0]: http://msdn.microsoft.com/library/gg429786.aspx
  [使用 ACS 的案例與解決方案]: http://msdn.microsoft.com/en-us/library/gg185920.aspx
  [ACS 的作法]: http://msdn.microsoft.com/en-us/library/windowsazure/gg185939.aspx
  [以宣告為基礎的身分識別與存取控制指南]: http://msdn.microsoft.com/en-us/library/ff423674.aspx
  [身分識別開發人員訓練套件]: http://www.microsoft.com/en-us/download/details.aspx?id=14347
  [MSDN 代管的身分識別開發人員訓練課程]: http://msdn.microsoft.com/en-us/IdentityTrainingCourse
  [AD FS 2.0 內容對應表]: http://social.technet.microsoft.com/wiki/contents/articles/2735.ad-fs-2-0-content-map.aspx
  [網頁 SSO 設計]: http://technet.microsoft.com/en-us/library/dd807033(WS.10).aspx
  [同盟網頁 SSO 設計]: http://technet.microsoft.com/en-us/library/dd807050(WS.10).aspx
  [管理對 Blob 和容器的存取]: http://msdn.microsoft.com/en-us/library/ee393343.aspx
  ...
  [作法：使用 ACS 建立我的第一個宣告感知 ASP.NET 應用程式]: http://msdn.microsoft.com/en-us/library/gg429779.aspx
  [作法：在您的 ASP.NET Web 應用程式中主控登入頁面]: http://msdn.microsoft.com/en-us/library/gg185926.aspx
  [作法：使用 WIF 與 ACS 在宣告感知 ASP.NET 應用程式中實作宣告授權]: http://msdn.microsoft.com/en-us/library/gg185907.aspx
  [作法：使用 WIF 與 ACS 在宣告感知 ASP.NET 應用程式中實作以角色為基礎的存取控制 (RBAC)]: http://msdn.microsoft.com/en-us/library/gg185914.aspx
  [作法：使用 X.509 憑證設定 ACS 與 ASP.NET Web 應用程式之間的信任]: http://msdn.microsoft.com/en-us/library/gg185947.aspx
  [程式碼範例：ASP.NET 簡易表單]: http://msdn.microsoft.com/en-us/library/gg185938.aspx
  ...
  [作法：以用戶端憑證驗證受 ACS 保護的 WCF 服務]: http://msdn.microsoft.com/en-us/library/hh289316.aspx
  [作法：以使用者名稱與密碼驗證受 ACS 保護的 WCF 服務]: http://msdn.microsoft.com/en-us/library/gg185954.aspx
  [程式碼範例：WCF 憑證驗證]: http://msdn.microsoft.com/en-us/library/gg185952.aspx
  [程式碼範例：WCF 使用者名稱驗證]: http://msdn.microsoft.com/en-us/library/gg185927.aspx
  ...
  [作法：使用管理服務來將 AD FS 2.0 設定為企業身分識別提供者]: http://msdn.microsoft.com/en-us/library/gg185905.aspx
  [程式碼範例：以 AD FS 2.0 驗證 WCF 同盟]: http://msdn.microsoft.com/en-us/library/hh127796.aspx
  [REST 服務]: ./media/SecurityRX/06_RESTService.gif
  [作法：使用對稱金鑰設定 ACS 與 WCF 服務之間的信任]: http://msdn.microsoft.com/en-us/library/gg185958.aspx
  [作法：使用 ACS 驗證部署到 Azure 的 REST WCF 服務]: http://msdn.microsoft.com/en-us/library/hh289317.aspx
  [程式碼範例：ASP.NET Web 服務]: http://msdn.microsoft.com/en-us/library/gg983271.aspx
  ...
  [作法：將 Google 設定為身分識別提供者]: http://msdn.microsoft.com/en-us/library/gg185976.aspx
  [作法：將 Facebook 設定為身分識別提供者]: http://msdn.microsoft.com/en-us/library/gg185919.aspx
  [作法：將 Yahoo! 設定為身分識別提供者]: http://msdn.microsoft.com/en-us/library/gg185977.aspx
  ...
  [作法：使用規則實作權杖轉換邏輯]: http://msdn.microsoft.com/en-us/library/gg185955.aspx
  ...
  [安全性方針和限制 (SQL Database)]: http://msdn.microsoft.com/en-us/library/windowsazure/ff394108.aspx#authentication
  [作法：使用 sqlcmd 連線到 SQL Database]: http://msdn.microsoft.com/en-us/library/windowsazure/ee336280.aspx
  [作法：使用 ADO.NET 連線到 SQL Database]: http://msdn.microsoft.com/en-us/library/windowsazure/ee336243.aspx
  [作法：透過 ASP.NET 連線到 SQL Database]: http://msdn.microsoft.com/en-us/library/windowsazure/ee621781.aspx
  [作法：透過 WCF 資料服務連線到 SQL Database]: http://msdn.microsoft.com/en-us/library/windowsazure/ee621789.aspx
  [作法：使用 PHP 連線到 SQL Database]: http://msdn.microsoft.com/en-us/library/windowsazure/ff394110.aspx
  [作法：使用 JDBC 連線到 SQL Database]: http://msdn.microsoft.com/en-us/library/windowsazure/gg715284.aspx
  [作法：使用 ADO.NET Entity Framework 連線到 SQL Database]: http://msdn.microsoft.com/en-us/library/windowsazure/ff951633.aspx
  ...
  [利用存取控制服務進行服務匯流排驗證與授權]: http://msdn.microsoft.com/en-us/library/hh403962.aspx
  [9]: ./media/SecurityRX/17_WindowsAzureCacheIdentity.gif
  [作法：透過程式設計方式設定 Azure 快取的快取用戶端]: http://msdn.microsoft.com/en-us/library/windowsazure/gg618003.aspx
  [作法：使用應用程式組態檔設定 Azure 快取的快取用戶端]: http://msdn.microsoft.com/en-us/library/windowsazure/gg278346.aspx
  [Azure 服務匯流排和快取範例]: http://msdn.microsoft.com/en-us/library/ee706741.aspx
  [10]: ./media/SecurityRX/18_IAccessMyDataset.gif
  [在您的 Marketplace 應用程式中使用 HTTP 基本驗證]: http://msdn.microsoft.com/en-us/library/gg193417.aspx
  ...
  [WIF 設計考量]: http://msdn.microsoft.com/en-us/library/ee517298.aspx



  [憑證與金鑰管理指導方針]: http://msdn.microsoft.com/en-us/library/hh204521.aspx
  ...
  [存取控制服務]: http://msdn.microsoft.com/en-us/library/windowsazure/gg429786.aspx
