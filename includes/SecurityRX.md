...
在移往雲端時，您無需再擔心基礎結構和網路，因為資料中心會使用世界級安全性作法、工具、技術來管理基礎結構和網路。但另一方面，雲端在本質上會為您的應用程式公開較多的介面區域，因而有可能會遭到攻擊者所利用。這是因為許多雲端技術和服務會以端點 (而非記憶體內部元件) 的方式公開。Azure 儲存體, 服務匯流排, SQL Database (先前稱為 SQL Azure) 和許多其他服務是透過網路端點加以存取。
...
如需威脅、攻擊、弱點及對策的完整清單，請參閱模式與作法的[速查表：Web 應用程式安全性架構](http://msdn.microsoft.com/library/ff649461.aspx)和[應用程式索引安全性指南](http://msdn.microsoft.com/library/ff650760.aspx)。

雲端的驗證和存取控制機制，與內部部署應用程式所使用的驗證和存取控制機制有極大的差異。雲端應用程式有更多的驗證和存取選項，這些驗證和存取選項有可能會引起混淆，最終讓實作出現瑕疵。在定義什麼是雲端應用程式時則會產生更多的誤解。例如，您可在雲端中部署應用程式，但可能由 Active Directory 提供其驗證機制。另一方面，應用程式可能會在內部部署，但在雲端 (例如，由 Azure 的驗證機制 Active Directory Access Control (先前稱為存取控制服務或 ACS) 進行驗證)。
...
下圖視為有害的項目 (從 J.D.部署至 Azure 之 Web 應用程式相關壞東西檢視 Meier 的 [Azure 安全性注意事項 PDF](http://blogs.msdn.com/b/jmeier/archive/2010/08/03/now-available-azure-security-notes-pdf.aspx))：
...
**Windows Identity Foundation (WIF)。**WIF 是.NET 執行階段程式庫隨附於.NET Framework 4.5 (它也是可個別下載的.NET 3.5/4.0 中)。WIF 擔負處理通訊協定，例如 WS-同盟和 Ws-trust 及權杖處理這類的重責大任，安全性判斷提示標記語言 (SAML) 讓您不需要在您的應用程式中撰寫非常複雜的安全性相關程式碼。下列資源提供有關 WIF 的深入資訊：
...
服務 (STS)，例如，例如公司 Active Directory 或網際網路 Idp 可讓與不同的身分識別提供者 (idp 建立) 同盟
Windows Live ID/Microsoft 帳戶、Facebook、Google、Yahoo! 和 Open
ID 2.0 等身分識別提供者)。下列資源提供有關 Azure AD 存取控制的深入資訊：
...
**Active Directory Federation Services (AD FS)。**Active Directory Federation Services (AD FS) 2.0 支援包含 Windows Server 的宣告感知身分識別解決方案。與 Active Directory 技術。AD FS 2.0 支援 WS-Trust、WS-同盟和 SAML 通訊協定。下列資源提供有關 AD FS 的深入資訊：
...
**Azure 共用存取簽章。** 共用存取簽章可讓您微調對 blob 或容器資源的存取。下列資源提供有關共用存取的深入資訊簽章的深入資訊。
...
-   **使用同盟驗證的 ASP.NET Web Form 應用程式。**在這個案例中您必須控制存取至 ASP.NET Web Form 應用程式使用任一網際網路身分識別，例如 Live ID / Microsoft 帳戶或公司的身份識別管理 Windows Server Active Directory 中。
-   **使用同盟驗證的 WCF (SOAP) 服務。**在此案例中，您將使用由 Azure AD 存取控制所管理的服務身分識別，來控制對 WCF (SOAP) 服務的存取。
-   **使用同盟驗證，在 Active Directory 中的身分識別的 WCF (SOAP) 服務。**在這種情況下您控制對 WCF (SOAP) web 服務使用由企業 Windows Server Active Directory 管理身分識別存取。
-   **使用同盟驗證的 WCF (REST) 服務。**在此案例中，您將使用由 Azure AD 存取控制所管理的服務身分識別，來控制對 WCF (REST) 服務的存取。
-   **WCF (REST) 服務使用 Live ID / Microsoft 帳戶、 Facebook、 Google、 yahoo!、 Open id。**在這個案例中您控制存取對 WCF (REST) 服務的使用網際網路身分識別，例如 Live ID / Microsoft 帳戶。
-   **ASP.NET Web 應用程式到 REST WCF 服務使用共用 SWT 權杖。** 在這個案例中您散發應用程式使用前端 ASP.NET web 應用程式和下游 REST 服務，而且您想要透過實體層的使用者的內容。
-   **宣告感知應用程式與服務中的角色型存取控制 (RBAC) 授權。**在這個案例中您想要在角色為基礎的應用程式中實作授權邏輯。
-   **在宣告感知應用程式和服務中的宣告型授權。**在這個案例中您想要根據複雜的授權規則的應用程式中實作授權邏輯。
...
-   [做法：使用 ACS 建立我的第一個宣告感知 ASP.NET 應用程式](http://msdn.microsoft.com/library/gg429779.aspx)
-   [做法：在您的 ASP.NET Web 應用程式中主控登入頁面](http://msdn.microsoft.com/library/gg185926.aspx)
-   [做法：使用 WIF 與 ACS 在宣告感知 ASP.NET 應用程式中實作宣告授權](http://msdn.microsoft.com/library/gg185907.aspx)    
-   [做法：使用 WIF 與 ACS 在宣告感知 ASP.NET 應用程式中實作以角色為基礎的存取控制 (RBAC)](http://msdn.microsoft.com/library/gg185914.aspx)
-   [做法：使用 X.509 憑證設定 ACS 與 ASP.NET Web 應用程式之間的信任](http://msdn.microsoft.com/library/gg185947.aspx)
...
這與將應用程式部署到 Windows Server 和 IIS 時所使用的 IIS 應用程式集區帳戶十分類似。設定 Azure Azure AD 存取控制，簽發將由 WCF (SOAP) 服務中的 WIF 進行處理的 SAML 權杖。
...
-   [做法：使用 X.509 憑證、密碼或對稱金鑰新增服務身分識別](http://msdn.microsoft.com/library/gg185924.aspx)
-   [做法：以用戶端憑證驗證受 ACS 保護的 WCF 服務](http://msdn.microsoft.com/library/hh289316.aspx)
-   [做法：以使用者名稱與密碼驗證受 ACS 保護的 WCF 服務](http://msdn.microsoft.com/library/gg185954.aspx)
...
-   [做法：使用 X.509 憑證、密碼或對稱金鑰新增服務身分識別](http://msdn.microsoft.com/library/gg185924.aspx)
-   [做法：將 AD FS 2.0 設定為身分識別提供者](http://msdn.microsoft.com/library/gg185961.aspx)
-   [做法：使用管理服務來將 AD FS 2.0 設定為企業身分識別提供者](http://msdn.microsoft.com/library/gg185905.aspx)
...
-   [做法：使用對稱金鑰設定 ACS 與 WCF 服務之間的信任](http://msdn.microsoft.com/library/gg185958.aspx)
-   [做法：使用 ACS 驗證部署到 Azure 的 REST WCF 服務](http://msdn.microsoft.com/library/hh289317.aspx)
...
要注意的重點是，為了實作此案例，應用程式必須使用網頁瀏覽器控制項來收集使用者認證。如此一來，它便不適用於從 ASP.NET Web 應用程式存取 REST 服務的案例。它只適用於由使用者用戶端應用程式 (例如 Windows Phone 7 應用程式或豐富型桌面用戶端) 存取 REST 服務的案例。提出網頁瀏覽器控制項的主要原因是，網際網路身分識別並不直接支援作用中的設定檔案例 (web 服務案例)。網際網路身分識別主要支援依賴瀏覽器重新導向的被動式設定檔案例 (web 應用程式)： 這是 web 瀏覽器控制項能派上用場的地方。
...
-   [做法：使用 ACS 驗證部署到 Azure 的 REST WCF 服務](http://msdn.microsoft.com/library/hh289317.aspx)
-   [做法：將 Google 設定為身分識別提供者](http://msdn.microsoft.com/library/gg185976.aspx)
-   [做法：將 Facebook 設定為身分識別提供者](http://msdn.microsoft.com/library/gg185919.aspx)
-   [做法：將 Yahoo! 設定為身分識別提供者](http://msdn.microsoft.com/library/gg185977.aspx)
-  [程式碼範例：Windows Phone 7 應用程式](http://msdn.microsoft.com/library/gg983271.aspx)
-   [使用由 Azure 存取控制服務 (ACS) 簽發之 SWT 權杖的 REST WCF](http://code.msdn.microsoft.com/REST-WCF-With-SWT-Token-123d93c0)
...
在此案例中，您擁有使用前端 ASP.NET web 應用程式和下游 REST 服務，並且想要跨實體層維護使用者的內容。在下游 REST 服務中實作授權邏輯或根據使用者的身分識別登入時，這一點有時候是必要的。
...
**Azure AD 存取控制**.在此實作中， 我們將會用到 Azure AD 存取控制宣告轉換規則引擎。透過宣告轉換規則引擎規則，您可以將任何傳入宣告轉換成角色類型宣告，因此當權杖傳到應用程式或服務時，WIF 可以剖析此角色宣告以確保順利呼叫 IsInRole 方法。

![][09]

**WIF ClaimsAuthenticationManager**.在此實作中，我們將會使用 ClaimsAuthenticationManager 做為 WIF 的延伸點。透過此方法，您可以在應用程式中將任何任意傳入宣告轉換成角色宣告類型。轉換的複雜度只會受到所編寫程式碼的限制。

![][10]

**samlSecurityTokenRequriement mapping**.在此實作中，您將使用 web.config 中的 samlSecurityTokenRequirement 組態，來告訴 WIF 哪個宣告類型要表現出角色宣告類型的行為方式。例如，如果權杖夾帶了群組類型的宣告，則您可以將它對應至角色宣告類型。若使用此方法，您只能執行簡易對應。

![][11]

**自訂 RoleManager。**在此實作中，您實作自訂 RoleManger。實作自訂 RoleManager 介面方法 (例如 GetAllRoles()) 時，我們將使用 WIF 來檢查傳入宣告。
...
-   [做法：使用 WIF 與 ACS 在宣告感知 ASP.NET 應用程式中實作以角色為基礎的存取控制 (RBAC)](http://msdn.microsoft.com/library/gg185914.aspx)
-   [做法：使用規則實作權杖轉換邏輯](http://msdn.microsoft.com/library/gg185955.aspx)
...
-   程式碼範例：在 [Windows Identity Foundation SDK](http://www.microsoft.com/downloads/details.aspx?FamilyID=c148b2df-c7af-46bb-9162-2c9422208504) 的
...
使用共用存取簽章。若要從您自己的應用程式存取儲存服務帳戶，請在設定和管理儲存體服務帳戶時使用可透過 Azure 入口網站取得的共用雜湊。想要提供他人對儲存服務帳戶中 Blob 和容器的存取權時，請使用共用存取簽章 URL。
URL。
...
SQL Database 僅支援 SQL Server 驗證。Windows 驗證 (整合式安全性)。使用者必須在每次連線到 SQL Database 時提供認證 (登入和密碼)。管理使用者名稱和密碼時請特別小心，以避免資訊外洩。
...
-   [做法：使用 sqlcmd 連線到 SQL Database](http://msdn.microsoft.com/library/windowsazure/ee336280.aspx)
-   [做法：使用 ADO.NET 連線到 SQL Database](http://msdn.microsoft.com/library/windowsazure/ee336243.aspx)
-   [做法：透過 ASP.NET 連線到 SQL Database](http://msdn.microsoft.com/library/windowsazure/ee621781.aspx)
-   [做法：透過 WCF 資料服務連線到 SQL Database](http://msdn.microsoft.com/library/windowsazure/ee621789.aspx)
-  [做法：使用 PHP 連線到 SQL Database](http://msdn.microsoft.com/library/windowsazure/ff394110.aspx)
-   [做法：使用 JDBC 連線到 SQL Database](http://msdn.microsoft.com/library/windowsazure/gg715284.aspx)
-   [做法：使用 ADO.NET Entity Framework 連線到 SQL Database](http://msdn.microsoft.com/library/windowsazure/ff951633.aspx)
...
-   [使用 ACS 保護服務匯流排的安全](http://channel9.msdn.com/posts/Securing-Service-Bus-with-ACS) (影片)
...
-   [做法：透過程式設計方式設定 Azure 快取的快取用戶端](http://msdn.microsoft.com/library/windowsazure/gg618003.aspx)
-   [做法：使用應用程式組態檔設定 Azure 快取的快取用戶端](http://msdn.microsoft.com/library/windowsazure/gg278346.aspx)
...
在此案例中，您打算建立將使用 Marketplace 訂閱中之資料集的應用程式。您是應用程式的使用者。
...
請細想在 Azure AD 存取控制部署中的下列安全性旋鈕。以下資訊是摘錄自 [ACS 安全性指導方針](http://msdn.microsoft.com/library/gg185962.aspx)和[憑證和金鑰管理指導方針](http://msdn.microsoft.com/library/hh204521.aspx).
...
[網頁 SSO 設計]: http://technet.microsoft.com/library/dd807033(WS.10).aspx
[同盟網頁 SSO 設計]: http://technet.microsoft.com/library/dd807050(WS.10).aspx
