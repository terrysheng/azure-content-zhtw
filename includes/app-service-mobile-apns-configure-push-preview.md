...
* 在 Azure 預覽入口網站中，按一下 [**瀏覽**] > [**行動應用程式**] > 您的應用程式 > [**推播通知服務**] > [**Apple Push Notification Services**] > [**上傳憑證**]。上傳 .p12 檔案，並確定已選取正確的 [**模式**] (不是 [沙箱] 就是 [實際執行]，這對應於您產生的用戶端 SSL 憑證為 [開發] 或 [配送])。 您的服務現在已設定成在 iOS 上使用推播通知！
