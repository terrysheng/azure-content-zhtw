...
若要為自訂網域 (如 **contoso.com**) 啟用 HTTPS，您必須先向網域名稱註冊機構註冊自訂網域名稱。如需有關如何設定 Web 應用程式網域名稱的詳細資訊，請參閱[設定 Azure 網站的自訂網域名稱](/zh-tw/develop/net/common-tasks/custom-dns-web-site/). 一旦您註冊自訂網域名稱並設定 Web 應用程式回應自訂名稱之後，您必須要求網域的 SSL 憑證。 

> [AZURE.NOTE] 若要為自訂網域名稱啟用 HTTPS，您必須設定 Web 應用程式為使用 [標準] 模式。如果您目前正使用免費或共用模式，則換成使用標準模式可能會帶來額外成本。如需有關共用與**標準**定價的詳細資訊，請參閱[定價詳細資料][pricing]。 
...
搭配 Web 應用程式使用的 SSL 憑證必須由[憑證授權單位](http://en.wikipedia.org/wiki/Certificate_authority) (CA) 簽署。如果您還沒有此類憑證，則必須向發行 SSL 憑證的公司取得。如需憑證授權單位的清單，請參閱 Microsoft TechNet Wiki 上的 [Windows 與 Windows Phone 8 SSL 根憑證計劃 (成員 CA)][cas]。
...
如果 CA 使用**[中繼憑證](http://en.wikipedia.org/wiki/Intermediate_certificate_authorities)** (也稱為鏈結憑證)，則您還需要取得這些中繼憑證。使用中繼憑證比使用 'unchained certificates'安全，因此常會見到 CA 使用中繼憑證。中繼憑證通常是從 CA 網站個別下載而得。本文中的步驟提供關於如何確保任何中繼憑證已與上傳至您 Web 應用程式的憑證相合併的步驟。 
...
1. 開啟 [記事本]，然後建立含有下列內容的新文件。將 Subject 一行中的 **mysite.com** 取代為您 Web 應用程式的自訂網域名稱。例如，Subject = "CN=www.contoso.com"。
	...
	如需上面所指定之選項以及其他可用選項的詳細資訊，請參閱 [Certreq 參考文件](http://technet.microsoft.com/library/cc725793.aspx)。
...
4. 在命令提示字元中，使用下列命令來建立憑證要求檔案：
	...
	指定步驟 1 中所建立之 **myrequest.txt** 檔案的路徑，以及在建立 **myrequest.csr** 檔案時所要使用的路徑。
...
6. 等到憑證授權單位提供憑證檔 (.CER) 給您後，將該檔案儲存至用來產生要求的電腦，然後使用下列命令來接受要求並完成憑證產生程序。
	...
	在此情況下，從憑證授權單位收到的 **mycert.cer** 憑證將會用來完成憑證簽章。沒有檔案會建立；憑證會儲存至 Windows 憑證存放區中。

6. 如果 CA 使用中繼憑證，您必須先安裝這些憑證，再於接下來的步驟匯出憑證。這些憑證通常是從 CA 個別下載而得，並且有分數種格式來用於不同的網頁伺服器類型。請選取針對 Microsoft IIS 所提供的版本。

	下載憑證之後，請在檔案總管中於該憑證上按一下滑鼠右鍵，然後選取 [安裝憑證]。使用 [憑證匯入精靈] 中的預設值，然後繼續選取 [下一步]，直到匯入完成為止。

7. 若要從憑證存放區中匯出憑證，請從 [開始畫面] 或 [開始功能表] 執行 **certmgr.msc**。出現 [憑證管理員] 時，展開 [個人] 資料夾，然後選取 [憑證]。在 [核發給] 欄位中，找出有哪個項目具有您為了它而要求憑證的自訂網域名稱。在 [核發者] 欄位中，應該會列出您用於此憑證的憑證授權單位。
...
12. 提供將包含所匯出憑證的路徑和檔名。檔案名稱的副檔名應該為 **.pfx**。按 [下一步]，完成此程序。
...
2. 當系統提示時，輸入適當資訊。例如：
		...
		請輸入以下要與您的憑證要求一起傳送的  'extra' 屬性

       	挑戰密碼 []： 

	一旦此程序完成之後，您應該有兩個檔案；**myserver.key** 和 **server.csr**。**server.csr** 包含憑證簽署要求。
...
5. 從命令列、Bash 或終端機工作階段中，使用下列命令，將 **myserver.key** 和 **myserver.crt** 轉換為 **myserver.pfx** (Azure App Service 所需的格式)：
	...
	> 
	> 下列命令示範如何建立包含中繼憑證 (其包含在 **intermediate-cets.pem** 檔案中) 的 .pfx 檔案：  
	>
	>
	`````
...
1. 使用 IIS 管理員產生 CSR，以傳送至憑證授權單位。如需有關產生 CSR 的詳細資訊，請參閱[要求網際網路伺服器憑證 (IIS 7)][iiscsr]。
...
4. 從 IIS 管理員匯出憑證。如需有關匯出憑證的詳細資訊，請參閱[匯出伺服器憑證 (IIS 7)][exportcertiis]。後面的步驟中會將所匯出的檔案上傳至 Azure，已搭配您的 Web 應用程式使用。

	> [AZURE.NOTE] 在匯出程序期間，請務必選取 [是，匯出私密金鑰]<strong></strong> 選項。這樣會在匯出的憑證中納入私密金鑰。
...
4. 從 Windows PowerShell 提示字元中，使用下列命令匯出先前建立的憑證：
	...
	這會將指定的密碼以安全字串形式儲存在 $mypwd 中，並使用 **dnsname** 參數所指定的 DNS 名稱尋找憑證，然後匯出至 **filepath** 參數所指定的檔案。而含有密碼的安全字串會用來保護匯出的檔案。

<a name="bkmk_ssopenssl"></a>
####使用 OpenSSL 產生自我簽署憑證 ####

1. 建立名稱為 **serverauth.cnf** 的新文件，並使用下列程式碼做為此檔案的內容：
...
> [AZURE.NOTE] 在將 Web 應用程式從 [免費] 模式切換至 [標準] 模式之前，您應該先移除 Web 應用程式訂閱的花費上限，以免帳單期間還未結束，網站就因為已達花費上限而變得無法使用。如需有關共用與 [標準] 模式定價的詳細資訊，請參閱[定價詳細資料][pricing]。
...
6.	按一下 [調整]
	![[調整] 索引標籤][scale]
7.	在 [調整] 區段中，按一下 [選取] 以設定 App Service 計劃模式。
	![定價層級][sslreserved]
...
6.	按一下 [自訂網域和 SSL]。
	![[設定] 索引標籤][sslconfig]
7.	在 [憑證] 區段中，按一下 [上傳]
8.	使用 [上傳憑證] 對話方塊，選取之前使用 IIS 管理員或 OpenSSL 建立的 .pfx 憑證檔案。指定當初用來保護 .pfx 檔案的密碼 (如果有的話)。最後，按一下 [檢查] 以上傳憑證。
	![SSL 上傳][ssluploadcert]
9. 在 [SSL 設定] 索引標籤的 [SSL 繫結] 區段中，使用下拉式清單選取要以 SSL 保護的網域名稱，以及要使用的憑證。您也可以選擇要使用[伺服器名稱指示][sni] (SNI) 還是 IP SSL。
	...
	* SNI SSL 是 SSL 和[傳輸層安全性][tls] (TLS) 的延伸，可讓多個網域共用相同的 IP 位址，而每個網域都有個別的安全性憑證。現今大部分的瀏覽器 (包括 Internet Explorer、Chrome、Firefox 和 Opera) 都支援 SNI，不過，較舊的瀏覽器可能不支援 SNI。如需有關 SNI 的詳細資訊，請參閱 Wikipedia 上的[伺服器名稱指示][sni]一文。

10. 按一下 [儲存]，儲存變更並啟用 SSL。

> [AZURE.NOTE] 如果您已選取 [IP SSL]，而且您的自訂網域是以 A 記錄設定，則必須執行下列額外步驟：
>
> 1. 設定 IP SSL 繫結之後，您的 Web 應用程式會獲指派專用的 IP 位址。您可以在 Web 應用程式的 [儀表板] 頁面上 (在 [快速概覽] 區段中) 找到這個 IP 位址。它將會列出為 [虛擬 IP 位址]：
>    
...
> [AZURE.NOTE] 如果您的應用程式是以 **Node.js**、**PHP**、**Python Django** 或 **Java** 撰寫而成，則它可能不包含 web.config 檔案。不過，在 Azure App Service 上託管時，**Node.js**、**Python Django** 和 **Java** 都會確實用到 web.config - Azure 會在部署期間自動建立此檔案，因此您永遠看不到此檔案。如果您在應用程式中加入一個 web.config 檔案，則它將覆寫 Azure 自動產生的同名檔案。

###.NET

對於 .NET 應用程式，修改您應用程式的 web.config 檔案，並將[範例]中的 **&lt;rewrite>** 區段(#example) 加入至 **&lt;system.WebServer>** 區段。

如果您的 web.config 檔案已包含 **&lt;rewrite>** 區段，則加入[範例]中的 **&lt;rule>**，(#example) 做為 **&lt;rules>** 區段中的第一個項目。

###PHP

對於 PHP 應用程式，只要將[範例](#example) 當做 web.config 檔案，儲存在應用程式的根目錄，然後將應用程式重新部署至您的 Web 應用程式即可。
...
1. 使用 FTP 下載檔案 (請參閱[透過 FTP 上傳/下載檔案及收集診斷記錄](http://blogs.msdn.com/b/avkashchauhan/archive/2012/06/19/windows-azure-website-uploading-downloading-files-over-ftp-and-collecting-diagnostics-logs.aspx))。
...
如需有關 IIS URL Rewrite 模組的詳細資訊，請參閱 [URL Rewrite](http://www.iis.net/downloads/microsoft/url-rewrite) 文件。 
