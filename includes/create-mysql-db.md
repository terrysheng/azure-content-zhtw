...
本指南將示範如何從 [Azure 市集][]使用 [ClearDB][] 建立 MySQL 資料庫，以及如何在建立 [Azure 網站][]時建立 MySQL 資料庫作為連結的資源。[ClearDB][] 是一項可容錯的「資料庫即服務」提供者，可讓您在 Azure 資料中心內執行和管理 MySQL 資料庫，以及從任何應用程式連線至這些資料庫。

## 目錄

-   [作法：從 Azure 市集建立 MySQL 資料庫][]
-   [作法：為 Azure 網站建立 MySQL 資料庫作為連結的資源][]
...
若要在建立 [Azure 網站][]時建立 MySQL 資料庫作為連結的資源，請執行下列動作：
