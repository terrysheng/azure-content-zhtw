...
本指南將示範如何從 [Azure 市集][Azure 市集]使用 [ClearDB][ClearDB] 建立 MySQL 資料庫，以及如何在建立 [Azure 網站][Azure 網站]時建立 MySQL 資料庫作為連結的資源。[ClearDB][ClearDB] 是一項可容錯的「資料庫即服務」提供者，可讓您在 Azure 資料中心內執行和管理 MySQL 資料庫，以及從任何應用程式連線至這些資料庫。

## 目錄

-   [作法：從 Azure 市集建立 MySQL 資料庫][作法：從 Azure 市集建立 MySQL 資料庫]
-   [作法：為 Azure 網站建立 MySQL 資料庫作為連結的資源][作法：為 Azure 網站建立 MySQL 資料庫作為連結的資源]
...
## <span id="CreateFromStore"></span></a>作法：從 Azure 市集建立 MySQL 資料庫

若要從 [Azure 市集][Azure 市集]建立 MySQL 資料庫，請執行下列動作：

1.  登入 [Azure 管理入口網站][Azure 管理入口網站]。
2.  按一下頁面底部的 [+新增]，然後選取 [市集]。

    ![從市集中選取附加元件][從市集中選取附加元件]

3.  選取 [ClearDB MySQL 資料庫]，然後按一下框架底部的箭頭。

    ![選取 ClearDB MySQL 資料庫][選取 ClearDB MySQL 資料庫]

4.  選取方案、輸入資料庫名稱、選取區域，然後按一下框架底部的箭頭。

    ![從市集購買 MySQL 資料庫][從市集購買 MySQL 資料庫]
...
6.  建立資料庫之後，您可以從管理入口網站的 [附加元件] 索引標籤加以管理。
    ...
    ![在 Azure 入口網站中管理 MySQL 資料庫][在 Azure 入口網站中管理 MySQL 資料庫]

7.  按一下頁面底部的 [連接資訊] (如下所示)，即可取得資料庫連接資訊。

    ![MySql 連接資訊][MySql 連接資訊]

## <span id="CreateForWebSite"></span></a>作法：為 Azure 網站建立 MySQL 資料庫作為連結的資源

若要在建立 [Azure 網站][Azure 網站]時建立 MySQL 資料庫作為連結的資源，請執行下列動作：

1.  登入 [Azure 管理入口網站][Azure 管理入口網站]。
2.  按一下頁面底部的 [+新增]，然後依序選取 [運算]、[網站] 和 [和資料庫一起建立]。

    ![和資料庫一起建立網站][和資料庫一起建立網站]
...
> [WACOM.NOTE] 您的網站應用程式可以根據連接字串名稱來存取連接字串。在 .NET 應用程式中，連接字串位於 **connectionStrings** 物件中。在其他程式設計語言中，連接字串則可以環境變數的形式受到存取。如需詳細資訊，請參閱[如何設定網站][如何設定網站]。
