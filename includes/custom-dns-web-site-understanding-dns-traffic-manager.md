...
> [AZURE.NOTE] 您只能搭配使用 CNAME 記錄與流量管理員

另有多種記錄類型，每種記錄都有其本身的功能與限制，但對於設定為流量管理員端點的網站而言，我們只需要關注一種記錄，即  *CNAME*。

###CNAME 或別名記錄

CNAME 記錄會將 *specific* DNS 名稱 (例如 **mail.contoso.com** 或 **www.contoso.com**) 對應至其他 (正式) 網域名稱。在使用流量管理員的 Azure 網站案例中，正式網域名稱為您的流量管理員設定檔的 **&lt;myapp>.trafficmanager.net** 網域名稱。建立之後，CNAME 還會建立 **&lt;myapp>.trafficmanager.net** 網域名稱的別名。CNAME 項目會自動解析 **&lt;myapp>.trafficmanager.net** 網域名稱的 IP 位址，就算網站的 IP 位址變更，您也不需要採取任何動作。
