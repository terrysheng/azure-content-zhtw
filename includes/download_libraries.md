...
Azure Libraries for Java 依據 [Apache 授權 2.0 版][Apache 授權 2.0 版]發行。請按一下[這裡][這裡]，以取得含程式庫及所有相依性的 ZIP 檔案。此檔案由 Microsoft Open Technologies, Inc. 提供。關於授權及其他資訊，請參閱 ZIP 內的 license.txt 和 ThirdPartyNotices.txt 檔案。
...
在 `<version>` 元素內，請將 *n.n.n* 換成有效的版本號碼，可從 [Azure Libraries Repository on Maven][Azure Libraries Repository on Maven] 取得。
