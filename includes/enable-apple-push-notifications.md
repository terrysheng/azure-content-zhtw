...
2. 將 [**開發**] 下方的 [**iOS 應用程式開發**] 選為佈建設定檔類型，然後按一下 [**繼續**]
...
7. 在 Xcode 中開啟 [組合管理] 並選取 [裝置] 檢視，在左窗格的 [**程式庫**] 區段中選取 [**佈建設定檔**]，然後按一下中間窗格底部的 [**重新整理**] 按鈕。

8. 或者，在 Xcode 功能表中依序選取 [**喜好設定**] 和 [**帳戶**]。在左邊窗格中選取您的 Apple 開發人員 ID。按一下右邊的 [**檢視詳細資料**] 按鈕。在快顯視窗中，按一下圓形的 [**重新整理**] 按鈕。這會重新整理佈建設定檔的清單。此程序可能需要數分鐘的時間。建議您按一下 [**重新整理**] 2 到 3 次，直到您看到新的佈建設定檔為止。此外，確認此 Xcode 專案的組合套件識別碼與您到目前為止建立之 App ID 和佈建設定檔相關聯的套件組合識別碼相同。

    ![](./media/enable-apple-push-notifications/mobile-services-ios-push-01.png)

9. 在 [**目標**] 下方按一下 [**快速入門**]、展開 [**程式碼簽署身分識別**]，然後在 [**偵錯**] 下方選取新的設定檔。這將確保 Xcode 專案使用新的設定檔進行程式碼簽署。接著，您必須將憑證上傳到 Azure。
