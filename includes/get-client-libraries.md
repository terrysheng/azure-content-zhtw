...
1.  從 [GitHub][3] 下載含有程式庫的 .zip 封存檔。或者，分岔儲存機制並複製到本機電腦(後面這個選項需要有 GitHub 帳戶並在本機安裝 Git)。
    
    <div  class="dev-callout" markdown="1">
 	</div>
**注意**
PHP Client Libraries for Azure 相依於 [HTTP\_Request2][4]、[Mail\_mime][5] 和 [Mail\_mimeDecode][6] PEAR 封裝。建議使用 [PEAR 封裝管理員][7]來安裝這些封裝，以解決這些相依性。
...
如需關於安裝 PHP Client Libraries for Azure 的詳細資訊 (包括關於以 PEAR 封裝來安裝的詳細資訊)，請參閱[下載 Azure SDK for PHP](../php-download-sdk/)(英文)。
