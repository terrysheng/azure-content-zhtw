佈建 MongoLab 資料庫之後，您隨時都可以存取 MongoLab Web UI 來管理該資料庫。我們透過 Azure
管理入口網站，提供方便的單一登入 (SSO) 存取。

MongoLab UI 可讓您執行大部分的資料庫工作，例如插入和查詢文件、取得資料庫統計資料，以及執行實用的資料庫命令。若要深入了解
MongoLab UI，請造訪我們的[支援頁面][1]和[部落格][2]。
