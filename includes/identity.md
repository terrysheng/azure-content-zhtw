...
在 Azure VM 中執行 Windows Server AD，與在內部部署中執行 Windows Server AD 極為類似。[圖 1](#fig1) 顯示其外觀的典型範例。
...
假設有應用程式希望讓使用者使用多個  *identity providers (IdPs)* 所簽發的權杖進行登入。現今有為數眾多的身分識別提供者，包括 Facebook、Google、Microsoft 和其他提供者，而應用程式經常讓使用者使用上述其中一個身分識別進行登入。既然可以依賴已經存在的身分識別，應用程式為何還要麻煩地維護自己的使用者和密碼清單？對於要記住不只一個使用者名稱和密碼的使用者，以及建立應用程式且不必再維護自己的使用者名稱和密碼清單的人而言，接受現有的身分識別比較方便。
