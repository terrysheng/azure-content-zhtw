...
9.  (選擇性) mongod.exe 支援安裝並執行 Windows 服務。要安裝 mongod.exe 做為服務，請在命令提示字元視窗中執行：
    ...
    這樣會建立一個名為 "Mongo DB" 的服務，其說明為 "Mongo DB"。**--logpath** 選項必須用來指定記錄檔，因為執行中的服務沒有命令視窗可以顯示輸出。**--logpath** 選項指出重新啟動服務會導致輸出附加在現有的記錄檔案中。**--dbpath** 選項指出資料目錄的位置。F如需更多服務相關的命令列選項，請參閱[服務相關的命令列選項][服務相關的命令列選項]。
