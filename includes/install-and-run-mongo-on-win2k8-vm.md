...
2. 選取右上方的 [工具] 按鈕。在 [網際網路選項] 中，選取 [安全性] 索引標籤，接著選取 [受信任的網站] 圖示，最後按一下 [網站] 按鈕。將 \__https://*.mongodb.org_ 新增至受信任的網站清單。
...
7. 若要執行資料庫，執行：
	...
	在 mongod.exe 伺服器啟動和預先配置日誌檔案時，所有記錄訊息都會傳送至 *F:\\MongoLogs\\mongolog.log* 檔案。MongoDB 可能需要花費數分鐘來預先配置日誌檔案，並開始接聽連線。當您的 MongoDB 執行個體正在執行，命令提示字元將專注於這項工作。
...
> [AZURE.NOTE] 連接埠 27017 是 MongoDB 使用的預設連接埠。啟動 mongod.exe 伺服器時，您可以用 _--port_ 參數予以變更。請務必為上述指示中的防火牆和 "Mongo" 端點指定相同的連接埠號碼。
