我們的權杖快取應該只在簡單案例中使用，在權杖過期或被撤銷時會有什麼影響？我們必須能夠偵測到過期的權杖。當應用程式停止運作的時候，那麼權杖可能過期了。這表示權杖快取會失效。然而在應用程式直接發出呼叫或行動服務程式庫發出呼叫期間，實際執行應用程式，權杖也有可能過期。這將會導致出現 HTTP 狀態碼 401「未經授權」。因此我們需要找方法偵測這狀況並且更新權杖。若要這麼做，我們可以使用 [Android 用戶端程式庫][Android 用戶端程式庫]的 [ServiceFilter][ServiceFilter]。
