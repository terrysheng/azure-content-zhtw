...
4.  在 **建立行動服務** 頁面中，選取 **Create a free 20 MB SQL database**，然後在
    **URL** 文字方塊中輸入新行動服務的子網域名稱，並等候進行名稱驗證。一旦名稱驗證完成之後，按一下向右鍵按鈕前往下一頁。
...
**注意**
作為本教學課程的一部分，您需要建立新的 SQL Database 執行個體和伺服器。您可以重複使用這個新的資料庫，並像管理其他任何
SQL Database 執行個體一樣管理這個新的資料庫。如果新行動服務的區域已經有資料庫存在，您可以改選
**[使用現有資料庫]**，然後選取該資料庫。不建議您使用位在不同區域的資料庫，因為這會需要額外的頻寬成本和產生更高的延遲。
