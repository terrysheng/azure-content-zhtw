...
2.  將下列新屬性新增至 **TodoItem** 類別：
    ...
    > [WACOM.NOTE] 使用預設資料庫初始設定式時，當 Entity Framework 在 Code First 定義中偵測到資料模型變更時，就會捨棄並重新建立資料庫。若要進行此資料模型變更，並保有資料庫的現有資料，必須使用 Code First Migrations。無法針對 Azure 中的 SQL Database 使用預設的初始設定式。如需詳細資訊，請參閱[如何使用 Code First Migrations 更新資料模型][如何使用 Code First Migrations 更新資料模型] (英文)。
