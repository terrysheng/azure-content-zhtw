...
2.  將下列程式碼新增至新控制器：
    ...
    在上述程式碼中，以資料模型之 DbContext 的名稱取代 `todolistContext`，該名稱必須是附加 `Context` 的行動服務名稱。此程式碼使用 [Database 類別](http://msdn.microsoft.com/zh-tw/library/system.data.entity.database(v=vs.113).aspx) (英文) 直接存取 **TodoItems** 資料表，以在所有項目上設定完成旗標。此方法支援 POST 要求，且系統會以整數值將變更的列數傳回給用戶端。
