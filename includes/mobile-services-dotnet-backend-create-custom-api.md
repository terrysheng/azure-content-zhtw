...
1. 在 Visual Studio [方案總管] 中，於行動服務專案的 Controllers 資料夾上按一下滑鼠右鍵，展開 [**新增**]，然後按一下 [**新增 Scaffolded 項目**]。

	這會顯示 [新增 Scaffold] 對話方塊。

2. 展開 [**Azure 行動服務**] 並按一下 [**Azure 行動服務自訂控制器**]，然後按一下 [**新增**]，提供 `CompleteAllController` 的 [**控制器名稱**]，再按一下 [**新增**]。
...
5. 將下列程式碼新增至新控制器：
	...
	此程式碼使用 [Database 類別](http://msdn.microsoft.com/zh-tw/library/system.data.entity.database.aspx) (英文) 直接存取 **TodoItems** 資料表，以在所有項目上設定完成旗標。此方法支援 POST 要求，且系統會以整數值將變更的列數傳回給用戶端。
