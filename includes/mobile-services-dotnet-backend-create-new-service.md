...
<b>注意</b> 
<p>作為本教學課程的一部分，您需要建立新的 SQL Database 執行個體和伺服器。您可以重新使用這個新資料庫並加以管理，或使用任何其他的 SQL Database 執行個體。如果新行動服務的區域已經有資料庫存在，您可以改選 <strong>[使用現有資料庫]</strong>，然後選取該資料庫。不建議您使用位在不同區域的資料庫，因為這會需要額外的頻寬成本和產生更高的延遲。</p></div>
