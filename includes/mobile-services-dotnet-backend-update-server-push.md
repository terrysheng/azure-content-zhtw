1.  在 Visual Studio 的 [方案總管] 中，展開行動服務專案中的 **Controllers** 資料夾。開啟 TodoItemController.cs，並以下列程式碼更新 `PostTodoItem` 方法定義：
    ...
    > [WACOM.NOTE] 您可以使用範本通知傳送單一推播通知至多個平台的用戶端。如需詳細資訊，請參閱[從單一行動服務支援多個用戶端][從單一行動服務支援多個用戶端] (英文)。

2.  將行動服務專案重新發佈至 Azure。

  [從單一行動服務支援多個用戶端]: /zh-tw/documentation/articles/mobile-services-how-to-use-multiple-clients-single-service/#push
