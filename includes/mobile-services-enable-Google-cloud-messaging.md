...
1. 導覽至 <a href="http://cloud.google.com/console" target="_blank">Google 雲端主控台</a>網站，並使用 Google 帳戶認證登入，然後按一下 [建立專案]。
	...
	>[AZURE.NOTE]如果您目前已有專案，登入後會將您重新導向至 [專案] <strong></strong>頁面。若要從 [儀表板] 建立新的專案，請展開 [API 專案]，並按一下 [其他專案] 下的 [建立...]，然後輸入專案名稱，並按一下 [建立專案]。
...
4. 在左欄中，展開 [API 與驗證]，按一下 [API]，然後向下捲動並按一下切換開關，啟用 [Google Cloud Messaging for Android]。在下一頁中，按一下 [啟用 API] 並接受服務條款。
