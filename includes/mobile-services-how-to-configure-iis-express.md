...
5.  開啟 Windows 防火牆主控台，然後建立新的連接埠規則，以允許對連接埠的連線。如需建立新的 Windows 防火牆連接埠規則的詳細資訊，請參閱[如何新增 Windows 防火牆連接埠規則](http://go.microsoft.com/fwlink/?LinkId=392240)。

    > [WACOM.NOTE] 如果您的測試機器已加入某個網域，則防火牆例外可能會由網域原則所控制。在此情況下，您必須與網域管理員連絡，以了解機器連接埠的例外。

    現在，您應已完成對代管行動服務的 IIS Express 進行測試所需的設定。

    > [WACOM.NOTE] 在完成對服務的本機測試後，您應刪除先前建立的 Windows 防火牆規則。
