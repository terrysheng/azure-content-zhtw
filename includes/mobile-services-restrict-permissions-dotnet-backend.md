...
4. 將下列 AuthorizeLevel 屬性套用至 **TodoItemController** 類別：
	...
	>[AZURE.NOTE]將 AuthorizeLevel 屬性套用至個別方法，以在控制器公開的方法上設定特定的授權層級。

5. 如果您想要在本機偵測驗證錯誤，請展開 App_Start 資料夾，開啟 WebApiConfig.cs 專案檔案，然後將下列程式碼新增至 **Register** 方法：

		config.SetIsHosted(true);
	
	這會指示本機行動服務專案依循在 Azure 中受到代管的型態執行，包括接受 AuthorizeLevel 設定。如果沒有此設定，則無論 AuthorizeLevel 設定為何，對 *localhost* 的所有 HTTP 要求都會在未經驗證的情況下受到允許。  
