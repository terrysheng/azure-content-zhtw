...
1.  (選用) 使用訊息檢查軟體 (例如瀏覽器開發人員工具或 [Fiddler](http://go.microsoft.com/fwlink/?LinkID=262412)) 檢視傳送至行動服務之要求的 URI。

	請注意，`Take(3)` 方法在查詢 URI 中會轉譯成查詢選項 `$top=3`。

2.  再次以下列程式碼更新 **RefreshTodoItems** 方法：
    ...
    > [WACOM.NOTE] 此教學課程使用的是將硬式編碼分頁值傳遞至 **Take** 和 **Skip** 方法的簡化案例。在實際的應用程式中，您可以搭配頁面巡覽區控制項或類似的 UI 來使用類似上述的查詢，讓使用者導覽至上一頁和下一頁。您也可以呼叫 **IncludeTotalCount** 方法，以取得伺服器上可用項目的總計數和分頁資料。

3.  (選用) 檢視傳送至行動服務之要求的 URI。
