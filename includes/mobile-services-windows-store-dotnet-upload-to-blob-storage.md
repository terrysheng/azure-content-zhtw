...
3.  以下列程式碼取代 **DataTemplate** 中的 **StackPanel** 元素：
	...
	這樣可以在 **ItemTemplate** 中新增一個影像，並將其繫結來源設為 Blob 儲存體服務中已上傳影像的 URI。
...
1.  使用下列程式碼來取代現有的 `InsertTodoItem` 方法：
...
3.  新項目以及上傳的影像會顯示在清單檢視中。

    ![](./media/mobile-services-windows-store-dotnet-upload-to-blob-storage/mobile-quickstart-blob-ie.png)

	>[WACOM.NOTE] 將新項目的 <code data-morhtml="true">imageUri</code> 屬性繫結至 <strong data-morhtml="true">Image</strong> 控制項時，會從 Blob 儲存體服務自動下載影像。
