...
如果您使用行動服務，請參閱[開始使用推播](mobile-services-dotnet-backend-windows-store-dotnet-get-started-push.md)教學課程。如果您想要使用 Java 或 PHP，請參閱[如何從 Java/PHP 使用通知中樞](notification-hubs-java-backend-how-to.md)。您可以使用[通知中心 REST 介面]，從任何後端傳送通知。
...
4. 在  `Program` 類別中，新增或取代 (如果方法已存在) 下列方法：
	...
	> [AZURE.NOTE] 此後端程式碼支援 Windows 市集、Windows Phone、iOS 和 Android 用戶端。當尚未針對特定用戶端平台設定通知中心時，傳送方法會傳回錯誤回應。 
