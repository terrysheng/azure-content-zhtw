...
Azure 網站支援原始程式碼控制和儲存機制工具 (如 BitBucket、CodePlex、Dropbox、Git、GitHub、Mercurial 和 TFS) 的持續部署。您可以使用這些工具來維護網站的內容和程式碼，然後快速輕鬆地隨時將變更推送到網站。

在本文中，您將了解如何使用 Git 從本機電腦直接發佈到 Azure 網站 (在 Azure 中，這個發佈方法稱為**本機 Git**)。您還可以了解到如何啟用儲存機制網站 (如 BitBucket、CodePlex、DropBox、GitHub 或 Mercurial) 的持續部署。如需有關使用 TFS 持續部署的詳細資訊，請參閱＜[使用 Visual Studio Online 持續傳遞至 Azure][1]＞(英文)。
...
Azure 網站支援以各種程式設計語言所建立的應用程式。以此範例為例，您將使用靜態 .html 檔案。如需以其他程式設計語言將網站發佈至 Azure 的相關資料，請參閱 [Azure 開發人員中心][3] (英文)。
...
4.  在 **Quick Glance** 區段中，選取 **設定從原始檔控制進行部署**。下列 **SET UP DEPLOYMENT** 對話方塊隨即出現。
...
在此時，入口網站會顯示初始化本機儲存機制和新增檔案的指示。您已在本主題先前的步驟中完成此作業。不過，如果您尚未設定部署認證，則必須遵循指示中的步驟 3。此步驟將引導您前往標示為**重設您的部署認證**的連結。
...
使用本機 Git 將本機檔案推送至 Azure，可讓您將更新從本機專案手動推送至 Azure 網站，但若從 BitBucket、CodePlex、Dropbox、GitHub 或 Mercurial 部署則會轉變成持續部署程序，Azure 將從專案取得最近的更新。

以上兩種方法的結果都會將專案部署到 Azure 網站，當您有多人同時執行專案，並想確保無論是誰做出最近更新都一定會發佈最新版本時，持續部署是非常有用的。如果您使用上述其中一個工具作為應用程式的中央儲存機制，持續部署也很有用。

無論是從 GitHub、CodePlex 或 BitBucket 部署檔案，都會要求您將本機專案發佈到這些服務的其中之一。如需將專案發佈到這些服務的詳細資訊，請參閱[建立儲存機制 (GitHub)][5]、[使用 Git 與 CodePlex][6]、[建立儲存機制 (BitBucket)][7]、[使用 Dropbox
...
8.  此時，您已將專案從您選擇的儲存機制部署到 Azure 網站。若要驗證此網站是否正在使用中，請按一下入口網站底部的 **瀏覽** 連結。瀏覽器應會瀏覽進入網站。

9.  若要驗證是否發生持續部署，請對您的專案進行變更，然後將此變更推送至與此網站相關聯的儲存機制。在推送至儲存機制完成後不久，您的網站應會進行更新以反映變更。您可以在網站的 **部署** 頁面上確認它是否已取得更新。
...
當對您的儲存機制進行更新時，POST 要求會被傳送到此 URL，於是您的 Azure 網站會收到該儲存機制已更新的通知。此時，它會擷取更新並在您的網站上部署更新。
