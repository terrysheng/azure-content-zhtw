...
4.  在 [URL] 中，為儲存體帳戶輸入要在 URI 中使用的
    ...
    與數字。此值會變成 URI 內，用來
    定址訂閱的 Blob、佇列或對應表資源的主機名稱
    。

5.  選擇要放置儲存體所在的 [區域/同質群組]
    。如果您會從您的 Azure 應用程式使用儲存體，
    選取將部署您應用程式所在的相同區域
    。

6. 或者，您可以選取帳戶所需的複寫類型。地理區域備援複寫是預設值，而且提供最高的持久性。如需有關複寫選項的詳細資訊，請參閱 [Azure 儲存體備援選項](http://msdn.microsoft.com/library/azure/dn727290.aspx)和 [Azure 儲存體小組部落格](http://blogs.msdn.com/b/windowsazurestorage/)。
