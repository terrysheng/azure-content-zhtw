...
1. 從管理入口網站尋找 [**主機名稱**] 和 [**連接埠資訊**]。您可以從虛擬機器的儀表板中找到您需要的資訊。按一下虛擬機器名稱，然後在儀表板 的 [**快速概覽**] 區段中尋找 [**SSH 詳細資料**]。
...
在 Linux 上，資源磁碟通常由 Azure Linux 代理程式管理，並自動掛接到 **/mnt/resource** (或 Ubuntu 映像中的 **/mnt**)。換句話說，在 Linux 上，核心有可能將資料磁碟命名為 `/dev/sdc`，且使用者必須分割、格式化及掛接該資源。如需詳細資訊，請參閱 [Azure Linux 代理程式使用者指南](http://azure.microsoft.com/manage/linux/how-to-guides/linux-agent-guide/) (英文)。
...
4. 已為您定義 [**虛擬機器名稱**]、[**儲存體位置**] 和 [**檔案名稱**]。您只需要輸入想要的磁碟大小。在 [**大小**] 欄位中輸入 **5**。
	...
	**注意：**所有磁碟都是從 Azure 儲存體的 VHD 檔案中建立。您可以為加入儲存體的 VHD 檔案命名，但磁碟的名稱是自動產生的。
...
11. 將新的磁碟機新增至 /etc/fstab：
	...
	**注意：**不當編輯 / /etc/fstab 檔案可能會導致系統無法開機。如果不確定，請參閱散發套件的文件，以取得如何適當編輯此檔案的詳細資訊。另外建議您在編輯之前先建立 /etc/fstab 檔案的備份。
