...
本白皮書著重於安全性挑戰和建議方法，為 Microsoft 的 Azure 平台設計和開發更安全的應用程式。
...
<td style="width: 400px;">
本白皮書將說明在 Azure 內實作的控制項陣列，客戶可藉此判斷這些功能和控制項是否適合他們的獨特需求。
...
<td style="width: 400px;">
本白皮書包含使用 Windows Identity Foundation、Azure 和 Active Directory Federation Services (AD FS) 2.0，在所有 Web 應用程式 (同時部署在內部部署和雲端) 中達成 SSO 的逐步指示。
...
<td style="width: 400px;">
本白皮書將說明 Azure 資料表，是指由 Azure 平台所提供的結構化儲存體。它支援雲端中可大幅擴充的資料表，可包含數十億個實體以及數 TB 的資料。
...
<td style="width: 400px;">
本白皮書著重於如何使用服務匯流排，來克服現今世界固有的一些常見網際網路連線挑戰。
...
<td style="width: 400px;">
本白皮書將說明 Azure 佇列，以及如何使用它來提供可靠的訊息傳遞機制。
...
<td style="width: 400px;">
本白皮書將說明 Azure 磁碟機，以及如何使用它來為 Azure 應用程式提供持久 NTFS 磁碟區。
...
<td style="width: 400px;">
本白皮書將介紹 Microsoft 的雲端平台 Azure，Windows Azure 可供開發人員在網際網路可存取的資料中心虛擬機器上執行應用程式並儲存資料。
...
<td style="width: 400px;">
本白皮書將說明 Azure 二進位大型物件，以及如何使用它來讓應用程式能夠在雲端中儲存大型物件 (每個物件最多 50GB)。
...
<td style="width: 400px;">
本白皮書將討論*平台即服務*雲端提供項目的好處，以及如何在 Azure 內實作此服務。
...
<td style="width: 400px;">
本白皮書將說明 Windows HPC Server R2 和 Azure 可以如何搭配使用。
...
<td style="width: 400px;">
本白皮書將強調 SQL Azure 和 SQL Server 之間的相似性與差異性。它還包括在雲端中使用 SQL Azure 作為資料庫服務的好處。
...
<td style="width: 400px;">
本白皮書將討論 Azure 如何輔助 Windows Server，以及如何相互結合可讓企業、區域服務提供者和 Microsoft 本身，透過一致的身分識別、管理和應用程式架構，跨私人和公用雲端傳遞應用程式。
...
<td style="width: 400px;">
本白皮書將提供安全性方針概觀，對象是即將連線到 SQL Azure Database，並在 SQL Azure 上建立安全應用程式的客戶。
...
<td style="width: 400px;">
本白皮書將提供 Azure 平台 AppFabric 的入門簡介 (其中包括服務匯流排和存取控制)，並在將功能延伸到雲端 (尤其是 Azure 平台) 時，提供 .NET 應用程式所需的常見建置組塊。
...
<td style="width: 400px;">
本白皮書將討論為何使用雲端有可能會增加 ISV 的收入並減少它的成本。
...
<td style="width: 400px;">
本白皮書將討論雲端運算如何提供組織全新的部署和維護企業應用程式的方式 - 允許更大的彈性和降低複雜度。
...
<td style="width: 400px;">
本白皮書將介紹一個全方位的雲端中介軟體平台 Azure AppFabric，可用於開發、部署及管理 Azure 平台上的應用程式。
...
<td style="width: 400px;">
本白皮書將評估雲端的經濟體系，方法是使用詳細模型功能，然後使用此架構來深入了解長期 IT 版圖。
...
<td style="width: 400px;">
本白皮書將說明 Microsoft 系統整合器合作夥伴如何使用 Azure，來開發可快速部署、容易管理、可立即擴充及價格具有競爭力的應用程式和服務。
...
<td style="width: 400px;">
本白皮書將從業務角度來探討雲端平台的優勢與風險。若要做出好的決策，您必須對這兩者有所了解。
...
<td style="width: 400px;">
本白皮書將探討 Azure 快取提供哪些快取服務可支援的不同類型快取模式。它還包括在架構充分利用快取功能的應用程式時，您必須考量因快取而產生的關鍵問題。
...
<td style="width: 400px;">
本白皮書將說明 SQL Azure 如何使用低成本、快速佈建的雲端型儲存體，來提供強固且具成本效益的解決方案，以便在大型或共用資料集上建立應用程式、提供可依需要擴充的資料儲存體，並強化內部部署資料基礎結構。
...
<td style="width: 400px;">
本白皮書將說明 IIS/SQL Server 應用程式架構模式，並說明使用這個模式或類似模式來移轉應用程式的特定考量和建議方法。
...
<td style="width: 400px;">
本白皮書將說明自訂 Web 應用程式 (快速擴充) 架構模式，並說明使用這個模式或類似模式來移轉應用程式的特定考量和建議方法。
...
<td style="width: 400px;">
本白皮書將說明自訂電子商務應用程式 (彈性焦點) 架構模式，並說明使用這個模式或類似模式來移轉應用程式的特定考量和建議方法。
...
<td style="width: 400px;">
本白皮書將提供如何將現有的內部部署 SQL Server 資料庫部署到 SQL Azure 的方針。還會討論與資料移轉相關的最佳作法。
